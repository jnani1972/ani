...
ani interacts deeply with your filesystem. It reads source files across your entire codebase, writes to agent configuration files, and spawns background processes. This is inherent to what it does — not a bug.
...
metadata from
`https://api.github.com/repos/jnani1972/ani/releases/latest`.
...
Explicit install, package-manager, and `ani update` flows are
...
1. **Do NOT open a public issue, PR, or social-media post** for security
   vulnerabilities.
2. **Preferred:** use GitHub's [private vulnerability reporting](https://github.com/jnani1972/ani/security/advisories/new)
...
[GitHub Security Advisory](https://github.com/jnani1972/ani/security/advisories)
...
- **Shell injection prevention** — `ani_validate_shell_arg()` rejects metacharacters before all `popen()`/`system()` calls
...
gh attestation verify <downloaded-file> \
  --repo jnani1972/ani \
  --signer-workflow jnani1972/ani/.github/workflows/_build.yml
...
Policy identifier: `ani-vt-candidate-selection-v1`. It is reviewed at least
