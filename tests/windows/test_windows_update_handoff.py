...
the process into install.ps1, which runs while ANI is NOT running.
...
* ``update`` refuses to reach the network first: it hands off before it
  consults ANI_DOWNLOAD_URL, so it stays fast even when that URL is a black
  hole.
...
Usage:
    python test_windows_update_handoff.py <ani.exe>
"""
...
def assert_update_hands_off_to_install_script(source, env, work):
    ...
    command_env["ANI_DOWNLOAD_URL"] = "https://127.0.0.1:1"
    ...
    require(
        not (binary.parent / "ani.payload.exe").exists(),
...
def main():
    if os.name != "nt":
        ...
        return 2
    if len(sys.argv) != 2:
        print("usage: python test_windows_update_handoff.py <ani.exe>")
        return 2
    ...
    work = pathlib.Path(tempfile.mkdtemp(prefix="ani_win_update_"))
    try:
