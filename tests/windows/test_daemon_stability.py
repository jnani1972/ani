...
each other and from any interactive ANI use on the host.

Exit code: 0 == all sections green, 1 == regression, 2 == setup error.

Usage:
    python test_daemon_stability.py <path-to-ani[.exe]>
"""
import json
import os
import re
...
def section_hook_fail_open(binary, work):
    ...
    if first.returncode != 0:
        ...
        return False
    if "systemMessage" not in first_out or "no ANI daemon" not in first_err:
...
def main():
    ...
    work = tempfile.mkdtemp(prefix="ani_daemon_stab_")
    sections = [
        section_params,
