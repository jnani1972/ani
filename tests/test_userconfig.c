 ...
 *
 * Tests ani_userconfig_load(), ani_userconfig_lookup(), and the
 * ani_set_user_lang_config() / ani_language_for_extension() integration.
 */
...
TEST(userconfig_project_basic) {
    /* Write a .ani.json in a temp dir */
    char dir[256];
    snprintf(dir, sizeof(dir), "%s/uctest_proj_basic", ani_tmpdir());
    ani_mkdir_p(dir, 0755); /* from compat_fs.h via compat.h */

    char proj[512];
    snprintf(proj, sizeof(proj), "%s/.ani.json", dir);
    ASSERT_EQ(
        write_json(proj, "{\"extra_extensions\":{\".blade.php\":\"php\",\".mjs\":\"javascript\"}}"),
        0);

    ani_userconfig_t *cfg = ani_userconfig_load(dir);
    ASSERT_NOT_NULL(cfg);

    ASSERT_EQ(ani_userconfig_lookup(cfg, ".blade.php"), ANI_LANG_PHP);
    ASSERT_EQ(ani_userconfig_lookup(cfg, ".mjs"), ANI_LANG_JAVASCRIPT);
    ASSERT_EQ(ani_userconfig_lookup(cfg, ".go"), ANI_LANG_COUNT); /* not in user config */

    ani_userconfig_free(cfg);
    remove(proj);
    PASS();
}
...
TEST(userconfig_global_via_env) {
    ...
    char cfg_dir[256];
    snprintf(cfg_dir, sizeof(cfg_dir), "%s/uctest_global_xdg", ani_tmpdir());

    char app_dir[512];
    snprintf(app_dir, sizeof(app_dir), "%s/ani", cfg_dir);
    ani_mkdir_p(app_dir, 0755);
    ...
    ani_safe_getenv("APPDATA", old_appdata, sizeof(old_appdata), NULL);
    ani_setenv("APPDATA", cfg_dir, 1);
#else
    ani_setenv("XDG_CONFIG_HOME", cfg_dir, 1);
#endif
    ani_userconfig_t *cfg = ani_userconfig_load(NULL); /* no project dir */
#ifdef _WIN32
    if (old_appdata[0]) {
        ani_setenv("APPDATA", old_appdata, 1);
    } else {
        ani_unsetenv("APPDATA");
    }
#else
    ...
    ASSERT_NOT_NULL(cfg);
    ASSERT_EQ(ani_userconfig_lookup(cfg, ".twig"), ANI_LANG_HTML);
...
TEST(userconfig_project_wins_over_global) {
    ...
    char xdg_dir[256];
    snprintf(xdg_dir, sizeof(xdg_dir), "%s/uctest_priority_xdg", ani_tmpdir());

    char app_dir[512];
    snprintf(app_dir, sizeof(app_dir), "%s/ani", xdg_dir);
    ani_mkdir_p(app_dir, 0755);
    ...
    char proj_dir[256];
    snprintf(proj_dir, sizeof(proj_dir), "%s/uctest_priority_proj", ani_tmpdir());
    ani_mkdir_p(proj_dir, 0755);

    char proj_path[512];
    snprintf(proj_path, sizeof(proj_path), "%s/.ani.json", proj_dir);
    ASSERT_EQ(
        write_json(proj_path, "{\"extra_extensions\":{\".xyz\":\"rust\"}}"),
        0);

    ani_setenv("XDG_CONFIG_HOME", xdg_dir, 1);
    ani_userconfig_t *cfg = ani_userconfig_load(proj_dir);
    ani_unsetenv("XDG_CONFIG_HOME");

    ASSERT_NOT_NULL(cfg);
    /* Project definition (rust) must win */
    ASSERT_EQ(ani_userconfig_lookup(cfg, ".xyz"), ANI_LANG_RUST);
...
TEST(userconfig_unknown_lang_skipped) {
    char dir[256];
    snprintf(dir, sizeof(dir), "%s/uctest_unknown_lang", ani_tmpdir());
    ...
    ASSERT_EQ(ani_userconfig_lookup(cfg, ".wasm"), ANI_LANG_COUNT);
...
TEST(userconfig_missing_files_ok) {
    ...
    ani_userconfig_t *cfg = ani_userconfig_load("/tmp/__nonexistent_repo_12345__");
...
/* ── Tests: integration with ani_language_for_extension ─────────── */

TEST(userconfig_integration_override) {
    /* Verify that setting the global config makes ani_language_for_extension
     * respect the override. We map ".blade.php" → PHP, which is not in the
     * built-in table. */
    char dir[256];
    snprintf(dir, sizeof(dir), "%s/uctest_integ", ani_tmpdir());
    ani_mkdir_p(dir, 0755);
    ...
    ASSERT_EQ(ani_language_for_extension(".blade.php"), ANI_LANG_COUNT);

    ani_set_user_lang_config(cfg);
    /* After setting, .blade.php → PHP */
    ASSERT_EQ(ani_language_for_extension(".blade.php"), ANI_LANG_PHP);
...
TEST(userconfig_free_null) {
    ani_userconfig_free(NULL); /* must not crash */
    PASS();
}
