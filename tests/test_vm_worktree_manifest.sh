...
FIXTURE="$(mktemp -d "${TMPDIR:-/tmp}/ani-vm-manifest-test.XXXXXX")"
manifest="$(mktemp "${TMPDIR:-/tmp}/ani-vm-manifest-output.XXXXXX")"
...
ani_vm_write_untracked_manifest "$FIXTURE" "$manifest"
...
if grep -Fq 'cd $VM_REPO && git apply' <<<"$sync_block" ||
   grep -Fq 'cd $VM_REPO && tar -xf' <<<"$sync_block"; then
    echo "FAIL: Windows VM sync must expand the selected checkout locally before transport" >&2
    exit 1
fi
if ! grep -Fq "cd '\$VM_REPO' && git apply" <<<"$sync_block" ||
   ! grep -Fq "cd '\$VM_REPO' && tar -xf" <<<"$sync_block"; then
...
if ! grep -Fq 'ANI_CI_RUN_ID" =~ ^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$' "$driver"; then
...
if grep -Fq 'C:\\ani\\scripts\\ci' <<<"$preflight_block" ||
...
for entrypoint in "$driver" "$provisioner"; do
    ...
    if ! grep -Fq 'ani_vm_require_safe_branch "$BRANCH"' "$entrypoint"; then
...
if ! grep -Fq 'ani_vm_sync_windows_clock()' \
...
clock_helper=$(sed -n '/^ani_vm_sync_windows_clock() {$/,/^}$/p' \
...
for entrypoint in "$driver" "$provisioner"; do
    if ! grep -Fq 'ani_vm_sync_windows_clock "${SSH[@]}"' "$entrypoint"; then
        echo "FAIL: Windows VM entry point must repair suspended-guest clock drift: $entrypoint" >&2
        exit 1
    fi
done
if ! grep -Fq 'ani_vm_run_powershell()' \
    "$ROOT/test-infrastructure/vm/ssh-common.sh" ||
...
# The guards leg builds the CI-shaped UI product and its external pack into an
...
if ! grep -Fq 'scripts/build.sh --with-ui CC=clang CXX=clang++ SANITIZE= BUILD_DIR=build/guards' \
    ...
    <<<"$guards_block" ||
    ! grep -Fq -- '-GuardsOnly -Binary build\\guards\\ani.exe' <<<"$guards_block"; then
...
if ! grep -Fq 'cd /d ${VM_REPO_WIN}' <<<"$guards_block" ||
   grep -Fq 'cd /d C:\\ani' <<<"$guards_block"; then
...
if ! grep -Fq 'ANI_VM_BRANCH' "$ROOT/test-infrastructure/vm/README.md"; then
