...
FIX="$(mktemp -d "${TMPDIR:-/tmp}/ani-mcpb-contract.XXXXXX")"
trap 'rm -rf "$FIX"' EXIT
...
cat >"$FIX/stub.c" <<'EOF'
#include <stdio.h>
static const char keep[] = "ani OMIT_LOAD_EXTENSION";
int main(void) { puts(keep); return 0; }
EOF
SELECTED="$BUILD_DIR/ani"
case "$(uname -s)" in
...
check_bundle(fix / "darwin" / "ani-darwin-arm64.mcpb",
             binary="server/ani", platform="darwin",
             version="0.0.0-contract")
check_bundle(fix / "windows" / "ani-windows-amd64.mcpb",
             binary="server/ani.exe", platform="win32",
             version="0.0.0-contract")
check_bundle(fix / "portable" / "ani-linux-amd64-portable.mcpb",
             binary="server/ani", platform="linux",
             version="0.0.0-contract")

# Without VERSION the manifest must say so loudly, not invent a release.
check_bundle(fix / "unversioned" / "ani-darwin-arm64.mcpb",
             binary="server/ani", platform="darwin",
             version="0.0.0-dev")
...
if not (plain / "ani-linux-amd64.tar.gz").is_file():
...
check_archive(fix / "darwin" / "ani-darwin-arm64.tar.gz",
              "ani")
check_archive(fix / "windows" / "ani-windows-amd64.zip",
              "ani.exe")
check_archive(fix / "portable" / "ani-linux-amd64-portable.tar.gz",
              "ani")
check_archive(fix / "plainlinux" / "ani-linux-amd64.tar.gz",
              "ani")
