 ...
 *
 * Verifies that ani_store_checkpoint() does not truncate the on-disk
...
TEST(checkpoint_does_not_truncate_wal) {
    ...
    snprintf(db_path, sizeof(db_path), "%s/ani_test_ckpt_%d.db", ani_tmpdir(), (int)getpid());
    ...
    int rc_sql = ani_store_exec(s, "INSERT OR IGNORE INTO projects(name, indexed_at, root_path) "
    ...
    int rc_ckpt = ani_store_checkpoint(s);
    ASSERT_EQ(rc_ckpt, 0); /* ANI_STORE_OK */
...
TEST(seal_for_atomic_publish_makes_main_file_self_contained) {
    char db_path[256];
    char wal_path[300];
    snprintf(db_path, sizeof(db_path), "%s/ani_test_seal_%d.db", ani_tmpdir(), (int)getpid());
    snprintf(wal_path, sizeof(wal_path), "%s-wal", db_path);
    tsc_cleanup_db(db_path);

    ani_store_t *store = ani_store_open_path(db_path);
    ASSERT_NOT_NULL(store);
    ASSERT_EQ(ani_store_upsert_project(store, "sealed", "/tmp/sealed"), ANI_STORE_OK);
    ...
    ASSERT_EQ(ani_store_seal_for_atomic_publish(store), ANI_STORE_OK);
...
TEST(seal_for_atomic_publish_fails_closed_while_reader_pins_wal) {
    char db_path[256];
    char wal_path[300];
    snprintf(db_path, sizeof(db_path), "%s/ani_test_seal_busy_%d.db", ani_tmpdir(), (int)getpid());
    snprintf(wal_path, sizeof(wal_path), "%s-wal", db_path);
    tsc_cleanup_db(db_path);

    ani_store_t *store = ani_store_open_path(db_path);
    ASSERT_NOT_NULL(store);
    ASSERT_EQ(ani_store_upsert_project(store, "reader_snapshot", "/tmp/reader"), ANI_STORE_OK);
    ...
    ASSERT_EQ(ani_store_upsert_project(store, "after_snapshot", "/tmp/after"), ANI_STORE_OK);
    ASSERT_EQ(ani_store_exec(store, "PRAGMA busy_timeout=0;"), ANI_STORE_OK);
    struct stat wal_before = {0};
    ASSERT_EQ(stat(wal_path, &wal_before), 0);
    ASSERT_TRUE(wal_before.st_size > 0);
    ASSERT_EQ(ani_store_seal_for_atomic_publish(store), ANI_STORE_ERR);
...
TEST(cached_count_queries_release_delete_mode_reader_lock) {
    char db_path[256];
    snprintf(db_path, sizeof(db_path), "%s/ani_test_count_lock_%d.db", ani_tmpdir(), (int)getpid());
    tsc_cleanup_db(db_path);

    ani_store_t *setup = ani_store_open_path(db_path);
    ASSERT_NOT_NULL(setup);
    ASSERT_EQ(ani_store_upsert_project(setup, "count_lock", "/tmp/count_lock"), ANI_STORE_OK);
    ani_node_t node = {.project = "count_lock",
                       .label = "Function",
                       .name = "target",
    ...
    ASSERT_EQ(ani_store_seal_for_atomic_publish(setup), ANI_STORE_OK);
    ...
    ani_store_t *reader = ani_store_open_path_query(db_path);
    ASSERT_NOT_NULL(reader);
    ASSERT_EQ(ani_store_count_nodes(reader, "count_lock"), 1);
    ASSERT_EQ(ani_store_count_edges(reader, "count_lock"), 0);

    ani_store_t *writer = ani_store_open_path(db_path);
    ASSERT_NOT_NULL(writer);

    ani_store_close(writer);
    ani_store_close(reader);
    tsc_cleanup_db(db_path);
    PASS();
}
...
TEST(cached_node_lookups_release_delete_mode_reader_lock) {
    char db_path[256];
    snprintf(db_path, sizeof(db_path), "%s/ani_test_lookup_lock_%d.db", ani_tmpdir(),
             (int)getpid());
    tsc_cleanup_db(db_path);

    ani_store_t *setup = ani_store_open_path(db_path);
    ASSERT_NOT_NULL(setup);
    ASSERT_EQ(ani_store_upsert_project(setup, "lookup_lock", "/tmp/lookup_lock"), ANI_STORE_OK);
    ani_node_t node = {.project = "lookup_lock",
                       .label = "Function",
                       .name = "target",
                       ...
                       .start_line = 1,
                       .end_line = 1};
    int64_t node_id = ani_store_upsert_node(setup, &node);
    ...
    ASSERT_NOT_NULL(reader);
    ani_node_t found = {0};
    ASSERT_EQ(ani_store_find_node_by_id(reader, node_id, &found), ANI_STORE_OK);
    ani_node_free_fields(&found);
    ASSERT_EQ(ani_store_find_node_by_qn(reader, "lookup_lock", "lookup_lock.target", &found),
              ANI_STORE_OK);
    ani_node_free_fields(&found);
    ASSERT_EQ(ani_store_find_node_by_qn_any(reader, "lookup_lock.target", &found), ANI_STORE_OK);
    ani_node_free_fields(&found);
 ...
 * ani_store_dump_to_file, reopen — the stale generation's row must NOT be
...
TEST(dump_install_ignores_stale_wal_sidecar) {
    char *td = th_mktempdir("ani_stalewal");
    char db_path[512];
    char wal_path[512];
    char stale_copy[512];
    ...
    ASSERT_NOT_NULL(s1);
    ani_store_upsert_project(s1, "walgen", "/tmp/walgen");
    ani_node_t stale = {.project = "walgen",
                        .label = "Function",
                        .name = "stale_gen_node",
                        ...
                        .start_line = 1,
                        .end_line = 2};
    ASSERT_TRUE(ani_store_upsert_node(s1, &stale) > 0);
    ...
    ani_store_close(s1); /* clean close checkpoints + removes the WAL */
    ...
    ASSERT_NOT_NULL(s2);
    ani_store_upsert_project(s2, "walgen", "/tmp/walgen");
    ani_node_t fresh = {.project = "walgen",
                        .label = "Function",
                        .name = "fresh_gen_node",
                        ...
                        .start_line = 1,
                        .end_line = 2};
    ASSERT_TRUE(ani_store_upsert_node(s2, &fresh) > 0);
    ASSERT_EQ(ani_store_dump_to_file(s2, db_path), ANI_STORE_OK);
    ani_store_close(s2);

    /* Reader: the stale WAL must not have been replayed onto gen 2. */
    ani_store_t *s3 = ani_store_open_path(db_path);
    ASSERT_NOT_NULL(s3);
    ani_node_t *hits = NULL;
    int hit_count = 0;
    ASSERT_EQ(ani_store_find_nodes_by_name(s3, "walgen", "fresh_gen_node", &hits, &hit_count),
              ANI_STORE_OK);
    ASSERT_TRUE(hit_count >= 1);
    ani_store_free_nodes(hits, hit_count);
    hits = NULL;
    hit_count = 0;
    ASSERT_EQ(ani_store_find_nodes_by_name(s3, "walgen", "stale_gen_node", &hits, &hit_count),
              ANI_STORE_OK);
...
TEST(remove_db_sidecars_rejects_truncated_suffix_path) {
    ...
    ASSERT_TRUE(ani_remove_db_sidecars(db_path) != 0);
    PASS();
}
