...
is the one synchronous user request in that flow: it must wait for the *ANI HTTP
...
performed and the two negative cases can use a short deterministic deadline:
    ...
    make -f Makefile.ani ani-with-ui TEST_SEAMS=1 BUILD_DIR=build/ui-open-test
    python3 tests/test_daemon_open_readiness.py build/ui-open-test/ani
...
OPEN_MARKER_ENV = "ANI_TEST_DAEMON_OPEN_MARKER"
READY_TIMEOUT_ENV = "ANI_TEST_DAEMON_UI_READY_TIMEOUT_MS"
RUNTIME_PARENT_ENV = "ANI_TEST_DAEMON_RUNTIME_PARENT"
...
class OldMarkerResponder:
    ...
    BODY = b"<!doctype html><html><head><title>Ani</title></head></html>"
...
def fixture_environment(work, cache, marker, timeout_ms):
    ...
    env["ANI_TEST_FAKE_BROWSER_MARKER"] = marker
    ...
    if os.name != "nt":
        ...
        if not os.path.exists(opener_path):
            with open(opener_path, "w", encoding="utf-8", newline="\n") as handle:
                handle.write("#!/bin/sh\nprintf '%s' \"$1\" > \"$ANI_TEST_FAKE_BROWSER_MARKER\"\n")
...
def assert_delayed_success(binary, work):
    ...
    try:
        ...
        # Releasing the port lets the background retry publish the real ANI UI.
        blocker.close()
        blocker = None
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="ani_uiopen_", dir=short_temp_root) as work:
