 ...
 *
 * SKIPPED (with reason):
 *   systemverilog — ANI_LANG_SYSTEMVERILOG has no extension entry in the
 *     EXT_TABLE (language.c); the only SV extension ".sv" maps to
 *     ANI_LANG_VERILOG.  File-based index_repository cannot route to
 *     ANI_LANG_SYSTEMVERILOG via extension alone.  It is already exercised
...
static ani_store_t *gpd_open_indexed(GpdProj *lp) {
...
static ani_store_t *gpd_index_files(GpdProj *lp, const GpdFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/ani_gpd_XXXXXX");
...
static void gpd_cleanup(GpdProj *lp, ani_store_t *store) {
...
static int gpd_count_label(ani_store_t *store, const char *project, const char *label) {
...
static int gpd_type_nodes(ani_store_t *store, const char *project) {
...
static GpdMetrics gpd_metrics_files(const GpdFile *files, int nfiles) {
    GpdProj lp;
    ani_store_t *store = gpd_index_files(&lp, files, nfiles);
    GpdMetrics m = {0};
    if (store) {
        m.ok          = 1;
 ...
 * Extension: .bicep → ANI_LANG_BICEP.
 * ══════════════════════════════════════════════════════════════════ */
 ...
 * Extension: .cfc → ANI_LANG_CFSCRIPT.
 * ══════════════════════════════════════════════════════════════════ */
 ...
 * Spec: ANI_LANG_JANET uses empty_types for all definition types — no function
 *       extraction is configured; the grammar emits only a source/Module node.
 * Extension: .janet → ANI_LANG_JANET.
 * ══════════════════════════════════════════════════════════════════ */
 ...
 * Extension: .m → disambiguated to ANI_LANG_OBJC when @interface/@implementation
 *             markers are present (via ani_disambiguate_m).
 * ══════════════════════════════════════════════════════════════════ */
...
TEST(probe_objc_method_nodes) {
     ...
     *        sets def.label = "Method" for ANI_LANG_OBJC method_definition),
 ...
 * Extension: .purs → ANI_LANG_PURESCRIPT.
 * ══════════════════════════════════════════════════════════════════ */
 ...
 * Extension: .smali → ANI_LANG_SMALI.
 * ══════════════════════════════════════════════════════════════════ */
 ...
 * Extension: .td → ANI_LANG_TABLEGEN.
 * ══════════════════════════════════════════════════════════════════ */
 ...
 * Extension: .tla → ANI_LANG_TLAPLUS.
 * ══════════════════════════════════════════════════════════════════ */
 ...
 * Extension: .v → ANI_LANG_VERILOG.
 * Note: .sv also maps to ANI_LANG_VERILOG (not ANI_LANG_SYSTEMVERILOG).
 * ══════════════════════════════════════════════════════════════════ */
...
/* Verilog: SystemVerilog-style file (.sv) — lands on ANI_LANG_VERILOG via EXT_TABLE.
 * No separate ANI_LANG_SYSTEMVERILOG routing is available through file extension. */
TEST(probe_verilog_sv_extension) {
    GpdMetrics m = gpd_metrics("adder.sv",
        ...
        "endmodule\n");
    ASSERT_TRUE(m.ok);
    /* GREEN: .sv → ANI_LANG_VERILOG; module_declaration must produce type-like node. */
    ASSERT_TRUE(m.types >= 1);
    PASS();
}
 ...
 * Extension: .vhd / .vhdl → ANI_LANG_VHDL.
 * ══════════════════════════════════════════════════════════════════ */
 ...
 * Extension: .wl / .wls → ANI_LANG_WOLFRAM.
 * ══════════════════════════════════════════════════════════════════ */
...
SUITE(grammar_probe_d) {
    ...
    /* Verilog (.v — also covers .sv routed as ANI_LANG_VERILOG) */
