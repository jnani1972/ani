 ...
 * histogram that ani_extract_file() produces for the shared fixture (e.g.
 ...
 * golden is stable. Labels come from ANIDefinition.label assigned during
...
static void label_histogram(ANIFileResult *r, char *out, size_t out_sz) {
...
TEST(grammar_label_goldens) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        ANIFileResult *r =
            ani_extract_file(c->src, (int)strlen(c->src), c->lang, "lbl", c->path, 0, NULL, NULL);
        if (!r) {
...
static int non_module_defs(ANIFileResult *r) {
    int n = 0;
    for (int i = 0; i < r->defs.count; i++) {
...
TEST(grammar_code_extracts_defs) {
    ...
    for (int k = 0; MUST_EXTRACT_DEFS[k]; k++) {
        ...
        for (int i = 0; i < (int)ANI_GRAMMAR_CASES_COUNT; i++) {
            if (strcmp(ANI_GRAMMAR_CASES[i].name, name) == 0) {
                c = &ANI_GRAMMAR_CASES[i];
                break;
            }
        }
        if (!c) {
            fprintf(stderr, "  [CODE-DEFS] FAIL %-14s — no fixture in ANI_GRAMMAR_CASES\n", name);
            failures++;
            continue;
        }
        ANIFileResult *r =
            ani_extract_file(c->src, (int)strlen(c->src), c->lang, "cd", c->path, 0, NULL, NULL);
        if (!r) {
