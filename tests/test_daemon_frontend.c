...
static bool frontend_maintenance_fixture_start(frontend_maintenance_fixture_t *fixture,
    ...
    fixture->manager = fixture->endpoint ? ani_version_cohort_manager_new(fixture->endpoint) : NULL;
...
static void frontend_maintenance_fixture_finish(frontend_maintenance_fixture_t *fixture) {
    while (fixture->manager &&
           ani_version_cohort_manager_free(&fixture->manager) != ANI_PRIVATE_FILE_LOCK_OK) {
        ani_usleep(1000);
    }
...
#if defined(ANI_ENABLE_TEST_SEAMS) && ANI_ENABLE_TEST_SEAMS
static ani_daemon_runtime_application_session_t *frontend_idle_session_open(
    void *context, ani_daemon_client_id_t client_id, uint64_t authenticated_process_id) {
    return context && client_id != ANI_DAEMON_CLIENT_ID_INVALID && authenticated_process_id != 0
               ? (ani_daemon_runtime_application_session_t *)context
               : NULL;
}

static ani_daemon_runtime_application_status_t frontend_idle_request(
    void *context, ani_daemon_runtime_application_session_t *session,
    ...
    (void)request_token;
    if (!context || session != (ani_daemon_runtime_application_session_t *)context ||
...
static bool frontend_idle_fixture_start(frontend_idle_fixture_t *fixture) {
    ...
    if (log_written <= 0 || log_written >= (int)sizeof(fixture->conflict_log) ||
        !ani_daemon_runtime_process_build_fingerprint(frontend_test_process_id(), build)) {
        return false;
    }
    ...
    fixture->client = fixture->service
                          ? ani_daemon_runtime_client_connect(fixture->maintenance.endpoint, &identity,
                                                              30000, &connect_result)
                          : NULL;
    return fixture->client && connect_result.status == ANI_DAEMON_RUNTIME_CONNECT_ACCEPTED;
}

static bool frontend_idle_fixture_finish(frontend_idle_fixture_t *fixture) {
    bool ok = true;
    if (fixture->client) {
        ok = ani_daemon_runtime_client_close(fixture->client, 30000) && ok;
        fixture->client = NULL;
    }
    if (fixture->service) {
        if (ani_daemon_runtime_service_state(fixture->service) !=
            ANI_DAEMON_RUNTIME_SERVICE_EXITED) {
            ok = ani_daemon_runtime_service_stop(fixture->service, 30000) && ok;
        }
        ok = ani_daemon_runtime_service_free(fixture->service) && ok;
...
static void *frontend_idle_run(void *opaque) {
    ...
    run->result =
        ani_daemon_frontend_mcp_run(run->client, run->manager, run->input, run->output);
    return NULL;
}
#endif
...
static ani_daemon_build_identity_t frontend_test_identity(void) {
    ani_daemon_build_identity_t identity = {
        .semantic_version = "2.4.0",
...
static ani_daemon_runtime_application_session_t *frontend_eof_application_session_open(
...
static ani_daemon_runtime_application_status_t frontend_eof_application_request(
...
static void *frontend_eof_writer(void *opaque) {
    ...
    uint64_t deadline = ani_now_ms() + FRONTEND_EOF_TEST_REQUEST_TIMEOUT_MS;
    while (
        ok &&
...
static bool frontend_eof_fixture_start(frontend_eof_fixture_t *fixture, const char *parent) {
    ...
    if (log_written <= 0 || log_written >= (int)sizeof(fixture->conflict_log) ||
        ...
        !ani_daemon_runtime_process_build_fingerprint((uint64_t)getpid(), build)) {
        return false;
    }
    fixture->endpoint = ani_daemon_ipc_endpoint_new(key, parent);
    fixture->manager = fixture->endpoint ? ani_version_cohort_manager_new(fixture->endpoint) : NULL;
    ani_daemon_build_identity_t identity = {
        .semantic_version = "2.4.0",
        .build_fingerprint = build,
    ...
    fixture->service = fixture->manager ? ani_daemon_runtime_service_start(&config) : NULL;
    ani_daemon_runtime_connect_result_t connect_result = {0};
    fixture->client = fixture->service
                          ? ani_daemon_runtime_client_connect(fixture->endpoint, &identity,
...
static bool frontend_eof_fixture_finish(frontend_eof_fixture_t *fixture) {
    bool ok = true;
    if (fixture->client) {
        ok = ani_daemon_runtime_client_close(fixture->client,
                                             FRONTEND_EOF_TEST_REQUEST_TIMEOUT_MS) &&
             ok;
        fixture->client = NULL;
    }
    if (fixture->service) {
        if (ani_daemon_runtime_service_state(fixture->service) !=
            ANI_DAEMON_RUNTIME_SERVICE_EXITED) {
            ok = ani_daemon_runtime_service_stop(fixture->service,
                                                 FRONTEND_EOF_TEST_REQUEST_TIMEOUT_MS) &&
                 ok;
        }
        ok = ani_daemon_runtime_service_free(fixture->service) && ok;
    ...
    if (fixture->manager) {
        ok = ani_version_cohort_manager_free(&fixture->manager) == ANI_PRIVATE_FILE_LOCK_OK && ok;
    }
    ani_daemon_ipc_endpoint_free(fixture->endpoint);
...
static int frontend_eof_child_run(const char *parent, bool overflow) {
    ...
    bool writer_started =
        input && output && ani_thread_create(&writer_thread, 0, frontend_eof_writer, &writer) == 0;
    if (!writer_started) {
        if (input) {
            (void)fclose(input);
        } else {
    ...
    ani_daemon_runtime_client_t *frontend_client = fixture.client;
    fixture.client = NULL; /* ani_daemon_frontend_mcp_run consumes it. */
    int result = ani_daemon_frontend_mcp_run(frontend_client, fixture.manager, input, output);
    bool joined = ani_thread_join(&writer_thread) == 0;
...
static bool frontend_eof_run_isolated(const char *tag, bool overflow) {
    char parent[FRONTEND_TEST_PATH_CAP];
    int written =
        snprintf(parent, sizeof(parent), "%s/ani-frontend-eof-%s-XXXXXX", ani_tmpdir(), tag);
    if (written <= 0 || written >= (int)sizeof(parent) || !ani_mkdtemp(parent)) {
        return false;
    }
    pid_t child = fork();
    if (child == 0) {
...
static void frontend_test_release_lease(ani_version_cohort_lease_t **lease) {
    while (lease && *lease && ani_version_cohort_lease_release(lease) != ANI_PRIVATE_FILE_LOCK_OK) {
        ani_usleep(1000);
    }
}

static bool frontend_test_release_lease_until(ani_version_cohort_lease_t **lease,
                                              uint64_t deadline_ms) {
    while (lease && *lease) {
        if (ani_version_cohort_lease_release(lease) == ANI_PRIVATE_FILE_LOCK_OK) {
            continue;
        }
...
static ani_version_cohort_quiesce_result_t frontend_test_quiesce_requested(void *context);

static bool frontend_backpressure_identity(ani_daemon_build_identity_t *identity,
                                           const char build[ANI_DAEMON_BUILD_FINGERPRINT_SIZE]) {
    if (!identity || !build || strlen(build) != ANI_DAEMON_BUILD_FINGERPRINT_SIZE - 1) {
        return false;
    }
...
static int frontend_backpressure_daemon_run(const char *parent, int ready_fd, int cancel_fd,
    ...
    char key[ANI_DAEMON_KEY_SIZE];
    char conflict_log[FRONTEND_TEST_PATH_CAP];
    ani_daemon_build_identity_t identity = {0};
    ...
    ani_daemon_ipc_endpoint_t *endpoint =
        log_written > 0 && log_written < (int)sizeof(conflict_log) &&
                ani_daemon_rendezvous_key(key) && frontend_backpressure_identity(&identity, build)
    ...
    ani_daemon_runtime_service_t *service =
        endpoint ? ani_daemon_runtime_service_start(&config) : NULL;
    const char ready = 'R';
    bool announced = service && write(ready_fd, &ready, 1) == 1;
    uint64_t cancellation_deadline = ani_now_ms() + FRONTEND_BACKPRESSURE_RUNTIME_TIMEOUT_MS;
    while (announced &&
           atomic_load_explicit(&application.session_cancels, memory_order_acquire) == 0 &&
           ani_now_ms() < cancellation_deadline) {
        ani_usleep(1000);
    }
    ...
    bool shutdown_released =
        ...
        frontend_test_read_byte(control_fd, &control,
                                ani_now_ms() + FRONTEND_BACKPRESSURE_RUNTIME_TIMEOUT_MS) &&
        control == 'S';
    ...
    bool stopped =
        service && ani_daemon_runtime_service_stop(service, FRONTEND_EOF_TEST_REQUEST_TIMEOUT_MS);
    bool exited = stopped && ani_daemon_runtime_service_wait_exited(service, 0);
    bool freed = exited && ani_daemon_runtime_service_free(service);
...
static int frontend_backpressure_frontend_run(const char *parent, int input_fd, int input_write_fd,
    ...
    ani_version_cohort_status_t admitted =
        manager ? ani_version_cohort_acquire(manager, &identity, ani_now_ms() + 2000U, &participant,
                                             &conflict)
                : ANI_VERSION_COHORT_IO;
    ani_daemon_runtime_connect_result_t connect_result = {0};
    ani_daemon_runtime_client_t *client =
        admitted == ANI_VERSION_COHORT_OK
            ? ani_daemon_runtime_client_connect(
    ...
    bool writer_started =
        ...
        ani_thread_create(&writer_thread, 0, frontend_backpressure_writer, &writer) == 0;
    if (!writer_started) {
        if (client) {
            (void)ani_daemon_runtime_client_close(client, FRONTEND_EOF_TEST_REQUEST_TIMEOUT_MS);
        }
        if (input) {
            (void)fclose(input);
        } else {
    ...
    int result = ani_daemon_frontend_mcp_run(client, manager, input, output);
    bool joined = ani_thread_join(&writer_thread) == 0;
    bool writer_ok = atomic_load_explicit(&writer.finished, memory_order_acquire) &&
                     atomic_load_explicit(&writer.succeeded, memory_order_acquire);
    bool input_closed = fclose(input) == 0;
    ...
    frontend_test_release_lease(&participant);
    bool manager_closed = ani_version_cohort_manager_free(&manager) == ANI_PRIVATE_FILE_LOCK_OK;
    ani_daemon_ipc_endpoint_free(endpoint);
...
static bool frontend_backpressure_run_isolated(bool maintenance) {
    char parent[FRONTEND_TEST_PATH_CAP];
    char build[ANI_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    int path_written = snprintf(parent, sizeof(parent),
                                maintenance ? "%s/ani-frontend-maintenance-backpressure-XXXXXX"
                                            : "%s/ani-frontend-backpressure-XXXXXX",
                                ani_tmpdir());
    int ready_pipe[2] = {-1, -1};
    int cancel_pipe[2] = {-1, -1};
    int control_pipe[2] = {-1, -1};
    bool directory_ready =
        path_written > 0 && path_written < (int)sizeof(parent) && ani_mkdtemp(parent);
    bool identity_ready =
        directory_ready && ani_daemon_runtime_process_build_fingerprint((uint64_t)getpid(), build);
    ...
    /* The daemon child runs ani_daemon_runtime_service_start before it can
    ...
    bool announced_read =
        daemon > 0 && frontend_test_read_byte(ready_pipe[0], &announced_marker,
                                              ani_now_ms() + announce_backstop_ms);
    ...
    char sentinel_key[ANI_DAEMON_KEY_SIZE];
    ani_daemon_build_identity_t sentinel_identity = {0};
    ani_daemon_ipc_endpoint_t *sentinel_endpoint =
        announced && ani_daemon_rendezvous_key(sentinel_key) &&
                frontend_backpressure_identity(&sentinel_identity, build)
            ? ani_daemon_ipc_endpoint_new(sentinel_key, parent)
            : NULL;
    ani_daemon_runtime_connect_result_t sentinel_connect = {0};
    ani_daemon_runtime_client_t *sentinel =
        sentinel_endpoint ? ani_daemon_runtime_client_connect(sentinel_endpoint, &sentinel_identity,
                                                              FRONTEND_EOF_TEST_REQUEST_TIMEOUT_MS,
                                                              &sentinel_connect)
                          : NULL;
    bool sentinel_ready =
        sentinel && sentinel_connect.status == ANI_DAEMON_RUNTIME_CONNECT_ACCEPTED;
    ...
    ani_daemon_ipc_endpoint_t *mutation_endpoint = NULL;
    ani_version_cohort_manager_t *mutation_manager = NULL;
    ...
    ani_version_cohort_status_t mutation_status = ANI_VERSION_COHORT_OK;
    if (maintenance) {
        char mutation_key[ANI_DAEMON_KEY_SIZE];
        mutation_endpoint = output_observed && ani_daemon_rendezvous_key(mutation_key)
                                ? ani_daemon_ipc_endpoint_new(mutation_key, parent)
                                : NULL;
        mutation_manager =
            mutation_endpoint ? ani_version_cohort_manager_new(mutation_endpoint) : NULL;
        mutation_status = mutation_manager
                              ? ani_version_cohort_reserve_for_mutation(
                                    mutation_manager, ani_now_ms() + 8000U,
                                    frontend_test_quiesce_requested, NULL, &quiesce, &mutation)
                              : ANI_VERSION_COHORT_IO;
        if (mutation_status != ANI_VERSION_COHORT_OK && frontend > 0) {
    ...
    bool checkpoint_read =
        frontend_test_read_byte(ready_pipe[0], &checkpoint_marker, ani_now_ms() + 4000U);
    ...
    bool sentinel_usable =
        ...
        ani_daemon_runtime_client_heartbeat(sentinel, FRONTEND_EOF_TEST_REQUEST_TIMEOUT_MS);
    bool sentinel_closed =
        sentinel && ani_daemon_runtime_client_close(sentinel, FRONTEND_EOF_TEST_REQUEST_TIMEOUT_MS);
    sentinel = NULL;
    ani_daemon_ipc_endpoint_free(sentinel_endpoint);
    ...
    char done_marker = '\0';
    bool done_read =
        shutdown_sent && frontend_test_read_byte(ready_pipe[0], &done_marker, ani_now_ms() + 4000U);
    ...
    uint64_t cleanup_now = ani_now_ms();
    ...
    while (mutation_released && mutation_manager && ani_now_ms() < cleanup_deadline &&
           ani_version_cohort_manager_free(&mutation_manager) != ANI_PRIVATE_FILE_LOCK_OK) {
        ani_usleep(1000);
    }
    bool mutation_manager_closed = mutation_manager == NULL;
    ani_daemon_ipc_endpoint_free(mutation_endpoint);
    ...
    bool maintenance_completed =
        !maintenance || (output_observed && mutation_status == ANI_VERSION_COHORT_OK &&
                         quiesce == ANI_VERSION_COHORT_QUIESCE_REQUESTED);
...
TEST(daemon_frontend_recognizes_exact_cancellation_notification) {
    ASSERT_TRUE(ani_daemon_frontend_is_cancellation_notification(
...
TEST(daemon_frontend_correlates_cancellation_to_exact_request) {
    ASSERT_TRUE(ani_daemon_frontend_cancellation_matches_request(
        "{\"jsonrpc\":\"2.0\",\"method\":\"notifications/cancelled\","
        "\"params\":{\"requestId\":7}}",
        7, NULL));
    ASSERT_FALSE(ani_daemon_frontend_cancellation_matches_request(
        "{\"jsonrpc\":\"2.0\",\"method\":\"notifications/cancelled\","
        "\"params\":{\"requestId\":8}}",
        7, NULL));
    ASSERT_TRUE(ani_daemon_frontend_cancellation_matches_request(
    ...
    ASSERT_FALSE(ani_daemon_frontend_cancellation_matches_request(
        ...
        7, NULL));
    ASSERT_FALSE(
        ani_daemon_frontend_cancellation_matches_request("{\"jsonrpc\":\"2.0\",\"id\":9,"
...
TEST(daemon_frontend_ignores_cancellation_text_in_string_content) {
    ASSERT_FALSE(ani_daemon_frontend_is_cancellation_notification(
...
TEST(daemon_frontend_rejects_non_notification_cancellation_shapes) {
    ...
    ASSERT_FALSE(ani_daemon_frontend_is_cancellation_notification(
        ...
        "\"notifications/cancelled\"}}"));
    ASSERT_FALSE(ani_daemon_frontend_is_cancellation_notification("not-json"));
    ASSERT_FALSE(ani_daemon_frontend_is_cancellation_notification(NULL));
    PASS();
}
...
TEST(daemon_frontend_idle_uses_one_maintenance_observer) {
    ...
    int input_pipe[2] = {-1, -1};
    bool pipe_ready = fixture_ready && ani_pipe(input_pipe) == 0;
    ...
    ani_daemon_frontend_test_observer_reset(true);
    ani_thread_t frontend_thread;
    bool thread_started =
        output && ani_thread_create(&frontend_thread, 0, frontend_idle_run, &run) == 0;
    uint64_t deadline = ani_now_ms() + 10000U;
    while (thread_started && ani_now_ms() < deadline &&
           (!ani_daemon_frontend_test_monitor_waiting() ||
            ani_daemon_frontend_test_worker_idle_cycles() == 0)) {
        ani_usleep(1000);
    }
    bool monitor_waiting = ani_daemon_frontend_test_monitor_waiting();
    uint64_t monitor_observations = ani_daemon_frontend_test_monitor_observations();
    uint64_t worker_observations = ani_daemon_frontend_test_worker_observations();
    uint64_t idle_cycles = ani_daemon_frontend_test_worker_idle_cycles();
    ani_daemon_frontend_test_observer_release();
    bool input_closed = !pipe_ready || frontend_test_close_fd(input_pipe[1]) == 0;
    bool joined = thread_started && ani_thread_join(&frontend_thread) == 0;
...
TEST(daemon_frontend_maintenance_exits_while_stdio_reader_is_blocked) {
    ...
    if (child == 0) {
        ...
        (void)close(ready_pipe[0]);
        ani_daemon_ipc_endpoint_t *endpoint =
            ani_daemon_ipc_endpoint_new("0123456789abcdef", fixture.parent);
        ani_version_cohort_manager_t *manager =
            endpoint ? ani_version_cohort_manager_new(endpoint) : NULL;
        ani_version_cohort_lease_t *participant = NULL;
        ani_daemon_conflict_t conflict;
        ani_daemon_build_identity_t identity = frontend_test_identity();
        ani_version_cohort_status_t admitted =
            manager ? ani_version_cohort_acquire(manager, &identity, ani_now_ms() + 2000U,
                                                 &participant, &conflict)
                    : ANI_VERSION_COHORT_IO;
        FILE *input = admitted == ANI_VERSION_COHORT_OK ? fdopen(input_pipe[0], "rb") : NULL;
        FILE *output = input ? tmpfile() : NULL;
        const char ready = 'R';
        ...
        int result = ani_daemon_frontend_mcp_run((ani_daemon_runtime_client_t *)(uintptr_t)1,
    ...
    bool announced = child > 0 && frontend_test_wait_byte(ready_pipe[0], 'R', ani_now_ms() + 2000U);
    ani_version_cohort_lease_t *mutation = NULL;
    ani_version_cohort_quiesce_result_t quiesce = ANI_VERSION_COHORT_QUIESCE_NOT_NEEDED;
    ani_version_cohort_status_t status =
        announced ? ani_version_cohort_reserve_for_mutation(fixture.manager, ani_now_ms() + 3000U,
                                                            frontend_test_quiesce_requested, NULL,
                                                            &quiesce, &mutation)
                  : ANI_VERSION_COHORT_IO;
    if (status != ANI_VERSION_COHORT_OK && child > 0) {
    ...
    ASSERT_EQ(status, ANI_VERSION_COHORT_OK);
    ASSERT_EQ(quiesce, ANI_VERSION_COHORT_QUIESCE_REQUESTED);
    ASSERT_TRUE(waited);
...
TEST(daemon_local_participant_monitor_cancels_then_bounds_active_operation) {
    ...
    if (child == 0) {
        ...
        ani_version_cohort_status_t admitted =
            manager ? ani_version_cohort_acquire(manager, &identity, ani_now_ms() + 2000U,
                                                 &participant, &conflict)
                    : ANI_VERSION_COHORT_IO;
        ani_daemon_maintenance_monitor_t *monitor =
            admitted == ANI_VERSION_COHORT_OK
                ? ani_daemon_maintenance_monitor_start(manager, frontend_test_cancel_active,
    ...
    ani_version_cohort_quiesce_result_t quiesce = ANI_VERSION_COHORT_QUIESCE_NOT_NEEDED;
    ani_version_cohort_status_t status =
        announced ? ani_version_cohort_reserve_for_mutation(fixture.manager, ani_now_ms() + 5000U,
                                                            frontend_test_quiesce_requested, NULL,
                                                            &quiesce, &mutation)
                  : ANI_VERSION_COHORT_IO;
    bool cancelled = status == ANI_VERSION_COHORT_OK &&
                     frontend_test_wait_byte(cancel_pipe[0], 'C', ani_now_ms() + 1000U);
    if (status != ANI_VERSION_COHORT_OK && child > 0) {
        (void)kill(child, SIGKILL);
    }
    int child_status = 0;
...
TEST(daemon_local_participant_monitor_allows_supervisor_containment_window) {
    ...
    if (child == 0) {
        ...
        ani_daemon_maintenance_monitor_t *monitor =
            admitted == ANI_VERSION_COHORT_OK
                ? ani_daemon_maintenance_monitor_start(manager, frontend_test_cancel_and_mark,
        ...
        ani_usleep(2100U * 1000U);
        bool stopped = ani_daemon_maintenance_monitor_stop(&monitor);
        frontend_test_release_lease(&participant);
...
TEST(daemon_local_participant_monitor_joins_before_manager_teardown) {
    ...
    ani_daemon_maintenance_monitor_t *monitor = ani_daemon_maintenance_monitor_start(
        fixture.manager, NULL, NULL, EXIT_FAILURE, "test-idle-command");
    bool started = monitor != NULL;
    bool stopped = started && ani_daemon_maintenance_monitor_stop(&monitor);
