...
static void imp_run_pass(ani_gbuf_t *gb) {
...
static int imp_check_label(ani_store_t *s, const char *project, const char *label, int expect) {
    ani_node_t *nodes = NULL;
    int count = 0;
    if (ani_store_find_nodes_by_label(s, project, label, &nodes, &count) != ANI_STORE_OK) {
        return -1;
    }
    int bad = 0;
...
static int imp_collect_scores(const char *db_path, const char *project, ImpScoreSet *out) {
    ...
    for (int li = 0; li < 3; li++) {
        ani_node_t *nodes = NULL;
        int n = 0;
        if (ani_store_find_nodes_by_label(s, project, kLabels[li], &nodes, &n) != ANI_STORE_OK) {
            rc = -1;
            break;
        }
...
TEST(importance_pass_actually_runs_in_full_pipeline) {
    char *tmp = th_mktempdir("ani_imp_run");
    if (!tmp) {
        FAIL("tmpdir");
    }
    char root[512];
    ...
    ani_pipeline_t *p = ani_pipeline_new(root, db_path, ANI_MODE_FULL);
    ASSERT_NOT_NULL(p);
    ...
    ani_node_t *files = NULL;
    int filec = 0;
    ASSERT_EQ(ani_store_find_nodes_by_label(s, project, "File", &files, &filec), ANI_STORE_OK);
    for (int i = 0; i < filec; i++) {
        ASSERT_EQ(imp_key_count(files[i].properties_json), 0);
    }
    ani_store_free_nodes(files, filec);
...
TEST(importance_base_is_sqrt_of_incoming_refs) {
    ...
    int64_t target =
        ani_gbuf_upsert_node(gb, "Function", "target", "pkg.target", "pkg/main.go", 1, 1, "{}");
    int64_t c1 = ani_gbuf_upsert_node(gb, "Function", "c1", "pkg.c1", "pkg/main.go", 2, 2, "{}");
    int64_t c2 = ani_gbuf_upsert_node(gb, "Function", "c2", "pkg.c2", "pkg/main.go", 3, 3, "{}");
    int64_t c3 = ani_gbuf_upsert_node(gb, "Function", "c3", "pkg.c3", "pkg/main.go", 4, 4, "{}");
    int64_t lonely =
        ani_gbuf_upsert_node(gb, "Function", "lonely", "pkg.lonely", "pkg/main.go", 5, 5, "{}");
    ASSERT_GT(target, 0);
    ASSERT_GT(lonely, 0);

    ani_gbuf_insert_edge(gb, c1, target, "CALLS", "{}");
    ani_gbuf_insert_edge(gb, c2, target, "CALLS", "{}");
    ani_gbuf_insert_edge(gb, c3, target, "USAGE", "{}"); /* spans BOTH edge types */

    imp_run_pass(gb);

    const ani_gbuf_node_t *tn = ani_gbuf_find_by_id(gb, target);
    ASSERT_NOT_NULL(tn);
    ...
    const ani_gbuf_node_t *ln = ani_gbuf_find_by_id(gb, lonely);
    ASSERT_NOT_NULL(ln);
...
TEST(importance_private_names_are_demoted) {
    ...
    int64_t priv =
        ani_gbuf_upsert_node(gb, "Function", "_run", "pkg._run", "pkg/main.go", 1, 1, "{}");
    int64_t pub = ani_gbuf_upsert_node(gb, "Function", "run", "pkg.run", "pkg/main.go", 2, 2, "{}");
    for (int i = 0; i < 4; i++) {
        char name[ANI_SZ_32];
        char qn[ANI_SZ_64];
        ...
        int64_t c =
            ani_gbuf_upsert_node(gb, "Function", name, qn, "pkg/main.go", 10 + i, 10 + i, "{}");
        ani_gbuf_insert_edge(gb, c, priv, "CALLS", "{}");
        snprintf(name, sizeof(name), "q%d", i);
        snprintf(qn, sizeof(qn), "pkg.q%d", i);
        c = ani_gbuf_upsert_node(gb, "Function", name, qn, "pkg/main.go", 20 + i, 20 + i, "{}");
        ani_gbuf_insert_edge(gb, c, pub, "CALLS", "{}");
    }

    imp_run_pass(gb);

    ASSERT_FLOAT_EQ(imp_value(ani_gbuf_find_by_id(gb, priv)->properties_json), sqrt(4.0) * 0.1,
                    1e-6);
    ASSERT_FLOAT_EQ(imp_value(ani_gbuf_find_by_id(gb, pub)->properties_json), sqrt(4.0), 1e-6);
...
TEST(importance_generic_names_are_demoted_on_distinct_files) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        int64_t id = ani_gbuf_upsert_node(gb, "Function", "generic", qn, file, 1, 1, "{}");
        if (i == 0) {
            generic = id;
        }
    }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        int64_t id = ani_gbuf_upsert_node(gb, "Function", "narrow", qn, file, 1, 1, "{}");
        if (i == 0) {
            narrow = id;
        }
    }
    ani_gbuf_upsert_node(gb, "Function", "narrow", "n0.narrow_b", "n0/main.go", 9, 9, "{}");
    ani_gbuf_upsert_node(gb, "Function", "narrow", "n1.narrow_b", "n1/main.go", 9, 9, "{}");
    ...
    for (int i = 0; i < 9; i++) {
        ...
        int64_t c = ani_gbuf_upsert_node(gb, "Function", name, qn, "z/main.go", i, i, "{}");
        ani_gbuf_insert_edge(gb, c, i < 4 ? generic : narrow, "CALLS", "{}");
    }

    imp_run_pass(gb);

    /* generic: sqrt(4) * 0.1;  narrow: sqrt(5), undemoted. */
    ASSERT_FLOAT_EQ(imp_value(ani_gbuf_find_by_id(gb, generic)->properties_json), sqrt(4.0) * 0.1,
                    1e-6);
    ASSERT_FLOAT_EQ(imp_value(ani_gbuf_find_by_id(gb, narrow)->properties_json), sqrt(5.0), 1e-6);
...
TEST(importance_distinctive_names_are_promoted) {
    ...
    int64_t snake = ani_gbuf_upsert_node(gb, "Function", "load_user_profile",
                                         "pkg.load_user_profile", "pkg/a.go", 1, 1, "{}");
    int64_t camel = ani_gbuf_upsert_node(gb, "Function", "loadUserProfile", "pkg.loadUserProfile",
                                         "pkg/b.go", 1, 1, "{}");
    int64_t plain =
        ani_gbuf_upsert_node(gb, "Function", "aggregate", "pkg.aggregate", "pkg/c.go", 1, 1, "{}");
    int64_t shortnm =
        ani_gbuf_upsert_node(gb, "Function", "get_x", "pkg.get_x", "pkg/d.go", 1, 1, "{}");
    ...
    for (int t = 0; t < 4; t++) {
        ...
        int64_t c = ani_gbuf_upsert_node(gb, "Function", name, qn, "z/main.go", t, t, "{}");
        ani_gbuf_insert_edge(gb, c, targets[t], "CALLS", "{}");
    }

    imp_run_pass(gb);

    ASSERT_FLOAT_EQ(imp_value(ani_gbuf_find_by_id(gb, snake)->properties_json), 1.0 * 10.0, 1e-6);
    ASSERT_FLOAT_EQ(imp_value(ani_gbuf_find_by_id(gb, camel)->properties_json), 1.0 * 10.0, 1e-6);
    ASSERT_FLOAT_EQ(imp_value(ani_gbuf_find_by_id(gb, plain)->properties_json), 1.0, 1e-6);
    ASSERT_FLOAT_EQ(imp_value(ani_gbuf_find_by_id(gb, shortnm)->properties_json), 1.0, 1e-6);
 ...
 * ani_is_test_path classifier) or being the target of a TESTS edge. */
TEST(importance_test_scaffolding_is_demoted) {
    ani_gbuf_t *gb = ani_gbuf_new("test-proj", "/tmp/test");
    ASSERT_NOT_NULL(gb);

    int64_t in_test_file = ani_gbuf_upsert_node(gb, "Function", "fixture", "t.fixture",
                                                "pkg/thing_test.go", 1, 1, "{}");
    int64_t tests_target =
        ani_gbuf_upsert_node(gb, "Function", "helper", "pkg.helper", "pkg/util.go", 1, 1, "{}");
    int64_t plain =
        ani_gbuf_upsert_node(gb, "Function", "worker", "pkg.worker", "pkg/util.go", 2, 2, "{}");
    /* A production file whose NAME merely contains "test" must NOT be demoted. */
    int64_t testutil = ani_gbuf_upsert_node(gb, "Function", "spawn", "pkg.spawn",
    ...
    int64_t tfn = ani_gbuf_upsert_node(gb, "Function", "TestHelper", "t.TestHelper",
                                       "pkg/util_test.go", 5, 5, "{}");
    ani_gbuf_insert_edge(gb, tfn, tests_target, "TESTS", "{}");

    imp_run_pass(gb);

    ASSERT_FLOAT_EQ(imp_value(ani_gbuf_find_by_id(gb, in_test_file)->properties_json), 1.0 * 0.1,
                    1e-6);
    ASSERT_FLOAT_EQ(imp_value(ani_gbuf_find_by_id(gb, tests_target)->properties_json), 1.0 * 0.1,
                    1e-6);
    ASSERT_FLOAT_EQ(imp_value(ani_gbuf_find_by_id(gb, plain)->properties_json), 1.0, 1e-6);
    ASSERT_FLOAT_EQ(imp_value(ani_gbuf_find_by_id(gb, testutil)->properties_json), 1.0, 1e-6);
...
TEST(importance_append_prop_overwrites_instead_of_duplicating) {
    ani_gbuf_t *gb = ani_gbuf_new("test-proj", "/tmp/test");
    ASSERT_NOT_NULL(gb);
    int64_t id = ani_gbuf_upsert_node(gb, "Function", "f", "p.f", "p/a.go", 1, 1,
                                      "{\"loop_depth\":2,\"recursive\":false}");
    ani_gbuf_node_t *n = (ani_gbuf_node_t *)ani_gbuf_find_by_id(gb, id);
    ASSERT_NOT_NULL(n);

    ani_pipeline_importance_append_prop(n, 1.5);
    ASSERT_EQ(imp_key_count(n->properties_json), 1);
    ASSERT_FLOAT_EQ(imp_value(n->properties_json), 1.5, 1e-6);

    ani_pipeline_importance_append_prop(n, 42.25);
    ...
    ani_pipeline_importance_append_prop(n, 3.0);
    ASSERT_EQ(imp_key_count(n->properties_json), 1);
    ...
    ASSERT_NOT_NULL(n->properties_json);
    ani_pipeline_importance_append_prop(n, 2.0);
...
TEST(importance_pass_is_idempotent_across_reruns) {
    ...
    int64_t t =
        ani_gbuf_upsert_node(gb, "Function", "target", "pkg.target", "pkg/main.go", 1, 1, "{}");
    int64_t c =
        ani_gbuf_upsert_node(gb, "Function", "caller", "pkg.caller", "pkg/main.go", 2, 2, "{}");
    ani_gbuf_insert_edge(gb, c, t, "CALLS", "{}");

    imp_run_pass(gb);
    char first[ANI_SZ_512];
    snprintf(first, sizeof(first), "%s", ani_gbuf_find_by_id(gb, t)->properties_json);
    imp_run_pass(gb);
    const char *second = ani_gbuf_find_by_id(gb, t)->properties_json;
...
TEST(importance_rehydrated_node_keeps_exactly_one_key) {
    char *tmp = th_mktempdir("ani_imp_rehy");
    if (!tmp) {
        FAIL("tmpdir");
    }
    char root[512];
    ...
    ani_gbuf_t *gb = ani_gbuf_new(project, root);
    ASSERT_NOT_NULL(gb);
    ASSERT_EQ(ani_gbuf_load_from_db(gb, db_path, project), 0);

    /* Precondition: the rehydrated nodes already carry the key. Without this
     * the test could pass while proving nothing. */
    const ani_gbuf_node_t **fns = NULL;
    int fc = 0;
    ASSERT_EQ(ani_gbuf_find_by_label(gb, "Function", &fns, &fc), 0);
    ASSERT_GT(fc, 0);
    for (int i = 0; i < fc; i++) {
...
TEST(importance_second_index_run_keeps_exactly_one_key) {
    char *tmp = th_mktempdir("ani_imp_incr");
    if (!tmp) {
        FAIL("tmpdir");
    }
    char root[512];
    ...
    ani_log_set_sink_ex(imp_pass_sink, ANI_LOG_SINK_TEE);

    ani_pipeline_t *p1 = ani_pipeline_new(root, db_path, ANI_MODE_FULL);
    ASSERT_NOT_NULL(p1);
    ASSERT_EQ(ani_pipeline_run(p1), 0);
    char project[256];
    snprintf(project, sizeof(project), "%s", ani_pipeline_project_name(p1));
    ...
    ani_pipeline_t *p2 = ani_pipeline_new(root, db_path, ANI_MODE_FULL);
    ASSERT_NOT_NULL(p2);
    ...
    ASSERT_EQ((int)route, (int)ANI_INCREMENTAL_ROUTE_LEGACY_PARTIAL);
 ...
 * symbol with 25 real callers. The fix rescores in SQL after ani_delta_patch
 * has re-linked the snapshotted inbound edges. */
TEST(importance_closure_delta_keeps_referenced_symbol_sane) {
    enum { CALLERS = 25 };
    char *tmp = th_mktempdir("ani_imp_delta");
    if (!tmp) {
        FAIL("tmpdir");
    }
    char root[512];
    snprintf(root, sizeof(root), "%s", tmp);
    char *tmp2 = th_mktempdir("ani_imp_deltadb");
    if (!tmp2) {
        th_rmtree(root);
        FAIL("tmpdir2");
    }
    ...
    printf("    route=%d (closure_repair=%d)  sql_rescored_rows=%llu\n", (int)route,
           (int)ANI_INCREMENTAL_ROUTE_CLOSURE_REPAIR, (unsigned long long)sql_rows);
    ASSERT_EQ((int)route, (int)ANI_INCREMENTAL_ROUTE_CLOSURE_REPAIR);
...
TEST(importance_full_and_incremental_converge_on_same_scores) {
    enum { CALLERS = 12 };
    char *tmp = th_mktempdir("ani_imp_equiv");
    if (!tmp) {
        FAIL("tmpdir");
    }
    char root[512];
    snprintf(root, sizeof(root), "%s", tmp);
    char *tmp2 = th_mktempdir("ani_imp_equivdb");
    if (!tmp2) {
        th_rmtree(root);
        FAIL("tmpdir2");
    }
    ...
    ani_pipeline_t *p1 = ani_pipeline_new(root, db_incr, ANI_MODE_FULL);
    ASSERT_NOT_NULL(p1);
    ASSERT_EQ(ani_pipeline_run(p1), 0);
    char project[256];
    snprintf(project, sizeof(project), "%s", ani_pipeline_project_name(p1));
    ...
    uint64_t sql0 = atomic_load_explicit(&g_importance_store_rows, memory_order_relaxed);
    ani_pipeline_t *p2 = ani_pipeline_new(root, db_incr, ANI_MODE_FULL);
    ASSERT_NOT_NULL(p2);
    ASSERT_EQ(ani_pipeline_run(p2), 0);
    ani_incremental_route_t route = ani_pipeline_incremental_test_last_route();
    ani_pipeline_free(p2);
    ...
    ani_pipeline_t *p3 = ani_pipeline_new(root, db_full, ANI_MODE_FULL);
    ASSERT_NOT_NULL(p3);
    ASSERT_EQ(ani_pipeline_run(p3), 0);
    ani_pipeline_free(p3);
...
TEST(importance_distinct_file_count_is_linear_in_group_size) {
    ...
    for (int leg = 0; leg < 2; leg++) {
        ...
        for (int i = 0; i < k; i++) {
            ...
            ASSERT_GT(ani_gbuf_upsert_node(gb, "Method", "toString", qn, file, 1, 1, "{}"), 0);
        }
