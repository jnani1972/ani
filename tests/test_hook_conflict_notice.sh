...
# builds one): ANI_TEST_HOOK_CLIENT_BUILD forces the client fingerprint.
#
...
if ! LC_ALL=C grep -a -q -F 'ANI_TEST_HOOK_CLIENT_BUILD' "${BINARY}"; then
  echo "binary lacks the hook-client build seam: ${BINARY}" >&2
  echo "  rebuild it with:  make -f Makefile.ani ani TEST_SEAMS=1" >&2
...
cleanup() {
  ANI_CACHE_DIR="${tmpdir}/cache" "${BINARY}" daemon stop >/dev/null 2>&1 || true
  ani_test_runtime_cleanup "${BINARY}"
}
trap cleanup EXIT

if ! ANI_CACHE_DIR="${tmpdir}/cache" "${BINARY}" daemon start >"${tmpdir}/daemon-start.log" 2>&1; then
...
for _attempt in $(seq 1 40); do
  ...
  set +e
  out="$(printf '%s' "${payload}" | ANI_CACHE_DIR="${tmpdir}/cache" \
    ANI_TEST_HOOK_CLIENT_BUILD="${forced_build}" \
  ...
  if grep -q "conflicting ANI process" "${tmpdir}/probe.err"; then
    break
  fi
  sleep 0.5
done
...
if ! grep -q "conflicting ANI process" "${tmpdir}/probe.err"; then
  ...
  ANI_CACHE_DIR="${tmpdir}/cache" "${BINARY}" daemon status >&2 2>&1 || true
  exit 2
fi
