 ...
 *
 * The resolver populates result->resolved_calls with ANIResolvedCall edges. Per
 ...
 * QN is `test.main` (from the ani_extract_file "test"/"main.pl" args), so every
...
static ANIFileResult *extract_perl(const char *source) {
    return ani_extract_file(source, (int)strlen(source), ANI_LANG_PERL, "test", "main.pl", 0, NULL,
                            NULL);
}
...
static const ANIResolvedCall *find_resolved_with_strategy(const ANIFileResult *r,
...
TEST(perllsp_method_via_bless_assignment) {
    const char *src = "package Foo;\n"
                      ...
                      "    $obj->bar;\n"
                      "}\n";
    ANIFileResult *r = extract_perl(src);
    ASSERT(r);
...
TEST(perllsp_static_package_call) {
    ...
    /* The same edge is retrievable as a full ANIResolvedCall; static calls carry
     * the perl_static_call strategy. */
    const ANIResolvedCall *rc =
...
TEST(perllsp_cpan_exported_function) {
     ...
     * ani_perl_stdlib_register as "Scalar.Util.blessed". The bare call must
...
TEST(perllsp_repeated_target_calls_join_by_exact_site) {
    ...
    ANIFileResult *r = extract_perl(source);
    ASSERT_NOT_NULL(r);
    ...
    const ANIResolvedCall *first_semantic = NULL;
    const ANIResolvedCall *second_semantic = NULL;
    ...
    const ANIResolvedCall *first_joined =
        ani_pipeline_find_lsp_resolution(&r->resolved_calls, first_call, false);
    const ANIResolvedCall *second_joined =
        ani_pipeline_find_lsp_resolution(&r->resolved_calls, second_call, false);
...
static int assert_perl_repeated_exact_join(const char *source, const char *caller_fragment,
    ...
    ASSERT_TRUE(ani_pipeline_find_lsp_resolution(&r->resolved_calls, calls[0], false) ==
                semantics[0]);
    ASSERT_TRUE(ani_pipeline_find_lsp_resolution(&r->resolved_calls, calls[1], false) ==
                semantics[1]);
