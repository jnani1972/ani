...
static int run_configlink(ani_gbuf_t *gb, const char *project, const char *repo_path) {
    ...
    return ani_pipeline_pass_configlink(&ctx);
}
#include <sys/stat.h>
#include <unistd.h>
...
static bool has_strategy(const ani_gbuf_edge_t **edges, int count, const char *strategy) {
    char needle[64];
...
static bool has_strategy_with_confidence(const ani_gbuf_edge_t **edges, int count,
...
static bool has_strategy_with_key(const ani_gbuf_edge_t **edges, int count, const char *strategy,
...
TEST(configlink_key_symbol_exact_match) {
    ani_gbuf_t *gb = ani_gbuf_new("test", "/tmp/test");

    /* Config Variable node: max_connections from config.toml */
    int64_t cfg_id = ani_gbuf_upsert_node(gb, "Variable", "max_connections",
                                          "test.config.max_connections", "config.toml", 0, 0, NULL);
    ASSERT_GT(cfg_id, 0);

    /* Code Function node: getMaxConnections from main.go */
    int64_t func_id = ani_gbuf_upsert_node(gb, "Function", "getMaxConnections",
    ...
    int count = 0;
    ani_gbuf_find_edges_by_type(gb, "CONFIGURES", &edges, &count);
...
TEST(configlink_key_symbol_substring_match) {
    ...
    ani_gbuf_upsert_node(gb, "Variable", "request_timeout", "test.config.request_timeout",
                         "config.toml", 0, 0, NULL);

    ani_gbuf_upsert_node(gb, "Function", "getRequestTimeoutSeconds",
...
TEST(configlink_key_symbol_short_key_skipped) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        ani_gbuf_upsert_node(gb, "Variable", short_keys[i], qn, "config.toml", 0, 0, NULL);
    }

    ani_gbuf_upsert_node(gb, "Function", "getPort", "test.main.getPort", "main.go", 0, 0, NULL);
...
TEST(configlink_key_symbol_camel_case) {
    ...
    ani_gbuf_upsert_node(gb, "Variable", "maxRetryCount", "test.config.maxRetryCount",
                         "config.json", 0, 0, NULL);

    ani_gbuf_upsert_node(gb, "Function", "getMaxRetryCount", "test.handler.getMaxRetryCount",
                         "handler.go", 0, 0, NULL);
...
TEST(configlink_key_symbol_no_false_positive) {
    ...
    ani_gbuf_upsert_node(gb, "Variable", "url", "test.db.url", "config.toml", 0, 0, NULL);

    ani_gbuf_upsert_node(gb, "Function", "parseURL", "test.main.parseURL", "main.go", 0, 0, NULL);
...
TEST(configlink_dep_import_package_json) {
    ...
    ani_gbuf_upsert_node(gb, "Variable", "express", "test.package.dependencies.express",
                         "package.json", 0, 0, NULL);

    /* Module node for app.js (source of import) */
    int64_t app_id = ani_gbuf_upsert_node(gb, "Module", "app", "test.app", "app.js", 0, 0, NULL);

    /* Import target node (what 'express' resolves to) */
    int64_t target_id = ani_gbuf_upsert_node(gb, "Module", "express", "test.node_modules.express",
                                             "node_modules/express/index.js", 0, 0, NULL);

    /* IMPORTS edge: app.js → express module */
    ani_gbuf_insert_edge(gb, app_id, target_id, "IMPORTS", NULL);
...
TEST(configlink_file_ref_no_false_positive) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/ani_cfglink_XXXXXX");
    ...
    char *project = ani_project_name_from_path(tmpdir);
    ani_gbuf_t *gb = ani_gbuf_new(project, tmpdir);

    /* data.csv Module (NOT a config extension) */
    char *csv_qn = ani_pipeline_fqn_module(project, "data.csv");
    ani_gbuf_upsert_node(gb, "Module", "data", csv_qn, "data.csv", 0, 0, NULL);

    /* Source Module */
    char *main_qn = ani_pipeline_fqn_module(project, "main.go");
    ani_gbuf_upsert_node(gb, "Module", "main", main_qn, "main.go", 0, 0, NULL);
...
static char *configures_fingerprint(ani_gbuf_t *gb) {
    const ani_gbuf_edge_t **edges = NULL;
    int count = 0;
    ani_gbuf_find_edges_by_type(gb, "CONFIGURES", &edges, &count);
    ...
    int n = 0;
    for (int i = 0; i < count; i++) {
        const ani_gbuf_node_t *s = ani_gbuf_find_by_id(gb, edges[i]->source_id);
        const ani_gbuf_node_t *t = ani_gbuf_find_by_id(gb, edges[i]->target_id);
...
static ani_gbuf_t *build_oversized_corpus(bool reverse) {
    ani_gbuf_t *gb = ani_gbuf_new("test", "/tmp/test");

    /* One config key every candidate can match on: "max_connections". */
    ani_gbuf_upsert_node(gb, "Variable", "max_connections", "test.config.max_connections",
                         "config.toml", 0, 0, NULL);
    ...
    for (int k = 0; k < CL_DET_CANDIDATES; k++) {
        ...
        ani_gbuf_upsert_node(gb, "Function", name, qn, path, 0, 0, NULL);
    }
    return gb;
}
...
TEST(configlink_candidate_truncation_is_order_independent) {
    ani_gbuf_t *fwd = build_oversized_corpus(false);
    run_configlink(fwd, "test", NULL);
    char *fp_fwd = configures_fingerprint(fwd);

    ani_gbuf_t *rev = build_oversized_corpus(true);
