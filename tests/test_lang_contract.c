...
/* Normalize backslashes to forward slashes in place. ani_mkdtemp on Windows
...
static ani_store_t *lang_open_indexed(LangProj *lp) {
    ...
    char cache_dir[512];
    const char *configured_cache = getenv("ANI_CACHE_DIR");
...
static ani_store_t *lang_index_files(LangProj *lp, const LangFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/ani_lc_XXXXXX");
...
static ani_store_t *lang_index(LangProj *lp, const char *filename, const char *content) {
    LangFile f = {filename, content};
    return lang_index_files(lp, &f, 1);
}

static void lang_cleanup(LangProj *lp, ani_store_t *store) {
    if (store) {
...
static int label_with_outbound(ani_store_t *store, const char *project, const char *label) {
    ani_search_params_t p = {0};
    p.project = project;
    p.label = label;
    p.min_degree = 1;
    p.max_degree = -1;
    ...
    int n = -1;
    if (ani_store_search(store, &p, &out) == ANI_STORE_OK) {
        n = out.count;
    }
...
static int callables_with_outbound(ani_store_t *store, const char *project) {
...
static int count_label(ani_store_t *store, const char *project, const char *label) {
...
static int type_like_nodes(ani_store_t *store, const char *project) {
...
/* Run ani_extract_file in a forked child; return true if the child died from a
...
static bool extract_crashes(const char *content, ANILanguage lang, const char *relpath) {
#if defined(_WIN32)
    ANIFileResult *r =
        ani_extract_file(content, (int)strlen(content), lang, "lc", relpath, 0, NULL, NULL);
    if (r) {
        ani_free_result(r);
    }
    return false;
 ...
 * This probe asserts ani_extract_file captures them; it is the precise, reliable
...
TEST(contract_kotlin_imports_extracted) {
    ANIFileResult *r =
        ani_extract_file(KT_SRC, (int)strlen(KT_SRC), ANI_LANG_KOTLIN, "lc", "a.kt", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
...
TEST(contract_c_calls_attributed_to_function) {
    LangProj lp;
    ani_store_t *store = lang_index(&lp, "a.c", C_SRC);
...
TEST(contract_java_extract_no_crash) {
    bool crashed = extract_crashes(JAVA_SRC, ANI_LANG_JAVA, "Tracker.java");
...
static LangMetrics lang_metrics(const LangFile *files, int nfiles) {
    LangProj lp;
    ani_store_t *store = lang_index_files(&lp, files, nfiles);
    LangMetrics m = {0};
    if (store) {
        m.ok = 1;
        m.calls = ani_store_count_edges_by_type(store, lp.project, "CALLS");
...
TEST(contract_python_aliased_from_import_calls) {
    ...
    LangProj lp;
    ani_store_t *store = lang_index_files(&lp, f, 2);
    ASSERT_TRUE(store != NULL);
    ani_node_t *nodes = NULL;
    int ncount = 0;
    ASSERT_EQ(ani_store_find_nodes_by_name(store, lp.project, "submit_task", &nodes, &ncount),
              ANI_STORE_OK);
    ...
    int n_callers = 0;
    int n_callees = 0;
    ASSERT_EQ(ani_store_node_neighbor_names(store, nodes[0].id, 32, &callers, &n_callers, &callees,
                                            &n_callees),
              0);
    int saw_execute = 0;
...
TEST(contract_python_absolute_aliased_from_import_calls) {
    ...
    LangProj lp;
    ani_store_t *store = lang_index_files(&lp, f, 5);
    ASSERT_TRUE(store != NULL);
    ani_node_t *nodes = NULL;
    int ncount = 0;
    ASSERT_EQ(ani_store_find_nodes_by_name(store, lp.project, "submit_task", &nodes, &ncount),
...
TEST(contract_python_ghost_module_aliased_from_import_no_calls) {
    static const LangFile f[] = {
        {"router.py",
         ...
         "def submit_task(y):\n    return alias(y)\n"}};
    LangProj lp;
    ani_store_t *store = lang_index_files(&lp, f, 1);
    ASSERT_TRUE(store != NULL);
    ani_node_t *nodes = NULL;
    int ncount = 0;
    ASSERT_EQ(ani_store_find_nodes_by_name(store, lp.project, "submit_task", &nodes, &ncount),
...
static int def_nodes_for_file(ani_store_t *store, const char *project, const char *rel) {
    ani_node_t *nodes = NULL;
    int count = 0;
    if (ani_store_find_nodes_by_file(store, project, rel, &nodes, &count) != ANI_STORE_OK) {
        return -1;
    }
    int defs = 0;
...
static void breadth_diag(ani_store_t *store, const char *project, const char *rel,
                         const GrammarCase *c) {
    ANIFileResult *dr =
        ani_extract_file(c->src, (int)strlen(c->src), c->lang, "lc", c->path, 0, NULL, NULL);
 ...
 * registry (ani_label_is_registry_symbol). Chialisp's condition codes,
 ...
 * the registry, they are seeded nowhere, ani_registry_resolve returns nothing,
...
TEST(contract_chialisp_constant_resolves_across_files) {
    ...
    ani_node_t *consts = NULL;
    int nconst = 0;
    ASSERT_EQ(ani_store_find_nodes_by_name(store, lp.project, "CREATE_COIN", &consts, &nconst),
              ANI_STORE_OK);
    ...
    int nedges = 0;
    ASSERT_EQ(ani_store_find_edges_by_target_type(store, consts[0].id, "USAGE", &edges, &nedges),
              ANI_STORE_OK);
    int cross_file = 0;
    for (int i = 0; i < nedges; i++) {
        ani_node_t src;
        if (ani_store_find_node_by_id(store, edges[i].source_id, &src) != ANI_STORE_OK) {
            continue;
        }
    ...
    ani_store_free_edges(edges, nedges);
    ani_store_free_nodes(consts, nconst);
    lang_cleanup(&lp, store);
...
TEST(contract_all_grammars_in_graph) {
    int n = (int)ANI_GRAMMAR_CASES_COUNT;
    ...
    for (int i = 0; i < n; i++) {
        snprintf(names[i], sizeof(names[i]), "g%03d/%s", i, ANI_GRAMMAR_CASES[i].path);
        files[i].name = names[i];
        files[i].content = ANI_GRAMMAR_CASES[i].src;
    }

    LangProj lp;
    ani_store_t *store = lang_index_files(&lp, files, n);
    int total_nodes = store ? ani_store_count_nodes(store, lp.project) : -1;
...
static void dump_edge_histogram(ani_store_t *store, const char *project) {
    if (!store) {
    ...
    for (int i = 0; ALL_EDGE_TYPES[i]; i++) {
        int c = ani_store_count_edges_by_type(store, project, ALL_EDGE_TYPES[i]);
...
static int edge_present(const LangFile *files, int nfiles, const char *edge, int floor) {
    LangProj lp;
    ani_store_t *store = lang_index_files(&lp, files, nfiles);
    int got = store ? ani_store_count_edges_by_type(store, lp.project, edge) : -1;
    if (got < floor) {
...
TEST(contract_edge_imports_alias_no_phantom_folder_edge_issue767) {
    ...
    ani_store_t *store = lang_index_files(&lp, f, 3);
    int got = store ? ani_store_count_edges_by_type(store, lp.project, "IMPORTS") : -1;
    if (got != 0) {
...
static int calls_edge_between(ani_store_t *store, const char *project, const char *src_suffix,
    ...
    for (int i = 0; i < n && !found; i++) {
        ani_node_t s, t;
        if (ani_store_find_node_by_id(store, edges[i].source_id, &s) != ANI_STORE_OK)
            continue;
        if (ani_store_find_node_by_id(store, edges[i].target_id, &t) != ANI_STORE_OK) {
            ani_node_free_fields(&s);
            continue;
        }
 ...
 * pallets/flask). */
static int count_infra_routes_matching(ani_store_t *store, const char *project,
                                       const char *substr) {
    ani_node_t *nodes = NULL;
    int count = 0;
    if (ani_store_find_nodes_by_label(store, project, "Route", &nodes, &count) != ANI_STORE_OK)
        return -1;
    int hits = 0;
    for (int i = 0; i < count; i++) {
...
static int calls_edge_targets(ani_store_t *store, const char *project, const char *label,
    ...
    for (int i = 0; i < n && !found; i++) {
        ani_node_t tgt;
        if (ani_store_find_node_by_id(store, edges[i].target_id, &tgt) != ANI_STORE_OK)
            continue;
...
TEST(contract_edge_python_aliased_import_call_resolves_issue988) {
    ...
    static const LangFile f[] = {{"m.py", "def f(x):\n    return x + 1\n"},
                                 ...
                                 {"caller_dotalias.py", "import pkg.dm as dz\n"
                                                        ...
                                                        "def use_dotalias(x):\n"
                                                        "    return dz.h(x)\n"}};
    ani_store_t *store = lang_index_files(&lp, f, 7);
    ASSERT_TRUE(store != NULL);
...
static ani_store_t *index_parallel_fixture(LangProj *lp, const LangFile *meaningful, int n_mean) {
...
TEST(contract_edge_parallel_service_edges) {
    ...
    int graphql = store ? ani_store_count_edges_by_type(store, lp.project, "GRAPHQL_CALLS") : -1;
    int grpc = store ? ani_store_count_edges_by_type(store, lp.project, "GRPC_CALLS") : -1;
    int trpc = store ? ani_store_count_edges_by_type(store, lp.project, "TRPC_CALLS") : -1;
    int infra = store ? ani_store_count_edges_by_type(store, lp.project, "INFRA_MAPS") : -1;
    int invalid_props = -1;
    if (store) {
...
TEST(contract_edge_file_changes_with) {
    LangProj lp;
    memset(&lp, 0, sizeof(lp));
    snprintf(lp.tmpdir, sizeof(lp.tmpdir), "/tmp/ani_fcw_XXXXXX");
    ASSERT_NOT_NULL(ani_mkdtemp(lp.tmpdir));
    lc_to_fwd_slashes(lp.tmpdir);
    ...
    ani_store_t *store = lang_open_indexed(&lp);
    int fcw = store ? ani_store_count_edges_by_type(store, lp.project, "FILE_CHANGES_WITH") : -1;
    if (fcw < 1) {
        dump_edge_histogram(store, lp.project);
    }
    lang_cleanup(&lp, store);
