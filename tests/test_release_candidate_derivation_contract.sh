...
FIX="$(mktemp -d "${TMPDIR:-/tmp}/ani-candidate-derive.XXXXXX")"
trap 'rm -rf "$FIX"' EXIT
...
static const char release_canary[] =
    "ani OMIT_LOAD_EXTENSION candidate-derivation-contract";
...
int main(int argc, char **argv) {
    const char *sentinel = getenv("ANI_CANDIDATE_EXEC_SENTINEL");
    if (sentinel) {
    ...
    if (argc == 2 && strcmp(argv[1], "--version") == 0) {
        puts("ani 0.0.0-candidate-contract");
        return 0;
    }
    puts(release_canary);
...
OUT="$FIX/published"
env ANI_CANDIDATE_EXEC_SENTINEL="$FIX/executed-before-selection" \
...
if not lines or lines[0] != "# ani-release-candidate-provenance-v1":
