...
FIX="$(mktemp -d "${TMPDIR:-/tmp}/ani-mcpb-entries.XXXXXX")"
trap 'rm -rf "$FIX"' EXIT
...
def fresh_fixture():
    ...
    (fix / "checksums.txt").write_text(
        f"{SHA_B}  ani-windows-amd64.zip\n"
        f"{SHA_B}  ani-windows-amd64.mcpb\n"
        f"{SHA_A}  ani-darwin-arm64.mcpb\n",
        encoding="utf-8",
    )
...
if len(mcpb) != 2:
    ...
    first = mcpb[0]
    wanted_url = ("https://github.com/jnani1972/ani/releases/"
                  "download/v9.9.9/ani-darwin-arm64.mcpb")
...
with (fix / "checksums.txt").open("a", encoding="utf-8") as handle:
    handle.write("deadbeef  ani-darwin-amd64.mcpb\n")
result = run()
if result.returncode == 0:
