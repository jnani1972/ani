...
static ANIFileResult *extract_php(const char *source) {
    return ani_extract_file(source, (int)strlen(source), ANI_LANG_PHP, "test", "main.php", 0, NULL,
                            NULL);
}
...
TEST(phplsp_local_method_via_new_assignment) {
    const char *src =
        ...
        "    }\n"
        "}\n";
    ANIFileResult *r = extract_php(src);
    ASSERT(r);
...
TEST(phplsp_unindexed_receiver_emits_block) {
    ...
    if (!rc) {
        printf("  expected php_method_typed_unindexed for Foo.value, got %d entries\n",
               r->resolved_calls.count);
        for (int i = 0; i < r->resolved_calls.count; i++) {
            const ANIResolvedCall *rcd = &r->resolved_calls.items[i];
...
TEST(phplsp_regression_prompt_value_not_routed_to_helper) {
    ...
    ANIFileResult *r = extract_php(src);
    ASSERT(r);
    const ANIResolvedCall *rc = find_resolved_with_strategy(r, "C.configure", "Prompt.value",
 ...
 * re-matched the loop filter while ani_registry_add_func() grew the
...
TEST(phplsp_ordinary_same_leaf_calls_join_by_exact_site) {
    ...
    ANIFileResult *r = extract_php(source);
    ASSERT_NOT_NULL(r);
