...
static bool diagnostics_capture_paths(diagnostics_paths_t *paths) {
    return ani_diag_test_copy_paths(paths->directory, sizeof(paths->directory), paths->snapshot,
...
static void diagnostics_cleanup_outputs(const diagnostics_paths_t *paths) {
    ...
    if (snprintf(temporary, sizeof(temporary), "%s.tmp", paths->snapshot) > 0) {
        (void)ani_unlink(temporary);
    }
    if (snprintf(rotated, sizeof(rotated), "%s.1", paths->trajectory) > 0) {
        (void)ani_unlink(rotated);
    }
    (void)ani_unlink(paths->snapshot);
    (void)ani_unlink(paths->trajectory);
    (void)ani_rmdir(paths->directory);
}
...
TEST(diagnostics_rejects_predictable_tmp_symlinks) {
    ...
    ASSERT_LT(
        snprintf(snapshot_link, sizeof(snapshot_link), "/tmp/ani-diagnostics-%d.json.tmp", pid),
        (int)sizeof(snapshot_link));
    ASSERT_LT(
        snprintf(trajectory_link, sizeof(trajectory_link), "/tmp/ani-diagnostics-%d.ndjson", pid),
        (int)sizeof(trajectory_link));
    ASSERT_LT(snprintf(snapshot_target, sizeof(snapshot_target),
                       "/tmp/ani-diagnostics-snapshot-sentinel-%d", pid),
              (int)sizeof(snapshot_target));
    ASSERT_LT(snprintf(trajectory_target, sizeof(trajectory_target),
                       "/tmp/ani-diagnostics-trajectory-sentinel-%d", pid),
              (int)sizeof(trajectory_target));

    (void)ani_unlink(snapshot_link);
    (void)ani_unlink(trajectory_link);
    (void)ani_unlink(snapshot_target);
    (void)ani_unlink(trajectory_target);

    FILE *snapshot = ani_fopen(snapshot_target, "wb");
    ASSERT_NOT_NULL(snapshot);
    ASSERT_GTE(fputs(snapshot_seed, snapshot), 0);
    ASSERT_EQ(fclose(snapshot), 0);
    FILE *trajectory = ani_fopen(trajectory_target, "wb");
    ...
    ASSERT_EQ(ani_setenv("ANI_DIAGNOSTICS", "1", 1), 0);
    ASSERT_TRUE(ani_diag_start());
    ...
    ani_diag_stop();
    ASSERT_EQ(ani_unsetenv("ANI_DIAGNOSTICS"), 0);
...
TEST(diagnostics_stop_interrupts_periodic_wait) {
    ...
    int pid = (int)getpid();
    ASSERT_LT(snprintf(legacy_snapshot, sizeof(legacy_snapshot), "%s/ani-diagnostics-%d.json",
                       ani_tmpdir(), pid),
              (int)sizeof(legacy_snapshot));
    ASSERT_LT(snprintf(legacy_trajectory, sizeof(legacy_trajectory), "%s/ani-diagnostics-%d.ndjson",
                       ani_tmpdir(), pid),
              (int)sizeof(legacy_trajectory));
    (void)ani_unlink(legacy_snapshot);
    (void)ani_unlink(legacy_trajectory);
    ...
    bool emitted = captured && diagnostics_wait_for_file(outputs.snapshot, 1000);
    uint64_t started_ms = ani_now_ms();
    ani_diag_stop();
    uint64_t elapsed_ms = ani_now_ms() - started_ms;
    ASSERT_EQ(ani_unsetenv("ANI_DIAGNOSTICS"), 0);
    (void)ani_unlink(legacy_snapshot);
    (void)ani_unlink(legacy_trajectory);
    if (captured) {
...
TEST(diagnostics_stalled_writer_cannot_retain_shutdown) {
    ani_diag_test_hold_writer(true);
    ASSERT_EQ(ani_setenv("ANI_DIAGNOSTICS", "1", 1), 0);
    if (!ani_diag_start()) {
        ani_diag_test_hold_writer(false);
        (void)ani_unsetenv("ANI_DIAGNOSTICS");
        FAIL("failed to start held diagnostics writer");
    }
    diagnostics_paths_t outputs;
    bool captured = diagnostics_capture_paths(&outputs);
    uint64_t reach_deadline = ani_now_ms() + 1000;
    while (!ani_diag_test_writer_reached() && ani_now_ms() < reach_deadline) {
        diagnostics_wait_ms(10);
    }
    bool reached = ani_diag_test_writer_reached();
    if (!captured || !reached) {
        ani_diag_test_hold_writer(false);
        ani_diag_stop();
        (void)ani_unsetenv("ANI_DIAGNOSTICS");
    ...
    ani_diag_stop();
    uint64_t elapsed_ms = ani_now_ms() - started_ms;
    bool abandoned = ani_diag_test_abandoned();

    ani_diag_test_hold_writer(false);
    bool reset = false;
    uint64_t reset_deadline = ani_now_ms() + 1000;
    while (!reset && ani_now_ms() < reset_deadline) {
        reset = ani_diag_test_reset_abandoned();
        if (!reset) {
            diagnostics_wait_ms(10);
        }
    }
...
TEST(diagnostics_soak_discovers_daemon_emitted_paths) {
    ...
    bool isolates_daemon = strstr(script, "ANI_CACHE_DIR=\"$SOAK_CACHE_DIR_VALUE\"") != NULL;
    bool legacy_predictable_path = strstr(script, "/tmp/ani-diagnostics-${SERVER_PID}") != NULL;
    free(script);
...
TEST(diagnostics_discovery_record_survives_suppressed_log_level) {
    ...
    ani_log_set_level(ANI_LOG_NONE);
    ani_log_set_sink_ex(diagnostics_discovery_sink, ANI_LOG_SINK_REPLACE);
    ASSERT_EQ(ani_setenv("ANI_DIAGNOSTICS", "1", 1), 0);
    bool started = ani_diag_start();
    diagnostics_paths_t outputs;
    ...
    ASSERT_TRUE(started);
    ASSERT_TRUE(captured);
    /* Exactly one discovery record, delivered despite ANI_LOG_LEVEL=none, and
...
TEST(diagnostics_placement_honors_tmpdir_with_spaces) {
    ...
    char base[256];
    ASSERT_LT(snprintf(base, sizeof(base), "/tmp/ani diag spaces %d", (int)getpid()),
              (int)sizeof(base));
    (void)ani_rmdir(base);
    ASSERT_EQ(mkdir(base, 0700), 0);
    ASSERT_EQ(ani_setenv("TMPDIR", base, 1), 0);
    diagnostics_discovery_records = 0;
    diagnostics_discovery_line[0] = '\0';
    ani_log_set_sink_ex(diagnostics_discovery_sink, ANI_LOG_SINK_REPLACE);
    ASSERT_EQ(ani_setenv("ANI_DIAGNOSTICS", "1", 1), 0);
    ...
    ani_log_set_sink(NULL);
    if (had_tmpdir) {
        (void)ani_setenv("TMPDIR", saved_tmpdir, 1);
    } else {
        (void)ani_unsetenv("TMPDIR");
    }
 ...
 * timing anything. A test that tried to prove the detector works by generating
 * a genuinely quadratic workload would be asserting on the scheduler, and would
 * go flaky on a loaded CI box — so the fit is a pure function and gets fed
 * synthetic points instead. */

TEST(scale_fit_k_recognises_linear_and_quadratic_growth) {
    /* 8x the items, 8x the time => k = 1 */
    ASSERT_TRUE(ani_scale_fit_k(1000, 1000, 8000, 8000) > 0.99);
    ASSERT_TRUE(ani_scale_fit_k(1000, 1000, 8000, 8000) < 1.01);

    /* 8x the items, 64x the time => k = 2 */
    ASSERT_TRUE(ani_scale_fit_k(1000, 1000, 8000, 64000) > 1.99);
    ASSERT_TRUE(ani_scale_fit_k(1000, 1000, 8000, 64000) < 2.01);

    /* n log n sits between, and must NOT be mistaken for quadratic */
    double k_nlogn = ani_scale_fit_k(1000, 1000, 8000, 8000 * 13 / 10);
    ASSERT_TRUE(k_nlogn > 1.0);
    ASSERT_TRUE(k_nlogn < ANI_SCALE_WARN_K);
    PASS();
}

TEST(scale_fit_k_rejects_degenerate_input) {
    ASSERT_TRUE(ani_scale_fit_k(0, 1000, 8000, 8000) < 0.0);    /* no first point */
    ASSERT_TRUE(ani_scale_fit_k(1000, 0, 8000, 8000) < 0.0);    /* zero elapsed */
    ASSERT_TRUE(ani_scale_fit_k(1000, 1000, 1000, 8000) < 0.0); /* n did not grow */
    ASSERT_TRUE(ani_scale_fit_k(1000, 1000, 500, 8000) < 0.0);  /* n went backwards */
    ASSERT_TRUE(ani_scale_fit_k(1000, 1000, 8000, 0) < 0.0);    /* zero total */
    PASS();
}

TEST(scale_probe_records_checkpoints_at_eighths) {
    ani_scale_probe_t p;
    ani_scale_begin(&p, "unit", 1024);
...
TEST(scale_probe_ignores_runs_too_small_to_judge) {
    ...
    ani_scale_probe_t p;
    ani_scale_begin(&p, "tiny", 64);
    for (long i = 0; i <= 64; i++) {
        ani_scale_tick(&p, i);
    }
    ASSERT_EQ(atomic_load(&p.next_cp), 0);
    ani_scale_end(&p); /* must not emit, must not crash */
    PASS();
}
