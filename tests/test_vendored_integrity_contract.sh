...
FIXTURE="$(mktemp -d "${TMPDIR:-/tmp}/ani-vendored-integrity.XXXXXX")"
...
mkdir -p "$grammar/internal/ani/vendored/grammars/example"
printf '%s\n' 'int tree_sitter_example(void) { return 1; }' \
  > "$grammar/internal/ani/vendored/grammars/example/parser.c"
