...
#include "../src/foundation/compat.h" /* ani_setenv / ani_unsetenv (Windows-portable) */
...
/* Worker staging files land under ANI_CACHE_DIR, which users may place at
...
TEST(platform_file_apis_survive_max_path_overflow) {
    char base[ANI_SZ_512];
    int written = snprintf(base, sizeof(base), "/tmp/ani-longpath-XXXXXX");
    ASSERT_TRUE(written > 0 && written < (int)sizeof(base));
    ASSERT_NOT_NULL(ani_mkdtemp(base));
    ...
    ASSERT_TRUE(ani_mkdir_p(deep, 0700));

    char file_path[ANI_SZ_1K];
    written = snprintf(file_path, sizeof(file_path), "%s/store.db", deep);
    ASSERT_TRUE(written > 0 && written < (int)sizeof(file_path));
    FILE *file = ani_fopen(file_path, "wb");
    ASSERT_NOT_NULL(file);
    ASSERT_GTE(fputs("deep\n", file), 0);
    ASSERT_EQ(fclose(file), 0);
    ASSERT_TRUE(ani_file_exists(file_path));
    ASSERT_TRUE(ani_is_dir(deep));
    ASSERT_EQ(ani_unlink(file_path), 0);
    char cursor[ANI_SZ_1K];
    (void)snprintf(cursor, sizeof(cursor), "%s", deep);
    for (int index = 0; index < LONG_SEGMENTS; index++) {
        ASSERT_EQ(ani_rmdir(cursor), 0);
...
TEST(platform_mkstemp_and_mkdtemp_survive_non_ascii_directory) {
    char base[ANI_SZ_256];
    int written = snprintf(base, sizeof(base), "/tmp/ani-utf8-̩̬-XXXXXX");
    ASSERT_TRUE(written > 0 && written < (int)sizeof(base));
    ASSERT_NOT_NULL(ani_mkdtemp(base));

    char file_template[ANI_SZ_512];
    ...
    int descriptor = ani_mkstemp(file_template);
...
typedef struct {
    ...
    int created;
    bool ok;
    char paths[PLATFORM_MKDTEMP_ITERATIONS][ANI_SZ_512];
    ...
    for (int index = 0; index < PLATFORM_MKDTEMP_ITERATIONS; index++) {
        int written =
            snprintf(worker->paths[index], sizeof(worker->paths[index]),
                     "/tmp/ani-mkdtemp-concurrent-%d-%d-XXXXXX", worker->worker_index, index);
        if (written <= 0 || written >= (int)sizeof(worker->paths[index]) ||
            !ani_mkdtemp(worker->paths[index])) {
            worker->ok = false;
            break;
        }
        worker->created++;
...
/* MCP daemon sessions can enter ani_mkdtemp simultaneously. Every request must
 * retain its own expanded path and create a distinct directory. */
TEST(platform_mkdtemp_is_thread_safe) {
    ani_thread_t threads[PLATFORM_MKDTEMP_THREADS];
    ...
    for (; started < PLATFORM_MKDTEMP_THREADS; started++) {
        ...
        if (ani_thread_create(&threads[started], 0, platform_mkdtemp_concurrent_worker,
                              &workers[started]) != 0) {
            break;
        }
    }
    ...
    for (int index = 0; index < started; index++) {
        ...
        for (int path_index = 0; path_index < workers[index].created; path_index++) {
            all_ok = all_ok && ani_is_dir(workers[index].paths[path_index]);
            (void)ani_rmdir(workers[index].paths[path_index]);
        }
    }
    ASSERT_TRUE(all_ok);
    PASS();
...
TEST(platform_counter_scaling_avoids_intermediate_overflow) {
    ...
    ASSERT(ani_platform_scale_counter_ns(counter, frequency) == UINT64_C(2000000000000));
    ASSERT(ani_platform_scale_counter_ns(UINT64_MAX, UINT64_MAX) == UINT64_C(1000000000));
    ASSERT(ani_platform_scale_counter_ns(UINT64_MAX, 1) == UINT64_MAX);
    PASS();
}
...
TEST(platform_counter_scaling_preserves_monotonic_deadlines) {
    ...
    const uint64_t now_ns = ani_platform_scale_counter_ns(counter, frequency);
    const uint64_t next_ns = ani_platform_scale_counter_ns(counter + 1, frequency);
    const uint64_t deadline_ns = ani_platform_scale_counter_ns(counter + 5 * frequency, frequency);
...
static void *platform_time_first_call(void *opaque) {
    ...
    worker->value = ani_now_ns();
    return NULL;
}

TEST(platform_now_ns_concurrent_first_call) {
    ani_thread_t threads[PLATFORM_TIME_THREADS];
    ...
    for (; created < PLATFORM_TIME_THREADS; created++) {
        ...
        if (ani_thread_create(&threads[created], 0, platform_time_first_call, &workers[created]) !=
            0) {
            break;
        }
    }
...
TEST(platform_now_ns) {
    uint64_t t1 = ani_now_ns();
    ASSERT_GT(t1, 0);
    /* Busy-wait a tiny bit */
    for (volatile int i = 0; i < 100000; i++) {}
    uint64_t t2 = ani_now_ns();
    ASSERT_GT(t2, t1);
    PASS();
}

TEST(platform_now_ms) {
    uint64_t t1 = ani_now_ms();
    ASSERT_GT(t1, 0);
    PASS();
}
...
TEST(platform_file_exists) {
    ...
    ASSERT_TRUE(ani_file_exists("tests/test_platform.c"));
    ASSERT_FALSE(ani_file_exists("nonexistent_file_xyz.txt"));
    PASS();
}

TEST(platform_is_dir) {
    ASSERT_TRUE(ani_is_dir("tests"));
    ASSERT_FALSE(ani_is_dir("tests/test_platform.c"));
    ASSERT_FALSE(ani_is_dir("nonexistent_dir"));
    PASS();
}

TEST(platform_file_size) {
    int64_t sz = ani_file_size("tests/test_platform.c");
    ASSERT_GT(sz, 0);
    ASSERT_EQ(ani_file_size("nonexistent_file_xyz.txt"), -1);
    PASS();
}
...
TEST(platform_mmap) {
    ...
    size_t sz = 0;
    void *data = ani_mmap_read("tests/test_platform.c", &sz);
...
TEST(platform_mmap_nonexistent) {
    size_t sz = 0;
    void *data = ani_mmap_read("nonexistent_xyz.txt", &sz);
    ASSERT_NULL(data);
    PASS();
}
...
static void *platform_capture_path_buffers(void *opaque) {
    ...
    result->home = ani_get_home_dir();
    result->cache = ani_resolve_cache_dir();
...
TEST(platform_path_helpers_use_per_thread_storage) {
    ...
    ASSERT_EQ(ani_setenv("HOME", "/tmp/ani-platform-thread-home", 1), 0);
    ASSERT_EQ(ani_setenv("ANI_CACHE_DIR", "/tmp/ani-platform-thread-cache", 1), 0);
    ...
    bool started0 =
        ani_thread_create(&threads[0], 0, platform_capture_path_buffers, &results[0]) == 0;
    bool started1 =
        ani_thread_create(&threads[1], 0, platform_capture_path_buffers, &results[1]) == 0;
    ...
    if (saved_home_copy) {
        (void)ani_setenv("HOME", saved_home_copy, 1);
    } else {
        (void)ani_unsetenv("HOME");
    }
    if (saved_cache_copy) {
        (void)ani_setenv("ANI_CACHE_DIR", saved_cache_copy, 1);
    } else {
...
TEST(platform_cache_dir_rejects_truncated_override) {
    ...
    value[length] = '\0';
    ASSERT_EQ(ani_setenv("ANI_CACHE_DIR", value, 1), 0);

    const char *resolved = ani_resolve_cache_dir();

    if (saved_copy) {
        (void)ani_setenv("ANI_CACHE_DIR", saved_copy, 1);
    } else {
        (void)ani_unsetenv("ANI_CACHE_DIR");
    }
    free(saved_copy);
...
#ifdef _WIN32
/* ani_safe_getenv reads Windows' wide environment as UTF-8. Its matching
...
TEST(platform_setenv_preserves_utf8_in_wide_environment) {
    static const char utf8[] = "C:/ani-cache-\xce\x94-\xe6\x97\xa5\xe6\x9c\xac";
    static const wchar_t wide[] = L"C:/ani-cache-\u0394-\u65e5\u672c";
    ASSERT_EQ(ani_setenv("ANI_CACHE_DIR", utf8, 1), 0);
    wchar_t observed_wide[128];
    ASSERT_EQ(GetEnvironmentVariableW(L"ANI_CACHE_DIR", observed_wide, 128), (DWORD)(wcslen(wide)));
    ASSERT_EQ(wcscmp(observed_wide, wide), 0);
    char observed_utf8[128];
    ASSERT_NOT_NULL(ani_safe_getenv("ANI_CACHE_DIR", observed_utf8, sizeof(observed_utf8), NULL));
 ...
 * particular, an explicitly empty ANI_CACHE_DIR means "use the default"; it
...
TEST(platform_windows_empty_environment_is_read_and_unset_idempotently) {
    ASSERT_EQ(ani_setenv("ANI_CACHE_DIR", "", 1), 0);
    char observed[9] = "sentinel";
    ASSERT_NOT_NULL(ani_safe_getenv("ANI_CACHE_DIR", observed, sizeof(observed), "fallback"));
    ASSERT_STR_EQ(observed, "");
    ASSERT_EQ(ani_unsetenv("ANI_CACHE_DIR"), 0);
    ASSERT_EQ(ani_unsetenv("ANI_CACHE_DIR"), 0);
    PASS();
}
#endif
...
/*
 * ANI_WORKERS env override for ani_default_worker_count.
 *
 * Containers running ani on a host with more CPUs than the cgroup's
...
TEST(platform_default_workers_env_invalid) {
    ...
    int baseline = ani_default_worker_count(true);
    ASSERT_GT(baseline, 0);

    ani_setenv("ANI_WORKERS", "0", 1);
    ASSERT_EQ(ani_default_worker_count(true), baseline);

    ani_setenv("ANI_WORKERS", "-1", 1);
    ASSERT_EQ(ani_default_worker_count(true), baseline);

    ani_setenv("ANI_WORKERS", "9999", 1);
    ASSERT_EQ(ani_default_worker_count(true), baseline);

    ani_setenv("ANI_WORKERS", "not-a-number", 1);
    ASSERT_EQ(ani_default_worker_count(true), baseline);
...
TEST(platform_default_workers_env_unset) {
    /* When ANI_WORKERS is unset the result matches today's behaviour
     * (info.total_cores for initial=true, perf_cores-1 for false). */
    ani_unsetenv("ANI_WORKERS");
    ani_system_info_t info = ani_system_info();
    ASSERT_EQ(ani_default_worker_count(true), info.total_cores);
    PASS();
}
...
static int cgroup_test_setup(char *root, size_t root_sz) {
    strncpy(root, "/tmp/ani_cgroup_test_XXXXXX", root_sz);
...
TEST(cgroup_v2_cpu_quota) {
    ...
    ASSERT_EQ(ani_detect_cgroup_cpus(root), 2);
...
TEST(cgroup_v2_cpu_unlimited) {
    ...
    ASSERT_EQ(ani_detect_cgroup_cpus(root), -1);
    cgroup_test_teardown(root);
    PASS();
}
...
TEST(cgroup_v2_mem) {
    ...
    ASSERT_EQ(ani_detect_cgroup_mem(root), (size_t)2147483648UL);
    cgroup_test_teardown(root);
    PASS();
}
...
TEST(cgroup_v2_mem_unlimited) {
    ...
    ASSERT_EQ(ani_detect_cgroup_mem(root), (size_t)0);
    cgroup_test_teardown(root);
    PASS();
}
...
TEST(cgroup_v1_mem) {
    ...
    ASSERT_EQ(ani_detect_cgroup_mem(root), (size_t)1073741824UL);
    cgroup_test_teardown(root);
    PASS();
}
