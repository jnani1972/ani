...
# permanent launcher (ani.exe) plus the real product binary
# (ani.payload.exe). The launcher existed for exactly one
...
# install.ps1, which runs while ANI is NOT running. So this file asserts the
...
makefile = read("Makefile.ani")
require(
    "ani-launcher" not in makefile and "WINDOWS_LAUNCHER" not in makefile,
    "Makefile.ani must not expose a Windows launcher target or variable",
)
...
# install.ps1 runs while ANI is not running, renames the installed binary to
...
single_binary_contracts = {
    "pkg/npm/install.js": (
        r"const\s+WINDOWS_BINARY_NAME\s*=\s*['\"]ani\.exe['\"]",
        r"installWindowsBinaryAtomically\(",
        r"windowsBinaryReady\(",
    ),
    "pkg/npm/bin.js": (
        r"binName\s*=\s*isWindows\s*\?\s*['\"]ani\.exe['\"]",
        r"const\s+executionPath\s*=\s*binPath",
    ),
    "pkg/pypi/src/ani/_cli.py": (
        r"_WINDOWS_BINARY_NAME\s*=\s*['\"]ani\.exe['\"]",
...
for relative, patterns in single_binary_contracts.items():
    ...
    require(
        ".ani/generations" not in source and "current-v1" not in source,
...
guidance_contracts = {
    "pkg/npm/bin.js": (
        "npm install ani@latest",
        "npm uninstall ani",
        "ani install --yes",
    ),
    "pkg/pypi/src/ani/_cli.py": (
        "python -m pip install --upgrade ani",
        "python -m pip uninstall ani",
        "install --yes",
    ),
}
...
require(
    all(
        ...
        for needle in (
            ...
            '("ani-windows-guards-root-" + [guid]::NewGuid().ToString("N"))',
            '$env:TEMP = $guardRoot',
            '$env:TMP = $guardRoot',
...
for relative, source in (
    ...
    require(
        "ANI_VM_HOST_KEY_SHA256" in source,
...
require(
    ...
    and "ANI_WIN_VM_SSH" not in read("test-infrastructure/run.sh"),
...
require(
    ...
    and "ani_vm_write_untracked_manifest" in sync_case.group("body")
...
require(
    ...
    # (per-run isolation), so pinning the literal `/c/ani` was asserting the
...
require(
    'scripts/smoke-test.sh "$SMOKE_DIR/ani.exe"' in vm_smoke,
    f"Windows smoke wrapper must execute the canonical {binary}",
)
require(
    "bash test-infrastructure/vm/vm-smoke.sh" in windows_smoke
    and 'ANI_SMOKE_ARTIFACT_DIR="$(cygpath -u "$RUNNER_TEMP")/ani-artifact"' in windows_smoke,
...
require(
    all(
        ...
        for needle in (
            ...
            'SMOKE_DIR="$(mktemp -d "$PROFILE_ROOT/ani-vm-smoke.XXXXXX")"',
            'cp "$BINARY_SRC" "$SMOKE_DIR/ani.exe"',
            'ANI_CACHE_DIR="$(cygpath -m "$SMOKE_DIR/cache")"',
...
windows_release_version_blocks = [
    ...
    if 'LAUNCH_DIR="$(mktemp -d "$PROFILE_ROOT/ani-release-version.XXXXXX")"' in block
]
require(
    ...
    and all(
        ...
        for needle in (
            'PROFILE_ROOT="$(cygpath -u "$USERPROFILE")"',
            'cp "$ARTIFACT_DIR/ani.exe" "$LAUNCH_DIR/"',
            '"$LAUNCH_DIR/ani.exe" --version',
        )
    ),
...
runner_temp_allowed = (
    re.compile(r'ARTIFACT_DIR="\$\(cygpath -u "\$RUNNER_TEMP"\)/ani-artifact"'),
    re.compile(r'Join-Path \$env:RUNNER_TEMP "ani-artifact"'),
)
...
windows_release_security_blocks = [
    re.sub(r"\s+", " ", re.sub(r"\\\s*\n\s*", " ", block)).strip()
    for block in smoke_blocks
    if 'scripts/security-install.sh "$SECURITY_DIR/ani.exe"' in block
]
require(
    ...
    and all(
        ...
        for needle in (
            'PROFILE_ROOT="$(cygpath -u "$USERPROFILE")"',
            'SECURITY_DIR="$(mktemp -d "$PROFILE_ROOT/ani-release-security.XXXXXX")"',
            'cp "$ARTIFACT_DIR/ani.exe" "$SECURITY_DIR/"',
            'TMPDIR="$SECURITY_DIR" '
            'scripts/security-install.sh "$SECURITY_DIR/ani.exe"',
        )
    ),
...
require(
    'copy_smoke_binary "$FAKE_HOME/.local/bin/ani.exe"' not in smoke_script
    and 'copy_smoke_binary "$UPDATE_HOME/.local/bin/ani.exe"' not in smoke_script,
...
require(
    ...
    and 'UPDATE_DOWNLOAD_URL="file:///$UPDATE_FIXTURE_DIR"' in smoke_script
    and 'ANI_DOWNLOAD_URL="$UPDATE_DOWNLOAD_URL"' in smoke_script,
...
require(
    'ANI_DOWNLOAD_URL="$SMOKE_DOWNLOAD_URL"' in smoke_script
...
require(
    ...
    and smoke_workflow.count("ANI_SMOKE_ARTIFACT_DIR") >= 2,
    "Unix and Windows release smoke must identify their local update fixture via the "
    "canonical wrappers",
)

cli_source = read("src/cli/cli.c")
probe_start = cli_source.find("ani_json_mcp_probe_windows_command_path(")
...
require(
    re.search(r'ani_safe_getenv\s*\(\s*"ANI_DOWNLOAD_URL"', file_override) is not None
...
download_helpers = cli_source[
    cli_source.find("static int ani_download_to_file(") : cli_source.find("/* ── macOS ad-hoc signing")
]
require(
...
update_start = cli_source.find("int ani_cmd_update(int argc, char **argv) {")
...
require(
    ...
    "ani_cmd_update must print the install.ps1 command on Windows",
)
...
require(
    ...
    "ani_cmd_update must not print an execution-policy override "
    "(document it instead of shipping the pattern in the binary)",
)
require(
    "ani_windows_launcher" not in cli_source
