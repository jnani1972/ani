...
TEST(intern_create_free) {
    ANIInternPool *pool = ani_intern_create();
    ASSERT_NOT_NULL(pool);
    ASSERT_EQ(ani_intern_count(pool), 0);
    ani_intern_free(pool);
    PASS();
}

TEST(intern_basic) {
    ANIInternPool *pool = ani_intern_create();
    const char *s1 = ani_intern(pool, "hello");
    ASSERT_NOT_NULL(s1);
    ASSERT_STR_EQ(s1, "hello");
    ASSERT_EQ(ani_intern_count(pool), 1);
    ani_intern_free(pool);
    PASS();
}
...
TEST(intern_dedup) {
    ANIInternPool *pool = ani_intern_create();
    const char *s1 = ani_intern(pool, "hello");
    const char *s2 = ani_intern(pool, "hello");
    /* Must return same pointer */
    ASSERT_EQ((uintptr_t)s1, (uintptr_t)s2);
    ASSERT_EQ(ani_intern_count(pool), 1); /* still 1 */
    ani_intern_free(pool);
    PASS();
}
...
TEST(intern_different_strings) {
    ANIInternPool *pool = ani_intern_create();
    const char *s1 = ani_intern(pool, "hello");
    const char *s2 = ani_intern(pool, "world");
    ASSERT_NEQ((uintptr_t)s1, (uintptr_t)s2);
    ASSERT_STR_EQ(s1, "hello");
    ASSERT_STR_EQ(s2, "world");
    ASSERT_EQ(ani_intern_count(pool), 2);
    ani_intern_free(pool);
    PASS();
}
...
TEST(intern_n_with_length) {
    ANIInternPool *pool = ani_intern_create();
    const char *s1 = ani_intern_n(pool, "hello world", 5);
    ASSERT_STR_EQ(s1, "hello");
...
TEST(intern_many_strings) {
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        const char *s = ani_intern(pool, buf);
        ASSERT_NOT_NULL(s);
    }
    ASSERT_EQ(ani_intern_count(pool), 1000);
    ...
    ASSERT_EQ(ani_intern_count(pool), 1000); /* still 1000 */
    ani_intern_free(pool);
    PASS();
}
...
TEST(intern_bytes) {
    ANIInternPool *pool = ani_intern_create();
    ani_intern(pool, "abc");   /* 3 bytes + NUL */
    ani_intern(pool, "defgh"); /* 5 bytes + NUL */
    /* bytes should be at least 8 (3+5 for the content) */
    ASSERT_GTE(ani_intern_bytes(pool), 8);
    ani_intern_free(pool);
    PASS();
}
...
TEST(intern_null_returns_null) {
    ANIInternPool *pool = ani_intern_create();
    const char *s = ani_intern(pool, NULL);
    ASSERT_NULL(s);
    ASSERT_EQ(ani_intern_count(pool), 0);
    ani_intern_free(pool);
    PASS();
...
TEST(intern_n_zero_len) {
    ...
    ANIInternPool *pool = ani_intern_create();
    const char *s = ani_intern_n(pool, "anything", 0);
    ASSERT_NOT_NULL(s);
    ASSERT_STR_EQ(s, "");
    ASSERT_EQ(ani_intern_count(pool), 1);
    /* Should dedup with intern("") */
    const char *s2 = ani_intern(pool, "");
...
TEST(intern_same_prefix_different_lengths) {
    ...
    const char *s3 = ani_intern_n(pool, src, 3); /* "abc" */
    const char *s5 = ani_intern_n(pool, src, 5); /* "abcde" */
    const char *s8 = ani_intern_n(pool, src, 8); /* "abcdefgh" */
    ...
    ASSERT_EQ(ani_intern_count(pool), 3);
    ani_intern_free(pool);
    PASS();
}
...
TEST(intern_pointer_stability) {
    ...
    ANIInternPool *pool = ani_intern_create();
    const char *original = ani_intern(pool, "sentinel_value");
    ...
    ASSERT_EQ(ani_intern_count(pool), 10001);
    ...
    const char *again = ani_intern(pool, "sentinel_value");
...
TEST(intern_two_pools_independent) {
    ...
    ANIInternPool *p1 = ani_intern_create();
    ANIInternPool *p2 = ani_intern_create();
    const char *s1 = ani_intern(p1, "shared");
    const char *s2 = ani_intern(p2, "shared");
...
TEST(intern_free_empty_pool) {
    ...
    ANIInternPool *pool = ani_intern_create();
    ASSERT_EQ(ani_intern_count(pool), 0);
    ASSERT_EQ(ani_intern_bytes(pool), 0);
    ani_intern_free(pool);
    PASS();
}
...
TEST(intern_bytes_tracking) {
    ...
    ani_intern(pool, "abc");   /* 3 bytes */
    ASSERT_EQ(ani_intern_bytes(pool), 3);

    ani_intern(pool, "defgh"); /* 5 bytes */
    ASSERT_EQ(ani_intern_bytes(pool), 8);

    /* Duplicate should not add bytes */
    ani_intern(pool, "abc");
    ASSERT_EQ(ani_intern_bytes(pool), 8);

    ani_intern(pool, "");      /* 0 bytes */
    ASSERT_EQ(ani_intern_bytes(pool), 8);

    ani_intern(pool, "x");     /* 1 byte */
    ASSERT_EQ(ani_intern_bytes(pool), 9);
...
TEST(intern_free_null_pool) {
    ...
    ani_intern_free(NULL);
    PASS();
}

TEST(intern_count_null_pool) {
    ASSERT_EQ(ani_intern_count(NULL), 0);
    PASS();
}
