 ...
 * ani_extract_file with ANI_LANG_PYTHON, then assertions search the
...
static ANIFileResult *extract_py(const char *source) {
    return ani_extract_file(source, (int)strlen(source), ANI_LANG_PYTHON,
...
static void bind_imports_into_ctx(PyLSPContext *ctx, ANIArena *a, ANITypeRegistry *reg,
...
TEST(pylsp_import_simple) {
    ...
    const ANIType *t = py_lsp_lookup_in_scope(&ctx, "os");
    ASSERT(ani_type_is_module(t));
...
TEST(pylsp_import_aliased) {
    ...
    const char *qns[] = {"json"};
    bind_imports_into_ctx(&ctx, &a, &reg, locals, qns, 1);
    const ANIType *t = py_lsp_lookup_in_scope(&ctx, "j");
    ASSERT(ani_type_is_module(t));
    ASSERT_STR_EQ(t->data.module.module_qn, "json");
    /* original name "json" not bound */
    const ANIType *miss = py_lsp_lookup_in_scope(&ctx, "json");
    ASSERT(ani_type_is_unknown(miss));
    ani_arena_destroy(&a);
    PASS();
}
...
TEST(pylsp_import_from) {
    ...
    bind_imports_into_ctx(&ctx, &a, &reg, locals, qns, 1);
    const ANIType *t = py_lsp_lookup_in_scope(&ctx, "Path");
    ASSERT_EQ(t->kind, ANI_TYPE_NAMED);
...
TEST(pylsp_import_from_aliased) {
    ...
    const char *qns[] = {"pathlib.Path"};
    bind_imports_into_ctx(&ctx, &a, &reg, locals, qns, 1);
    const ANIType *t = py_lsp_lookup_in_scope(&ctx, "P");
...
TEST(pylsp_import_relative_one_dot) {
    ...
    bind_imports_into_ctx(&ctx, &a, &reg, locals, qns, 1);
    const ANIType *t = py_lsp_lookup_in_scope(&ctx, "sibling");
    ASSERT(!ani_type_is_unknown(t));
    ani_arena_destroy(&a);
    PASS();
}
...
TEST(pylsp_import_relative_two_dots) {
    ...
    bind_imports_into_ctx(&ctx, &a, &reg, locals, qns, 1);
    const ANIType *t = py_lsp_lookup_in_scope(&ctx, "x");
    ASSERT(!ani_type_is_unknown(t));
    ani_arena_destroy(&a);
    PASS();
}
...
TEST(pylsp_import_star_best_effort) {
    ...
    const char *qns[] = {"X"};
    bind_imports_into_ctx(&ctx, &a, &reg, locals, qns, 1);
    const ANIType *star_miss = py_lsp_lookup_in_scope(&ctx, "*");
    ASSERT(ani_type_is_unknown(star_miss));
...
TEST(pylsp_import_typing_only_still_binds) {
     ...
     * emits ANIImport entries regardless of guard. py_lsp binds them. */
    ANIArena a;
    ani_arena_init(&a);
    ANITypeRegistry reg;
    ani_registry_init(&reg, &a);
    ...
    bind_imports_into_ctx(&ctx, &a, &reg, locals, qns, 1);
    const ANIType *t = py_lsp_lookup_in_scope(&ctx, "List");
    ASSERT(!ani_type_is_unknown(t));
    ASSERT_EQ(t->kind, ANI_TYPE_NAMED);
    ani_arena_destroy(&a);
    PASS();
...
static PyResolvedSiteCounts count_invocation_sites(const ANIResolvedCallArray *arr,
    ...
    for (int i = 0; i < arr->count; i++) {
        const ANIResolvedCall *rc = &arr->items[i];
        if (rc->kind != ANI_RESOLVED_INVOCATION || !rc->caller_qn || !rc->callee_qn ||
...
TEST(pylsp_crossfile_method_dispatch) {
     ...
     * the method on a typed parameter. Reuses ANILSPDef to feed the
    ...
    ani_run_py_lsp_cross(&arena, source, (int)strlen(source), "test.main", defs, 2, imp_names,
...
TEST(pylsp_fused_self_attr_chain_via_overlay) {
    ...
    defs[2].lang = ANI_LANG_PYTHON;

    ANITypeRegistry *reg = ani_py_build_cross_registry(&arena, defs, 3);
    ASSERT_NOT_NULL(reg);
    ...
    const ANIRegisteredType *foo = ani_registry_lookup_type(reg, "test.mod.Foo");
    ASSERT_NOT_NULL(foo);
...
TEST(pylsp_crossfile_inheritance) {
    ...
    ani_run_py_lsp_cross(&arena, source, (int)strlen(source), "test.main", defs, 4, imp_names,
                         imp_qns, 1, NULL, &out, NULL);
...
TEST(pylsp_batch_two_files) {
    ...
    ANILSPDef b_combined[2];
    memcpy(&b_combined[0], &a_defs[0], sizeof(ANILSPDef));
    memcpy(&b_combined[1], &b_defs[0], sizeof(ANILSPDef));
    ...
    ANIArena arena;
    ani_arena_init(&arena);
    ANIResolvedCallArray outs[2];
    memset(outs, 0, sizeof(outs));

    ani_batch_py_lsp_cross(&arena, files, 2, outs);
...
static int pylsp_exact_reference_count(const ANIResolvedCallArray *out, const char *caller,
                                        const char *callee_qn, const char *reason,
                                        uint32_t site_start, uint32_t site_end) {
    int count = 0;
    for (int i = 0; out && i < out->count; i++) {
        const ANIResolvedCall *resolved = &out->items[i];
...
static PyImportReferenceCounts pylsp_import_reference_counts(
    ...
    defs[0].short_name = ani_lsp_bare_segment(first_target_qn);
    defs[0].label = "Function";
    defs[0].def_module_qn = "project.target";
    int def_count = 1;
    if (second_target_qn) {
        defs[1].qualified_name = second_target_qn;
        defs[1].short_name = ani_lsp_bare_segment(second_target_qn);
    ...
    ani_arena_init(&arena);
    ANIResolvedCallArray out = {0};
    ani_batch_py_lsp_cross(&arena, &file, 1, &out);
...
TEST(pylsp_same_target_calls_have_distinct_exact_semantic_sites) {
    ...
    ANIFileResult *r = extract_py(source);
    ASSERT_NOT_NULL(r);
...
/* ani_batch_py_lsp_cross resolves in a temporary per-file arena, then copies
 ...
 * inside ani_pxc_run_one. A Python dunder carrier created only by that cross
...
TEST(pylsp_scratch_cross_dunder_carrier_survives_copy) {
    ...
    ani_pxc_run_one(ANI_LANG_PYTHON, &result, source, (int)strlen(source), "scratch", defs, 3, NULL,
                    NULL, 0);
...
TEST(pylsp_round1_dotted_import_alias_matching_root) {
    ...
    ANIFileResult *r = extract_py("import os.path as os\n"
                                  "def use():\n"
                                  "    return os.join('a', 'b')\n");
    ASSERT_NOT_NULL(r);
    ASSERT_EQ(r->imports.count, 1);
...
TEST(pylsp_dunder_same_leaf_occurrences_join_by_exact_site) {
    ...
    const ANICall *dunder_calls[2] = {0};
    int dunder_count = 0;
    ...
    const ANICall *alpha_call = dunder_calls[0];
    const ANICall *beta_call = dunder_calls[1];
    if (alpha_call->site_start_byte > beta_call->site_start_byte) {
        const ANICall *tmp = alpha_call;
    ...
    const ANIResolvedCall *alpha_resolution =
        ani_pipeline_find_lsp_resolution(&r->resolved_calls, alpha_call, false);
    const ANIResolvedCall *beta_resolution =
...
TEST(pylsp_lambda_rewalk_dedupes_dunder_occurrence) {
    ...
    ASSERT_EQ(semantic->kind, ANI_RESOLVED_INVOCATION);
    ...
    const ANIResolvedCall *joined =
        ani_pipeline_find_lsp_resolution(&r->resolved_calls, carrier, false);
    ASSERT_NOT_NULL(joined);
 ...
 * in one caller; each ANICall must join the semantic record for its own site. */
TEST(pylsp_ordinary_same_leaf_calls_join_by_exact_site) {
    ANIFileResult *r = extract_py("class Alpha:\n"
                                  "    def render(self):\n"
                                  "        return 1\n"
                                  "class Beta:\n"
                                  "    def render(self):\n"
    ...
    const ANIResolvedCall *alpha_hit =
        calls[0] ? ani_pipeline_find_lsp_resolution(&r->resolved_calls, calls[0], false) : NULL;
    const ANIResolvedCall *beta_hit =
        calls[1] ? ani_pipeline_find_lsp_resolution(&r->resolved_calls, calls[1], false) : NULL;
...
TEST(pylsp_eval_steps_budget_degrades_gracefully) {
    ...
    ANIFileResult *r = extract_py(src);
    free(src);
    ASSERT_NOT_NULL(r);
