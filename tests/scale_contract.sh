...
#
# This tier runs the COMPILED binary on the ani-bench-validate repos and asserts
...
#   ANI_SCALE_INCLUDE_C=1 bash tests/scale_contract.sh c   # add the slow C tier
#   ANI_BENCH_DIR=/path bash tests/scale_contract.sh       # override repo root
...
BENCH="${ANI_BENCH_DIR:-$HOME/ani-bench-validate}"
PY="${PYTHON:-python3.9}"
FAILURES=0
RAN=0
...
# project name == ani_project_name_from_path(): strip leading '/', map every
...
if [ "${#LANGS[@]}" -eq 0 ]; then
    ...
    [ "${ANI_SCALE_INCLUDE_C:-0}" = "1" ] && LANGS+=(c)
fi
