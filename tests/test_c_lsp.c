 ...
 *
 * AUTO-GENERATED from internal/ani/lsp_c_test.go by scripts/port_clsp_tests.py
...
static int count_resolved(const ANIFileResult *r, const char *callerSub, const char *calleeSub) {
    int n = 0;
    for (int i = 0; i < r->resolved_calls.count; i++) {
        const ANIResolvedCall *rc = &r->resolved_calls.items[i];
        if (rc->caller_qn && strstr(rc->caller_qn, callerSub) && rc->callee_qn &&
...
static ANIFileResult *extract_c(const char *src) {
    return ani_extract_file(src, (int)strlen(src), ANI_LANG_C, "test", "main.c", 0, NULL, NULL);
}

static ANIFileResult *extract_cpp(const char *src) {
    return ani_extract_file(src, (int)strlen(src), ANI_LANG_CPP, "test", "main.cpp", 0, NULL, NULL);
}

static ANIFileResult *extract_c_family(const char *src, ANILanguage language) {
    const char *path = language == ANI_LANG_C      ? "main.c"
                       : language == ANI_LANG_CUDA ? "main.cu"
                                                   : "main.cpp";
    return ani_extract_file(src, (int)strlen(src), language, "test", path, 0, NULL, NULL);
}

TEST(clsp_simple_var_decl) {
    ANIFileResult *r = extract_c("\n"
                                 "struct Foo {\n"
                                 "    int value;\n"
                                 "};\n"
                                 "\n"
...
TEST(clsp_calls_attributed_to_function_issue220) {
    ANIFileResult *r = extract_cpp("void helper() {}\n"
                                   "void doWork() {\n"
                                   "    helper();\n"
                                   "}\n");
    ASSERT_NOT_NULL(r);
...
TEST(clsp_nocrash_issue355_xxhash_header) {
    ...
    buf[rd] = '\0';
    ANIFileResult *r =
        ani_extract_file(buf, (int)rd, ANI_LANG_C, "test", "xxhash.h", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
    ani_free_result(r);
    free(buf);
    PASS();
...
TEST(clsp_nocrash_issue312_default_template_auto_param) {
    ANIFileResult *r = extract_cpp("template<typename T, typename U = int>\n"
                                   "U f(auto, T t) {\n"
                                   "    return t;\n"
                                   "}\n"
                                   "\n"
...
TEST(clsp_nocrash_empty_source) {
    ANIFileResult *r = ani_extract_file("", 0, ANI_LANG_CPP, "test", "main.cpp", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
    ani_free_result(r);
    PASS();
}
...
TEST(clsp_nocrash_extremely_large_function) {
    ...
    ANIFileResult *r = extract_cpp(src);
    ASSERT_NOT_NULL(r);
 ...
 * (ani_c_build_cross_registry), FINALIZES it (O(1) hash lookups), then shares it
 * READ-ONLY across the parallel resolve workers (ctx.registry_shared = true).
 * ani_run_c_lsp_cross_with_registry must therefore NOT mutate it.
 *
...
TEST(clsp_tier2_shared_registry_readonly_c) {
    ...
    ANITypeRegistry *reg = ani_c_build_cross_registry(&arena, NULL, 0);
    ASSERT_NOT_NULL(reg);
    ...
    ani_run_c_lsp_cross_with_registry(&arena, src, (int)strlen(src), "test.mod",
...
TEST(registry_short_name_indexes) {
    ...
    for (int i = 0; i < 512; i++) {
        memset(&t, 0, sizeof(t));
        t.qualified_name = ani_arena_sprintf(&arena, "bulk.mod%d.Name%d", i, i % 23);
    ...
    ANITypeShortIter nit;
    ani_registry_types_by_short_name(&reg, "Trait", &nit);
    {
        int expected[] = {0, 4, 5};
        int exact_count = 0;
        int candidate;
        while ((candidate = ani_type_short_iter_next(&nit)) >= 0) {
    ...
    ani_registry_types_by_short_name(&reg, "Missing", &nit);
    {
        int exact_count = 0;
        int candidate;
        while ((candidate = ani_type_short_iter_next(&nit)) >= 0) {
    ...
    for (int name_i = 0; name_i < 23; name_i++) {
        ...
        ani_registry_types_by_short_name(&reg, short_name, &nit);
        int linear_i = 0;
        for (;;) {
            int expected = -1;
            ...
            int actual;
            for (;;) {
                actual = ani_type_short_iter_next(&nit);
                if (actual < 0)
                    break;
    ...
    ANITypeEmbedIter it;
    ani_registry_types_by_embedded_bare(&reg, "Trait", &it);
    ASSERT_EQ(ani_type_embed_iter_next(&it), 1);
    ASSERT_EQ(ani_type_embed_iter_next(&it), 2);
    ASSERT_EQ(ani_type_embed_iter_next(&it), -1);

    /* Bare "Other": only type 3 (C). */
    ani_registry_types_by_embedded_bare(&reg, "Other", &it);
    ASSERT_EQ(ani_type_embed_iter_next(&it), 3);
    ASSERT_EQ(ani_type_embed_iter_next(&it), -1);

    /* Bare with no implementers: empty. */
    ani_registry_types_by_embedded_bare(&reg, "Nope", &it);
    ASSERT_EQ(ani_type_embed_iter_next(&it), -1);

    /* Free-func index for "helper": func 0 then 1 (ascending), NOT the method (2). */
    ANIFreeFuncIter fit;
    ani_registry_free_funcs_by_short_name(&reg, "helper", &fit);
    ASSERT_EQ(ani_free_func_iter_next(&fit), 0);
    ASSERT_EQ(ani_free_func_iter_next(&fit), 1);
    ASSERT_EQ(ani_free_func_iter_next(&fit), -1);

    /* Free-func "other": only func 3. */
    ani_registry_free_funcs_by_short_name(&reg, "other", &fit);
    ASSERT_EQ(ani_free_func_iter_next(&fit), 3);
    ASSERT_EQ(ani_free_func_iter_next(&fit), -1);
    ...
    ani_registry_add_type(&reg, t);
    ani_registry_types_by_short_name(&reg, "Trait", &nit);
    ASSERT_EQ(ani_type_short_iter_next(&nit), 0);
    ASSERT_EQ(ani_type_short_iter_next(&nit), 4);
    ASSERT_EQ(ani_type_short_iter_next(&nit), 5);
    ASSERT_EQ(ani_type_short_iter_next(&nit), tail_type_i);
    ASSERT_EQ(ani_type_short_iter_next(&nit), -1);
    ...
    arena.nblocks = ANI_ARENA_MAX_BLOCKS;
...
TEST(seal_py_shared_registry_readonly) {
    ANIArena arena;
    ani_arena_init(&arena);
    ANITypeRegistry *reg = ani_py_build_cross_registry(&arena, NULL, 0);
    ASSERT_NOT_NULL(reg);
    int fb = reg->func_count, tb = reg->type_count;
    const char *src = "def helper(x):\n    return x + 1\n\ndef caller():\n    return helper(1)\n";
    ANIResolvedCallArray out = {0};
    ani_run_py_lsp_cross_with_registry(&arena, src, (int)strlen(src), "test.mod", reg, NULL, NULL,
                                       0, NULL, &out, NULL);
 ...
 * ani_registry_add_func/_type (which the read_only seal no-ops). But
 ...
 * read_only registry (pipeline.c:901 → ani_py_build_cross_registry), so this is a
...
TEST(seal_py_shared_registry_readonly_fields) {
    ...
    defs[0].lang = ANI_LANG_PYTHON;
    defs[1].qualified_name = "test.mod.Foo.m";
    defs[1].short_name = "m";
    defs[1].label = "Method";
    defs[1].receiver_type = "test.mod.Foo";
    defs[1].def_module_qn = "test.mod";
    defs[1].lang = ANI_LANG_PYTHON;

    ANITypeRegistry *reg = ani_py_build_cross_registry(&arena, defs, 2);
    ASSERT_NOT_NULL(reg);
    ...
    const ANIRegisteredType *rt = ani_registry_lookup_type(reg, "test.mod.Foo");
    ASSERT_NOT_NULL(rt);
...
TEST(seal_cs_shared_registry_readonly) {
    ANIArena arena;
    ani_arena_init(&arena);
    ANITypeRegistry *reg = ani_cs_build_cross_registry(&arena, NULL, 0);
    ASSERT_NOT_NULL(reg);
    int fb = reg->func_count, tb = reg->type_count;
    const char *src = "namespace N { class Box { int Unwrap() { return 0; }\n"
                      "  int Caller() { return Unwrap(); } } }\n";
    ANIResolvedCallArray out = {0};
    ani_run_cs_lsp_cross_with_registry(&arena, src, (int)strlen(src), "test.mod", reg, NULL, 0,
                                       NULL, &out);
    ASSERT_EQ(reg->func_count, fb);
    ASSERT_EQ(reg->type_count, tb);
    ani_arena_destroy(&arena);
...
TEST(seal_ts_shared_registry_readonly) {
    ANIArena arena;
    ani_arena_init(&arena);
    ANITypeRegistry *reg = ani_ts_build_cross_registry(&arena, NULL, 0);
    ASSERT_NOT_NULL(reg);
    int fb = reg->func_count, tb = reg->type_count;
    const char *src = "class Box { unwrap(): number { return 0; } }\n"
                      "function caller(b: Box): number { return b.unwrap(); }\n";
    ANIResolvedCallArray out = {0};
    ani_run_ts_lsp_cross_with_registry(&arena, src, (int)strlen(src), "test.mod", false, false,
...
TEST(seal_go_shared_registry_readonly) {
    ANIArena arena;
    ani_arena_init(&arena);
    ANITypeRegistry *reg = ani_go_build_cross_registry(&arena, NULL, 0);
    ASSERT_NOT_NULL(reg);
    int fb = reg->func_count, tb = reg->type_count;
    const char *src = "package main\nfunc helper(x int) int { return x + 1 }\n"
                      "func caller() int { return helper(1) }\n";
    ANIResolvedCallArray out = {0};
    ani_run_go_lsp_cross_with_registry(&arena, src, (int)strlen(src), "test.mod", reg, NULL, NULL,
                                       0, NULL, &out);
    ASSERT_EQ(reg->func_count, fb);
    ASSERT_EQ(reg->type_count, tb);
    ani_arena_destroy(&arena);
...
static int assert_cpp_family_repeated_method_occurrences(ANILanguage language) {
    ...
    ANIFileResult *result = extract_c_family(source, language);
    ASSERT_NOT_NULL(result);
    const ANICall *calls[2] = {NULL, NULL};
    const ANIResolvedCall *semantics[2] = {NULL, NULL};
    int carrier_count = 0;
    ...
    for (int i = 0; i < result->calls.count; i++) {
        ...
        if (!call->enclosing_func_qn || !strstr(call->enclosing_func_qn, "occurrence_probe") ||
            !call->callee_name || strcmp(ani_lsp_bare_segment(call->callee_name), "render") != 0) {
            continue;
        }
        carrier_count++;
    ...
    for (int i = 0; i < result->resolved_calls.count; i++) {
        ...
        if (resolved->kind != ANI_RESOLVED_INVOCATION || resolved->confidence <= 0.0f ||
            !resolved->caller_qn || !strstr(resolved->caller_qn, "occurrence_probe") ||
            !resolved->callee_qn ||
            strcmp(ani_lsp_bare_segment(resolved->callee_qn), "render") != 0) {
            continue;
        }
        semantic_count++;
        for (int occurrence = 0; occurrence < 2; occurrence++) {
    ...
    ASSERT_TRUE(ani_pipeline_find_lsp_resolution(&result->resolved_calls, calls[0], false) ==
                semantics[0]);
    ASSERT_TRUE(ani_pipeline_find_lsp_resolution(&result->resolved_calls, calls[1], false) ==
                semantics[1]);
...
TEST(clsp_cpp_repeated_same_leaf_calls_join_exact_occurrences) {
    return assert_cpp_family_repeated_method_occurrences(ANI_LANG_CPP);
}

TEST(clsp_cuda_repeated_same_leaf_calls_join_exact_occurrences) {
    return assert_cpp_family_repeated_method_occurrences(ANI_LANG_CUDA);
}
...
TEST(clsp_c_reassigned_function_pointer_calls_join_exact_occurrences) {
    ...
    ANIFileResult *result = extract_c_family(source, ANI_LANG_C);
    ASSERT_NOT_NULL(result);
    const ANICall *calls[2] = {NULL, NULL};
    const ANIResolvedCall *semantics[2] = {NULL, NULL};
    int carrier_count = 0;
...
TEST(clsp_c_nested_function_pointer_shadow_restores_outer_target) {
    ...
    ANIFileResult *result = extract_c_family(source, ANI_LANG_C);
    ASSERT_NOT_NULL(result);
    const ANIResolvedCall *inner_semantic = NULL;
    const ANIResolvedCall *outer_semantic = NULL;
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const ANIResolvedCall *resolved = &result->resolved_calls.items[i];
        if (resolved->kind != ANI_RESOLVED_INVOCATION || !resolved->reason ||
...
TEST(clsp_cpp_new_expression_joins_exact_occurrence) {
    ...
    ANIFileResult *result = extract_c_family(source, ANI_LANG_CPP);
    ASSERT_NOT_NULL(result);
    const ANICall *carrier = NULL;
    const ANIResolvedCall *semantic = NULL;
    for (int i = 0; i < result->calls.count; i++) {
        const ANICall *call = &result->calls.items[i];
    ...
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const ANIResolvedCall *resolved = &result->resolved_calls.items[i];
        if (resolved->kind == ANI_RESOLVED_INVOCATION && resolved->callee_qn &&
    ...
    ASSERT_TRUE(ani_pipeline_find_lsp_resolution(&result->resolved_calls, carrier, false) ==
                semantic);
    ani_free_result(result);
    PASS();
}
...
static int assert_cpp_family_preprocessed_collision_isolated(ANILanguage language) {
    ...
    ANIFileResult *result = extract_c_family(source, language);
    ASSERT_NOT_NULL(result);
    const ANICall *raw_alpha = NULL;
    const ANICall *expanded_bravo = NULL;
    ...
    const ANIResolvedCall *raw_alpha_semantic = NULL;
    const ANIResolvedCall *expanded_bravo_semantic = NULL;
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const ANIResolvedCall *resolved = &result->resolved_calls.items[i];
        if (resolved->kind != ANI_RESOLVED_INVOCATION || resolved->confidence <= 0.0f ||
    ...
    ASSERT_TRUE(ani_pipeline_find_lsp_resolution(&result->resolved_calls, raw_alpha, false) ==
                raw_alpha_semantic);
    ASSERT_TRUE(ani_pipeline_find_lsp_resolution(&result->resolved_calls, expanded_bravo, false) ==
...
TEST(clsp_cpp_preprocessed_coordinate_collision_isolated) {
    return assert_cpp_family_preprocessed_collision_isolated(ANI_LANG_CPP);
}

TEST(clsp_cuda_preprocessed_coordinate_collision_isolated) {
    return assert_cpp_family_preprocessed_collision_isolated(ANI_LANG_CUDA);
}
...
TEST(clsp_c_preprocessed_coordinate_collision_isolated) {
    ...
    ANIFileResult *result = extract_c_family(source, ANI_LANG_C);
    ASSERT_NOT_NULL(result);
    const ANICall *colliding_calls[2] = {NULL, NULL};
    int colliding_carriers = 0;
    for (int i = 0; i < result->calls.count; i++) {
        const ANICall *call = &result->calls.items[i];
        if (call->enclosing_func_qn && strstr(call->enclosing_func_qn, "occurrence_probe") &&
    ...
    const ANIResolvedCall *bravo_semantic = NULL;
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const ANIResolvedCall *resolved = &result->resolved_calls.items[i];
        if (resolved->kind != ANI_RESOLVED_INVOCATION || resolved->confidence <= 0.0f ||
            resolved->site_start_byte != collision_start ||
    ...
    const ANIResolvedCall *first_join =
        ani_pipeline_find_lsp_resolution(&result->resolved_calls, colliding_calls[0], false);
    const ANIResolvedCall *second_join =
        ani_pipeline_find_lsp_resolution(&result->resolved_calls, colliding_calls[1], false);
...
TEST(clsp_preprocessed_destructor_rewrite_respects_origin_during_rewrite) {
    ...
    char *expanded = ani_preprocess(source, written, "main.cpp", NULL, NULL, true);
    ...
    expanded = ani_preprocess(source, written, "main.cpp", NULL, NULL, true);
    ASSERT_NOT_NULL(expanded);
    ...
    ANIFileResult *result = extract_cpp(source);
    ASSERT_NOT_NULL(result);
    const ANICall *raw_carrier = NULL;
    const ANICall *preprocessed_carrier = NULL;
    const ANIResolvedCall *raw_semantic = NULL;
    const ANIResolvedCall *preprocessed_semantic = NULL;
    for (int i = 0; i < result->calls.count; i++) {
        const ANICall *call = &result->calls.items[i];
        ...
        if (call->source_origin == ANI_SOURCE_ORIGIN_RAW) {
            raw_carrier = call;
        } else if (call->source_origin == ANI_SOURCE_ORIGIN_PREPROCESSED) {
            preprocessed_carrier = call;
        }
    }
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const ANIResolvedCall *resolved = &result->resolved_calls.items[i];
        if (resolved->kind != ANI_RESOLVED_INVOCATION || !resolved->strategy ||
            strcmp(resolved->strategy, "lsp_destructor") != 0 || !resolved->callee_qn ||
            resolved->site_start_byte != collision_start ||
            resolved->site_end_byte != collision_end) {
            continue;
        }
        if (resolved->source_origin == ANI_SOURCE_ORIGIN_RAW &&
            strstr(resolved->callee_qn, "~Alpha")) {
            raw_semantic = resolved;
        } else if (resolved->source_origin == ANI_SOURCE_ORIGIN_PREPROCESSED &&
    ...
    bool raw_name_ok = raw_carrier->callee_name &&
                       strcmp(ani_lsp_bare_segment(raw_carrier->callee_name), "~Alpha") == 0;
    bool preprocessed_name_ok =
        preprocessed_carrier->callee_name &&
        strcmp(ani_lsp_bare_segment(preprocessed_carrier->callee_name), "~Bravo") == 0;
    bool raw_join_ok = ani_pipeline_find_lsp_resolution(&result->resolved_calls, raw_carrier,
                                                        false) == raw_semantic;
    bool preprocessed_join_ok =
        ani_pipeline_find_lsp_resolution(&result->resolved_calls, preprocessed_carrier, false) ==
