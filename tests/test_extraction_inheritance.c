 ...
 * DESIGN INTENT
 * ─────────────
 * Every row asserts the CORRECT behaviour: that `ANIDefinition.base_classes`
...
static ANIDefinition *find_def(ANIFileResult *r, const char *label, const char *name) {
...
static ANIDefinition *find_def_any(ANIFileResult *r, const char *name) {
    for (int i = 0; i < r->defs.count; i++) {
        ANIDefinition *d = &r->defs.items[i];
...
static int bases_contain(ANIDefinition *d, const char *want) {
    if (!d->base_classes)
        return 0;
...
static int bases_contain_substr(ANIDefinition *d, const char *substr) {
    if (!d->base_classes)
        return 0;
    for (const char **b = d->base_classes; *b; b++) {
        if (strstr(*b, substr))
...
static int bases_count(ANIDefinition *d) {
    if (!d->base_classes)
        return 0;
    int n = 0;
    for (const char **b = d->base_classes; *b; b++)
...
static ANIDefinition *find_def_flex(ANIFileResult *r, const char *name) {
    ANIDefinition *d;
...
static int run_inherit_case(const inherit_case_t *tc) {
    ANIFileResult *r =
        ani_extract_file(tc->src, (int)strlen(tc->src), tc->lang, "t", tc->path, 0, NULL, NULL);
    if (!r) {
        printf("  FAIL  [%s] ani_extract_file returned NULL\n", tc->class_name);
        return 1;
    }

    ANIDefinition *cls = find_def_flex(r, tc->class_name);
    if (!cls) {
 ...
 * extractor populates ANIFileResult.impl_traits (ANIImplTrait array)
 * with {trait_name, struct_name} pairs.
 * ═══════════════════════════════════════════════════════════════════ */

/* Rust helper: check impl_traits array. */
static int rust_has_impl(ANIFileResult *r, const char *trait_name, const char *struct_name) {
    for (int i = 0; i < r->impl_traits.count; i++) {
        ANIImplTrait *it = &r->impl_traits.items[i];
...
static int run_rust_impl(const rust_impl_case_t *tc) {
    ANIFileResult *r = ani_extract_file(tc->src, (int)strlen(tc->src), ANI_LANG_RUST, "t", "lib.rs",
                                        0, NULL, NULL);
    if (!r) {
