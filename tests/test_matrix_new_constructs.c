...
static ani_store_t *mn_open_indexed(MN_LangProj *lp) {
...
static ani_store_t *mn_index_files(MN_LangProj *lp, const MN_LangFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/ani_mn_XXXXXX");
...
static void mn_cleanup(MN_LangProj *lp, ani_store_t *store) {
...
static int mn_count_label(ani_store_t *store, const char *project, const char *label) {
...
static int mn_named_node_match_count(ani_store_t *store, const char *project, const char *name,
                                     const char *label, const char *qn_suffix) {
    ani_node_t *nodes = NULL;
    int count = 0;
    if (ani_store_find_nodes_by_name(store, project, name, &nodes, &count) != ANI_STORE_OK) {
        return -1;
    }
    int matches = 0;
...
static int mn_exact_edge_by_qn_suffix(ani_store_t *store, const char *project,
    ...
    int source_count = 0;
    int target_count = 0;
    if (ani_store_find_nodes_by_qn_suffix(store, project, source_suffix, &sources, &source_count) !=
            ANI_STORE_OK ||
...
static int mn_type_nodes(ani_store_t *store, const char *project) {
...
static int mn_callables_with_outbound(ani_store_t *store, const char *project) {
...
static MN_Metrics mn_metrics_files(const MN_LangFile *files, int nfiles) {
    MN_LangProj lp;
    ani_store_t *store = mn_index_files(&lp, files, nfiles);
    MN_Metrics m = {0};
    if (store) {
        m.ok             = 1;
        ...
        m.implements     = ani_store_count_edges_by_type(store, lp.project, "IMPLEMENTS");
        m.defines_method = ani_store_count_edges_by_type(store, lp.project, "DEFINES_METHOD");
    }
    mn_cleanup(&lp, store);
    return m;
}
...
static MN_TSNamespaceObservation mn_observe_ts_namespace(const char *filename) {
    ...
    MN_LangProj lp;
    ani_store_t *store = mn_index_files(&lp, &file, 1);
...
TEST(mn_module_python_package) {
    static const MN_LangFile f[] = {
        ...
        {"main.py",
         "from .pkg import add\n\n\ndef run():\n    return add(1, 2)\n"}};
    MN_LangProj lp;
    ani_store_t *store = mn_index_files(&lp, f, 3);
    int functions = store ? mn_count_label(store, lp.project, "Function") : 0;
    int imports   = store ? ani_store_count_edges_by_type(store, lp.project, "IMPORTS") : 0;
    int ok        = (store != NULL);
    mn_cleanup(&lp, store);
    ASSERT_TRUE(ok);
