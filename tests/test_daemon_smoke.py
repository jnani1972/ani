#!/usr/bin/env python3
"""Real-binary smoke test for the mandatory per-account ANI daemon.
...
def probe_future_generation_rendezvous(
    ...
    header = struct.pack(
        ">4sBBHI", b"ANID", 1, 1, 1, RENDEZVOUS_REQUEST_SIZE
    )
    with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as stream:
        ...
        check(magic == b"ANID", "future probe received wrong frame magic")
...
def run_successful_activation(
    ...
    check(
        "Stopping active ANI sessions and operations for {}...".format(action)
        in stdout,
    ...
    records = [
        ...
        if record.get("event") == "ani.activation"
...
def create_local_update_release(release_dir, candidate):
    ...
    asset_name = "ani-{}-{}{}.tar.gz".format(
        os_name, arch, portable
    )
    archive = release_dir / asset_name
    with tarfile.open(archive, "w:gz") as stream:
        stream.add(str(candidate), arcname="ani", recursive=False)
...
def assert_no_activation_artifacts(directory, label):
    artifacts = (
        sorted(path.name for path in directory.iterdir() if path.name.startswith(".ani-"))
        if directory.exists()
        else []
    )
...
def main():
    if os.name == "nt" or sys.platform.startswith(("cygwin", "msys")):
        if os.environ.get("ANI_DAEMON_SMOKE_REQUIRE_RUN") == "1":
            raise SmokeFailure(
    ...
    binary = Path(sys.argv[1] if len(sys.argv) > 1 else root / "build/c/ani")
    ...
    runtime_dir = runtime_parent / ("ani-daemon-" + str(os.geteuid()))
    socket_path = runtime_dir / ("ani-" + RENDEZVOUS_KEY + ".sock")
    startup_lock = runtime_dir / ("ani-" + RENDEZVOUS_KEY + ".lock")
    lifetime_lock = runtime_dir / ("ani-" + RENDEZVOUS_KEY + ".lifetime.lock")
    cohort_admission_lock = runtime_dir / "ani-version-cohort-admission-v1.lock"
    cohort_lifetime_lock = runtime_dir / "ani-version-cohort-lifetime-v1.lock"
    cohort_maintenance_lock = runtime_dir / "ani-version-cohort-maintenance-v1.lock"
    cohort_daemon_lock = runtime_dir / "ani-version-cohort-daemon-v1.lock"
    coordination_locks = (
        (startup_lock, False),
        (lifetime_lock, True),
    ...
    if any(status == "held" for _, status in coordination_before):
        if os.environ.get("ANI_DAEMON_SMOKE_REQUIRE_RUN") == "1":
            raise SmokeFailure(
                "another ANI process or activation is active or starting; "
                "required smoke needs a clean account-wide rendezvous"
            )
        print("SKIP: another ANI process or activation is active or starting")
        return 0
    for path, status in coordination_before:
        check(status == "free", "unsafe coordination lock {}: {}".format(path, status))

    clients = []
    with tempfile.TemporaryDirectory(prefix="ani-daemon-smoke-") as raw_tmpdir:
        ...
        env.update(
            {
                "HOME": str(home),
                "ANI_CACHE_DIR": str(cache_alias),
                "ANI_LOG_LEVEL": "info",
                "ANI_LOG_FORMAT": "json",
                "ANI_TEST_HANG_ON": "hang_me",
                "ANI_TEST_WORKER_DESCENDANT_PID_FILE": str(descendant_pid_path),
            }
        )
        ...
        try:
            ...
            check(
                "Preparing one-shot local ANI command" in local_cli.stderr,
            ...
            initialize_params = {
                ...
                "capabilities": {},
                "clientInfo": {"name": "ani-daemon-smoke", "version": "1"},
            }
            c1.send(
            ...
            mismatched_env["ANI_CACHE_DIR"] = str(mismatched_cache)
            ...
            check(
                ...
                "mismatched ANI_CACHE_DIR unexpectedly joined the account daemon",
            )
            check(
                "active account daemon uses a different cache directory"
                in mismatched_result.stderr
                and "ANI_CACHE_DIR" in mismatched_result.stderr,
            ...
            # even if the original ANI_CACHE_DIR symlink is retargeted later.
            ...
            conflict_binary = tmpdir / "ani-conflict"
            ...
            if sys.platform == "darwin":
                ...
                signed = subprocess.run(
                    [
                        ...
                        "-",
                        "--identifier",
                        "com.jnani1972.ani.daemon-smoke-conflict",
                        str(conflict_binary),
                    ],
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE,
            ...
            cli_first_env["ANI_TEST_WORKER_DESCENDANT_PID_FILE"] = str(
                cli_first_pid_path
            )
            cli_first_env["ANI_TEST_WORKER_PROJECT_LOCK_PID_FILE"] = str(
                cli_first_lock_owner_path
            )
            ...
            def competitor_waiting_or_exited():
                try:
                    ...
                    text = ""
                return (
                    "Waiting for another ANI mutation of smoke-cli-first" in text
            ...
            check(
                ...
                and "Waiting for another ANI mutation of smoke-cli-first"
            ...
            install_target = install_dir / "ani"
            ...
            activation_local_env["ANI_TEST_WORKER_DESCENDANT_PID_FILE"] = str(
                install_pid_path
            )
            activation_local_env["ANI_TEST_WORKER_PROJECT_LOCK_PID_FILE"] = str(
            ...
            # the next daemon. ANI_DOWNLOAD_URL points at this file:// fixture,
            ...
            update_env = env.copy()
            update_env["ANI_DOWNLOAD_URL"] = create_local_update_release(
