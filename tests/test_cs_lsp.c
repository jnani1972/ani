...
static ANIFileResult *extract_cs(const char *source) {
    return ani_extract_file(source, (int)strlen(source), ANI_LANG_CSHARP, "test", "main.cs", 0,
                            NULL, NULL);
}
...
static int count_resolved_with_strategy(const ANIFileResult *r, const char *strategy) {
    int n = 0;
    for (int i = 0; i < r->resolved_calls.count; i++) {
        const ANIResolvedCall *rc = &r->resolved_calls.items[i];
...
TEST(cslsp_local_method_via_new) {
    const char *src =
        ...
        "    }\n"
        "}\n";
    ANIFileResult *r = extract_cs(src);
    ASSERT(r);
...
TEST(cslsp_ordinary_same_leaf_calls_join_by_exact_site) {
    ...
    ANIFileResult *r = extract_cs(source);
    ASSERT_NOT_NULL(r);
