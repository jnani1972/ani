...
static ANILogLevel g_prev_log_level;

static void setup_artifact_test(void) {
    snprintf(g_tmpdir, sizeof(g_tmpdir), "%s/ani_test_artifact_XXXXXX", ani_tmpdir());
    ani_mkdtemp(g_tmpdir);

    snprintf(g_repo, sizeof(g_repo), "%s/repo", g_tmpdir);
    ani_mkdir_p(g_repo, 0755);
...
static void create_test_db(const char *path) {
    ...
    ani_store_exec(s, "INSERT OR IGNORE INTO projects(name, indexed_at, root_path) "
                      "VALUES('test-proj', '2026-01-01', '/tmp/test');");

    ani_store_exec(s, "INSERT INTO nodes(project, label, name, qualified_name, file_path) "
                      "VALUES('test-proj', 'Function', 'foo', 'test-proj.foo', 'main.c');");
    ani_store_exec(s, "INSERT INTO nodes(project, label, name, qualified_name, file_path) "
                      "VALUES('test-proj', 'Function', 'bar', 'test-proj.bar', 'main.c');");

    ani_store_exec(s, "INSERT INTO edges(project, source_id, target_id, type) "
...
static void capture_logs_start(void) {
    g_log_capture[0] = '\0';
    g_prev_log_level = ani_log_get_level();
    ani_log_set_level(ANI_LOG_DEBUG);
    ani_log_set_sink(capture_log_sink);
}

static const char *capture_logs_end(void) {
    ani_log_set_sink(NULL);
    ani_log_set_level(g_prev_log_level);
...
TEST(artifact_import_rejects_size_mismatch) {
    setup_artifact_test();
    create_test_db(g_db);
    ASSERT_EQ(ani_artifact_export(g_db, g_repo, "test-proj", ANI_ARTIFACT_FAST), 0);

    char meta[1024];
    snprintf(meta, sizeof(meta), "%s/.ani/artifact.json", g_repo);
    ASSERT_TRUE(
    ...
    int rc = ani_artifact_import(g_repo, import_db);
...
TEST(artifact_export_fast_roundtrip) {
    ...
    int rc = ani_artifact_export(g_db, g_repo, "test-proj", ANI_ARTIFACT_FAST);
    ASSERT_EQ(rc, 0);

    /* Verify artifact files exist */
    char zst[1024];
    snprintf(zst, sizeof(zst), "%s/.ani/graph.db.zst", g_repo);
    struct stat st;
    ...
    char import_db[1024];
    snprintf(import_db, sizeof(import_db), "%s/imported.db", g_tmpdir);
    rc = ani_artifact_import(g_repo, import_db);
    ASSERT_EQ(rc, 0);

    /* Verify imported DB has correct data */
    ani_store_t *s = ani_store_open_path(import_db);
    ASSERT_NOT_NULL(s);
    int nodes = ani_store_count_nodes(s, "test-proj");
    int edges = ani_store_count_edges(s, "test-proj");
    ASSERT_EQ(nodes, 2);
    ASSERT_EQ(edges, 1);
    ani_store_close(s);
...
TEST(artifact_export_best_roundtrip) {
    ...
    int rc = ani_artifact_export(g_db, g_repo, "test-proj", ANI_ARTIFACT_BEST);
    ASSERT_EQ(rc, 0);

    /* Source DB should be untouched (VACUUM INTO doesn't modify source) */
    ani_store_t *src = ani_store_open_path(g_db);
    ASSERT_NOT_NULL(src);
    ASSERT_EQ(ani_store_count_nodes(src, "test-proj"), 2);
    ani_store_close(src);
    ...
    ani_store_t *s = ani_store_open_path(import_db);
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(ani_store_count_nodes(s, "test-proj"), 2);
    ASSERT_EQ(ani_store_count_edges(s, "test-proj"), 1);
    ani_store_close(s);
...
TEST(artifact_exists_check) {
    ...
    /* No artifact yet */
    ASSERT_FALSE(ani_artifact_exists(g_repo));

    /* Export creates the artifact */
    ani_artifact_export(g_db, g_repo, "test-proj", ANI_ARTIFACT_FAST);
    ASSERT_TRUE(ani_artifact_exists(g_repo));
...
TEST(artifact_commit_hash) {
    ...
    char *commit = ani_artifact_commit(g_repo);
...
TEST(artifact_gitattributes_created) {
    ...
    char ga[1024];
    snprintf(ga, sizeof(ga), "%s/.ani/.gitattributes", g_repo);
    struct stat st;
    ...
    (void)fclose(gaf);
    ASSERT_TRUE(rd > 0);
    ASSERT_NOT_NULL(strstr(content, ANI_ARTIFACT_FILENAME " binary merge=ours"));
...
TEST(artifact_export_rename_failure_logs_specific_error) {
    ...
    char art_dir[1024];
    snprintf(art_dir, sizeof(art_dir), "%s/.ani", g_repo);
    ani_mkdir_p(art_dir, 0755);

    char zst[1024];
    snprintf(zst, sizeof(zst), "%s/graph.db.zst", art_dir);
    ani_mkdir_p(zst, 0755);
    ...
    ASSERT_NEQ(rc, 0);
    ASSERT_FALSE(ani_artifact_exists(g_repo));
    ASSERT_NOT_NULL(ani_artifact_export_last_error());
    ASSERT(strstr(ani_artifact_export_last_error(), "write_artifact") != NULL);
    ASSERT(strstr(ani_artifact_export_last_error(), "rename_temp") != NULL);
...
TEST(pipeline_persistence_export_failure_returns_error) {
    ...
    ani_pipeline_t *p = ani_pipeline_new(g_repo, g_db, ANI_MODE_FAST);
    ASSERT_NOT_NULL(p);
    ani_pipeline_set_persistence(p, true);
...
TEST(artifact_null_safety) {
    ASSERT_NEQ(ani_artifact_export(NULL, "/tmp", "p", 0), 0);
    ASSERT_NEQ(ani_artifact_export("/tmp/x.db", NULL, "p", 0), 0);
    ASSERT_NEQ(ani_artifact_import(NULL, "/tmp/x.db"), 0);
    ASSERT_NEQ(ani_artifact_import("/tmp", NULL), 0);
    ASSERT_FALSE(ani_artifact_exists(NULL));
    ASSERT_NULL(ani_artifact_commit(NULL));
    ASSERT_EQ(ani_artifact_reconcile_hashes(NULL, "/tmp/x.db", "p"), -1);
    ASSERT_EQ(ani_artifact_reconcile_hashes("/tmp", NULL, "p"), -1);
    ASSERT_EQ(ani_artifact_reconcile_hashes("/tmp", "/tmp/x.db", NULL), -1);
    PASS();
}

/* ── git shell-out path safety ────────────────────────────────────────────────
 *
 * artifact.c shells out to git via ani_popen with the repo path interpolated into
 ...
 * quotes; ani_artifact_repo_path_is_shell_safe() is the guard. Rejecting quotes and
...
TEST(artifact_repo_path_shell_safe_accepts_plain_and_spaced) {
    ASSERT_TRUE(ani_artifact_repo_path_is_shell_safe("/home/user/repo"));
    ASSERT_TRUE(ani_artifact_repo_path_is_shell_safe("C:/Users/me/repo"));
    ASSERT_TRUE(ani_artifact_repo_path_is_shell_safe("/home/user/my repo")); /* space OK */
    PASS();
}

TEST(artifact_repo_path_shell_safe_rejects_injection) {
    ASSERT_FALSE(ani_artifact_repo_path_is_shell_safe(NULL));
    ASSERT_FALSE(ani_artifact_repo_path_is_shell_safe("it's"));        /* single quote */
    ASSERT_FALSE(ani_artifact_repo_path_is_shell_safe("a\"b"));        /* double quote */
    ASSERT_FALSE(ani_artifact_repo_path_is_shell_safe("x; rm -rf /")); /* command sep */
    ASSERT_FALSE(ani_artifact_repo_path_is_shell_safe("$(whoami)"));   /* substitution */
    ASSERT_FALSE(ani_artifact_repo_path_is_shell_safe("a`id`b"));      /* backtick */
    ASSERT_FALSE(ani_artifact_repo_path_is_shell_safe("a|b"));         /* pipe */
    PASS();
}
...
TEST(artifact_repo_path_shell_safe_rejects_cmd_metachars_on_windows) {
    ...
    ASSERT_FALSE(ani_artifact_repo_path_is_shell_safe("C:/a%USERPROFILE%b"));
    ASSERT_FALSE(ani_artifact_repo_path_is_shell_safe("C:/a!b"));
    ASSERT_FALSE(ani_artifact_repo_path_is_shell_safe("C:/a^b"));
#else
    /* POSIX shells treat % ! ^ literally inside double quotes — allowed. */
    ASSERT_TRUE(ani_artifact_repo_path_is_shell_safe("/a%b"));
    ASSERT_TRUE(ani_artifact_repo_path_is_shell_safe("/a^b"));
#endif
    PASS();
}
...
TEST(artifact_fast_export_snapshots_live_wal_store) {
    ...
    ani_store_t *s = ani_store_open_path(g_db);
    ASSERT_NOT_NULL(s);
    ...
    for (int i = 0; i < WAL_NODES; i++) {
        ...
        ani_node_t n = {.project = "test-proj",
                        .label = "Function",
                        .name = name,
                        .qualified_name = qn,
                        .file_path = "mod.py",
                        .start_line = i + 1,
                        .end_line = i + 2};
        ASSERT_TRUE(ani_store_upsert_node(s, &n) > 0);
    }
    ...
    ASSERT_EQ(ani_artifact_import(g_repo, import_db), 0);

    ani_store_t *imp = ani_store_open_path(import_db);
    ASSERT_NOT_NULL(imp);
    /* Torn snapshot = the WAL-resident rows are missing. */
    ASSERT_EQ(ani_store_count_nodes(imp, "test-proj"), WAL_NODES);
    ani_store_close(imp);
    PASS();
}
...
TEST(store_deep_integrity_detects_page_corruption) {
    ...
    ani_store_t *s = ani_store_open_path(db2);
    ASSERT_NOT_NULL(s);
    ani_store_upsert_project(s, "deep", "/tmp/deep");
    ...
    ani_store_t *ok = ani_store_open_path(db2);
    ASSERT_NOT_NULL(ok);
    ASSERT_TRUE(ani_store_check_integrity_deep(ok));
    ani_store_close(ok);
    ...
    ani_store_t *bad = ani_store_open_path(db2);
    ASSERT_NOT_NULL(bad);
    ASSERT_FALSE(ani_store_check_integrity_deep(bad));
    ani_store_close(bad);
    PASS();
}
...
/* True iff <repo>/.ani/artifact.json contains substr. */
static bool meta_contains(const char *repo, const char *substr) {
    char path[1024];
    snprintf(path, sizeof(path), "%s/.ani/artifact.json", repo);
...
static int64_t row_mtime(ani_file_hash_t *rows, int n, const char *rel) {
    for (int i = 0; i < n; i++) {
...
static bool stamp_row_mtime(const char *db, const char *proj, const char *rel, int64_t sentinel,
    ...
    int rc = ani_store_upsert_file_hash(s, proj, rel, sha ? sha : "", sentinel, 1);
    ani_store_close(s);
    return rc == ANI_STORE_OK;
}
 ...
 * on the thread that ran the pipeline (ani_pipeline_run exports inline). */
static void describe_missing_marker(const char *repo) {
    const char *blocker = ani_artifact_reconcile_basis_last_blocker();
    char meta[1152];
    snprintf(meta, sizeof(meta), "%s/.ani/artifact.json", repo);
    struct stat st;
    build_repo_errf(
 ...
 * -> reconcile_basis marker set), and commit .ani so a clone receives
...
static char *build_trusted_artifact_repo(void) {
    ...
    ani_pipeline_t *p = ani_pipeline_new(g_repo, g_db, ANI_MODE_FAST);
    if (!p) {
        build_repo_errf("step=pipeline_new: ani_pipeline_new(repo=%s, db=%s) returned NULL", g_repo,
                        g_db);
        return NULL;
    }
    ani_pipeline_set_persistence(p, true);
    ...
    ani_pipeline_free(p);
    if (rc != 0) {
        build_repo_errf("step=pipeline_run: ani_pipeline_run(repo=%s) rc=%d", g_repo, rc);
        return NULL;
    }
    char *project = ani_project_name_from_path(g_repo);
    if (!project) {
        build_repo_errf("step=project_name: ani_project_name_from_path(%s) returned NULL", g_repo);
        return NULL;
    }
...
TEST(artifact_export_marks_clean_basis) {
    ...
    char *proj = ani_project_name_from_path(g_repo);
     ...
     * the tree is non-clean outside .ani (and the a.rs hash row no
     * longer matches disk). */
    write_text_file(src, "pub fn dirty() {}\n");
    ASSERT_EQ(ani_artifact_export(g_db, g_repo, proj, ANI_ARTIFACT_FAST), 0);
...
TEST(artifact_reconcile_restamps_unchanged) {
    ...
    ASSERT_EQ(ani_artifact_import(repoB, dbB), 0);
    ...
    int restamped = ani_artifact_reconcile_hashes(repoB, dbB, proj);
    ...
    ani_store_t *s = ani_store_open_path(dbB);
    ASSERT_NOT_NULL(s);
    ani_file_hash_t *rows = NULL;
    int n = 0;
    ASSERT_EQ(ani_store_get_file_hashes(s, proj, &rows, &n), 0);
    char c_path[1152];
    ...
    ani_store_free_file_hashes(rows, n);
    ani_store_close(s);
...
TEST(artifact_reconcile_skips_untracked_rows) {
    ...
    /* B: a gitignored-yet-indexed file (the .aniignore-negation shape, #500).
    ...
    ASSERT_EQ(ani_artifact_reconcile_hashes(repoB, dbB, proj), 5);

    ani_store_t *s = ani_store_open_path(dbB);
    ASSERT_NOT_NULL(s);
    ani_file_hash_t *rows = NULL;
    int n = 0;
    ASSERT_EQ(ani_store_get_file_hashes(s, proj, &rows, &n), ANI_STORE_OK);
...
TEST(artifact_reconcile_skips_untrusted_metadata) {
    ...
    char meta[1152];
    snprintf(meta, sizeof(meta), "%s/.ani/artifact.json", repoB);
    ...
    ASSERT_EQ(ani_artifact_reconcile_hashes(repoB, dbB, proj), -1);

    /* Rows untouched: still the foreign value captured before. */
    s = ani_store_open_path(dbB);
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(ani_store_get_file_hashes(s, proj, &rows, &n), ANI_STORE_OK);
...
TEST(artifact_reconcile_skips_without_git) {
    ...
    ASSERT_EQ(ani_rename_replace(gitdir, disabled), 0);
