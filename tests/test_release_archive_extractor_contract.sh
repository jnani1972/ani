...
FIX="$(mktemp -d "${TMPDIR:-/tmp}/ani-extract-contract.XXXXXX")"
trap 'rm -rf "$FIX"' EXIT
...
ARCHIVES = tuple(f"ani-{t}.tar.gz" for t in UNIX) + \
           tuple(f"ani-{t}.zip" for t in WINDOWS) + \
           tuple(f"ani-{t}.mcpb" for t in MCPB)
...
def members(archive):
    ...
    if archive.endswith(".mcpb"):
        binary = "server/ani.exe" if windows else "server/ani"
        return {
    ...
    binary = "ani.exe" if windows else "ani"
...
if result.returncode != 0:
    ...
    assoc_meta, assoc = read_manifest(out / "associations.tsv",
                                      "ani-release-scan-associations-v3")
    set_meta, scan_set = read_manifest(out / "scan-set.tsv", "ani-release-scan-set-v2")
...
BROKEN_MCPB = "ani-darwin-arm64.mcpb"
...
for label, slug, manifest_bytes, expect in (
    ...
    ("manifest without a version", "m2",
     make_manifest("server/ani").replace(b'"version": "0.0.0-test", ', b""),
     "lacks a version"),
    ...
    ("manifest command not targeting the entry_point", "m4",
     make_manifest("server/ani").replace(
         b'${__dirname}/server/ani', b"/usr/bin/env"),
