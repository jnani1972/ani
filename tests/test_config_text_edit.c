...
#define CTE_BEGIN "<!-- ani:start -->"
#define CTE_END "<!-- ani:end -->"
...
static int cte_fixture(char *dir, size_t dir_size, char *path, size_t path_size) {
    char *created = th_mktempdir("ani_text_edit");
    if (!created) {
        return -1;
    }
...
static size_t cte_temp_count(const char *dir) {
    ...
    while ((entry = ani_readdir(directory)) != NULL) {
        if (strstr(entry->name, ".ani-text-") != NULL) {
            count++;
        }
    }
...
static void cte_replace_closed_temp(const char *path, void *context) {
    cte_precommit_change_t *change = (cte_precommit_change_t *)context;
    if (!change->backup_path || ani_rename_replace(path, change->backup_path) != 0) {
        change->result = -1;
        return;
    }
    change->result = th_write_file(path, change->content);
...
TEST(config_text_managed_insert_preserves_bom_comments_and_is_idempotent) {
    ...
    ASSERT_EQ(ani_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "Use search_graph first.\n"),
              0);
...
TEST(config_text_managed_replace_preserves_crlf_surroundings) {
    ...
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ASSERT_EQ(cte_write_bytes(path, original, sizeof(original) - 1U), 0);
    ASSERT_EQ(ani_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "new one\nnew two"), 0);
    ASSERT(cte_assert_bytes(path, expected, sizeof(expected) - 1U));
    th_cleanup(dir);
    PASS();
}
...
TEST(config_text_managed_no_final_newline_round_trip) {
    ...
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ASSERT_EQ(cte_write_bytes(path, original, sizeof(original) - 1U), 0);
    ASSERT_EQ(ani_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned"), 0);
    ASSERT(cte_assert_bytes(path, installed, sizeof(installed) - 1U));
    ASSERT_EQ(ani_text_remove_managed_block(path, CTE_BEGIN, CTE_END), 0);
...
TEST(config_text_managed_malformed_markers_fail_closed) {
    ...
    for (size_t i = 0U; i < sizeof(cases) / sizeof(cases[0]); i++) {
        ...
        ASSERT_EQ(ani_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "new"), -1);
        ASSERT(cte_assert_bytes(path, cases[i], len));
        ASSERT_EQ(ani_text_remove_managed_block(path, CTE_BEGIN, CTE_END), -1);
        ASSERT(cte_assert_bytes(path, cases[i], len));
    }
    th_cleanup(dir);
    PASS();
...
TEST(config_text_managed_rejects_unsafe_markers_and_owned_content) {
    ...
    ASSERT_EQ(ani_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "nested " CTE_BEGIN), -1);
    ASSERT_EQ(ani_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "nested " CTE_END), -1);
    ASSERT_EQ(ani_text_upsert_managed_block(path, "same", "same", "owned"), -1);
    ASSERT_EQ(ani_text_upsert_managed_block(path, "bad\nmarker", CTE_END, "owned"), -1);
    ASSERT_EQ(ani_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "bad\x01text"), -1);
...
TEST(config_text_owned_document_write_remove_exact_only) {
    ...
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ASSERT_EQ(ani_text_write_owned_document(path, owned), 0);
    ASSERT_EQ(ani_text_create_owned_document(path, owned), -1);
    ASSERT_EQ(ani_text_ensure_owned_document(path, owned), 0);
    ASSERT(cte_assert_bytes(path, owned, sizeof(owned) - 1U));
    ASSERT_EQ(ani_text_write_owned_document(path, owned), 0);
    ASSERT_EQ(th_write_file(path, "# User changed this\n"), 0);
    ASSERT_EQ(ani_text_ensure_owned_document(path, owned), -1);
    ASSERT_EQ(ani_text_remove_owned_document(path, owned), 1);
    ASSERT(cte_assert_bytes(path, "# User changed this\n", strlen("# User changed this\n")));
    ASSERT_EQ(ani_text_write_owned_document(path, owned), 0);
    ASSERT_EQ(ani_text_remove_owned_document(path, owned), 0);
    ASSERT(!cte_path_exists(path));
    ASSERT_EQ(ani_text_ensure_owned_document(path, owned), 0);
    ASSERT(cte_assert_bytes(path, owned, sizeof(owned) - 1U));
    ASSERT_EQ(ani_text_remove_owned_document(path, owned), 0);
    ASSERT_EQ(ani_text_create_owned_document(path, owned), 0);
    ASSERT(cte_assert_bytes(path, owned, sizeof(owned) - 1U));
    ASSERT_EQ(ani_text_remove_owned_document(path, owned), 0);
    ASSERT_EQ(ani_text_remove_owned_document(path, owned), 0);
    th_cleanup(dir);
...
TEST(config_text_owned_document_migrates_exact_releases_only) {
    char dir[CTE_PATH_CAP];
    char path[CTE_PATH_CAP];
    const char *current = "name: ani\ntier: verify\n";
    const char *released[] = {"name: ani\n", "name: ani\nlegacy: 2\n"};
    const char *modified = "name: ani\nuser-note: keep\n";
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);

    ASSERT_EQ(ani_text_migrate_owned_document(path, current, released, 2U), 0);
    ...
    ASSERT_EQ(ani_text_migrate_owned_document(path, current, released, 2U), 1);
    ASSERT(cte_assert_bytes(path, modified, strlen(modified)));
    ASSERT_EQ(ani_text_migrate_owned_document_mode(path, current, released, 2U, 04755U), -1);
    ASSERT(cte_assert_bytes(path, modified, strlen(modified)));
    ASSERT_EQ(ani_text_migrate_owned_document_mode(path, current, released, 2U, 0200U), -1);
    ASSERT(cte_assert_bytes(path, modified, strlen(modified)));

    ASSERT_EQ(ani_text_remove_owned_document_any(path, current, released, 2U), 1);
    ASSERT(cte_assert_bytes(path, modified, strlen(modified)));
    ASSERT_EQ(th_write_file(path, released[0]), 0);
    ASSERT_EQ(ani_text_remove_owned_document_any(path, current, released, 2U), 0);
...
TEST(config_text_rejects_invalid_utf8_and_controls) {
    ...
    ASSERT_EQ(ani_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned"), -1);
    ...
    ASSERT_EQ(ani_text_write_owned_document(path, "replacement\n"), -1);
    ...
    ASSERT_EQ(ani_text_write_owned_document(path, "bad\x7ftext"), -1);
    th_cleanup(dir);
    PASS();
}
...
TEST(config_text_rejects_non_regular_paths) {
    ...
    ASSERT_EQ(ani_text_write_owned_document(path, "owned\n"), -1);
...
TEST(config_text_rejects_links_privileged_mode_and_preserves_metadata) {
    ...
    ASSERT_EQ(ani_text_write_owned_document(path, "owned\n"), -1);
    ASSERT_EQ(ani_text_migrate_owned_document_mode(path, "owned\n", NULL, 0U, 0755U), -1);
    ASSERT_EQ(ani_unlink(path), 0);
...
TEST(config_text_owned_document_mode_publishes_exact_bytes_atomically) {
    ...
    ASSERT_EQ(ani_text_migrate_owned_document_mode(path, current, released, 1U, 0755U), 0);
    struct stat state;
    ASSERT_EQ(stat(path, &state), 0);
    ...
    ASSERT_EQ(chmod(path, 0640), 0);
    ASSERT_EQ(ani_text_migrate_owned_document_mode(path, current, released, 1U, 0755U), 1);
    ASSERT_EQ(stat(path, &state), 0);
...
TEST(config_text_owned_document_mode_rejects_prepublish_replacement) {
    ...
    ani_text_set_prepublish_hook_for_testing(cte_change_before_commit, &race);
    int result = ani_text_migrate_owned_document_mode(path, current, NULL, 0U, 0755U);
    ani_text_set_prepublish_hook_for_testing(NULL, NULL);
...
TEST(config_text_owned_document_mode_rejects_closed_temp_replacement) {
    ...
    ani_text_set_temp_closed_hook_for_testing(cte_replace_closed_temp, &race);
    int result = ani_text_migrate_owned_document_mode(path, current, NULL, 0U, 0755U);
    ani_text_set_temp_closed_hook_for_testing(NULL, NULL);
...
TEST(config_text_rejects_stale_content_and_identity) {
    ...
    ani_text_set_precommit_hook_for_testing(cte_change_before_commit, &content_change);
    int result = ani_text_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned");
    ani_text_set_precommit_hook_for_testing(NULL, NULL);
    ...
    ani_text_set_precommit_hook_for_testing(cte_change_before_commit, &identity_change);
    result = ani_text_write_owned_document(path, "owned\n");
    ani_text_set_precommit_hook_for_testing(NULL, NULL);
...
TEST(config_text_missing_target_race_does_not_replace_winner) {
    ...
    cte_precommit_change_t race = {
        .content = "winner\n", .backup_path = NULL, .replace_identity = 0, .result = -1};
    ani_text_set_precommit_hook_for_testing(cte_change_before_commit, &race);
    int result = ani_text_write_owned_document(path, "owned\n");
    ani_text_set_precommit_hook_for_testing(NULL, NULL);
    ASSERT_EQ(race.result, 0);
    ASSERT_EQ(result, -1);
...
TEST(config_text_exact_remove_rechecks_stale_snapshot) {
    ...
    cte_precommit_change_t change = {
        .content = "winner\n", .backup_path = NULL, .replace_identity = 0, .result = -1};
    ani_text_set_precommit_hook_for_testing(cte_change_before_commit, &change);
    int result = ani_text_remove_owned_document(path, owned);
    ani_text_set_precommit_hook_for_testing(NULL, NULL);
...
TEST(config_text_write_owned_document_if_unchanged_rejects_stale_snapshot) {
    ...
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ASSERT_EQ(th_write_file(path, original), 0);
    ASSERT_EQ(ani_text_write_owned_document_if_unchanged(path, updated, original, strlen(original)),
              0);
    ...
    ASSERT_EQ(ani_text_write_owned_document_if_unchanged(path, updated, NULL, 0U), -1);
    ASSERT(cte_assert_bytes(path, winner, strlen(winner)));
    ASSERT_EQ(cte_temp_count(dir), 0U);
    th_cleanup(dir);
    PASS();
