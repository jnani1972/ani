 ...
 */
static ani_store_t *setup_search_store(int64_t *ids) {
    ...
    ids[0] = ani_store_upsert_node(s, &n1);
    ids[1] = ani_store_upsert_node(s, &n2);
    ids[2] = ani_store_upsert_node(s, &n3);
...
TEST(store_search_empty_label_ignored) {
    ...
    ani_search_params_t empty_label = {.project = "test",
                                       .name_pattern = ".*Submit.*",
                                       .label = "",
                                       .min_degree = -1,
                                       .max_degree = -1};
    ani_search_output_t out = {0};
    int rc = ani_store_search(s, &empty_label, &out);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(out.count, 1);
    ASSERT_STR_EQ(out.results[0].node.name, "SubmitOrder");
    ani_store_search_free(&out);
...
TEST(store_search_pagination) {
    ...
    /* limit=1, offset=1 */
    params.offset = 1;
    rc = ani_store_search(s, &params, &out);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(out.count, 1);
    ASSERT_EQ(out.total, 3);
    ani_store_search_free(&out);
...
static int search_result_index_by_name(const ani_search_output_t *out, const char *name) {
...
TEST(store_search_degree_counts_inherits) {
    ...
    ani_node_t parent = {.project = "test",
                         .label = "Class",
                         .name = "AbstractAttachmentDto",
                         .qualified_name = "test.AbstractAttachmentDto"};
    int64_t parent_id = ani_store_upsert_node(s, &parent);
    ...
    for (int i = 0; i < 3; i++) {
        ani_node_t child = {.project = "test",
                            .label = "Class",
                            .name = child_names[i],
                            .qualified_name = child_qns[i]};
        int64_t child_id = ani_store_upsert_node(s, &child);
        ASSERT_GT(child_id, 0);
        ani_edge_t edge = {
            .project = "test", .source_id = child_id, .target_id = parent_id, .type = "INHERITS"};
        ASSERT_GT(ani_store_insert_edge(s, &edge), 0);
    }

    int in_deg = 0;
    int out_deg = 0;
    ani_store_node_degree(s, parent_id, &in_deg, &out_deg);
    ASSERT_EQ(in_deg, 0);
    ASSERT_EQ(out_deg, 0);
...
TEST(store_search_degree_calls_plus_inherits_no_double_count) {
    ...
    ani_node_t fn = {.project = "test",
                     .label = "Function",
                     .name = "normalizeAttachment",
                     .qualified_name = "test.normalizeAttachment"};
    int64_t child_id = ani_store_upsert_node(s, &child);
    int64_t parent_id = ani_store_upsert_node(s, &parent);
    int64_t fn_id = ani_store_upsert_node(s, &fn);
    ASSERT_GT(child_id, 0);
    ASSERT_GT(parent_id, 0);
    ASSERT_GT(fn_id, 0);
    ...
    ani_edge_t inherits = {
        .project = "test", .source_id = child_id, .target_id = parent_id, .type = "INHERITS"};
    ASSERT_GT(ani_store_insert_edge(s, &calls), 0);
    ASSERT_GT(ani_store_insert_edge(s, &inherits), 0);

    int in_deg = 0;
    int out_deg = 0;
    ani_store_node_degree(s, child_id, &in_deg, &out_deg);
    ASSERT_EQ(in_deg, 0);
    ASSERT_EQ(out_deg, 1);
...
TEST(store_search_isolated_node_zero_degree) {
    ...
    int64_t node_id = ani_store_upsert_node(s, &node);
    ASSERT_GT(node_id, 0);

    int in_deg = 0;
    int out_deg = 0;
    ani_store_node_degree(s, node_id, &in_deg, &out_deg);
    ASSERT_EQ(in_deg, 0);
    ASSERT_EQ(out_deg, 0);
...
TEST(store_search_all) {
    ...
    ani_search_params_t params = {.project = "test", .min_degree = -1, .max_degree = -1};
    ani_search_output_t out = {0};
    int rc = ani_store_search(s, &params, &out);
    ASSERT_EQ(rc, ANI_STORE_OK);
...
TEST(store_bfs_outbound) {
    ...
    ids[0] = ani_store_upsert_node(s, &na);
    ids[1] = ani_store_upsert_node(s, &nb);
    ids[2] = ani_store_upsert_node(s, &nc);
    ids[3] = ani_store_upsert_node(s, &nd);
    ...
    ani_edge_t e3 = {.project = "test", .source_id = ids[2], .target_id = ids[3], .type = "CALLS"};
    ...
    ani_traverse_result_t result = {0};
    int rc = ani_store_bfs(s, ids[0], "outbound", types, 1, 3, 100, &result);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_STR_EQ(result.root.name, "A");
    ASSERT_GTE(result.visited_count, 3); /* B, C, D */
    ani_store_traverse_free(&result);

    /* BFS with depth=1 */
    rc = ani_store_bfs(s, ids[0], "outbound", types, 1, 1, 100, &result);
    ASSERT_EQ(rc, ANI_STORE_OK);
...
TEST(store_bfs_inbound) {
    ...
    const char *types[] = {"CALLS"};
    ani_traverse_result_t result = {0};
    int rc = ani_store_bfs(s, ids[2], "inbound", types, 1, 3, 100, &result);
    ASSERT_EQ(rc, ANI_STORE_OK);
...
TEST(store_bulk_write_mode) {
    ...
    for (int i = 0; i < 50; i++) {
        ...
        ani_node_t n = {.project = "test", .label = "Function", .name = name, .qualified_name = qn};
        ani_store_upsert_node(s, &n);
    }

    ani_store_create_indexes(s);
...
TEST(store_schema_info) {
    ...
    int rc = ani_store_get_schema(s, "test", &schema);
    ASSERT_EQ(rc, ANI_STORE_OK);
...
TEST(store_search_exclude_labels) {
    ...
    ani_search_params_t params2 = {.project = "test",
                                   .limit = 100,
                                   .min_degree = -1,
                                   .max_degree = -1,
                                   .exclude_labels = excl};
    ani_search_output_t out2 = {0};
    rc = ani_store_search(s, &params2, &out2);
    ASSERT_EQ(rc, ANI_STORE_OK);
...
TEST(store_dump_to_file) {
    ...
    /* Dump to temp file */
    char *td = th_mktempdir("ani_dump");
    char path[256];
    snprintf(path, sizeof(path), "%s/test.db", td);

    int rc = ani_store_dump_to_file(s, path);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ani_store_close(s);

    /* Open dumped file and verify data */
    ani_store_t *disk = ani_store_open_path(path);
    ASSERT_NOT_NULL(disk);

    ani_node_t found = {0};
    rc = ani_store_find_node_by_qn(disk, "test", "test.main.Hello", &found);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_STR_EQ(found.name, "Hello");
    ani_node_free_fields(&found);

    ani_store_close(disk);
    unlink(path);
    PASS();
}
...
TEST(store_bfs_cross_service) {
    ...
    ani_edge_t e = {.project = "test", .source_id = idA, .target_id = idB, .type = "HTTP_CALLS"};
    ani_store_insert_edge(s, &e);

    /* BFS from A with both CALLS and HTTP_CALLS */
    const char *types[] = {"CALLS", "HTTP_CALLS"};
    ani_traverse_result_t result = {0};
    int rc = ani_store_bfs(s, idA, "outbound", types, 2, 1, 200, &result);
    ASSERT_EQ(rc, ANI_STORE_OK);
...
TEST(store_bfs_depth_chain) {
    ...
    ani_edge_t e2 = {.project = "test", .source_id = idB, .target_id = idC, .type = "CALLS"};
    ani_edge_t e3 = {.project = "test", .source_id = idC, .target_id = idD, .type = "CALLS"};
    ani_store_insert_edge(s, &e1);
    ani_store_insert_edge(s, &e2);
    ani_store_insert_edge(s, &e3);
    ...
    const char *types[] = {"CALLS"};
    ani_traverse_result_t result = {0};
    int rc = ani_store_bfs(s, idA, "outbound", types, 1, 3, 100, &result);
    ASSERT_EQ(rc, ANI_STORE_OK);
...
TEST(store_hop_to_risk) {
    ASSERT_STR_EQ(ani_risk_label(ani_hop_to_risk(1)), "CRITICAL");
    ASSERT_STR_EQ(ani_risk_label(ani_hop_to_risk(2)), "HIGH");
    ASSERT_STR_EQ(ani_risk_label(ani_hop_to_risk(3)), "MEDIUM");
    ASSERT_STR_EQ(ani_risk_label(ani_hop_to_risk(4)), "LOW");
    ASSERT_STR_EQ(ani_risk_label(ani_hop_to_risk(5)), "LOW");
    ASSERT_STR_EQ(ani_risk_label(ani_hop_to_risk(10)), "LOW");
    PASS();
}
...
TEST(store_cross_service_detection) {
    ani_node_hop_t hops[1] = {{.node = {.id = 1}, .hop = 1}};

    ani_edge_info_t edges_http[1] = {
        {.from_name = "A", .to_name = "B", .type = "HTTP_CALLS"},
    };
    ani_impact_summary_t s1 = ani_build_impact_summary(hops, 1, edges_http, 1);
    ASSERT_TRUE(s1.has_cross_service);

    ani_edge_info_t edges_async[1] = {
        {.from_name = "A", .to_name = "B", .type = "ASYNC_CALLS"},
    };
    ani_impact_summary_t s2 = ani_build_impact_summary(hops, 1, edges_async, 1);
...
TEST(store_deduplicate_hops) {
    ...
    ani_node_hop_t *result = NULL;
    int count = 0;
    int rc = ani_deduplicate_hops(hops, 4, &result, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 3);
...
TEST(store_bfs_with_risk_labels) {
    ...
    ani_node_t nd = {
        .project = "test", .label = "Function", .name = "D", .qualified_name = "test.D"};
    int64_t idA = ani_store_upsert_node(s, &na);
    (void)ani_store_upsert_node(s, &nb);
    (void)ani_store_upsert_node(s, &nc);
    (void)ani_store_upsert_node(s, &nd);

    ani_edge_t e1 = {.project = "test", .source_id = idA, .target_id = idA + 1, .type = "CALLS"};
    ani_edge_t e2 = {
    ...
    const char *types[] = {"CALLS"};
    ani_traverse_result_t result = {0};
    int rc = ani_store_bfs(s, idA, "outbound", types, 1, 3, 200, &result);
    ASSERT_EQ(rc, ANI_STORE_OK);

    /* Deduplicate */
    ani_node_hop_t *deduped = NULL;
    int dcount = 0;
    ani_deduplicate_hops(result.visited, result.visited_count, &deduped, &dcount);
    ASSERT_EQ(dcount, 3);

    /* Verify risk labels */
    for (int i = 0; i < dcount; i++) {
        if (strcmp(deduped[i].node.name, "B") == 0)
            ASSERT_STR_EQ(ani_risk_label(ani_hop_to_risk(deduped[i].hop)), "CRITICAL");
        if (strcmp(deduped[i].node.name, "C") == 0)
            ASSERT_STR_EQ(ani_risk_label(ani_hop_to_risk(deduped[i].hop)), "HIGH");
        if (strcmp(deduped[i].node.name, "D") == 0)
            ASSERT_STR_EQ(ani_risk_label(ani_hop_to_risk(deduped[i].hop)), "MEDIUM");
    }

    /* Build summary */
    ani_impact_summary_t summary =
        ani_build_impact_summary(deduped, dcount, result.edges, result.edge_count);
...
TEST(store_bfs_reachability_is_not_trail_capped) {
    ...
    for (int i = 0; i < NODE_COUNT; i++) {
        ...
        int64_t leaf_id = ani_store_upsert_node(s, &leaf);
        ani_edge_t edge = {
    ...
    const char *types[] = {"CALLS"};
    ani_traverse_result_t result = {0};
    int rc = ani_store_bfs(s, root_id, "outbound", types, 1, 1, 5000, &result);
    ASSERT_EQ(rc, ANI_STORE_OK);
...
TEST(store_bfs_trail_warns_when_path_rows_are_truncated) {
    ...
    trail_log[0] = '\0';
    ani_log_set_sink(capture_trail_log);
    const char *types[] = {"CALLS"};
    ani_traverse_result_t result = {0};
    int rc = ani_store_bfs_trail(s, ids[0], "outbound", types, 1, 10, 5000, &result);
...
TEST(store_bfs_trail_preserves_deeper_match_under_hub_budget) {
    ...
    int64_t root_id = ani_store_upsert_node(s, &root);
    int64_t branch_id = ani_store_upsert_node(s, &branch);
    int64_t target_id = ani_store_upsert_node(s, &target);
    ...
    const char *types[] = {"CALLS"};
    ani_traverse_result_t result = {0};
    int rc = ani_store_bfs_trail(s, root_id, "outbound", types, 1, 2, 5000, &result);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_TRUE(result.truncated);
...
TEST(store_bfs_cross_service_summary) {
    ...
    ani_impact_summary_t summary = ani_build_impact_summary(result.visited, result.visited_count,
...
TEST(store_glob_to_like) {
    ...
    for (int i = 0; i < 8; i++) {
        char *got = ani_glob_to_like(tests[i].pattern);
        ASSERT_NOT_NULL(got);
    ...
    /* NULL returns NULL */
    ASSERT_NULL(ani_glob_to_like(NULL));
...
TEST(store_extract_like_hints) {
    ...
    n = ani_extract_like_hints(".*handler.*", hints, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(hints[0], "handler");
    free(hints[0]);

    /* Multiple segments: .*Order.*Handler.* → ["Order", "Handler"] */
    n = ani_extract_like_hints(".*Order.*Handler.*", hints, 16);
    ASSERT_EQ(n, 2);
    ...
    n = ani_extract_like_hints("^handleRequest$", hints, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(hints[0], "handleRequest");
    free(hints[0]);

    /* Too generic: .* → no hints */
    n = ani_extract_like_hints(".*", hints, 16);
    ASSERT_EQ(n, 0);

    /* Short literal: .*ab.* → "ab" is only 2 chars, below threshold */
    n = ani_extract_like_hints(".*ab.*", hints, 16);
    ASSERT_EQ(n, 0);

    /* Exactly 3 chars: .*abc.* → ["abc"] */
    n = ani_extract_like_hints(".*abc.*", hints, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(hints[0], "abc");
    free(hints[0]);

    /* Alternation: bail out */
    n = ani_extract_like_hints(".*foo|.*bar", hints, 16);
    ASSERT_EQ(n, 0);
    ...
    n = ani_extract_like_hints(".*test_.*helper.*", hints, 16);
    ASSERT_EQ(n, 2);
    ...
    /* NULL safety */
    n = ani_extract_like_hints(NULL, hints, 16);
    ASSERT_EQ(n, 0);
...
TEST(store_ensure_case_insensitive) {
    ASSERT_STR_EQ(ani_ensure_case_insensitive("handler"), "(?i)handler");
    ASSERT_STR_EQ(ani_ensure_case_insensitive("(?i)handler"), "(?i)handler");
    ASSERT_STR_EQ(ani_ensure_case_insensitive(".*Order.*"), "(?i).*Order.*");
    ASSERT_STR_EQ(ani_ensure_case_insensitive(""), "(?i)");
    PASS();
}
...
TEST(store_strip_case_flag) {
    ASSERT_STR_EQ(ani_strip_case_flag("(?i)handler"), "handler");
    ASSERT_STR_EQ(ani_strip_case_flag("handler"), "handler");
    ASSERT_STR_EQ(ani_strip_case_flag("(?i)(?i)double"), "(?i)double");
    PASS();
}
...
TEST(store_batch_count_degrees) {
    ...
    int in3[3], out3[3];
    int rc = ani_store_batch_count_degrees(s, ids3, 3, NULL, in3, out3);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ...
    int in2[2], out2[2];
    rc = ani_store_batch_count_degrees(s, ids2, 2, "CALLS", in2, out2);
    ASSERT_EQ(rc, ANI_STORE_OK);
...
TEST(store_glob_to_like_dot_and_brackets) {
    ...
    char *got = ani_glob_to_like("src/[abc]/*.ts");
    ASSERT_NOT_NULL(got);
...
TEST(store_glob_to_like_question_marks) {
    ...
    char *got = ani_glob_to_like("f???.txt");
    ASSERT_NOT_NULL(got);
...
TEST(store_extract_like_hints_null_out) {
    /* NULL out array */
    int n = ani_extract_like_hints(".*handler.*", NULL, 16);
    ASSERT_EQ(n, 0);
    PASS();
}
...
TEST(store_extract_like_hints_zero_max) {
    char *hints[4];
    int n = ani_extract_like_hints(".*handler.*", hints, 0);
    ASSERT_EQ(n, 0);
    PASS();
}
...
TEST(store_extract_like_hints_alternation_complex) {
    ...
    int n = ani_extract_like_hints("(foo|bar)baz", hints, 16);
    ASSERT_EQ(n, 0);
    PASS();
}
...
TEST(store_extract_like_hints_short_segments) {
    ...
    int n = ani_extract_like_hints(".*ab.*cd.*", hints, 16);
    ASSERT_EQ(n, 0);
    PASS();
}
...
TEST(store_extract_like_hints_complex_multi_segment) {
    ...
    int n = ani_extract_like_hints(".*Foo.*Bar.*", hints, 16);
    ASSERT_EQ(n, 2);
...
TEST(store_extract_like_hints_max_out_limit) {
    ...
    int n = ani_extract_like_hints(".*aaa.*bbb.*ccc.*ddd.*", hints, 2);
    ASSERT_EQ(n, 2);
...
TEST(store_extract_like_hints_escaped_chars) {
    ...
    int n = ani_extract_like_hints("\\.handler", hints, 16);
    ASSERT_EQ(n, 1);
    ASSERT_STR_EQ(hints[0], ".handler");
    free(hints[0]);
    PASS();
...
TEST(store_ensure_case_insensitive_null) {
    const char *result = ani_ensure_case_insensitive(NULL);
...
TEST(store_ensure_case_insensitive_already_ci) {
    ...
    ASSERT_STR_EQ(ani_ensure_case_insensitive("(?i).*Order.*"), "(?i).*Order.*");
    PASS();
}

TEST(store_ensure_case_insensitive_plain) {
    ASSERT_STR_EQ(ani_ensure_case_insensitive("FooBar"), "(?i)FooBar");
    PASS();
}

TEST(store_strip_case_flag_null) {
    const char *result = ani_strip_case_flag(NULL);
    ASSERT_STR_EQ(result, "");
    PASS();
}

TEST(store_strip_case_flag_no_flag) {
    ASSERT_STR_EQ(ani_strip_case_flag("plain_pattern"), "plain_pattern");
    PASS();
}

TEST(store_strip_case_flag_empty) {
    ASSERT_STR_EQ(ani_strip_case_flag(""), "");
    PASS();
}
...
TEST(store_qn_to_top_package_single_segment) {
    ASSERT_STR_EQ(ani_qn_to_top_package("nodots"), "");
    PASS();
}

TEST(store_qn_to_top_package_two_segments) {
    /* project.dir — returns "dir" */
    ASSERT_STR_EQ(ani_qn_to_top_package("proj.dir"), "dir");
    PASS();
}

TEST(store_qn_to_top_package_many_segments) {
    /* Always returns segment[1] regardless of depth */
    ASSERT_STR_EQ(ani_qn_to_top_package("proj.dir.sub.Func"), "dir");
    PASS();
}

TEST(store_qn_to_top_package_null) {
    ASSERT_STR_EQ(ani_qn_to_top_package(NULL), "");
    PASS();
}

TEST(store_is_test_file_various) {
    /* Positive cases */
    ASSERT_TRUE(ani_is_test_file_path("test_handler.py"));
    ASSERT_TRUE(ani_is_test_file_path("handler_test.go"));
    ASSERT_TRUE(ani_is_test_file_path("handler.test.ts"));
    ASSERT_FALSE(ani_is_test_file_path("handler.spec.ts")); /* "spec" not "test" — no match */
    ASSERT_TRUE(ani_is_test_file_path("src/__tests__/handler.js"));
    ASSERT_TRUE(ani_is_test_file_path("tests/unit/handler.py"));

    /* Negative cases */
    ASSERT_FALSE(ani_is_test_file_path("handler.go"));
    ASSERT_FALSE(ani_is_test_file_path("main.py"));
    ASSERT_FALSE(ani_is_test_file_path("service.ts"));

    /* Edge: NULL and empty */
    ASSERT_FALSE(ani_is_test_file_path(NULL));
    ASSERT_FALSE(ani_is_test_file_path(""));
    PASS();
}
...
TEST(store_hop_to_risk_all_levels) {
    ...
    ASSERT_EQ(ani_hop_to_risk(0), ANI_RISK_LOW);
    /* hop 1 → CRITICAL */
    ASSERT_EQ(ani_hop_to_risk(1), ANI_RISK_CRITICAL);
    /* hop 2 → HIGH */
    ASSERT_EQ(ani_hop_to_risk(2), ANI_RISK_HIGH);
    /* hop 3 → MEDIUM */
    ASSERT_EQ(ani_hop_to_risk(3), ANI_RISK_MEDIUM);
    /* hop 4+ → LOW */
    ASSERT_EQ(ani_hop_to_risk(4), ANI_RISK_LOW);
    ASSERT_EQ(ani_hop_to_risk(100), ANI_RISK_LOW);
    /* negative → LOW (default) */
    ASSERT_EQ(ani_hop_to_risk(-1), ANI_RISK_LOW);
    PASS();
}

TEST(store_risk_label_all_levels) {
    ASSERT_STR_EQ(ani_risk_label(ANI_RISK_CRITICAL), "CRITICAL");
    ASSERT_STR_EQ(ani_risk_label(ANI_RISK_HIGH), "HIGH");
    ASSERT_STR_EQ(ani_risk_label(ANI_RISK_MEDIUM), "MEDIUM");
    ASSERT_STR_EQ(ani_risk_label(ANI_RISK_LOW), "LOW");
    /* Out-of-range enum value falls to default → LOW */
    ASSERT_STR_EQ(ani_risk_label((ani_risk_level_t)99), "LOW");
    PASS();
}

TEST(store_impact_summary_empty) {
    /* Zero hops and edges */
    ani_impact_summary_t s = ani_build_impact_summary(NULL, 0, NULL, 0);
    ASSERT_EQ(s.total, 0);
...
TEST(store_find_nodes_rejects_null_store_without_ub) {
    ani_node_t *nodes = (ani_node_t *)(uintptr_t)1U;
    int count = -1;
    ASSERT_EQ(ani_store_find_nodes_by_name(NULL, "project", "name", &nodes, &count), ANI_STORE_ERR);
    ASSERT_NULL(nodes);
    ASSERT_EQ(count, 0);
    nodes = (ani_node_t *)(uintptr_t)1U;
    count = -1;
    ASSERT_EQ(ani_store_find_nodes_by_label(NULL, "project", "Function", &nodes, &count),
              ANI_STORE_ERR);
    ASSERT_NULL(nodes);
    ASSERT_EQ(count, 0);
    nodes = (ani_node_t *)(uintptr_t)1U;
    count = -1;
    ASSERT_EQ(ani_store_find_nodes_by_file(NULL, "project", "file.c", &nodes, &count),
              ANI_STORE_ERR);
    ASSERT_NULL(nodes);
    ASSERT_EQ(count, 0);
    PASS();
...
static int fts_match_count(ani_store_t *s, const char *match) {
...
static void seed_prose_nodes(ani_store_t *s) {
    ani_store_upsert_project(s, "p", "/tmp/p");
    ani_node_t sec = {.project = "p",
                      .label = "Section",
                      .name = "Installation",
    ...
    ani_store_upsert_node(s, &sec);
    ani_node_t fn = {.project = "p",
                     .label = "Function",
                     .name = "plainFunction",
                     .qualified_name = "p.main.plainFunction",
                     .file_path = "main.c"};
    ani_store_upsert_node(s, &fn);
}

static ani_store_t *setup_prose_store(void) {
    ani_store_t *s = ani_store_open_memory();
    seed_prose_nodes(s);
    return s;
}
...
TEST(store_fts_rebuild_indexes_docstring_as_body_issue518) {
    ani_store_t *s = setup_prose_store();
    ASSERT_EQ(ani_store_fts_rebuild(s, NULL, 0), ANI_STORE_OK);
     ...
     * splitter. ani_camel_split("getUserById") yields "getUserById get User By
     ...
     * anyone ever wraps the body expression in ani_camel_split(). */
...
TEST(store_fts_rebuild_survives_malformed_properties_json) {
    ...
    ani_store_t *s = setup_prose_store();
    ani_node_t broken = {.project = "p",
                         .label = "Function",
                         .name = "brokenProps",
                         .qualified_name = "p.main.brokenProps",
                         .file_path = "main.c",
                         .properties_json = "{\"docstring\":\"unterminated"};
    ASSERT_TRUE(ani_store_upsert_node(s, &broken) > 0);
...
TEST(store_fts_rebuild_tolerates_legacy_four_column_table) {
    /* ANI_INDEX_FORMAT_VERSION deliberately does NOT move for the body column,
    ...
    char *td = th_mktempdir("ani_fts_legacy");
    ASSERT_NOT_NULL(td);
    char path[512];
    ...
    ani_store_t *s = ani_store_open_path(path);
...
TEST(store_fts_rebuild_incremental_adds_only_nodes_above_watermark) {
    ...
    sqlite3_stmt *st = NULL;
    ASSERT_EQ(sqlite3_prepare_v2(ani_store_get_db(s), "SELECT COALESCE(MAX(id),0) FROM nodes", -1,
                                 &st, NULL),
              SQLITE_OK);
    ...
    ani_node_t added = {.project = "p",
                        .label = "Section",
                        .name = "Upgrading",
                        .qualified_name = "p.README.Upgrading",
                        .file_path = "README.md",
                        .properties_json = "{\"docstring\":\"migrates the retention ledger\"}"};
    ASSERT_TRUE(ani_store_upsert_node(s, &added) > 0);

    ASSERT_EQ(ani_store_fts_rebuild(s, "p", watermark), ANI_STORE_OK);
