 ...
 */
#define ANI_TOML_EDIT_ENABLE_TEST_API 1
...
static const char *CTE_BEGIN = "# BEGIN ani";
static const char *CTE_END = "# END ani";
static const char *CTE_TABLE = "mcp_servers";
static const char *CTE_KEY = "name";
static const char *CTE_IDENTITY = "ani";
static const char *CTE_BODY = "name = \"ani\"\n"
                              "command = \"ani\"\n";
static const char *CTE_CODEX_BEGIN = "# >>> ani SessionStart >>>";
static const char *CTE_CODEX_END = "# <<< ani SessionStart <<<";
static const char *CTE_CODEX_COMMAND = "ani hook-augment";
static const char *CTE_CODEX_BLOCK =
    "[[hooks.SessionStart]]\n"
    "matcher = \"startup|resume|clear|compact\"\n\n"
    "[[hooks.SessionStart.hooks]]\n"
    "type = \"command\"\ncommand = \"/opt/ani hook-augment\"\n"
    "command_windows = \"& C:\\\\bin\\\\ani.exe hook-augment\"\ntimeout = 5\n\n"
    "[[hooks.SubagentStart]]\nmatcher = \"*\"\n\n"
    "[[hooks.SubagentStart.hooks]]\n"
    "type = \"command\"\ncommand = \"/opt/ani hook-augment\"\n"
    "command_windows = \"& C:\\\\bin\\\\ani.exe hook-augment\"\ntimeout = 5\n";

static int cte_codex_edit(const char *path, ani_toml_codex_hook_action_t action, int check_only) {
    return ani_toml_reconcile_codex_hooks(path, CTE_CODEX_BEGIN, CTE_CODEX_END, CTE_CODEX_COMMAND,
...
static int cte_codex_edit_commands_detailed(const char *path, const char *command,
    ...
    return ani_toml_reconcile_codex_hooks_detailed(path, CTE_CODEX_BEGIN, CTE_CODEX_END, command,
                                                   command_windows, action, check_only, failure);
}

static int cte_codex_edit_detailed(const char *path, ani_toml_codex_hook_action_t action,
                                   int check_only, ani_toml_codex_hook_failure_t *failure) {
...
static int cte_fixture(char *dir, size_t dir_size, char *path, size_t path_size) {
    char *created = th_mktempdir("ani_toml_edit");
    if (!created) {
        return -1;
    }
...
static size_t cte_temp_count(const char *dir) {
    ani_dir_t *directory = ani_opendir(dir);
    if (!directory) {
        return SIZE_MAX;
    }
    size_t count = 0;
...
static int cte_assert_unchanged_after_managed_upsert(const char *path, const char *original,
    ...
    return th_write_file(path, original) == 0 &&
                   ani_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, block) == -1 &&
...
static int cte_assert_unchanged_after_vibe_upsert(const char *path, const char *original,
    ...
    char actual[CTE_FILE_CAP];
    return th_write_file(path, original) == 0 &&
                   ani_toml_upsert_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY,
                                                     body) == -1 &&
                   cte_read(path, actual, sizeof(actual)) == 0 && strcmp(actual, original) == 0
               ? 1
               : 0;
...
TEST(config_toml_rejects_stale_content_and_identity) {
    ...
    ani_toml_set_precommit_hook_for_testing(cte_change_before_commit, &content_change);
    int result = ani_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned = true\n");
    ani_toml_set_precommit_hook_for_testing(NULL, NULL);
    ...
    ani_toml_set_precommit_hook_for_testing(cte_change_before_commit, &identity_change);
    result = ani_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned = true\n");
    ani_toml_set_precommit_hook_for_testing(NULL, NULL);
...
TEST(config_toml_missing_target_race_does_not_replace_winner) {
    ...
    ani_toml_set_prepublish_hook_for_testing(cte_change_before_commit, &race);
    int result = ani_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned = true\n");
    ani_toml_set_prepublish_hook_for_testing(NULL, NULL);
...
TEST(config_toml_rejects_non_regular_path) {
    ...
    char path[CTE_PATH_CAP];
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ASSERT_EQ(ani_mkdir(path), 0);
    ASSERT_EQ(ani_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned = true\n"), -1);
    ASSERT_EQ(cte_temp_count(dir), 0U);
    ASSERT_EQ(ani_rmdir(path), 0);
    th_cleanup(dir);
    PASS();
}
...
TEST(config_toml_managed_markers_ignore_multiline_strings) {
    ...
    ASSERT_EQ(ani_toml_remove_managed_block(path, CTE_BEGIN, CTE_END), 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
...
TEST(config_toml_managed_rejects_marker_in_block_and_unclosed_multiline) {
    ...
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ASSERT_EQ(th_write_file(path, "keep = true\n"), 0);
    ASSERT_EQ(ani_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END,
                                            "value = true\n# END ani\n"),
              -1);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ASSERT_STR_EQ(actual, "keep = true\n");
...
TEST(config_toml_codex_semantic_conflicts_fail_closed) {
    char dir[CTE_PATH_CAP];
    char path[CTE_PATH_CAP];
    const char *codex_block = "[mcp_servers.ani]\ncommand = \"new\"\n";
    static const char *conflicts[] = {
        "[mcp_servers.\"ani\"]\ncommand = \"old\"\n",
        "[\"mcp_servers\".'ani']\ncommand = \"old\"\n",
        "mcp_servers.\"ani\".command = \"old\"\n",
        "[mcp_servers]\n\"ani\".command = \"old\"\n",
        ("[mcp_servers.ani]\ncommand = \"one\"\n"
         "[mcp_servers.\"ani\"]\ncommand = \"two\"\n"),
    };
...
TEST(config_toml_target_table_rejects_significant_nonassignments_byte_identically) {
    ...
    for (size_t i = 0U; i < sizeof(malformed) / sizeof(malformed[0]); ++i) {
        ...
        ASSERT_EQ(
            ani_toml_upsert_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY, CTE_BODY),
            -1);
        ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
        ASSERT_STR_EQ(actual, malformed[i]);

        ASSERT_EQ(ani_toml_remove_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY), -1);
        ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
        ASSERT_STR_EQ(actual, malformed[i]);
    }

    const char *malformed_regular = "[mcp_servers.ani]\n"
                                    "command = \"winner\"\n"
                                    "this is not an assignment\n"
                                    "[unrelated]\n"
                                    "keep = true\n";
    ...
    ASSERT_EQ(
        ani_toml_remove_legacy_table(path, "mcp_servers.ani", CTE_BEGIN, CTE_END),
        -1);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
...
TEST(config_toml_vibe_remove_includes_descendant_tables) {
    ...
    ASSERT_EQ(th_write_file(path, "[[mcp_servers]]\n"
                                  ...
                                  "command = \"keep\"\n"),
              0);
    ASSERT_EQ(ani_toml_remove_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY), 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
...
TEST(config_toml_vibe_reinstall_preserves_user_fields_and_descendants) {
    ...
    char path[CTE_PATH_CAP];
    char actual[CTE_FILE_CAP];
    const char *desired = "name = \"ani\"\n"
                          "transport = \"stdio\"\n"
                          "command = \"new\"\n";
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ASSERT_EQ(th_write_file(path, "[[mcp_servers]]\n"
                                  "name = \"ani\" # identity comment\n"
                                  "command = \"old\"\n"
                                  ...
                                  "KEEP = \"yes\"\n"),
              0);
    ASSERT_EQ(ani_toml_upsert_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY, desired),
              0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
...
TEST(config_toml_preserves_bom_crlf_and_handles_no_final_newline) {
    ...
    ASSERT_NOT_NULL(strstr(actual, "# BEGIN ani\r\nowned = true\r\n"));
    ASSERT_EQ(ani_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned = true\n"), 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ...
    ASSERT_EQ(th_write_file(path, "\xEF\xBB\xBF# BEGIN ani\r\n"
                                  "old = true\r\n"
                                  "# END ani\r\n"),
              0);
    ASSERT_EQ(ani_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, "new = true\n"), 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ...
    ASSERT_EQ(th_write_file(path, "\xEF\xBB\xBF[[\"mcp_servers\"]]\r\n"
                                  ...
                                  "command = \"old\""),
              0);
    ASSERT_EQ(ani_toml_upsert_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY, CTE_BODY),
              0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ASSERT(memcmp(actual, "\xEF\xBB\xBF", 3U) == 0);
    ASSERT_NOT_NULL(strstr(actual, "command = \"ani\"\r\n"));
...
TEST(config_toml_rejects_oversized_input_file) {
    ...
    ASSERT_EQ(ani_toml_remove_managed_block(path, CTE_BEGIN, CTE_END), -1);
    th_cleanup(dir);
    PASS();
}
...
TEST(config_toml_escape_windows_path_quotes) {
    char escaped[256];
    ASSERT_EQ(
        ani_toml_escape_basic_string("C:\\Users\\Jane\\tool \"quoted\"", escaped, sizeof(escaped)),
        0);
...
TEST(config_toml_escape_newlines_controls) {
    char escaped[256];
    ASSERT_EQ(ani_toml_escape_basic_string("line1\nline2\t\r\b\f\x01", escaped, sizeof(escaped)),
              0);
    ASSERT_STR_EQ(escaped, "line1\\nline2\\t\\r\\b\\f\\u0001");

    char too_small[4] = "bad";
    ASSERT_EQ(ani_toml_escape_basic_string("overflow", too_small, sizeof(too_small)), -1);
    ASSERT_STR_EQ(too_small, "");
    ASSERT_EQ(ani_toml_escape_basic_string(NULL, escaped, sizeof(escaped)), -1);
    ASSERT_EQ(ani_toml_escape_basic_string("value", NULL, 0), -1);
    char invalid_utf8[] = {(char)0xff, '\0'};
    ASSERT_EQ(ani_toml_escape_basic_string(invalid_utf8, escaped, sizeof(escaped)), -1);
...
TEST(config_toml_managed_fresh_missing_file) {
    ...
    ASSERT_EQ(ani_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, "enabled = true\n"), 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ASSERT_STR_EQ(actual, "# BEGIN ani\n"
                          "enabled = true\n"
                          "# END ani\n");
    th_cleanup(dir);
    PASS();
...
TEST(config_toml_managed_replace_preserves_unrelated) {
    ...
    ASSERT_EQ(ani_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, "new = \"value\""), 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
...
TEST(config_toml_managed_unbalanced_duplicate_fail_closed) {
    ...
    const char *duplicate = "# BEGIN ani\n"
                            "one = true\n"
                            "# END ani\n"
                            "# BEGIN ani\n"
                            "two = true\n"
...
TEST(config_toml_vibe_replace_target_preserves_comments_tables) {
    ...
    char path[CTE_PATH_CAP];
    char actual[CTE_FILE_CAP];
    const char *replacement = "name = \"ani\"\n"
                              "command = \"/new/path\"\n";
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ...
    ASSERT_EQ(
        ani_toml_upsert_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY, replacement), 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
...
TEST(config_toml_vibe_owned_table_installs_idempotently_and_removes_exact_state) {
    ...
    ASSERT_EQ(
        ani_toml_upsert_owned_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY, canonical),
        0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ...
    ASSERT_EQ(
        ani_toml_remove_owned_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY, canonical),
        0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
...
TEST(config_toml_vibe_owned_table_preserves_foreign_same_name_state) {
    ...
    for (size_t i = 0U; i < sizeof(foreign_cases) / sizeof(foreign_cases[0]); i++) {
        ...
        ASSERT_EQ(ani_toml_upsert_owned_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY,
                                                          canonical),
                  ANI_TOML_OWNED_EDIT_FOREIGN);
        ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
        ASSERT_STR_EQ(actual, foreign_cases[i]);

        ASSERT_EQ(ani_toml_remove_owned_named_array_table(path, CTE_TABLE, CTE_KEY, CTE_IDENTITY,
                                                          canonical),
                  ANI_TOML_OWNED_EDIT_FOREIGN);
        ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
        ASSERT_STR_EQ(actual, foreign_cases[i]);
...
TEST(config_toml_vibe_ambiguous_target_fail_closed) {
    ...
    ASSERT_NOT_NULL(strstr(actual, "name = \"other\""));
    ASSERT_NOT_NULL(strstr(actual, "name = \"ani\""));
    th_cleanup(dir);
    PASS();
}
...
TEST(config_toml_codex_reconciles_minimal_owned_forms) {
    ...
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ASSERT_EQ(cte_codex_edit(path, ANI_TOML_CODEX_HOOK_UPSERT, 1), 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), -1);
    ASSERT_EQ(cte_codex_edit(path, ANI_TOML_CODEX_HOOK_UPSERT, 0), 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ASSERT_NOT_NULL(strstr(actual, CTE_CODEX_BEGIN));
    ASSERT_EQ(cte_codex_edit(path, ANI_TOML_CODEX_HOOK_REMOVE, 0), 0);
    ...
    static const char *fixtures[] = {
        ...
        "hooks=[{type='command', command='/opt/ani hook-augment', "
        "command_windows='& C:\\\\bin\\\\ani.exe hook-augment',timeout=5}]} ]\n",
    };
...
TEST(config_toml_codex_accepts_v0102_managed_windows_crlf) {
    static const char *command =
        "'C:\\Users\\Example\\AppData\\Local\\Programs\\ani\\"
        "ani.exe' hook-augment";
    ...
    ani_toml_codex_hook_failure_t failure = ANI_TOML_CODEX_HOOK_FAILURE_INVALID_ARGUMENT;
    int reconcile_rc = cte_codex_edit_commands_detailed(path, command, command_windows,
                                                        ANI_TOML_CODEX_HOOK_UPSERT, 1, &failure);
    ASSERT_EQ(failure, ANI_TOML_CODEX_HOOK_FAILURE_NONE);
    ASSERT_EQ(reconcile_rc, 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ASSERT_STR_EQ(actual, before);

    ASSERT_EQ(cte_codex_edit_commands_detailed(path, command, command_windows,
                                               ANI_TOML_CODEX_HOOK_UPSERT, 0, &failure),
              0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ASSERT_STR_EQ(actual, before);
    ASSERT_EQ(cte_occurrences(actual, CTE_CODEX_BEGIN), 1);
    ...
    ASSERT_EQ(cte_codex_edit_commands_detailed(path, command, command_windows,
                                               ANI_TOML_CODEX_HOOK_REMOVE, 0, &failure),
              0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ASSERT_NOT_NULL(strstr(actual, "[mcp_servers.other]"));
    ASSERT_NOT_NULL(strstr(actual, "keep = true"));
    ...
    for (size_t i = 0U; i < sizeof(invalid) / sizeof(invalid[0]); ++i) {
        ...
        failure = ANI_TOML_CODEX_HOOK_FAILURE_NONE;
        ASSERT_EQ(cte_codex_edit_commands_detailed(path, command, command_windows,
                                                   ANI_TOML_CODEX_HOOK_UPSERT, 1, &failure),
                  -1);
        ASSERT_EQ(failure, ANI_TOML_CODEX_HOOK_FAILURE_AMBIGUOUS_OWNERSHIP);
        ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
...
TEST(config_toml_codex_rejects_ambiguous_inline_byte_identically) {
    ...
    static const char *invalid[] = {
        ("[hooks]\nSessionStart = [{matcher='startup|resume|clear|compact',hooks=[{type="
         "'command',command='ani hook-augment',command_windows="
         "'ani hook-augment',timeout=5},{type='command',command='foreign'}]}]\n"),
        ("[hooks]\nSessionStart = [{matcher='startup|resume|clear|compact',hooks=[{type="
         "'command',command='ani hook-augment',command_windows="
         "'ani hook-augment',timeout=5,owner='user'}]}]\n"),
        "[hooks]\nSessionStart=[]\nSessionStart=[]\n",
        "[hooks]\nSessionStart = [\n { matcher = 'startup|resume|clear|compact' }\n]\n",
        ("[hooks]\nSessionStart = [{matcher='startup|resume|clear|compact',hooks=[{type="
         "'command',command='echo \"Code discovery: prefer ani\"'}]}] # keep\n"),
        "[hooks]\nSessionStart = [{ matcher = 'startup|resume|clear|compact'\n",
    };
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    for (size_t i = 0U; i < sizeof(invalid) / sizeof(invalid[0]); ++i) {
        ASSERT_EQ(th_write_file(path, invalid[i]), 0);
        ASSERT_EQ(cte_codex_edit(path, ANI_TOML_CODEX_HOOK_UPSERT, 1), -1);
        ASSERT_EQ(cte_codex_edit(path, ANI_TOML_CODEX_HOOK_UPSERT, 0), -1);
        ASSERT_EQ(cte_codex_edit(path, ANI_TOML_CODEX_HOOK_REMOVE, 0), -1);
        ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
        ASSERT_STR_EQ(actual, invalid[i]);
    }
    th_cleanup(dir);
...
TEST(config_toml_codex_reports_stable_failure_reasons) {
    ...
    static const struct {
        const char *content;
        ani_toml_codex_hook_failure_t expected;
        const char *name;
    } cases[] = {
        {
            .content =
            ...
            .expected = ANI_TOML_CODEX_HOOK_FAILURE_AMBIGUOUS_OWNERSHIP,
        ...
        },
        {
            .content =
                "[hooks]\n"
                "SessionStart = [{ matcher = 'startup|resume|clear|compact', hooks = [{ type = "
                "'command', command = 'echo \"Code discovery: prefer ani\"' }] "
                "}]\n"
                "SessionStart = [{ matcher = 'startup|resume|clear|compact', hooks = [{ type = "
                "'command', command = 'echo \"Code discovery: prefer ani\"' }] "
                "}]\n",
            .expected = ANI_TOML_CODEX_HOOK_FAILURE_CONFLICTING_HOOKS,
            .name = "conflicting_hook_representations",
        },
        {
            .content = "[hooks\n",
            .expected = ANI_TOML_CODEX_HOOK_FAILURE_MALFORMED_CONFIG,
            .name = "malformed_config",
        },
    };
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    for (size_t i = 0U; i < sizeof(cases) / sizeof(cases[0]); ++i) {
        ani_toml_codex_hook_failure_t failure = ANI_TOML_CODEX_HOOK_FAILURE_NONE;
        ASSERT_EQ(th_write_file(path, cases[i].content), 0);
        ASSERT_EQ(cte_codex_edit_detailed(path, ANI_TOML_CODEX_HOOK_UPSERT, 1, &failure), -1);
        ASSERT_EQ(failure, cases[i].expected);
        ASSERT_STR_EQ(ani_toml_codex_hook_failure_name(failure), cases[i].name);
        ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ...
    ani_toml_codex_hook_failure_t failure = ANI_TOML_CODEX_HOOK_FAILURE_INVALID_ARGUMENT;
    ASSERT_EQ(th_write_file(path, "keep = true\n"), 0);
    ASSERT_EQ(cte_codex_edit_detailed(path, ANI_TOML_CODEX_HOOK_UPSERT, 1, &failure), 0);
    ASSERT_EQ(failure, ANI_TOML_CODEX_HOOK_FAILURE_NONE);
    th_cleanup(dir);
    PASS();
}
...
TEST(config_toml_remove_self_heals_orphan_closing_marker_issue1558) {
    ...
    ASSERT_EQ(th_write_file(path, "user_key = true\n"
                                  ...
                                  "other_key = 1\n"),
              0);
    ASSERT_EQ(ani_toml_remove_managed_block(path, "# >>> ani SessionStart >>>",
                                            "# <<< ani SessionStart <<<"),
              0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
...
TEST(config_toml_remove_self_heals_orphan_opening_marker_issue1558) {
    ...
    char actual[CTE_FILE_CAP];
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ASSERT_EQ(th_write_file(path, "# >>> ani SessionStart >>>\n"
                                  "keep = true\n"),
              0);
    ASSERT_EQ(ani_toml_remove_managed_block(path, "# >>> ani SessionStart >>>",
                                            "# <<< ani SessionStart <<<"),
