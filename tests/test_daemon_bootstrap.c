...
typedef struct {
    ...
    ani_daemon_bootstrap_probe_status_t forced_probe;
    ani_version_cohort_status_t forced_cohort;
    char diagnostic[ANI_DAEMON_CONFLICT_MESSAGE_SIZE];
} bootstrap_fake_ops_t;

typedef struct {
    const ani_daemon_bootstrap_config_t *config;
    const ani_daemon_bootstrap_ops_t *ops;
    atomic_int *ready;
    atomic_bool *go;
    ani_daemon_bootstrap_result_t result;
    ani_daemon_bootstrap_status_t status;
} bootstrap_thread_call_t;

static ani_daemon_build_identity_t bootstrap_identity(const char *version, const char *build) {
...
static bool bootstrap_endpoint_fixture_start(bootstrap_endpoint_fixture_t *fixture,
    ...
    int written = snprintf(fixture->parent, sizeof(fixture->parent), "%s/ani-bootstrap-%s-XXXXXX",
                           ani_tmpdir(), tag);
    if (written <= 0 || written >= (int)sizeof(fixture->parent) || !ani_mkdtemp(fixture->parent)) {
        return false;
    }
    fixture->endpoint = ani_daemon_bootstrap_endpoint_new(fixture->parent);
    const char *runtime_dir =
        fixture->endpoint ? ani_daemon_ipc_endpoint_runtime_dir(fixture->endpoint) : NULL;
    if (!runtime_dir) {
        return false;
    }
...
static ani_daemon_bootstrap_probe_status_t bootstrap_fake_probe(
    void *opaque, const ani_daemon_ipc_endpoint_t *endpoint,
    ...
    *client_out = NULL;
    ...
    if (fake->forced_probe == ANI_DAEMON_BOOTSTRAP_PROBE_RESERVED) {
        return fake->forced_probe;
    }
    if (fake->forced_probe == ANI_DAEMON_BOOTSTRAP_PROBE_CONFLICT) {
        result_out->status = ANI_DAEMON_RUNTIME_CONNECT_CONFLICT;
        result_out->hello_status = ANI_DAEMON_HELLO_BUILD_CONFLICT;
        snprintf(result_out->message, sizeof(result_out->message),
                 "ANI daemon could not start: conflicting versions");
        return fake->forced_probe;
    }
    if (fake->forced_probe == ANI_DAEMON_BOOTSTRAP_PROBE_TERMINAL) {
        return fake->forced_probe;
    }
    ...
    if (atomic_load(&fake->available)) {
        result_out->status = ANI_DAEMON_RUNTIME_CONNECT_ACCEPTED;
        result_out->hello_status = ANI_DAEMON_HELLO_COMPATIBLE;
        result_out->client_id = 1;
        *client_out = (ani_daemon_runtime_client_t *)(uintptr_t)1;
        return ANI_DAEMON_BOOTSTRAP_PROBE_CONNECTED;
    }
    return ANI_DAEMON_BOOTSTRAP_PROBE_UNAVAILABLE;
}

static ani_version_cohort_status_t bootstrap_fake_cohort_acquire(
    void *opaque, const ani_daemon_ipc_endpoint_t *endpoint,
    ...
    *cohort_out = NULL;
    ...
    if (fake->forced_cohort == ANI_VERSION_COHORT_CONFLICT) {
        ani_daemon_build_identity_t active = bootstrap_identity("2.3.0", BOOTSTRAP_BUILD_A);
        (void)ani_daemon_hello_compare(&active, identity, conflict_out);
        return fake->forced_cohort;
    }
    if (fake->forced_cohort != ANI_VERSION_COHORT_OK) {
        return fake->forced_cohort;
    }
    *cohort_out = fake;
...
static void bootstrap_fake_cohort_release(void *opaque, ani_daemon_bootstrap_cohort_t cohort) {
    bootstrap_fake_ops_t *fake = opaque;
    if (cohort == fake) {
...
static int bootstrap_fake_lock(void *opaque, const ani_daemon_ipc_endpoint_t *endpoint,
...
static bool bootstrap_fake_unlock(void *opaque, ani_daemon_bootstrap_lock_t *lock_io) {
    bootstrap_fake_ops_t *fake = opaque;
...
static bool bootstrap_fake_handoff(void *opaque, ani_daemon_bootstrap_lock_t lock) {
    bootstrap_fake_ops_t *fake = opaque;
...
static bool bootstrap_fake_spawn(void *opaque, const ani_daemon_bootstrap_launch_spec_t *spec) {
    bootstrap_fake_ops_t *fake = opaque;
    ...
    bool exact = spec && spec->argc == 2U && spec->argv[0] && spec->argv[1] && !spec->argv[2] &&
                 strcmp(spec->argv[1], ANI_DAEMON_INTERNAL_ARG) == 0 && spec->detached &&
...
static ani_daemon_bootstrap_ops_t bootstrap_fake_callbacks(bootstrap_fake_ops_t *fake) {
    ani_daemon_bootstrap_ops_t ops = {
        .context = fake,
...
static void *bootstrap_thread_execute(void *opaque) {
    ...
    call->status = ani_daemon_bootstrap_execute_with_ops(call->config, call->ops, &call->result);
    return NULL;
}

static ani_daemon_process_role_t classify(int argc, char **argv) {
    return ani_daemon_process_role(argc, argv);
}

TEST(daemon_bootstrap_classifies_default_and_ui_as_mcp_clients) {
    char *plain[] = {"ani", NULL};
    char *ui[] = {"ani", "--ui=true", "--port=9750", NULL};
    ASSERT_EQ(classify(1, plain), ANI_DAEMON_PROCESS_MCP_CLIENT);
    ASSERT_EQ(classify(3, ui), ANI_DAEMON_PROCESS_MCP_CLIENT);
    ASSERT_TRUE(ani_daemon_process_role_requires_client(ANI_DAEMON_PROCESS_MCP_CLIENT));
    PASS();
}

TEST(daemon_bootstrap_classifies_stateless_commands_without_client) {
    char *version[] = {"ani", "--version", NULL};
    char *help[] = {"ani", "--profile", "--help", NULL};
    char *install[] = {"ani", "install", "--dry-run", NULL};
    char *uninstall[] = {"ani", "uninstall", NULL};
    char *update[] = {"ani", "update", "-n", NULL};
    ASSERT_EQ(classify(2, version), ANI_DAEMON_PROCESS_STATELESS);
    ASSERT_EQ(classify(3, help), ANI_DAEMON_PROCESS_STATELESS);
    ASSERT_EQ(classify(3, install), ANI_DAEMON_PROCESS_STATELESS);
    ASSERT_EQ(classify(2, uninstall), ANI_DAEMON_PROCESS_STATELESS);
    ASSERT_EQ(classify(3, update), ANI_DAEMON_PROCESS_STATELESS);
    ASSERT_FALSE(ani_daemon_process_role_requires_client(ANI_DAEMON_PROCESS_STATELESS));
    PASS();
}

TEST(daemon_bootstrap_classifies_config_as_coordinated_local_cli) {
    char *list[] = {"ani", "config", "list", NULL};
    char *set[] = {"ani", "config", "set", "auto_watch", "false", NULL};
    char *help[] = {"ani", "config", "--help", NULL};
    ASSERT_EQ(classify(3, list), ANI_DAEMON_PROCESS_LOCAL_CLI);
    ASSERT_EQ(classify(5, set), ANI_DAEMON_PROCESS_LOCAL_CLI);
    ASSERT_EQ(classify(3, help), ANI_DAEMON_PROCESS_STATELESS);
    ASSERT_FALSE(ani_daemon_process_role_requires_client(ANI_DAEMON_PROCESS_LOCAL_CLI));
    PASS();
}

TEST(daemon_bootstrap_cli_help_is_stateless_but_tool_calls_are_local) {
    char *tool_help[] = {"ani", "cli", "search_graph", "--help", NULL};
    char *tool_call[] = {"ani", "cli", "search_graph", "{}", NULL};
    ASSERT_EQ(classify(4, tool_help), ANI_DAEMON_PROCESS_STATELESS);
    ASSERT_EQ(classify(4, tool_call), ANI_DAEMON_PROCESS_LOCAL_CLI);
    ASSERT_FALSE(ani_daemon_process_role_requires_client(ANI_DAEMON_PROCESS_LOCAL_CLI));
    PASS();
}
...
TEST(daemon_bootstrap_cli_arguments_cannot_reclassify_the_process) {
    char *install_value[] = {
        "ani", "cli", "search_code", "--query", "install", NULL};
    char *version_value[] = {"ani", "cli", "search_code", "--query",
                             "--version",           NULL};
    ASSERT_EQ(classify(5, install_value), ANI_DAEMON_PROCESS_LOCAL_CLI);
    ASSERT_EQ(classify(5, version_value), ANI_DAEMON_PROCESS_LOCAL_CLI);
    PASS();
}
...
TEST(daemon_bootstrap_internal_roles_never_take_client_leases) {
    ...
    char *daemon[] = {"ani", ANI_DAEMON_INTERNAL_ARG, NULL};
    char *worker[] = {"ani", "cli", "--index-worker", "--index-worker-build", build,
                      "index_repository",    "{}",  "--response-out", "/tmp/response",        NULL};
    char *malformed_worker[] = {"ani", "cli", "--index-worker",
                                "index_repository",    "{}",  NULL};
    char *reserved_user_value[] = {"ani", "cli", "search_code", "--query",
                                   "--index-worker",      NULL};
    char *hook[] = {"ani", "hook-augment", NULL};
    ASSERT_EQ(classify(2, daemon), ANI_DAEMON_PROCESS_DAEMON);
    ASSERT_EQ(classify(9, worker), ANI_DAEMON_PROCESS_WORKER);
    ASSERT_EQ(classify(5, malformed_worker), ANI_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(classify(5, reserved_user_value), ANI_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(classify(2, hook), ANI_DAEMON_PROCESS_HOOK_CLIENT);
    ASSERT_FALSE(ani_daemon_process_role_requires_client(ANI_DAEMON_PROCESS_DAEMON));
    ASSERT_FALSE(ani_daemon_process_role_requires_client(ANI_DAEMON_PROCESS_WORKER));
    ASSERT_TRUE(ani_daemon_process_role_requires_client(ANI_DAEMON_PROCESS_HOOK_CLIENT));
    PASS();
}

TEST(daemon_bootstrap_rejects_ambiguous_internal_daemon_argv) {
    char *missing[] = {NULL};
    char *mixed[] = {"ani", ANI_DAEMON_INTERNAL_ARG, "cli", NULL};
    ASSERT_EQ(classify(0, missing), ANI_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(classify(3, mixed), ANI_DAEMON_PROCESS_INVALID);
    ASSERT_FALSE(ani_daemon_process_role_requires_client(ANI_DAEMON_PROCESS_INVALID));
    PASS();
}
...
TEST(daemon_bootstrap_uses_one_stable_per_account_endpoint) {
    ...
    ani_daemon_ipc_endpoint_t *second = ani_daemon_bootstrap_endpoint_new(fixture.parent);
    ASSERT_NOT_NULL(second);
    ASSERT_STR_EQ(ani_daemon_ipc_endpoint_address(fixture.endpoint),
                  ani_daemon_ipc_endpoint_address(second));
    ani_daemon_ipc_endpoint_free(second);
 ...
 * included, and the operator cannot reconfigure their way out. ANI_RUNTIME_DIR
...
TEST(daemon_bootstrap_runtime_dir_env_relocates_rendezvous) {
    ...
    int written = snprintf(override_parent, sizeof(override_parent),
                           "%s/ani-bootstrap-runtime-env-XXXXXX", ani_tmpdir());
    if (written <= 0 || written >= (int)sizeof(override_parent) || !ani_mkdtemp(override_parent)) {
    ...
    bool prepared =
        ...
        ani_canonical_path(override_parent, canonical_override, sizeof(canonical_override)) != 0 &&
        ani_setenv("ANI_RUNTIME_DIR", override_parent, 1) == 0;

    /* NULL parent == every product call site: daemon, MCP client, local CLI,
     * index worker, activation. */
    ani_daemon_ipc_endpoint_t *relocated =
        prepared ? ani_daemon_bootstrap_endpoint_new(NULL) : NULL;
    const char *relocated_dir = relocated ? ani_daemon_ipc_endpoint_runtime_dir(relocated) : NULL;
    if (relocated_dir) {
    ...
    bool explicit_canonical =
        explicit_started &&
        ani_canonical_path(fixture.parent, canonical_explicit, sizeof(canonical_explicit)) != 0;
    if (explicit_started) {
    ...
    bool unusable_set = prepared && ani_setenv("ANI_RUNTIME_DIR", unusable, 1) == 0;
    ani_daemon_ipc_endpoint_t *refused =
        unusable_set ? ani_daemon_bootstrap_endpoint_new(NULL) : NULL;

    /* Restore before asserting: a failed assertion returns immediately, and a
     * leaked ANI_RUNTIME_DIR would follow every later suite in this process. */
    (void)ani_unsetenv("ANI_RUNTIME_DIR");
    ani_daemon_ipc_endpoint_free(refused);
    ani_daemon_ipc_endpoint_free(relocated);
    if (relocated_runtime[0] != '\0') {
        (void)ani_rmdir(relocated_runtime);
    }
    if (explicit_started) {
        bootstrap_endpoint_fixture_finish(&fixture);
    }
    (void)ani_rmdir(override_parent);
...
TEST(daemon_bootstrap_launches_only_exact_detached_hidden_role) {
    ...
    ASSERT_TRUE(ani_daemon_bootstrap_launch_spec_init("/tmp/ani exact", &spec));
    ASSERT_EQ(spec.argc, 2U);
    ASSERT_STR_EQ(spec.executable_path, "/tmp/ani exact");
    ASSERT_STR_EQ(spec.argv[0], "/tmp/ani exact");
    ASSERT_STR_EQ(spec.argv[1], ANI_DAEMON_INTERNAL_ARG);
...
TEST(daemon_bootstrap_permanent_daemon_argv_is_byte_exact) {
    char *permanent[] = {"ani", ANI_DAEMON_INTERNAL_ARG, ANI_DAEMON_PERMANENT_ARG,
                         NULL};
    char *reordered[] = {"ani", ANI_DAEMON_PERMANENT_ARG, ANI_DAEMON_INTERNAL_ARG,
                         NULL};
    char *repeated[] = {"ani", ANI_DAEMON_INTERNAL_ARG, ANI_DAEMON_INTERNAL_ARG,
                        NULL};
    char *extended[] = {"ani", ANI_DAEMON_INTERNAL_ARG, ANI_DAEMON_PERMANENT_ARG,
                        "extra", NULL};
    char *wrong_flag[] = {"ani", ANI_DAEMON_INTERNAL_ARG, "--permanent", NULL};
    ASSERT_EQ(classify(3, permanent), ANI_DAEMON_PROCESS_DAEMON);
    ASSERT_EQ(classify(3, reordered), ANI_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(classify(3, repeated), ANI_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(classify(4, extended), ANI_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(classify(3, wrong_flag), ANI_DAEMON_PROCESS_INVALID);
    PASS();
}

TEST(daemon_bootstrap_daemon_ctl_token_routes_after_cli) {
    char *start[] = {"ani", "daemon", "start", NULL};
    char *stop[] = {"ani", "daemon", "stop", NULL};
    char *status[] = {"ani", "daemon", "status", NULL};
    char *help[] = {"ani", "daemon", "--help", NULL};
    /* `daemon` after `cli` is opaque tool input, never a control command. */
    char *opaque[] = {"ani", "cli", "search_code", "daemon", "start", NULL};
    ASSERT_EQ(classify(3, start), ANI_DAEMON_PROCESS_DAEMON_CTL);
    ASSERT_EQ(classify(3, stop), ANI_DAEMON_PROCESS_DAEMON_CTL);
    ASSERT_EQ(classify(3, status), ANI_DAEMON_PROCESS_DAEMON_CTL);
    ASSERT_EQ(classify(3, help), ANI_DAEMON_PROCESS_STATELESS);
    ASSERT_EQ(classify(5, opaque), ANI_DAEMON_PROCESS_LOCAL_CLI);
    ASSERT_FALSE(ani_daemon_process_role_requires_client(ANI_DAEMON_PROCESS_DAEMON_CTL));
    PASS();
}

TEST(daemon_bootstrap_permanent_launch_spec_is_exact) {
    ani_daemon_bootstrap_launch_spec_t spec;
    ASSERT_TRUE(ani_daemon_bootstrap_launch_spec_init_permanent("/tmp/ani exact", &spec));
    ASSERT_EQ(spec.argc, 3U);
    ASSERT_STR_EQ(spec.argv[0], "/tmp/ani exact");
    ASSERT_STR_EQ(spec.argv[1], ANI_DAEMON_INTERNAL_ARG);
    ASSERT_STR_EQ(spec.argv[2], ANI_DAEMON_PERMANENT_ARG);
...
TEST(daemon_bootstrap_stateless_roles_bypass_every_daemon_operation) {
    ...
    ani_daemon_bootstrap_ops_t ops = bootstrap_fake_callbacks(&fake);
    ani_daemon_bootstrap_config_t config = {
        .role = ANI_DAEMON_PROCESS_STATELESS,
    };
    ani_daemon_bootstrap_result_t result;
    ASSERT_EQ(ani_daemon_bootstrap_execute_with_ops(&config, &ops, &result),
              ANI_DAEMON_BOOTSTRAP_BYPASSED);
    ASSERT_EQ(result.status, ANI_DAEMON_BOOTSTRAP_BYPASSED);
...
TEST(daemon_bootstrap_cohort_conflict_is_visible_before_probe_or_spawn) {
    ...
    bootstrap_fake_ops_t fake = {0};
    fake.forced_cohort = ANI_VERSION_COHORT_CONFLICT;
    ani_daemon_bootstrap_ops_t ops = bootstrap_fake_callbacks(&fake);
    ani_daemon_build_identity_t identity = bootstrap_identity("2.4.0", BOOTSTRAP_BUILD_B);
    ani_daemon_bootstrap_config_t config = {
        .role = ANI_DAEMON_PROCESS_MCP_CLIENT,
        .endpoint = fixture.endpoint,
        .identity = &identity,
        .executable_path = "/tmp/ani",
    ...
    ASSERT_EQ(result.status, ANI_DAEMON_BOOTSTRAP_CONFLICT);
    ...
    ASSERT_NOT_NULL(strstr(fake.diagnostic, "conflicting ANI process is active"));
    ASSERT_NOT_NULL(strstr(fake.diagnostic, "Close all ANI sessions and commands"));
    bootstrap_endpoint_fixture_finish(&fixture);
    PASS();
}
...
TEST(daemon_bootstrap_existing_exact_daemon_connects_without_spawn) {
    ...
    atomic_store(&fake.available, true);
    ani_daemon_bootstrap_ops_t ops = bootstrap_fake_callbacks(&fake);
    ani_daemon_build_identity_t identity = bootstrap_identity("2.4.0", BOOTSTRAP_BUILD_A);
    ani_daemon_bootstrap_config_t config = {
        .role = ANI_DAEMON_PROCESS_MCP_CLIENT,
        .endpoint = fixture.endpoint,
        .identity = &identity,
    ...
    ASSERT_EQ(result.status, ANI_DAEMON_BOOTSTRAP_CONNECTED);
...
TEST(daemon_bootstrap_conflict_is_visible_and_never_spawns) {
    ...
    bootstrap_fake_ops_t fake = {0};
    fake.forced_probe = ANI_DAEMON_BOOTSTRAP_PROBE_CONFLICT;
    ani_daemon_bootstrap_ops_t ops = bootstrap_fake_callbacks(&fake);
    ani_daemon_build_identity_t identity = bootstrap_identity("2.4.0", BOOTSTRAP_BUILD_B);
    ani_daemon_bootstrap_config_t config = {
        .role = ANI_DAEMON_PROCESS_MCP_CLIENT,
...
TEST(daemon_bootstrap_terminal_generation_that_never_exits_is_not_replaced) {
    ...
    bootstrap_fake_ops_t fake = {0};
    fake.forced_probe = ANI_DAEMON_BOOTSTRAP_PROBE_TERMINAL;
    ani_daemon_bootstrap_ops_t ops = bootstrap_fake_callbacks(&fake);
    ani_daemon_build_identity_t identity = bootstrap_identity("2.4.0", BOOTSTRAP_BUILD_A);
    ani_daemon_bootstrap_config_t config = {
        .role = ANI_DAEMON_PROCESS_HOOK_CLIENT,
        .endpoint = fixture.endpoint,
        .identity = &identity,
        .executable_path = "/tmp/ani",
...
TEST(daemon_bootstrap_rejected_connect_is_reserved_and_never_unavailable) {
    ani_daemon_runtime_connect_result_t capacity = {0};
    capacity.status = ANI_DAEMON_RUNTIME_CONNECT_REJECTED;
    snprintf(capacity.message, sizeof(capacity.message), "ANI daemon connection capacity reached");
    ASSERT_EQ(ani_daemon_bootstrap_classify_failed_connect(&capacity, 1),
              ANI_DAEMON_BOOTSTRAP_PROBE_RESERVED);
    ASSERT_EQ(ani_daemon_bootstrap_classify_failed_connect(&capacity, 0),
              ANI_DAEMON_BOOTSTRAP_PROBE_RESERVED);
    ASSERT_EQ(ani_daemon_bootstrap_classify_failed_connect(&capacity, -1),
              ANI_DAEMON_BOOTSTRAP_PROBE_RESERVED);

    ani_daemon_runtime_connect_result_t stopping = capacity;
    snprintf(stopping.message, sizeof(stopping.message), "ANI daemon is stopping");
    ASSERT_EQ(ani_daemon_bootstrap_classify_failed_connect(&stopping, 1),
              ANI_DAEMON_BOOTSTRAP_PROBE_TERMINAL);

    ani_daemon_runtime_connect_result_t absent = {0};
    absent.status = ANI_DAEMON_RUNTIME_CONNECT_ERROR;
    ASSERT_EQ(ani_daemon_bootstrap_classify_failed_connect(&absent, 1),
              ANI_DAEMON_BOOTSTRAP_PROBE_RESERVED);
    ASSERT_EQ(ani_daemon_bootstrap_classify_failed_connect(&absent, 0),
...
TEST(daemon_bootstrap_mute_endpoint_holder_is_reserved_and_never_unavailable) {
    ani_daemon_runtime_connect_result_t mute = {0};
    mute.status = ANI_DAEMON_RUNTIME_CONNECT_ERROR;
    mute.muted_endpoint_holder_pid = 4242;
    ASSERT_EQ(ani_daemon_bootstrap_classify_failed_connect(&mute, 1),
              ANI_DAEMON_BOOTSTRAP_PROBE_RESERVED);
    ASSERT_EQ(ani_daemon_bootstrap_classify_failed_connect(&mute, 0),
              ANI_DAEMON_BOOTSTRAP_PROBE_RESERVED);
    ASSERT_EQ(ani_daemon_bootstrap_classify_failed_connect(&mute, -1),
              ANI_DAEMON_BOOTSTRAP_PROBE_RESERVED);
    ...
    ani_daemon_runtime_connect_result_t rejected = {0};
    rejected.status = ANI_DAEMON_RUNTIME_CONNECT_REJECTED;
    rejected.muted_endpoint_holder_pid = 4242;
    snprintf(rejected.message, sizeof(rejected.message), "ANI daemon is stopping");
    ASSERT_EQ(ani_daemon_bootstrap_classify_failed_connect(&rejected, 0),
              ANI_DAEMON_BOOTSTRAP_PROBE_TERMINAL);
    PASS();
}
...
TEST(daemon_bootstrap_concurrent_first_clients_spawn_one_daemon) {
    ...
    ASSERT_EQ(ani_thread_create(&threads[0], 0, bootstrap_thread_execute, &calls[0]), 0);
    ASSERT_EQ(ani_thread_create(&threads[1], 0, bootstrap_thread_execute, &calls[1]), 0);
    uint64_t ready_deadline = ani_now_ms() + BOOTSTRAP_TEST_TIMEOUT_MS;
    while (atomic_load(&ready) != 2 && ani_now_ms() < ready_deadline) {
        const struct timespec pause = {0, 1000000L};
        (void)ani_nanosleep(&pause, NULL);
    }
    atomic_store(&go, true);
    ASSERT_EQ(ani_thread_join(&threads[0]), 0);
    ASSERT_EQ(ani_thread_join(&threads[1]), 0);
    ASSERT_EQ(calls[0].status, ANI_DAEMON_BOOTSTRAP_CONNECTED);
    ASSERT_EQ(calls[1].status, ANI_DAEMON_BOOTSTRAP_CONNECTED);
    ASSERT_EQ(atomic_load(&fake.spawn_count), 1);
...
TEST(daemon_bootstrap_darwin_launch_failure_is_synchronous) {
    ...
    int written = snprintf(missing, sizeof(missing), "%s/definitely-missing-ani", fixture.parent);
    ...
    ASSERT_EQ(ani_daemon_bootstrap_execute(&config, &result), ANI_DAEMON_BOOTSTRAP_FAILED);
