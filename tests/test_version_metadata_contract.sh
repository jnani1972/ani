...
#
# Why there is no single canonical version file to check against: ANI_VERSION is
# injected at BUILD time (-DANI_VERSION, see Makefile.ani) and the release is
...
SURFACES=(
    ...
    "pkg/pypi/src/ani/_cli.py|release"
    "pkg/go/cmd/ani/main.go|release"
    "pkg/chocolatey/ani.nuspec|release"
    "pkg/chocolatey/tools/chocolateyInstall.ps1|release"
    "pkg/homebrew/Formula/ani.rb|pin:0.10.3"
    "pkg/scoop/ani.json|pin:0.8.1"
...
PIN_REASONS=(
    "pkg/homebrew/Formula/ani.rb|pins 4 per-asset sha256 (darwin/linux x arm/intel); last re-pinned for v0.10.3"
    "pkg/scoop/ani.json|pins the windows-amd64.zip sha256; last re-pinned for v0.8.1"
