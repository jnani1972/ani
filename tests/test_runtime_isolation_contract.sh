...
if [[ "$1 $2" == "install -y" ]]; then
    printf '%s\t%s\t%s\n' "$HOME" "$ANI_CACHE_DIR" "$ANI_RUNTIME_DIR" > "$ANI_TEST_INSTALL_ENV_PROBE"
fi
exit 0
EOF
...
(
    export ANI_RUNTIME_DIR="$CALLER_ROOT/runtime"
    export ANI_CACHE_DIR="$CALLER_ROOT/cache"
    source "$ROOT/scripts/test-runtime.sh"
    ani_test_runtime_init
    [[ "$ANI_RUNTIME_DIR" != "$CALLER_ROOT/runtime" ]] || fail "inherited runtime was reused"
    [[ "$ANI_CACHE_DIR" != "$CALLER_ROOT/cache" ]] || fail "inherited cache was reused"
    private_root="$ANI_TEST_RUNTIME_ROOT"
    [[ -d "$private_root/runtime" && -d "$private_root/cache" ]] || fail "private directories missing"
    ani_test_runtime_cleanup "$CONTROL"
...
for slot in 1 2; do
    (
        source "$ROOT/scripts/test-runtime.sh"
        ani_test_runtime_init
        printf '%s\n' "$ANI_TEST_RUNTIME_ROOT" > "$WORKDIR/root-$slot"
        ani_test_runtime_cleanup "$CONTROL"
    ) &
done
wait
...
(
    ...
    source "$ROOT/scripts/test-runtime.sh"
    case "$(uname -s)" in
        MINGW*|MSYS*|CYGWIN*) export ANI_CI_TEMP_ROOT="$(cygpath -m "$WORKDIR/windows-ci-root")" ;;
    esac
    mktemp() { return 1; }
    ani_test_runtime_init
    ANI_TEST_FAILURE_MARKER="$WORKDIR/failure.marker" "$FAIL_FIXTURE"
...
ANI_CACHE_DIR="$WORKDIR/caller-cache" \
ANI_RUNTIME_DIR="$WORKDIR/caller-runtime" \
ANI_TEST_INSTALL_ENV_PROBE="$WORKDIR/install.env" \
...
for entry in "${ENTRY_POINTS[@]}"; do
    ...
    grep -q 'ani_test_runtime_init' "$ROOT/$entry" || fail "$entry does not initialize isolation"
    grep -q 'ani_test_runtime_cleanup' "$ROOT/$entry" || fail "$entry does not clean isolation"
done
