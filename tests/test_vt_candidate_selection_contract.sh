...
FIX="$(mktemp -d "${TMPDIR:-/tmp}/ani-vt-select.XXXXXX")"
trap 'rm -rf "$FIX"' EXIT
...
def candidate_bytes(target: str, variant: str) -> bytes:
    linked = f"ani:{target}:same-linker-output\n".encode()
    suffix = {
...
def make_artifacts(directory: pathlib.Path, *, reverse: bool = False) -> dict[tuple[str, str], bytes]:
    ...
    for index, target in enumerate(targets):
        ...
        write_tsv(artifact / "candidate-provenance.tsv",
                  "ani-release-candidate-provenance-v1", {}, FIELDS, rows)
...
require(cmeta["marker"] == "# ani-release-candidates-v1", "candidate marker changed")
...
def replace_candidate_with_symlink(path: pathlib.Path) -> None:
    ...
    target = provenances[1].parent / "unstripped" / (
        "ani.exe" if "windows" in str(provenances[1])
        else "ani"
    )
...
def write_results(path: pathlib.Path, rows: list[dict[str, object]]) -> None:
    write_tsv(path, "ani-virustotal-results-v2", {
...
require(smeta["marker"] == "# ani-release-selection-v1", "selection marker changed")
...
for row in selections:
    ...
    path = selected / "selected" / target / ("ani.exe" if target.startswith("windows-") else "ani")
...
for name in ARCHIVES:
    target = next(target for target in TARGETS if name.startswith(f"ani-{target}."))
    destination = archives / target / ("mcpb" if name.endswith(".mcpb") else "archive") / name
    destination.parent.mkdir(parents=True, exist_ok=True)
    binary_name = "ani.exe" if target.startswith("windows-") else "ani"
...
def mutate_zip(path: pathlib.Path) -> None:
    archive = next(path.rglob("ani-windows-amd64.zip"))
    with zipfile.ZipFile(archive, "w") as handle:
        handle.writestr("ani.exe", b"mutated-selected-binary")
...
def remove_unix_executable_mode(path: pathlib.Path) -> None:
    archive = next(path.rglob("ani-darwin-arm64.mcpb"))
    with zipfile.ZipFile(archive, "r") as handle:
        entries = [(info, handle.read(info)) for info in handle.infolist()]
    with zipfile.ZipFile(archive, "w") as handle:
        for info, payload in entries:
            if info.filename == "server/ani":
                info.create_system = 3
...
def wrong_target(path: pathlib.Path) -> None:
    archive = next(path.rglob("ani-linux-amd64.tar.gz"))
    with tarfile.open(archive, "w:gz") as handle:
        data = archive_for["linux-arm64"]
        info = tarfile.TarInfo("ani")
        info.size = len(data)
