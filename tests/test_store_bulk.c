 ...
 *
 * Verifies that ani_store_begin_bulk / ani_store_end_bulk never switch away
 ...
 * independent of any state held inside the ani_store_t under test. */
...
static void make_temp_path(char *buf, size_t n) {
    snprintf(buf, n, "%s/cmm_bulk_test_%d.db", ani_tmpdir(), (int)getpid());
}
...
TEST(bulk_crash_recovery) {
    ...
    ani_store_t *s = ani_store_open_path(db_path);
    ASSERT_NOT_NULL(s);
    int rc = ani_store_upsert_project(s, "baseline", "/tmp/baseline");
    ASSERT_EQ(rc, ANI_STORE_OK);
    ani_store_close(s);
    ...
    pid_t pid = fork();
    if (pid == 0) {
        ani_store_t *cs = ani_store_open_path(db_path);
        if (!cs)
            _exit(1);
        ani_store_begin_bulk(cs);
        ani_store_begin(cs); /* explicit open transaction */
        ani_store_upsert_project(cs, "crashed", "/tmp/crashed");
    ...
    ani_store_t *recovered = ani_store_open_path(db_path);
    ASSERT_NOT_NULL(recovered); /* NULL would indicate corruption */

    /* Baseline commit must survive. */
    ani_project_t p = {0};
    rc = ani_store_get_project(recovered, "baseline", &p);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ...
    ani_project_t p2 = {0};
    int rc2 = ani_store_get_project(recovered, "crashed", &p2);
    ASSERT_NEQ(rc2, ANI_STORE_OK); /* row must be absent */

    ani_store_close(recovered);
    cleanup_db(db_path);
    PASS();
}
#endif /* _WIN32 */
