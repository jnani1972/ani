...
# test-only injector (ANI_TEST_HANG_ON) busy-spins on one file, so the worker
...
# never ship — see src/main.c and TEST_SEAMS in Makefile.ani). Against a
...
if ! LC_ALL=C grep -a -q -F 'ANI_TEST_WORKER_DESCENDANT_PID_FILE' "${BINARY}"; then
...
ANI_BINARY="${BINARY}" BUILD_FINGERPRINT="${BUILD_FINGERPRINT}" TMPDIR_PATH="${tmpdir}" \
  ARGS_JSON="{\"repo_path\":\"${tmpdir}/repo\"}" \
  ANI_TEST_HANG_ON=hang_me \
  ANI_TEST_WORKER_DESCENDANT_PID_FILE="${tmpdir}/descendant.pid" \
  "${tmpdir}/wrapper.sh" &
wrapper_pid=$!
