...
static git_env_snapshot_t save_git_env(void) {
    ...
    snapshot.home = home ? ani_strdup(home) : NULL;
    snapshot.xdg_config_home = xdg ? ani_strdup(xdg) : NULL;
    return snapshot;
}

static void restore_git_env(git_env_snapshot_t *snapshot) {
    if (snapshot->home) {
        ani_setenv("HOME", snapshot->home, 1);
        free(snapshot->home);
    } else {
        ani_unsetenv("HOME");
    }
    ...
    if (snapshot->xdg_config_home) {
        ani_setenv("XDG_CONFIG_HOME", snapshot->xdg_config_home, 1);
...
static bool discover_has_rel_path(const ani_file_info_t *files, int count, const char *rel_path) {
...
TEST(skip_git) {
    ASSERT_TRUE(ani_should_skip_dir(".git", ANI_MODE_FULL));
    PASS();
}
TEST(skip_node_modules) {
    ASSERT_TRUE(ani_should_skip_dir("node_modules", ANI_MODE_FULL));
    PASS();
}
TEST(skip_pycache) {
    ASSERT_TRUE(ani_should_skip_dir("__pycache__", ANI_MODE_FULL));
    PASS();
}
TEST(skip_venv) {
    ASSERT_TRUE(ani_should_skip_dir("venv", ANI_MODE_FULL));
    PASS();
}
TEST(skip_dist) {
    ASSERT_TRUE(ani_should_skip_dir("dist", ANI_MODE_FULL));
    PASS();
}
TEST(skip_target) {
    ASSERT_TRUE(ani_should_skip_dir("target", ANI_MODE_FULL));
    PASS();
}
TEST(skip_vendor) {
    ASSERT_TRUE(ani_should_skip_dir("vendor", ANI_MODE_FULL));
    PASS();
}
TEST(skip_vendored) {
    ASSERT_TRUE(ani_should_skip_dir("vendored", ANI_MODE_FULL));
    PASS();
}
TEST(skip_terraform) {
    ASSERT_TRUE(ani_should_skip_dir(".terraform", ANI_MODE_FULL));
    PASS();
}
TEST(skip_coverage) {
    ASSERT_TRUE(ani_should_skip_dir("coverage", ANI_MODE_FULL));
    PASS();
}
TEST(skip_idea) {
    ASSERT_TRUE(ani_should_skip_dir(".idea", ANI_MODE_FULL));
    PASS();
}
TEST(skip_claude) {
    ASSERT_TRUE(ani_should_skip_dir(".claude", ANI_MODE_FULL));
    PASS();
}

/* Not skipped in full mode */
TEST(no_skip_src) {
    ASSERT_FALSE(ani_should_skip_dir("src", ANI_MODE_FULL));
    PASS();
}
TEST(no_skip_lib) {
    ASSERT_FALSE(ani_should_skip_dir("lib", ANI_MODE_FULL));
    PASS();
}
TEST(no_skip_docs_full) {
    ASSERT_FALSE(ani_should_skip_dir("docs", ANI_MODE_FULL));
    PASS();
}
TEST(no_skip_test_full) {
    ASSERT_FALSE(ani_should_skip_dir("__tests__", ANI_MODE_FULL));
    PASS();
}

/* Fast mode additional skips */
TEST(skip_fast_docs) {
    ASSERT_TRUE(ani_should_skip_dir("docs", ANI_MODE_FAST));
    PASS();
}
TEST(skip_fast_examples) {
    ASSERT_TRUE(ani_should_skip_dir("examples", ANI_MODE_FAST));
    PASS();
}
TEST(skip_fast_tests) {
    ASSERT_TRUE(ani_should_skip_dir("__tests__", ANI_MODE_FAST));
    PASS();
}
TEST(skip_fast_fixtures) {
    ASSERT_TRUE(ani_should_skip_dir("fixtures", ANI_MODE_FAST));
    PASS();
}
TEST(skip_fast_testdata) {
    ASSERT_TRUE(ani_should_skip_dir("testdata", ANI_MODE_FAST));
    PASS();
}
TEST(skip_fast_generated) {
    ASSERT_TRUE(ani_should_skip_dir("generated", ANI_MODE_FAST));
    PASS();
}
TEST(skip_fast_assets) {
    ASSERT_TRUE(ani_should_skip_dir("assets", ANI_MODE_FAST));
    PASS();
}
TEST(skip_fast_3rdparty) {
    ASSERT_TRUE(ani_should_skip_dir("third_party", ANI_MODE_FAST));
    PASS();
}
TEST(skip_fast_e2e) {
    ASSERT_TRUE(ani_should_skip_dir("e2e", ANI_MODE_FAST));
    PASS();
}
...
TEST(suffix_pyc) {
    ASSERT_TRUE(ani_has_ignored_suffix("module.pyc", ANI_MODE_FULL));
    PASS();
}
TEST(suffix_o) {
    ASSERT_TRUE(ani_has_ignored_suffix("main.o", ANI_MODE_FULL));
    PASS();
}
TEST(suffix_so) {
    ASSERT_TRUE(ani_has_ignored_suffix("lib.so", ANI_MODE_FULL));
    PASS();
}
TEST(suffix_png) {
    ASSERT_TRUE(ani_has_ignored_suffix("icon.png", ANI_MODE_FULL));
    PASS();
}
TEST(suffix_jpg) {
    ASSERT_TRUE(ani_has_ignored_suffix("photo.jpg", ANI_MODE_FULL));
    PASS();
}
TEST(suffix_wasm) {
    ASSERT_TRUE(ani_has_ignored_suffix("app.wasm", ANI_MODE_FULL));
    PASS();
}
TEST(suffix_db) {
    ASSERT_TRUE(ani_has_ignored_suffix("data.db", ANI_MODE_FULL));
    PASS();
}
TEST(suffix_sqlite) {
    ASSERT_TRUE(ani_has_ignored_suffix("store.sqlite3", ANI_MODE_FULL));
    PASS();
}
TEST(suffix_tmp) {
    ASSERT_TRUE(ani_has_ignored_suffix("file.tmp", ANI_MODE_FULL));
    PASS();
}
TEST(suffix_tilde) {
    ASSERT_TRUE(ani_has_ignored_suffix("file~", ANI_MODE_FULL));
    PASS();
}

/* Not ignored */
TEST(suffix_go) {
    ASSERT_FALSE(ani_has_ignored_suffix("main.go", ANI_MODE_FULL));
    PASS();
}
TEST(suffix_py) {
    ASSERT_FALSE(ani_has_ignored_suffix("app.py", ANI_MODE_FULL));
    PASS();
}
TEST(suffix_c) {
    ASSERT_FALSE(ani_has_ignored_suffix("lib.c", ANI_MODE_FULL));
    PASS();
}

/* Fast mode additional suffixes */
TEST(suffix_fast_zip) {
    ASSERT_TRUE(ani_has_ignored_suffix("archive.zip", ANI_MODE_FAST));
    PASS();
}
TEST(suffix_fast_pdf) {
    ASSERT_TRUE(ani_has_ignored_suffix("manual.pdf", ANI_MODE_FAST));
    PASS();
}
TEST(suffix_fast_mp3) {
    ASSERT_TRUE(ani_has_ignored_suffix("sound.mp3", ANI_MODE_FAST));
    PASS();
}
TEST(suffix_fast_pem) {
    ASSERT_TRUE(ani_has_ignored_suffix("cert.pem", ANI_MODE_FAST));
    PASS();
}
...
TEST(fn_skip_license) {
    ASSERT_TRUE(ani_should_skip_filename("LICENSE", ANI_MODE_FAST));
    PASS();
}
TEST(fn_skip_changelog) {
    ASSERT_TRUE(ani_should_skip_filename("CHANGELOG.md", ANI_MODE_FAST));
    PASS();
}
TEST(fn_skip_gosum) {
    ASSERT_TRUE(ani_should_skip_filename("go.sum", ANI_MODE_FAST));
    PASS();
}
TEST(fn_skip_yarnlock) {
    ASSERT_TRUE(ani_should_skip_filename("yarn.lock", ANI_MODE_FAST));
    PASS();
}
TEST(fn_skip_pkglock) {
    ASSERT_TRUE(ani_should_skip_filename("package-lock.json", ANI_MODE_FAST));
    PASS();
}

/* Not skipped in full mode */
TEST(fn_no_skip_license_full) {
    ASSERT_FALSE(ani_should_skip_filename("LICENSE", ANI_MODE_FULL));
    PASS();
}
...
TEST(pattern_dts) {
    ASSERT_TRUE(ani_matches_fast_pattern("types.d.ts", ANI_MODE_FAST));
    PASS();
}
TEST(pattern_pbgo) {
    ASSERT_TRUE(ani_matches_fast_pattern("service.pb.go", ANI_MODE_FAST));
    PASS();
}
TEST(pattern_pb2py) {
    ASSERT_TRUE(ani_matches_fast_pattern("api_pb2.py", ANI_MODE_FAST));
    PASS();
}
TEST(pattern_mock) {
    ASSERT_TRUE(ani_matches_fast_pattern("mock_service.go", ANI_MODE_FAST));
    PASS();
}
TEST(pattern_test_dot) {
    ASSERT_TRUE(ani_matches_fast_pattern("App.test.js", ANI_MODE_FAST));
    PASS();
}
TEST(pattern_spec) {
    ASSERT_TRUE(ani_matches_fast_pattern("App.spec.ts", ANI_MODE_FAST));
    PASS();
}
TEST(pattern_stories) {
    ASSERT_TRUE(ani_matches_fast_pattern("Button.stories.tsx", ANI_MODE_FAST));
    PASS();
}

/* Not matched in full mode */
TEST(pattern_dts_full) {
    ASSERT_FALSE(ani_matches_fast_pattern("types.d.ts", ANI_MODE_FULL));
    PASS();
}

/* ── File discovery (integration) — cross-platform via test_helpers.h ── */

/* A custom ANI_CACHE_DIR may legitimately sit inside a repository — tests do it
...
TEST(discover_prunes_the_cache_tree) {
    char *base = th_mktempdir("ani_disc_cache");
    ASSERT(base != NULL);
    ...
    ani_discover_opts_t opts = {0};
    ani_file_info_t *files = NULL;
    int count = 0;
    int rc = ani_discover(base, &opts, &files, &count);
    ...
    ani_discover_free(files, count);
    th_cleanup(base);
    PASS();
}

TEST(discover_simple) {
    char *base = th_mktempdir("ani_disc_simple");
    ASSERT(base != NULL);
    ...
    for (int i = 0; i < count; i++) {
        if (files[i].language == ANI_LANG_GO)
            found_go = true;
...
TEST(discover_wide_sibling_fanout_exceeds_initial_walk_stack) {
    char *base = th_mktempdir("ani_disc_wide");
    ASSERT(base != NULL);
...
TEST(discover_bounded_count_is_allocation_free_and_limit_exact) {
    char *base = th_mktempdir("ani_disc_bounded");
    ASSERT(base != NULL);
    ...
    int limited_count = -1;
    ani_discover_status_t limited =
        ani_discover_count_bounded(base, &opts, 1, ani_now_ms() + 2000, &limited_count);
    int exact_count = -1;
    ani_discover_status_t exact =
        ani_discover_count_bounded(base, &opts, 2, ani_now_ms() + 2000, &exact_count);

    th_cleanup(base);
    ASSERT_EQ(limited, ANI_DISCOVER_LIMIT_EXCEEDED);
    ASSERT_EQ(limited_count, 1);
    ASSERT_EQ(exact, ANI_DISCOVER_OK);
    ASSERT_EQ(exact_count, 2);
    PASS();
}

TEST(discover_bounded_count_fails_closed_after_deadline) {
    char *base = th_mktempdir("ani_disc_deadline");
    ASSERT(base != NULL);
    th_write_file(TH_PATH(base, "source.c"), "int source;\n");

    ani_discover_opts_t opts = {.mode = ANI_MODE_FULL};
    int count = 99;
    ani_discover_status_t status = ani_discover_count_bounded(base, &opts, 100, 1, &count);

    th_cleanup(base);
    ASSERT_EQ(status, ANI_DISCOVER_ERROR);
    ASSERT_EQ(count, -1);
    PASS();
}
...
TEST(discover_bounded_count_matches_shebang_discovery) {
    char *base = th_mktempdir("ani_disc_bounded_sb");
    ASSERT(base != NULL);
    ...
    ani_file_info_t *files = NULL;
    int count = 0;
    int full_rc = ani_discover(base, &opts, &files, &count);
    ...
    int exact_count = -1;
    ani_discover_status_t exact =
        ani_discover_count_bounded(base, &opts, 3, ani_now_ms() + 2000, &exact_count);
    int limited_count = -1;
    ani_discover_status_t limited =
        ani_discover_count_bounded(base, &opts, 2, ani_now_ms() + 2000, &limited_count);
...
TEST(discover_skips_git_dir) {
    char *base = th_mktempdir("ani_disc_git");
    ASSERT(base != NULL);
...
TEST(discover_with_gitignore) {
    char *base = th_mktempdir("ani_disc_gi");
    ASSERT(base != NULL);
    ...
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(count, 1);
    ASSERT_EQ(files[0].language, ANI_LANG_GO);
...
TEST(discover_with_global_xdg_ignore) {
    ...
    char *tmp = th_mktempdir("ani_disc_global_xdg");
    ASSERT(tmp != NULL);
    ...
    ani_setenv("XDG_CONFIG_HOME", xdg_env, 1);
    ...
    int rc = ani_discover(repo, &opts, &files, &count);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(count, 1);
...
TEST(discover_global_excludesfile_from_gitconfig_tilde) {
    git_env_snapshot_t env = save_git_env();
    char *tmp = th_mktempdir("ani_disc_global_cfg");
    ASSERT(tmp != NULL);
...
TEST(discover_repo_local_excludesfile_is_ignored) {
    git_env_snapshot_t env = save_git_env();
    char *tmp = th_mktempdir("ani_disc_repo_cfg_ignored");
    ASSERT(tmp != NULL);
...
TEST(discover_missing_global_excludes_is_noop) {
    git_env_snapshot_t env = save_git_env();
    char *tmp = th_mktempdir("ani_disc_global_missing");
    ASSERT(tmp != NULL);
...
TEST(discover_aniignore_negates_global_ignore) {
    git_env_snapshot_t env = save_git_env();
    char *tmp = th_mktempdir("ani_disc_global_neg");
    ASSERT(tmp != NULL);
    ...
    th_write_file(TH_PATH(repo, ".aniignore"), "!rescued.go\n");
    th_write_file(TH_PATH(repo, "main.go"), "package main\n");
...
TEST(discover_gitignore_dir_excluded_issue234) {
    char *base = th_mktempdir("ani_disc_gi234");
    ASSERT(base != NULL);
...
TEST(discover_max_file_size) {
    char *base = th_mktempdir("ani_disc_size");
    ASSERT(base != NULL);
...
TEST(discover_null_path) {
    ani_file_info_t *files = NULL;
    int count = 0;
    int rc = ani_discover(NULL, NULL, &files, &count);
    ASSERT_EQ(rc, -1);
    PASS();
}

TEST(discover_nonexistent_path) {
    char *base = th_mktempdir("ani_disc_noexist");
    char fake[512];
    snprintf(fake, sizeof(fake), "%s/nonexistent_12345", base ? base : "/tmp");
    ani_file_info_t *files = NULL;
    int count = 0;
    int rc = ani_discover(fake, NULL, &files, &count);
    ASSERT_EQ(rc, -1);
    th_cleanup(base);
    PASS();
}
...
TEST(discover_free_null) {
    ani_discover_free(NULL, 0);
    PASS();
}

TEST(discover_skips_worktrees) {
    char *base = th_mktempdir("ani_disc_wt");
    ASSERT(base != NULL);
...
TEST(discover_aniignore) {
    char *base = th_mktempdir("ani_disc_anii");
    ASSERT(base != NULL);

    th_mkdir_p(TH_PATH(base, ".git"));
    th_write_file(TH_PATH(base, ".aniignore"), "generated/\n*.pb.go\n");
...
TEST(discover_aniignore_stacks) {
    char *base = th_mktempdir("ani_disc_stack");
    ASSERT(base != NULL);

    th_mkdir_p(TH_PATH(base, ".git"));
    th_write_file(TH_PATH(base, ".gitignore"), "*.log\n");
    th_write_file(TH_PATH(base, ".aniignore"), "docs/\n");
    th_write_file(TH_PATH(base, "main.go"), "package main\n");
...
#else
    char *base = th_mktempdir("ani_disc_sym");
    ASSERT(base != NULL);
...
TEST(discover_new_ignore_patterns) {
    char *base = th_mktempdir("ani_disc_newign");
    ASSERT(base != NULL);
...
TEST(discover_generic_dirs_full_mode) {
    char *base = th_mktempdir("ani_disc_genfull");
    ASSERT(base != NULL);
...
TEST(discover_generic_dirs_fast_mode) {
    char *base = th_mktempdir("ani_disc_genfast");
    ASSERT(base != NULL);
    ...
    ani_discover_opts_t opts = {.mode = ANI_MODE_FAST};
    ani_file_info_t *files = NULL;
    int count = 0;
...
TEST(discover_deploy_excluded_full_mode) {
    char *base = th_mktempdir("ani_disc_deploy");
    ASSERT(base != NULL);
...
TEST(discover_aniignore_no_git) {
    char *base = th_mktempdir("ani_disc_nogit");
    ASSERT(base != NULL);

    th_write_file(TH_PATH(base, ".aniignore"), "scratch/\n");
    th_write_file(TH_PATH(base, "main.go"), "package main\n");
...
/* ── .aniignore negation vs built-in skip dirs (issue #500) ────── */
...
/* A "!obj/" negation in .aniignore must un-skip the built-in ALWAYS_SKIP
...
TEST(discover_aniignore_negates_always_skip_dir) {
    char *base = th_mktempdir("ani_disc_anii_neg_obj");
    ASSERT(base != NULL);

    th_write_file(TH_PATH(base, ".aniignore"), "!obj/\n");
    th_write_file(TH_PATH(base, "main.go"), "package main\n");
    ...
    int rc = ani_discover_ex(base, &opts, &files, &count, &excluded, &excluded_count);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(count, 2);
    ASSERT_TRUE(discover_has_rel_path(files, count, "main.go"));
    ...
    ani_discover_free_excluded(excluded, excluded_count);
    ani_discover_free(files, count);
    th_cleanup(base);
    PASS();
}
...
TEST(discover_aniignore_negates_only_nested_skip_dir) {
    char *base = th_mktempdir("ani_disc_anii_neg_nested");
    ASSERT(base != NULL);

    th_write_file(TH_PATH(base, ".aniignore"), "!src/target/\n");
...
TEST(discover_aniignore_negates_fast_skip_dir) {
    char *base = th_mktempdir("ani_disc_anii_neg_fast");
    ASSERT(base != NULL);
...
TEST(discover_aniignore_negation_last_match_wins) {
    char *base = th_mktempdir("ani_disc_anii_order1");
    ASSERT(base != NULL);
    ...
    base = th_mktempdir("ani_disc_anii_order2");
    ASSERT(base != NULL);
    ...
    rc = ani_discover(base, &opts, &files, &count);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(count, 1);
    ASSERT_TRUE(discover_has_rel_path(files, count, "main.go"));
 ...
 * .aniignore negation. Green by construction; guards the policy. */
TEST(discover_aniignore_negation_cannot_unskip_safety_core) {
    char *base = th_mktempdir("ani_disc_anii_safety");
    ASSERT(base != NULL);

    th_write_file(TH_PATH(base, ".aniignore"),
                  "!.git/\n!node_modules/\n!.worktrees/\n!.claude-worktrees/\n");
    th_write_file(TH_PATH(base, "main.go"), "package main\n");
...
TEST(discover_git_info_exclude) {
    char *base = th_mktempdir("ani_disc_exc");
    ASSERT(base != NULL);
...
TEST(discover_git_info_exclude_stacks_with_gitignore) {
    char *base = th_mktempdir("ani_disc_exc_stack");
    ASSERT(base != NULL);
...
TEST(discover_worktree_info_exclude) {
    char *base = th_mktempdir("ani_disc_wt_exc");
    ASSERT(base != NULL);
...
TEST(discover_worktree_committed_gitignore) {
    char *base = th_mktempdir("ani_disc_wt_gi");
    ASSERT(base != NULL);
...
TEST(discover_nested_gitignore) {
    char *base = th_mktempdir("ani_disc_ngi");
    ASSERT(base != NULL);
...
TEST(discover_nested_gitignore_stacks_with_root) {
    char *base = th_mktempdir("ani_disc_ngi_stack");
    ASSERT(base != NULL);
...
TEST(discover_many_nested_gitignores_do_not_exhaust_matcher_ownership) {
    ...
    char *base = th_mktempdir("ani_disc_many_ngi");
    ASSERT(base != NULL);
    ...
    ani_file_info_t *files = NULL;
    int count = 0;
    int discover_rc = fixture_ready ? ani_discover(base, &opts, &files, &count) : -1;
    int bounded_count = -1;
    ani_discover_status_t bounded_status =
        fixture_ready
            ? ani_discover_count_bounded(base, &opts, PACKAGE_COUNT + 1, 0, &bounded_count)
            : ANI_DISCOVER_ERROR;
    ...
    ASSERT_EQ(bounded_status, ANI_DISCOVER_OK);
...
/* Language detected for a discovered file by relative path, or ANI_LANG_COUNT
 * if the file was not indexed. */
static ANILanguage discover_lang_of(const ani_file_info_t *files, int count, const char *rel) {
    for (int i = 0; i < count; i++) {
 ...
 * language it was classified as via *out_lang (ANI_LANG_COUNT if unindexed).
 ...
 * -- otherwise a negative expectation (ANI_LANG_COUNT) could pass vacuously
 * when setup silently failed. */
static bool shebang_probe(const char *prefix, const char *rel, const char *content,
                          ANILanguage *out_lang) {
    *out_lang = ANI_LANG_COUNT;
    ...
    if (th_write_file(TH_PATH(base, rel), content) == 0) {
        ...
        ani_file_info_t *files = NULL;
        int count = 0;
        if (ani_discover(base, &opts, &files, &count) == 0) {
...
TEST(shebang_direct_python_path) {
    ANILanguage lang;
    ASSERT(shebang_probe("ani_sb_dpy", "bin/run", "#!/usr/bin/python3\nprint(1)\n", &lang));
    ASSERT_EQ(lang, ANI_LANG_PYTHON);
    PASS();
}

TEST(shebang_env_python) {
    ANILanguage lang;
    ASSERT(shebang_probe("ani_sb_epy", "bin/run", "#!/usr/bin/env python3\nprint(1)\n", &lang));
    ASSERT_EQ(lang, ANI_LANG_PYTHON);
    PASS();
}
...
TEST(shebang_env_split_string_python_args) {
    ANILanguage lang;
    ASSERT(
        shebang_probe("ani_sb_spy", "bin/run", "#!/usr/bin/env -S python3 -u\nprint(1)\n", &lang));
    ASSERT_EQ(lang, ANI_LANG_PYTHON);
    PASS();
}
...
TEST(shebang_versioned_python) {
    ANILanguage lang;
    ASSERT(shebang_probe("ani_sb_vpy", "bin/run", "#!/usr/bin/python3.12\nprint(1)\n", &lang));
    ASSERT_EQ(lang, ANI_LANG_PYTHON);
    PASS();
}
...
TEST(shebang_bash_direct) {
    ANILanguage lang;
    ASSERT(shebang_probe("ani_sb_bash", "bin/run", "#!/bin/bash\necho hi\n", &lang));
    ASSERT_EQ(lang, ANI_LANG_BASH);
    PASS();
}

TEST(shebang_env_node_javascript) {
    ANILanguage lang;
    ASSERT(shebang_probe("ani_sb_node", "bin/run", "#!/usr/bin/env node\nconsole.log(1)\n", &lang));
    ASSERT_EQ(lang, ANI_LANG_JAVASCRIPT);
    PASS();
}

TEST(shebang_crlf_first_line) {
    ANILanguage lang;
    ASSERT(shebang_probe("ani_sb_crlf", "bin/run", "#!/usr/bin/env python3\r\nprint(1)\n", &lang));
    ASSERT_EQ(lang, ANI_LANG_PYTHON);
    PASS();
}
...
TEST(shebang_extension_authoritative) {
    ANILanguage lang;
    ASSERT(shebang_probe("ani_sb_ext", "weird.py", "#!/bin/bash\nprint(1)\n", &lang));
    ASSERT_EQ(lang, ANI_LANG_PYTHON);
    PASS();
}
...
TEST(shebang_special_filename_authoritative) {
    ANILanguage lang;
    ASSERT(shebang_probe("ani_sb_mk", "Makefile", "#!/usr/bin/env python3\nall:\n\techo\n", &lang));
    ASSERT_EQ(lang, ANI_LANG_MAKEFILE);
    PASS();
}

TEST(shebang_plain_text_unindexed) {
    ANILanguage lang;
    ASSERT(shebang_probe("ani_sb_txt", "notes", "just some plain text\nno shebang here\n", &lang));
    ASSERT_EQ(lang, ANI_LANG_COUNT);
    PASS();
}
...
TEST(shebang_malformed_unindexed) {
    ...
    ANILanguage lang;
    ASSERT(shebang_probe("ani_sb_mal", "notes", "#/usr/bin/python3\n", &lang));
    ASSERT_EQ(lang, ANI_LANG_COUNT);
    PASS();
}
...
TEST(shebang_unknown_interpreter_unindexed) {
    ANILanguage lang;
    ASSERT(shebang_probe("ani_sb_unk", "notes", "#!/usr/bin/frobnicate\ndata\n", &lang));
    ASSERT_EQ(lang, ANI_LANG_COUNT);
    PASS();
}
...
TEST(shebang_prefix_not_matched_unindexed) {
    ANILanguage lang;
    ASSERT(shebang_probe("ani_sb_pw", "notes", "#!/usr/bin/python-wrapper\nx\n", &lang));
    ASSERT_EQ(lang, ANI_LANG_COUNT);
    PASS();
}
...
TEST(shebang_malformed_python_version_unindexed) {
    ANILanguage lang;
    ASSERT(shebang_probe("ani_sb_pdot", "notes", "#!/usr/bin/python3.\nx\n", &lang));
    ASSERT_EQ(lang, ANI_LANG_COUNT);
    ASSERT(shebang_probe("ani_sb_pddot", "notes", "#!/usr/bin/python3..12\nx\n", &lang));
    ASSERT_EQ(lang, ANI_LANG_COUNT);
    PASS();
}
...
TEST(shebang_env_unsupported_option_unindexed) {
    ANILanguage lang;
    ASSERT(shebang_probe("ani_sb_eopt", "notes", "#!/usr/bin/env -i python3\nx\n", &lang));
    ASSERT_EQ(lang, ANI_LANG_COUNT);
    ASSERT(shebang_probe("ani_sb_eopt2", "notes", "#!/usr/bin/env -S -i python3\nx\n", &lang));
    ASSERT_EQ(lang, ANI_LANG_COUNT);
    PASS();
}

TEST(shebang_embedded_nul_unindexed) {
    char *base = th_mktempdir("ani_sb_nul");
    ASSERT(base != NULL);
    ...
    ani_file_info_t *files = NULL;
    int count = 0;
    ASSERT_EQ(ani_discover(base, &opts, &files, &count), 0);
...
TEST(shebang_oversized_first_line_unindexed) {
    char *base = th_mktempdir("ani_sb_long");
    ASSERT(base != NULL);
...
SUITE(discover) {
    ...
    RUN_TEST(discover_aniignore_negates_global_ignore);
    ...
    RUN_TEST(discover_aniignore);
    RUN_TEST(discover_aniignore_stacks);
    ...
    RUN_TEST(discover_aniignore_no_git);

    /* .aniignore negation vs built-in skip dirs (issue #500) */
    RUN_TEST(discover_aniignore_negates_always_skip_dir);
    RUN_TEST(discover_aniignore_negates_only_nested_skip_dir);
    RUN_TEST(discover_aniignore_negates_fast_skip_dir);
    RUN_TEST(discover_aniignore_negation_last_match_wins);
    RUN_TEST(discover_aniignore_negation_cannot_unskip_safety_core);
