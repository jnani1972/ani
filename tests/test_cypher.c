...
TEST(cypher_lex_simple_match) {
    ani_lex_result_t r = {0};
    int rc = ani_lex("MATCH (n:Function)", &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
...
TEST(cypher_lex_string_overflow) {
    ...
    ani_lex_result_t r = {0};
    int rc = ani_lex(query, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
    ASSERT_GTE(r.count, 1);
    ASSERT_EQ(r.tokens[0].type, TOK_STRING);
    /* The string should be truncated to ANI_SZ_4K - 1 (4095) characters. */
...
TEST(cypher_lex_keywords_case_insensitive) {
    ani_lex_result_t r = {0};
    int rc = ani_lex("match WHERE Return limit", &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.tokens[0].type, TOK_MATCH);
...
TEST(cypher_lex_pipe_and_star) {
    ani_lex_result_t r = {0};
    int rc = ani_lex("[:TYPE1|TYPE2*1..3]", &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_simple_node) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f:Function)", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(err);
    ASSERT_NOT_NULL(q);

    ASSERT_EQ(ani_query_pattern(q).node_count, 1);
    ASSERT_EQ(ani_query_pattern(q).rel_count, 0);
    ASSERT_STR_EQ(ani_query_pattern(q).nodes[0].variable, "f");
    ASSERT_STR_EQ(ani_query_pattern(q).nodes[0].label, "Function");
...
TEST(cypher_parse_rejects_trailing_tokens) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f:Function) RETURN f.name AS n BANANA SPLIT 99", &q, &err);
    ASSERT_NEQ(rc, 0);
    ASSERT_NOT_NULL(err);
    ASSERT_NULL(q);
...
TEST(cypher_parse_rejects_second_with_clause) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f:Function) "
...
TEST(cypher_parse_relationship_outbound) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f:Function)-[:CALLS]->(g:Function)", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);

    ASSERT_EQ(ani_query_pattern(q).node_count, 2);
    ASSERT_EQ(ani_query_pattern(q).rel_count, 1);
    ASSERT_STR_EQ(ani_query_pattern(q).rels[0].types[0], "CALLS");
    ASSERT_STR_EQ(ani_query_pattern(q).rels[0].direction, "outbound");
    ASSERT_EQ(ani_query_pattern(q).rels[0].min_hops, 1);
    ASSERT_EQ(ani_query_pattern(q).rels[0].max_hops, 1);
...
TEST(cypher_parse_relationship_inbound) {
    ...
    ASSERT_STR_EQ(ani_query_pattern(q).rels[0].direction, "inbound");
...
TEST(cypher_parse_relationship_any) {
    ...
    ASSERT_STR_EQ(ani_query_pattern(q).rels[0].direction, "any");
...
TEST(cypher_parse_variable_length) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f:Function)-[:CALLS*1..3]->(g:Function)", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);

    ASSERT_EQ(ani_query_pattern(q).rels[0].min_hops, 1);
    ASSERT_EQ(ani_query_pattern(q).rels[0].max_hops, 3);
...
TEST(cypher_parse_variable_length_unbounded) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f)-[:CALLS*]->(g)", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);

    ASSERT_EQ(ani_query_pattern(q).rels[0].min_hops, 1);
    ASSERT_EQ(ani_query_pattern(q).rels[0].max_hops, 0); /* 0 = unbounded */
...
TEST(cypher_parse_multiple_edge_types) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f)-[:CALLS|HTTP_CALLS]->(g)", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);

    ASSERT_EQ(ani_query_pattern(q).rels[0].type_count, 2);
    ASSERT_STR_EQ(ani_query_pattern(q).rels[0].types[0], "CALLS");
    ASSERT_STR_EQ(ani_query_pattern(q).rels[0].types[1], "HTTP_CALLS");
...
TEST(cypher_parse_where_clause) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f:Function) WHERE f.name = \"Foo\"", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);
...
TEST(cypher_parse_where_regex) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f:Function) WHERE f.name =~ \".*Order.*\"", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->where->root);
    ASSERT_EQ(q->where->root->type, EXPR_CONDITION);
    ASSERT_STR_EQ(q->where->root->cond.op, "=~");
...
TEST(cypher_parse_where_and) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f:Function) WHERE f.name = \"A\" AND f.label = \"Function\"",
                              &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->where->root);
...
TEST(cypher_parse_return_simple) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f:Function) RETURN f.name, f.qualified_name", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_return_count) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f)-[:CALLS]->(g) RETURN f.name, COUNT(g) AS cnt", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(q->ret->count, 2);
...
TEST(cypher_parse_return_order_limit) {
    ...
    char *err = NULL;
    int rc =
        ani_cypher_parse("MATCH (f:Function) RETURN f.name ORDER BY f.name DESC LIMIT 5", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_order_by_over_cap_rejected_issue1334) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f:Function) RETURN f.name ORDER BY "
...
TEST(cypher_parse_return_distinct) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f:Function) RETURN DISTINCT f.label", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_inline_props) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f:Function {name: \"Foo\"})", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(ani_query_pattern(q).nodes[0].prop_count, 1);
    ASSERT_STR_EQ(ani_query_pattern(q).nodes[0].props[0].key, "name");
    ASSERT_STR_EQ(ani_query_pattern(q).nodes[0].props[0].value, "Foo");
...
TEST(cypher_parse_error) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("INVALID QUERY", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
    free(err);
    PASS();
 ...
 */
static ani_store_t *setup_cypher_store(void) {
    ...
    int64_t id3 = ani_store_upsert_node(s, &n3);
    int64_t id4 = ani_store_upsert_node(s, &n4);
    int64_t id5 = ani_store_upsert_node(s, &n5);

    ani_edge_t e1 = {.project = "test", .source_id = id1, .target_id = id2, .type = "CALLS"};
    ani_edge_t e2 = {.project = "test", .source_id = id2, .target_id = id3, .type = "CALLS"};
    ani_edge_t e3 = {.project = "test", .source_id = id1, .target_id = id5, .type = "CALLS"};
    ani_edge_t e4 = {.project = "test", .source_id = id4, .target_id = id1, .type = "DEFINES"};
...
TEST(cypher_deep_nesting_rejected_not_crash) {
    ...
    if (pid == 0) {
        ...
        q[n] = '\0';
        ani_store_t *s = setup_cypher_store();
        ani_cypher_result_t r = {0};
        ...
        (void)ani_cypher_execute(s, q, "test", 0, &r);
        ani_cypher_result_free(&r);
        ani_store_close(s);
        free(q);
        _exit(0);
    }
...
TEST(cypher_exec_match_all_functions) {
    ...
    int rc = ani_cypher_execute(s, "MATCH (f:Function)", "test", 0, &r);
    ASSERT_EQ(rc, 0);
 ...
 * bounds-checked in size_t by ani_cypher_cross_join_alloc — exercised at its
...
TEST(cypher_exec_optional_empty_label_no_overflow) {
    ani_store_t *s = setup_cypher_store(); /* 4 Function nodes */
    ani_cypher_result_t r = {0};

    int rc = ani_cypher_execute(
...
TEST(cypher_exec_optional_rel_saturated_no_overflow) {
    ...
    int64_t hub_id = ani_store_upsert_node(s, &hub);
    for (int i = 0; i < 20; i++) {
        char nm[32];
        char qn[48];
    ...
    for (int i = 0; i < 300; i++) {
        ...
        ani_node_t callee = {.project = "test", .label = "Var", .name = nm, .qualified_name = qn};
        int64_t cid = ani_store_upsert_node(s, &callee);
        ani_edge_t e = {.project = "test", .source_id = hub_id, .target_id = cid, .type = "CALLS"};
...
TEST(cypher_exec_optional_saturated_does_not_fabricate_no_match) {
    ...
    int64_t a_id = ani_store_upsert_node(s, &a);
    (void)ani_store_upsert_node(s, &b); /* B: no outgoing CALLS at all */
    int64_t c_id = ani_store_upsert_node(s, &c);
    ASSERT_GT(a_id, 0);
    ASSERT_GT(c_id, 0);
    ...
    for (int i = 0; i < 40; i++) {
        ...
        ani_node_t t = {.project = "test", .label = "Var", .name = nm, .qualified_name = qn};
        int64_t tid = ani_store_upsert_node(s, &t);
        ani_edge_t e = {.project = "test", .source_id = a_id, .target_id = tid, .type = "CALLS"};
        ani_store_insert_edge(s, &e);
    }
    for (int i = 0; i < 5; i++) {
        char nm[32];
        ...
        ani_node_t t = {.project = "test", .label = "Var", .name = nm, .qualified_name = qn};
        int64_t tid = ani_store_upsert_node(s, &t);
        ani_edge_t e = {.project = "test", .source_id = c_id, .target_id = tid, .type = "CALLS"};
        ani_store_insert_edge(s, &e);
    }
 ...
 * #627 failure mode). ani_cypher_cross_join_alloc now computes it in size_t and
...
TEST(cypher_exec_optional_rel_leaf_fallback_survives) {
     ...
     * CYPHER_RESULT_CEILING (100000) in ani_cypher_execute before bind_cap is
...
TEST(cypher_exec_bound_terminal_optional_fallback_survives) {
    ...
    for (int i = 0; i < 21; i++) {
        ...
        ani_node_t caller = {.project = "test", .label = "Var", .name = nm, .qualified_name = qn};
        int64_t cid = ani_store_upsert_node(s, &caller);
        ani_edge_t e = {.project = "test", .source_id = cid, .target_id = hub_id, .type = "CALLS"};
        ani_store_insert_edge(s, &e);
    }
...
TEST(cypher_exec_bound_terminal_saturation_no_false_deadcode) {
    ...
    int64_t hubA_id = ani_store_upsert_node(s, &hubA);
    ani_node_t hubB = {
        .project = "test", .label = "Function", .name = "hubB", .qualified_name = "test.hubB"};
    int64_t hubB_id = ani_store_upsert_node(s, &hubB);
    ani_node_t leaf = {
        .project = "test", .label = "Function", .name = "leaf", .qualified_name = "test.leaf"};
    ani_store_upsert_node(s, &leaf);
    ...
    for (int i = 0; i < 70; i++) {
        ...
        ani_edge_t e = {.project = "test", .source_id = cid, .target_id = tgt, .type = "CALLS"};
        ani_store_insert_edge(s, &e);
    }
...
TEST(cypher_exec_where_eq) {
    ...
    int rc =
        ani_cypher_execute(s, "MATCH (f:Function) WHERE f.name = \"HandleOrder\"", "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_unlabeled_where_beyond_result_limit_issue1196) {
    ...
    for (int i = 0; i < 11; i++) {
        ...
        ani_node_t distractor = {.project = "test",
                                 .label = "Function",
                                 .name = name,
                                 .qualified_name = qn,
                                 .file_path = "early.py"};
        ASSERT_GT(ani_store_upsert_node(s, &distractor), 0);
    }

    ani_node_t late = {.project = "test",
                       .label = "Function",
                       .name = "zz_late_match",
                       .qualified_name = "test.zz_late_match",
                       .file_path = "late.py"};
    ASSERT_GT(ani_store_upsert_node(s, &late), 0);

    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s, "MATCH (n) WHERE n.name = \"zz_late_match\" RETURN n.name",
                                "test", 1, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_exec_aggregate_sees_all_edges_beyond_expansion_cap_issue1196) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        snprintf(qn, sizeof(qn), "test.%s", name);
        ani_node_t src = {.project = "test",
                          .label = "File",
                          .name = name,
                          .qualified_name = qn,
                          .file_path = name};
        src_ids[i] = ani_store_upsert_node(s, &src);
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 30; j++) {
            ...
            snprintf(qn, sizeof(qn), "test.%s", name);
            ani_node_t target = {.project = "test",
                                 .label = "Function",
                                 .name = name,
                                 .qualified_name = qn,
                                 .file_path = "defs.py"};
            int64_t tid = ani_store_upsert_node(s, &target);
            ASSERT_GT(tid, 0);
            ani_edge_t e = {
    ...
    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s, "MATCH (a:File)-[rel]->(b) RETURN count(rel)", "test", 2, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
    ...
    ani_cypher_result_t r2 = {0};
    rc = ani_cypher_execute(s, "MATCH (a:File)-[rel]->(b) RETURN b.name", "test", 2, &r2);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r2.row_count, 2);
    ani_cypher_result_free(&r2);
...
TEST(cypher_exec_varlength_path_semantics_issue797) {
    ...
    ani_node_t leaf = {.project = "test",
                       ...
                       .start_line = 5,
                       .end_line = 6};
    int64_t id_loopy = ani_store_upsert_node(s, &loopy);
    int64_t id_mid = ani_store_upsert_node(s, &mid);
    int64_t id_leaf = ani_store_upsert_node(s, &leaf);
    ASSERT_GT(id_loopy, 0);
    ani_edge_t self_loop = {
        .project = "test", .source_id = id_loopy, .target_id = id_loopy, .type = "CALLS"};
    ani_edge_t e1 = {
        .project = "test", .source_id = id_loopy, .target_id = id_mid, .type = "CALLS"};
    ani_edge_t e2 = {.project = "test", .source_id = id_mid, .target_id = id_leaf, .type = "CALLS"};
    ani_store_insert_edge(s, &self_loop);
    ani_store_insert_edge(s, &e1);
    ani_store_insert_edge(s, &e2);

    /* Bug 1: repeated variable must unify — only the self-loop matches. */
    ani_cypher_result_t r1 = {0};
    ASSERT_EQ(ani_cypher_execute(s, "MATCH (a)-[:CALLS]->(a) RETURN a.name", "test", 0, &r1), 0);
    ASSERT_EQ(r1.row_count, 1);
    ani_cypher_result_free(&r1);
    ...
    ani_cypher_result_t r2 = {0};
    ASSERT_EQ(ani_cypher_execute(s,
...
TEST(cypher_exec_where_coalesce_issue874) {
    ...
    ani_node_t c = {.project = "test",
                    ...
                    .start_line = 5,
                    .end_line = 6};
    ASSERT_GT(ani_store_upsert_node(s, &a), 0);
    ASSERT_GT(ani_store_upsert_node(s, &b), 0);
    ASSERT_GT(ani_store_upsert_node(s, &c), 0);

    /* Default FAILS the predicate: only the node with depth 3 matches. */
    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s,
...
TEST(cypher_exec_where_regex) {
    ...
    int rc =
        ani_cypher_execute(s, "MATCH (f:Function) WHERE f.name =~ \".*Order.*\"", "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_contains) {
    ...
    int rc =
        ani_cypher_execute(s, "MATCH (f:Function) WHERE f.name CONTAINS \"Order\"", "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_starts_with) {
    ...
    int rc = ani_cypher_execute(s, "MATCH (f:Function) WHERE f.name STARTS WITH \"Handle\"", "test",
                                0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_func_id) {
    ani_store_t *s = setup_cypher_store();
    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s, "MATCH (f:Function) WHERE f.name = \"HandleOrder\" RETURN id(f)",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_issue874_where_unsupported_func_error) {
    ...
    char *err = NULL;
    int rc =
        ani_cypher_parse("MATCH (f:Function) WHERE foo(f.name) = \"x\" RETURN f.name", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_exec_limit) {
    ...
    int rc = ani_cypher_execute(s, "MATCH (f:Function) RETURN f.name LIMIT 2", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 2);
...
TEST(cypher_exec_order_by) {
    ...
    int rc = ani_cypher_execute(s, "MATCH (f:Function) RETURN f.name ORDER BY f.name ASC", "test",
                                0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 4);
 ...
 * capped at the engine ceiling (ani_cypher_max_depth(), default 10). On
 ...
 * to ani_store_bfs — an unbounded traversal (a DoS on cyclic graphs). RED before
...
TEST(cypher_exec_var_length_no_reuse_self_loop) {
    ...
    ani_edge_t e = {.project = "test", .source_id = id, .target_id = id, .type = "CALLS"};
    ani_store_insert_edge(s, &e);
...
TEST(cypher_exec_var_length_truncation_surfaces_warning) {
    ...
    for (int source = 0; source < 17; source++) {
        for (int target = source + 1; target < 18; target++) {
            ani_edge_t edge = {.project = "test",
                               .source_id = ids[source],
                               .target_id = ids[target],
                               .type = "CALLS"};
            ani_store_insert_edge(s, &edge);
        }
    }
...
TEST(cypher_exec_no_results) {
    ...
    int rc =
        ani_cypher_execute(s, "MATCH (f:Function) WHERE f.name = \"NonExistent\"", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 0);
...
TEST(cypher_exec_distinct) {
    ...
    int rc = ani_cypher_execute(s, "MATCH (f:Function) RETURN DISTINCT f.label", "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_with_distinct_issue238) {
    ...
    int rc = ani_cypher_execute(s, "MATCH (f:Function) WITH DISTINCT f.label AS lbl RETURN lbl",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
    ani_cypher_result_free(&r);
    ...
    ani_cypher_result_t r2 = {0};
    rc = ani_cypher_execute(s, "MATCH (f:Function) WITH f.label AS lbl RETURN lbl", "test", 0, &r2);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_label_test_issue241) {
    ...
    ani_cypher_result_t r = {0};
    int rc =
        ani_cypher_execute(s, "MATCH (f:Function) WHERE f:Function RETURN f.name", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 4);
    ani_cypher_result_free(&r);
    ...
    ani_cypher_result_t r2 = {0};
    rc = ani_cypher_execute(s, "MATCH (f:Function) WHERE f:Class RETURN f.name", "test", 0, &r2);
    ASSERT_EQ(rc, 0);
    ...
    ani_cypher_result_t r3 = {0};
    rc =
        ani_cypher_execute(s, "MATCH (f:Function) WHERE NOT f:Class RETURN f.name", "test", 0, &r3);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_count_distinct_issue239) {
    ...
    ani_cypher_result_t r = {0};
    int rc =
        ani_cypher_execute(s, "MATCH (f:Function) RETURN count(DISTINCT f.label)", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
    ...
    ani_cypher_result_t r2 = {0};
    rc = ani_cypher_execute(s, "MATCH (f:Function) RETURN count(f.label)", "test", 0, &r2);
    ASSERT_EQ(rc, 0);
    ...
    ani_cypher_result_t r3 = {0};
    rc = ani_cypher_execute(s, "MATCH (f:Function) RETURN count(DISTINCT f.name)", "test", 0, &r3);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_unknown_func_return_errors) {
    ...
    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s, "MATCH (f:Function) RETURN nosuchfunc(f.name)", "test", 0, &r);
    ASSERT_TRUE(rc != 0);
    ASSERT_NOT_NULL(r.error);
...
TEST(cypher_exec_label_alternation_issue242) {
    ...
    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s, "MATCH (n:Function|Module) RETURN n.name", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ...
    ani_cypher_result_t r2 = {0};
    rc = ani_cypher_execute(s, "MATCH (n:Function|Class) RETURN n.name", "test", 0, &r2);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r2.row_count, 4);
    ani_cypher_result_free(&r2);
...
TEST(cypher_parse_where_starts_with) {
    ...
    char *err = NULL;
    int rc =
        ani_cypher_parse("MATCH (f:Function) WHERE f.name STARTS WITH \"Send\" RETURN f", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);
    ASSERT_NOT_NULL(q->where);
    ASSERT_NOT_NULL(q->where->root);
...
TEST(cypher_parse_where_contains) {
    ...
    char *err = NULL;
    int rc =
        ani_cypher_parse("MATCH (f:Function) WHERE f.name CONTAINS \"Handler\" RETURN f", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);
    ASSERT_NOT_NULL(q->where);
    ASSERT_NOT_NULL(q->where->root);
...
TEST(cypher_parse_where_numeric) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f:Function) WHERE f.start_line > 10 RETURN f", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);
    ASSERT_NOT_NULL(q->where);
    ASSERT_NOT_NULL(q->where->root);
...
static ani_store_t *setup_cypher_http_store(void) {
    ani_store_t *s = ani_store_open_memory();
    ani_store_upsert_project(s, "test", "/tmp/test");
    ...
    ani_store_insert_edge(s, &http);

    return s;
}

/* Helper: set up store with TWO HTTP_CALLS edges for filtering tests. */
static ani_store_t *setup_cypher_multi_edge_store(void) {
    ani_store_t *s = ani_store_open_memory();
    ani_store_upsert_project(s, "testproj", "/tmp/test");

    ani_node_t n1 = {.project = "testproj",
                     .label = "Function",
                     .name = "SendOrder",
                     .qualified_name = "testproj.caller.SendOrder",
                     .file_path = "caller/client.go"};
    ani_node_t n2 = {.project = "testproj",
                     .label = "Function",
                     .name = "HandleOrder",
                     .qualified_name = "testproj.handler.HandleOrder",
                     .file_path = "handler/routes.go"};
    ani_node_t n3 = {.project = "testproj",
                     .label = "Function",
                     .name = "HandleHealth",
    ...
    ani_edge_t e1 = {.project = "testproj",
                     .source_id = id1,
                     .target_id = id2,
                     .type = "HTTP_CALLS",
                     .properties_json =
                         "{\"url_path\":\"/api/orders\",\"confidence\":0.85,\"method\":\"POST\"}"};
    ani_edge_t e2 = {.project = "testproj",
                     .source_id = id1,
                     .target_id = id3,
                     .type = "HTTP_CALLS",
...
static const char *cypher_get_col(const ani_cypher_result_t *r, int row, const char *col) {
...
static bool cypher_has_row_with(const ani_cypher_result_t *r, const char *col, const char *val) {
    int ci = -1;
    for (int c = 0; c < r->col_count; c++) {
        if (strcmp(r->columns[c], col) == 0) {
            ci = c;
...
TEST(cypher_edge_prop_access) {
    ani_store_t *s = setup_cypher_http_store();
    ani_cypher_result_t r = {0};
...
static void *cypher_edge_props_concurrently(void *opaque) {
    ...
    ani_store_t *store = setup_cypher_http_store();
    if (!store) {
        return NULL;
    }
    ...
    ani_node_t source = {.project = "test",
                         .label = "Function",
                         .name = "HandleOrder",
                         .qualified_name = "test.main.HandleOrder",
                         .file_path = "main.go",
                         .start_line = 10,
                         .end_line = 30};
    int64_t source_id = ani_store_upsert_node(store, &source);
    for (int i = 0; i < 256; i++) {
        char name[64];
        ...
        int64_t target_id = ani_store_upsert_node(store, &target);
        ani_edge_t edge = {
            .project = "test",
            .source_id = source_id,
            .target_id = target_id,
            ...
            .properties_json =
                "{\"url_path\":\"/api/orders\",\"confidence\":0.85,\"method\":\"POST\"}"};
        if (source_id < 0 || target_id < 0 || ani_store_insert_edge(store, &edge) < 0) {
    ...
    for (int i = 0; i < 128; i++) {
        ani_cypher_result_t result = {0};
        int rc = ani_cypher_execute(store,
                                    "MATCH (a:Function)-[r:HTTP_CALLS]->(b:Function) "
...
TEST(cypher_edge_prop_storage_is_per_thread) {
    ...
    bool started0 = ani_thread_create(&threads[0], 0, cypher_edge_props_concurrently, &ctx[0]) == 0;
    bool started1 = ani_thread_create(&threads[1], 0, cypher_edge_props_concurrently, &ctx[1]) == 0;
...
TEST(cypher_edge_type_prop) {
    ...
    int rc = ani_cypher_execute(s, "MATCH (a)-[r:HTTP_CALLS]->(b) RETURN r.type", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_edge_filter_contains) {
    ani_store_t *s = setup_cypher_multi_edge_store();
    ani_cypher_result_t r = {0};
...
TEST(cypher_bare_edge_return_exposes_properties_json) {
    ...
    int rc = ani_cypher_execute(s, "MATCH (a)-[r:HTTP_CALLS]->(b) WHERE r.method = 'POST' RETURN r",
                                "testproj", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_apply_limit) {
    ...
    /* LIMIT 5 → 5 rows */
    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s, "MATCH (f:Function) RETURN f.name LIMIT 5", "lim", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 5);
    ani_cypher_result_free(&r);
    ...
    memset(&r, 0, sizeof(r));
    rc = ani_cypher_execute(s, "MATCH (f:Function) RETURN f.name", "lim", 10, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 10);
    ani_cypher_result_free(&r);
    ...
    memset(&r, 0, sizeof(r));
    rc = ani_cypher_execute(s, "MATCH (f:Function) RETURN f.name LIMIT 30", "lim", 10, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_not_in) {
    ani_store_t *s = setup_cypher_store();
    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s, "MATCH (f) WHERE NOT f.label IN [\"Module\"] RETURN f.name",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_is_null) {
    ...
    ani_store_upsert_node(s, &n2);
    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s, "MATCH (f:Function) WHERE f.file_path IS NULL RETURN f.name",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_is_not_null) {
    ...
    ani_store_upsert_node(s, &n2);
    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s, "MATCH (f:Function) WHERE f.file_path IS NOT NULL RETURN f.name",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_return_star) {
    ani_store_t *s = setup_cypher_store();
    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s, "MATCH (f:Function) RETURN * LIMIT 3", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 3);
...
TEST(cypher_parse_neq) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f:Function) WHERE f.name <> \"X\"", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->where);
    ASSERT_NOT_NULL(q->where->root);
    ASSERT_EQ(q->where->root->type, EXPR_CONDITION);
...
TEST(cypher_parse_in) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f) WHERE f.label IN [\"Function\", \"Module\"]", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->where->root);
    ASSERT_EQ(q->where->root->type, EXPR_CONDITION);
...
TEST(cypher_parse_is_null) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f) WHERE f.file_path IS NULL", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->where->root);
...
TEST(cypher_parse_expr_tree_and_or) {
    ...
    char *err = NULL;
    int rc =
        ani_cypher_parse("MATCH (f) WHERE f.a = \"1\" AND f.b = \"2\" OR f.c = \"3\"", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_expr_tree_nested) {
    ...
    char *err = NULL;
    int rc =
        ani_cypher_parse("MATCH (f) WHERE (f.a = \"1\" OR f.b = \"2\") AND f.c = \"3\"", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_error_create) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("CREATE (n:Node {name: \"X\"})", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_error_delete) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("DELETE n", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_error_set) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("SET n.name = \"X\"", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_error_merge) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MERGE (n:Node)", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_error_call) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("CALL db.labels()", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_exec_skip) {
    ani_store_t *s = setup_cypher_store();
    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s, "MATCH (f:Function) RETURN f.name ORDER BY f.name ASC SKIP 2",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_sum) {
    ...
    int rc = ani_cypher_execute(s, "MATCH (f:Function) RETURN SUM(f.start_line) AS total", "test",
                                0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_exec_avg) {
    ...
    int rc = ani_cypher_execute(s, "MATCH (f:Function) RETURN AVG(f.start_line) AS avg_line",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_exec_min) {
    ...
    int rc =
        ani_cypher_execute(s, "MATCH (f:Function) RETURN MIN(f.start_line) AS mn", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_exec_max) {
    ...
    ani_cypher_result_t r = {0};
    int rc =
        ani_cypher_execute(s, "MATCH (f:Function) RETURN MAX(f.start_line) AS mx", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_exec_count_star) {
    ani_store_t *s = setup_cypher_store();
    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s, "MATCH (f:Function) RETURN COUNT(*) AS n", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_parse_skip) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f) RETURN f.name SKIP 5 LIMIT 10", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->ret);
...
TEST(cypher_parse_sum_avg) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f) RETURN SUM(f.x) AS s, AVG(f.y) AS a", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(q->ret->count, 2);
...
TEST(cypher_parse_collect) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f)-[:CALLS]->(g) RETURN f.name, COLLECT(g.name) AS names", &q,
                              &err);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(q->ret->count, 2);
...
TEST(cypher_parse_tolower) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f) RETURN toLower(f.name) AS n", &q, &err);
    ASSERT_EQ(rc, 0);
...
/* #1111 follow-up (review feedback on #1221): with_agg_find_or_create's
...
TEST(cypher_parse_with_where) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("MATCH (f)-[:CALLS]->(g) WITH f.name AS caller, COUNT(g) AS cnt "
...
TEST(cypher_parse_multi_match) {
    ...
    char *err = NULL;
    int rc =
        ani_cypher_parse("MATCH (a:Module) MATCH (b:Function) RETURN a.name, b.name", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(q->pattern_count, 2);
    ASSERT(!q->pattern_optional[0]);
    ASSERT(!q->pattern_optional[1]);
...
TEST(cypher_parse_union) {
    ...
    char *err = NULL;
    int rc =
        ani_cypher_parse("MATCH (f) RETURN f.name UNION ALL MATCH (g) RETURN g.name", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT(q->union_all);
...
TEST(cypher_parse_unwind) {
    ...
    char *err = NULL;
    int rc =
        ani_cypher_parse("UNWIND [\"a\", \"b\", \"c\"] AS x MATCH (f) RETURN f.name", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_unwind_var) {
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse("UNWIND items AS item MATCH (f) RETURN f.name", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_unwind_oversized_literal_no_overflow) {
    ...
    ani_query_t *q = NULL;
    char *err = NULL;
    int rc = ani_cypher_parse(query, &q, &err);
...
TEST(cypher_issue240_labels_function) {
    ani_store_t *s = setup_cypher_store();
    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s, "MATCH (n:Module) RETURN labels(n) AS lbl", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
    ASSERT_EQ(r.row_count, 1);
    ani_cypher_result_free(&r);
...
TEST(cypher_issue873_distinct_limit_dedupes_before_limit) {
    ...
    ani_cypher_result_t r = {0};
    int rc =
        ani_cypher_execute(s, "MATCH (n) RETURN DISTINCT n.label AS label LIMIT 2", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
    ASSERT_EQ(r.row_count, 2);
    ani_cypher_result_free(&r);
...
TEST(cypher_issue252_tointeger) {
    ani_store_t *s = setup_cypher_store();
    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s, "MATCH (f:Function) RETURN toInteger(f.start_line) AS ln",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
    ani_cypher_result_free(&r);
...
TEST(cypher_issue305_count_star_alias) {
    ani_store_t *s = setup_cypher_store();
    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s, "MATCH (n) RETURN count(*) AS total", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
    ASSERT_EQ(r.row_count, 1);
    ani_cypher_result_free(&r);
 ...
 * (vals[ANI_SZ_32] / func_bufs[ANI_SZ_32][…] in execute_return_simple and its
...
TEST(cypher_exec_prop_array_with_internal_commas) {
    ...
    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s, "MATCH (m:Method) RETURN m.decorators, m.lines", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_exec_prop_string_with_escaped_quote) {
    ...
    ani_cypher_result_t r = {0};
    int rc = ani_cypher_execute(s, "MATCH (f:Function) RETURN f.signature", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_wide_return_projection_bounded) {
    ...
    for (int i = 0; i < 48; i++) { /* 48 > ANI_SZ_32 (32) fixed columns */
    ...
    if (pid == 0) {
        ani_store_t *s = setup_cypher_store();
        ani_cypher_result_t r = {0};
        (void)ani_cypher_execute(s, query, "test", 0, &r);
        ani_cypher_result_free(&r);
        ani_store_close(s);
...
TEST(cypher_exec_deadline_aborts_runaway_query_issue601) {
    ...
    ani_cypher_test_set_deadline_ms(0); /* trip on the first hot-loop check */
    int rc = ani_cypher_execute(
        s, "MATCH (a) OPTIONAL MATCH (a)-[:CALLS]->(b) RETURN a.qualified_name, count(b)", "test",
        0, &r);
    ani_cypher_test_set_deadline_ms(-1); /* restore default before asserting (thread-local) */

    ASSERT_TRUE(rc != 0); /* ANI_NOT_FOUND (-1) — query aborted, not success */
    ASSERT_NOT_NULL(r.error);
