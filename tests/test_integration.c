...
static int integration_setup(void) {
    ...
    g_project = ani_project_name_from_path(g_tmpdir);
    if (!g_project)
        return -1;
    ...
    char *resp = ani_mcp_handle_tool(g_srv, "index_repository", args);
    if (!resp)
        return -1;
...
static char *call_tool(const char *tool, const char *args) {
    if (!g_srv)
        return NULL;
    return ani_mcp_handle_tool(g_srv, tool, args);
}

TEST(integ_index_has_nodes) {
    /* Open the indexed db directly and check node counts */
    ani_store_t *store = ani_store_open_path_existing(g_dbpath);
    ASSERT_NOT_NULL(store);

    int nodes = ani_store_count_nodes(store, g_project);
...
TEST(integ_index_has_edges) {
    ...
    int edges = ani_store_count_edges(store, g_project);
...
TEST(integ_index_has_functions) {
    ...
    int count = 0;
    int rc = ani_store_find_nodes_by_label(store, g_project, "Function", &funcs, &count);
    ...
    ani_store_free_nodes(funcs, count);
    ani_store_close(store);
    PASS();
}
...
TEST(integ_index_has_files) {
    ...
    int count = 0;
    int rc = ani_store_find_nodes_by_label(store, g_project, "File", &files, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ...
    ani_store_free_nodes(files, count);
    ani_store_close(store);
    PASS();
}
...
TEST(integ_index_has_calls) {
    ...
    int call_count = ani_store_count_edges_by_type(store, g_project, "CALLS");
...
TEST(integ_mcp_trace_path_cross_service) {
    ...
    int src_count = 0;
    int dst_count = 0;
    ani_store_find_nodes_by_name(store, g_project, "greet", &src, &src_count);
    ani_store_find_nodes_by_name(store, g_project, "farewell", &dst, &dst_count);
    ASSERT_TRUE(src_count > 0 && dst_count > 0);

    ani_edge_t edge = {.project = g_project,
                       .source_id = src[0].id,
                       .target_id = dst[0].id,
                       .type = "CROSS_HTTP_CALLS"};
    ASSERT_TRUE(ani_store_insert_edge(store, &edge) > 0);

    ani_store_free_nodes(src, src_count);
    ani_store_free_nodes(dst, dst_count);
    ani_store_close(store);
    ...
    snprintf(args, sizeof(args),
             ...
             g_project);
    char *resp = ani_mcp_handle_tool(srv, "trace_path", args);
    ASSERT_NOT_NULL(resp);
    ...
    snprintf(args, sizeof(args),
             ...
             g_project);
    resp = ani_mcp_handle_tool(srv, "trace_path", args);
    ASSERT_NOT_NULL(resp);
...
TEST(integ_pipeline_fqn_compute) {
    char *fqn = ani_pipeline_fqn_compute("myproject", "src/utils.go", "Add");
    ASSERT_NOT_NULL(fqn);
...
TEST(integ_pipeline_fqn_module) {
    char *fqn = ani_pipeline_fqn_module("myproject", "src/utils.go");
    ASSERT_NOT_NULL(fqn);
...
TEST(integ_store_search_by_degree) {
    ...
    int rc = ani_store_search(store, &params, &out);
    ASSERT_EQ(rc, ANI_STORE_OK);
...
TEST(integ_store_find_by_file) {
    ...
    int count = 0;
    int rc = ani_store_find_nodes_by_file(store, g_project, "main.py", &nodes, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
...
TEST(integ_store_bfs_traversal) {
    ...
    ani_node_t *results = NULL;
    int count = 0;
    ani_store_find_nodes_by_name(store, g_project, "Multiply", &results, &count);

    if (count > 0) {
        /* BFS outbound from Multiply */
        ani_traverse_result_t trav = {0};
        int rc = ani_store_bfs(store, results[0].id, "outbound", NULL, 0, 3, 20, &trav);
        ASSERT_EQ(rc, ANI_STORE_OK);
        /* Should visit at least Add */
        ASSERT_TRUE(trav.visited_count >= 0); /* might be 0 if no edges */
        ani_store_traverse_free(&trav);
    }

    ani_store_free_nodes(results, count);
    ani_store_close(store);
    PASS();
}
...
TEST(store_bfs_edges_survive_large_visited_set) {
    ...
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(ani_store_upsert_project(s, "star", "/tmp/star"), ANI_STORE_OK);
    ...
    for (int i = 0; i < SPOKES; i++) {
        ...
        int64_t sid = ani_store_upsert_node(s, &sp);
        ASSERT_GT(sid, 0);
        ani_edge_t e = {0};
        e.project = "star";
        e.source_id = sid;
        e.target_id = hub_id;
        e.type = "CALLS";
        ASSERT_GT(ani_store_insert_edge(s, &e), 0); /* returns the edge id */
    }

    ani_traverse_result_t tr = {0};
    ASSERT_EQ(ani_store_bfs(s, hub_id, "inbound", NULL, 0, 1, SPOKES + 10, &tr), ANI_STORE_OK);
...
TEST(store_bfs_multi_excludes_seeds_and_takes_min_hop) {
    ani_store_t *s = ani_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(ani_store_upsert_project(s, "impact", "/tmp/impact"), ANI_STORE_OK);
    ...
    for (size_t i = 0; i < sizeof(edges) / sizeof(edges[0]); i++) {
        ani_edge_t e = {.project = "impact",
                        .source_id = edges[i].from,
                        .target_id = edges[i].to,
                        .type = "CALLS"};
        ASSERT_GT(ani_store_insert_edge(s, &e), 0);
    }

    int64_t seeds[2] = {A, B};
    ani_traverse_result_t tr = {0};
    bool truncated = true;
    ASSERT_EQ(ani_store_bfs_multi(s, seeds, 2, "outbound", NULL, 0, 5, 100, &tr, &truncated),
              ANI_STORE_OK);
...
TEST(store_bfs_multi_reports_truncation_at_ceiling) {
    ani_store_t *s = ani_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(ani_store_upsert_project(s, "cap", "/tmp/cap"), ANI_STORE_OK);
    ani_node_t hub = {.project = "cap",
                      .label = "Function",
                      .name = "hub",
    ...
    for (int i = 0; i < N; i++) {
        ...
        ani_node_t n = {.project = "cap",
                        ...
                        .start_line = 1,
                        .end_line = 2};
        int64_t nid = ani_store_upsert_node(s, &n);
        ASSERT_GT(nid, 0);
        ani_edge_t e = {.project = "cap", .source_id = hub_id, .target_id = nid, .type = "CALLS"};
        ASSERT_GT(ani_store_insert_edge(s, &e), 0);
    }
    ani_traverse_result_t tr = {0};
    bool truncated = false;
    ASSERT_EQ(ani_store_bfs_multi(s, &hub_id, 1, "outbound", NULL, 0, 5, CEIL, &tr, &truncated),
              ANI_STORE_OK);
...
TEST(index_reports_excluded_subtrees_issue411) {
    char tmp[256];
    snprintf(tmp, sizeof(tmp), "/tmp/ani_excl_XXXXXX");
