 ...
 * We test the EXTRACTION layer only: does ani_extract_file() populate
 ...
 * PHP        UNCERTAIN             ANI_LANG_PHP → parse_generic_imports("expression_
 ...
 * C#         UNCERTAIN             ANI_LANG_CSHARP → parse_generic_imports("using_directive")
 ...
 *
 * ── Field names used ────────────────────────────────────────────────────────
 * ANIImport.module_path  — the full import/use path string
 * ANIImport.local_name   — last segment or alias name
 * r->imports.count       — number of extracted imports
 * r->imports.items[i]    — array of ANIImport
 */
...
static int imp_has(ANIFileResult *r, const char *path_substr) {
...
static int imp_local(ANIFileResult *r, const char *local) {
    for (int i = 0; i < r->imports.count; i++) {
...
static ANIFileResult *do_extract(const char *src, ANILanguage lang,
                                  const char *path) {
    return ani_extract_file(src, (int)strlen(src), lang, "testproj", path,
                             0, NULL, NULL);
}
...
static int run_import_case(const import_case_t *tc) {
    ANIFileResult *r = do_extract(tc->src, tc->lang, tc->path);
    if (!r) {
        printf("  FAIL [%s]: ani_extract_file returned NULL\n", tc->label);
        return 1;
    }
    if (r->has_error) {
...
static const import_case_t csharp_cases[] = {
    ...
    /* Newtonsoft.Json */
    { ANI_LANG_CSHARP, "I.cs",
      "using Newtonsoft.Json;\nusing Newtonsoft.Json.Linq;\npublic class I {}\n",
      {"Newtonsoft.Json", NULL}, 2, "cs_json" },

    /* multiple aliases */
    { ANI_LANG_CSHARP, "J.cs",
...
static const zero_import_case_t zero_cases[] = {
    { ANI_LANG_RUST,       "a.rs",   "fn main() { println!(\"hello\"); }\n",
      "rust_no_imports" },
    { ANI_LANG_JAVA,       "A.java", "public class A { public static void main(String[] a) {} }\n",
      "java_no_imports" },
    { ANI_LANG_KOTLIN,     "A.kt",   "fun main() { println(\"hello\") }\n",
      "kotlin_no_imports" },
    { ANI_LANG_CSHARP,     "A.cs",   "public class A { static void Main() {} }\n",
      "cs_no_imports" },
    { ANI_LANG_PHP,        "a.php",  "<?php\necho 'hello';\n",
      "php_no_imports" },
    { ANI_LANG_GO,         "a.go",   "package main\nfunc main() {}\n",
      "go_no_imports" },
    { ANI_LANG_PYTHON,     "a.py",   "def main(): pass\n",
      "py_no_imports" },
    { ANI_LANG_TYPESCRIPT, "a.ts",   "function hello(): void { console.log('hi'); }\n",
      "ts_no_imports" },
    { ANI_LANG_JAVASCRIPT, "a.js",   "function hello() { return 42; }\n",
      "js_no_imports" },
};
