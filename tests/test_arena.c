...
TEST(arena_init_default) {
    ...
    ASSERT_EQ(a.block_size, ANI_ARENA_DEFAULT_BLOCK_SIZE);
    ASSERT_EQ(a.used, 0);
...
TEST(arena_alloc_null_arena) {
    void *p = ani_arena_alloc(NULL, 16);
    ASSERT_NULL(p);
    PASS();
}
...
TEST(arena_alloc_alignment) {
    ...
    void *p1 = ani_arena_alloc(&a, 1);
    void *p2 = ani_arena_alloc(&a, 1);
    ASSERT_NOT_NULL(p1);
    ASSERT_NOT_NULL(p2);
...
TEST(arena_alloc_grows_blocks) {
    ANIArena a;
    ani_arena_init_sized(&a, 64); /* tiny block to force growth */
    /* Allocate more than one block's worth */
    void *p1 = ani_arena_alloc(&a, 48);
    ASSERT_NOT_NULL(p1);
    ASSERT_EQ(a.nblocks, 1);
    void *p2 = ani_arena_alloc(&a, 48);
    ASSERT_NOT_NULL(p2);
...
TEST(arena_calloc) {
    ANIArena a;
    ani_arena_init(&a);
    unsigned char *p = (unsigned char *)ani_arena_calloc(&a, 64);
    ASSERT_NOT_NULL(p);
...
TEST(arena_strdup) {
    ANIArena a;
    ani_arena_init(&a);
    char *s = ani_arena_strdup(&a, "hello world");
    ASSERT_NOT_NULL(s);
...
TEST(arena_strdup_null) {
    ANIArena a;
    ani_arena_init(&a);
    char *s = ani_arena_strdup(&a, NULL);
    ASSERT_NULL(s);
    ani_arena_destroy(&a);
    PASS();
}
...
TEST(arena_strndup) {
    ANIArena a;
    ani_arena_init(&a);
    char *s = ani_arena_strndup(&a, "hello world", 5);
    ASSERT_NOT_NULL(s);
...
TEST(arena_sprintf) {
    ANIArena a;
    ani_arena_init(&a);
    char *s = ani_arena_sprintf(&a, "%s.%s.%s", "project", "path", "name");
    ASSERT_NOT_NULL(s);
...
TEST(arena_sprintf_int) {
    ANIArena a;
    ani_arena_init(&a);
    char *s = ani_arena_sprintf(&a, "count=%d", 42);
    ASSERT_NOT_NULL(s);
...
TEST(arena_total) {
    ANIArena a;
    ani_arena_init(&a);
    ASSERT_EQ(ani_arena_total(&a), 0);
    ani_arena_alloc(&a, 100);
    ASSERT_GTE(ani_arena_total(&a), 100); /* at least 100 (may be aligned) */
    ani_arena_alloc(&a, 200);
    ASSERT_GTE(ani_arena_total(&a), 300);
    ani_arena_destroy(&a);
    PASS();
}
...
TEST(arena_multiple_resets) {
    ...
    void *p = ani_arena_alloc(&a, 60000);
    ASSERT_NOT_NULL(p);
    ani_arena_alloc(&a, 60000);
    ASSERT_GTE(a.nblocks, 2);
    ani_arena_reset(&a);
    ASSERT_EQ(a.nblocks, 1);
    ASSERT_EQ(a.used, 0);
...
TEST(arena_reset_single_block) {
    ...
    ANIArena a;
    ani_arena_init_sized(&a, 1024);
    ani_arena_alloc(&a, 16);
    ASSERT_EQ(a.nblocks, 1);
    ani_arena_reset(&a);
    ASSERT_EQ(a.nblocks, 1);
    ASSERT_EQ(a.used, 0);
    /* Still usable */
    void *p = ani_arena_alloc(&a, 8);
    ASSERT_NOT_NULL(p);
    ani_arena_destroy(&a);
    PASS();
}
...
TEST(arena_strndup_len_exceeds_string) {
    ...
    char *s = ani_arena_strndup(&a, src, 3);
    ASSERT_NOT_NULL(s);
    ASSERT_STR_EQ(s, "abc");
    /* Now test with len matching exactly */
    s = ani_arena_strndup(&a, src, 2);
    ASSERT_NOT_NULL(s);
    ASSERT_STR_EQ(s, "ab");
    ani_arena_destroy(&a);
    PASS();
...
TEST(arena_many_large_allocs_block_growth) {
    ...
    ANIArena a;
    ani_arena_init_sized(&a, 64); /* tiny blocks */
    for (int i = 0; i < 50; i++) {
        void *p = ani_arena_alloc(&a, 128); /* each > block_size initially */
        ASSERT_NOT_NULL(p);
    }
    /* Should have grown many blocks */
    ASSERT_GT(a.nblocks, 1);
    ASSERT_GTE(ani_arena_total(&a), 50 * 128);
    ani_arena_destroy(&a);
    PASS();
}
...
TEST(arena_total_through_reset) {
    ...
    ani_arena_init_sized(&a, 1024);
    ani_arena_alloc(&a, 100);
    ASSERT_GTE(ani_arena_total(&a), 100);
    size_t before_reset = ani_arena_total(&a);
    ...
    ani_arena_alloc(&a, 50);
    ASSERT_GTE(ani_arena_total(&a), 50);
    /* But should be less than before_reset (fresh accumulation) */
    ASSERT_LTE(ani_arena_total(&a), before_reset);
    ani_arena_destroy(&a);
    PASS();
}
...
TEST(arena_reset_block_size_invariant) {
    ...
    ani_arena_alloc(&a, 100);
    ani_arena_alloc(&a, 100); /* triggers grow */
    ASSERT_GTE(a.nblocks, 2);
...
TEST(arena_strndup_zero_len) {
    ...
    ANIArena a;
    ani_arena_init(&a);
    char *s = ani_arena_strndup(&a, "hello", 0);
