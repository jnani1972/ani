...
TEST(da_struct_type) {
    ...
    ANI_DYN_ARRAY(Point) pts = {0};
    ani_da_push(&pts, ((Point){.x = 1, .y = 2.5f}));
    ani_da_push(&pts, ((Point){.x = 3, .y = 4.5f}));
...
TEST(da_string_ptrs) {
    ANI_DYN_ARRAY(const char *) strs = {0};
    ani_da_push(&strs, "hello");
    ani_da_push(&strs, "world");
...
TEST(da_insert) {
    ...
    ani_da_push(&arr, 10);
    ani_da_push(&arr, 30);
    ani_da_insert(&arr, 1, 20); /* insert 20 at index 1 */
    ASSERT_EQ(arr.count, 3);
    ASSERT_EQ(arr.items[0], 10);
    ASSERT_EQ(arr.items[1], 20);
    ASSERT_EQ(arr.items[2], 30);
...
TEST(da_push_ptr_struct) {
    ...
    ANI_DYN_ARRAY(Record) arr = {0};

    Record *r = ani_da_push_ptr(&arr);
    r->id = 42;
    r->value = 3.14;
    r->tag = "first";

    r = ani_da_push_ptr(&arr);
    r->id = 99;
    r->value = 2.72;
    r->tag = "second";
...
TEST(da_insert_at_beginning) {
    ...
    ani_da_push(&arr, 30);
    ani_da_push(&arr, 40);
    ani_da_insert(&arr, 0, 10); /* insert at front */
...
TEST(da_insert_at_end) {
    ...
    ani_da_push(&arr, 10);
    ani_da_push(&arr, 20);
    ani_da_insert(&arr, 2, 30); /* insert at count (append) */
    ASSERT_EQ(arr.count, 3);
    ASSERT_EQ(arr.items[0], 10);
    ASSERT_EQ(arr.items[1], 20);
    ASSERT_EQ(arr.items[2], 30);
...
TEST(da_remove_first) {
    ...
    ani_da_push(&arr, 20);
    ani_da_push(&arr, 30);
    ani_da_remove(&arr, 0); /* remove first */
    ASSERT_EQ(arr.count, 2);
...
TEST(da_remove_last) {
    ...
    ani_da_push(&arr, 20);
    ani_da_push(&arr, 30);
    ani_da_remove(&arr, 2); /* remove last element */
    ASSERT_EQ(arr.count, 2);
    ASSERT_EQ(arr.items[0], 10);
    ASSERT_EQ(arr.items[1], 20);
    ani_da_free(&arr);
...
TEST(da_remove_sequence) {
    ANI_DYN_ARRAY(int) arr = {0};
    for (int i = 0; i < 5; i++)
        ani_da_push(&arr, i * 10); /* 0, 10, 20, 30, 40 */

    ani_da_remove(&arr, 2); /* remove 20 -> 0,10,30,40 */
    ASSERT_EQ(arr.count, 4);
    ASSERT_EQ(arr.items[2], 30);

    ani_da_remove(&arr, 0); /* remove 0 -> 10,30,40 */
    ASSERT_EQ(arr.count, 3);
    ASSERT_EQ(arr.items[0], 10);

    ani_da_remove(&arr, 2); /* remove 40 -> 10,30 */
    ASSERT_EQ(arr.count, 2);
    ASSERT_EQ(arr.items[0], 10);
    ASSERT_EQ(arr.items[1], 30);
    ani_da_free(&arr);
...
TEST(da_large_array_10k) {
    ANI_DYN_ARRAY(int64_t) arr = {0};
    for (int i = 0; i < 10000; i++)
        ani_da_push(&arr, (int64_t)i * 1000);
...
TEST(da_nested_struct_with_ptrs) {
    ...
    ANI_DYN_ARRAY(Entry) arr = {0};

    ani_da_push(&arr, ((Entry){.name = "alpha", .scores = {10, 20, 30}, .category = "A"}));
    ani_da_push(&arr, ((Entry){.name = "beta", .scores = {40, 50, 60}, .category = "B"}));
    ani_da_push(&arr, ((Entry){.name = "gamma", .scores = {70, 80, 90}, .category = "A"}));
...
TEST(da_push_pop_lifo) {
    ...
    ASSERT_EQ(ani_da_pop(&arr), 5);
    ASSERT_EQ(ani_da_pop(&arr), 4);
    ASSERT_EQ(ani_da_pop(&arr), 3);
    ASSERT_EQ(ani_da_pop(&arr), 2);
    ASSERT_EQ(ani_da_pop(&arr), 1);
    ASSERT_EQ(arr.count, 0);
...
TEST(da_mixed_operations) {
    ...
    /* last should be 30 */
    ASSERT_EQ(ani_da_last(&arr), 30);

    /* insert at end -> 15,20,30,50 */
    ani_da_insert(&arr, 3, 50);
    ASSERT_EQ(arr.count, 4);
