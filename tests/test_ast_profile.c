...
static ani_ast_profile_t make_profile(void) {
    ani_ast_profile_t p;
    memset(&p, 0, sizeof(p));
    p.if_count = 5;
    p.for_count = 3;
...
TEST(ast_profile_roundtrip) {
    ani_ast_profile_t original = make_profile();
    char buf[200];
    ani_ast_profile_to_str(&original, buf, sizeof(buf));

    ani_ast_profile_t decoded;
    ASSERT_TRUE(ani_ast_profile_from_str(buf, &decoded));
...
TEST(ast_profile_to_str_null) {
    char buf[200];
    ani_ast_profile_to_str(NULL, buf, sizeof(buf)); /* should not crash */
    PASS();
}

TEST(ast_profile_to_str_small_buf) {
    ani_ast_profile_t p = make_profile();
    /* buf too small: should write empty string */
    char buf[1] = {'X'};
    ani_ast_profile_to_str(&p, buf, 0);
    /* 0-length buffer: function should handle gracefully */
    PASS();
}

TEST(ast_profile_from_str_null) {
    ASSERT_FALSE(ani_ast_profile_from_str(NULL, NULL));
    PASS();
}

TEST(ast_profile_from_str_invalid) {
    ani_ast_profile_t out;
    ASSERT_FALSE(ani_ast_profile_from_str("not,a,valid,string", &out));
    ASSERT_FALSE(ani_ast_profile_from_str("", &out));
    PASS();
}
...
TEST(ast_profile_from_str_too_few_fields) {
    ...
    ASSERT_FALSE(ani_ast_profile_from_str("1,2,3,4,5", &out));
    PASS();
}
...
TEST(ast_profile_to_vector_range) {
    ani_ast_profile_t p = make_profile();
    float vec[25];
    ani_ast_profile_to_vector(&p, vec);
...
TEST(ast_profile_to_vector_null) {
    ...
    ani_ast_profile_to_vector(NULL, vec); /* should not crash or write */
    PASS();
}
