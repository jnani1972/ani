 ...
 *
 * Ports from internal/ani/lsp_test.go (49 tests).
 * Categories:
...
/* Extract a Go file using ani_extract_file (runs single-file LSP internally) */
static ANIFileResult *extract_go(const char *source) {
    return ani_extract_file(source, (int)strlen(source), ANI_LANG_GO, "test", "main.go", 0, NULL,
                            NULL);
}
...
static int find_resolved_arr_confident(const ANIResolvedCallArray *arr, const char *callerSub,
                                       const char *calleeSub) {
    for (int i = 0; i < arr->count; i++) {
        const ANIResolvedCall *rc = &arr->items[i];
...
TEST(golsp_param_type_simple) {
    ANIFileResult *r = extract_go("package main\n\n"
...
TEST(golsp_ordinary_same_leaf_calls_join_by_exact_site) {
    ...
    ANIFileResult *r = extract_go(source);
    ASSERT_NOT_NULL(r);
    ...
    const ANIResolvedCall *a_semantic = NULL;
    const ANIResolvedCall *b_semantic = NULL;
    ...
    const ANIResolvedCall *a_joined =
        ani_pipeline_find_lsp_resolution(&r->resolved_calls, a_call, false);
    const ANIResolvedCall *b_joined =
        ani_pipeline_find_lsp_resolution(&r->resolved_calls, b_call, false);
...
/* ── Cross-file tests (use ani_run_go_lsp_cross directly) ──────── */
...
TEST(golsp_fast_crossfile_same_leaf_calls_preserve_exact_sites) {
    ...
    const char *a_receiver =
        ani_arena_strndup(&r->arena, a_unresolved_qn, (size_t)(a_leaf - a_unresolved_qn));
    const char *b_receiver =
        ani_arena_strndup(&r->arena, b_unresolved_qn, (size_t)(b_leaf - b_unresolved_qn));
    ...
    defs[0].lang = ANI_LANG_GO;
    defs[1] = defs[0];
    defs[1].qualified_name = b_unresolved_qn;
    defs[1].receiver_type = b_receiver;

    ANITypeRegistry *reg = ani_go_build_cross_registry(&r->arena, defs, 2);
    ASSERT_NOT_NULL(reg);
    ASSERT_EQ(ani_go_fast_resolve_qualified_calls(r, reg, NULL, NULL, 0), 0);
    ...
    for (int i = 0; i < r->resolved_calls.count; i++) {
        ...
        if (!rc->strategy || strcmp(rc->strategy, "lsp_strategy_cross_file") != 0 ||
            rc->kind != ANI_RESOLVED_INVOCATION || !rc->caller_qn ||
            !strstr(rc->caller_qn, "run") || !rc->callee_qn || !strstr(rc->callee_qn, ".Render")) {
            continue;
        }
...
TEST(golsp_crossfile_method_dispatch) {
    ...
    ani_run_go_lsp_cross(&arena, source, (int)strlen(source), "test.main", defs, 4, imp_names,
                         imp_qns, 1, NULL, &out);
...
TEST(golsp_crossfile_interface_field_chain) {
    ...
    ani_run_go_lsp_cross(&arena, source, (int)strlen(source), "test.main", defs, 6, imp_names,
                         imp_qns, 1, NULL, &out);
...
TEST(golsp_crossfile_map_index) {
    ...
    ani_run_go_lsp_cross(&arena, source, (int)strlen(source), "test.main", defs, 3, imp_names,
                         imp_qns, 1, NULL, &out);
...
TEST(golsp_crossfile_stdlib_interface) {
    ...
    ani_run_go_lsp_cross(&arena, source, (int)strlen(source), "test.main", defs, 1, imp_names,
                         imp_qns, 1, NULL, &out);
...
TEST(golsp_crossfile_aliased_field_requal) {
    ...
    ANITypeRegistry *reg = ani_go_build_cross_registry(&arena, defs, 6);
    ASSERT_NOT_NULL(reg);

    ani_run_go_lsp_cross_with_registry(&arena, source, (int)strlen(source), "test.main", reg,
    ...
    if (svc_idx < 0) {
        ...
        for (int i = 0; i < out.count; i++) {
            const ANIResolvedCall *rc = &out.items[i];
            printf("    %s -> %s [%s %.2f]\n", rc->caller_qn ? rc->caller_qn : "(null)",
...
TEST(golsp_crossfile_local_interface_single_impl) {
    ...
    ani_run_go_lsp_cross(&arena, source, (int)strlen(source), "test.main", defs, 5, imp_names,
                         imp_qns, 1, NULL, &out);
