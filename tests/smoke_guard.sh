...
BINARY="$PROJECT_ROOT/build/c/ani"
FAKE_PROJECT="nonexistent_smoke_test_xyz"
CACHE_DIR="${HOME}/.cache/ani"
GHOST_FILE="$CACHE_DIR/${FAKE_PROJECT}.db"
FAILURES=0

# ── Step 1: Build ─────────────────────────────────────────────────
echo "[smoke_guard] Building project..."
make -f "$PROJECT_ROOT/Makefile.ani" ani -C "$PROJECT_ROOT" --quiet 2>&1
if [ ! -x "$BINARY" ]; then
...
check_handler() {
    ...
    # For a missing .db file, ani_store_open_path_query returns NULL so
