...
static ANIFileResult *extract_java(const char *source) {
    return ani_extract_file(source, (int)strlen(source), ANI_LANG_JAVA, "test", "Main.java", 0,
                            NULL, NULL);
}

static ANIFileResult *extract_java_at(const char *source, const char *rel_path) {
    return ani_extract_file(source, (int)strlen(source), ANI_LANG_JAVA, "test", rel_path, 0, NULL,
                            NULL);
}
...
static int count_resolved(const ANIFileResult *r) {
    int n = 0;
    for (int i = 0; i < r->resolved_calls.count; i++) {
...
TEST(jlsp_string_length) {
    const char *src =
        ...
        "  }\n"
        "}\n";
    ANIFileResult *r = extract_java(src);
    ASSERT_NOT_NULL(r);
...
TEST(jlsp_user_enum_methods) {
    ...
    for (int i = 0; i < r->defs.count; i++) {
        const ANIDefinition *definition = &r->defs.items[i];
...
TEST(jlsp_file_path) {
    ...
    ANIFileResult *r = extract_java_at(src, "src/main/java/com/example/Main.java");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "String.length"), 0);
    ani_free_result(r);
    PASS();
