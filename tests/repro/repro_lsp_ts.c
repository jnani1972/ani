 ...
 * JavaScript / JSX hybrid LSP (internal/ani/lsp/ts_lsp.c).
 *
 ...
 *   the source (grep '"lsp_' internal/ani/lsp/ts_lsp.c), with its keying site:
 ...
 *   production indexer does: ".ts" → ANI_LANG_TYPESCRIPT, ".tsx" → ANI_LANG_TSX.
 *   jsx_mode (required by resolve_jsx_element, ts_lsp.c:2620) is enabled ONLY for
 *   ANI_LANG_TSX (ani.c:619, pass_lsp_cross.c:267), so the two JSX fixtures use
 ...
 * ani_registry_lookup_symbol_by_args hits on the module QN). */
 ...
 * ani_registry_lookup_symbol on the module QN. App() renders <Widget/> defined
 * in the same file. */
