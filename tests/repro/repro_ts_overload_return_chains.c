...
static int count_resolved_suffix(const ANIResolvedCallArray *calls, const char *callee_suffix,
                                 const char *strategy) {
    if (!calls)
        return 0;
    int count = 0;
    for (int i = 0; i < calls->count; i++) {
        const ANIResolvedCall *call = &calls->items[i];
        if (call->kind == ANI_RESOLVED_INVOCATION && call->confidence >= 0.5f && call->caller_qn &&
...
static int count_any_callee_suffix(const ANIResolvedCallArray *calls, const char *callee_suffix) {
    if (!calls)
        return 0;
    int count = 0;
    for (int i = 0; i < calls->count; i++) {
...
static void print_resolved_calls(const ANIResolvedCallArray *calls) {
    if (!calls)
        return;
    for (int i = 0; i < calls->count; i++) {
        const ANIResolvedCall *call = &calls->items[i];
...
static void init_overload_defs(ANILSPDef defs[OVERLOAD_DEF_COUNT], ANILanguage language,
...
static int assert_both_return_chains(const ANIResolvedCallArray *calls, const char *language_name,
...
static int assert_free_function_overload_returns(ANILanguage language, bool jsx_mode,
    ...
    ANILSPDef defs[OVERLOAD_DEF_COUNT];
    init_overload_defs(defs, language, &names);

    ANIArena arena;
    ani_arena_init(&arena);
    ANITypeRegistry *registry = ani_ts_build_cross_registry(&arena, defs, OVERLOAD_DEF_COUNT);
    ANIResolvedCallArray calls = {0};
    if (registry) {
        ani_run_ts_lsp_cross_with_registry(&arena, kFreeFunctionSource,
...
static int assert_namespace_overload_returns(ANILanguage language, bool jsx_mode,
                                             const char *language_name) {
    static const TSOverloadFixtureNames names = {
        "repro.provider",
        "repro.consumer",
    ...
    ANIResolvedCallArray calls = {0};
    if (registry) {
        ani_run_ts_lsp_cross_with_registry(&arena, kNamespaceSource, (int)strlen(kNamespaceSource),
...
static int assert_exact_symbol_import_overload_returns(ANILanguage language, bool jsx_mode,
                                                       const char *language_name) {
    static const TSOverloadFixtureNames names = {
    ...
    ANIResolvedCallArray calls = {0};
    if (registry) {
        ani_run_ts_lsp_cross_with_registry(
...
static int assert_contextual_callback_uses_selected_overload(ANILanguage language, bool jsx_mode,
                                                             const char *language_name) {
    static const TSOverloadFixtureNames names = {
        "repro.callback",
        "repro.callback",
    ...
    ANIResolvedCallArray calls = {0};
    if (registry) {
        ani_run_ts_lsp_cross_with_registry(&arena, kContextualCallbackOverloadSource,
...
static int assert_no_shadowed_return_chains(const ANIResolvedCallArray *calls,
...
static int assert_parameter_shadows_free_function(ANILanguage language, bool jsx_mode,
                                                  const char *language_name) {
    static const TSOverloadFixtureNames names = {
        "repro.shadow.free",
        "repro.shadow.free",
    ...
    ANIResolvedCallArray calls = {0};
    if (registry) {
        ani_run_ts_lsp_cross_with_registry(&arena, kShadowedFreeFunctionSource,
...
static int assert_parameter_shadows_namespace(ANILanguage language, bool jsx_mode,
                                              const char *language_name) {
    static const TSOverloadFixtureNames names = {
...
TEST(repro_ts_free_function_overload_returns_typescript) {
    return assert_free_function_overload_returns(ANI_LANG_TYPESCRIPT, false, "TypeScript");
}

TEST(repro_ts_free_function_overload_returns_tsx) {
    return assert_free_function_overload_returns(ANI_LANG_TSX, true, "TSX");
}

TEST(repro_ts_namespace_overload_returns_typescript) {
    return assert_namespace_overload_returns(ANI_LANG_TYPESCRIPT, false, "TypeScript");
}

TEST(repro_ts_namespace_overload_returns_tsx) {
    return assert_namespace_overload_returns(ANI_LANG_TSX, true, "TSX");
}

TEST(repro_ts_exact_symbol_import_overload_returns_typescript) {
    return assert_exact_symbol_import_overload_returns(ANI_LANG_TYPESCRIPT, false, "TypeScript");
}

TEST(repro_ts_exact_symbol_import_overload_returns_tsx) {
    return assert_exact_symbol_import_overload_returns(ANI_LANG_TSX, true, "TSX");
}

TEST(repro_ts_contextual_callback_uses_selected_overload_typescript) {
    return assert_contextual_callback_uses_selected_overload(ANI_LANG_TYPESCRIPT, false,
                                                             "TypeScript");
}

TEST(repro_ts_contextual_callback_uses_selected_overload_tsx) {
    return assert_contextual_callback_uses_selected_overload(ANI_LANG_TSX, true, "TSX");
}

TEST(repro_ts_parameter_shadows_free_function_typescript) {
    return assert_parameter_shadows_free_function(ANI_LANG_TYPESCRIPT, false, "TypeScript");
}

TEST(repro_ts_parameter_shadows_free_function_tsx) {
    return assert_parameter_shadows_free_function(ANI_LANG_TSX, true, "TSX");
}

TEST(repro_ts_parameter_shadows_namespace_typescript) {
    return assert_parameter_shadows_namespace(ANI_LANG_TYPESCRIPT, false, "TypeScript");
}

TEST(repro_ts_parameter_shadows_namespace_tsx) {
    return assert_parameter_shadows_namespace(ANI_LANG_TSX, true, "TSX");
}
