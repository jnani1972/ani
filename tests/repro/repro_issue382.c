 ...
 *   extract_decorators() in internal/ani/extract_defs.c first scans
...
static ANIFileResult *rx(const char *src, ANILanguage lang,
                         const char *proj, const char *path) {
    return ani_extract_file(src, (int)strlen(src), lang, proj, path,
                            0, NULL, NULL);
}
...
static ANIDefinition *find_def(ANIFileResult *r, const char *label,
                               const char *name) {
 ...
 * ani_extract_file carry their Java annotations in .decorators:
 *
 *   @Entity
 *   @RestController
 *   public class User {
...
TEST(repro_issue382_java_annotations_on_nodes) {
    ANIFileResult *r = rx(
        ...
        "    public String getUser(String id) { return id; }\n"
        "}\n",
        ANI_LANG_JAVA, "t", "User.java");

    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);

    /* ── Class node: two class-level marker_annotations ── */
    ANIDefinition *cls = find_def(r, "Class", "User");
    ASSERT_NOT_NULL(cls);
    ...
    ANIDefinition *method = find_def(r, "Method", "getUser");
