 ...
 *   is supposed to:
 *     1. Walk the repo filesystem for package.json manifests (ani_pkgmap_scan_repo).
 *     2. Parse each sibling package.json, mapping its `"name"` field to its
 *        entry-point QN (parse_package_json → pkg_entries_push).
 *     3. On import resolution (ani_pipeline_resolve_module), perform an exact
 ...
 *   maintainer confirmed: on macOS/Linux ani_pkgmap_scan_repo may resolve
 *   workspace names at the manifest-parse level (ani_pkgmap_try_parse), but
 ...
 *   in ani_pipeline_resolve_module (step 3) silently falls through to
...
TEST(repro_issue408_workspace_crosspkg_import) {
    /*
     * Fixture layout mirrors the reporter's /tmp/ani-issue408-repro tree
    ...
    RProj lp;
    ani_store_t *store = rh_index_files(&lp, files, 5);
     ...
     * does not match any graph node, so ani_pipeline_resolve_import_node
