 ...
 *   Graph quality audit over the real ani repo showed only
 *   3.69% of CALLS edges are callable-sourced (207/5607). The dominant
 *   failure mode is ani_enclosing_func_qn returning the module QN when
 *   ani_find_enclosing_func cannot walk the TSNode ancestry back to a
 ...
 *   type matches the list, ani_find_enclosing_func returns a null node and
 *   ani_enclosing_func_qn falls back to the module QN. The LSP rescue path
 ...
 *                  Rust LSP is hybrid but ani_pxc_has_cross_lsp returns false
 *                  for ANI_LANG_RUST (pass_lsp_cross.c:281). The enclosing-
 ...
 *
 * ASSERTION (per edge):
 *   For every ani_edge_t e where e.type == "CALLS":
 *     ani_store_find_node_by_id(store, e.source_id, &src) == ANI_STORE_OK
...
static int assert_calls_callable_sourced(const char *lang_tag,
    ...
    for (int i = 0; i < nedges; i++) {
        ani_node_t src;
        if (ani_store_find_node_by_id(store, edges[i].source_id, &src)
                != ANI_STORE_OK) {
 ...
 * ani_enclosing_func_qn path does not correctly resolve the caller QN and
 ...
 * drop the class qualifier (issue #554 / helpers.c ani_enclosing_func_qn).
 ...
 * ani_pxc_has_cross_lsp returns false for ANI_LANG_RUST (pass_lsp_cross.c:281)
