...
static int raw_usage_count(const ANIFileResult *r, const char *caller, const char *target) {
    int count = 0;
    for (int i = 0; i < r->usages.count; i++) {
        const ANIUsage *u = &r->usages.items[i];
...
static int raw_usage_count_kind(const ANIFileResult *r, const char *caller, const char *target,
                                ANIUsageKind kind) {
    int count = 0;
    for (int i = 0; i < r->usages.count; i++) {
        const ANIUsage *u = &r->usages.items[i];
...
static int raw_usage_candidate_count(const ANIFileResult *r, const char *caller,
                                     const char *target) {
    int count = 0;
    for (int i = 0; i < r->usages.count; i++) {
        const ANIUsage *u = &r->usages.items[i];
        if (u->kind == ANI_USAGE_VALUE && u->may_be_call_reference && u->ref_name &&
...
static int raw_usage_exists(const ANIFileResult *r, const char *caller, const char *target) {
    return raw_usage_count(r, caller, target) > 0;
}

static int raw_call_count(const ANIFileResult *r, const char *caller, const char *target) {
    int count = 0;
...
static int raw_call_exists(const ANIFileResult *r, const char *caller, const char *target) {
    return raw_call_count(r, caller, target) > 0;
}

static int resolved_call_count(const ANIFileResult *r, const char *caller, const char *target) {
    int count = 0;
    for (int i = 0; i < r->resolved_calls.count; i++) {
        const ANIResolvedCall *c = &r->resolved_calls.items[i];
        if (c->kind == ANI_RESOLVED_INVOCATION && qn_ends_with_name(c->caller_qn, caller) &&
...
static int edge_connects(ani_store_t *store, const char *project, const char *type,
                         const char *caller, const char *target) {
    ani_edge_t *edges = NULL;
    int count = 0;
    if (ani_store_find_edges_by_type(store, project, type, &edges, &count) != ANI_STORE_OK)
        return 0;
    ...
    for (int i = 0; i < count && !found; i++) {
        ani_node_t src = {0};
        ani_node_t dst = {0};
        int src_ok = ani_store_find_node_by_id(store, edges[i].source_id, &src) == ANI_STORE_OK;
        int dst_ok = ani_store_find_node_by_id(store, edges[i].target_id, &dst) == ANI_STORE_OK;
        if (src_ok && dst_ok && src.name && dst.name && strcmp(src.name, caller) == 0 &&
            strcmp(dst.name, target) == 0) {
            found = 1;
        }
        ani_node_free_fields(&src);
        ani_node_free_fields(&dst);
    }
    ani_store_free_edges(edges, count);
    return found;
}

static int edge_connects_callable(ani_store_t *store, const char *project, const char *type,
                                  const char *caller, const char *target) {
    ani_edge_t *edges = NULL;
    int count = 0;
    if (ani_store_find_edges_by_type(store, project, type, &edges, &count) != ANI_STORE_OK)
...
static int edge_connects_target_qn(ani_store_t *store, const char *project, const char *type,
...
static int check_raw_argument_case(const ArgUsageCase *c) {
    ANIFileResult *r = ani_extract_file(c->source, (int)strlen(c->source), c->language, "repro",
...
#define CHECK_RAW(condition, invariant)                                                \
    ...
    ANIUsageKind expected_kind = ANI_USAGE_VALUE;
    ANIUsageKind rejected_kind = ANI_USAGE_CALL_REFERENCE;
...
TEST(repro_python_local_shadow_does_not_link_global_callable) {
    static const char *source = "def handler():\n    return 1\n"
                                ...
                                "def direct():\n    return handler()\n";
    RProj project = {0};
    ani_store_t *store = rh_index(&project, "main.py", source);
    if (!store) {
...
TEST(repro_cpp_bare_function_value_usage) {
    static const char *source = "using Callback = void (*)();\n"
                                "void handler() {}\n"
                                "void bare() { Callback cb = handler; (void)cb; }\n";
    ANIFileResult *r = ani_extract_file(source, (int)strlen(source), ANI_LANG_CPP, "repro",
...
static int check_ts_lsp_mode(int disabled) {
    ...
    ANIFileResult *r = ani_extract_file(source, (int)strlen(source), ANI_LANG_TYPESCRIPT, "repro",
                                        "main.ts", 0, NULL, NULL);
    int failures = 0;
    if (!r) {
        failures++;
...
TEST(repro_call_argument_ts_lsp_cannot_rescue) {
    const char *prior = getenv("ANI_LSP_DISABLED");
    char *saved = prior ? ani_strdup(prior) : NULL;
    int failures = check_ts_lsp_mode(1) + check_ts_lsp_mode(0);
    if (saved) {
        ani_setenv("ANI_LSP_DISABLED", saved, 1);
        free(saved);
    } else {
        ani_unsetenv("ANI_LSP_DISABLED");
    }
...
static int check_callable_reference_edges(const char *tag, const RFile *files, int nfiles,
                                          ...
                                          const char *target) {
    RProj project;
    ani_store_t *store = rh_index_files(&project, files, nfiles);
    if (!store) {
...
TEST(repro_cpp_signature_types_preserve_positions_without_changing_legacy_types) {
    ...
    ANIFileResult *result = ani_extract_file(source, (int)strlen(source), ANI_LANG_CPP, "repro",
                                             "signature.cpp", 0, NULL, NULL);
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);

    const ANIDefinition *operator_def = NULL;
    const ANIDefinition *positional_def = NULL;
...
static int check_signature_position_case(const SignaturePositionCase *c) {
    ...
    const ANIDefinition *found = NULL;
    for (int i = 0; i < result->defs.count; i++) {
        const ANIDefinition *definition = &result->defs.items[i];
...
TEST(repro_cpp_operator_lsp_join_is_occurrence_exact) {
    ...
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    const ANICall *custom = NULL;
    const ANICall *primitive = NULL;
    ...
    ASSERT_NOT_NULL(ani_pipeline_find_lsp_resolution(&result->resolved_calls, custom, false));
    ASSERT_NULL(ani_pipeline_find_lsp_resolution(&result->resolved_calls, primitive, false));
    ani_free_result(result);
    PASS();
}
...
static int check_compound_assignment_case(const CompoundAssignmentCase *test_case) {
    ...
    ANIFileResult *result = ani_extract_file(
...
#define CHECK_COMPOUND(condition, invariant)                                                  \
    ...
    const ANICall *member_carrier = NULL;
    const ANICall *free_carrier = NULL;
    const ANICall *primitive_carrier = NULL;
    int member_carrier_count = 0;
    int free_carrier_count = 0;
    int primitive_carrier_count = 0;
    for (int i = 0; i < result->calls.count; i++) {
        const ANICall *call = &result->calls.items[i];
        if (!call->requires_lsp_resolution || call->source_origin != ANI_SOURCE_ORIGIN_RAW ||
    ...
    const ANIResolvedCall *member_semantic = NULL;
    const ANIResolvedCall *free_semantic = NULL;
    ...
    for (int i = 0; i < result->resolved_calls.count; i++) {
        ...
        if (resolved->kind != ANI_RESOLVED_INVOCATION ||
            resolved->source_origin != ANI_SOURCE_ORIGIN_RAW || !resolved->callee_qn ||
    ...
    const ANIResolvedCall *member_join =
        member_carrier
            ? ani_pipeline_find_lsp_resolution(&result->resolved_calls, member_carrier, false)
            : NULL;
    const ANIResolvedCall *free_join =
        free_carrier
            ? ani_pipeline_find_lsp_resolution(&result->resolved_calls, free_carrier, false)
            : NULL;
    const ANIResolvedCall *primitive_join =
        primitive_carrier
            ? ani_pipeline_find_lsp_resolution(&result->resolved_calls, primitive_carrier, false)
            : NULL;
    ...
    RProj project;
    ani_store_t *store = rh_index(&project, test_case->filename, COMPOUND_ASSIGNMENT_SOURCE);
    if (!store) {
...
TEST(repro_cpp_cuda_compound_assignment_calls_are_semantic_and_occurrence_exact) {
    static const CompoundAssignmentCase cases[] = {
        {"cpp", "compound_assignment.cpp", ANI_LANG_CPP},
        {"cuda", "compound_assignment.cu", ANI_LANG_CUDA},
    };
    int failures = 0;
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++)
...
TEST(repro_kotlin_synthetic_calls_require_semantic_resolution) {
    static const char source[] = "class Box { operator fun plus(other: Box): Box = this }\n"
                                 ...
                                 "  val primitive = a + b\n"
                                 "}\n";
    ANIFileResult *result = ani_extract_file(source, (int)strlen(source), ANI_LANG_KOTLIN, "repro",
    ...
    ASSERT_NOT_NULL(custom);
    ASSERT_NOT_NULL(primitive);
    const ANIResolvedCall *custom_resolution =
        ani_pipeline_find_lsp_resolution(&result->resolved_calls, custom, true);
    const ANIResolvedCall *primitive_resolution =
        ani_pipeline_find_lsp_resolution(&result->resolved_calls, primitive, true);
...
TEST(repro_callable_reference_parallel_pipeline) {
    ...
    const char *prior_workers = getenv("ANI_WORKERS");
    char *saved_workers = prior_workers ? ani_strdup(prior_workers) : NULL;
    ani_setenv("ANI_WORKERS", "2", 1);

    RProj project;
    memset(&project, 0, sizeof(project));
    ani_store_t *store = built == TOTAL_FILES ? rh_index_files(&project, files, built) : NULL;
    int failures = 0;
    if (!store) {
