...
TEST(repro_issue520_detect_changes_includes_new_untracked_file) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/ani_r520_XXXXXX");
    ...
    if (!srv) {
        th_rmtree(tmpdir);
        FAIL("ani_mcp_server_new returned NULL");
    }
    ...
    char *dc_project = ani_project_name_from_path(tmpdir);
    if (!dc_project) {
        ani_mcp_server_free(srv);
        th_rmtree(tmpdir);
        FAIL("ani_project_name_from_path failed");
    }
    char dc_args[640];
