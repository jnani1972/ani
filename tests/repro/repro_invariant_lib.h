 ...
 *
 * Two harness tiers:
 *   - single-file extraction:  inv_rx() / the inv_extract_* checks (ani_extract_file)
...
#include "repro_harness.h" /* RProj/RFile, rh_index*, ani_store, <store/store.h> */
#include "ani.h"
#include <string.h>

/* ── Single-file extraction ─────────────────────────────────────── */

static inline ANIFileResult *inv_rx(const char *src, ANILanguage lang, const char *file) {
    return ani_extract_file(src, (int)strlen(src), lang, "t", file, 0, NULL, NULL);
}
...
static inline int inv_extract_clean(const char *src, ANILanguage lang, const char *file) {
...
static inline int inv_count_bad_labels(ANIFileResult *r) {
    int bad = 0;
...
static inline int inv_count_bad_fqns(ANIFileResult *r) {
    int bad = 0;
    for (int i = 0; i < r->defs.count; i++)
...
static inline int inv_count_bad_ranges(ANIFileResult *r) {
    int bad = 0;
    for (int i = 0; i < r->defs.count; i++) {
        ANIDefinition *d = &r->defs.items[i];
...
static inline int inv_count_label(ANIFileResult *r, const char *label) {
    int c = 0;
    for (int i = 0; i < r->defs.count; i++)
...
static inline int inv_has_call(ANIFileResult *r, const char *callee) {
...
static inline void inv_count_calls_by_source(ani_store_t *store, const char *project,
...
static inline int inv_count_dangling_edges(ani_store_t *store, const char *project,
                                           const char *type) {
    ani_edge_t *edges = NULL;
    int n = 0;
    if (ani_store_find_edges_by_type(store, project, type, &edges, &n) != ANI_STORE_OK)
        return -1;
    int dangling = 0;
    for (int i = 0; i < n; i++) {
        ani_node_t a, b;
        if (ani_store_find_node_by_id(store, edges[i].source_id, &a) != ANI_STORE_OK)
            dangling++;
        else if (ani_store_find_node_by_id(store, edges[i].target_id, &b) != ANI_STORE_OK)
            dangling++;
    }
    ani_store_free_edges(edges, n);
    return dangling;
...
static inline int inv_edge_has_strategy(ani_store_t *store, const char *project,
                                        const char *strategy) {
    ani_edge_t *edges = NULL;
    int n = 0;
    if (ani_store_find_edges_by_type(store, project, "CALLS", &edges, &n) != ANI_STORE_OK)
...
static inline int inv_no_calls_edge_to_qn(ani_store_t *store, const char *project,
...
static inline int inv_calls_target_qn_suffix(ani_store_t *store, const char *project,
                                             const char *suffix) {
    ani_edge_t *edges = NULL;
    int n = 0;
    if (ani_store_find_edges_by_type(store, project, "CALLS", &edges, &n) != ANI_STORE_OK)
