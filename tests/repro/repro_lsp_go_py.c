 ...
 * Python hybrid LSPs (internal/ani/lsp/go_lsp.c, internal/ani/lsp/py_lsp.c).
 *
 ...
 *   these can silently fail: ani_pipeline_find_lsp_resolution joins each
 ...
 *   from the source (grep '"lsp_' internal/ani/lsp/go_lsp.c), with its keying
 *   site:
 ...
 *   (grep '"lsp_' internal/ani/lsp/py_lsp.c), with its keying site:
 ...
 * func_node is a bare identifier resolved via ani_registry_lookup_symbol on the
 * package QN). */
 ...
 * resolves via ani_registry_lookup_symbol on the module QN). */
 ...
 * (py_lsp.c:1620-1624: ani_scope_lookup yields a NAMED type → emit constructor
 ...
 * MODULE and ani_registry_lookup_symbol(module_qn, attr) hits → lsp_module_attr).
 ...
 * ani_registry_lookup_symbol("builtins", fname) hits). EXPECTED RED in a
 ...
 * constructor (py_lsp.c:1640-1643: ani_registry_lookup_type("builtins.str")
...
TEST(repro_lsp_go_strategy_cross_file) {
     ...
     * cross-file pass (ani_go_fast_resolve_qualified_calls), which runs only when
