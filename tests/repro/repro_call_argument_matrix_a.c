...
static int definition_count(const ANIFileResult *result, const char *name) {
    int count = 0;
...
static int call_count(const ANIFileResult *result, const char *caller, const char *target) {
    int count = 0;
...
static int usage_within_call_count(const ANIFileResult *result, const char *caller,
...
static int usage_count(const ANIFileResult *result, const char *caller, const char *target) {
    int count = 0;
...
static int usage_kind_count(const ANIFileResult *result, const char *caller, const char *target,
                            ANIUsageKind kind) {
    int count = 0;
    for (int i = 0; i < result->usages.count; i++) {
        const ANIUsage *usage = &result->usages.items[i];
...
static int local_shadow_usage_count(const ANIFileResult *result, const char *caller,
                                    const char *target) {
    int count = 0;
    for (int i = 0; i < result->usages.count; i++) {
        const ANIUsage *usage = &result->usages.items[i];
...
static int check_ast_contract(const MatrixCase *matrix_case, const char *source, const char *phase,
    ...
    int failures = 0;
    const TSLanguage *language = ani_ts_language(matrix_case->language);
...
static ANIFileResult *extract_matrix_source(const MatrixCase *matrix_case, const char *source,
                                            const char *filename, const char *phase) {
    ANIFileResult *result = ani_extract_file(source, (int)strlen(source), matrix_case->language,
...
static int check_reference_fixture(const MatrixCase *matrix_case, const char *source,
    ...
    ANIFileResult *result = extract_matrix_source(matrix_case, source, filename, phase);
    if (!result)
        return 1;
    ...
    CHECK_EXACT(matrix_case->tag, "reference_is_ordinary_usage",
                usage_kind_count(result, caller, value, ANI_USAGE_VALUE), 1);
    CHECK_EXACT(matrix_case->tag, "reference_not_call_reference",
                usage_kind_count(result, caller, value, ANI_USAGE_CALL_REFERENCE), 0);
...
static int check_semantic_case(const MatrixCase *matrix_case) {
    ...
    CHECK_EXACT(matrix_case->tag, "inside_value_is_ordinary_usage",
                usage_kind_count(result, "argument", matrix_case->value_name, ANI_USAGE_VALUE), 1);
    CHECK_EXACT(
        matrix_case->tag, "inside_value_not_call_reference",
        usage_kind_count(result, "argument", matrix_case->value_name, ANI_USAGE_CALL_REFERENCE), 0);
    ...
    CHECK_EXACT(matrix_case->tag, "bare_value_is_ordinary_usage",
                usage_kind_count(result, "bare", matrix_case->value_name, ANI_USAGE_VALUE), 1);
    CHECK_EXACT(matrix_case->tag, "bare_value_not_call_reference",
                usage_kind_count(result, "bare", matrix_case->value_name, ANI_USAGE_CALL_REFERENCE),
                0);
...
static int check_native_scope_case(const MatrixCase *matrix_case) {
    int failures = 0;
    const ANILangSpec *spec = ani_lang_spec(matrix_case->language);
