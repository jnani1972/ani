...
static ANIFileResult *rx_py(const char *src) {
    return ani_extract_file(src, (int)strlen(src), ANI_LANG_PYTHON, "proj", "mod.py",
                            0, NULL, NULL);
}

static int count_var_defs(ANIFileResult *r) {
    int n = 0;
...
static int has_var_def(ANIFileResult *r, const char *name) {
    for (int i = 0; i < r->defs.count; i++) {
