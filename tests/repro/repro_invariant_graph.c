 ...
 *   /Users/martinvogel/project_dir/ani-quality-contracts/QUALITY_ANALYSIS.md
 *
...
TEST(invariant_discovery_hygiene) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "%s/ani_inv_disc_XXXXXX", ani_tmpdir());
 ...
 *
 * WHY RED today:
 *   ani_fqn_compute() in internal/ani/helpers.c calls strip_ext_len() on the
...
TEST(invariant_fqn_same_stem_distinct) {
    ...
    int node_count = 0;
    int rc = ani_store_find_nodes_by_name(store, lp.project, "api_init",
                                          &nodes, &node_count);
 ...
 * resolve to an existing node via ani_store_find_node_by_id.
 *
 ...
 * ──────────────────────────────────────────────────────────────────────── */
static int count_dangling_edges(ani_store_t *store, const char *project,
                                const char *edge_type) {
    ani_edge_t *edges = NULL;
    int edge_count = 0;
    int rc = ani_store_find_edges_by_type(store, project, edge_type,
                                          &edges, &edge_count);
    if (rc != ANI_STORE_OK) {
        return -1;
    }
    ...
    for (int i = 0; i < edge_count; i++) {
        ...
        if (ani_store_find_node_by_id(store, edges[i].source_id,
                                      &src_node) != ANI_STORE_OK) {
            dangling++;
        }
        if (ani_store_find_node_by_id(store, edges[i].target_id,
                                      &tgt_node) != ANI_STORE_OK) {
            dangling++;
        }
    }
 ...
 *
 * QUALITY_ANALYSIS.md gap #3: ani_find_enclosing_func() in helpers.c uses a
 * hardcoded func_kinds_for_lang switch that has drifted from the
 * function_node_types field in ANILangSpec (lang_specs.c).
 *
 * Evidence from source:
 *   lang_specs.c  perl_func_types[] = {"subroutine_declaration_statement", NULL}
 *   helpers.c     func_kinds_for_lang(ANI_LANG_PERL) falls through to default
 ...
 * ani_find_enclosing_func() can NEVER find an enclosing function for Perl
 * call nodes, and ani_enclosing_func_qn() always returns the module QN.
 ...
 *
 * WHY RED today:
 *   helpers.c func_kinds_for_lang has no ANI_LANG_PERL case.  The Perl
 ...
 *
 * Fix location:
 *   internal/ani/helpers.c, function func_kinds_for_lang():
 *   Add a ANI_LANG_PERL case returning {"subroutine_declaration_statement", NULL}.
 * ──────────────────────────────────────────────────────────────────────── */
TEST(invariant_enclosing_func_perl_parity) {
    ...
    RProj lp;
    ani_store_t *store = rh_index(&lp, "main.pl", perl_src);
    ASSERT_NOT_NULL(store);

    /* Retrieve all CALLS edges for this project */
    ani_edge_t *edges = NULL;
    int edge_count = 0;
    int rc = ani_store_find_edges_by_type(store, lp.project, "CALLS",
                                          &edges, &edge_count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ...
    /*
     * RED: callable_sourced == 0 because helpers.c has no ANI_LANG_PERL case.
     ...
     *
     * GREEN when helpers.c adds ANI_LANG_PERL -> {"subroutine_declaration_statement"}.
     */
