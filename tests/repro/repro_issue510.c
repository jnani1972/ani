 ...
 *   ani_discover_ex() loads the root .gitignore ONLY when a .git directory is
 ...
 *   When ani_discover() is called on a directory that is NOT a git repo root
 ...
 *   ani_discover() returns the excluded file as a normal discovered file
 ...
 *   and writes secret.py + keep.py.  After ani_discover(), the loop below
 ...
 *   Additionally, ani_discover_ex() should attempt to load a root .gitignore
 ...
 *   ani_gitignore_matches(gi, "secret.py", false) == true.
 ...
 *   After ani_discover(), "secret.py" must NOT appear in the file list.
 *
...
TEST(repro_issue510_nested_gitignore_honored) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "%s/ani_repro510_XXXXXX", ani_tmpdir());
    ...
    ani_gitignore_t *gi = ani_gitignore_parse("secret.py\n");
    ASSERT_NOT_NULL(gi);
    /* If this assertion fails, the bug is in the gitignore matcher, not
     * in discovery — a different bug, not #510. */
    ASSERT_TRUE(ani_gitignore_matches(gi, "secret.py", false));
