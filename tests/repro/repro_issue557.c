 ...
 *
 * Issue: #557 -- "ani v0.8.1 silently deletes project DBs on 'corrupt'
 ...
 *
 *   The sequence is:
 *     1. resolve_store() opens the project DB with ani_store_open_path_query().
 *     2. It calls ani_store_check_integrity() (src/store/store.c:664).
 ...
 *     3. On false, resolve_store() calls ani_unlink(path) at mcp.c:803,
 *        then ani_unlink(wal_path) and ani_unlink(shm_path) -- with NO rename,
 ...
 *   After ani_store_check_integrity() returns false and resolve_store()
 ...
 *   ani_unlink(path) at mcp.c:803 destroys the DB file.  After resolve_store()
 ...
 *   On buggy code ani_unlink() runs with no rename, so both conditions are
 ...
 *
 *   1. Set ANI_CACHE_DIR to a temp directory so the DB lands in a controlled
 *      location and does not pollute the real cache.
 *   2. Create the DB via ani_store_open_path() (creates schema + tables).
 ...
 *      ani_store_check_integrity() because '8' is not '/', 'A'-'Z', or 'a'-'z'.
 *   4. Close the store, verify the DB file exists (precondition).
 *   5. Call ani_mcp_handle_tool(srv, "search_graph", ...) with the project
 ...
 *      and executes the destroying ani_unlink() at mcp.c:803.
 ...
 *   dependent on kill timing or WAL state.  ani_store_check_integrity() is
 ...
 *   Replace ani_unlink(path) with a rename to a timestamped .corrupt path,
...
/* Project name used throughout: must pass ani_validate_project_name().
 * Kept short and slug-safe so it is valid on every platform. */
#define REPRO557_PROJECT "ani-repro557-test"
...
static int unique_corrupt_main_exists(const char *cache_dir) {
    ani_dir_t *dir = ani_opendir(cache_dir);
    if (!dir) {
        return 0;
    }
...
static int is_backup_artifact_name(const char *name) {
    char corrupt_prefix[ANI_DIRENT_NAME_MAX];
    char bak_prefix[ANI_DIRENT_NAME_MAX];
...
TEST(repro_issue557_corrupt_db_not_silently_deleted) {
    /* ── Step 1: redirect ANI_CACHE_DIR to a temp dir ─────────────────
     *
     * ani_resolve_cache_dir() checks the ANI_CACHE_DIR env var first.
     ...
     *
     * The static buffer in ani_resolve_cache_dir() is updated on the
     * next call because it re-reads ANI_CACHE_DIR each time.  We must
     * also call ani_mkdir on the directory before opening the store.
     */
    char tmp_cache[512];
    snprintf(tmp_cache, sizeof(tmp_cache), "/tmp/ani_repro557_XXXXXX");
    if (!ani_mkdtemp(tmp_cache)) {
        /* mkdtemp failed -- cannot run the test */
        ASSERT_NOT_NULL(NULL); /* marks setup failure clearly */
    }

    /* Set the env var so all subsequent ani_resolve_cache_dir() calls
     * return tmp_cache.  setenv is POSIX; Windows uses _putenv_s. */
#if defined(_WIN32)
    char ev[600];
    snprintf(ev, sizeof(ev), "ANI_CACHE_DIR=%s", tmp_cache);
    _putenv(ev);
#else
    setenv("ANI_CACHE_DIR", tmp_cache, 1 /* overwrite */);
#endif
    ...
    /* ── Step 3: create the DB via ani_store_open_path() ──────────────
     *
     * ani_store_open_path() calls store_open_internal() with
     ...
     */
    ani_store_t *setup_store = ani_store_open_path(db_path);
     ...
     * SQL in ani_store_check_integrity():
     *
     ...
     * the query returns the row and ani_store_check_integrity() returns
     * false -- which is the exact trigger for the destroying path.
     *
     * ani_store_upsert_project() is the store's own public API for
     * writing project rows (used by the pipeline on every full index).
     */
    int rc = ani_store_upsert_project(setup_store, REPRO557_PROJECT, "826");
    ASSERT_EQ(rc, ANI_STORE_OK); /* precondition: row must be written */

    ani_store_close(setup_store);
    setup_store = NULL;
    ...
    /* ── Step 6: drive resolve_store() via ani_mcp_handle_tool ────────
     *
     * search_graph is the lightest query tool that reaches resolve_store().
     * The tool handler calls resolve_store(srv, project) which:
     *   1. Calls ani_store_open_path_query(path) -- opens read-write/no-create.
     *      The DB was created in step 3 so SQLITE_OPEN_READWRITE succeeds.
     *   2. Calls ani_store_check_integrity() -- returns false (root_path="826").
     *   3. Closes the store and calls ani_unlink(path) at mcp.c:803.
     *      Then ani_unlink(wal_path) and ani_unlink(shm_path).
     ...
     *   ani_unlink(path) at mcp.c:803 removes the file.
...
#if defined(_WIN32)
    _putenv("ANI_CACHE_DIR=");
#else
    unsetenv("ANI_CACHE_DIR");
#endif
     ...
     *
     * On buggy code: both are 0 because ani_unlink() ran with no backup.
