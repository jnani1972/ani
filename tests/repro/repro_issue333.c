 ...
 *
 * FIX LOCATION:
 *   `push_nested_class_nodes` in internal/ani/extract_defs.c (~line 4900):
 ...
 */
static int count_method_defs_named(ANIFileResult *r, const char *name) {
    int n = 0;
 ...
 */
static int count_defs_with_label_local(ANIFileResult *r, const char *label) {
    int n = 0;
    for (int i = 0; i < r->defs.count; i++) {
