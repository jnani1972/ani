 ...
 *
 * Root cause (as filed):
 *   ani_install_hook_gate_script wrote the legacy blocking shell gate
 ...
 *
 * Expected (correct) behaviour after ani_upsert_claude_hooks +
 * ani_install_hook_gate_script:
 *   1. The gate script written to
 *      <home>/.claude/hooks/ani-code-discovery-gate
 ...
 *      "ani-code-discovery-gate" (the shim), not an inline blocking script.
 *
 ...
 *   Then running both ani_upsert_claude_hooks + ani_install_hook_gate_script
 *   (the actual install/update code path) and asserting the CORRECT result.
 *
 *   This is the critical gap: existing tests call ani_install_hook_gate_script
 ...
 *
 * After calling ani_upsert_claude_hooks + ani_install_hook_gate_script
 ...
 *   2. settings.json PreToolUse command references "ani-code-discovery-gate"
 ...
 *   - settings.json does NOT contain "ani-code-discovery-gate" (wrong command)
 *
 * Oracle used: ani_upsert_claude_hooks(settings_path) +
 *              ani_install_hook_gate_script(home, binary_path)
...
TEST(repro_issue409_install_wires_hook_augment_not_blocking_gate) {
     ...
     * overwritten (truncated) by ani_install_hook_gate_script. */
    char script_path[512];
    snprintf(script_path, sizeof(script_path),
             "%s/ani-code-discovery-gate", hooks_dir);
    rp409_write_file(script_path,
        "#!/bin/bash\n"
        "# Gate hook: nudges Claude toward ani for code discovery.\n"
        "# First Grep/Glob/Read per session -> block. Subsequent -> allow.\n"
        "# PPID = Claude Code process PID, unique per session.\n"
        "GATE=/tmp/ani-code-discovery-gate-$PPID\n"
        "if [ -f \"$GATE\" ]; then exit 0; fi\n"
        "touch \"$GATE\"\n"
        "echo 'BLOCKED: use ani' >&2\n"
        "exit 2\n");
     ...
     * /tmp/ani-code-discovery-gate-$PPID as a per-invocation state file.
    ...
    ASSERT(strstr(settings_data, "ani-code-discovery-gate") != NULL);
