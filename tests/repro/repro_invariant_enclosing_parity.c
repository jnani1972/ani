 ...
 *   helpers.c  ani_find_enclosing_func() (helpers.c:700) walks a call node's
 ...
 *   generic strings, ani_find_enclosing_func() never matches, returns the null
 *   node, and ani_enclosing_func_qn() falls back to the MODULE qn. Every call
 ...
 *   language's spec->func_types (lang_specs.c) so ani_find_enclosing_func uses
 ...
 *   ani_store_find_node_by_id(store, e.source_id, &src) == ANI_STORE_OK AND
...
static int run_parity_case(const parity_case_t *c) {
    ...
    int         nedges = 0;
    int rc = ani_store_find_edges_by_type(store, lp.project, "CALLS", &edges, &nedges);
    if (rc != ANI_STORE_OK) {
        printf("  %sFAIL%s  [%s] ani_store_find_edges_by_type rc=%d\n", RED, RST, c->name, rc);
        rh_cleanup(&lp, store);
        return 1;
    }
...
static const parity_case_t case_fortran = {
    ANI_LANG_FORTRAN, "Fortran", "a.f90",
 ...
 * ANI_LANG_VERILOG via EXT_TABLE.)
 */
static const parity_case_t case_verilog = {
    ANI_LANG_VERILOG, "Verilog", "a.sv",
    "module m;\n"
...
static const parity_case_t case_commonlisp = {
    ANI_LANG_COMMONLISP, "CommonLisp", "a.lisp",
    "(defun helper (x)\n"
    "  (* x 2))\n"
    "\n"
    "(defun run ()\n"
...
static const parity_case_t case_emacslisp = {
    ANI_LANG_EMACSLISP, "EmacsLisp", "a.el",
    "(defmacro run (n)\n"
 ...
 * CALLS edge ever forms. Verified to fail identically under ANI_LANG_SYSTEMVERILOG
 * (function_subroutine_call). This is a tree-sitter grammar defect, not a ani
