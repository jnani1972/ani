 ...
 *
  * Root cause (identified by the maintainer + reporter halindrome):
 *   handle_trace_call_path() calls ani_store_find_nodes_by_name() to locate
 ...
 *   ani_store_find_node_by_qn() also returns nothing, so the handler exits
 ...
 *   graph, so ani_store_bfs() returns visited_count == 0 and the "callers"
 ...
 *   the resolved node id is passed to ani_store_bfs(), NOT to edge creation.
 *
 ...
 *   FAILS because ani_store_bfs() finds no hops from the resolved start node.
 *
 ...
 *   ani_store_find_nodes_by_name() or ani_store_bfs() in
...
TEST(repro_issue480_trace_path_nonempty_with_calls) {
     ...
     *   project        — lp.project (derived from tmpdir by ani_project_name_from_path)
     ...
     * ani_store_bfs() returning 0 hops serialises \"callers\":[] (no caller
