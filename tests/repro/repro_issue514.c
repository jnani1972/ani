 ...
 *   append_args_json serializes each ANICallArg as {"i":<index>,"e":"<expr>,...}
 *   into the CALLS edge properties_json column.  However,
 *   bfs_to_json_array() (mcp.c ~line 2283) only emits the node fields (name,
 *   qualified_name, hop, risk, is_test) from ani_node_hop_t.  The edge that
 *   carried the arg expressions is NOT propagated by ani_store_bfs() into the
 *   ani_traverse_result_t (ani_edge_info_t carries only from_name, to_name,
 ...
 *   ani_store_bfs() in src/store/store.c must propagate edge properties_json
 *   into the ani_traverse_result_t (extend ani_edge_info_t or ani_node_hop_t).
 ...
 * positional argument to callee().  The extractor captures this as a ANICallArg
...
TEST(repro_issue514_data_flow_surfaces_arg_expr) {
    RProj lp;
    ani_store_t *store = rh_index_files(&lp, k_files,
    ...
    char *resp = ani_mcp_handle_tool(lp.srv, "trace_path", args);
     ...
     *
     * WHY RED on current code:
     *   bfs_to_json_array() (mcp.c ~line 2283) only emits ani_node_hop_t fields
     *   (name, qualified_name, hop).  ani_edge_info_t (store.h ~line 146) does
