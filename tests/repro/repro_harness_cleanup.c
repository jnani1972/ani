...
TEST(repro_harness_cleans_temp_project_after_fixture_write_failure) {
    ...
    ani_store_t *store = rh_index_files(&project, &invalid_file, 1);
    ...
    if (!project.dbpath[0]) {
        snprintf(project.dbpath, sizeof(project.dbpath), "/tmp/ani_repro_cleanup_guard_%ld.db",
                 (long)getpid());
    }
...
TEST(repro_harness_cleanup_empty_dbpath_preserves_relative_sidecars) {
    ...
    char private_dir[256] = "/tmp/ani_repro_empty_dbpath_XXXXXX";
    _Static_assert(sizeof(private_dir) >= 256,
                   "ani_mkdtemp requires a 256-byte caller buffer on Windows");
