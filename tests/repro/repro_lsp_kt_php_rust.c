 ...
 *   (internal/ani/lsp/kotlin_lsp.c, php_lsp.c, rust_lsp.c).
 *
 ...
 *   ANI_LANG_RUST through the project-wide Rust registry, including exact
...
static int assert_rust_dispatch_files(const RFile *files, int nfiles, const char *caller_substr,
    ...
    int edge_count = 0;
    if (ani_store_find_edges_by_type(store, lp.project, "CALLS", &edges, &edge_count) !=
        ANI_STORE_OK) {
 ...
 * ani_registry_lookup_method finds a func whose receiver_type == recv type and
...
TEST(repro_lsp_rust_macro) {
    ...
    RProj lp;
    ani_store_t *store = rh_index(&lp, "main.rs", kRustMacro);
    if (!store) {
