 ...
 *
 * Issue: #363 — "Linux: ani_system_info / ani_default_worker_count don't
 ...
 *     ani_detect_cgroup_cpus() reads /sys/fs/cgroup/cpu.max (v2) or
 *     .../cpu/cpu.cfs_quota_us + .../cpu/cpu.cfs_period_us (v1); the result
 *     feeds detect_system_linux(). ani_default_worker_count() also honours the
 *     ANI_WORKERS env override (commit d952238). Both tested in test_platform.c.
 *
 *   Memory axis — FIXED.
 *     ani_detect_cgroup_mem() reads /sys/fs/cgroup/memory.max (v2) or
 *     .../memory/memory.limit_in_bytes (v1); detect_system_linux() applies
 *     min(cgroup, host). The missing env knob — the "EXACT OPEN GAP" this
 *     repro was filed for — now exists: ani_mem_init() reads ANI_MEM_BUDGET_MB
 *     and routes it through the pure ani_mem_resolve_budget() (explicit
 *     override > implicit detection, mirroring ANI_WORKERS).
 *
 * IMPORTANT — clamp semantics:
 *   ani_mem_resolve_budget() clamps the override to effective (cgroup/host) RAM
 ...
 *
 * NOTE on ani_mem_init() caching:
 *   g_budget is initialised once via atomic_compare_exchange_strong, so this
 *   guard relies on running before any suite that calls ani_mem_init(); the
 ...
 *
 * Precondition: ANI_MEM_BUDGET_MB is set before the process's first
 * ani_mem_init(). The budget must equal the override (honoured exactly, since
 * it is below any host's effective RAM), proving the env knob is wired.
 */
TEST(repro_issue363_mem_budget_env_override) {
    ani_setenv("ANI_MEM_BUDGET_MB", "128", 1);

    ani_mem_init(0.5);

    size_t budget = ani_mem_budget();

    ani_unsetenv("ANI_MEM_BUDGET_MB");
