...
int ani_suite_enabled(const char *name) {
    const char *only = getenv("ANI_REPRO_ONLY");
    if (!only || !*only) {
        return 1;
    }
...
static char *save_selector(void) {
    const char *prior = getenv("ANI_REPRO_ONLY");
    return prior ? ani_strdup(prior) : NULL;
}

static void restore_selector(char *saved) {
    if (saved) {
        ani_setenv("ANI_REPRO_ONLY", saved, 1);
        free(saved);
    } else {
        ani_unsetenv("ANI_REPRO_ONLY");
    }
}
...
TEST(repro_runner_filter_accepts_suite_substring) {
    ...
    ani_setenv("ANI_REPRO_ONLY", "call_argument", 1);
    int enabled = ani_suite_enabled("repro_call_argument_usages");
...
TEST(repro_runner_filter_accepts_comma_list) {
    char *saved = save_selector();
    ani_setenv("ANI_REPRO_ONLY", "language_registry,call_argument", 1);
    int first = ani_suite_enabled("repro_language_registry");
    int second = ani_suite_enabled("repro_call_argument_usages");
    restore_selector(saved);
    if (!first || !second)
...
TEST(repro_runner_filter_rejects_nonmatch) {
    char *saved = save_selector();
    ani_setenv("ANI_REPRO_ONLY", "language_registry", 1);
    int enabled = ani_suite_enabled("repro_call_argument_usages");
    restore_selector(saved);
    if (enabled)
...
TEST(repro_runner_filter_unset_or_empty_enables_all) {
    char *saved = save_selector();
    ani_unsetenv("ANI_REPRO_ONLY");
    int unset_enabled = ani_suite_enabled("repro_call_argument_usages");
    ani_setenv("ANI_REPRO_ONLY", "", 1);
    int empty_enabled = ani_suite_enabled("repro_call_argument_usages");
    restore_selector(saved);
