...
static char *rpd_edge_fingerprint(ani_store_t *s, const char *project) {
    struct sqlite3 *db = ani_store_get_db(s);
    if (!db)
        return NULL;
 ...
 * fingerprint. Honors ANI_INDEX_SINGLE_THREAD from the environment. */
static char *rpd_index_and_fingerprint(const char *repo, const char *dbpath) {
    ...
    ani_pipeline_t *p = ani_pipeline_new(repo, dbpath, ANI_MODE_FULL);
    if (!p)
        return NULL;
...
TEST(repro_parallel_edge_determinism) {
    ...
    char dbpath[512];
    snprintf(dbpath, sizeof(dbpath), "%s/ani_rpd_par_det.db", ani_tmpdir());
...
TEST(repro_seq_parallel_equivalence) {
    ...
    char dbpath[512];
    snprintf(dbpath, sizeof(dbpath), "%s/ani_rpd_seq_par.db", ani_tmpdir());
