...
static void check_exact_ast_kind(const BehaviorCase *test_case, int *failures) {
    const TSLanguage *language = ani_ts_language(test_case->language);
...
static ANIFileResult *extract_case_with_macros(const BehaviorCase *test_case,
                                               const ANIMacroTable *macro_table, int *failures) {
    ANIFileResult *result =
        ani_extract_file_ex(test_case->source, (int)strlen(test_case->source), test_case->language,
...
static ANIFileResult *extract_case(const BehaviorCase *test_case, int *failures) {
    return extract_case_with_macros(test_case, NULL, failures);
}

static int call_count_exact(const ANIFileResult *result, const char *caller_qn,
                            const char *callee) {
    int count = 0;
...
static int usage_count_exact(const ANIFileResult *result, const char *caller_qn,
...
static int value_usage_count_exact(const ANIFileResult *result, const char *caller_qn,
                                   const char *reference) {
    int count = 0;
    for (int i = 0; i < result->usages.count; i++) {
        const ANIUsage *usage = &result->usages.items[i];
...
static int callable_definition_count_exact(const ANIFileResult *result, const char *name,
...
static int call_count_in_routine(const ANIFileResult *result, const char *caller,
                                 const char *callee) {
    int count = 0;
    for (int i = 0; i < result->calls.count; i++) {
        const ANICall *call = &result->calls.items[i];
...
static int usage_count_in_routine(const ANIFileResult *result, const char *caller,
                                  const char *reference) {
    int count = 0;
    for (int i = 0; i < result->usages.count; i++) {
        const ANIUsage *usage = &result->usages.items[i];
...
static int callable_reference_count_in_routine(const ANIFileResult *result, const char *caller,
                                               const char *reference) {
    int count = 0;
    for (int i = 0; i < result->usages.count; i++) {
        const ANIUsage *usage = &result->usages.items[i];
...
static int semantic_reference_candidate_count_in_routine(const ANIFileResult *result,
                                                         const char *caller,
                                                         const char *reference) {
    int count = 0;
    for (int i = 0; i < result->usages.count; i++) {
...
static int resolved_call_count_in_routine(const ANIFileResult *result, const char *caller,
                                          const char *callee) {
    int count = 0;
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const ANIResolvedCall *call = &result->resolved_calls.items[i];
        if (call->kind == ANI_RESOLVED_INVOCATION &&
...
static int resolved_call_total_in_routine(const ANIFileResult *result, const char *caller) {
    int count = 0;
    for (int i = 0; i < result->resolved_calls.count; i++) {
        if (result->resolved_calls.items[i].kind == ANI_RESOLVED_INVOCATION &&
...
static int resolved_reference_count_in_routine(const ANIFileResult *result, const char *caller,
                                               const char *callee) {
    int count = 0;
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const ANIResolvedCall *reference = &result->resolved_calls.items[i];
        if (reference->kind == ANI_RESOLVED_CALL_REFERENCE &&
...
static void check_caller_definition(const BehaviorCase *test_case, const ANIFileResult *result,
...
static const char *checked_module_qn(const BehaviorCase *test_case, const ANIFileResult *result,
                                     int *failures) {
...
static int begin_routine_case(const BehaviorCase *test_case, ANIFileResult **result_out,
                              int *failures) {
    check_exact_ast_kind(test_case, failures);
    ANIFileResult *result = extract_case(test_case, failures);
    *result_out = result;
    if (!result)
        return 0;
...
static int finish_case(ANIFileResult *result, int failures) {
    if (result)
...
#define DEFINE_CPP_CUDA_BEHAVIOR_TESTS(name, source_value, ast_value, runner, reason_value) \
    TEST(repro_call_node_behavior_cpp_##name) {                                             \
        static const BehaviorCase test_case = {"cpp/" #name, ANI_LANG_CPP, "behavior.cpp",  \
                                               source_value, ast_value,    reason_value};   \
        return runner(&test_case);                                                          \
    }                                                                                       \
    TEST(repro_call_node_behavior_cuda_##name) {                                            \
        static const BehaviorCase test_case = {"cuda/" #name, ANI_LANG_CUDA, "behavior.cu", \
                                               source_value,  ast_value,     reason_value}; \
        return runner(&test_case);                                                          \
    }
...
TEST(repro_call_node_behavior_objectscript_udl_macro) {
    ...
    ANIMacroTable *macro_table = calloc(1, sizeof(*macro_table));
    ASSERT_NOT_NULL(macro_table);
    ani_macro_table_init_system(macro_table);
    ...
    ani_macro_table_free(macro_table);
    return status;
}
