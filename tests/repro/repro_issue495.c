 ...
 *     def.qualified_name = ani_fqn_compute(project, rel_path, name)
...
/* Extract a Rust source string and return the raw ANIFileResult.
 * Caller must ani_free_result() the returned pointer. */
static ANIFileResult *rx(const char *src, const char *proj, const char *path) {
    return ani_extract_file(src, (int)strlen(src), ANI_LANG_RUST, proj, path, 0, NULL, NULL);
}
...
static ANIDefinition *nth_def_named(ANIFileResult *r, const char *label, const char *name, int nth) {
    int seen = 0;
    for (int i = 0; i < r->defs.count; i++) {
        ANIDefinition *d = &r->defs.items[i];
        if (label && (!d->label || strcmp(d->label, label) != 0))
...
TEST(repro_issue495_cfg_gated_twins_distinct) {
    ...
    ANIFileResult *r = rx(src, "t", "src.rs");
    ASSERT_NOT_NULL(r);
    ...
    ANIDefinition *d0 = nth_def_named(r, "Function", "try_extract_pdf_text", 0);
    ANIDefinition *d1 = nth_def_named(r, "Function", "try_extract_pdf_text", 1);
    ASSERT_NOT_NULL(d0);
    ASSERT_NOT_NULL(d1);
     ...
     * the qualified_name. */
    ANIDefinition *real_def = NULL;  /* #[cfg(feature = "rag-pdf")]     */
    ANIDefinition *stub_def = NULL;  /* #[cfg(not(feature = "rag-pdf"))] */
