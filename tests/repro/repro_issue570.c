 ...
 *   https://github.com/jnani1972/ani/issues/570
 *
 * TWO FILES WRONGLY WRITTEN (Codex SessionStart hook):
 *   ~/.codex/config.toml   -- always written by ani_upsert_codex_hooks()
 ...
 *   target to ani_upsert_codex_hooks():
 *
 *     snprintf(cp, sizeof(cp), "%s/.codex/config.toml", home);
 *     ...
 *     ani_upsert_codex_hooks(cp);
 *
 ...
 *
 *   So ani_build_install_plan_json() always lists config.toml as the Codex
 *   hook target, even when hooks.json is already in use.
 *
 * EXPECTED vs ACTUAL (oracle: ani_build_install_plan_json plan JSON):
 ...
 *   ani_detect_agents() sets agents.codex = dir_exists("~/.codex").
 ...
 *     - If it does, pass that path to ani_upsert_codex_session_hooks_json()
 ...
 *
 * Oracle: ani_build_install_plan_json(home, binary) -- dry-run plan, no writes.
 *
...
TEST(repro_issue570_no_dual_hook_write) {
    char home[256];
    snprintf(home, sizeof(home), "/tmp/ani-repro570-XXXXXX");
    if (!ani_mkdtemp(home))
    ...
    char *json = ani_build_install_plan_json(home, "/usr/local/bin/ani");
