...
static bool has_resolved_suffix(const ANIResolvedCallArray *calls, const char *caller_sub,
...
static bool has_resolved_exact(const ANIResolvedCallArray *calls, const char *caller_sub,
...
static int assert_local_chained_target(ANILanguage language, const char *rel_path,
                                       const char *source, const char *callee_suffix,
                                       const char *strategy) {
    ANIFileResult *result = ani_extract_file(source, (int)strlen(source), language,
...
TEST(repro_lsp_ordered_local_go_grouped_generic) {
    ...
    return assert_local_chained_target(ANI_LANG_GO, "main.go", kGoGroupedGeneric, "RightOnly",
                                       "lsp_type_dispatch");
}
...
static int assert_reference_return_operator_arity(ANILanguage language, const char *rel_path,
                                                  const char *language_name) {
    ANIFileResult *result =
        ani_extract_file(kCppReferenceReturnOperator, (int)strlen(kCppReferenceReturnOperator),
...
TEST(repro_lsp_ordered_local_cpp_reference_return_operator_arity) {
    return assert_reference_return_operator_arity(ANI_LANG_CPP, "main.cpp", "C++");
}
...
TEST(repro_lsp_ordered_local_cuda_reference_return_operator_arity) {
    return assert_reference_return_operator_arity(ANI_LANG_CUDA, "main.cu", "CUDA");
}
...
static int assert_deep_pointer_return_arity(ANILanguage language, const char *rel_path,
                                            const char *language_name) {
    ANIFileResult *result =
        ani_extract_file(kDeepPointerReturn, (int)strlen(kDeepPointerReturn), language,
                         "repro_ordered_local", rel_path, 0, NULL, NULL);
    if (!result || result->has_error) {
...
TEST(repro_lsp_ordered_local_c_deep_pointer_return_arity) {
    return assert_deep_pointer_return_arity(ANI_LANG_C, "main.c", "C");
}

TEST(repro_lsp_ordered_local_cpp_deep_pointer_return_arity) {
    return assert_deep_pointer_return_arity(ANI_LANG_CPP, "main.cpp", "C++");
}

TEST(repro_lsp_ordered_local_cuda_deep_pointer_return_arity) {
    return assert_deep_pointer_return_arity(ANI_LANG_CUDA, "main.cu", "CUDA");
}
...
TEST(repro_lsp_ordered_local_control_typescript_this_receiver) {
    return assert_local_chained_target(ANI_LANG_TYPESCRIPT, "main.ts", kTsThisGeneric,
                                       "Right.rightOnly", "lsp_ts_method");
}

TEST(repro_lsp_ordered_local_control_tsx_this_receiver) {
    return assert_local_chained_target(ANI_LANG_TSX, "main.tsx", kTsThisGeneric, "Right.rightOnly",
                                       "lsp_ts_method");
}
...
static int assert_ts_interface_overload_return_chains(ANILanguage language, bool jsx_mode,
                                                      const char *language_name) {
    ...
    ANITypeRegistry *registry = ani_ts_build_cross_registry(&arena, defs, 5);
...
TEST(repro_lsp_ordered_local_typescript_interface_overload_return_chains) {
    return assert_ts_interface_overload_return_chains(ANI_LANG_TYPESCRIPT, false, "TypeScript");
}

TEST(repro_lsp_ordered_local_tsx_interface_overload_return_chains) {
    return assert_ts_interface_overload_return_chains(ANI_LANG_TSX, true, "TSX");
}
...
TEST(repro_lsp_ordered_local_java_overload_arity) {
    ...
    defs[2].lang = ANI_LANG_JAVA;

    ANIArena arena;
    ani_arena_init(&arena);
    ANIResolvedCallArray calls = {0};
    ani_run_java_lsp_cross(&arena, source, (int)strlen(source), "demo", defs, 3, NULL, NULL, 0,
                           NULL, &calls);
