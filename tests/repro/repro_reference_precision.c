...
static int rp_extracts_clean(const char *source, ANILanguage language, const char *filename) {
    ANIFileResult *result =
        ani_extract_file(source, (int)strlen(source), language, "repro", filename, 0, NULL, NULL);
    if (!result)
        return 0;
...
static int rp_node_count(ani_store_t *store, const char *project, const char *label,
...
static int rp_edge_count(ani_store_t *store, const char *project, const char *edge_type,
                         const char *source_name, const char *target_label, const char *target_name,
                         const char *target_qn_suffix) {
    ani_edge_t *edges = NULL;
    int edge_count = 0;
    if (ani_store_find_edges_by_type(store, project, edge_type, &edges, &edge_count) !=
        ANI_STORE_OK) {
        return -1;
    }
    ...
    for (int i = 0; i < edge_count; i++) {
        ...
        ani_node_t target = {0};
        int source_ok =
            ani_store_find_node_by_id(store, edges[i].source_id, &source) == ANI_STORE_OK;
        int target_ok =
            ani_store_find_node_by_id(store, edges[i].target_id, &target) == ANI_STORE_OK;
...
static int rp_is_constructor_node(const ani_node_t *node, const char *class_name) {
...
static int rp_constructor_node_count(ani_store_t *store, const char *project,
    ...
    for (size_t li = 0; li < sizeof(labels) / sizeof(labels[0]); li++) {
        ani_node_t *nodes = NULL;
        int count = 0;
        if (ani_store_find_nodes_by_label(store, project, labels[li], &nodes, &count) !=
            ANI_STORE_OK) {
            return -1;
        }
        for (int i = 0; i < count; i++) {
...
static int rp_constructor_edge_count(ani_store_t *store, const char *project, const char *edge_type,
...
static int rp_raw_usage_count(const ANIFileResult *result, const char *caller, const char *target,
                              ANIUsageKind kind) {
    int count = 0;
    for (int i = 0; result && i < result->usages.count; i++) {
        const ANIUsage *usage = &result->usages.items[i];
...
static int rp_raw_source_callable_def_count(const ANIFileResult *result, const char *source_path) {
...
TEST(repro_kotlin_property_reference_does_not_borrow_same_name_function) {
    ...
    ASSERT_TRUE(rp_extracts_clean(files[0].content, ANI_LANG_KOTLIN, files[0].name));
    ASSERT_TRUE(rp_extracts_clean(files[1].content, ANI_LANG_KOTLIN, files[1].name));
    ASSERT_TRUE(rp_extracts_clean(files[2].content, ANI_LANG_KOTLIN, files[2].name));

    RProj project;
    ani_store_t *store = rh_index_files(&project, files, 3);
    if (!store) {
...
TEST(repro_kotlin_constructor_reference_requires_concrete_constructor) {
    ...
    RProj project;
    ani_store_t *store = rh_index_files(&project, files, 1);
    if (!store) {
        rh_cleanup(&project, store);
...
TEST(repro_kotlin_local_reference_never_binds_same_name_top_level) {
    ...
    RProj project;
    ani_store_t *store = rh_index_files(&project, files, 2);
    if (!store) {
        rh_cleanup(&project, store);
...
TEST(repro_php_dynamic_first_class_terminal_is_not_precise) {
    static const char source[] =
        ...
        "}\n";
    ANIFileResult *raw = ani_extract_file(source, (int)strlen(source), ANI_LANG_PHP, "repro",
                                          "Dynamic.php", 0, NULL, NULL);
    ASSERT_NOT_NULL(raw);
    ASSERT_FALSE(raw->has_error || raw->parse_incomplete);
    int ordinary_terminal = rp_raw_usage_count(raw, "dynamicReference", "$method", ANI_USAGE_VALUE);
    int precise_terminal =
        rp_raw_usage_count(raw, "dynamicReference", "$method", ANI_USAGE_CALL_REFERENCE);
    ani_free_result(raw);
    ASSERT_EQ(ordinary_terminal, 1);
    ASSERT_EQ(precise_terminal, 0);

    RProj project;
    ani_store_t *store = rh_index(&project, "Dynamic.php", source);
    if (!store) {
        rh_cleanup(&project, store);
...
TEST(repro_php_static_magic_first_class_reference_targets_callstatic) {
    ...
    ASSERT_TRUE(rp_extracts_clean(source, ANI_LANG_PHP, "MagicStatic.php"));

    RProj project;
    ani_store_t *store = rh_index(&project, "MagicStatic.php", source);
    if (!store) {
        rh_cleanup(&project, store);
...
TEST(repro_php_typed_invokable_first_class_reference_targets_invoke) {
    ...
    ASSERT_TRUE(rp_extracts_clean(source, ANI_LANG_PHP, "Invokable.php"));

    RProj project;
    ani_store_t *store = rh_index(&project, "Invokable.php", source);
    if (!store) {
        rh_cleanup(&project, store);
...
TEST(repro_java_same_name_receiver_calls_keep_occurrence_identity) {
    static const char source[] = "class Alpha { void handle() {} }\n"
                                 ...
                                 "  }\n"
                                 "}\n";
    ASSERT_TRUE(rp_extracts_clean(source, ANI_LANG_JAVA, "Calls.java"));

    RProj project;
    ani_store_t *store = rh_index(&project, "Calls.java", source);
    if (!store) {
        rh_cleanup(&project, store);
...
TEST(repro_typescript_function_value_upgrades_only_with_exact_semantic_target) {
    ...
    ASSERT_TRUE(rp_extracts_clean(files[0].content, ANI_LANG_TYPESCRIPT, files[0].name));
    ASSERT_TRUE(rp_extracts_clean(files[1].content, ANI_LANG_TYPESCRIPT, files[1].name));

    ANIFileResult *raw =
        ani_extract_file(files[1].content, (int)strlen(files[1].content), ANI_LANG_TYPESCRIPT,
                         "repro", files[1].name, 0, NULL, NULL);
    ASSERT_NOT_NULL(raw);
    ASSERT_EQ(rp_raw_usage_count(raw, "exactReference", "handler", ANI_USAGE_VALUE), 1);
    ASSERT_EQ(rp_raw_usage_count(raw, "exactReference", "handler", ANI_USAGE_CALL_REFERENCE), 0);
    ASSERT_EQ(rp_raw_usage_count(raw, "shadowedValue", "handler", ANI_USAGE_VALUE), 1);
    ani_free_result(raw);
...
TEST(repro_kotlin_empty_cross_filter_does_not_fail_open) {
    ...
    ANIFileResult *consumer =
        ani_extract_file(files[1].content, (int)strlen(files[1].content), ANI_LANG_KOTLIN, "repro",
...
static int rp_assert_callable_alias_graph(const char *filename, const char *source,
                                          ANILanguage language, const char *caller,
                                          const char *callable_label) {
    ASSERT_TRUE(rp_extracts_clean(source, language, filename));

    RProj project;
    ani_store_t *store = rh_index(&project, filename, source);
    if (!store) {
        rh_cleanup(&project, store);
...
TEST(repro_go_callable_alias_invocation_targets_exact_value) {
    static const char source[] = "package aliases\n"
                                 ...
                                 "  return shadowed()\n"
                                 "}\n";
    int rc = rp_assert_callable_alias_graph("aliases.go", source, ANI_LANG_GO, "aliasCaller",
                                            "Function");
    if (rc != 0)
        return rc;
    PASS();
...
TEST(repro_python_callable_alias_invocation_targets_exact_value) {
    static const char source[] = "from typing import Callable\n"
                                 ...
                                 "    return shadowed()\n";
    int rc = rp_assert_callable_alias_graph("aliases.py", source, ANI_LANG_PYTHON, "aliasCaller",
                                            "Function");
    if (rc != 0)
        return rc;
    PASS();
...
TEST(repro_rust_callable_alias_invocation_targets_exact_value) {
    static const char source[] = "fn shadowed() -> i32 { 1 }\n"
                                 ...
                                 "    shadowed()\n"
                                 "}\n";
    int rc = rp_assert_callable_alias_graph("aliases.rs", source, ANI_LANG_RUST, "aliasCaller",
                                            "Function");
    if (rc != 0)
        return rc;
    PASS();
...
TEST(repro_kotlin_callable_alias_invocation_targets_exact_value) {
    static const char source[] = "fun shadowed(): Int = 1\n"
                                 ...
                                 "  return shadowed()\n"
                                 "}\n";
    int rc = rp_assert_callable_alias_graph("Aliases.kt", source, ANI_LANG_KOTLIN, "aliasCaller",
                                            "Function");
    if (rc != 0)
        return rc;
    PASS();
...
TEST(repro_csharp_callable_alias_invocation_targets_exact_value) {
    static const char source[] = "using System;\n"
                                 ...
                                 "  }\n"
                                 "}\n";
    int rc = rp_assert_callable_alias_graph("Aliases.cs", source, ANI_LANG_CSHARP, "aliasCaller",
                                            "Method");
    if (rc != 0)
        return rc;
    PASS();
...
TEST(repro_go_exact_function_value_is_call_reference) {
    ...
    int rc = rp_assert_exact_function_value_reference("refs.go", source, ANI_LANG_GO, "Function");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_python_exact_function_value_is_call_reference) {
    static const char source[] = "def handler():\n"
                                 ...
                                 "    accept(handler)\n";
    int rc =
        rp_assert_exact_function_value_reference("refs.py", source, ANI_LANG_PYTHON, "Function");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_javascript_exact_function_value_is_call_reference) {
    ...
    int rc = rp_assert_exact_function_value_reference("refs.js", source, ANI_LANG_JAVASCRIPT,
                                                      "Function");
    if (rc != 0)
        return rc;
    PASS();
...
TEST(repro_c_exact_function_value_is_call_reference) {
    ...
    int rc = rp_assert_exact_function_value_reference("refs.c", source, ANI_LANG_C, "Function");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_cpp_exact_function_value_is_call_reference) {
    ...
    int rc = rp_assert_exact_function_value_reference("refs.cpp", source, ANI_LANG_CPP, "Function");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_rust_exact_function_value_is_call_reference) {
    ...
    int rc = rp_assert_exact_function_value_reference("refs.rs", source, ANI_LANG_RUST, "Function");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_rust_direct_scoped_function_value_is_exact_reference) {
    ...
    for (size_t i = 0; i < sizeof(files) / sizeof(files[0]); i++) {
        ASSERT_TRUE(rp_extracts_clean(files[i].content, ANI_LANG_RUST, files[i].name));
    }

    ANIFileResult *raw = ani_extract_file(source, (int)strlen(source), ANI_LANG_RUST, "repro",
    ...
    ani_run_rust_lsp_cross(&raw->arena, source, (int)strlen(source), "repro.main", defs, 3, NULL,
    ...
    const ANIUsage *site_usage = NULL;
    for (int i = 0; i < raw->usages.count; i++) {
        const ANIUsage *usage = &raw->usages.items[i];
        if (usage->kind == ANI_USAGE_VALUE && usage->ref_name &&
    ...
    for (int i = 0; i < raw->resolved_calls.count; i++) {
        const ANIResolvedCall *resolved = &raw->resolved_calls.items[i];
    ...
    const ANIResolvedCall *joined =
        site_usage ? ani_pipeline_find_lsp_reference(&raw->resolved_calls, site_usage, false)
                   : NULL;
    bool semantic_join_is_exact = joined && joined->kind == ANI_RESOLVED_CALL_REFERENCE &&
                                  joined->callee_qn &&
...
TEST(repro_csharp_exact_function_value_is_call_reference) {
    static const char source[] = "using System;\n"
                                 ...
                                 "}\n";
    int rc = rp_assert_exact_function_value_reference("Refs.cs", source, ANI_LANG_CSHARP, "Method");
    if (rc != 0)
        return rc;
    PASS();
}
...
static int rp_assert_csharp_callable_value_shape(const char *filename, const char *source,
    ...
    ASSERT_TRUE(rp_extracts_clean(source, ANI_LANG_CSHARP, filename));
...
TEST(repro_csharp_ambiguous_using_static_method_group_is_not_precise) {
    ...
    ANIFileResult *raw = ani_extract_file(source, (int)strlen(source), ANI_LANG_CSHARP, "repro",
    ...
    int ordinary_usage = rp_raw_usage_count(raw, "ambiguousArgument", "handler", ANI_USAGE_VALUE);
    int precise_rows = 0;
    int a_rows = 0;
    int b_rows = 0;
    for (int i = 0; i < raw->resolved_calls.count; i++) {
        const ANIResolvedCall *resolved = &raw->resolved_calls.items[i];
        if (resolved->kind != ANI_RESOLVED_CALL_REFERENCE || !resolved->caller_qn ||
    ...
    RProj project;
    ani_store_t *store = rh_index(&project, "AmbiguousStaticReference.cs", source);
    if (!store) {
        rh_cleanup(&project, store);
...
TEST(repro_cuda_exact_function_value_is_call_reference) {
    static const char source[] = "using Callback = void (*)();\n"
                                 ...
                                 "void accept(Callback callback) {}\n"
                                 "void argument() { accept(handler); }\n";
    int rc = rp_assert_exact_function_value_reference("refs.cu", source, ANI_LANG_CUDA, "Function");
    if (rc != 0)
        return rc;
    PASS();
}
...
static int rp_check_exact_function_value_lsp_site(const RPExactReferenceSiteCase *c) {
    ANIFileResult *result = ani_extract_file(c->source, (int)strlen(c->source), c->language,
    ...
    const bool ts_family = c->language == ANI_LANG_JAVASCRIPT ||
                           c->language == ANI_LANG_TYPESCRIPT || c->language == ANI_LANG_TSX;
    ANIArena cross_arena;
    ANIResolvedCallArray cross_resolved = {0};
    if (ts_family) {
        ...
        ani_arena_init(&cross_arena);
        ani_run_ts_lsp_cross(&cross_arena, c->source, (int)strlen(c->source), "repro.refs",
                             c->language == ANI_LANG_JAVASCRIPT, c->language == ANI_LANG_TSX, false,
                             defs, (int)(sizeof(defs) / sizeof(defs[0])), NULL, NULL, 0, NULL,
                             &cross_resolved);
    }
    const ANIResolvedCallArray *resolved_calls =
    ...
    for (int i = 0; i < resolved_calls->count; i++) {
        const ANIResolvedCall *resolved = &resolved_calls->items[i];
        ...
        if (resolved->kind == ANI_RESOLVED_CALL_REFERENCE && resolved->site_start_byte == start &&
            resolved->site_end_byte == end) {
            exact_references++;
        }
        if (resolved->kind == ANI_RESOLVED_INVOCATION && resolved->site_start_byte == start &&
            resolved->site_end_byte == end) {
            handler_invocations++;
        }
    }
...
TEST(repro_typescript_exact_bound_method_is_call_reference) {
    ...
    int rc =
        rp_assert_exact_function_value_reference("bound.ts", source, ANI_LANG_TYPESCRIPT, "Method");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_tsx_exact_bound_method_is_call_reference) {
    static const char source[] =
        ...
        "function accept(callback: () => void): void {}\n"
        "function argument(service: Service): void { accept(service.handler); }\n";
    int rc = rp_assert_exact_function_value_reference("bound.tsx", source, ANI_LANG_TSX, "Method");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_perl_exact_coderef_is_call_reference) {
    ...
    int rc = rp_assert_exact_function_value_reference("refs.pl", source, ANI_LANG_PERL, "Function");
    if (rc != 0)
        return rc;
    PASS();
}
...
static int rp_assert_cross_file_exact_function_value(const RFile *files, int file_count,
    ...
    RProj project;
    ani_store_t *store = rh_index_files(&project, files, file_count);
    if (!store) {
        rh_cleanup(&project, store);
...
TEST(repro_go_cross_file_exact_function_value_is_call_reference) {
    ...
    int rc = rp_assert_cross_file_exact_function_value(files, 2, ANI_LANG_GO, "Function");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_python_cross_file_exact_function_value_is_call_reference) {
    ...
    int rc = rp_assert_cross_file_exact_function_value(files, 2, ANI_LANG_PYTHON, "Function");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_python_aliased_import_function_value_is_call_reference) {
    ...
    for (int i = 0; i < 2; i++)
        ASSERT_TRUE(rp_extracts_clean(files[i].content, ANI_LANG_PYTHON, files[i].name));
    RProj project;
    ani_store_t *store = rh_index_files(&project, files, 2);
    if (!store) {
        rh_cleanup(&project, store);
...
static int rp_assert_python_callback_occurrence_stays_usage(const char *source,
                                                            const char *caller) {
    ANIFileResult *raw = ani_extract_file(source, (int)strlen(source), ANI_LANG_PYTHON, "repro",
                                          "use.py", 0, NULL, NULL);
    ASSERT_NOT_NULL(raw);
    ASSERT_FALSE(raw->has_error || raw->parse_incomplete);
    ...
    for (int i = 0; i < raw->usages.count; i++) {
        ...
        if (!usage->ref_name || strcmp(usage->ref_name, "callback") != 0 ||
            ...
            continue;
        if (usage->kind == ANI_USAGE_VALUE)
            ordinary_usage++;
        else if (usage->kind == ANI_USAGE_CALL_REFERENCE)
            promoted_usage++;
    }
    for (int i = 0; i < raw->resolved_calls.count; i++) {
        const ANIResolvedCall *resolved = &raw->resolved_calls.items[i];
...
static int rp_assert_python_exact_reference_occurrence(const char *filename, const char *source,
    ...
    for (int i = 0; i < raw->resolved_calls.count; i++) {
        ...
        target_reference_rows += resolved->kind == ANI_RESOLVED_CALL_REFERENCE;
        target_invocation_rows += resolved->kind == ANI_RESOLVED_INVOCATION;
    }
    const ANIResolvedCall *joined =
        carrier ? ani_pipeline_find_lsp_reference(&raw->resolved_calls, carrier, false) : NULL;
    bool carrier_is_candidate = carrier && carrier->may_be_call_reference;
    bool exact_join = joined && joined->kind == ANI_RESOLVED_CALL_REFERENCE &&
    ...
    return rp_assert_exact_function_value_reference(filename, source, ANI_LANG_PYTHON,
                                                    "Function");
}
...
TEST(repro_python_read_before_local_assignment_has_no_graph_reference) {
    static const char source[] = "def handler():\n"
                                 ...
                                 "    accept(handler)\n"
                                 "    handler = 0\n";
    ASSERT_TRUE(rp_extracts_clean(source, ANI_LANG_PYTHON, "read_before_write.py"));

    RProj project;
    ani_store_t *store = rh_index(&project, "read_before_write.py", source);
    if (!store) {
        rh_cleanup(&project, store);
...
TEST(repro_rust_cross_file_exact_function_value_is_call_reference) {
    ...
    int rc = rp_assert_cross_file_exact_function_value(files, 2, ANI_LANG_RUST, "Function");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_csharp_cross_file_exact_function_value_is_call_reference) {
    ...
    int rc = rp_assert_cross_file_exact_function_value(files, 2, ANI_LANG_CSHARP, "Method");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_csharp_cross_file_callable_alias_invocation_targets_exact_value) {
    ...
    int rc = rp_assert_cross_file_callable_alias_graph(files, 2, ANI_LANG_CSHARP, "Method");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_kotlin_cross_file_callable_alias_invocation_targets_exact_value) {
    ...
    int rc = rp_assert_cross_file_callable_alias_graph(files, 2, ANI_LANG_KOTLIN, "Function");
    if (rc != 0)
        return rc;
    PASS();
}
...
static int rp_assert_alias_argument_exact_join(const char *filename, const char *source,
    ...
    const ANIResolvedCall *joined =
        site_usage
            ? ani_pipeline_find_lsp_reference(&raw->resolved_calls, site_usage, allow_tail_match)
            : NULL;
...
TEST(repro_csharp_callable_alias_argument_is_exact_reference) {
    static const char source[] = "using System;\n"
                                 ...
                                 "  }\n"
                                 "}\n";
    int rc = rp_assert_alias_argument_exact_join("AliasArgument.cs", source, ANI_LANG_CSHARP,
...
TEST(repro_kotlin_callable_alias_argument_is_exact_reference) {
    static const char source[] = "fun handler(): Unit {}\n"
                                 ...
                                 "  accept(callback)\n"
                                 "}\n";
    int rc = rp_assert_alias_argument_exact_join("AliasArgument.kt", source, ANI_LANG_KOTLIN,
...
TEST(repro_kotlin_receiver_bound_alias_invocation_targets_exact_method) {
    static const char source[] = "class Service { fun run(): Int = 2 }\n"
                                 ...
                                 "}\n";
    RProj project;
    ani_store_t *store = rh_index(&project, "ReceiverAlias.kt", source);
    if (!store) {
        rh_cleanup(&project, store);
...
TEST(repro_javascript_complex_function_value_stays_usage) {
    ...
    int rc = rp_assert_complex_function_values_stay_usage("complex.js", source, ANI_LANG_JAVASCRIPT,
                                                          "Function");
    if (rc != 0)
        return rc;
    PASS();
...
TEST(repro_typescript_complex_function_value_stays_usage) {
    ...
    int rc = rp_assert_complex_function_values_stay_usage("complex.ts", source, ANI_LANG_TYPESCRIPT,
                                                          "Function");
    if (rc != 0)
        return rc;
    PASS();
...
TEST(repro_tsx_complex_function_value_stays_usage) {
    static const char source[] =
        ...
        "function accept(callback: () => void): void {}\n"
        "function complex(flag: boolean): void { accept(flag ? handler : alternate); }\n";
    int rc = rp_assert_complex_function_values_stay_usage("complex.tsx", source, ANI_LANG_TSX,
                                                          "Function");
    if (rc != 0)
        return rc;
    PASS();
...
TEST(repro_go_complex_function_value_stays_usage) {
    ...
    int rc =
        rp_assert_complex_function_values_stay_usage("complex.go", source, ANI_LANG_GO, "Function");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_python_complex_function_value_stays_usage) {
    ...
    int rc = rp_assert_complex_function_values_stay_usage("complex.py", source, ANI_LANG_PYTHON,
                                                          "Function");
    if (rc != 0)
        return rc;
    PASS();
...
TEST(repro_c_complex_function_value_stays_usage) {
    ...
    int rc =
        rp_assert_complex_function_values_stay_usage("complex.c", source, ANI_LANG_C, "Function");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_cpp_complex_function_value_stays_usage) {
    ...
    int rc = rp_assert_complex_function_values_stay_usage("complex.cpp", source, ANI_LANG_CPP,
                                                          "Function");
    if (rc != 0)
        return rc;
    PASS();
...
TEST(repro_cuda_complex_function_value_stays_usage) {
    static const char source[] =
        ...
        "void accept(Callback callback) {}\n"
        "void complex(bool flag) { accept(flag ? handler : alternate); }\n";
    int rc = rp_assert_complex_function_values_stay_usage("complex.cu", source, ANI_LANG_CUDA,
                                                          "Function");
    if (rc != 0)
        return rc;
    PASS();
...
TEST(repro_rust_complex_function_value_stays_usage) {
    ...
    int rc = rp_assert_complex_function_values_stay_usage("complex.rs", source, ANI_LANG_RUST,
                                                          "Function");
    if (rc != 0)
        return rc;
    PASS();
...
TEST(repro_csharp_complex_function_value_stays_usage) {
    static const char source[] =
        ...
        "}\n";
    int rc = rp_assert_complex_function_values_stay_usage("Complex.cs", source, ANI_LANG_CSHARP,
                                                          "Method");
    if (rc != 0)
        return rc;
    PASS();
...
TEST(repro_java_complex_method_reference_stays_usage) {
    static const char source[] = "interface Task { void run(); }\n"
                                 ...
                                 "}\n";
    int rc =
        rp_assert_complex_function_values_stay_usage("Refs.java", source, ANI_LANG_JAVA, "Method");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_kotlin_complex_callable_reference_stays_usage) {
    static const char source[] = "fun handler() {}\n"
                                 ...
                                 "accept(if (flag) ::handler else ::alternate) }\n";
    int rc = rp_assert_complex_function_values_stay_usage("Refs.kt", source, ANI_LANG_KOTLIN,
                                                          "Function");
    if (rc != 0)
        return rc;
    PASS();
...
TEST(repro_php_complex_first_class_callable_stays_usage) {
    ...
    int rc =
        rp_assert_complex_function_values_stay_usage("refs.php", source, ANI_LANG_PHP, "Function");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_perl_conditional_coderefs_stay_usage) {
    static const char source[] = "sub handler {}\n"
                                 ...
                                 "accept($flag ? \\&handler : \\&alternate); }\n";
    int rc = rp_assert_complex_function_values_stay_usage("complex.pl", source, ANI_LANG_PERL,
                                                          "Function");
    if (rc != 0)
        return rc;
    PASS();
...
TEST(repro_c_ambiguous_alias_reassignment_drops_stale_target) {
    ...
    ASSERT_TRUE(rp_extracts_clean(source, ANI_LANG_C, "stale_alias.c"));
    RProj project;
    ani_store_t *store = rh_index(&project, "stale_alias.c", source);
    if (!store) {
        rh_cleanup(&project, store);
...
TEST(repro_c_conditional_alias_reassignment_drops_stale_target) {
    ...
    ASSERT_TRUE(rp_extracts_clean(source, ANI_LANG_C, "conditional_alias.c"));
    RProj project;
    ani_store_t *store = rh_index(&project, "conditional_alias.c", source);
    if (!store) {
        rh_cleanup(&project, store);
...
TEST(repro_c_noncallable_shadow_blocks_outer_alias_target) {
    ...
    ASSERT_TRUE(rp_extracts_clean(source, ANI_LANG_C, "shadowed_alias.c"));
    RProj project;
    ani_store_t *store = rh_index(&project, "shadowed_alias.c", source);
    if (!store) {
        rh_cleanup(&project, store);
...
static int rp_assert_conditional_alias_reference_fails_closed(const char *filename,
    ...
    for (int i = 0; i < raw->resolved_calls.count; i++) {
        ...
        if (resolved->kind == ANI_RESOLVED_CALL_REFERENCE) {
            precise_reference++;
        } else if (resolved->kind == ANI_RESOLVED_INVOCATION) {
...
TEST(repro_cpp_conditional_alias_reference_fails_closed) {
    ...
    int rc = rp_assert_conditional_alias_reference_fails_closed(
        "conditional_alias.cpp", source, ANI_LANG_CPP, "conditionalAliasCaller");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_cuda_conditional_alias_reference_fails_closed) {
    static const char source[] = "void actual() {}\n"
                                 ...
                                 "}\n";
    int rc = rp_assert_conditional_alias_reference_fails_closed(
        "conditional_alias.cu", source, ANI_LANG_CUDA, "conditionalAliasCaller");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_go_conditional_alias_reference_fails_closed) {
    static const char source[] = "package refs\n"
                                 ...
                                 "}\n";
    int rc = rp_assert_conditional_alias_reference_fails_closed(
        "conditional_alias.go", source, ANI_LANG_GO, "conditionalAliasCaller");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_python_conditional_alias_reference_fails_closed) {
    static const char source[] = "def actual():\n    pass\n"
                                 ...
                                 "    accept(callback)\n";
    int rc = rp_assert_conditional_alias_reference_fails_closed(
        "conditional_alias.py", source, ANI_LANG_PYTHON, "conditionalAliasCaller");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_rust_conditional_alias_reference_fails_closed) {
    static const char source[] = "fn actual() {}\n"
                                 ...
                                 "}\n";
    int rc = rp_assert_conditional_alias_reference_fails_closed(
        "conditional_alias.rs", source, ANI_LANG_RUST, "conditionalAliasCaller");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_kotlin_conditional_alias_reference_fails_closed) {
    static const char source[] = "fun actual(): Unit {}\n"
                                 ...
                                 "}\n";
    int rc = rp_assert_conditional_alias_reference_fails_closed(
        "ConditionalAlias.kt", source, ANI_LANG_KOTLIN, "conditionalAliasCaller");
    if (rc != 0)
        return rc;
    PASS();
}
...
TEST(repro_csharp_conditional_alias_reference_fails_closed) {
    static const char source[] = "using System;\n"
                                 ...
                                 "}\n";
    int rc = rp_assert_conditional_alias_reference_fails_closed(
        "ConditionalAlias.cs", source, ANI_LANG_CSHARP, "conditionalAliasCaller");
    if (rc != 0)
        return rc;
    PASS();
}
