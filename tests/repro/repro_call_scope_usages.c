...
static int usage_count(const ANIFileResult *r, const char *caller, const char *target) {
    int count = 0;
    for (int i = 0; i < r->usages.count; i++) {
        const ANIUsage *usage = &r->usages.items[i];
...
static int call_reference_count(const ANIFileResult *r, const char *caller, const char *target) {
    int count = 0;
    for (int i = 0; i < r->usages.count; i++) {
        const ANIUsage *usage = &r->usages.items[i];
        if (usage->kind == ANI_USAGE_CALL_REFERENCE && usage->ref_name &&
            strcmp(usage->ref_name, target) == 0 &&
            (!caller || qn_ends_with(usage->enclosing_func_qn, caller))) {
            count++;
        }
...
static int call_count(const ANIFileResult *r, const char *caller, const char *target) {
    int count = 0;
...
static int exact_usage_count(const ANIFileResult *r, const char *caller_qn, const char *target) {
    int count = 0;
    for (int i = 0; i < r->usages.count; i++) {
        const ANIUsage *usage = &r->usages.items[i];
        if (usage->kind == ANI_USAGE_VALUE && usage->ref_name && usage->enclosing_func_qn &&
            strcmp(usage->ref_name, target) == 0 &&
...
static int exact_call_count(const ANIFileResult *r, const char *caller_qn, const char *callee) {
    int count = 0;
    for (int i = 0; i < r->calls.count; i++) {
        const ANICall *call = &r->calls.items[i];
...
static int exact_definition_count(const ANIFileResult *r, const char *name,
                                  const char *qualified_name) {
    int count = 0;
    for (int i = 0; i < r->defs.count; i++) {
        const ANIDefinition *def = &r->defs.items[i];
...
static int definition_count(const ANIFileResult *r, const char *name) {
    int count = 0;
    for (int i = 0; i < r->defs.count; i++) {
        const ANIDefinition *def = &r->defs.items[i];
...
static ANIFileResult *extract_case(const char *tag, const char *source, ANILanguage language,
                                   const char *filename) {
    ANIFileResult *r =
...
TEST(repro_call_scope_same_spelling_callee_and_argument) {
    ...
    ANIFileResult *r = extract_case("same_spelling", source, ANI_LANG_JAVASCRIPT, "main.js");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_nested_calls_retain_each_argument) {
    ...
    ANIFileResult *r = extract_case("nested_calls", source, ANI_LANG_TYPESCRIPT, "main.ts");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_inline_callback_body_usage) {
    ...
    ANIFileResult *r = extract_case("inline_callback", source, ANI_LANG_JAVASCRIPT, "main.js");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_member_and_computed_components) {
    ...
    ANIFileResult *r = extract_case("member_computed", source, ANI_LANG_TYPESCRIPT, "main.ts");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_constructor_and_generic_arguments) {
    ...
    ANIFileResult *r = extract_case("constructor_generic", source, ANI_LANG_TYPESCRIPT, "main.ts");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_c_function_pointer_invocation_and_arguments) {
    ...
    ANIFileResult *r = extract_case("c_function_pointer", source, ANI_LANG_C, "main.c");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_cpp_function_pointer_invocation_and_arguments) {
    ...
    ANIFileResult *r = extract_case("cpp_function_pointer", source, ANI_LANG_CPP, "main.cpp");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_python_keyword_argument) {
    ...
    ANIFileResult *r = extract_case("python_keyword", source, ANI_LANG_PYTHON, "main.py");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_python_with_body_usage) {
    static const char source[] = "watched = 1\n"
                                 "def manager():\n    pass\n"
                                 "def run():\n    with manager():\n        watched\n";
    ANIFileResult *r = extract_case("python_with", source, ANI_LANG_PYTHON, "main.py");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_rust_macro_argument) {
    ...
    ANIFileResult *r = extract_case("rust_macro", source, ANI_LANG_RUST, "main.rs");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_elixir_binary_operator_operand) {
    static const char source[] = "defmodule Sample do\n"
                                 ...
                                 "end\n";
    ANIFileResult *r = extract_case("elixir_binary", source, ANI_LANG_ELIXIR, "sample.ex");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_fortran_keyword_argument) {
    static const char source[] = "module sample\n"
                                 ...
                                 "  end function run\n"
                                 "end module sample\n";
    ANIFileResult *r = extract_case("fortran_keyword", source, ANI_LANG_FORTRAN, "main.f90");
    ASSERT_NOT_NULL(r);
    int failures = 0;
 ...
 * subroutine invocation currently emits no ANICall. Keep this independent from
...
TEST(repro_fortran_subroutine_call_is_extracted) {
    static const char source[] = "subroutine consume(value)\n"
                                 ...
                                 "end subroutine run\n";
    ANIFileResult *r =
        extract_case("fortran_subroutine_call", source, ANI_LANG_FORTRAN, "main.f90");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_kotlin_property_navigation) {
    ...
    ANIFileResult *r = extract_case("kotlin_navigation", source, ANI_LANG_KOTLIN, "Main.kt");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_php_first_class_callable) {
    static const char source[] = "<?php\n"
                                 ...
                                 "  foo(...$args);\n"
                                 "}\n";
    ANIFileResult *r = extract_case("php_first_class", source, ANI_LANG_PHP, "main.php");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_scope_stack_preserves_deepest_function) {
    ...
    ANIFileResult *r = extract_case("deep_scope", source, ANI_LANG_JAVASCRIPT, "deep.js");
    free(source);
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_usage_declarations_are_not_references) {
    static const char source[] = "function run(unusedParameter: number,\n"
                                 ...
                                 "}\n";
    ANIFileResult *r = extract_case("declaration_usage", source, ANI_LANG_TYPESCRIPT, "main.ts");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_objectscript_typed_receiver_exact_owner) {
    ...
    ANIFileResult *r = extract_case("objectscript_typed_receiver", source,
                                    ANI_LANG_OBJECTSCRIPT_UDL, "Behavior.cls");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_objectscript_routine_tag_exact_owner) {
    ...
    ANIFileResult *r = extract_case("objectscript_routine_scope", source,
                                    ANI_LANG_OBJECTSCRIPT_ROUTINE, "Behavior.mac");
    ASSERT_NOT_NULL(r);
    int failures = 0;
