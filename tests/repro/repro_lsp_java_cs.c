 ...
 * (internal/ani/lsp/java_lsp.c) and C# (internal/ani/lsp/cs_lsp.c) hybrid LSPs.
 *
 ...
 *   resolver can still be ABSENT here if ani_pipeline_find_lsp_resolution
 ...
 *   grepped from source (grep '"lsp_' internal/ani/lsp/java_lsp.c):
 ...
 *   from source (grep '"cs_' internal/ani/lsp/cs_lsp.c):
...
static int assert_lsp_strategy(const char *filename, const char *src,
                               const char *strategy) {
    RProj lp;
    ani_store_t *store = rh_index(&lp, filename, src);
    if (!store) {
...
static int count_edges_from_callable(ani_store_t *store, const char *project, const char *edge_type,
    ...
    for (int i = 0; i < edge_count; i++) {
        ani_node_t source = {0};
        if (ani_store_find_node_by_id(store, edges[i].source_id, &source) == ANI_STORE_OK &&
...
static int assert_method_reference_semantics(const char *filename, const char *src,
    ...
    RProj project;
    ani_store_t *store = rh_index(&project, filename, src);
    if (!store) {
