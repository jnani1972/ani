 ...
 *
 * Cluster (TIER A, in-process via ani_extract_file):
...
static ANIFileResult *rx(const char *src, ANILanguage lang, const char *proj, const char *path) {
...
TEST(repro_issue554_cpp_out_of_line_method_class_qualified) {
    ANIFileResult *r = rx("struct Foo { void bar(); };\n"
    ...
    ANIDefinition *method = find_def(r, "Method", "bar");
    if (!method)
