 ...
 *   checks:
 *     ani_kind_in_set(method_node, spec->function_node_types)
 ...
 *   A. ani_extract_file must emit a Method def with name="handleClick"
 ...
 *      line ~3620 guards on ani_kind_in_set(method_node, spec->function_node_types);
...
static ANIFileResult *rx_ts(const char *src) {
    return ani_extract_file(src, (int)strlen(src), ANI_LANG_TYPESCRIPT, "proj", "ts.ts",
                            0, NULL, NULL);
}

static ANIDefinition *find_def_by_name(ANIFileResult *r, const char *label, const char *name) {
...
TEST(repro_new_ts_class_field_arrow_method_def_dropped) {
    ...
    ANIFileResult *r = rx_ts(src);
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);

    /* Precondition: the class Foo itself must be extracted. */
    ANIDefinition *cls = find_def_by_name(r, "Class", "Foo");
    ASSERT_NOT_NULL(cls);

    /* Precondition: the free helper() function must be extracted. */
    ANIDefinition *helper = find_def_by_name(r, "Function", "helper");
     ...
     * WHY RED: extract_class_methods bails out at the ani_kind_in_set check
    ...
    ANIDefinition *method = find_def_by_name(r, "Method", "handleClick");
