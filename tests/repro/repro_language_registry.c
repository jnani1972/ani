/* Reproductions for ANILanguage-to-spec registry invariants.
 *
...
size_t repro_call_argument_matrix_a_copy_language_ids(ANILanguage *language_ids, size_t capacity);
...
#define CALL_WITH_REFERENCE_VOCAB(lang)                        \
    [ANI_LANG_##lang] = {CAP_CALL_WITH_REFERENCE_VOCAB, #lang, \
                         "call metadata and a currently recognized reference node"}
#define CALL_WITHOUT_REFERENCE_VOCAB(lang)                        \
    [ANI_LANG_##lang] = {CAP_CALL_WITHOUT_REFERENCE_VOCAB, #lang, \
                         "call metadata but no reference node recognized by extract_usages"}
#define NO_CALL(lang) \
    [ANI_LANG_##lang] = {CAP_NO_CALL, #lang, "no call-node metadata; callback case inapplicable"}
#define TRANSFORM_ONLY(lang)                        \
    [ANI_LANG_##lang] = {CAP_TRANSFORM_ONLY, #lang, \
                         "source is transformed into another registered language first"}
#define UNSUPPORTED(lang) \
    [ANI_LANG_##lang] = {CAP_UNSUPPORTED, #lang, "enum retained without a registered grammar"}
...
static const LanguageCapabilityEntry LANGUAGE_CAPABILITIES[ANI_LANG_COUNT] = {
...
_Static_assert(sizeof(LANGUAGE_CAPABILITIES) / sizeof(LANGUAGE_CAPABILITIES[0]) == ANI_LANG_COUNT,
               "language capability ledger must track ANI_LANG_COUNT");
 ...
 * the zero-initialized ANI_LANG_GO slot. */
...
TEST(repro_language_capability_ledger_covers_every_enum) {
    ...
    for (int value = 0; value < ANI_LANG_COUNT; value++) {
        ...
        const ANILangSpec *spec = ani_lang_spec((ANILanguage)value);
        ...
        if (!spec || spec->language != (ANILanguage)value) {
...
TEST(repro_call_argument_matrices_equal_call_capability_ledger) {
    ...
    ANILanguage matrix_a_ids[ANI_LANG_COUNT];
    ANILanguage matrix_b_ids[ANI_LANG_COUNT];
    unsigned int matrix_a_counts[ANI_LANG_COUNT] = {0};
    unsigned int matrix_b_counts[ANI_LANG_COUNT] = {0};
    ...
    size_t matrix_a_copied =
        matrix_a_rows < ANI_LANG_COUNT ? matrix_a_rows : (size_t)ANI_LANG_COUNT;
    for (size_t row = 0; row < matrix_a_copied; row++) {
        int language_id = (int)matrix_a_ids[row];
        if (language_id < 0 || language_id >= ANI_LANG_COUNT) {
            fprintf(stderr,
    ...
    size_t matrix_b_copied =
        matrix_b_rows < ANI_LANG_COUNT ? matrix_b_rows : (size_t)ANI_LANG_COUNT;
    ...
    for (int language_id = 0; language_id < ANI_LANG_COUNT; language_id++) {
        ...
        int expected_domain_control =
            language_id == ANI_LANG_DIFF || language_id == ANI_LANG_BIBTEX;
...
TEST(repro_primary_call_metadata_names_exist_in_grammars) {
    ...
    for (int value = 0; value < ANI_LANG_COUNT; value++) {
        const LanguageCapabilityEntry *entry = &LANGUAGE_CAPABILITIES[value];
        const ANILangSpec *spec = ani_lang_spec((ANILanguage)value);
        const TSLanguage *language = ani_ts_language((ANILanguage)value);
...
TEST(repro_linkerscript_root_is_registered_as_module) {
    ...
    const ANILangSpec *spec = ani_lang_spec(ANI_LANG_LINKERSCRIPT);
    const TSLanguage *language = ani_ts_language(ANI_LANG_LINKERSCRIPT);
