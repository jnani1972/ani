 ...
 *   ani_pipeline_find_lsp_resolution  (src/pipeline/lsp_resolve.h:48)
 *   joins each LSP-resolved call (ANIResolvedCall) to the tree-sitter call
 *   (ANICall) with EXACT string equality on the caller QN:
 *
 *       lsp_resolve.h:65:
 ...
 *
 *   When tree-sitter's enclosing-func walk FAILS, ani_enclosing_func_qn
 *   falls back to the MODULE QN, so call->enclosing_func_qn is the module
 *   QN. The C/C++ LSP cross resolver (internal/ani/lsp/c_lsp.c) builds its
 ...
 *   (c_process_function, c_lsp.c:4138-4143) and emits a ANIResolvedCall
 ...
 *   ANI_LSP_CONFIDENCE_FLOOR (0.6f, lsp_resolve.h:36).
 *
 ...
 *   out-of-line method body, tree-sitter's ani_find_enclosing_func cannot
 ...
 *   class-qualified method QN, so ani_enclosing_func_qn falls back to the
 *   module QN (issue #554 / extract_defs.c + c_lsp.c dominate the
 *   QUALITY_ANALYSIS Module-sourced-CALLS top-file list). C/C++ has a
 *   cross-file LSP wired up (ani_pxc_has_cross_lsp, pass_lsp_cross.c:281),
 ...
 */
static int find_call_edge_to_helper(ani_store_t *store, const char *project,
                                    ani_node_t *out_src, char *out_props,
                                    size_t props_cap) {
    ani_edge_t *edges = NULL;
    int nedges = 0;
    if (ani_store_find_edges_by_type(store, project, "CALLS", &edges, &nedges)
            != ANI_STORE_OK) {
        return 0;
    }
    ...
    for (int i = 0; i < nedges; i++) {
        ani_node_t tgt;
        if (ani_store_find_node_by_id(store, edges[i].target_id, &tgt)
                != ANI_STORE_OK) {
            continue;
        }
        ...
        if (ani_store_find_node_by_id(store, edges[i].source_id, out_src)
                == ANI_STORE_OK) {
 ...
 *
 * Today the join in ani_pipeline_find_lsp_resolution (lsp_resolve.h:65)
...
TEST(repro_invariant_lsp_rescue_source) {
    RProj lp;
    ani_store_t *store = rh_index(&lp, "main.cpp", kCppOutOfLine);
