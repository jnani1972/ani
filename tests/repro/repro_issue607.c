 ...
 *   src/cli/cli.c  ani_cmd_install()  printed
 *     "Found %d existing index(es) that must be rebuilt:\n"
 *   then called ani_remove_indexes(home) which unlinked every .db and NEVER
 ...
 *
 *     int ani_install_handle_existing_indexes(const char *home,
 ...
 *        ani_install_handle_existing_indexes(home, reset=false, dry_run=false)
 ...
 *        ani_install_handle_existing_indexes(home, reset=true, dry_run=false)
 ...
 *      ANI_ASSUME_YES so the test is non-interactive.
 *
...
#define REPRO607_PROJECT "ani-repro607-test"
...
static int repro607_make_index(const char *tmp_cache, char *db_path, size_t db_path_sz) {
    ...
    int upsert_rc =
        ani_store_upsert_project(setup_store, REPRO607_PROJECT, "/home/user/my-project");
    ani_store_close(setup_store);
    return (upsert_rc == ANI_STORE_OK) ? 1 : 0;
}
...
TEST(repro_issue607_reinstall_preserves_index) {
    /* Redirect ANI_CACHE_DIR to a fresh temp dir so the real user cache is
     * never touched and count_db_indexes()/ani_list_indexes() see only the
     * DB we create here. */
    char tmp_cache[512];
    snprintf(tmp_cache, sizeof(tmp_cache), "/tmp/ani_repro607_XXXXXX");
     ...
     * call ani_remove_indexes(). The fix must NOT delete here regardless.
     */
    int proceed =
        ani_install_handle_existing_indexes(tmp_cache /* fake home */, false /* reset */,
                                            false /* dry_run */);
 ...
 * --reset-indexes flag. Auto-answers the delete prompt via ANI_ASSUME_YES so
 * the test stays non-interactive.
 * ─────────────────────────────────────────────────────────────────── */
TEST(repro_issue607_reset_indexes_deletes) {
    char tmp_cache[512];
    snprintf(tmp_cache, sizeof(tmp_cache), "/tmp/ani_repro607r_XXXXXX");
    if (!ani_mkdtemp(tmp_cache)) {
    ...
    ani_set_auto_answer_for_test(1 /* AUTO_YES */);

    /* Opt-in destructive path: reset=true must delete the index. */
    int proceed =
        ani_install_handle_existing_indexes(tmp_cache /* fake home */, true /* reset */,
                                            false /* dry_run */);
    int proceeded = (proceed == 1);
    ...
    ani_set_auto_answer_for_test(0 /* prompt */);
