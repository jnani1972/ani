 ...
 *
 * The public ANIDefinition.param_types array is a legacy type-usage projection:
 * it is not positional.  Cross-LSP registration instead receives the internal
 * ANILSPDef.signature_param_types + signature_param_count carrier.  These tests
...
typedef ANITypeRegistry *(*CrossRegistryBuilder)(ANIArena *, ANILSPDef *, int);
...
static TSTree *parse_lsp_fixture(ANILanguage language, const char *source) {
    const TSLanguage *grammar = ani_ts_language(language);
...
static ANITypeRegistry *alloc_test_registry(ANIArena *arena) {
    ANITypeRegistry *registry = (ANITypeRegistry *)ani_arena_alloc(arena, sizeof(*registry));
    if (!registry)
        return NULL;
...
static ANITypeRegistry *build_java_cross_registry_for_contract(ANIArena *arena, ANILSPDef *defs,
                                                               int def_count) {
    ANITypeRegistry *registry = alloc_test_registry(arena);
    if (registry)
        ani_java_register_lsp_defs(arena, registry, defs, def_count);
    return registry;
}

static ANITypeRegistry *build_kotlin_cross_registry_for_contract(ANIArena *arena, ANILSPDef *defs,
                                                                 int def_count) {
    ANITypeRegistry *registry = alloc_test_registry(arena);
    if (registry)
        ani_kotlin_register_lsp_defs(arena, registry, defs, def_count, NULL);
    return registry;
}

static ANITypeRegistry *build_php_cross_registry_for_contract(ANIArena *arena, ANILSPDef *defs,
                                                              int def_count) {
    ANITypeRegistry *registry = alloc_test_registry(arena);
    if (registry)
        ani_php_register_lsp_defs(arena, NULL, registry, defs, def_count);
    return registry;
}

static int func_param_count(const ANIType *signature) {
    if (!signature || signature->kind != ANI_TYPE_FUNC || !signature->data.func.param_types)
        return 0;
    int count = 0;
...
static const char *type_atom_name(const ANIType *type) {
    if (!type)
        return NULL;
    switch (type->kind) {
...
static int param_matches(const ANIType *actual, const char *expected) {
    if (strcmp(expected, "?") == 0)
        return ani_type_is_unknown(actual);
    if (!actual || ani_type_is_unknown(actual))
        return 0;
...
static int check_signature(const ANITypeRegistry *registry, const char *qualified_name,
                           const char *case_name, const char *const *expected, int expected_count) {
    const ANIRegisteredFunc *func = ani_registry_lookup_func(registry, qualified_name);
    if (!func) {
        fprintf(stderr, "  [ordered-signature] case=%s missing function=%s\n", case_name,
                qualified_name);
        return 1;
    }
    if (!func->signature || func->signature->kind != ANI_TYPE_FUNC) {
    ...
    for (int i = 0; i < expected_count; i++) {
        const ANIType *actual = func->signature->data.func.param_types[i];
        if (!param_matches(actual, expected[i])) {
            ...
            fprintf(stderr,
                    ...
                    actual_name ? actual_name
                                : (ani_type_is_unknown(actual) ? "?" : "<structured>"),
...
static int check_single_return(const ANITypeRegistry *registry, const char *qualified_name,
                               const char *case_name, const char *expected) {
    const ANIRegisteredFunc *func = ani_registry_lookup_func(registry, qualified_name);
    if (!func || !func->signature || func->signature->kind != ANI_TYPE_FUNC ||
    ...
    const ANIType *actual = func->signature->data.func.return_types[0];
    if (!param_matches(actual, expected)) {
        ...
        fprintf(stderr,
                ...
                actual_name ? actual_name : (ani_type_is_unknown(actual) ? "?" : "<structured>"),
                (int)actual->kind);
        return 1;
    }
    return 0;
...
static int check_nullable_signature(const ANITypeRegistry *registry, const char *qualified_name,
    ...
    if (ani_type_is_unknown(func->signature->data.func.param_types[0])) {
        fprintf(stderr,
...
static void init_function_def(ANILSPDef *def, ANIArena *arena,
                              const OrderedSignatureLanguage *language, const char *case_name,
                              const char **param_types, int param_count) {
    memset(def, 0, sizeof(*def));
    def->qualified_name = ani_arena_sprintf(arena, "ani.ordered.%s.%s", language->tag, case_name);
    def->short_name = case_name;
    def->label = "Function";
    def->def_module_qn = ani_arena_sprintf(arena, "ani.ordered.%s", language->tag);
...
static int run_typed_language_contract(const OrderedSignatureLanguage *language) {
    ...
    ANITypeRegistry *registry = language->build(&arena, defs, 5);
    if (!registry) {
...
static int run_javascript_unknown_contract(const OrderedSignatureLanguage *language) {
    ...
    ANITypeRegistry *registry = language->build(&arena, defs, 2);
    if (!registry) {
        fprintf(stderr, "  [ordered-signature] lang=%s registry build failed\n", language->tag);
        ani_arena_destroy(&arena);
        return 1;
...
static const OrderedSignatureLanguage C_LANGUAGE = {
    "c", ANI_LANG_C, ani_c_build_cross_registry, "int", "string", "Thing *", "void"};
static const OrderedSignatureLanguage CPP_LANGUAGE = {
    "cpp", ANI_LANG_CPP, ani_c_build_cross_registry, "int", "string", "std::optional<Thing>",
    "void"};
static const OrderedSignatureLanguage CUDA_LANGUAGE = {
    "cuda", ANI_LANG_CUDA, ani_c_build_cross_registry, "int", "string", "Thing *", "void"};
static const OrderedSignatureLanguage GO_LANGUAGE = {
    "go", ANI_LANG_GO, ani_go_build_cross_registry, "int", "string", "*Thing", "void"};
static const OrderedSignatureLanguage JS_LANGUAGE = {
    "javascript", ANI_LANG_JAVASCRIPT, ani_ts_build_cross_registry, NULL, NULL, NULL, "void"};
static const OrderedSignatureLanguage TS_LANGUAGE = {"typescript",
                                                     ANI_LANG_TYPESCRIPT,
                                                     ani_ts_build_cross_registry,
                                                     "number",
                                                     "string",
                                                     "Thing | null",
                                                     "void"};
static const OrderedSignatureLanguage TSX_LANGUAGE = {
    "tsx", ANI_LANG_TSX, ani_ts_build_cross_registry, "number", "string", "Thing | null", "void"};
static const OrderedSignatureLanguage PYTHON_LANGUAGE = {
    "python", ANI_LANG_PYTHON, ani_py_build_cross_registry, "int", "str", "Thing | None", "None"};
static const OrderedSignatureLanguage CSHARP_LANGUAGE = {
    "csharp", ANI_LANG_CSHARP, ani_cs_build_cross_registry, "int", "string", "Thing?", "void"};
static const OrderedSignatureLanguage RUST_LANGUAGE = {
    "rust", ANI_LANG_RUST, ani_rust_build_cross_registry, "i32", "String", "Option<Thing>", "()"};
static const OrderedSignatureLanguage JAVA_LANGUAGE = {
    "java", ANI_LANG_JAVA, build_java_cross_registry_for_contract,
    "int",  "String",      "java.util.List<?>",
    "void"};
static const OrderedSignatureLanguage KOTLIN_LANGUAGE = {
    "kotlin", ANI_LANG_KOTLIN, build_kotlin_cross_registry_for_contract, "Int", "String",
    "Thing?", "Unit"};
static const OrderedSignatureLanguage PHP_LANGUAGE = {
    "php", ANI_LANG_PHP, build_php_cross_registry_for_contract, "int", "string", "?Thing", "void"};

static void add_fixture_type(ANITypeRegistry *registry, const char *qualified_name,
                             const char *short_name) {
    ANIRegisteredType type = {0};
    type.qualified_name = qualified_name;
    type.short_name = short_name;
    ani_registry_add_type(registry, type);
}

static void add_php_seeded_method(ANIArena *arena, ANITypeRegistry *registry,
    ...
    const ANIType *param_types[] = {ani_type_builtin(arena, "int"), ani_type_named(arena, thing_qn),
                                    ani_type_builtin(arena, "int"), NULL};
    const ANIType *return_types[] = {return_type, NULL};
    ANIRegisteredFunc function = {0};
    ...
    function.signature = ani_type_func(arena, param_names, param_types, return_types);
    ani_registry_add_func(registry, function);
}

static void add_csharp_seeded_method(ANIArena *arena, ANITypeRegistry *registry,
                                     const char *qualified_name, const char *short_name,
                                     const char *receiver_type, const char *thing_qn) {
    const ANIType *param_types[] = {ani_type_builtin(arena, "int"), ani_type_named(arena, thing_qn),
                                    ani_type_builtin(arena, "int"), NULL};
    const ANIType *return_types[] = {ani_type_unknown(), NULL};
    ANIRegisteredFunc function = {0};
    function.qualified_name = qualified_name;
    function.short_name = short_name;
    function.receiver_type = receiver_type;
    function.min_params = 3;
    function.signature = ani_type_func(arena, NULL, param_types, return_types);
    ani_registry_add_func(registry, function);
}
...
static const ANIType *parse_signature_probe_type(ANIArena *arena, const char *text,
                                                 void *parser_ctx) {
...
TEST(repro_lsp_signature_param_materializer_contract) {
    ...
    const ANIType **ordered = ani_type_materialize_signature_params(
    ...
    ASSERT_EQ(ordered[0]->kind, ANI_TYPE_BUILTIN);
    ASSERT_STR_EQ(ordered[0]->data.builtin.name, "int");
    ASSERT_TRUE(ani_type_is_unknown(ordered[1]));
    ASSERT_EQ(ordered[2]->kind, ANI_TYPE_BUILTIN);
    ASSERT_STR_EQ(ordered[2]->data.builtin.name, "string");
    ASSERT_NULL(ordered[3]);

    memset(&probe, 0, sizeof(probe));
    const char *duplicate_texts[] = {"int", "int"};
    const ANIType **duplicates = ani_type_materialize_signature_params(
    ...
    const ANIType **trailing = ani_type_materialize_signature_params(
    ...
    ASSERT_TRUE(ani_type_is_unknown(trailing[2]));
    ASSERT_NULL(trailing[3]);

    memset(&probe, 0, sizeof(probe));
    const char *sentinel_texts[] = {"?", "?Foo", "Foo?", "parser-null", "", NULL};
    const ANIType **sentinels = ani_type_materialize_signature_params(
    ...
    ASSERT_TRUE(ani_type_is_unknown(sentinels[0]));
    ASSERT_EQ(sentinels[1]->kind, ANI_TYPE_BUILTIN);
    ASSERT_STR_EQ(sentinels[1]->data.builtin.name, "?Foo");
    ASSERT_EQ(sentinels[2]->kind, ANI_TYPE_BUILTIN);
    ASSERT_STR_EQ(sentinels[2]->data.builtin.name, "Foo?");
    ASSERT_TRUE(ani_type_is_unknown(sentinels[3]));
    ASSERT_TRUE(ani_type_is_unknown(sentinels[4]));
    ASSERT_TRUE(ani_type_is_unknown(sentinels[5]));
    ASSERT_NULL(sentinels[6]);

    memset(&probe, 0, sizeof(probe));
    const ANIType **missing_texts =
        ani_type_materialize_signature_params(&arena, NULL, 3, parse_signature_probe_type, &probe);
    ASSERT_NOT_NULL(missing_texts);
    ASSERT_EQ(probe.call_count, 0);
    ASSERT_TRUE(ani_type_is_unknown(missing_texts[0]));
    ASSERT_TRUE(ani_type_is_unknown(missing_texts[1]));
    ASSERT_TRUE(ani_type_is_unknown(missing_texts[2]));
    ASSERT_NULL(missing_texts[3]);
    ASSERT_NULL(ani_type_materialize_signature_params(&arena, ordered_texts, 0,
...
TEST(repro_lsp_func_replace_returns_contract) {
    ...
    const ANIType *integer = ani_type_builtin(&arena, "int");
    const ANIType *string = ani_type_builtin(&arena, "string");
    const ANIType *old_return = ani_type_builtin(&arena, "old-return");
    const ANIType *new_return = ani_type_builtin(&arena, "new-return");
    const char *param_names[] = {"first", "middle", "last", NULL};
    const ANIType *param_types[] = {integer, ani_type_unknown(), string, NULL};
    const ANIType *old_returns[] = {old_return, NULL};
    const ANIType *old_signature = ani_type_func(&arena, param_names, param_types, old_returns);
    ASSERT_NOT_NULL(old_signature);
    ASSERT_EQ(old_signature->kind, ANI_TYPE_FUNC);

    const ANIType *new_returns[] = {new_return, NULL};
    const ANIType *replaced = ani_type_func_replace_returns(&arena, old_signature, new_returns);
    ASSERT_NOT_NULL(replaced);
    ASSERT_EQ(replaced->kind, ANI_TYPE_FUNC);
    ...
    ASSERT_TRUE(ani_type_is_unknown(replaced->data.func.param_types[1]));
    ...
    ASSERT_TRUE(ani_type_is_unknown(old_signature->data.func.param_types[1]));
...
static int run_c_explicit_template_value_signature_contract(const char *source,
    ...
    TSTree *tree = parse_lsp_fixture(ANI_LANG_CPP, source);
    ...
    const ANIType *param_types[] = {ani_type_builtin(&arena, "long"),
                                    ani_type_builtin(&arena, "bool"), NULL};
    const ANIType *return_types[] = {ani_type_type_param(&arena, "T"), NULL};
    const char *type_params[] = {"T", NULL};
    ANIRegisteredFunc registered = {0};
    registered.qualified_name = registered_qn;
    registered.short_name = "materialize";
    registered.signature = ani_type_func(&arena, param_names, param_types, return_types);
    registered.type_param_names = type_params;
    registered.min_params = 2;
    ani_registry_add_func(&registry, registered);

    ANIResolvedCallArray out = {0};
    CLSPContext ctx;
    c_lsp_init(&ctx, &arena, source, (int)strlen(source), &registry, NULL,
               /*cpp_mode=*/true, &out);
    const ANIType *value_type = c_eval_expr_type(&ctx, function);

    ASSERT_NOT_NULL(value_type);
    ASSERT_EQ(value_type->kind, ANI_TYPE_FUNC);
    ASSERT_NOT_NULL(value_type->data.func.return_types);
    ASSERT_NOT_NULL(value_type->data.func.return_types[0]);
    ASSERT_EQ(value_type->data.func.return_types[0]->kind, ANI_TYPE_BUILTIN);
    ...
    ASSERT_EQ(value_type->data.func.param_types[0]->kind, ANI_TYPE_BUILTIN);
    ASSERT_STR_EQ(value_type->data.func.param_types[0]->data.builtin.name, "long");
    ASSERT_EQ(value_type->data.func.param_types[1]->kind, ANI_TYPE_BUILTIN);
...
TEST(repro_lsp_ordered_signatures_csharp_cross_return) {
    ...
    ANITypeRegistry *registry = ani_cs_build_cross_registry(&arena, &def, 1);
...
TEST(repro_lsp_ordered_signatures_csharp_ast_return_refinement) {
    const char *module_qn = "ani.ordered.csharp.local";
    const char *subject_qn = "ani.ordered.csharp.local.Subject";
    const char *thing_qn = "ani.ordered.csharp.local.Thing";
    const char *result_qn = "ani.ordered.csharp.local.Result";
    const char *method_qn = "ani.ordered.csharp.local.Subject.method";
    const char *source =
        "class Thing {}\n"
        "class Result {}\n"
        "class Subject {\n"
        ...
        "}\n";
    TSTree *tree = parse_lsp_fixture(ANI_LANG_CSHARP, source);
    ASSERT_NOT_NULL(tree);
    ...
    ani_cs_refine_ast_return_types(&ctx, &registry, root);
...
TEST(repro_lsp_ordered_signatures_rust_ast_return_refinement) {
    const char *module_qn = "ani.ordered.rust.local";
    const char *thing_qn = "ani.ordered.rust.local.Thing";
    const char *result_qn = "ani.ordered.rust.local.Result";
    const char *subject_qn = "ani.ordered.rust.local.Subject";
    const char *top_qn = "ani.ordered.rust.local.top";
    const char *method_qn = "ani.ordered.rust.local.Subject.method";
    const char *source =
        "struct Thing;\n"
        "struct Result;\n"
        "struct Subject;\n"
        ...
        "}\n";
    TSTree *tree = parse_lsp_fixture(ANI_LANG_RUST, source);
    ASSERT_NOT_NULL(tree);
    TSNode root = ts_tree_root_node(tree);
    ASSERT_FALSE(ts_node_has_error(root));

    const char *params[] = {"i32", "Thing", "i32", NULL};
    ANIDefinition defs[5] = {0};
    defs[0].name = "Thing";
    ...
    ani_arena_init(&arena);
    ANITypeRegistry registry;
    ani_rust_build_local_registry(&arena, &registry, &result, module_qn, root, source);
...
TEST(repro_lsp_ordered_signatures_kotlin_local_registry) {
    ...
    TSTree *tree = parse_lsp_fixture(ANI_LANG_KOTLIN, source);
    ASSERT_NOT_NULL(tree);
    TSNode root = ts_tree_root_node(tree);
    ASSERT_FALSE(ts_node_has_error(root));
    ...
    kotlin_lsp_init(&ctx, &arena, source, (int)strlen(source), &registry,
                    "ani.ordered.kotlin.local", "ani.ordered.kotlin.local", "ani", "Local.kt",
                    &out);
    ...
    const char *expected[] = {"Int", "Thing", "Int"};
    int failures = check_signature(&registry, "ani.ordered.kotlin.local.Holder.method",
                                   "kotlin-local-method", expected, 3);
    failures += check_single_return(&registry, "ani.ordered.kotlin.local.Holder.method",
                                    "kotlin-local-method", "Result");
    failures +=
        check_signature(&registry, "ani.ordered.kotlin.local.top", "kotlin-local-top", expected, 3);
    failures += check_single_return(&registry, "ani.ordered.kotlin.local.top", "kotlin-local-top",
                                    "Result");
...
TEST(repro_lsp_ordered_signatures_php_return_refinement) {
    const char *module_qn = "ani.ordered.php.local";
    const char *subject_qn = "ani.ordered.php.local.Subject";
    const char *thing_qn = "ani.ordered.php.local.Thing";
    const char *result_qn = "ani.ordered.php.local.Result";
    const char *source =
        "<?php\n"
        "class Thing {}\n"
        "class Result {}\n"
        ...
        "  }\n"
        "}\n";
    TSTree *tree = parse_lsp_fixture(ANI_LANG_PHP, source);
    ASSERT_NOT_NULL(tree);
    TSNode root = ts_tree_root_node(tree);
    ASSERT_FALSE(ts_node_has_error(root));
    ...
    add_php_seeded_method(&arena, &registry, "ani.ordered.php.local.Subject.declared", "declared",
                          subject_qn, thing_qn, ani_type_unknown());
    add_php_seeded_method(&arena, &registry, "ani.ordered.php.local.Subject.documented",
                          "documented", subject_qn, thing_qn, ani_type_unknown());

    ANIResolvedCallArray out = {0};
    PHPLSPContext ctx;
    php_lsp_init(&ctx, &arena, source, (int)strlen(source), &registry, module_qn, &out);
    ani_php_refine_lsp_registry(&ctx, &registry, root);

    const char *expected[] = {"int", "Thing", "int"};
    int failures = check_signature(&registry, "ani.ordered.php.local.Subject.declared",
                                   "php-declared-return-refinement", expected, 3);
    failures += check_single_return(&registry, "ani.ordered.php.local.Subject.declared",
                                    "php-declared-return-refinement", "Result");
    failures += check_signature(&registry, "ani.ordered.php.local.Subject.documented",
                                "phpdoc-return-refinement", expected, 3);
    failures += check_single_return(&registry, "ani.ordered.php.local.Subject.documented",
...
TEST(repro_lsp_ordered_signatures_php_trait_self_refinement) {
    const char *module_qn = "ani.ordered.php.trait";
    const char *thing_qn = "ani.ordered.php.trait.Thing";
    const char *trait_qn = "ani.ordered.php.trait.Fluent";
    const char *consumer_qn = "ani.ordered.php.trait.Consumer";
    const char *source =
        "<?php\n"
        "class Thing {}\n"
        "trait Fluent {\n"
    ...
    add_php_seeded_method(&arena, &registry, "ani.ordered.php.trait.Fluent.tap", "tap", trait_qn,
                          thing_qn, ani_type_named(&arena, trait_qn));
    ...
    const char *expected[] = {"int", "Thing", "int"};
    int failures = check_signature(&registry, "ani.ordered.php.trait.Consumer.tap",
                                   "php-trait-self-refinement", expected, 3);
    failures += check_single_return(&registry, "ani.ordered.php.trait.Consumer.tap",
