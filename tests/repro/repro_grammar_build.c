 ...
 *
 * Languages covered (15) and the ANI_LANG_* enum each uses (all verified in
 * internal/ani/ani.h; none missing, none skipped):
 ...
 *   Just          -> ANI_LANG_JUST
 *   K8s           -> ANI_LANG_K8S
 *   Kustomize     -> ANI_LANG_KUSTOMIZE
 *   GoMod         -> ANI_LANG_GOMOD
 *   Requirements  -> ANI_LANG_REQUIREMENTS
 *   Gitignore     -> ANI_LANG_GITIGNORE
 *   Gitattributes -> ANI_LANG_GITATTRIBUTES
 *   SSHConfig     -> ANI_LANG_SSHCONFIG
 *   BitBake       -> ANI_LANG_BITBAKE
 *   Puppet        -> ANI_LANG_PUPPET
 *
 * Langs NOT in ANI_LANG_* (skipped, noted):
 ...
 *     K8S            -- semantic extractor (ani_extract_k8s); extracts kind -> "Resource".
 *     KUSTOMIZE      -- semantic extractor (ani_extract_k8s); extracts kind -> "Resource".
 *
...
TEST(repro_grammar_build_dockerfile) {
    static const char src[] =
        ...
        "ARG VERSION=0.8.1\n"
        "COPY . .\n"
        "RUN go build -o /ani-server ./cmd/server\n"
        "\n"
        "FROM debian:bookworm-slim\n"
        "ENV PORT=8080\n"
        "ENV LOG_LEVEL=info\n"
        "COPY --from=builder /ani-server /usr/local/bin/ani-server\n"
        "EXPOSE 8080\n"
        "ENTRYPOINT [\"/usr/local/bin/ani-server\"]\n";
    static const char bad[] = "FROM golang:1.22 AS\n";
    if (build_struct_battery("Dockerfile", src, ANI_LANG_DOCKERFILE,
                             "Dockerfile", "Variable", NULL) != 0)
        return 1;
    return build_robustness("Dockerfile", bad, ANI_LANG_DOCKERFILE, "Dockerfile");
}
...
TEST(repro_grammar_build_makefile) {
    ...
    if (build_callable_battery("Makefile", src, ANI_LANG_MAKEFILE, "Makefile",
                               "Function", "shell") != 0)
        return 1;
    return build_robustness("Makefile", bad, ANI_LANG_MAKEFILE, "Makefile");
}
 ...
 *
 * Dims asserted: 1-6 + R.
 * Dim 5 expected GREEN: "Function" def for the function_def "ani_setup_target".
...
TEST(repro_grammar_build_cmake) {
    static const char src[] =
        ...
        "project(ani VERSION 0.8.1 LANGUAGES C)\n"
        "\n"
        "set(CMAKE_C_STANDARD 11)\n"
        "\n"
        "function(ani_setup_target target)\n"
        "    target_include_directories(${target} PRIVATE include)\n"
        "    target_compile_options(${target} PRIVATE -Wall -Wextra)\n"
        "endfunction()\n"
        "\n"
        "add_executable(ani-server src/main.c src/server.c)\n"
        "ani_setup_target(ani-server)\n"
        "target_link_libraries(ani-server PRIVATE sqlite3)\n";
    static const char bad[] = "cmake_minimum_required(VERSION 3.20\n";
    if (build_callable_battery("CMake", src, ANI_LANG_CMAKE, "CMakeLists.txt",
                               "Function", "add_executable") != 0)
        return 1;
    return build_robustness("CMake", bad, ANI_LANG_CMAKE, "CMakeLists.txt");
}
 ...
 *   "ani_flags". RED would indicate function_expression->Function extraction or
...
TEST(repro_grammar_build_meson) {
     ...
     * `ani_flags = func (target) ... endfunc` user-function syntax is not parsed
     ...
     * a ani bug. Original assertions below are preserved (unreachable). */
    ...
    static const char src[] =
        ...
        "\n"
        "ani_flags = func (target)\n"
        "    return ['-DVERSION=\"' + target + '\"']\n"
        "endfunc\n"
        "\n"
        "sqlite = dependency('sqlite3')\n"
        "executable('ani-server',\n"
        "    sources: ['src/main.c', 'src/server.c'],\n"
        "    dependencies: [sqlite],\n"
        "    install: true)\n";
    static const char bad[] = "project('ani', 'c',\n    version: '0.8.1'";
    if (build_callable_battery("Meson", src, ANI_LANG_MESON, "meson.build",
                               "Function", "executable") != 0)
        return 1;
    return build_robustness("Meson", bad, ANI_LANG_MESON, "meson.build");
}
...
TEST(repro_grammar_build_gn) {
    static const char src[] =
        ...
        "}\n";
    static const char bad[] = "executable(\"ani-server\") {\n  sources = [";
    if (build_callable_battery("GN", src, ANI_LANG_GN, "BUILD.gn",
                               NULL, "executable") != 0)
        return 1;
    return build_robustness("GN", bad, ANI_LANG_GN, "BUILD.gn");
}
...
TEST(repro_grammar_build_just) {
    static const char src[] =
        ...
        "    just test\n";
    static const char bad[] = "build:\n    go build -o ";
    if (build_callable_battery("Just", src, ANI_LANG_JUST, "justfile",
                               "Function", "build") != 0)
        return 1;
    if (build_robustness("Just", bad, ANI_LANG_JUST, "justfile") != 0)
        return 1;
 ...
 * kind: Deployment). The K8s/Kustomize semantic extractor ani_extract_k8s()
 * is called for ANI_LANG_K8S; it reads the kind field from the YAML tree and
 ...
 * Dim 5 expected GREEN: "Resource" def extracted by ani_extract_k8s for the kind.
...
TEST(repro_grammar_build_k8s) {
    ...
    if (build_struct_battery("K8s", src, ANI_LANG_K8S, "deployment.yaml",
                             "Resource", NULL) != 0)
        return 1;
    return build_robustness("K8s", bad, ANI_LANG_K8S, "deployment.yaml");
}

/* ── Kustomize ────────────────────────────────────────────────────────────────
 * Idiomatic kustomization.yaml: the Kustomize overlay tool's root file
 * (kind: Kustomization). ani_extract_k8s() is called for ANI_LANG_KUSTOMIZE
 * just as for ANI_LANG_K8S; it should mint a "Resource" def for the
 * "Kustomization" kind, which is the canonical Kustomize resource kind.
 *
 * Dims asserted: 1-5 + R ("Resource" for the Kustomization kind).
 * Dim 5 expected GREEN: "Resource" def for "Kustomization" from ani_extract_k8s.
...
TEST(repro_grammar_build_kustomize) {
    ...
    if (build_struct_battery("Kustomize", src, ANI_LANG_KUSTOMIZE,
                             "kustomization.yaml", "Resource", NULL) != 0)
        return 1;
    return build_robustness("Kustomize", bad, ANI_LANG_KUSTOMIZE,
                            "kustomization.yaml");
}
...
TEST(repro_grammar_build_gomod) {
    static const char src[] =
        "module github.com/jnani1972/ani\n"
        "\n"
        "go 1.22\n"
        "\n"
        "require (\n"
        ...
        ")\n";
    static const char bad[] = "module github.com/jnani1972/ani\nrequire (";
    if (build_struct_battery("GoMod", src, ANI_LANG_GOMOD, "go.mod",
                             "Variable", NULL) != 0)
        return 1;
    return build_robustness("GoMod", bad, ANI_LANG_GOMOD, "go.mod");
}
...
TEST(repro_grammar_build_requirements) {
    static const char src[] =
        ...
        "\n"
        "# URL requirement\n"
        "ani-client @ git+https://github.com/jnani1972/ani-client.git@v0.8.1\n";
    static const char bad[] = "requests==2.31.0\nbroken>=";
    if (build_base_battery("Requirements", src, ANI_LANG_REQUIREMENTS,
                           "requirements.txt") != 0)
        return 1;
    return build_robustness("Requirements", bad, ANI_LANG_REQUIREMENTS,
                            "requirements.txt");
}
...
TEST(repro_grammar_build_gitignore) {
    static const char src[] =
        ...
        "coverage.out\n"
        "*.prof\n";
    static const char bad[] = "ani-server\n[invalid";
    if (build_base_battery("Gitignore", src, ANI_LANG_GITIGNORE, ".gitignore") != 0)
        return 1;
    return build_robustness("Gitignore", bad, ANI_LANG_GITIGNORE, ".gitignore");
}
...
TEST(repro_grammar_build_gitattributes) {
    static const char src[] =
        ...
        "internal/ani/vendored/** linguist-vendored\n";
    static const char bad[] = "* text=auto eol=lf\n*.go [broken";
    if (build_base_battery("Gitattributes", src, ANI_LANG_GITATTRIBUTES,
                           ".gitattributes") != 0)
        return 1;
    return build_robustness("Gitattributes", bad, ANI_LANG_GITATTRIBUTES,
                            ".gitattributes");
}
...
TEST(repro_grammar_build_sshconfig) {
    ...
    if (build_base_battery("SSHConfig", src, ANI_LANG_SSHCONFIG, "config") != 0)
        return 1;
    return build_robustness("SSHConfig", bad, ANI_LANG_SSHCONFIG, "config");
}
...
TEST(repro_grammar_build_bitbake) {
    ...
    return -1; /* skip — not counted as pass or fail */
    static const char src[] =
        "DESCRIPTION = \"ANI MCP server component\"\n"
        "HOMEPAGE    = \"https://github.com/jnani1972/ani\"\n"
        "LICENSE     = \"MIT\"\n"
        "PV          = \"0.8.1\"\n"
        "\n"
        "SRC_URI = \"git://github.com/jnani1972/ani.git;protocol=https\"\n"
        "\n"
        "do_fetch() {\n"
        "    git clone ${SRC_URI} ${S}\n"
        "}\n"
        ...
        "\n"
        "do_compile() {\n"
        "    go build -o ${B}/ani-server ./cmd/server\n"
        "}\n"
        "\n"
        "do_install() {\n"
        "    install -d ${D}${bindir}\n"
        "    install -m 0755 ${B}/ani-server ${D}${bindir}/\n"
        "}\n";
    static const char bad[] = "DESCRIPTION = \"ANI\"\ndo_fetch() {\n    git clone ";
    if (build_callable_battery("BitBake", src, ANI_LANG_BITBAKE,
                               "ani-server_0.8.1.bb", "Function", "subprocess") != 0)
        return 1;
    if (build_robustness("BitBake", bad, ANI_LANG_BITBAKE,
                         "ani-server_0.8.1.bb") != 0)
        return 1;
    return build_pipeline_battery("BitBake", "ani-server_0.8.1.bb", src);
}
 ...
 *
 * Dims asserted: 1-8 (full battery).
 * Dim 5 expected GREEN: "Function" def for the function_declaration "ani_validate"
 *   AND "Class" def for the class_definition "ani". RED for either label
 ...
 * Dim 7 expected GREEN for calls inside function_declaration "ani_validate"
...
TEST(repro_grammar_build_puppet) {
    ...
    static const char src[] =
        ...
        "}\n"
        "\n"
        "define ani::port_config (\n"
        "  Integer $port,\n"
        ") {\n"
        "  file { '/etc/ani/port.conf':\n"
        "    content => \"port=${port}\\n\",\n"
        "  }\n"
        "}\n"
        "\n"
        "function ani_validate(String $version) >> Boolean {\n"
        "  $parts = split($version, /\\./ )\n"
        "  length($parts) == 3\n"
        "}\n";
    static const char bad[] = "class ani (\n  String $version = '0.8.1',\n) {\n  include";
    if (build_callable_battery("Puppet", src, ANI_LANG_PUPPET, "ani.pp",
                               "Function", "include") != 0)
        return 1;
    if (build_robustness("Puppet", bad, ANI_LANG_PUPPET, "ani.pp") != 0)
        return 1;
    return build_pipeline_battery("Puppet", "ani.pp", src);
}
