 ...
 * resolution — internal/ani/lsp/ts_lsp.c resolve_member_call/lookup_method
...
static int node_with_qn_suffix(ani_store_t *store, const char *project,
                               const char *label, const char *suffix) {
    ani_node_t *nodes = NULL;
    int count = 0;
    if (ani_store_find_nodes_by_label(store, project, label, &nodes, &count) !=
        ANI_STORE_OK)
        return 0;
    int found = 0;
 ...
 */
static int lsp_resolved_edge_exists(ani_store_t *store, const char *project,
                                    const char *caller_substr,
                                    const char *callee_suffix,
                                    const char *strategy_substr, int dump) {
    ani_edge_t *edges = NULL;
    int n = 0;
    if (ani_store_find_edges_by_type(store, project, "CALLS", &edges, &n) !=
        ANI_STORE_OK)
        return 0;
    int found = 0;
...
TEST(repro_ts_inherited_extraction_preconditions) {
    ...
    ASSERT_TRUE(inv_extract_clean(kTsInherited[0].content, ANI_LANG_TYPESCRIPT,
                                  "base.ts"));
    ANIFileResult *rb =
        inv_rx(kTsInherited[0].content, ANI_LANG_TYPESCRIPT, "base.ts");
    ASSERT_NOT_NULL(rb);
    int greet_methods = 0;
    for (int i = 0; i < rb->defs.count; i++) {
        ANIDefinition *d = &rb->defs.items[i];
    ...
    ASSERT_TRUE(inv_extract_clean(kTsInherited[1].content, ANI_LANG_TYPESCRIPT,
                                  "derived.ts"));
    ANIFileResult *rd =
        inv_rx(kTsInherited[1].content, ANI_LANG_TYPESCRIPT, "derived.ts");
    ASSERT_NOT_NULL(rd);
...
static int run_ts_inherited_pipeline(void) {
    RProj lp;
    ani_store_t *store = rh_index_files(
