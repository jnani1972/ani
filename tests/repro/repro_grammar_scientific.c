 ...
 *
 * Languages covered (15) and the ANI_LANG_* enum each uses (all verified present
 * in internal/ani/ani.h -- none missing, none skipped):
 *   GLSL     -> ANI_LANG_GLSL      (shader; reuses C node types)
 *   HLSL     -> ANI_LANG_HLSL      (shader; C++-family node types)
 *   WGSL     -> ANI_LANG_WGSL      (shader; own grammar)
 *   ISPC     -> ANI_LANG_ISPC      (shader/SIMD; C-family node types)
 *   Slang    -> ANI_LANG_SLANG     (shader; C++-family node types)
 *   Cairo    -> ANI_LANG_CAIRO     (smart-contract; Rust-like)
 *   Sway     -> ANI_LANG_SWAY      (smart-contract; Rust-like)
 *   FunC     -> ANI_LANG_FUNC      (smart-contract; TON)
 *   Wolfram  -> ANI_LANG_WOLFRAM   (CAS; assignment-as-definition)
 *   MATLAB   -> ANI_LANG_MATLAB    (numeric)
 *   Magma    -> ANI_LANG_MAGMA     (CAS)
 *   FORM     -> ANI_LANG_FORM      (symbolic; procedure_definition / call_statement)
 *   TLA+     -> ANI_LANG_TLAPLUS   (formal spec; operator_definition)
 *   Agda     -> ANI_LANG_AGDA      (dependently-typed)
 *   Apex     -> ANI_LANG_APEX      (Salesforce; Java-like, methods only)
 *
 ...
 * via the same ani_enclosing_func_qn -> Module fallback documented in
...
static int robustness_probe(const char *lang_tag, const char *bad_src,
    ...
    const char *RED = tf_red();
    const char *RST = tf_reset();
    ANIFileResult *r = inv_rx(bad_src, lang, file);
    if (!r) {
...
TEST(repro_grammar_scientific_glsl) {
    static const char src[] =
        ...
        "    gl_FragColor = vec4(v);\n"
        "}\n";
    if (single_file_battery("GLSL", src, ANI_LANG_GLSL, "shader.frag",
                            "Function", NULL, "scale") != 0)
        return 1;
    if (robustness_probe("GLSL", "void main() { float v = scale(",
                         ANI_LANG_GLSL, "shader.frag") != 0)
        return 1;
...
TEST(repro_grammar_scientific_hlsl) {
    static const char src[] =
        ...
        "}\n";
    if (single_file_battery("HLSL", src, ANI_LANG_HLSL, "shader.hlsl",
                            "Function", NULL, "scale") != 0)
        return 1;
    if (robustness_probe("HLSL", "float4 PSMain( { return scale(",
                         ANI_LANG_HLSL, "shader.hlsl") != 0)
        return 1;
...
TEST(repro_grammar_scientific_wgsl) {
    static const char src[] =
        ...
        "}\n";
    if (single_file_battery("WGSL", src, ANI_LANG_WGSL, "shader.wgsl",
                            "Function", NULL, "scale") != 0)
        return 1;
    if (robustness_probe("WGSL", "fn fs_main() -> { let v = scale(",
                         ANI_LANG_WGSL, "shader.wgsl") != 0)
        return 1;
...
TEST(repro_grammar_scientific_ispc) {
    static const char src[] =
        ...
        "    }\n"
        "}\n";
    if (single_file_battery("ISPC", src, ANI_LANG_ISPC, "kernel.ispc",
                            "Function", NULL, "scale") != 0)
        return 1;
    if (robustness_probe("ISPC", "export void run( { out[0] = scale(",
                         ANI_LANG_ISPC, "kernel.ispc") != 0)
        return 1;
...
TEST(repro_grammar_scientific_slang) {
    static const char src[] =
        ...
        "    outBuf[tid.x] = v;\n"
        "}\n";
    if (single_file_battery("Slang", src, ANI_LANG_SLANG, "shader.slang",
                            "Function", NULL, "scale") != 0)
        return 1;
    if (robustness_probe("Slang", "void csMain( { float v = scale(",
                         ANI_LANG_SLANG, "shader.slang") != 0)
        return 1;
...
TEST(repro_grammar_scientific_cairo) {
    static const char src[] =
        ...
        "    add(x, 1)\n"
        "}\n";
    if (single_file_battery("Cairo", src, ANI_LANG_CAIRO, "lib.cairo",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("Cairo", "fn compute(x: felt252) -> { add(",
                         ANI_LANG_CAIRO, "lib.cairo") != 0)
        return 1;
...
TEST(repro_grammar_scientific_sway) {
    static const char src[] =
        ...
        "    add(x, 1)\n"
        "}\n";
    if (single_file_battery("Sway", src, ANI_LANG_SWAY, "main.sw",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("Sway", "fn compute(x: u64) -> { add(",
                         ANI_LANG_SWAY, "main.sw") != 0)
        return 1;
...
TEST(repro_grammar_scientific_func) {
    static const char src[] =
        ...
        "    return add(x, 1);\n"
        "}\n";
    if (single_file_battery("FunC", src, ANI_LANG_FUNC, "contract.fc",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("FunC", "int compute(int x) { return add(",
                         ANI_LANG_FUNC, "contract.fc") != 0)
        return 1;
...
TEST(repro_grammar_scientific_wolfram) {
    static const char src[] =
        ...
        "\n"
        "compute[x_] := add[x, 1]\n";
    if (single_file_battery("Wolfram", src, ANI_LANG_WOLFRAM, "calc.wl",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("Wolfram", "compute[x_] := add[x,",
                         ANI_LANG_WOLFRAM, "calc.wl") != 0)
        return 1;
...
TEST(repro_grammar_scientific_matlab) {
    static const char src[] =
        ...
        "    s = a + b;\n"
        "end\n";
    if (single_file_battery("MATLAB", src, ANI_LANG_MATLAB, "calc.m",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("MATLAB", "function r = compute(x)\n  r = add(",
                         ANI_LANG_MATLAB, "calc.m") != 0)
        return 1;
...
TEST(repro_grammar_scientific_magma) {
    static const char src[] =
        ...
        "    return Add(x, 1);\n"
        "end function;\n";
    if (single_file_battery("Magma", src, ANI_LANG_MAGMA, "calc.magma",
                            "Function", NULL, "Add") != 0)
        return 1;
    if (robustness_probe("Magma", "function Compute(x)\n  return Add(",
                         ANI_LANG_MAGMA, "calc.magma") != 0)
        return 1;
...
TEST(repro_grammar_scientific_form) {
    static const char src[] =
        ...
        "    #call add(`y')\n"
        "#endprocedure\n";
    if (single_file_battery("FORM", src, ANI_LANG_FORM, "calc.frm",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("FORM", "#procedure compute(y)\n  #call add(",
                         ANI_LANG_FORM, "calc.frm") != 0)
        return 1;
...
TEST(repro_grammar_scientific_tlaplus) {
    static const char src[] =
        ...
        "Compute(x) == Add(x, 1)\n"
        "====\n";
    if (single_file_battery("TLA+", src, ANI_LANG_TLAPLUS, "Calc.tla",
                            "Function", NULL, "Add") != 0)
        return 1;
    if (robustness_probe("TLA+", "---- MODULE Calc ----\nCompute(x) == Add(",
                         ANI_LANG_TLAPLUS, "Calc.tla") != 0)
        return 1;
...
TEST(repro_grammar_scientific_agda) {
    static const char src[] =
        ...
        "compute : Nat -> Nat\n"
        "compute x = add x 1\n";
    if (single_file_battery("Agda", src, ANI_LANG_AGDA, "Calc.agda",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("Agda", "module Calc where\ncompute x = add x",
                         ANI_LANG_AGDA, "Calc.agda") != 0)
        return 1;
...
TEST(repro_grammar_scientific_apex) {
    static const char src[] =
        ...
        "    }\n"
        "}\n";
    if (single_file_battery("Apex", src, ANI_LANG_APEX, "Calculator.cls",
                            "Method", "Class", "add") != 0)
        return 1;
    if (robustness_probe("Apex", "public class Calculator { Integer compute() { return add(",
                         ANI_LANG_APEX, "Calculator.cls") != 0)
        return 1;
