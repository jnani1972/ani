...
//   https://github.com/jnani1972/ani/issues/581
//
// OBSERVED BEHAVIOUR:
//   ani in stdio MCP server mode grows from ~12 MB working
...
//
//   2. mimalloc page retention: ani_mem_collect() is called after
...
//
//   (a) RSS MEASUREMENT: we use ani_mem_rss() (src/foundation/mem.c) which
...
//
//   (d) LINUX-ONLY ALTERNATIVE: if ani_mem_rss() returns 0 (e.g. MI_OVERRIDE=0
//       without the OS fallback compiled), the test falls back to reading
//       /proc/self/statm directly below.  On macOS and Windows ani_mem_rss()
...
//   1. src/mcp/mcp.c, ani_mcp_server_run event loop (or after each tool call
//      in ani_mcp_handle_tool): periodically call
//        sqlite3_wal_checkpoint_v2(..., SQLITE_CHECKPOINT_TRUNCATE, ...)
//      and ani_mem_collect() after query bursts (e.g. every N=50 calls or
//      after exceeding a RSS threshold via ani_mem_over_budget()).
//   2. src/mcp/mcp.c, ani_mcp_server_evict_idle: on idle eviction, call
//      ani_mem_collect() so mimalloc returns pages to the OS, matching the
...
//
// GREEN after fix:
//   ani_mem_collect() and/or TRUNCATE checkpoint called periodically by the
...
TEST(repro_issue581_query_rss_stable) {
    ...
    for (int i = 0; i < ITER_TOTAL; i++) {
        char *resp = ani_mcp_handle_tool(lp.srv, "search_graph", args);
    ...
    //   (b) the fix — periodic SQLITE_CHECKPOINT_TRUNCATE + ani_mem_collect() in
