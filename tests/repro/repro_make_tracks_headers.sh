...
MAKE_DB="$(mktemp "${TMPDIR:-/tmp}/ani-repro-make-db.XXXXXX")"
trap 'rm -f "$MAKE_DB"' EXIT

cd "$PROJECT_ROOT"
make -f Makefile.ani -pn build/c/test-repro-runner >"$MAKE_DB"
...
production_target_line="$(grep '^build/c/ani:' "$MAKE_DB" | head -1 || true)"
...
for target_name in build/c/test-runner build/c/test-repro-runner build/c/test-runner-tsan \
    ...
    for header in src/pipeline/lsp_resolve.h internal/ani/lsp/type_registry.h; do
        if [[ "$target_line" != *"$header"* ]]; then
