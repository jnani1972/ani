 ...
 *
 * These tests inspect raw ANIUsage rows rather than graph edges.  A local
...
static LBBindingSite lb_binding_site(const ANIFileResult *result, const char *caller_suffix,
    ...
    for (int i = 0; result && i < result->usages.count; i++) {
        ...
        match.count++;
        match.value_count += usage->kind == ANI_USAGE_VALUE;
...
static ANIFileResult *lb_extract(const char *source, ANILanguage language, const char *filename) {
    return ani_extract_file(source, (int)strlen(source), language, "repro", filename, 0, NULL,
                            NULL);
}
...
TEST(repro_python_keyword_label_does_not_bind_local) {
    ...
    ANIFileResult *result = lb_extract(source, ANI_LANG_PYTHON, "keyword_label.py");
...
TEST(repro_python_function_name_does_not_shadow_itself) {
    ...
    ANIFileResult *result = lb_extract(source, ANI_LANG_PYTHON, "self_reference.py");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
...
TEST(repro_python_nested_function_binds_in_enclosing_function) {
    ...
    ANIFileResult *result = lb_extract(source, ANI_LANG_PYTHON, "nested_definition.py");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
...
TEST(repro_python_assignment_blocks_earlier_use_for_whole_function) {
    ...
    ANIFileResult *result = lb_extract(source, ANI_LANG_PYTHON, "whole_function_local.py");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
...
TEST(repro_javascript_block_local_stops_at_block_exit) {
    ...
    ANIFileResult *result = lb_extract(source, ANI_LANG_JAVASCRIPT, "block_scope.js");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
...
TEST(repro_csharp_overload_bodies_do_not_share_parameter_bindings) {
    ...
    ANIFileResult *result = lb_extract(source, ANI_LANG_CSHARP, "Overloads.cs");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
...
TEST(repro_powershell_parameter_shadow_is_case_insensitive) {
    ...
    ANIFileResult *result = lb_extract(source, ANI_LANG_POWERSHELL, "case_scope.ps1");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
...
TEST(repro_python_local_class_binding_stops_at_class_namespace) {
    ...
    uint32_t site = lb_identifier_offset(source, "accept(handler)", "handler");
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_PYTHON, "local_class.py");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    LBBindingSite usage = lb_binding_site(result, "outer", "handler", site);
    ani_free_result(result);
...
TEST(repro_python_method_lookup_bypasses_class_namespace) {
    ...
    ANIFileResult *result = lb_extract(source, ANI_LANG_PYTHON, "method_class_scope.py");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
...
TEST(repro_python_method_default_uses_class_namespace) {
    ...
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_PYTHON, "method_default_class_scope.py");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    LBBindingSite usage = lb_binding_site(result, "method", "handler", site);
    ani_free_result(result);
...
TEST(repro_python_local_import_alias_binds_whole_function) {
    ...
    ASSERT_NEQ(before_site, UINT32_MAX);
    ASSERT_NEQ(after_site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_PYTHON, "local_import_alias.py");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    LBBindingSite before = lb_binding_site(result, "caller", "handler", before_site);
    LBBindingSite after = lb_binding_site(result, "caller", "handler", after_site);
...
TEST(repro_python_local_from_import_alias_binds_whole_function) {
    ...
    uint32_t site = lb_identifier_offset(source, "accept(handler)", "handler");
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_PYTHON, "local_from_import_alias.py");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    LBBindingSite usage = lb_binding_site(result, "caller", "handler", site);
    ani_free_result(result);
...
TEST(repro_python_import_source_roles_do_not_bind) {
    ...
    uint32_t site = lb_identifier_offset(source, "accept(handler)", "handler");
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_PYTHON, "import_source_roles.py");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    LBBindingSite usage = lb_binding_site(result, "caller", "handler", site);
    ani_free_result(result);
...
TEST(repro_python_future_import_is_not_binding_or_usage) {
    ...
    ANIFileResult *result = lb_extract(source, ANI_LANG_PYTHON, "future_import.py");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
...
/* ANIImport metadata is consumed independently of lexical blockers. Preserve
...
TEST(repro_python_dotted_import_records_root_local_name) {
    ...
    ANIFileResult *result = lb_extract(source, ANI_LANG_PYTHON, "dotted_import.py");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
...
TEST(repro_typescript_import_alias_blocks_raw_callable_fallback) {
    ...
    uint32_t site = lb_identifier_offset(source, "accept(handler)", "handler");
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_TYPESCRIPT, "import_alias.ts");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    LBBindingSite usage = lb_binding_site(result, "caller", "handler", site);
    ani_free_result(result);
...
TEST(repro_typescript_import_source_name_does_not_bind) {
    ...
    uint32_t site = lb_identifier_offset(source, "accept(handler)", "handler");
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_TYPESCRIPT, "import_source_role.ts");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    LBBindingSite usage = lb_binding_site(result, "caller", "handler", site);
    ani_free_result(result);
...
TEST(repro_typescript_import_require_clause_binds_local) {
    ...
    uint32_t site = lb_identifier_offset(source, "accept(handler)", "handler");
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_TYPESCRIPT, "import_require.ts");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    LBBindingSite usage = lb_binding_site(result, "caller", "handler", site);
    ani_free_result(result);
...
TEST(repro_javascript_deep_block_nesting_keeps_lexical_binding) {
    ...
    uint32_t site = lb_identifier_offset(source, "accept(handler)", "handler");
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_JAVASCRIPT, "deep_blocks.js");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    LBBindingSite usage = lb_binding_site(result, "caller", "handler", site);
    ani_free_result(result);
...
TEST(repro_typescript_deep_block_nesting_keeps_lexical_binding) {
    ...
    uint32_t site = lb_identifier_offset(source, "accept(handler)", "handler");
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_TYPESCRIPT, "deep_blocks.ts");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    LBBindingSite usage = lb_binding_site(result, "caller", "handler", site);
    ani_free_result(result);
...
TEST(repro_rust_block_use_alias_binds_local_scope) {
    ...
    uint32_t site = lb_identifier_offset(source, "accept(handler)", "handler");
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_RUST, "block_use_alias.rs");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    LBBindingSite usage = lb_binding_site(result, "caller", "handler", site);
    ani_free_result(result);
...
TEST(repro_rust_use_source_name_does_not_bind) {
    ...
    uint32_t site = lb_identifier_offset(source, "accept(handler)", "handler");
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_RUST, "use_source_role.rs");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    LBBindingSite usage = lb_binding_site(result, "caller", "handler", site);
    ani_free_result(result);
...
TEST(repro_rust_extern_crate_alias_binds_only_alias) {
    ...
    ANIFileResult *result = lb_extract(source, ANI_LANG_RUST, "extern_alias.rs");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
...
TEST(repro_rust_brace_self_binds_prefix) {
    ...
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_RUST, "brace_self.rs");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
...
TEST(repro_rust_use_item_binds_before_declaration) {
    ...
    uint32_t site = lb_identifier_offset(source, "accept(handler)", "handler");
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_RUST, "use_before_declaration.rs");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    LBBindingSite usage = lb_binding_site(result, "caller", "handler", site);
    ani_free_result(result);
...
TEST(repro_rust_inline_module_import_does_not_leak) {
    ...
    ASSERT_NEQ(inside_site, UINT32_MAX);
    ASSERT_NEQ(outside_site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_RUST, "inline_module.rs");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
...
TEST(repro_objectscript_set_target_binds_routine_local) {
    ...
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_OBJECTSCRIPT_ROUTINE, "Binding.mac");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
...
TEST(repro_objectscript_udl_set_target_binds_method_local) {
    ...
    uint32_t site = lb_identifier_offset(source, "..Accept(watched)", "watched");
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_OBJECTSCRIPT_UDL, "Binding.cls");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    LBBindingSite usage = lb_binding_site(result, "Caller", "watched", site);
    ani_free_result(result);
...
TEST(repro_python_default_expression_uses_enclosing_namespace) {
    ...
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_PYTHON, "default_scope.py");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
...
TEST(repro_python_global_assignment_is_module_binding) {
    ...
    uint32_t site = lb_identifier_offset(source, "accept(handler)", "handler");
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_PYTHON, "global_scope.py");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    LBBindingSite usage = lb_binding_site(result, "caller", "handler", site);
    ani_free_result(result);
...
TEST(repro_python_module_assignment_blocks_callable_fallback) {
    ...
    uint32_t site = lb_identifier_offset(source, "accept(handler)", "handler");
    ASSERT_NEQ(site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_PYTHON, "module_binding.py");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    LBBindingSite usage = lb_binding_site(result, "caller", "handler", site);
    ani_free_result(result);
...
TEST(repro_go_function_literal_parameter_does_not_leak) {
    ...
    ASSERT_NEQ(inside_site, UINT32_MAX);
    ASSERT_NEQ(outside_site, UINT32_MAX);
    ANIFileResult *result = lb_extract(source, ANI_LANG_GO, "literal.go");
    ASSERT_NOT_NULL(result);
    ASSERT_FALSE(result->has_error || result->parse_incomplete);
    LBBindingSite inside = lb_binding_site(result, "caller", "handler", inside_site);
    LBBindingSite outside = lb_binding_site(result, "caller", "handler", outside_site);
