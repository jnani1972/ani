...
/* CONTRACT PIN. `ani_label_is_type_like()` is documented in ani.h as the single
...
TEST(sql_label_allowlists_match_ani_label_is_type_like) {
    ...
    for (size_t i = 0; i < sizeof(type_like) / sizeof(type_like[0]); i++) {
        ASSERT_TRUE(ani_label_is_type_like(type_like[i]));
        char quoted[64];
        snprintf(quoted, sizeof(quoted), "'%s'", type_like[i]);
        ASSERT_NOT_NULL(strstr(ANI_SQL_TYPE_LIKE_LABELS, quoted));
        ASSERT_NOT_NULL(strstr(ANI_SQL_CALLABLE_OR_TYPE_LABELS, quoted));
    }
    ...
    for (size_t i = 0; i < sizeof(not_type_like) / sizeof(not_type_like[0]); i++) {
        ASSERT_FALSE(ani_label_is_type_like(not_type_like[i]));
        char quoted[64];
        snprintf(quoted, sizeof(quoted), "'%s'", not_type_like[i]);
        ASSERT_NULL(strstr(ANI_SQL_TYPE_LIKE_LABELS, quoted));
    }
    /* The callable fragment carries exactly Function and Method on top. */
    ASSERT_NOT_NULL(strstr(ANI_SQL_CALLABLE_OR_TYPE_LABELS, "'Function'"));
    ASSERT_NOT_NULL(strstr(ANI_SQL_CALLABLE_OR_TYPE_LABELS, "'Method'"));
    PASS();
}
 ...
 * ani_registry_resolve vetoes them, so a code identifier sharing a table's
 * name never binds into the lineage layer. */
TEST(sql_relation_labels_match_ani_label_is_relation) {
    static const char *const relations[] = {"Table", "View", "Model"};
    for (size_t i = 0; i < sizeof(relations) / sizeof(relations[0]); i++) {
        ASSERT_TRUE(ani_label_is_relation(relations[i]));
        ASSERT_TRUE(ani_label_is_registry_symbol(relations[i]));
        ASSERT_FALSE(ani_label_is_type_like(relations[i]));
        char quoted[64];
        snprintf(quoted, sizeof(quoted), "'%s'", relations[i]);
        ASSERT_NOT_NULL(strstr(ANI_SQL_RELATION_LABELS, quoted));
        /* Relations must NOT ride in the callable/type fragments — the arch
         * queries opt in explicitly by appending ANI_SQL_RELATION_LABELS. */
        ASSERT_NULL(strstr(ANI_SQL_CALLABLE_OR_TYPE_LABELS, quoted));
    }
    /* ani_label_is_registry_symbol covers exactly the seeded families. */
    ASSERT_TRUE(ani_label_is_registry_symbol("Function"));
    ASSERT_TRUE(ani_label_is_registry_symbol("Method"));
    ASSERT_TRUE(ani_label_is_registry_symbol("Class"));
    ASSERT_TRUE(ani_label_is_registry_symbol("Variable"));
    ASSERT_TRUE(ani_label_is_registry_symbol("Field"));
    ASSERT_FALSE(ani_label_is_registry_symbol("Module"));
    ASSERT_FALSE(ani_label_is_registry_symbol("File"));
    ASSERT_FALSE(ani_label_is_relation("Class"));
    ASSERT_FALSE(ani_label_is_relation(NULL));
    PASS();
}
...
TEST(store_close_null) {
    ani_store_close(NULL); /* should not crash */
    PASS();
}

TEST(store_open_memory_twice) {
    ani_store_t *s1 = ani_store_open_memory();
    ani_store_t *s2 = ani_store_open_memory();
    ASSERT_NOT_NULL(s1);
    ASSERT_NOT_NULL(s2);
...
TEST(store_project_crud) {
    ...
    /* Create */
    int rc = ani_store_upsert_project(s, "myproject", "/home/user/myproject");
    ASSERT_EQ(rc, ANI_STORE_OK);

    /* Get */
    ani_project_t p = {0};
    rc = ani_store_get_project(s, "myproject", &p);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_STR_EQ(p.name, "myproject");
    ASSERT_STR_EQ(p.root_path, "/home/user/myproject");
    ASSERT_NOT_NULL(p.indexed_at);
    ani_project_free_fields(&p);

    /* List */
    ani_project_t *projects = NULL;
    int count = 0;
    rc = ani_store_list_projects(s, &projects, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 1);
    ASSERT_STR_EQ(projects[0].name, "myproject");
    ani_store_free_projects(projects, count);

    /* Get non-existent */
    ani_project_t p2 = {0};
    rc = ani_store_get_project(s, "nonexistent", &p2);
    ASSERT_EQ(rc, ANI_STORE_NOT_FOUND);
...
TEST(store_project_update) {
    ani_store_t *s = ani_store_open_memory();
    ani_store_upsert_project(s, "test", "/old/path");

    /* Update root path */
    ani_store_upsert_project(s, "test", "/new/path");

    ani_project_t p = {0};
    ani_store_get_project(s, "test", &p);
    ...
    ani_project_t *projects = NULL;
    int count = 0;
    ani_store_list_projects(s, &projects, &count);
    ASSERT_EQ(count, 1);
    ani_store_free_projects(projects, count);
...
TEST(store_project_delete) {
    ...
    int rc = ani_store_delete_project(s, "test");
    ASSERT_EQ(rc, ANI_STORE_OK);

    ani_project_t p = {0};
    rc = ani_store_get_project(s, "test", &p);
    ASSERT_EQ(rc, ANI_STORE_NOT_FOUND);
...
TEST(store_node_crud) {
    ...
    /* Find by QN */
    ani_node_t found = {0};
    int rc = ani_store_find_node_by_qn(s, "test", "test.main.Foo", &found);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ...
    ani_node_free_fields(&found);

    /* Find by ID */
    ani_node_t found2 = {0};
    rc = ani_store_find_node_by_id(s, id, &found2);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_STR_EQ(found2.qualified_name, "test.main.Foo");
    ani_node_free_fields(&found2);

    /* Find by name */
    ani_node_t *nodes = NULL;
    int count = 0;
    rc = ani_store_find_nodes_by_name(s, "test", "Foo", &nodes, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 1);
    ...
    /* Count */
    int cnt = ani_store_count_nodes(s, "test");
    ASSERT_EQ(cnt, 1);
...
TEST(store_node_dedup) {
    ...
    ani_node_t found = {0};
    ani_store_find_node_by_qn(s, "test", "test.main.Foo", &found);
    ASSERT_NOT_NULL(found.properties_json);
...
TEST(store_node_find_by_label) {
    ...
    ani_node_t n2 = {.project = "test", .label = "Class", .name = "B", .qualified_name = "test.B"};
    ani_node_t n3 = {
    ...
    ani_node_t *nodes = NULL;
    int count = 0;
    int rc = ani_store_find_nodes_by_label(s, "test", "Function", &nodes, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 2);
    ani_store_free_nodes(nodes, count);

    rc = ani_store_find_nodes_by_label(s, "test", "Class", &nodes, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 1);
...
TEST(store_node_find_by_file) {
    ...
    ani_node_t *nodes = NULL;
    int count = 0;
    int rc = ani_store_find_nodes_by_file(s, "test", "main.go", &nodes, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 2);
    ani_store_free_nodes(nodes, count);
...
TEST(store_file_outline_is_filtered_stable_bounded_and_cancellable_issue469) {
    ani_store_t *s = ani_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(ani_store_upsert_project(s, "outline", "/tmp/outline"), ANI_STORE_OK);
    ...
    for (size_t i = 0; i < sizeof(nodes) / sizeof(nodes[0]); i++) {
        ASSERT_GT(ani_store_upsert_node(s, &nodes[i]), 0);
    }
    ...
    int count = 0;
    int total = 0;
    ASSERT_EQ(ani_store_get_file_outline(s, "outline", "main.c", labels, 2, 2, 0, NULL, NULL, &rows,
                                         &count, &total),
              ANI_STORE_OK);
    ASSERT_EQ(total, 3);
    ASSERT_EQ(count, 2);
    ...
    ani_store_free_file_outline(rows, count);

    rows = NULL;
    count = 0;
    total = 0;
    ASSERT_EQ(ani_store_get_file_outline(s, "outline", "main.c", labels, 2, 2, 2, NULL, NULL, &rows,
                                         &count, &total),
              ANI_STORE_OK);
    ASSERT_EQ(total, 3);
    ASSERT_EQ(count, 1);
    ...
    count = 0;
    total = 0;
    ASSERT_EQ(ani_store_get_file_outline(s, "outline", "main.c", NULL, 0, 10, 0, NULL, NULL, &rows,
                                         &count, &total),
              ANI_STORE_OK);
    ASSERT_EQ(total, 4);
    ASSERT_EQ(count, 4);
    ...
    rows = (ani_file_outline_row_t *)(uintptr_t)1;
    count = 99;
    total = 99;
    ASSERT_EQ(ani_store_get_file_outline(s, "outline", "main.c", labels, 2, 2, 0,
    ...
    ASSERT_EQ(ani_store_get_file_outline(s, "outline", "main.c", empty_label, 1, 1, 0, NULL, NULL,
                                         &rows, &count, &total),
              ANI_STORE_ERR);
    ASSERT_EQ(ani_store_get_file_outline(s, "outline", "main.c", NULL, 0,
                                         ANI_STORE_FILE_OUTLINE_MAX_LIMIT + 1, 0, NULL, NULL, &rows,
                                         &count, &total),
              ANI_STORE_ERR);
...
TEST(store_file_outline_fails_closed_on_text_budget_issue469) {
    ani_store_t *s = ani_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(ani_store_upsert_project(s, "outline-budget", "/tmp/outline-budget"), ANI_STORE_OK);

    size_t qn_size = ANI_STORE_FILE_OUTLINE_MAX_TEXT_BYTES + 64U;
    char *large_qn = malloc(qn_size + 1U);
    ASSERT_NOT_NULL(large_qn);
    memset(large_qn, 'q', qn_size);
    large_qn[qn_size] = '\0';
    ani_node_t node = {.project = "outline-budget",
                       .label = "Function",
                       .name = "oversized",
                       ...
                       .start_line = 1,
                       .end_line = 2};
    ASSERT_GT(ani_store_upsert_node(s, &node), 0);
    free(large_qn);

    ani_file_outline_row_t *rows = (ani_file_outline_row_t *)(uintptr_t)1;
    int count = 99;
    int total = 99;
    ASSERT_EQ(ani_store_get_file_outline(s, "outline-budget", "large.c", NULL, 0, 1, 0, NULL, NULL,
                                         &rows, &count, &total),
              ANI_STORE_SCAN_LIMIT);
    ASSERT_NULL(rows);
    ASSERT_EQ(count, 0);
    ASSERT_EQ(total, 0);
...
TEST(store_node_find_not_found) {
    ...
    ani_node_t found = {0};
    int rc = ani_store_find_node_by_qn(s, "test", "nonexistent", &found);
    ASSERT_EQ(rc, ANI_STORE_NOT_FOUND);

    rc = ani_store_find_node_by_id(s, 99999, &found);
    ASSERT_EQ(rc, ANI_STORE_NOT_FOUND);
...
TEST(store_node_delete_by_file) {
    ...
    ani_store_delete_nodes_by_file(s, "test", "main.go");
    int cnt = ani_store_count_nodes(s, "test");
    ASSERT_EQ(cnt, 1);
...
TEST(store_node_delete_by_label) {
    ...
    ani_store_delete_nodes_by_label(s, "test", "Function");
    int cnt = ani_store_count_nodes(s, "test");
    ASSERT_EQ(cnt, 1);
...
TEST(store_node_batch_upsert) {
    ...
    int rc = ani_store_upsert_node_batch(s, nodes, 150, ids);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ...
    int64_t ids2[150];
    rc = ani_store_upsert_node_batch(s, nodes, 150, ids2);
    ASSERT_EQ(rc, ANI_STORE_OK);
    cnt = ani_store_count_nodes(s, "test");
    ASSERT_EQ(cnt, 150);
...
TEST(store_node_batch_empty) {
    ani_store_t *s = ani_store_open_memory();
    int rc = ani_store_upsert_node_batch(s, NULL, 0, NULL);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ani_store_close(s);
    PASS();
}
...
TEST(store_cascade_delete) {
    ...
    int64_t id1 = ani_store_upsert_node(s, &n1);
    int64_t id2 = ani_store_upsert_node(s, &n2);

    ani_edge_t e = {.project = "test", .source_id = id1, .target_id = id2, .type = "CALLS"};
    ani_store_insert_edge(s, &e);

    /* Delete project — should cascade */
    ani_store_delete_project(s, "test");

    int ncnt = ani_store_count_nodes(s, "test");
...
TEST(store_file_hash_crud) {
    ...
    /* Upsert */
    int rc = ani_store_upsert_file_hash(s, "test", "main.go", "abc123", 1000000, 512);
    ASSERT_EQ(rc, ANI_STORE_OK);

    /* Get */
    ani_file_hash_t *hashes = NULL;
    int count = 0;
    rc = ani_store_get_file_hashes(s, "test", &hashes, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 1);
    ...
    /* Update */
    rc = ani_store_upsert_file_hash(s, "test", "main.go", "def456", 2000000, 1024);
    ASSERT_EQ(rc, ANI_STORE_OK);
    rc = ani_store_get_file_hashes(s, "test", &hashes, &count);
    ASSERT_EQ(count, 1);
    ...
    /* Delete single */
    rc = ani_store_delete_file_hash(s, "test", "main.go");
    ASSERT_EQ(rc, ANI_STORE_OK);
    rc = ani_store_get_file_hashes(s, "test", &hashes, &count);
    ASSERT_EQ(count, 0);
    ani_store_free_file_hashes(hashes, count);
...
TEST(store_lsp_surface_round_trip) {
    ...
    int count = ANI_NOT_FOUND;
    ASSERT_EQ(ani_store_get_lsp_surfaces(s, "test", &rows, &count), ANI_STORE_OK);
    ASSERT_EQ(count, 0);
    ...
    ASSERT_EQ(ani_store_upsert_lsp_surface_batch(s, in, 2), ANI_STORE_OK);
    ...
    ani_lsp_surface_row_t update = {.project = "test",
                                    .rel_path = "pkg/a.go",
                                    .surface_sha = "sha-a2",
                                    .defs_json = "[]",
                                    .ref_bloom = NULL,
                                    .ref_bloom_len = 0,
                                    .config_ctx = ""};
    ASSERT_EQ(ani_store_upsert_lsp_surface_batch(s, &update, 1), ANI_STORE_OK);
    ASSERT_EQ(ani_store_get_lsp_surfaces(s, "test", &rows, &count), ANI_STORE_OK);
    ASSERT_EQ(count, 2);
    ...
    ASSERT_EQ(ani_store_delete_lsp_surfaces(s, "test"), ANI_STORE_OK);
    ASSERT_EQ(ani_store_get_lsp_surfaces(s, "test", &rows, &count), ANI_STORE_OK);
    ASSERT_EQ(count, 0);
    ani_store_free_lsp_surfaces(rows, count);
...
TEST(store_dependent_files_lookup) {
    ...
    for (int i = 0; i < 3; i++) {
        ani_edge_t e = {.project = "test",
                        ...
                        .type = "CALLS",
                        .properties_json = "{}"};
        ASSERT_TRUE(ani_store_insert_edge(s, &e) > 0);
    }
    ...
    ani_node_t folder = {.project = "test",
                         .label = "Folder",
                         .name = "pkg",
                         ...
                         .end_line = 0,
                         .properties_json = "{}"};
    int64_t folder_id = ani_store_upsert_node(s, &folder);
    ASSERT_TRUE(folder_id > 0);
    ani_edge_t contains = {.project = "test",
                           .source_id = folder_id,
                           .target_id = ids[1],
                           .type = "CONTAINS_FILE",
                           .properties_json = "{}"};
    ASSERT_TRUE(ani_store_insert_edge(s, &contains) > 0);

    const char *targets[] = {"b.py"};
    char **deps = NULL;
    int dep_count = 0;
    ASSERT_EQ(ani_store_get_dependent_files(s, "test", targets, 1, &deps, &dep_count),
              ANI_STORE_OK);
    ...
    ani_store_free_dependent_files(deps, dep_count);

    /* No inbound edges: an isolated target has no dependents. */
    const char *targets_d[] = {"d.py"};
    ASSERT_EQ(ani_store_get_dependent_files(s, "test", targets_d, 1, &deps, &dep_count),
              ANI_STORE_OK);
    ...
    ASSERT_EQ(ani_store_get_dependent_files(s, "test", targets_ab, 2, &deps, &dep_count),
              ANI_STORE_OK);
...
TEST(store_file_hash_upsert_rejects_null_required_fields) {
    /* Pins the API contract that `ani_store_upsert_file_hash` returns
     * ANI_STORE_ERR (not silent OK) when a NOT NULL column would receive
    ...
    rc = ani_store_upsert_file_hash(s, "test", "other.go", NULL, 2000000, 1024);
    ASSERT_EQ(rc, ANI_STORE_ERR);

    /* NULL rel_path violates NOT NULL on file_hashes.rel_path → must return ERR. */
    rc = ani_store_upsert_file_hash(s, "test", NULL, "deadbeef", 3000000, 2048);
    ASSERT_EQ(rc, ANI_STORE_ERR);

    /* NULL project violates NOT NULL on file_hashes.project → must return ERR. */
    rc = ani_store_upsert_file_hash(s, NULL, "third.go", "cafebabe", 4000000, 4096);
    ASSERT_EQ(rc, ANI_STORE_ERR);
    ...
    ani_file_hash_t *hashes = NULL;
    int count = 0;
    ani_store_get_file_hashes(s, "test", &hashes, &count);
    ASSERT_EQ(count, 1);
    ASSERT_STR_EQ(hashes[0].rel_path, "main.go");
    ani_store_free_file_hashes(hashes, count);
...
TEST(store_node_properties_json) {
    ...
    ani_node_t found = {0};
    ani_store_find_node_by_qn(s, "test", "test.Bar", &found);
    ASSERT_NOT_NULL(found.properties_json);
...
TEST(store_node_null_properties) {
    ...
    ani_node_t found = {0};
    ani_store_find_node_by_qn(s, "test", "test.Baz", &found);
    ASSERT_NOT_NULL(found.properties_json);
...
TEST(store_find_by_file_overlap) {
    ...
    ani_node_t nm = {.project = "test",
                     ...
                     .start_line = 1,
                     .end_line = 100};
    ani_store_upsert_node(s, &na);
    ani_store_upsert_node(s, &nb);
    ani_store_upsert_node(s, &nc);
    ani_store_upsert_node(s, &nm);

    /* Overlap with funcA (lines 5-8) */
    ani_node_t *nodes = NULL;
    int count = 0;
    int rc = ani_store_find_nodes_by_file_overlap(s, "test", "main.go", 5, 8, &nodes, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 1);
    ...
    rc = ani_store_find_nodes_by_file_overlap(s, "test", "main.go", 8, 15, &nodes, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 2);
    ani_store_free_nodes(nodes, count);

    /* No overlap (lines 26-30) */
    rc = ani_store_find_nodes_by_file_overlap(s, "test", "main.go", 26, 30, &nodes, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 0);
    ani_store_free_nodes(nodes, count);

    /* Different file */
    rc = ani_store_find_nodes_by_file_overlap(s, "test", "other.go", 1, 50, &nodes, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 1);
...
TEST(store_find_by_qn_suffix_single) {
    ...
    ani_node_t *nodes = NULL;
    int count = 0;
    int rc = ani_store_find_nodes_by_qn_suffix(s, "test", "main.HandleRequest", &nodes, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 1);
...
TEST(store_find_by_qn_suffix_no_match) {
    ...
    ani_node_t *nodes = NULL;
    int count = 0;
    int rc = ani_store_find_nodes_by_qn_suffix(s, "test", "main.Bar", &nodes, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 0);
    ani_store_free_nodes(nodes, count);
...
TEST(store_find_by_qn_suffix_multiple) {
    ...
    ani_node_t *nodes = NULL;
    int count = 0;
    int rc = ani_store_find_nodes_by_qn_suffix(s, "test", "Run", &nodes, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 2);
    ani_store_free_nodes(nodes, count);
...
TEST(store_find_by_qn_suffix_dot_boundary) {
    ...
    ani_node_t *nodes = NULL;
    int count = 0;
    int rc = ani_store_find_nodes_by_qn_suffix(s, "test", "HandleRequest", &nodes, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 1);
    ASSERT_STR_EQ(nodes[0].name, "HandleRequest");
    ani_store_free_nodes(nodes, count);
...
TEST(store_node_degree) {
    ...
    ani_edge_t e1 = {.project = "test", .source_id = idA, .target_id = idB, .type = "CALLS"};
    ani_edge_t e2 = {.project = "test", .source_id = idA, .target_id = idC, .type = "CALLS"};
    ani_edge_t e3 = {.project = "test", .source_id = idB, .target_id = idC, .type = "CALLS"};
    ani_edge_t e4 = {.project = "test", .source_id = idA, .target_id = idC, .type = "USAGE"};
    ...
    ani_store_node_degree(s, idA, &inA, &outA);
    ASSERT_EQ(inA, 0);
    ASSERT_EQ(outA, 2);

    ani_store_node_degree(s, idB, &inB, &outB);
    ASSERT_EQ(inB, 1);
    ASSERT_EQ(outB, 1);

    ani_store_node_degree(s, idC, &inC, &outC);
    ASSERT_EQ(inC, 2);
    ASSERT_EQ(outC, 0);
...
TEST(store_file_hash_batch) {
    ...
    int rc = ani_store_upsert_file_hash_batch(s, hashes, 3);
    ASSERT_EQ(rc, ANI_STORE_OK);

    ani_file_hash_t *stored = NULL;
    int count = 0;
    rc = ani_store_get_file_hashes(s, "test", &stored, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 3);
    ani_store_free_file_hashes(stored, count);

    /* Update hashes (should not duplicate) */
    hashes[0].sha256 = "updated";
    hashes[0].mtime_ns = 9000;
    rc = ani_store_upsert_file_hash_batch(s, hashes, 3);
    ASSERT_EQ(rc, ANI_STORE_OK);
 ...
 * per-file ani_store_upsert_file_hash loop to a single ani_store_upsert_file_hash_batch:
...
TEST(store_file_hash_batch_equals_loop) {
    ...
    ani_store_t *a = ani_store_open_memory();
    ani_store_upsert_project(a, "p", "/tmp/p");
    for (int i = 0; i < 4; i++) {
        ASSERT_EQ(ani_store_upsert_file_hash(a, rows[i].project, rows[i].rel_path, rows[i].sha256,
                                             rows[i].mtime_ns, rows[i].size),
                  ANI_STORE_OK);
    }

    /* Store B: the batched path. */
    ani_store_t *b = ani_store_open_memory();
    ani_store_upsert_project(b, "p", "/tmp/p");
    ASSERT_EQ(ani_store_upsert_file_hash_batch(b, rows, 4), ANI_STORE_OK);

    ani_file_hash_t *ha = NULL, *hb = NULL;
    int ca = 0, cb = 0;
    ASSERT_EQ(ani_store_get_file_hashes(a, "p", &ha, &ca), ANI_STORE_OK);
    ASSERT_EQ(ani_store_get_file_hashes(b, "p", &hb, &cb), ANI_STORE_OK);
    ASSERT_EQ(ca, 4);
    ASSERT_EQ(cb, 4);
    ...
    ani_store_free_file_hashes(ha, ca);
    ani_store_free_file_hashes(hb, cb);
    ani_store_close(a);
    ani_store_close(b);
    PASS();
}
...
TEST(store_find_edges_by_url_path) {
    ...
    ani_node_t nt = {.project = "test",
                     .label = "Function",
                     .name = "handler",
                     .qualified_name = "test.handler"};
    int64_t srcID = ani_store_upsert_node(s, &ns);
    int64_t tgtID = ani_store_upsert_node(s, &nt);
    ...
    int count = 0;
    int rc = ani_store_find_edges_by_url_path(s, "test", "orders", &edges, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 1);
    ...
    rc = ani_store_find_edges_by_url_path(s, "test", "users", &edges, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 0);
    ani_store_free_edges(edges, count);
...
TEST(store_restore_from) {
    ...
    ani_store_t *src = ani_store_open_memory();
    ani_store_upsert_project(src, "test", "/tmp/test");
    for (int i = 0; i < 10; i++) {
        char name[32], qn[64];
        ...
        ani_node_t n = {.project = "test",
                        ...
                        .start_line = i * 10,
                        .end_line = i * 10 + 5};
        ani_store_upsert_node(src, &n);
    }

    /* Create destination store */
    ani_store_t *dst = ani_store_open_memory();

    /* Restore: copy from src → dst */
    int rc = ani_store_restore_from(dst, src);
    ASSERT_EQ(rc, ANI_STORE_OK);

    /* Verify data survived */
    ani_node_t found = {0};
    rc = ani_store_find_node_by_qn(dst, "test", "test.main.Func5", &found);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_STR_EQ(found.name, "Func5");
    ani_node_free_fields(&found);

    int cnt = ani_store_count_nodes(dst, "test");
    ASSERT_EQ(cnt, 10);
...
TEST(store_find_node_ids_by_qns) {
    ...
    ani_node_t nb = {
        .project = "test", .label = "Function", .name = "B", .qualified_name = "test.B"};
    int64_t id1 = ani_store_upsert_node(s, &na);
    int64_t id2 = ani_store_upsert_node(s, &nb);
    ASSERT_TRUE(id1 > 0);
    ASSERT_TRUE(id2 > 0);
    ...
    int64_t ids[3];
    int found = ani_store_find_node_ids_by_qns(s, "test", qns, 3, ids);
    ASSERT_EQ(found, 2);
    ASSERT_EQ(ids[0], id1);
    ASSERT_EQ(ids[1], id2);
    ...
    /* Empty batch */
    int found2 = ani_store_find_node_ids_by_qns(s, "test", NULL, 0, ids);
    ASSERT_EQ(found2, 0);
...
TEST(store_integrity_clean) {
    ...
    ani_store_t *s = ani_store_open_memory();
    ASSERT_NOT_NULL(s);
    ani_store_upsert_project(s, "test-proj", "/tmp/test");
    ASSERT_TRUE(ani_store_check_integrity(s));
    ani_store_close(s);
    PASS();
}
...
TEST(store_integrity_windows_lowercase_drive_issue367) {
    ...
    ani_store_t *s = ani_store_open_memory();
    ASSERT_NOT_NULL(s);
    ani_store_upsert_project(s, "lc-drive", "c:/Users/dev/repo");
    ASSERT_TRUE(ani_store_check_integrity(s));
    ani_store_close(s);
    PASS();
}
...
TEST(store_integrity_verdict_healthy_is_ok) {
    ani_store_t *s = ani_store_open_memory();
    ASSERT_NOT_NULL(s);
    ani_store_upsert_project(s, "healthy-proj", "/tmp/healthy");
    ASSERT_EQ(ani_store_check_integrity_verdict(s), ANI_INTEGRITY_OK);
    ani_store_close(s);
    PASS();
}
...
TEST(store_integrity_verdict_real_corruption_is_corrupt) {
    ...
    sqlite3_exec(db,
                 ...
                 NULL, NULL, NULL);
    ASSERT_EQ(ani_store_check_integrity_verdict(s), ANI_INTEGRITY_CORRUPT);
    ani_store_close(s);
    PASS();
}
...
TEST(store_integrity_verdict_unopenable_is_transient_not_corrupt) {
    ...
    ASSERT_EQ(ani_store_check_integrity_verdict(NULL), ANI_INTEGRITY_TRANSIENT);
    PASS();
}

TEST(store_integrity_null_check) {
    /* NULL store should return false (not crash) */
    ASSERT_FALSE(ani_store_check_integrity(NULL));
    PASS();
}
...
TEST(store_node_null_qn) {
    ...
    ani_node_t n = {.project = "test", .label = "Function", .name = "Bar", .qualified_name = NULL};
    int64_t id = ani_store_upsert_node(s, &n);
...
TEST(store_node_empty_strings) {
    ...
    ani_node_t found = {0};
    int rc = ani_store_find_node_by_qn(s, "test", "", &found);
    ASSERT_EQ(rc, ANI_STORE_OK);
...
TEST(store_find_by_id_not_found) {
    ...
    ani_node_t found = {0};
    int rc = ani_store_find_node_by_id(s, 999999, &found);
    ASSERT_EQ(rc, ANI_STORE_NOT_FOUND);

    /* Negative ID should also be not found */
    rc = ani_store_find_node_by_id(s, -1, &found);
    ASSERT_EQ(rc, ANI_STORE_NOT_FOUND);
...
TEST(store_find_by_qn_not_found) {
    ...
    ani_node_t found = {0};
    int rc = ani_store_find_node_by_qn(s, "test", "test.DoesNotExist", &found);
    ASSERT_EQ(rc, ANI_STORE_NOT_FOUND);

    /* Wrong project */
    rc = ani_store_find_node_by_qn(s, "other-project", "test.Exists", &found);
    ASSERT_EQ(rc, ANI_STORE_NOT_FOUND);
...
TEST(store_find_by_qn_any_cross_project) {
    ani_store_t *s = ani_store_open_memory();
    ani_store_upsert_project(s, "proj-a", "/tmp/a");
    ani_store_upsert_project(s, "proj-b", "/tmp/b");

    ani_node_t na = {.project = "proj-a",
                     .label = "Function",
                     .name = "SharedFunc",
                     .qualified_name = "proj-a.main.SharedFunc"};
    ani_node_t nb = {.project = "proj-b",
                     .label = "Class",
                     .name = "Widget",
    ...
    ani_node_t found = {0};
    int rc = ani_store_find_node_by_qn_any(s, "proj-a.main.SharedFunc", &found);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_STR_EQ(found.name, "SharedFunc");
    ASSERT_STR_EQ(found.project, "proj-a");
    ani_node_free_fields(&found);

    rc = ani_store_find_node_by_qn_any(s, "proj-b.pkg.Widget", &found);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ...
    /* Non-existent QN */
    rc = ani_store_find_node_by_qn_any(s, "nonexistent.Nope", &found);
    ASSERT_EQ(rc, ANI_STORE_NOT_FOUND);
...
TEST(store_find_by_name_any_cross_project) {
    ...
    ani_node_t *nodes = NULL;
    int count = 0;
    int rc = ani_store_find_nodes_by_name_any(s, "Init", &nodes, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 2);
    ani_store_free_nodes(nodes, count);

    /* Name that doesn't exist */
    rc = ani_store_find_nodes_by_name_any(s, "Nonexistent", &nodes, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 0);
    ani_store_free_nodes(nodes, count);
...
TEST(store_find_by_file_no_match) {
    ...
    ani_node_t *nodes = NULL;
    int count = 0;
    int rc = ani_store_find_nodes_by_file(s, "test", "nonexistent.go", &nodes, &count);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(count, 0);
    ani_store_free_nodes(nodes, count);
...
TEST(store_node_batch_upsert_100) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        nodes[i] = (ani_node_t){.project = "test",
                                .label = "Function",
                                .name = names[i],
                                .qualified_name = qns[i],
                                .file_path = "stress.go",
    ...
    int rc = ani_store_upsert_node_batch(s, nodes, 100, ids);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ...
    ani_node_t found = {0};
    rc = ani_store_find_node_by_qn(s, "test", "test.stress.stress_0", &found);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_STR_EQ(found.name, "stress_0");
    ani_node_free_fields(&found);

    rc = ani_store_find_node_by_qn(s, "test", "test.stress.stress_99", &found);
    ASSERT_EQ(rc, ANI_STORE_OK);
...
TEST(store_delete_by_label_verify_remaining) {
    ...
    ani_store_upsert_node(s, &n2);
    ani_store_upsert_node(s, &n3);
    ani_store_upsert_node(s, &n4);

    /* Delete all Functions */
    int rc = ani_store_delete_nodes_by_label(s, "test", "Function");
    ASSERT_EQ(rc, ANI_STORE_OK);
    ...
    ani_node_t found = {0};
    rc = ani_store_find_node_by_qn(s, "test", "test.ClassB", &found);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_STR_EQ(found.label, "Class");
    ani_node_free_fields(&found);

    rc = ani_store_find_node_by_qn(s, "test", "test.MethodD", &found);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_STR_EQ(found.label, "Method");
    ani_node_free_fields(&found);

    /* Deleted ones should be gone */
    rc = ani_store_find_node_by_qn(s, "test", "test.FuncA", &found);
    ASSERT_EQ(rc, ANI_STORE_NOT_FOUND);
    rc = ani_store_find_node_by_qn(s, "test", "test.FuncC", &found);
    ASSERT_EQ(rc, ANI_STORE_NOT_FOUND);
...
TEST(store_delete_by_file_verify_remaining) {
    ...
    int rc = ani_store_delete_nodes_by_file(s, "test", "delete_me.go");
    ASSERT_EQ(rc, ANI_STORE_OK);
    ...
    ani_node_t found = {0};
    rc = ani_store_find_node_by_qn(s, "test", "test.B", &found);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_STR_EQ(found.file_path, "keep_me.go");
    ani_node_free_fields(&found);

    /* Deleted nodes should be gone */
    rc = ani_store_find_node_by_qn(s, "test", "test.A", &found);
    ASSERT_EQ(rc, ANI_STORE_NOT_FOUND);
    rc = ani_store_find_node_by_qn(s, "test", "test.C", &found);
    ASSERT_EQ(rc, ANI_STORE_NOT_FOUND);
...
TEST(store_node_upsert_updates_fields) {
    ...
    ani_node_t found = {0};
    int rc = ani_store_find_node_by_id(s, id1, &found);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_STR_EQ(found.label, "Method");
...
TEST(store_node_long_qn) {
    ...
    ani_node_t found = {0};
    int rc = ani_store_find_node_by_qn(s, "test", long_qn, &found);
    ASSERT_EQ(rc, ANI_STORE_OK);
...
TEST(store_node_properties_special_chars) {
    ...
    ani_node_t found = {0};
    int rc = ani_store_find_node_by_id(s, id, &found);
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_NOT_NULL(found.properties_json);
...
TEST(store_delete_nodes_nonexistent) {
    ...
    int rc = ani_store_delete_nodes_by_file(s, "test", "ghost.go");
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(ani_store_count_nodes(s, "test"), 1);

    /* Delete by non-existent label */
    rc = ani_store_delete_nodes_by_label(s, "test", "Interface");
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(ani_store_count_nodes(s, "test"), 1);

    /* Delete by non-existent project */
    rc = ani_store_delete_nodes_by_project(s, "no-such-project");
    ASSERT_EQ(rc, ANI_STORE_OK);
    ASSERT_EQ(ani_store_count_nodes(s, "test"), 1);
...
TEST(store_count_nodes_unknown_project) {
    ...
    int cnt = ani_store_count_nodes(s, "ghost-project");
    ASSERT_EQ(cnt, 0);
    ani_store_close(s);
    PASS();
}
...
TEST(store_coverage_roundtrip_prune_shadow) {
    ani_store_t *s = ani_store_open_memory();
    ani_store_upsert_project(s, "test", "/tmp/test");
    ani_store_upsert_file_hash(s, "test", "src/a.py", "", 1, 10);
    ...
    ASSERT_EQ(ani_store_coverage_replace(s, "test", rows, 4), ANI_STORE_OK);

    ani_coverage_row_t *got = NULL;
    int n = 0;
    ASSERT_EQ(ani_store_coverage_get(s, "test", &got, &n), ANI_STORE_OK);
    ...
    ani_node_t *nodes = NULL;
    int nc = 0;
    ASSERT_EQ(ani_store_find_nodes_by_label(s, "test::missed", "File", &nodes, &nc), ANI_STORE_OK);
    ASSERT_EQ(nc, 1);
    ...
    nodes = NULL;
    nc = 0;
    ASSERT_EQ(ani_store_find_nodes_by_label(s, "test::missed", "Folder", &nodes, &nc),
              ANI_STORE_OK);
    ASSERT_EQ(nc, 1);
    ...
    ASSERT_EQ(ani_store_coverage_replace(s, "test", NULL, 0), ANI_STORE_OK);
    got = NULL;
    n = 0;
    ASSERT_EQ(ani_store_coverage_get(s, "test", &got, &n), ANI_STORE_OK);
    ASSERT_EQ(n, 0);
...
TEST(store_coverage_targeted_path_and_scope_lookup) {
    ani_store_t *s = ani_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(ani_store_upsert_project(s, "coverage-targeted", "/tmp/coverage-targeted"),
              ANI_STORE_OK);
    ...
    ASSERT_EQ(ani_store_upsert_file_hash(s, "coverage-targeted", "src/partial.c", "", 10, 20),
              ANI_STORE_OK);
    ASSERT_EQ(ani_store_upsert_file_hash(s, "coverage-targeted", "src/skipped.c", "", 11, 21),
              ANI_STORE_OK);
    ASSERT_EQ(ani_store_upsert_file_hash(s, "coverage-targeted", "src2/other.c", "", 12, 22),
              ANI_STORE_OK);
    ...
    ASSERT_EQ(ani_store_coverage_replace(s, "coverage-targeted", rows, 5), ANI_STORE_OK);

    ani_coverage_row_t *got = NULL;
    int count = 0;
    ASSERT_EQ(ani_store_coverage_get_path(s, "coverage-targeted", "src/partial.c", &got, &count),
              ANI_STORE_OK);
    ASSERT_EQ(count, 1);
    ASSERT_STR_EQ(got[0].rel_path, "src/partial.c");
    ASSERT_STR_EQ(got[0].kind, "parse_partial");
    ASSERT_STR_EQ(got[0].detail, "7-9");
    ani_store_free_coverage(got, count);

    /* A file below an excluded directory inherits that directory row. */
    got = NULL;
    count = 0;
    ASSERT_EQ(ani_store_coverage_get_path(s, "coverage-targeted", "generated/nested/file.c", &got,
                                          &count),
              ANI_STORE_OK);
    ASSERT_EQ(count, 1);
    ...
    count = 0;
    ASSERT_EQ(
        ani_store_coverage_get_path(s, "coverage-targeted", "generated2/file.c", &got, &count),
        ANI_STORE_OK);
    ASSERT_EQ(count, 0);
    ani_store_free_coverage(got, count);
    ...
    got = NULL;
    count = 0;
    ASSERT_EQ(ani_store_coverage_get_scope(s, "coverage-targeted", "src", &got, &count),
              ANI_STORE_OK);
    ASSERT_EQ(count, 2);
    ASSERT_STR_EQ(got[0].rel_path, "src/partial.c");
    ...
    count = 0;
    ASSERT_EQ(
        ani_store_coverage_get_scope(s, "coverage-targeted", "generated/nested", &got, &count),
        ANI_STORE_OK);
    ASSERT_EQ(count, 1);
    ASSERT_STR_EQ(got[0].rel_path, "generated");
    ani_store_free_coverage(got, count);
    ...
    ASSERT_EQ(ani_store_get_file_hash(s, "coverage-targeted", "src/partial.c", &hash),
              ANI_STORE_OK);
    ...
    ASSERT_EQ(ani_store_get_file_hash(s, "coverage-targeted", "missing.c", &hash),
              ANI_STORE_NOT_FOUND);
    ani_store_clear_file_hash(&hash);
...
TEST(store_coverage_meta_zero_row_truncation_and_delete) {
    ani_store_t *s = ani_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(ani_store_upsert_project(s, "coverage-meta", "/tmp/coverage-meta"), ANI_STORE_OK);

    ani_coverage_meta_t write_meta = {
    ...
    ASSERT_EQ(ani_store_coverage_replace_ex(s, "coverage-meta", NULL, 0, &write_meta),
              ANI_STORE_OK);

    ani_coverage_row_t *rows = NULL;
    int count = 0;
    ASSERT_EQ(ani_store_coverage_get(s, "coverage-meta", &rows, &count), ANI_STORE_OK);
    ASSERT_EQ(count, 0);
    ani_store_free_coverage(rows, count);

    ani_coverage_meta_t got = {0};
    ASSERT_EQ(ani_store_coverage_meta_get(s, "coverage-meta", &got), ANI_STORE_OK);
    ...
    ani_store_coverage_meta_clear(&got);
    ...
    ASSERT_EQ(ani_store_coverage_replace(s, "coverage-meta", NULL, 0), ANI_STORE_OK);
    ASSERT_EQ(ani_store_coverage_meta_get(s, "coverage-meta", &got), ANI_STORE_NOT_FOUND);
    ani_store_coverage_meta_clear(&got);
    ...
    ASSERT_EQ(ani_store_upsert_file_hash(s, "coverage-meta", "bad.c", "", 1, 1), ANI_STORE_OK);
    ani_coverage_row_t failure = {.rel_path = "bad.c", .kind = "parse_partial", .detail = "1-2"};
    ASSERT_EQ(ani_store_coverage_replace_ex(s, "coverage-meta", &failure, 1, &write_meta),
              ANI_STORE_OK);
    ASSERT_EQ(ani_store_delete_project(s, "coverage-meta"), ANI_STORE_OK);
    ...
    ASSERT_EQ(ani_store_coverage_meta_get(s, "coverage-meta", &got), ANI_STORE_NOT_FOUND);
    ani_store_coverage_meta_clear(&got);
    ani_project_t shadow = {0};
    ASSERT_EQ(ani_store_get_project(s, "coverage-meta::missed", &shadow), ANI_STORE_NOT_FOUND);
    ani_project_free_fields(&shadow);
...
TEST(store_coverage_replace_rejects_invalid_row_arguments) {
    ani_store_t *s = ani_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(ani_store_upsert_project(s, "coverage-invalid", "/tmp/coverage-invalid"),
              ANI_STORE_OK);
    ASSERT_EQ(ani_store_upsert_file_hash(s, "coverage-invalid", "kept.c", "", 1, 1), ANI_STORE_OK);
    ani_coverage_row_t kept = {.rel_path = "kept.c", .kind = "parse_partial", .detail = "3-4"};
    ani_coverage_meta_t original_meta = {
    ...
    ASSERT_EQ(ani_store_coverage_replace_ex(s, "coverage-invalid", &kept, 1, &original_meta),
              ANI_STORE_OK);

    ani_coverage_meta_t replacement_meta = original_meta;
    replacement_meta.generation = "must-not-commit";
    ASSERT_EQ(ani_store_coverage_replace_ex(s, "coverage-invalid", &kept, -1, &replacement_meta),
              ANI_STORE_ERR);
    ASSERT_EQ(ani_store_coverage_replace_ex(s, "coverage-invalid", NULL, 1, &replacement_meta),
              ANI_STORE_ERR);

    ani_coverage_row_t *rows = NULL;
    int count = 0;
    ASSERT_EQ(ani_store_coverage_get(s, "coverage-invalid", &rows, &count), ANI_STORE_OK);
    ASSERT_EQ(count, 1);
    ASSERT_STR_EQ(rows[0].rel_path, "kept.c");
    ASSERT_STR_EQ(rows[0].detail, "3-4");
    ani_store_free_coverage(rows, count);
    ani_coverage_meta_t got = {0};
    ASSERT_EQ(ani_store_coverage_meta_get(s, "coverage-invalid", &got), ANI_STORE_OK);
...
TEST(store_coverage_replace_rolls_back_when_shadow_rebuild_fails) {
    ani_store_t *s = ani_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(ani_store_upsert_project(s, "coverage-shadow", "/tmp/coverage-shadow"), ANI_STORE_OK);
    ASSERT_EQ(ani_store_upsert_file_hash(s, "coverage-shadow", "old.c", "", 1, 1), ANI_STORE_OK);
    ASSERT_EQ(ani_store_upsert_file_hash(s, "coverage-shadow", "new.c", "", 2, 2), ANI_STORE_OK);

    ani_coverage_row_t old_row = {
        .rel_path = "old.c", .kind = "parse_partial", .detail = "old-detail"};
    ani_coverage_meta_t old_meta = {
    ...
    ASSERT_EQ(ani_store_coverage_replace_ex(s, "coverage-shadow", &old_row, 1, &old_meta),
              ANI_STORE_OK);
    int old_shadow_nodes = ani_store_count_nodes(s, "coverage-shadow::missed");
    ...
    ASSERT_EQ(ani_store_exec(s, "CREATE TRIGGER fail_missed_insert BEFORE INSERT ON nodes "
                                "WHEN NEW.project = 'coverage-shadow::missed' "
                                "BEGIN SELECT RAISE(ABORT, 'forced missed shadow failure'); END;"),
              ANI_STORE_OK);

    ani_coverage_row_t new_row = {
        .rel_path = "new.c", .kind = "parse_partial", .detail = "new-detail"};
    ani_coverage_meta_t new_meta = old_meta;
    new_meta.generation = "new-generation";
    ASSERT_EQ(ani_store_coverage_replace_ex(s, "coverage-shadow", &new_row, 1, &new_meta),
              ANI_STORE_ERR);

    ani_coverage_row_t *rows = NULL;
    int count = 0;
    ASSERT_EQ(ani_store_coverage_get(s, "coverage-shadow", &rows, &count), ANI_STORE_OK);
    ASSERT_EQ(count, 1);
    ...
    ani_coverage_meta_t got = {0};
    ASSERT_EQ(ani_store_coverage_meta_get(s, "coverage-shadow", &got), ANI_STORE_OK);
    ASSERT_STR_EQ(got.generation, "old-generation");
    ani_store_coverage_meta_clear(&got);
    ASSERT_EQ(ani_store_count_nodes(s, "coverage-shadow::missed"), old_shadow_nodes);
...
SUITE(store_nodes) {
    ...
    RUN_TEST(sql_label_allowlists_match_ani_label_is_type_like);
    RUN_TEST(sql_relation_labels_match_ani_label_is_relation);
