 ...
 * languages even though ANIFileResult.imports IS populated at extraction time:
 *
 ...
 *   ani_store_count_edges_by_type(store, project, "IMPORTS") >= N
 *
...
static ani_store_t *ei_index_files(EILangProj *lp, const EILangFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/ani_ei_XXXXXX");
...
static ani_store_t *ei_index_fixture_files(EILangProj *lp, const char *fixture_root,
...
static int64_t ei_node_id_for_file_label(ani_store_t *store, const char *project,
                                         const char *file_path, const char *label) {
    ani_node_t *nodes = NULL;
    int count = 0;
    if (ani_store_find_nodes_by_file(store, project, file_path, &nodes, &count) != ANI_STORE_OK) {
        return 0;
    }
    int64_t id = 0;
...
static void ei_cleanup(EILangProj *lp, ani_store_t *store) {
    if (store)
...
static int ei_edge_count_is(const EILangFile *files, int nfiles, const char *edge_type,
                            int expected) {
    EILangProj lp;
    ani_store_t *store = ei_index_files(&lp, files, nfiles);
    int got = store ? ani_store_count_edges_by_type(store, lp.project, edge_type) : -1;
    if (got != expected) {
...
TEST(ei_cpp_header_include_targets_header_file) {
    ...
    int edge_count = 0;
    int rc = ani_store_find_edges_by_source_type(store, main_id, "IMPORTS", &edges, &edge_count);
    ...
    for (int i = 0; i < edge_count; i++) {
        ani_node_t *target = (ani_node_t *)calloc(1, sizeof(ani_node_t));

        /* Pass target directly (no &) because it is already a pointer */
        ASSERT_EQ(ani_store_find_node_by_id(store, edges[i].target_id, target), ANI_STORE_OK);
