...
> **About / Provenance** — `ani` is derived from [`codebase-memory-mcp`](https://github.com/jnani1972/codebase-memory-mcp), originally by DeusData, used under the MIT License. The original copyright and license are preserved in [LICENSE](LICENSE).

[![GitHub Release](https://img.shields.io/github/v/release/jnani1972/ani?style=flat&color=blue)](https://github.com/jnani1972/ani/releases/latest)
[![License](https://img.shields.io/badge/license-MIT-green)](LICENSE)
[![CI](https://img.shields.io/github/actions/workflow/status/jnani1972/ani/dry-run.yml?label=CI)](https://github.com/jnani1972/ani/actions/workflows/dry-run.yml)
[![Tests](https://img.shields.io/badge/tests-6768_passing-brightgreen)](https://github.com/jnani1972/ani)
[![Languages](https://img.shields.io/badge/languages-162-orange)](https://github.com/jnani1972/ani)
[![Hybrid LSP](https://img.shields.io/badge/Hybrid_LSP-10_languages-blue)](#hybrid-lsp)
[![Agents](https://img.shields.io/badge/agent_surfaces-45-purple)](https://github.com/jnani1972/ani)
[![Pure C](https://img.shields.io/badge/pure_C-no_language_runtime-blue)](https://github.com/jnani1972/ani)
[![Platform](https://img.shields.io/badge/macOS_%7C_Linux_%7C_Windows-supported-lightgrey)](https://github.com/jnani1972/ani/releases/latest)
[![OpenSSF Scorecard](https://api.scorecard.dev/projects/github.com/jnani1972/ani/badge)](https://scorecard.dev/viewer/?uri=github.com/jnani1972/ani)
[![SLSA 3](https://slsa.dev/images/gh-badge-level3.svg)](https://slsa.dev)
[![VirusTotal](https://img.shields.io/badge/VirusTotal-scanned_every_release-brightgreen?logo=virustotal)](https://github.com/jnani1972/ani/releases/latest)
...
Invoke-WebRequest -Uri https://raw.githubusercontent.com/jnani1972/ani/main/install.ps1 -OutFile install.ps1
...
1. **Download** the archive for your platform from the [latest release](https://github.com/jnani1972/ani/releases/latest):
   - `ani-<os>-<arch>.tar.gz` (macOS/Linux) or `.zip` (Windows)
...
2. **Extract and install** (each archive includes `install.sh` or `install.ps1`):
   ...
   Windows (PowerShell):
   ```powershell
   Expand-Archive ani-windows-amd64.zip -DestinationPath .
   Unblock-File .\install.ps1
   .\install.ps1
   ```
...
ANI automatically shares one per-account coordination daemon across Claude Code, Codex, OpenCode, and every other configured client. There is no opt-in setting for MCP servers or hook clients: the first daemon-backed ANI session starts it, each session registers its own work, and the final session shuts it down. The daemon owns long-lived background services such as watchers, shared indexing jobs, and the optional UI. Closing one session cancels work owned only by that session, while work still needed by another session continues.

The detached daemon does not depend on an MCP frontend's stderr. It keeps owner-only durable records under the canonical `${ANI_CACHE_DIR}/logs` directory (default `~/.cache/ani/logs`):

| File | Contents |
|------|----------|
| `ani-daemon.log` | Daemon lifecycle, watcher/indexing, UI, resource, and error events. |
...
All active ANI processes must run the exact same version, executable build, coordination ABI, and canonical cache root. Equivalent `ANI_CACHE_DIR` aliases resolve to the same root; a genuinely different root is rejected while any ANI process is active. MCP servers, hooks, one-shot CLI commands, temporary index workers, and the daemon share a crash-safe OS admission barrier; starting an ordinary conflicting process fails before doing work and records an explicit conflict in `${ANI_CACHE_DIR}/logs/daemon-conflicts.ndjson`.

The native `install`, `update`, and `uninstall` commands are the deliberate exception to that conflict rule. Download, verification, and private same-filesystem staging happen first so a bad candidate never disrupts active work. Activation then publishes account-wide maintenance intent, asks the daemon and every temporary local operation to cancel, and waits to a finite deadline for all coordinated ANI processes to exit. It holds the admission and lifetime barriers exclusively while changing the active binary, configuration, PATH, or indexes. New ANI work cannot enter during this window. Activation progress and results are recorded in `${ANI_CACHE_DIR}/logs/activation-events.ndjson`, and a successful command tells you to restart open coding-agent sessions so they launch the activated build.

Package-manager setup (npm, PyPI, or Go) verifies and publishes a coherent private cached runtime set. Sidecars are replaced before the executable with per-file atomic renames; an interrupted multi-file publication is detected and repaired on the next launch rather than being described as one crash-atomic filesystem transaction. It does not replace the active native installation and therefore does not stop running ANI sessions. When that cached binary is executed, it still enters the same exact-build admission barrier. The shell and PowerShell installers invoke the verified candidate's native `install` command, so they do receive the full account-wide activation guarantee.
...
```bash
ani config set auto_index true
```
...
To turn the watcher off entirely, set `config set watcher_enabled false` (default `true`): the background poll thread never starts and no project is registered, while `auto_index` and manual `index_repository` keep working. Unlike `auto_watch` — which is consulted per session — `watcher_enabled` is read once when the background daemon starts, so run `ani daemon stop` after changing it; reconnecting your MCP client alone will not restart the daemon. See [docs/CONFIGURATION.md](docs/CONFIGURATION.md#2-cli-managed-runtime-settings).

### Keeping Up to Date

**Updates run from the install script on every platform, not from inside the running binary.** `ani update` validates your flags and then prints the exact command to run:
...
Why it works this way. On Windows it is a hard requirement — a running executable cannot replace its own image, so the swap has to happen from a process that is not the binary being replaced. On macOS and Linux it is a deliberate choice: an in-process updater is structurally a downloader (fetch an archive, verify it, unpack it, mark a file executable, run it), and shipping that composite in every binary to serve a command most people run a handful of times is a poor trade. The release archives now carry no download URLs at all, and **ani makes no network request of its own accord** — it does not check for new versions in the background, and nothing phones home. You find out about releases from the install script, your package manager, or GitHub.

If PowerShell refuses to run the script because the file came from the internet, `Unblock-File` it first.

Installed through **npm or pip**? Update with your package manager on every platform (`npm install -g ani@latest` / `pip install -U ani`).
...
- **BM25 full-text search** via SQLite FTS5 with `ani_camel_split` tokenizer (camelCase / snake_case aware)
...
- **Native runtime set, zero infrastructure services**: SQLite-backed, persists to `~/.cache/ani/`
- **Auto-sync**: Background watcher detects file changes and re-indexes automatically
- **Route nodes**: REST endpoints are first-class graph entities
- **CLI mode**: `ani cli search_graph '{"project": "my-project", "name_pattern": ".*Handler.*"}'`
...
`.ani/graph.db.zst` is a zstd-compressed snapshot of the knowledge graph that lives next to your source. When you index, the artifact is written or refreshed; when a teammate clones the repo and runs `ani` for the first time, the artifact is decompressed and incremental indexing fills in their local diff.
...
- **Optional**: never committed unless you want it. Add `.ani/` to `.gitignore` if you prefer everyone to reindex from scratch.
...
ani is a **structural analysis backend** — it builds and queries the knowledge graph. It does **not** include an LLM. Instead, it relies on your MCP client (Claude Code, or any MCP-compatible agent) to be the intelligence layer.
...
ani: executes graph query, returns structured results
...
**Token efficiency**: Five structural queries consumed ~3,400 tokens via ani versus ~412,000 tokens via file-by-file grep exploration — a **99.2% reduction**.
...
| Platform | Archive |
|----------|---------|
| macOS (Apple Silicon) | `ani-darwin-arm64.tar.gz` |
| macOS (Intel) | `ani-darwin-amd64.tar.gz` |
| Linux (x86_64) | `ani-linux-amd64.tar.gz` |
| Linux (ARM64) | `ani-linux-arm64.tar.gz` |
| Windows (x86_64) | `ani-windows-amd64.zip` |
...
```bash
curl -fsSL https://raw.githubusercontent.com/jnani1972/ani/main/scripts/setup.sh | bash
```

**Windows (PowerShell):**

```powershell
irm https://raw.githubusercontent.com/jnani1972/ani/main/scripts/setup-windows.ps1 | iex
```
...
The `ani-bin` package is available at: https://aur.archlinux.org/packages/codebase-memory-mcp-bin
...
| Package | Contents |
|---------|----------|
| `default` (`ani`) | Standard server, no UI |
| `ani-ui` | Server with the graph UI embedded (`--ui=true` works) |
...
```bash
# Standard server
nix run github:DeusData/ani

# Server with the embedded graph UI
nix run github:DeusData/ani#ani-ui -- --ui=true --port=9749
# then open http://127.0.0.1:9749
```

Or build a binary into `./result/bin/ani`:

```bash
nix build github:DeusData/ani                          # standard
nix build github:DeusData/ani#ani-ui   # with UI
```

Working in a clone? Use `.` in place of the flake URL, e.g. `nix run .#ani-ui -- --ui=true`, or drop into a shell that puts the binary on `PATH` with `nix shell .#ani-ui`.

> **Note:** launched by hand (not from an MCP client) the server exits as soon as `stdin` closes — that's normal MCP behaviour. Keep `stdin` open while testing the UI, e.g. `sleep infinity | ani --ui=true --port=9749`. The `ani-ui` package embeds the UI at build time; `nix run`'ing the standard `default` package with `--ui=true` will refuse to start the HTTP server.

### Install via Claude Code

```
You: "Install this MCP server: https://github.com/jnani1972/ani"
```
...
Restart your agent. Verify with `/mcp` — you should see `ani` with 15 tools.
...
| OpenHands | Detected | `.openhands/mcp.json` | Shared `.agents/skills/ani/SKILL.md` |
...
| Warp | Detected, skill only | UI, Warp Drive, or per invocation (manual) | Shared `~/.agents/skills/ani/SKILL.md` |
...
| Grok Build | Detected | `$GROK_HOME/config.toml` | Owned `rules/ani.md`, skill, three graph agents with named-server `mcpInheritance` and exact `server__tool` dispatcher ids; context hooks withheld because its passive hook events discard stdout |
...
| Amp | Detected | `~/.config/agents/skills/ani/mcp.json` | Colocated skill + `~/.config/amp/AGENTS.md`; no plugin |
...
| Continue / cn | Conditional | Existing `~/.continue/config.yaml` or `$ANI_CONTINUE_CONFIG_PATH` | MCP only |
| Visual Studio | Conditional, Windows | `~/.mcp.json` | MCP only |
| TRAE | Conditional | Existing `$ANI_TRAE_CONFIG_PATH` | MCP only |
| Roo Code | Conditional | Existing `$ANI_ROO_CONFIG_PATH` | MCP only |
...
| Sourcegraph Cody | Explicit opt-in | Existing `$ANI_CODY_CONFIG_PATH` | MCP only |
...
OpenClaw reinjects the `Codebase Knowledge Graph (ani)` AGENTS
...
The installed Claude shim is named `ani-code-discovery-gate` for backward
...
```bash
ani cli index_repository --repo-path /path/to/repo
ani cli list_projects

# Use the "name" returned by list_projects as the project value.
ani cli search_graph --project my-project --name-pattern '.*Handler.*' --label Function
ani cli trace_path --project my-project --function-name Search --direction both
ani cli query_graph --project my-project --query 'MATCH (f:Function) RETURN f.name LIMIT 5'

# Force human-readable progress without contaminating stdout.
ani cli --progress index_repository --repo-path /path/to/repo
ani cli search_graph --project my-project --label Function | jq '.results[].name'
```
...
Layered: hardcoded patterns (`.git`, `node_modules`, etc.) → `.gitignore` hierarchy → `.aniignore` (project-specific, gitignore syntax). Symlinks are always skipped.

See [docs/aniignore.md](docs/aniignore.md) for the full `.aniignore` how-to: syntax, precedence across the ignore layers, and negation semantics.

## Configuration

```bash
ani config list                          # show all settings
ani config set auto_index true           # auto-index on session start
ani config set auto_index_limit 50000    # max files for auto-index
ani config set auto_watch false          # don't register background git watcher (default: true)
ani config set watcher_enabled false     # stop the watcher thread entirely (default: true)
ani config reset auto_index              # reset to default
```
...
```json
// ~/.config/ani/config.json  (or $XDG_CONFIG_HOME/...)
...
SQLite databases stored at `~/.cache/ani/`. Persists across restarts (WAL mode, ACID-safe). To reset: `rm -rf ~/.cache/ani/`.
...
src/
  ...
  discover/           File discovery (.gitignore, .aniignore, symlink handling)
...
internal/ani/         Vendored tree-sitter grammars (162 languages) + AST extraction engine
```
...
- **SLSA Level 3** — cryptographic build provenance generated by the trusted GitHub Actions build workflow; verify with `gh attestation verify <file> --repo DeusData/ani --signer-workflow DeusData/ani/.github/workflows/_build.yml`
