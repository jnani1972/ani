...
//
// For ANI_LANG_KUSTOMIZE: walks top-level block_mapping_pair nodes whose key
// matches "resources", "bases", "patches", "components", or
// "patchesStrategicMerge", then emits one ANIImport per block_sequence item.
//
// For ANI_LANG_K8S: finds apiVersion, kind, and metadata.name scalars in the
// first document's block_mapping and emits one ANIDefinition with label
...
static const char *get_scalar_text(ANIArena *a, TSNode node, const char *source) {
    enum { MAX_UNWRAP = 4 };
    for (int depth = 0; depth < MAX_UNWRAP; depth++) {
        ...
        if (strcmp(type, "double_quote_scalar") == 0 || strcmp(type, "single_quote_scalar") == 0) {
            const char *raw = ani_node_text(a, node, source);
            if (!raw) {
                return NULL;
            }
            size_t len = strlen(raw);
            if (len >= PAIR_LEN) {
                return ani_arena_strndup(a, raw + SKIP_ONE, len - PAIR_LEN);
            }
            return raw;
        }
        break;
...
// Walk a block_sequence node and emit one ANIImport per block_sequence_item
// scalar child, using key_name as the local_name.
static void emit_kustomize_sequence(ANIExtractCtx *ctx, TSNode seq_node, const char *key_name) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        for (uint32_t j = 0; j < ic; j++) {
            ...
            ANIImport imp = {
                .local_name = ani_arena_strdup(a, key_name),
                .module_path = ani_arena_strdup(a, scalar),
            };
...
static void process_kustomize_pair(ANIExtractCtx *ctx, TSNode pair) {
...
static void emit_kustomize_kind_def(ANIExtractCtx *ctx, TSNode mapping) {
    ANIArena *a = ctx->arena;
    ...
    for (uint32_t pi = 0; pi < pair_n; pi++) {
        ...
        ANIDefinition def = {0};
        def.name = ani_arena_strdup(a, kind);
        def.qualified_name = ani_arena_sprintf(a, "%s.%s", ctx->module_qn, kind);
        def.label = ani_arena_strdup(a, "Resource");
...
static void extract_kustomize(ANIExtractCtx *ctx) {
...
static void extract_metadata_name(ANIArena *a, TSNode meta_mapping, const char *source,
...
static void extract_k8s_scalars(ANIExtractCtx *ctx, TSNode mapping, char *kind_buf, size_t kind_sz,
                                char *meta_name_buf, size_t meta_sz) {
    ANIArena *a = ctx->arena;
    kind_buf[0] = '\0';
    meta_name_buf[0] = '\0';
...
static void extract_k8s_manifest(ANIExtractCtx *ctx) {
    ANIArena *a = ctx->arena;
    ...
    for (uint32_t si = 0; si < root_n; si++) {
        ...
        ANIDefinition def = {0};
        def.name = ani_arena_strdup(a, def_name);
        def.qualified_name = ani_arena_sprintf(a, "%s.%s", ctx->module_qn, def_name);
...
void ani_extract_k8s(ANIExtractCtx *ctx) {
    if (ctx->language == ANI_LANG_KUSTOMIZE) {
        extract_kustomize(ctx);
    } else if (ctx->language == ANI_LANG_K8S) {
