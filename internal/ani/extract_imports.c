#include "ani.h"
#include "arena.h" // ANIArena, ani_arena_strdup/strndup/sprintf
#include "helpers.h"
#include "lang_specs.h"      // ANILangSpec, ANIEmbeddedLangSpec, ani_lang_spec, ani_ts_language
...
static void parse_go_imports(ANIExtractCtx *ctx);
static void parse_python_imports(ANIExtractCtx *ctx);
static void parse_es_imports(ANIExtractCtx *ctx);
static void parse_java_imports(ANIExtractCtx *ctx);
static void parse_rust_imports(ANIExtractCtx *ctx);
static void parse_c_imports(ANIExtractCtx *ctx);
static void parse_ruby_imports(ANIExtractCtx *ctx);
static void parse_lua_imports(ANIExtractCtx *ctx);
static void parse_r_imports(ANIExtractCtx *ctx);
static void parse_kotlin_imports(ANIExtractCtx *ctx);
static void parse_dart_imports(ANIExtractCtx *ctx);
static void parse_haskell_imports(ANIExtractCtx *ctx);
static void parse_zig_imports(ANIExtractCtx *ctx);
static void parse_generic_imports(ANIExtractCtx *ctx, const char *node_type);
static void parse_wolfram_imports(ANIExtractCtx *ctx);
static void parse_php_imports(ANIExtractCtx *ctx);
static void parse_csharp_imports(ANIExtractCtx *ctx);
static void parse_spec_imports(ANIExtractCtx *ctx);
static void parse_hare_imports(ANIExtractCtx *ctx);
static void parse_pascal_imports(ANIExtractCtx *ctx);
static void parse_powershell_imports(ANIExtractCtx *ctx);
static void parse_lisp_imports(ANIExtractCtx *ctx);
static void parse_starlark_imports(ANIExtractCtx *ctx);
static void parse_tcl_imports(ANIExtractCtx *ctx);
static void parse_teal_imports(ANIExtractCtx *ctx);
static void parse_zsh_imports(ANIExtractCtx *ctx);
static void parse_css_imports(ANIExtractCtx *ctx);
static void parse_html_imports(ANIExtractCtx *ctx);
static void parse_cmake_imports(ANIExtractCtx *ctx);
static void parse_bitbake_imports(ANIExtractCtx *ctx);
static void parse_kconfig_imports(ANIExtractCtx *ctx);
static void parse_gn_imports(ANIExtractCtx *ctx);
static void parse_just_imports(ANIExtractCtx *ctx);
static void parse_nix_imports(ANIExtractCtx *ctx);
static void parse_jsonnet_imports(ANIExtractCtx *ctx);
static void parse_pkl_imports(ANIExtractCtx *ctx);
static void parse_nickel_imports(ANIExtractCtx *ctx);
static void parse_thrift_imports(ANIExtractCtx *ctx);
static void parse_capnp_imports(ANIExtractCtx *ctx);
static void parse_dlang_imports(ANIExtractCtx *ctx);
static void parse_tablegen_imports(ANIExtractCtx *ctx);
static void parse_crystal_imports(ANIExtractCtx *ctx);
static void parse_fsharp_imports(ANIExtractCtx *ctx);
static void parse_ada_imports(ANIExtractCtx *ctx);
static void parse_elm_imports(ANIExtractCtx *ctx);
static void parse_move_imports(ANIExtractCtx *ctx);
static void parse_smali_imports(ANIExtractCtx *ctx);
static void parse_tlaplus_imports(ANIExtractCtx *ctx);
static void parse_vhdl_imports(ANIExtractCtx *ctx);
static void parse_wit_imports(ANIExtractCtx *ctx);
static void parse_smithy_imports(ANIExtractCtx *ctx);
static void parse_hyprlang_imports(ANIExtractCtx *ctx);

// Helper: strip quotes from a string literal
static char *strip_quotes(ANIArena *a, const char *s) {
    if (!s) {
        return NULL;
    }
    size_t len = strlen(s);
    if (len >= ANI_QUOTE_PAIR && (s[0] == '"' || s[0] == '\'') && s[len - SKIP_ONE] == s[0]) {
        return ani_arena_strndup(a, s + SKIP_ONE, len - PAIR_LEN);
    }
    return ani_arena_strdup(a, s);
}
...
static const char *path_last(ANIArena *a, const char *path) {
    if (!path) {
        return NULL;
    }
    ...
    if (last_sep) {
        return ani_arena_strdup(a, last_sep + SKIP_ONE);
    }
    return path;
}
...
static const char *python_import_root(ANIArena *a, const char *path) {
    if (!path) {
        return NULL;
    }
    const char *dot = strchr(path, '.');
    return dot ? ani_arena_strndup(a, path, (size_t)(dot - path)) : path;
}
...
static void parse_go_import_spec(ANIExtractCtx *ctx, TSNode spec) {
    ANIArena *a = ctx->arena;
    TSNode path_node = ts_node_child_by_field_name(spec, TS_FIELD("path"));
    if (ts_node_is_null(path_node)) {
        return;
    }
    char *path = strip_quotes(a, ani_node_text(a, path_node, ctx->source));
    if (!path || !path[0]) {
        return;
    }
    ...
    const char *local_name =
        !ts_node_is_null(name_node) ? ani_node_text(a, name_node, ctx->source) : path_last(a, path);

    ANIImport imp = {.local_name = local_name, .module_path = path};
    ani_imports_push(&ctx->result->imports, a, imp);
}
...
static void emit_py_aliased_import(ANIExtractCtx *ctx, TSNode child, const char *mod_prefix) {
    ANIArena *a = ctx->arena;
    ...
    char *name = ani_node_text(a, mod_node, ctx->source);
    if (!name || !name[0]) {
        return;
    }
    const char *local = !ts_node_is_null(alias_node) ? ani_node_text(a, alias_node, ctx->source)
                                                     : path_last(a, name);
    const char *full = mod_prefix ? ani_arena_sprintf(a, "%s.%s", mod_prefix, name) : name;
    ANIImport imp = {.local_name = local, .module_path = full};
    ani_imports_push(&ctx->result->imports, a, imp);
}

// Process a single Python import_statement node (import X, import X as Y).
static void process_py_import_stmt(ANIExtractCtx *ctx, TSNode node) {
    ANIArena *a = ctx->arena;
    ...
    if (ts_node_is_null(name_node)) {
        ...
        for (uint32_t j = 0; j < nc; j++) {
            ...
            if (strcmp(ck, "dotted_name") == 0 || strcmp(ck, "identifier") == 0) {
                char *mod = ani_node_text(a, child, ctx->source);
                if (mod && mod[0]) {
                    ANIImport imp = {.local_name = python_import_root(a, mod), .module_path = mod};
                    ani_imports_push(&ctx->result->imports, a, imp);
                }
        ...
        char *mod = ani_node_text(a, name_node, ctx->source);
        if (mod && mod[0]) {
            ANIImport imp = {.local_name = python_import_root(a, mod), .module_path = mod};
            ani_imports_push(&ctx->result->imports, a, imp);
        }
...
static void emit_py_import_from_name(ANIExtractCtx *ctx, TSNode child, const char *mod_path) {
    ANIArena *a = ctx->arena;
    char *name = ani_node_text(a, child, ctx->source);
    if (name && name[0]) {
        const char *full = mod_path ? ani_arena_sprintf(a, "%s.%s", mod_path, name) : name;
        ANIImport imp = {.local_name = name, .module_path = full};
        ani_imports_push(&ctx->result->imports, a, imp);
    }
}

// Process a single Python import_from_statement node (from X import Y [as Z]).
static void process_py_import_from(ANIExtractCtx *ctx, TSNode node) {
    ANIArena *a = ctx->arena;
    ...
    if (strcmp(ts_node_type(node), "future_import_statement") == 0) {
        ANIImport imp = {.local_name = ani_arena_strdup(a, "__future__"),
                         .module_path = ani_arena_strdup(a, "__future__")};
        ani_imports_push(&ctx->result->imports, a, imp);
        return;
    }
    TSNode module_node = resolve_py_module_node(node);
    char *mod_path =
        ts_node_is_null(module_node) ? NULL : ani_node_text(a, module_node, ctx->source);
    ...
    for (uint32_t j = 0; j < nc; j++) {
        ...
        if (strcmp(ck, "identifier") == 0 || strcmp(ck, "dotted_name") == 0) {
            ...
            if (mod_path && mod_path[0]) {
                ANIImport imp = {.local_name = path_last(a, mod_path), .module_path = mod_path};
                ani_imports_push(&ctx->result->imports, a, imp);
                emitted = true;
            }
        }
...
static bool process_named_imports(ANIExtractCtx *ctx, TSNode sub, const char *path) {
    ANIArena *a = ctx->arena;
    bool found = false;
    ...
    for (uint32_t m = 0; m < nc2; m++) {
        ...
        if (!ts_node_is_null(orig)) {
            char *local_name = !ts_node_is_null(local) ? ani_node_text(a, local, ctx->source)
                                                       : ani_node_text(a, orig, ctx->source);
            ANIImport imp = {.local_name = local_name, .module_path = path};
            ani_imports_push(&ctx->result->imports, a, imp);
            found = true;
        }
...
static bool process_import_clause(ANIExtractCtx *ctx, TSNode clause, const char *path) {
    ANIArena *a = ctx->arena;
    bool found = false;
    ...
    for (uint32_t k = 0; k < cc; k++) {
        ...
        if (strcmp(sk, "identifier") == 0) {
            char *name = ani_node_text(a, sub, ctx->source);
            ANIImport imp = {.local_name = name, .module_path = path};
            ani_imports_push(&ctx->result->imports, a, imp);
            found = true;
            ...
            if (!ts_node_is_null(as_name)) {
                char *name = ani_node_text(a, as_name, ctx->source);
                ANIImport imp = {.local_name = name, .module_path = path};
                ani_imports_push(&ctx->result->imports, a, imp);
                found = true;
            }
...
static bool process_es_import_statement(ANIExtractCtx *ctx, TSNode node) {
    ANIArena *a = ctx->arena;
    TSNode source_node = find_es_source_node(node);
    if (ts_node_is_null(source_node)) {
        return false;
    }
    char *path = strip_quotes(a, ani_node_text(a, source_node, ctx->source));
    if (!path || !path[0]) {
        return false;
    }
    uint32_t nc = ts_node_child_count(node);
    ...
    if (!found) {
        ANIImport imp = {.local_name = path_last(a, path), .module_path = path};
        ani_imports_push(&ctx->result->imports, a, imp);
    }
    return true;
}
...
static bool process_commonjs_require(ANIExtractCtx *ctx, TSNode call) {
    ANIArena *a = ctx->arena;
    ...
    char *fn_name = ani_node_text(a, fn, ctx->source);
    ...
    for (uint32_t i = 0; i < argc; i++) {
        ...
        if (strcmp(ak, "string") == 0 || strcmp(ak, "string_literal") == 0 ||
            ...
            path = strip_quotes(a, ani_node_text(a, arg, ctx->source));
            break;
        }
    }
    if (!path || !path[0]) {
    ...
    if (!ts_node_is_null(parent) && strcmp(ts_node_type(parent), "variable_declarator") == 0) {
        ...
        if (!ts_node_is_null(name_node) && strcmp(ts_node_type(name_node), "identifier") == 0) {
            local_name = ani_node_text(a, name_node, ctx->source);
        }
    }
    if (!local_name) {
...
static void walk_es_imports(ANIExtractCtx *ctx, TSNode root) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx, ANI_SZ_512);
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(kind, "import_statement") == 0) {
            ...
            if (!ts_node_is_null(src)) {
                char *path = strip_quotes(ctx->arena, ani_node_text(ctx->arena, src, ctx->source));
                if (path && path[0]) {
                    ANIImport imp = {.local_name = path_last(ctx->arena, path),
                                     .module_path = path};
                    ani_imports_push(&ctx->result->imports, ctx->arena, imp);
                }
            }
    ...
    do {
        ...
        for (uint32_t j = 0; j < nc; j++) {
            ...
            if (strcmp(ck, "scoped_identifier") == 0 || strcmp(ck, "identifier") == 0) {
                char *path = ani_node_text(a, child, ctx->source);
                if (path && path[0]) {
                    ANIImport imp = {.local_name = path_last(a, path), .module_path = path};
                    ani_imports_push(&ctx->result->imports, a, imp);
                }
...
static void parse_rust_imports(ANIExtractCtx *ctx) {
    ...
    do {
        ...
        char *full = ani_node_text(a, node, ctx->source);
        if (!full) {
            continue;
        }
        ...
        ANIImport imp = {.local_name = path_last(a, full), .module_path = full};
        ani_imports_push(&ctx->result->imports, a, imp);
    } while (ts_tree_cursor_goto_next_sibling(&cursor));
    ts_tree_cursor_delete(&cursor);
}
...
static char *strip_angle_brackets(ANIArena *a, char *path) {
    if (path && path[0] == '<') {
        size_t len = strlen(path);
        if (len > SKIP_ONE && path[len - SKIP_ONE] == '>') {
            return ani_arena_strndup(a, path + SKIP_ONE, len - PAIR_LEN);
        }
    }
    return path;
}
...
static const char *ruby_require_method(ANIArena *a, TSNode node, const char *source) {
    ...
    char *name = ani_node_text(a, method, source);
...
static char *extract_ruby_require_arg(ANIArena *a, TSNode node, const char *source) {
    ...
    for (uint32_t j = 0; j < ac; j++) {
        ...
        const char *ck = ts_node_type(c);
        if (strcmp(ck, "string") == 0 || strcmp(ck, "string_literal") == 0) {
            return strip_quotes(a, ani_node_text(a, c, source));
        }
    }
    return strip_quotes(a, ani_node_text(a, args, source));
}
...
static void parse_ruby_imports(ANIExtractCtx *ctx) {
    ...
    do {
        ...
        ANIImport imp = {.local_name = path_last(a, arg_text), .module_path = arg_text};
        ani_imports_push(&ctx->result->imports, a, imp);
    } while (ts_tree_cursor_goto_next_sibling(&cursor));
    ts_tree_cursor_delete(&cursor);
}
...
static void parse_lua_imports(ANIExtractCtx *ctx) {
    ...
    do {
        ...
        char *text = ani_node_text(a, node, ctx->source);
        if (!text) {
            continue;
        }
        ...
        char *mod = ani_arena_strndup(a, start, (size_t)(end - start));
        ANIImport imp = {.local_name = path_last(a, mod), .module_path = mod};
        ani_imports_push(&ctx->result->imports, a, imp);
    } while (ts_tree_cursor_goto_next_sibling(&cursor));
    ts_tree_cursor_delete(&cursor);
}
...
static void r_push_import(ANIExtractCtx *ctx, const char *raw) {
    ANIArena *a = ctx->arena;
    if (!raw || !raw[0]) {
        return;
    }
...
static void r_collect_imports(ANIExtractCtx *ctx, TSNode node) { // NOLINT(misc-no-recursion)
    if (strcmp(ts_node_type(node), "call") == 0) {
        ...
        if (!ts_node_is_null(fn) && !ts_node_is_null(args)) {
            ...
            if (strcmp(ft, "namespace_operator") == 0) {
                ...
                char *lt =
                    ts_node_is_null(lhs) ? NULL : ani_node_text(ctx->arena, lhs, ctx->source);
                char *rt =
                    ts_node_is_null(rhs) ? NULL : ani_node_text(ctx->arena, rhs, ctx->source);
                if (lt && rt && strcmp(lt, "box") == 0 && strcmp(rt, "use") == 0) {
                    ...
                    for (uint32_t i = 0; i < na; i++) {
                        ...
                        if (!ts_node_is_null(val)) {
                            r_push_import(ctx, ani_node_text(ctx->arena, val, ctx->source));
                        }
                    }
                }
...
static bool try_generic_path_fields(ANIExtractCtx *ctx, TSNode node) {
    ANIArena *a = ctx->arena;
...
static void generic_import_from_text(ANIExtractCtx *ctx, TSNode node) {
    ANIArena *a = ctx->arena;
    char *text = ani_node_text(a, node, ctx->source);
    if (!text || !text[0]) {
        return;
    ...
    if (text && text[0]) {
        ANIImport imp = {.local_name = path_last(a, text), .module_path = text};
        ani_imports_push(&ctx->result->imports, a, imp);
    }
}
...
static void extract_one_import_header(ANIExtractCtx *ctx, TSNode header) {
...
static void parse_dart_imports(ANIExtractCtx *ctx) {
    ...
    do {
        ...
        if (find_first_descendant_of(node, "string_literal", &uri)) {
            char *path = strip_quotes(a, ani_node_text(a, uri, ctx->source));
            if (path && path[0]) {
                ANIImport imp = {.local_name = path_last(a, path), .module_path = path};
                ani_imports_push(&ctx->result->imports, a, imp);
            }
...
static void parse_zig_imports(ANIExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "builtin_function") == 0) {
            char *bf = ani_node_text(a, node, ctx->source);
            if (bf && (strncmp(bf, "@import", sizeof("@import") - 1) == 0 ||
                ...
                if (find_first_descendant_of(node, "string", &str)) {
                    char *path = strip_quotes(a, ani_node_text(a, str, ctx->source));
                    if (path && path[0]) {
                        ANIImport imp = {.local_name = path_last(a, path), .module_path = path};
                        ani_imports_push(&ctx->result->imports, a, imp);
                    }
...
static void process_wolfram_get_top(ANIExtractCtx *ctx, TSNode node) {
    ANIArena *a = ctx->arena;
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ck, "string") == 0 || strcmp(ck, "user_symbol") == 0) {
            char *text = ani_node_text(a, child, ctx->source);
            if (text && text[0]) {
...
static void process_wolfram_needs(ANIExtractCtx *ctx, TSNode node) {
    ANIArena *a = ctx->arena;
    ...
    char *name = ani_node_text(a, head, ctx->source);
    if (!name || strcmp(name, "Needs") != 0) {
        return;
    }
    TSNode arg = ts_node_named_child(node, SECOND_IDX);
    char *text = ani_node_text(a, arg, ctx->source);
    if (text && text[0]) {
        char *path = strip_quotes(a, text);
        ANIImport imp = {.local_name = path_last(a, path), .module_path = path};
        ani_imports_push(&ctx->result->imports, a, imp);
...
static void walk_wolfram_imports(ANIExtractCtx *ctx, TSNode root) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx, ANI_SZ_512);
    ts_nstack_push(&stack, root);
...
static void emit_php_use_clause(ANIExtractCtx *ctx, TSNode clause, const char *group_prefix) {
    ANIArena *a = ctx->arena;
    ...
    char *path = ani_node_text(a, path_node, ctx->source);
    if (!path || !path[0]) {
        return;
    }
    if (group_prefix && group_prefix[0]) {
        path = ani_arena_sprintf(a, "%s\\%s", group_prefix, path);
    }
    ...
    const char *local =
        !ts_node_is_null(alias) ? ani_node_text(a, alias, ctx->source) : path_last(a, path);
    ANIImport imp = {.local_name = local, .module_path = path};
    ani_imports_push(&ctx->result->imports, a, imp);
}

static void emit_php_use_decl(ANIExtractCtx *ctx, TSNode decl) {
    ANIArena *a = ctx->arena;
    ...
    if (find_first_descendant_of(decl, "namespace_use_group", &group)) {
        ...
        for (uint32_t i = 0; i < dc; i++) {
            ...
            if (strcmp(ck, "qualified_name") == 0 || strcmp(ck, "namespace_name") == 0 ||
                strcmp(ck, "name") == 0) {
                prefix = ani_node_text(a, c, ctx->source);
                break;
            }
        }
...
static void parse_csharp_imports(ANIExtractCtx *ctx) {
    ...
    do {
        ...
        char *path = found ? ani_node_text(a, path_node, ctx->source) : NULL;
        if (!path || !path[0]) {
...
// `import_node_types` declared in the language's ANILangSpec.  Each root-level
...
// it.  The host's ANILangSpec.embedded_imports declares which content nodes
...
// resolved through ani_ts_language(spec->embedded_language), the same hook
...
static void embedded_collect_content_nodes(TSNode root, const ANIEmbeddedLangSpec *spec,
                                           ANIEmbeddedBlock *out, int *out_count, int max_out) {
        ...
        if (strcmp(kind, spec->script_node_type) == 0) {
            ...
            for (uint32_t k = 0; k < cc; k++) {
                ...
                if (strcmp(ts_node_type(c), spec->content_node_type) == 0) {
                    out[*out_count] = (ANIEmbeddedBlock){.script = node, .content = c};
                    (*out_count)++;
...
static bool vue_embedded_language(ANIExtractCtx *ctx, TSNode script, ANILanguage *language) {
    enum { VUE_ATTR_STACK_CAP = 128 };
    TSNode stack[VUE_ATTR_STACK_CAP];
    int top = 0;
    bool has_lang = false;
    bool lang_supported = true;
    ANILanguage selected = ANI_LANG_JAVASCRIPT;
    stack[top++] = script;
    while (top > 0) {
        TSNode node = stack[--top];
...
static void parse_one_embedded_block(ANIExtractCtx *ctx, const ANIEmbeddedBlock *block,
                                     ANILanguage embedded_language, bool extract_structure) {
    const TSLanguage *language = ani_ts_language(embedded_language);
    ...
    if (extract_structure) {
        ani_extract_definitions_without_module(&sub_ctx);
    }
    walk_es_imports(&sub_ctx, sub_ctx.root);
    if (extract_structure) {
        ani_extract_unified(&sub_ctx);
    }
...
static void parse_embedded_imports(ANIExtractCtx *ctx) {
    const ANILangSpec *spec = ani_lang_spec(ctx->language);
    if (!spec || !spec->embedded_imports) {
        return;
    }
    for (const ANIEmbeddedLangSpec *e = spec->embedded_imports; e->script_node_type != NULL; e++) {
        enum { MAX_EMBEDDED_BLOCKS = 16 };
        ANIEmbeddedBlock hits[MAX_EMBEDDED_BLOCKS];
        int hit_count = 0;
        embedded_collect_content_nodes(ctx->root, e, hits, &hit_count, MAX_EMBEDDED_BLOCKS);
        for (int i = 0; i < hit_count; i++) {
            ANILanguage embedded_language = e->embedded_language;
            ...
            bool extract_structure =
                ctx->language == ANI_LANG_VUE || ctx->language == ANI_LANG_CFML;
...
static void capture_namespace_decl(ANIExtractCtx *ctx) {
    ANIArena *a = ctx->arena;
    ...
    do {
        ...
        for (const char **nk = name_kinds; *nk; nk++) {
            ...
            if (find_first_descendant_of(node, *nk, &nn)) {
                char *ns = ani_node_text(a, nn, ctx->source);
                if (ns && ns[0]) {
        ...
        if (strcmp(ts_node_type(node), "use_statement") == 0) {
            ...
            for (uint32_t j = 0; j < nc; j++) {
                ...
                if (strcmp(ck, "identifier") == 0 || strcmp(ck, "scoped_identifier") == 0) {
                    char *mod = ani_node_text(a, c, ctx->source);
                    if (mod && mod[0]) {
                        ANIImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                        ani_imports_push(&ctx->result->imports, a, imp);
                    }
...
static void parse_powershell_imports(ANIExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "command") == 0) {
            ...
            char *nm = ts_node_is_null(name) ? NULL : ani_node_text(a, name, ctx->source);
            if (nm && strcmp(nm, "using") == 0) {
                ...
                TSNodeStack inner;
                ts_nstack_init(&inner, ctx, ANI_SZ_512);
                ...
                while (inner.count > 0) {
                    ...
                    if (strcmp(ts_node_type(c), "generic_token") == 0) {
                        char *t = ani_node_text(a, c, ctx->source);
                ...
                if (last_tok && last_tok[0]) {
                    ANIImport imp = {.local_name = path_last(a, last_tok),
...
static void lisp_push_module(ANIExtractCtx *ctx, TSNode mod_node) {
    ANIArena *a = ctx->arena;
    ...
    if (strcmp(mk, "symbol") == 0 || strcmp(mk, "string") == 0 || strcmp(mk, "sym_lit") == 0 ||
        ...
        mod = ani_node_text(a, mod_node, ctx->source);
        if (mod) {
            size_t len = strlen(mod);
            if (len >= 2 && mod[0] == '(' && mod[len - 1] == ')') {
                mod = ani_arena_strndup(a, mod + 1, len - 2);
            }
        }
    }
    if (mod && mod[0]) {
...
static char *lisp_strip_kw(ANIArena *a, const char *s) {
    if (!s) {
        return NULL;
    }
    if (s[0] == ':') {
...
static void lisp_set_namespace(ANIExtractCtx *ctx, TSNode name_node) {
    ANIArena *a = ctx->arena;
    char *ns = lisp_strip_kw(a, ani_node_text(a, name_node, ctx->source));
 ...
 * `(:require ...)` / `(:use ...)` list. */
static void lisp_push_clause_modules(ANIExtractCtx *ctx, TSNode clause) {
...
static char *lisp_head_text(ANIExtractCtx *ctx, TSNode list, TSNode *out_head) {
    ...
    return ani_node_text(ctx->arena, head, ctx->source);
}

/* Process one s-expression list as a potential import/namespace form. */
static void lisp_process_list(ANIExtractCtx *ctx, TSNode node) {
    ANIArena *a = ctx->arena;
    (void)a;
    TSNode head = node;
        ...
        for (uint32_t j = 2; j < nc; j++) {
            ...
            if (chn && (strcmp(chn, ":use") == 0 || strstr(chn, "import-from"))) {
                ...
                if (strcmp(c0k, "kwd_lit") == 0) {
                    char *kw = ani_node_text(ctx->arena, ch0, ctx->source);
        ...
        if (strcmp(hn, "include") == 0) {
            for (uint32_t j = 1; j < nc; j++) {
                ...
                if (strcmp(mk, "symbol") == 0 || strcmp(mk, "sym_lit") == 0) {
                    char *sig = ani_node_text(ctx->arena, mod_node, ctx->source);
                    if (sig && sig[0] == '*') {
...
static void parse_starlark_imports(ANIExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "call") == 0) {
            TSNode fn = ts_node_child_by_field_name(node, TS_FIELD("function"));
            char *fname = ts_node_is_null(fn) ? NULL : ani_node_text(a, fn, ctx->source);
                    ...
                    if (find_first_descendant_of(args, "string", &str)) {
                        ...
                        if (path && path[0] && (path[0] == '"' || path[0] == '\'')) {
                            ...
                            if (find_first_descendant_of(str, "string_content", &content)) {
                                path = ani_node_text(a, content, ctx->source);
                            }
                        }
                        if (path && path[0]) {
                            ANIImport imp = {.local_name = path_last(a, path), .module_path = path};
            ...
            if (nm && strcmp(nm, "source") == 0) {
                ...
                if (!ts_node_is_null(args)) {
                    ...
                    for (uint32_t j = 0; j < nc; j++) {
                        ...
                        char *mod = strip_quotes(a, ani_node_text(a, c, ctx->source));
                        if (mod && mod[0]) {
                            ANIImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                            ani_imports_push(&ctx->result->imports, a, imp);
                            break;
...
static void parse_teal_imports(ANIExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "function_call") == 0) {
            ...
            char *cn = ts_node_is_null(callee) ? NULL : ani_node_text(a, callee, ctx->source);
            if (cn && strcmp(cn, "require") == 0) {
                ...
                if (!ts_node_is_null(args) && find_first_descendant_of(args, "string", &str)) {
                    char *mod = strip_quotes(a, ani_node_text(a, str, ctx->source));
                    if (mod && mod[0] && (mod[0] == '"' || mod[0] == '\'')) {
                        TSNode content = str;
                        if (find_first_descendant_of(str, "string_content", &content)) {
                            mod = ani_node_text(a, content, ctx->source);
                        }
                    }
                    if (mod && mod[0]) {
                        ANIImport imp = {.local_name = path_last(a, mod), .module_path = mod};
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "command") == 0) {
            ...
            if (nm && (strcmp(nm, "source") == 0 || strcmp(nm, ".") == 0)) {
                ...
                if (!ts_node_is_null(arg)) {
                    char *mod = strip_quotes(a, ani_node_text(a, arg, ctx->source));
                    if (mod && mod[0]) {
                        ANIImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                        ani_imports_push(&ctx->result->imports, a, imp);
                    }
...
static void css_push_import_from_stmt(ANIExtractCtx *ctx, TSNode stmt) {
    ANIArena *a = ctx->arena;
    TSNode sv = stmt;
    ...
    if (find_first_descendant_of(sv, "string_content", &content)) {
        ...
        path = strip_quotes(a, ani_node_text(a, sv, ctx->source));
    }
    if (!path || !path[0]) {
        return;
    }
...
static void html_extract_tag_src(ANIExtractCtx *ctx, TSNode tag) {
    ANIArena *a = ctx->arena;
    ...
    for (uint32_t j = 0; j < nc; j++) {
        ...
        char *an = ani_node_text(a, name, ctx->source);
        ...
        if (find_first_descendant_of(attr, "attribute_value", &val)) {
            path = ani_node_text(a, val, ctx->source);
        }
        if (path && path[0]) {
            ANIImport imp = {.local_name = path_last(a, path), .module_path = path};
            ani_imports_push(&ctx->result->imports, a, imp);
...
static void push_path_import(ANIExtractCtx *ctx, TSNode node) {
    ANIArena *a = ctx->arena;
    char *path = strip_quotes(a, ani_node_text(a, node, ctx->source));
    if (!path || !path[0]) {
        return;
    }
...
static void push_string_descendant_import(ANIExtractCtx *ctx, TSNode node) {
    TSNode hit = node;
...
static void parse_cmake_imports(ANIExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "normal_command") == 0) {
            ...
            char *cmd = ani_node_text(a, id, ctx->source);
            if (cmd && (strcmp(cmd, "include") == 0 || strcmp(cmd, "add_subdirectory") == 0)) {
                TSNode args = ani_find_child_by_kind(node, "argument_list");
                if (!ts_node_is_null(args)) {
                    uint32_t nc = ts_node_named_child_count(args);
                    for (uint32_t j = 0; j < nc; j++) {
...
static void parse_meson_imports(ANIExtractCtx *ctx) {
    ANIArena *a = ctx->arena;
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx, ANI_SZ_512);
    ts_nstack_push(&stack, ctx->root);
    while (stack.count > 0) {
        ...
        if (strcmp(k, "normal_command") == 0) {
            ...
            if (nc >= 1) {
                ...
                char *fname = ani_node_text(a, fn, ctx->source);
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "import_declaration") == 0) {
            ...
            if (find_first_descendant_of(node, "module_fqn", &fqn)) {
                char *mod = ani_node_text(a, fqn, ctx->source);
                if (mod && mod[0]) {
                    ANIImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                    ani_imports_push(&ctx->result->imports, a, imp);
                }
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(k, "import_decl") == 0 || strcmp(k, "open_expression") == 0) {
            ...
            if (find_first_descendant_of(node, "long_identifier", &id) ||
                ...
                char *mod = ani_node_text(a, id, ctx->source);
                if (mod && mod[0]) {
                    ANIImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                    ani_imports_push(&ctx->result->imports, a, imp);
                }
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "import_clause") == 0) {
            ...
            if (find_first_descendant_of(node, "upper_case_qid", &name)) {
                char *mod = ani_node_text(a, name, ctx->source);
                if (mod && mod[0]) {
...
static void parse_move_imports(ANIExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "use_declaration") == 0) {
            ...
            char *mod = NULL;
            if (!ts_node_is_null(arg)) {
                mod = ani_node_text(a, arg, ctx->source);
...
static char *smali_demangle_descriptor(ANIArena *a, const char *desc) {
    if (!desc || !desc[0]) {
        return NULL;
    }
    ...
    char *out = ani_arena_strndup(a, start, len);
    if (out) {
        for (char *p = out; *p; p++) {
            if (*p == '/') {
                *p = '.';
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(k, "super_directive") == 0 || strcmp(k, "implements_directive") == 0) {
            ...
            if (find_first_descendant_of(node, "class_identifier", &cid)) {
                char *mod = smali_demangle_descriptor(a, ani_node_text(a, cid, ctx->source));
                if (mod && mod[0]) {
                    ANIImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                    ani_imports_push(&ctx->result->imports, a, imp);
                }
        ...
        if (strcmp(ts_node_type(node), "use_clause") == 0) {
            ...
            if (find_first_descendant_of(node, "selected_name", &sel)) {
                ...
                if (!ts_node_is_null(pkg)) {
                    mod = ani_node_text(a, pkg, ctx->source);
                }
                if (mod && mod[0]) {
                    ANIImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                    ani_imports_push(&ctx->result->imports, a, imp);
        ...
        if (strcmp(ts_node_type(node), "use_item") == 0) {
            ...
            if (find_first_descendant_of(node, "use_path", &path)) {
                ...
                if (find_first_descendant_of(path, "id", &idn)) {
                    char *mod = ani_node_text(a, idn, ctx->source);
                    if (mod && mod[0]) {
                        ANIImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                        ani_imports_push(&ctx->result->imports, a, imp);
                    }
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "use_statement") == 0) {
            ...
            if (find_first_descendant_of(node, "absolute_root_shape_id", &sid) ||
                ...
                char *mod = ani_node_text(a, sid, ctx->source);
                if (mod && mod[0]) {
...
static void parse_hyprlang_imports(ANIExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "source") == 0) {
            ...
            char *path = NULL;
            if (find_first_descendant_of(node, "string", &str)) {
                path = strip_quotes(a, ani_node_text(a, str, ctx->source));
            } else {
                path = strip_quotes(a, ani_node_text(a, node, ctx->source));
            }
            if (path && path[0]) {
                ANIImport imp = {.local_name = path_last(a, path), .module_path = path};
                ani_imports_push(&ctx->result->imports, a, imp);
...
void ani_extract_imports(ANIExtractCtx *ctx) {
    ...
    case ANI_LANG_CMAKE:
        ...
        break;
    case ANI_LANG_BITBAKE:
        parse_bitbake_imports(ctx);
        break;
    case ANI_LANG_KCONFIG:
    ...
    case ANI_LANG_DLANG:
        ...
        break;
    case ANI_LANG_TABLEGEN:
        parse_tablegen_imports(ctx);
        break;
    case ANI_LANG_CRYSTAL:
        parse_crystal_imports(ctx);
        break;
    case ANI_LANG_FSHARP:
    ...
    case ANI_LANG_WIT:
        ...
        break;
    case ANI_LANG_SMITHY:
        parse_smithy_imports(ctx);
        break;
    case ANI_LANG_HYPRLANG:
    ...
    default:
        ...
        // import_node_types declared in their ANILangSpec generically.
