 ...
 */
#ifndef ANI_SERVICE_PATTERNS_H
#define ANI_SERVICE_PATTERNS_H

#include <stdbool.h>

/* Edge type returned by pattern match. */
typedef enum {
    ANI_SVC_NONE = 0,      /* Not a service pattern — use normal CALLS */
    ANI_SVC_HTTP = 1,      /* Synchronous HTTP client call */
    ANI_SVC_ASYNC = 2,     /* Async dispatch (message broker, task queue) */
    ANI_SVC_CONFIG = 3,    /* Config/env accessor */
    ANI_SVC_ROUTE_REG = 4, /* Route registration (router.GET, app.get, Route::post) */
    ANI_SVC_GRPC = 5,      /* gRPC client call (protobuf stub invocation) */
    ANI_SVC_GRAPHQL = 6,   /* GraphQL client query/mutation */
    ANI_SVC_TRPC = 7,      /* tRPC client procedure call */
} ani_svc_kind_t;
 ...
 * Returns the pattern kind, or ANI_SVC_NONE if no match.
...
/* Per-worker TLS cache for ani_service_pattern_match results. The
