#include "ani.h"
#include "arena.h" // ANIArena, ani_arena_sprintf
#include "helpers.h"
...
#include "service_patterns.h" // ani_service_pattern_route_method (#952)
...
enum { MAX_STRING_ARG_LEN = ANI_SZ_512 };
...
static const char *lookup_string_constant(const ANIExtractCtx *ctx, const char *name) {
    if (!name || !name[0]) {
        return NULL;
    }
    const ANIStringConstantMap *map = &ctx->string_constants;
...
static const char *lookup_url_builder(const ANIExtractCtx *ctx, const char *name) {
    if (!name || !name[0]) {
        return NULL;
    }
    const ANIStringConstantMap *map = &ctx->string_constants;
...
static const char *strip_quotes(ANIArena *a, const char *text) {
    if (!text || !text[0]) {
        return NULL;
    }
    int len = (int)strlen(text);
    if (len >= ANI_QUOTE_PAIR && (text[0] == '"' || text[0] == '\'')) {
        return ani_arena_strndup(a, text + ANI_QUOTE_OFFSET, (size_t)(len - ANI_QUOTE_PAIR));
    }
    return text;
}
...
// rather than in ANILangSpec to avoid -Wmissing-field-initializers across 155
// language rows.
const char **ani_string_dispatch_suffixes(ANILanguage lang) {
...
static char *extract_callee_name(ANIArena *a, TSNode node, const char *source, ANILanguage lang);
static char *gotemplate_callee(ANIArena *a, TSNode node, const char *source);
static const char *strip_and_validate_string_arg(ANIArena *a, char *text);
 ...
 * Without this, ani_node_text returns full text including args/parens.
 * Iteratively walks the chain: selector → operand(call) → function(selector) → ... */
static char *resolve_chained_selector(ANIArena *a, TSNode sel, const char *source) {
    ...
    if (ts_node_is_null(operand) || ts_node_is_null(field) ||
        ...
        return ani_node_text(a, sel, source);
    }
    ...
    char *method = ani_node_text(a, field, source);
    ...
    for (int depth = 0; depth < MAX_CHAIN_DEPTH; depth++) {
        ...
        char *base = ani_node_text(a, fn, source);
        if (base && method) {
            return ani_arena_sprintf(a, "%s.%s", base, method);
        }
        return method;
    }
...
static char *extract_constructor_callee(ANIArena *a, TSNode node, const char *source,
                                        const char *nk) {
    ...
    for (const char **f = type_fields; *f; f++) {
        ...
        if (!ts_node_is_null(tn)) {
            ...
            char *t = strip_generic_args(ani_node_text(a, tn, source));
            if (t && t[0]) {
                return t;
            }
        }
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ck, "type_identifier") == 0 || strcmp(ck, "identifier") == 0 ||
            ...
            char *t = strip_generic_args(ani_node_text(a, child, source));
            if (t && t[0]) {
                return t;
            }
        }
...
static char *extract_callee_from_fields(ANIArena *a, TSNode node, const char *source) {
    // Try "function" field
    ...
    if (!ts_node_is_null(func_node)) {
        ...
        if (strcmp(fk, "identifier") == 0 || strcmp(fk, "simple_identifier") == 0 ||
            ...
            return ani_node_text(a, func_node, source);
        }
        ...
        if (strcmp(fk, "extract_operator") == 0) {
            ...
            if (!ts_node_is_null(rhs)) {
                char *rt = ani_node_text(a, rhs, source);
                if (!ts_node_is_null(lhs)) {
                    char *lt = ani_node_text(a, lhs, source);
                    if (lt && lt[0] && rt && rt[0]) {
                        return ani_arena_sprintf(a, "%s.%s", lt, rt);
                    }
                }
                return rt;
            }
    ...
    if (!ts_node_is_null(name_node)) {
        ...
        if (!ts_node_is_null(obj) && name) {
            char *obj_text = ani_node_text(a, obj, source);
            if (obj_text && obj_text[0]) {
                return ani_arena_sprintf(a, "%s.%s", obj_text, name);
            }
        }
        return name;
    }
    ...
    if (!ts_node_is_null(method_node)) {
        char *method = ani_node_text(a, method_node, source);
        TSNode recv = ts_node_child_by_field_name(node, TS_FIELD("receiver"));
        if (!ts_node_is_null(recv) && method) {
            char *recv_text = ani_node_text(a, recv, source);
            if (recv_text && recv_text[0]) {
                return ani_arena_sprintf(a, "%s.%s", recv_text, method);
            }
        }
        return method;
    }
...
static char *extract_fp_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    ...
    while (fp_is_apply_kind(nk) && ts_node_child_count(node) > 0) {
        ...
        if (strcmp(ck, "identifier") == 0 || strcmp(ck, "variable") == 0 ||
            ...
            return ani_node_text(a, callee, source);
        }
    ...
    if (strcmp(nk, "infix") == 0 || strcmp(nk, "infix_expression") == 0) {
        ...
        if (!ts_node_is_null(op)) {
            return ani_node_text(a, op, source);
        }
        enum { INFIX_MIN_CHILDREN = 3, INFIX_OP_IDX = 1 };
        if (ts_node_child_count(node) >= INFIX_MIN_CHILDREN) {
            return ani_node_text(a, ts_node_child(node, INFIX_OP_IDX), source);
        }
    }
    return NULL;
}
...
static char *extract_wolfram_callee(ANIArena *a, TSNode node, const char *source) {
    ...
    if (ts_node_named_child_count(node) > 0) {
        ...
        if (strcmp(hk, "user_symbol") == 0 || strcmp(hk, "builtin_symbol") == 0) {
            return ani_node_text(a, head, source);
        }
    }
    return NULL;
}
...
static char *extract_swift_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_scripting_callee(ANIArena *a, TSNode node, const char *source,
                                      ANILanguage lang, const char *nk) {
    if (lang == ANI_LANG_ELIXIR && strcmp(nk, "binary_operator") == 0) {
        ...
        char *operator_name = ani_node_text(a, op, source);
    ...
    if (lang == ANI_LANG_ELIXIR && strcmp(nk, "call") == 0 && ts_node_child_count(node) > 0) {
        TSNode first = ts_node_child(node, 0);
        const char *fk = ts_node_type(first);
        if (strcmp(fk, "identifier") == 0 || strcmp(fk, "dot") == 0) {
            return ani_node_text(a, first, source);
        }
        return NULL;
    }
    if (lang == ANI_LANG_PERL && ts_node_child_count(node) > 0) {
        ...
        char *pn = ani_node_text(a, name_node, source);
    ...
    if (lang == ANI_LANG_PHP) {
        ...
        char *pn = ts_node_is_null(func_node) ? NULL : ani_node_text(a, func_node, source);
        return pn;
    }
    if (lang == ANI_LANG_KOTLIN && ts_node_child_count(node) > 0) {
        return ani_node_text(a, ts_node_child(node, 0), source);
    }
    if (lang == ANI_LANG_MATLAB && strcmp(nk, "command") == 0 && ts_node_child_count(node) > 0) {
        return ani_node_text(a, ts_node_child(node, 0), source);
    }
    return NULL;
}
...
static char *extract_objc_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "message_expression") != 0) {
        return NULL;
    }
    TSNode selector = ts_node_child_by_field_name(node, TS_FIELD("selector"));
    return ts_node_is_null(selector) ? NULL : ani_node_text(a, selector, source);
}

// Erlang: extract callee from call node's first child.
static char *extract_erlang_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
...
static bool lisp_definition_head(ANILanguage lang, TSNode head, const char *source) {
    ...
    return call_node_text_in(head, source,
                             lang == ANI_LANG_COMMONLISP ? common_lisp_heads
                                                         : clojure_scheme_racket_heads);
}

static bool lisp_list_is_definition_role(ANILanguage lang, TSNode node, const char *source) {
    ...
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
        ...
        if (lang == ANI_LANG_COMMONLISP &&
            (strcmp(parent_kind, "defun_header") == 0 || strcmp(parent_kind, "lambda_list") == 0)) {
            return true;
        }
        if (lang == ANI_LANG_EMACSLISP && (strcmp(parent_kind, "function_definition") == 0 ||
...
static bool call_node_is_definition_container(ANILanguage lang, TSNode node, const char *source) {
    const char *kind = ts_node_type(node);
    if ((lang == ANI_LANG_CLOJURE || lang == ANI_LANG_SCHEME || lang == ANI_LANG_RACKET ||
         lang == ANI_LANG_COMMONLISP || lang == ANI_LANG_EMACSLISP) &&
    ...
    if (lang == ANI_LANG_TYPST && strcmp(kind, "call") == 0) {
        return typst_call_is_let_pattern(node);
    }
    if (lang == ANI_LANG_AGDA && strcmp(kind, "expr") == 0) {
        return agda_expr_is_definition_role(node);
    }
    return lang == ANI_LANG_ELIXIR && strcmp(kind, "call") == 0 &&
...
static bool chialisp_node_is_binder_list(ANIArena *a, TSNode node, const char *source) {
    ...
    for (int level = 0; level < 2; level++) {
        ...
        TSNode head = ani_lisp_named_child_skip_comments(parent, 0);
        if (!ts_node_is_null(head) && strcmp(ts_node_type(head), "symbol") == 0) {
            char *ht = ani_node_text(a, head, source);
            if (!ht) {
                return false;
            }
            ...
            if (strcmp(ht, "defun") == 0 || strcmp(ht, "defun-inline") == 0 ||
                ...
                TSNode params = ani_lisp_named_child_skip_comments(parent, 2);
            ...
            if (strcmp(ht, "mod") == 0 || strcmp(ht, "lambda") == 0 || strcmp(ht, "let") == 0 ||
                strcmp(ht, "let*") == 0) {
                TSNode binder = ani_lisp_named_child_skip_comments(parent, 1);
...
static char *extract_lisp_callee(ANIArena *a, TSNode node, const char *source, const char *nk,
                                 ANILanguage lang) {
...
static char *extract_fsharp_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_css_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "call_expression") != 0) {
        return NULL;
    }
    TSNode fn = ani_find_child_by_kind(node, "function_name");
    return ts_node_is_null(fn) ? NULL : ani_node_text(a, fn, source);
}
...
static char *extract_linkerscript_callee(ANIArena *a, TSNode node, const char *source,
                                         const char *nk) {
    if (strcmp(nk, "call_expression") != 0) {
        return NULL;
    }
    ...
    return ani_node_text(a, function, source);
}

// PowerShell: a `command` node's callee is its `command_name` child.
static char *extract_powershell_callee(ANIArena *a, TSNode node, const char *source,
                                       const char *nk) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (strcmp(ts_node_type(c), "command_name") == 0) {
            return ani_node_text(a, c, source);
        }
    }
    return NULL;
}
...
static char *extract_ada_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
 ...
 * ref_name alone. */
static char *extract_plsql_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "ref_call") != 0) {
        return NULL;
    }
    TSNode ref = ani_find_child_by_kind(node, "referenced_element");
    ...
    if (!ts_node_is_null(parent) && !ts_node_is_null(name)) {
        char *p = ani_node_text(a, parent, source);
        char *n = ani_node_text(a, name, source);
        if (p && n && p[0] && n[0]) {
            return ani_arena_sprintf(a, "%s.%s", p, n);
        }
    }
    if (!ts_node_is_null(name)) {
        return ani_node_text(a, name, source);
    }
    return ani_node_text(a, ref, source);
}
...
static char *extract_solidity_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_groovy_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_wgsl_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_dart_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "selector") == 0) {
        ...
        if (!ts_node_is_null(prev) && strcmp(ts_node_type(prev), "identifier") == 0) {
            return ani_node_text(a, prev, source);
        }
        return NULL;
    }
...
static char *extract_scss_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "include_statement") == 0) {
        TSNode id = ani_find_child_by_kind(node, "identifier");
        return ts_node_is_null(id) ? NULL : ani_node_text(a, id, source);
    }
    ...
    if (strcmp(nk, "call_expression") == 0) {
        TSNode fn = ani_find_child_by_kind(node, "function_name");
        if (!ts_node_is_null(fn)) {
            return ani_node_text(a, fn, source);
        }
    }
    return NULL;
}
...
static char *extract_sql_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    ...
    return ts_node_is_null(nm) ? NULL : ani_node_text(a, nm, source);
}
...
static char *extract_cobol_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "call_statement") != 0) {
        return NULL;
    }
    TSNode x = ts_node_child_by_field_name(node, TS_FIELD("x"));
    if (ts_node_is_null(x)) {
        x = ani_find_child_by_kind(node, "string");
    }
    if (ts_node_is_null(x)) {
        return NULL;
    }
    char *text = ani_node_text(a, x, source);
...
static char *extract_elm_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    ...
    TSNode ve = strcmp(ts_node_type(target), "value_expr") == 0
                    ? target
                    : ani_find_child_by_kind(target, "value_expr");
    if (ts_node_is_null(ve)) {
        return NULL;
    }
    TSNode qid = ts_node_child_by_field_name(ve, TS_FIELD("name"));
    if (ts_node_is_null(qid)) {
        qid = ani_find_child_by_kind(ve, "value_qid");
    }
    if (ts_node_is_null(qid)) {
        return NULL;
    }
    TSNode id = ani_find_child_by_kind(qid, "lower_case_identifier");
    if (ts_node_is_null(id)) {
        // module-qualified call: emit the whole qualified id text
        return ani_node_text(a, qid, source);
    }
    return ani_node_text(a, id, source);
}
...
static char *extract_jsonnet_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "functioncall") != 0) {
        return NULL;
    }
    TSNode id = ani_find_child_by_kind(node, "id");
    return ts_node_is_null(id) ? NULL : ani_node_text(a, id, source);
}
...
// nickel_call_types.)
static char *extract_nickel_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    ...
    for (int depth = 0; depth < NICKEL_APPLY_DEPTH && !ts_node_is_null(cur); depth++) {
        ...
        if (strcmp(ck, "ident") == 0) {
            return ani_node_text(a, cur, source);
        }
...
// (`utils.fallback(a)` -> "utils.fallback", module-qualified, which ani.c
...
static char *extract_pkl_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "newExpr") == 0) {
        // `new { ... }` with an inferred type has no declaredType child.
        TSNode dt = ani_find_child_by_kind(node, "declaredType");
        return ts_node_is_null(dt) ? NULL : ani_node_text(a, dt, source);
    }
    ...
    if (ts_node_is_null(ani_find_child_by_kind(node, "argumentList"))) {
        return NULL;
    }
    ...
    char *mn = ani_node_text(a, name, source);
    if (!mn || !mn[0]) {
        return NULL;
    }
    ...
    if (strcmp(ts_node_type(recv), "unqualifiedAccessExpr") == 0 &&
        ts_node_is_null(ani_find_child_by_kind(recv, "argumentList"))) {
        char *rt = ani_node_text(a, recv, source);
        if (rt && rt[0]) {
            return ani_arena_sprintf(a, "%s.%s", rt, mn);
        }
    }
    return mn;
}
...
static char *extract_typst_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "call") != 0) {
        return NULL;
    }
    TSNode item = ts_node_child_by_field_name(node, TS_FIELD("item"));
    return ts_node_is_null(item) ? NULL : ani_node_text(a, item, source);
}

// Meson: a builtin invocation (`executable(...)`, `dependency(...)`) is a
// `normal_command` whose `command` field is the called identifier.
static char *extract_meson_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "normal_command") != 0) {
        return NULL;
    }
    TSNode cmd = ts_node_child_by_field_name(node, TS_FIELD("command"));
    return ts_node_is_null(cmd) ? NULL : ani_node_text(a, cmd, source);
}
...
static char *first_leaf_identifier(ANIArena *a, TSNode node, const char *source) {
    TSNode cur = node;
    for (int depth = 0; depth < 8 && !ts_node_is_null(cur); depth++) {
        ...
        if (strcmp(k, "simple_identifier") == 0 || strcmp(k, "identifier") == 0 ||
            ...
            char *t = ani_node_text(a, cur, source);
            return (t && t[0]) ? t : NULL;
        }
...
static char *extract_hdl_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_vhdl_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    ...
    if (strcmp(pk, "library_function") == 0 || strcmp(pk, "identifier") == 0 ||
        ...
        char *t = ani_node_text(a, prev, source);
        return (t && t[0]) ? t : NULL;
    }
    return NULL;
}
...
static char *extract_nasm_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    ...
    char *m = ani_node_text(a, mnem, source);
...
static char *extract_llvm_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    ...
    if (!t) {
        t = ani_node_text(a, callee, source);
    }
    if (t && (t[0] == '@' || t[0] == '%')) {
        return t + 1;
    }
...
static char *extract_func_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_nix_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_agda_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "expr") != 0) {
        return NULL;
    }
    TSNode qid = agda_head_qid(agda_application_head_atom(node));
    return ts_node_is_null(qid) ? NULL : ani_node_text(a, qid, source);
}
...
static char *extract_make_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "shell_function") == 0) {
        return ani_arena_strndup(a, "shell", 5);
    }
...
static char *extract_just_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_puppet_callee(ANIArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "include_statement") == 0) {
        return ani_arena_strndup(a, "include", 7);
    }
    if (strcmp(nk, "function_call") == 0) {
        if (ts_node_named_child_count(node) > 0) {
            TSNode head = ts_node_named_child(node, 0);
...
static char *extract_callee_lang_specific(ANIArena *a, TSNode node, const char *source,
                                          ANILanguage lang) {
    const char *nk = ts_node_type(node);

    if (lang == ANI_LANG_FORTRAN && strcmp(nk, "subroutine_call") == 0) {
        TSNode subroutine = ts_node_child_by_field_name(node, TS_FIELD("subroutine"));
        if (!ts_node_is_null(subroutine)) {
            return ani_node_text(a, subroutine, source);
        }
    }
    ...
    if (lang == ANI_LANG_PYTHON && strcmp(nk, "call") == 0) {
    ...
    if (lang == ANI_LANG_ERLANG) {
        return extract_erlang_callee(a, node, source, nk);
    }
    if (lang == ANI_LANG_HASKELL || lang == ANI_LANG_OCAML || lang == ANI_LANG_PURESCRIPT ||
        lang == ANI_LANG_SCALA) {
        return extract_fp_callee(a, node, source, nk);
    }
    if (lang == ANI_LANG_WOLFRAM && strcmp(nk, "apply") == 0) {
        return extract_wolfram_callee(a, node, source);
    }
    if (lang == ANI_LANG_SWIFT) {
    ...
    if (lang == ANI_LANG_OBJECTSCRIPT_UDL || lang == ANI_LANG_OBJECTSCRIPT_ROUTINE) {
        ...
        if (strcmp(nk, "class_method_call") == 0) {
            TSNode class_ref = ani_find_child_by_kind(node, "class_ref");
            TSNode method_name = ani_find_child_by_kind(node, "method_name");
            if (!ts_node_is_null(class_ref) && !ts_node_is_null(method_name)) {
                TSNode cname = ani_find_child_by_kind(class_ref, "class_name");
                if (ts_node_is_null(cname)) {
                    return NULL;
                }
                char *cls = ani_node_text(a, cname, source);
                if (!cls || !cls[0]) {
                    return NULL;
                }
                ...
                char *meth = ani_node_text(a, mname_ident, source);
                if (!meth || !meth[0]) {
                    return cls;
                }
                return ani_arena_sprintf(a, "%s.%s", cls, meth);
            }
            return NULL;
        }
        ...
        if (strcmp(nk, "extrinsic_function") == 0 || strcmp(nk, "routine_tag_call") == 0) {
            TSNode line_ref = ani_find_child_by_kind(node, "line_ref");
            if (!ts_node_is_null(line_ref)) {
                return ani_node_text(a, line_ref, source);
            }
            return NULL;
        }
        // $$$Macro(...) -> raw "$$$Name" callee (expanded later in handle_calls)
        if (strcmp(nk, "macro") == 0) {
            char *raw = ani_node_text(a, node, source);
            ...
            return ani_arena_sprintf(a, "$$$%s", name_start);
        }
        return NULL;
    }
...
static const char *php_chain_prefix_arg(ANIArena *a, TSNode call_node, const char *source) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ts_node_type(inner), "string") == 0 ||
            ...
            char *txt = ani_node_text(a, inner, source);
            if (txt && txt[0]) {
...
static const char *php_group_prefix_for_call(ANIArena *a, TSNode node, const char *source) {
    ...
    for (int depth = 0; depth < PHP_GROUP_WALK_MAX && !ts_node_is_null(cur); depth++) {
        if (strcmp(ts_node_type(cur), "anonymous_function") == 0 ||
            ...
            if (!ts_node_is_null(p) && (strcmp(ts_node_type(p), "member_call_expression") == 0 ||
                ...
                char *gtxt = ts_node_is_null(gname) ? NULL : ani_node_text(a, gname, source);
                if (gtxt && strcmp(gtxt, "group") == 0) {
                    ...
                    for (int hops = 0; hops < PHP_GROUP_WALK_MAX && !ts_node_is_null(recv);
                        ...
                        if (strcmp(rk, "member_call_expression") == 0 ||
                            ...
                            char *rtxt =
                                ts_node_is_null(rname) ? NULL : ani_node_text(a, rname, source);
    ...
    buf[pos] = '\0';
    return pos ? ani_arena_strndup(a, buf, pos) : NULL;
}

static bool is_nested_verilog_call_wrapper(ANILanguage lang, TSNode node) {
    if (lang != ANI_LANG_VERILOG || strcmp(ts_node_type(node), "subroutine_call") != 0) {
        return false;
    }
...
static char *extract_callee_name(ANIArena *a, TSNode node, const char *source, ANILanguage lang) {
    ...
    if (lang == ANI_LANG_LEAN && strcmp(ts_node_type(node), "apply") == 0) {
    ...
    if (lang == ANI_LANG_RUBY) {
        ...
        if (!ts_node_is_null(m) && !ts_node_is_null(recv) &&
            ...
            char *mt = ani_node_text(a, m, source);
     ...
     * not recognized. */
    if (lang == ANI_LANG_PHP && strcmp(ts_node_type(node), "scoped_call_expression") == 0) {
        ...
        if (!ts_node_is_null(scope) && !ts_node_is_null(mname)) {
            char *sc = ani_node_text(a, scope, source);
            char *mn = ani_node_text(a, mname, source);
            if (sc && mn && strcmp(sc, "Route") == 0) {
                char *qual = ani_arena_sprintf(a, "%s::%s", sc, mn);
                if (qual && ani_service_pattern_route_method(qual) != NULL) {
                    return qual;
                }
            }
        }
...
static const char *strip_and_validate_string_arg(ANIArena *a, char *text) {
    ...
    int len = (int)strlen(text);
    if (len >= ANI_QUOTE_PAIR && (text[0] == '"' || text[0] == '\'')) {
        text = ani_arena_strndup(a, text + ANI_QUOTE_OFFSET, (size_t)(len - ANI_QUOTE_PAIR));
        len -= ANI_QUOTE_PAIR;
    }
...
static char *gotemplate_string_child(ANIArena *a, TSNode parent, const char *source) {
    TSNode s = ani_find_child_by_kind(parent, "interpreted_string_literal");
    if (ts_node_is_null(s)) {
        return NULL;
    }
    char *text = ani_node_text(a, s, source);
...
static char *gotemplate_callee(ANIArena *a, TSNode node, const char *source) {
    ...
    if (strcmp(k, "function_call") == 0) {
        TSNode fn = ani_find_child_by_kind(node, "identifier");
        if (ts_node_is_null(fn)) {
            return NULL;
        }
        char *fname = ani_node_text(a, fn, source);
        ...
        if (ts_node_is_null(args)) {
            args = ani_find_child_by_kind(node, "argument_list");
        }
        if (ts_node_is_null(args)) {
            return NULL;
        }
...
static const char *extract_nth_string_arg(ANIExtractCtx *ctx, TSNode args, uint32_t n) {
    uint32_t nc = ts_node_named_child_count(args);
    uint32_t found = 0;
    for (uint32_t ai = 0; ai < nc && ai < MAX_POSITIONAL_SCAN + n; ai++) {
        ...
        if (is_string_like(ak)) {
            if (found == n) {
                char *text = ani_node_text(ctx->arena, arg, ctx->source);
...
static void process_keyword_arg(ANIExtractCtx *ctx, TSNode arg_node, ANICallArg *ca) {
    ...
    if (!ts_node_is_null(key_n)) {
        ca->keyword = ani_node_text(ctx->arena, key_n, ctx->source);
    }
    if (!ts_node_is_null(val_n)) {
        ca->expr = ani_node_text(ctx->arena, val_n, ctx->source);
...
static void extract_call_args(ANIExtractCtx *ctx, TSNode args, ANICall *call) {
    uint32_t argc = ts_node_named_child_count(args);
    int positional_idx = 0;
    for (uint32_t ai = 0; ai < argc && call->arg_count < ANI_MAX_CALL_ARGS; ai++) {
        TSNode arg_node = ts_node_named_child(args, ai);
        const char *ak = ts_node_type(arg_node);
        ANICallArg *ca = &call->args[call->arg_count];
        ...
        if (strcmp(ak, "keyword_argument") == 0 || strcmp(ak, "pair") == 0) {
            ...
            ca->expr = ani_node_text(ctx->arena, arg_node, ctx->source);
            ca->index = positional_idx++;
            if (is_string_like(ak) && ca->expr) {
                ...
                ca->value = ani_template_string_text(ctx->arena, arg_node, ctx->source);
                ...
                if (!ts_node_is_null(fn) && strcmp(ts_node_type(fn), "identifier") == 0) {
                    char *fname = ani_node_text(ctx->arena, fn, ctx->source);
                    if (fname) {
...
static const char *extract_string_value(ANIExtractCtx *ctx, TSNode val_node) {
    const char *vk = ts_node_type(val_node);
    if (strcmp(vk, "template_string") == 0) {
        return ani_template_string_text(ctx->arena, val_node, ctx->source);
    }
    if (is_string_like(vk)) {
        char *text = ani_node_text(ctx->arena, val_node, ctx->source);
        if (text && text[0]) {
            return strip_quotes(ctx->arena, text);
        }
    } else if (strcmp(vk, "identifier") == 0) {
        char *const_name = ani_node_text(ctx->arena, val_node, ctx->source);
        if (const_name) {
...
static const char *extract_composite_queue_field(ANIExtractCtx *ctx, TSNode node) {
    ...
    if (ts_node_is_null(body)) {
        body = ani_find_child_by_kind(node, "literal_value");
    }
    if (ts_node_is_null(body)) {
        return NULL;
    }
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        char *key = ani_node_text(ctx->arena, key_n, ctx->source);
        ...
        char *raw = ani_node_text(ctx->arena, val_n, ctx->source);
        if (raw && raw[0]) {
            return raw;
        }
    }
...
static const char *extract_keyword_url(ANIExtractCtx *ctx, TSNode arg) {
    ...
    char *key = ani_node_text(ctx->arena, key_node, ctx->source);
...
static const char *extract_binary_concat_suffix(ANIExtractCtx *ctx, TSNode node) {
    TSNode op_node = ts_node_child_by_field_name(node, TS_FIELD("operator"));
    if (!ts_node_is_null(op_node)) {
        char *op = ani_node_text(ctx->arena, op_node, ctx->source);
        if (!op || strcmp(op, "+") != 0) {
            return NULL;
        }
    }
    ...
    return strip_and_validate_string_arg(ctx->arena, ani_node_text(ctx->arena, rhs, ctx->source));
}

// Try to extract URL/topic from a positional argument (string or constant).
static const char *extract_positional_url(ANIExtractCtx *ctx, TSNode arg, const char *ak) {
    ...
    if (strcmp(ak, "template_string") == 0) {
        const char *flat = ani_template_string_text(ctx->arena, arg, ctx->source);
        if (flat) {
    ...
    if (strcmp(ak, "identifier") == 0) {
        char *const_name = ani_node_text(ctx->arena, arg, ctx->source);
        if (const_name) {
            return lookup_string_constant(ctx, const_name);
        }
    }
...
static const char *extract_url_or_topic_arg(ANIExtractCtx *ctx, TSNode args) {
    uint32_t nc = ts_node_named_child_count(args);
...
static const char *normalize_string_handler(ANIArena *a, const char *raw) {
    ...
    if (at && at[1]) {
        return ani_arena_strdup(a, at + 1);
    }
    return unq;
}

static const char *extract_handler_arg(ANIExtractCtx *ctx, TSNode args) {
    ...
    for (uint32_t ai = HANDLER_START_IDX; ai < nc; ai++) {
        ...
        if (strcmp(ak2, "identifier") == 0 || strcmp(ak2, "member_expression") == 0 ||
            ...
            handler = ani_node_text(ctx->arena, arg2, ctx->source);
            continue;
        }
        if (is_string_like(ak2)) {
            const char *h =
                normalize_string_handler(ctx->arena, ani_node_text(ctx->arena, arg2, ctx->source));
            if (h && h[0]) {
                handler = h;
            }
        }
...
static void extract_jsx_component_ref(ANIExtractCtx *ctx, TSNode node, const char *kind,
    ...
    if (name && name[0] >= 'A' && name[0] <= 'Z') {
        ...
        ani_calls_push(&ctx->result->calls, ctx->arena, call);
    }
}
...
static bool kotlin_operator_call_matches(const ANICall *existing, const char *callee_name,
...
static void push_kotlin_operator_call(ANIExtractCtx *ctx, TSNode node, const char *callee_name,
                                      const char *enclosing_func_qn) {
    ...
    if (ctx->result->calls.count > 0) {
        const ANICall *existing = &ctx->result->calls.items[ctx->result->calls.count - SKIP_ONE];
...
static void extract_kotlin_operator_call(ANIExtractCtx *ctx, TSNode node, const char *kind,
                                         const char *enclosing_func_qn) {
...
static void kt_push_implicit_call(ANIExtractCtx *ctx, TSNode node, const char *callee,
                                  const char *enclosing_func_qn) {
    ANICall call = {0};
...
static void push_cpp_semantic_call_candidate(ANIExtractCtx *ctx, TSNode node,
...
static const char *cpp_operator_token(ANIExtractCtx *ctx, TSNode node) {
    for (uint32_t i = 0; i < ts_node_child_count(node); i++) {
        TSNode child = ts_node_child(node, i);
        if (!ts_node_is_named(child)) {
            const char *token = ani_node_text(ctx->arena, child, ctx->source);
            if (token && token[0]) {
                return token;
            }
        }
...
static void extract_cpp_operator_call(ANIExtractCtx *ctx, TSNode node, const char *kind,
                                      const char *enclosing_func_qn) {
    ...
    if (strcmp(kind, "unary_expression") == 0 || strcmp(kind, "pointer_expression") == 0 ||
        ...
        if (!ts_node_is_null(operand) &&
            ...
            push_cpp_semantic_call_candidate(
                ctx, node, ani_arena_sprintf(ctx->arena, "operator%s", token), enclosing_func_qn);
        }
        return;
    }
...
static void extract_cpp_implicit_calls(ANIExtractCtx *ctx, TSNode node, const char *kind,
                                       const char *enclosing_func_qn) {
    ...
    if (strcmp(kind, "delete_expression") == 0) {
        ...
        if (!ts_node_is_null(type) && !ts_node_is_null(decl) &&
            ...
            if (!ts_node_is_null(value) && strcmp(ts_node_type(value), "identifier") == 0) {
                char *tn = ani_node_text(ctx->arena, type, ctx->source);
                if (tn) {
...
static void extract_kotlin_desugared_calls(ANIExtractCtx *ctx, TSNode node, const char *kind,
                                           const char *enclosing_func_qn) {
    if (strcmp(kind, "property_declaration") == 0) {
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            for (uint32_t j = 0; j < vc; j++) {
                ...
                comp++;
                kt_push_implicit_call(ctx, node, ani_arena_sprintf(ctx->arena, "component%u", comp),
                                      enclosing_func_qn);
            }
            break;
        }
...
static char *resolve_objectscript_instance_call(ANIArena *a, TSNode node, const char *source,
    ...
    TSNode method_name_node = ani_find_child_by_kind(oref, "method_name");
    ...
    char *method = ani_node_text(a, mn_ident, source);
    if (!method || !method[0]) {
        return NULL;
    }
    ...
    if (nk_first && strcmp(nk_first, "relative_dot_property") == 0) {
        TSNode prop_name = ani_find_child_by_kind(receiver, "member_name");
        if (!ts_node_is_null(prop_name)) {
            char *pname = ani_node_text(a, prop_name, source);
            if (pname && pname[0]) {
                var_text = ani_arena_sprintf(a, "..%s", pname);
            }
        }
        if (!var_text) {
            var_text = ani_node_text(a, receiver, source);
        }
    } else {
        var_text = ani_node_text(a, receiver, source);
    }
    ...
    for (int i = 0; i < type_map->count; i++) {
        if (strcasecmp(type_map->entries[i].var_name, var_text) == 0) {
            return ani_arena_sprintf(a, "%s.%s", type_map->entries[i].class_name, method);
        }
    }
    return NULL;
}
...
static bool python_receiver_is_exempt(ANIExtractCtx *ctx, TSNode receiver) {
    if (ts_node_is_null(receiver)) {
        return false;
    }
    ...
    if (strcmp(ts_node_type(receiver), "call") == 0) {
        ...
        if (!ts_node_is_null(fn) && strcmp(ts_node_type(fn), "identifier") == 0) {
            char *name = ani_node_text(ctx->arena, fn, ctx->source);
    ...
    char *name = ani_node_text(ctx->arena, root, ctx->source);
    if (!name) {
        return false;
    }
...
static bool python_callee_is_bound_parameter(ANIExtractCtx *ctx, WalkState *state,
                                             TSNode callee_ident) {
    const char *callee_name = ani_node_text(ctx->arena, callee_ident, ctx->source);
    return ani_walk_python_param_is_bound(state, callee_name);
}

static bool is_objectscript_language(ANILanguage language) {
    return language == ANI_LANG_OBJECTSCRIPT_UDL || language == ANI_LANG_OBJECTSCRIPT_ROUTINE;
}
...
static const char *resolve_objectscript_callee(ANIExtractCtx *ctx, TSNode node, WalkState *state,
                                               const char *callee) {
    ...
    if ((!callee || !callee[0]) && strcmp(kind, "relative_dot_method") == 0 &&
        ...
        TSNode oref = ani_find_child_by_kind(node, "oref_method");
        TSNode method_name =
            ts_node_is_null(oref) ? (TSNode){0} : ani_find_child_by_kind(oref, "method_name");
        TSNode identifier =
            !ts_node_is_null(method_name) && ts_node_named_child_count(method_name) > 0
                ? ts_node_named_child(method_name, 0)
                : (TSNode){0};
        if (!ts_node_is_null(identifier)) {
            char *method = ani_node_text(ctx->arena, identifier, ctx->source);
            if (method && method[0]) {
                callee = ani_arena_sprintf(ctx->arena, "%s.%s", state->enclosing_class_qn, method);
            }
        }
    }

    if (callee && strncmp(callee, "$$$", 3) == 0 && ctx->macro_table) {
        const ANIMacroEntry *entry = ani_macro_table_find(ctx->macro_table, callee + 3);
        if (entry) {
            if (entry->resolved_callee) {
                callee = ani_arena_strdup(ctx->arena, entry->resolved_callee);
            } else if (entry->expansion) {
                callee = ani_macro_extract_callee(ctx->arena, entry->expansion);
            } else {
                callee = NULL;
            }
        }
...
static TSNode objectscript_call_args(TSNode node) {
    TSNode oref = ani_find_child_by_kind(node, "oref_method");
    if (!ts_node_is_null(oref)) {
        TSNode args = ani_find_child_by_kind(oref, "method_args");
        if (!ts_node_is_null(args)) {
            return args;
        }
    }

    TSNode args = ani_find_child_by_kind(node, "method_args");
    if (!ts_node_is_null(args)) {
        return args;
    }
    ...
    TSNode macro_function = ani_find_child_by_kind(node, "macro_function");
    return ts_node_is_null(macro_function) ? (TSNode){0}
                                           : ani_find_child_by_kind(macro_function, "method_args");
}
...
static bool is_callable_reference_value(ANILanguage language, TSNode node) {
    const char *kind = ts_node_type(node);
    if (language == ANI_LANG_JAVA && strcmp(kind, "method_reference") == 0) {
        return true;
    }
    if (language == ANI_LANG_KOTLIN && strcmp(kind, "callable_reference") == 0) {
    ...
    return language == ANI_LANG_PHP && php_is_first_class_callable(node);
}
...
static bool is_dynamic_callee_expr(const ANIExtractCtx *ctx, TSNode expr) {
...
static TSNode terminal_callee_leaf(const ANIExtractCtx *ctx, TSNode expr) {
...
static TSNode objectscript_callee_expr(TSNode node) {
    ...
    if (strcmp(kind, "class_method_call") == 0) {
        return ani_find_child_by_kind(node, "method_name");
    }
    if (strcmp(kind, "method_call") == 0 || strcmp(kind, "relative_dot_method") == 0) {
        TSNode oref = ani_find_child_by_kind(node, "oref_method");
        return ts_node_is_null(oref) ? (TSNode){0} : ani_find_child_by_kind(oref, "method_name");
    }
    if (strcmp(kind, "extrinsic_function") == 0 || strcmp(kind, "routine_tag_call") == 0) {
        return ani_find_child_by_kind(node, "line_ref");
    }
    return (TSNode){0};
}
...
static TSNode language_specific_callee_expr(ANILanguage language, TSNode node) {
    const char *kind = ts_node_type(node);
    ...
    if (language == ANI_LANG_SCALA && strcmp(kind, "infix_expression") == 0) {
    ...
    if (language == ANI_LANG_VHDL && strcmp(kind, "parenthesis_group") == 0) {
        TSNode previous = ts_node_prev_named_sibling(node);
    ...
    if (language == ANI_LANG_CSS && strcmp(kind, "call_expression") == 0) {
        return ani_find_child_by_kind(node, "function_name");
    }

    if (language == ANI_LANG_NASM) {
    ...
    if (language == ANI_LANG_LLVM_IR) {
        return ts_node_child_by_field_name(node, TS_FIELD("callee"));
    }

    if (language == ANI_LANG_AGDA && strcmp(kind, "expr") == 0) {
        return agda_application_head_atom(node);
    }

    if (language == ANI_LANG_ADA &&
    ...
    if (language == ANI_LANG_JUST && strcmp(kind, "dependency") == 0) {
        return (TSNode){0};
    }
    ...
    if (language == ANI_LANG_PUPPET && strcmp(kind, "include_statement") == 0) {
        return (TSNode){0};
    }
...
static TSNode language_specific_callee_leaf(ANILanguage language, TSNode expr,
                                            const ANIExtractCtx *ctx) {
    if (ts_node_is_null(expr)) {
        return (TSNode){0};
    }
    ...
    if (language == ANI_LANG_CSS && strcmp(ts_node_type(expr), "function_name") == 0) {
        return expr;
    }
    if (language == ANI_LANG_NASM) {
    ...
    if (language == ANI_LANG_AGDA) {
        return agda_head_qid(expr);
    }
    if (language == ANI_LANG_ELM) {
    ...
    if (language == ANI_LANG_PURESCRIPT) {
...
static ANIPrimaryCalleeSelection select_primary_callee(ANIExtractCtx *ctx, TSNode node,
                                                       WalkState *state) {
    ANIPrimaryCalleeSelection selection = {0};
    ...
    bool scala_named_infix =
        ctx->language == ANI_LANG_SCALA && strcmp(site_kind, "infix_expression") == 0;
...
static bool primary_callee_name_is_allowed(const ANIExtractCtx *ctx,
                                           const ANIPrimaryCalleeSelection *callee) {
    if (!callee->name || !callee->name[0]) {
        return false;
    }
    if (!ani_is_keyword(callee->name, ctx->language) ||
        ani_is_resolvable_builtin(callee->name, ctx->language)) {
        return true;
    }
    ...
    return ctx->language == ANI_LANG_CSS && !ts_node_is_null(callee->expr) &&
...
static ANIInvocationDescriptor describe_callable_reference(ANIExtractCtx *ctx, TSNode node) {
    ANIInvocationDescriptor descriptor = {0};
    descriptor.kind = ANI_INVOCATION_CALLABLE_REFERENCE;
    ...
    uint32_t count = ts_node_named_child_count(node);
    if (ctx->language == ANI_LANG_JAVA && count > 0) {
    ...
    } else if (ctx->language == ANI_LANG_KOTLIN && count > 0) {
        descriptor.callee_expr = ts_node_named_child(node, count - 1);
    } else if (ctx->language == ANI_LANG_PHP) {
    ...
    if (!ts_node_is_null(name_node)) {
        descriptor.callee_name = ani_node_text(ctx->arena, name_node, ctx->source);
    }
    return descriptor;
}

static ANIInvocationDescriptor describe_emitted_primary_call(
    TSNode node, const ANIPrimaryCalleeSelection *callee) {
    ANIInvocationDescriptor descriptor = {0};
    descriptor.kind = ANI_INVOCATION_PRIMARY;
    descriptor.site = node;
...
ANIInvocationDescriptor handle_calls(ANIExtractCtx *ctx, TSNode node, const ANILangSpec *spec,
                                     WalkState *state) {
    ANIInvocationDescriptor invocation = {0};
    ...
    if (!callable_reference && spec->call_node_types && spec->call_node_types[0] &&
        ani_kind_in_set(node, spec->call_node_types)) {
        ANIPrimaryCalleeSelection callee = select_primary_callee(ctx, node, state);
        ...
        if (primary_callee_name_is_allowed(ctx, &callee)) {
            ...
            if (ctx->language == ANI_LANG_PYTHON && strcmp(ts_node_type(node), "call") == 0) {
                TSNode fn = ts_node_child_by_field_name(node, TS_FIELD("function"));
            ...
            if (!ts_node_is_null(args)) {
                 ...
                 * only see the flat ANICall, so the enclosing chain can only
                 * be read here where the AST still exists. */
                if (ctx->language == ANI_LANG_PHP && call.first_string_arg &&
                    call.first_string_arg[0] == '/' && call.callee_name &&
                    ani_service_pattern_route_method(call.callee_name) != NULL) {
                    ...
                    if (gp && gp[0]) {
                        ...
                        call.first_string_arg =
                            rel[0] ? ani_arena_sprintf(ctx->arena, "%s/%s", gp, rel)
                                   : ani_arena_strndup(ctx->arena, gp, strlen(gp));
                    }
                }
                ...
                if (ctx->language == ANI_LANG_OBJECTSCRIPT_UDL ||
                    ...
                    for (uint32_t ai = 0;
                         ai < ts_node_named_child_count(args) && call.arg_count < ANI_MAX_CALL_ARGS;
                         ai++) {
                        ...
                        ca->expr = ani_node_text(ctx->arena, achild, ctx->source);
            ...
            const char **dispatch_suffixes = ani_string_dispatch_suffixes(ctx->language);
            if (dispatch_suffixes && !ts_node_is_null(args)) {
                ...
                for (const char **nm = dispatch_suffixes; *nm; nm++) {
                    ...
                    if (len >= nlen && strcmp(cn + len - nlen, *nm) == 0) {
                        ...
                        if (cls && mth) {
                            ANICall xcall = {0};
                            xcall.callee_name = ani_arena_sprintf(ctx->arena, "%s.%s", cls, mth);
                            ...
                            ani_calls_push(&ctx->result->calls, ctx->arena, xcall);
                        }
                        break;
                    }
                }
    ...
    if (ctx->language == ANI_LANG_TSX || ctx->language == ANI_LANG_JAVASCRIPT) {
