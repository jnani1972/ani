...
static const char *unquote_string(ANIArena *a, const char *s) {
    if (!s) {
        return NULL;
    }
    size_t len = strlen(s);
    if (len < ANI_QUOTE_PAIR) {
        return NULL;
    }
    char first = s[0];
    char last = s[len - ANI_QUOTE_OFFSET];
    if ((first == '"' && last == '"') || (first == '\'' && last == '\'') ||
        (first == '`' && last == '`')) {
        return ani_arena_strndup(a, s + ANI_QUOTE_OFFSET, len - ANI_QUOTE_PAIR);
    }
    return NULL;
}
...
static const char *literal_from_arg(ANIExtractCtx *ctx, TSNode arg) {
...
static const char *literal_from_first_child(ANIExtractCtx *ctx, TSNode node) {
...
static void scan_string_consts_js(ANIExtractCtx *ctx, chan_const_table_t *tbl) {
    TSNodeStack stack;
        ...
        if (strcmp(kind, "variable_declarator") == 0) {
            ...
            if (!ts_node_is_null(name_node) && !ts_node_is_null(value_node)) {
                ...
                if (strcmp(nk, "identifier") == 0 &&
                    ...
                    char *name_text = ani_node_text(ctx->arena, name_node, ctx->source);
                    char *value_text = ani_node_text(ctx->arena, value_node, ctx->source);
...
static void scan_string_consts_python(ANIExtractCtx *ctx, chan_const_table_t *tbl) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx, CHAN_STACK_CAP);
    ts_nstack_push(&stack, ctx->root);
    ...
    while (stack.count > 0 && tbl->count < CHAN_CONST_CAP) {
        ...
        if (strcmp(kind, "assignment") == 0) {
            ...
            if (!ts_node_is_null(left) && !ts_node_is_null(right) &&
                ...
                char *name = ani_node_text(ctx->arena, left, ctx->source);
...
static const char *enclosing_function_qn(ANIExtractCtx *ctx, TSNode node) {
...
static const char *extract_channel_name(ANIExtractCtx *ctx, TSNode args,
    ...
    if (!channel_name && consts) {
        ...
        if (strcmp(kind, "identifier") == 0) {
            char *ident = ani_node_text(ctx->arena, first, ctx->source);
...
static void push_channel(ANIExtractCtx *ctx, const char *channel_name, const char *transport,
                         ANIChannelDirection direction, TSNode call) {
    ANIChannel ch = {
    ...
    ani_channels_push(&ctx->result->channels, ctx->arena, ch);
}
...
static const char *js_classify_receiver(ANIExtractCtx *ctx, TSNode object_node) {
    char *text = ani_node_text(ctx->arena, object_node, ctx->source);
    if (!text) {
        return NULL;
    }
...
static void js_process_call(ANIExtractCtx *ctx, TSNode call, const chan_const_table_t *consts) {
    ...
    char *method = ani_node_text(ctx->arena, property, ctx->source);
    ...
    if (strcmp(transport, "kafka") == 0) {
        if (js_is_kafka_send(method)) {
            direction = ANI_CHANNEL_EMIT;
        } else if (js_is_kafka_listen(method)) {
            direction = ANI_CHANNEL_LISTEN;
        } else {
            return;
        }
...
static void extract_channels_js(ANIExtractCtx *ctx) {
...
static const char *py_classify_receiver(ANIExtractCtx *ctx, TSNode object_node) {
    char *text = ani_node_text(ctx->arena, object_node, ctx->source);
    if (!text) {
        return NULL;
    }
...
static const struct {
    ...
    {"kafka", "send", ANI_CHANNEL_EMIT},
    {"kafka", "produce", ANI_CHANNEL_EMIT},
    {"kafka", "subscribe", ANI_CHANNEL_LISTEN},
    {"kafka", "poll", ANI_CHANNEL_LISTEN},
    {"django_channels", "send", ANI_CHANNEL_EMIT},
    {"django_channels", "group_send", ANI_CHANNEL_EMIT},
    {"django_channels", "receive", ANI_CHANNEL_LISTEN},
    {"django_channels", "group_add", ANI_CHANNEL_LISTEN},
    {"websocket", "send", ANI_CHANNEL_EMIT},
    {"websocket", "send_text", ANI_CHANNEL_EMIT},
    {"websocket", "send_json", ANI_CHANNEL_EMIT},
    {"websocket", "send_bytes", ANI_CHANNEL_EMIT},
    {"websocket", "receive", ANI_CHANNEL_LISTEN},
    {"websocket", "receive_text", ANI_CHANNEL_LISTEN},
    {"websocket", "receive_json", ANI_CHANNEL_LISTEN},
    {"websocket", "receive_bytes", ANI_CHANNEL_LISTEN},
    {NULL, "emit", ANI_CHANNEL_EMIT},
    {NULL, "send", ANI_CHANNEL_EMIT},
    {NULL, "on", ANI_CHANNEL_LISTEN},
...
static void py_process_call(ANIExtractCtx *ctx, TSNode call, const chan_const_table_t *consts) {
    ...
    char *method = ani_node_text(ctx->arena, attr, ctx->source);
    ...
    ANIChannelDirection direction = (ANIChannelDirection)dir;
...
static void py_process_decorator(ANIExtractCtx *ctx, TSNode decorator,
                                 const chan_const_table_t *consts) {
    ...
    if (strcmp(ek, "call") == 0) {
        ...
        push_channel(ctx, channel_name, transport, ANI_CHANNEL_LISTEN, decorator);
    }
}

static void extract_channels_python(ANIExtractCtx *ctx) {
    chan_const_table_t consts = {0};
...
static void go_process_call(ANIExtractCtx *ctx, TSNode call) {
    TSNode func = ts_node_child_by_field_name(call, TS_FIELD("function"));
    if (ts_node_is_null(func)) {
        return;
    }
    ...
    char *method = ani_node_text(ctx->arena, field, ctx->source);
    if (!method) {
        return;
    }
    ...
    char *recv = ani_node_text(ctx->arena, operand, ctx->source);
    if (!recv) {
        return;
    }
...
static void extract_channels_go(ANIExtractCtx *ctx) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx, CHAN_STACK_CAP);
    ts_nstack_push(&stack, ctx->root);
...
static void java_process_call(ANIExtractCtx *ctx, TSNode call) {
    ...
    char *method = ani_node_text(ctx->arena, func, ctx->source);
    if (!method) {
        return;
    }
    ...
    if (strcmp(method, "convertAndSend") == 0 || strcmp(method, "convertAndSendToUser") == 0) {
        ...
        if (channel_name) {
            push_channel(ctx, channel_name, "spring_websocket", ANI_CHANNEL_EMIT, call);
        }
        return;
    }
    ...
    if (strcmp(method, "sendText") == 0 || strcmp(method, "sendObject") == 0 ||
        ...
        const char *func_name = enclosing_function_qn(ctx, call);
        const char *channel_name = func_name ? func_name : "(websocket)";
        push_channel(ctx, channel_name, "websocket", ANI_CHANNEL_EMIT, call);
        return;
    }
...
static void java_process_annotation(ANIExtractCtx *ctx, TSNode annotation) {
    ...
    if (strcmp(name, "OnMessage") == 0 || strcmp(name, "OnOpen") == 0 ||
        ...
        const char *channel_name = func_name ? func_name : "(websocket)";
        push_channel(ctx, channel_name, "websocket", ANI_CHANNEL_LISTEN, annotation);
        ...
        if (!ts_node_is_null(args)) {
            ...
            if (path) {
                push_channel(ctx, path, "spring_websocket", ANI_CHANNEL_LISTEN, annotation);
                return;
            }
        }
        push_channel(ctx, "(spring_ws)", "spring_websocket", ANI_CHANNEL_LISTEN, annotation);
    } else if (strcmp(name, "ServerEndpoint") == 0) {
        TSNode args = ts_node_child_by_field_name(annotation, TS_FIELD("arguments"));
        if (!ts_node_is_null(args)) {
            const char *path = extract_channel_name(ctx, args, NULL);
            if (path) {
                push_channel(ctx, path, "websocket", ANI_CHANNEL_LISTEN, annotation);
                return;
            }
        }
    }
...
static void extract_channels_java(ANIExtractCtx *ctx) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx, CHAN_STACK_CAP);
    ts_nstack_push(&stack, ctx->root);
...
static void csharp_process_call(ANIExtractCtx *ctx, TSNode call) {
    TSNode func = ts_node_child_by_field_name(call, TS_FIELD("function"));
    if (ts_node_is_null(func)) {
        return;
    }
    ...
    char *method = ani_node_text(ctx->arena, name_node, ctx->source);
    if (!method) {
        return;
    }
    ...
    if (strcmp(method, "SendAsync") == 0 || strcmp(method, "SendCoreAsync") == 0) {
        char *full = ani_node_text(ctx->arena, func, ctx->source);
        if (full && (strstr(full, "Clients") || strstr(full, "clients"))) {
            TSNode args = ts_node_child_by_field_name(call, TS_FIELD("arguments"));
            if (!ts_node_is_null(args)) {
                const char *channel_name = extract_channel_name(ctx, args, NULL);
                if (channel_name) {
                    push_channel(ctx, channel_name, "signalr", ANI_CHANNEL_EMIT, call);
                }
            }
        }
        return;
    ...
    if (strcmp(method, "On") == 0) {
        ...
        if (!ts_node_is_null(args)) {
            const char *channel_name = extract_channel_name(ctx, args, NULL);
            if (channel_name) {
                push_channel(ctx, channel_name, "signalr", ANI_CHANNEL_LISTEN, call);
            }
        }
    }
}

static void extract_channels_csharp(ANIExtractCtx *ctx) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx, CHAN_STACK_CAP);
    ts_nstack_push(&stack, ctx->root);
...
static void ruby_process_call(ANIExtractCtx *ctx, TSNode call) {
    ...
    char *method = ani_node_text(ctx->arena, method_node, ctx->source);
    if (!method) {
        return;
    }
    ...
    if (strcmp(method, "broadcast") == 0) {
        ...
        if (!ts_node_is_null(args)) {
            const char *channel_name = extract_channel_name(ctx, args, NULL);
            if (channel_name) {
                push_channel(ctx, channel_name, "actioncable", ANI_CHANNEL_EMIT, call);
            }
        }
        return;
    }
    ...
    if (strcmp(method, "stream_from") == 0 || strcmp(method, "stream_for") == 0) {
        ...
        if (!ts_node_is_null(args)) {
            const char *channel_name = extract_channel_name(ctx, args, NULL);
            if (channel_name) {
                push_channel(ctx, channel_name, "actioncable", ANI_CHANNEL_LISTEN, call);
            }
        }
    }
}

static void extract_channels_ruby(ANIExtractCtx *ctx) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx, CHAN_STACK_CAP);
    ts_nstack_push(&stack, ctx->root);
...
static const char *elixir_nth_arg_literal(ANIExtractCtx *ctx, TSNode args, uint32_t index) {
...
static void elixir_emit_second_arg(ANIExtractCtx *ctx, TSNode call, TSNode args,
                                   const char *transport, ANIChannelDirection direction) {
    if (ts_node_is_null(args)) {
        return;
    }
...
static void elixir_process_call(ANIExtractCtx *ctx, TSNode call) {
    TSNode target = ts_node_child_by_field_name(call, TS_FIELD("target"));
    if (ts_node_is_null(target)) {
        return;
    }
    char *target_text = ani_node_text(ctx->arena, target, ctx->source);
    if (!target_text) {
        return;
    }
    ...
    if (strstr(target_text, "PubSub.broadcast") || strstr(target_text, "PubSub.local_broadcast")) {
        elixir_emit_second_arg(ctx, call, args, "phoenix_pubsub", ANI_CHANNEL_EMIT);
        return;
    }
    if (strstr(target_text, "PubSub.subscribe")) {
        elixir_emit_second_arg(ctx, call, args, "phoenix_pubsub", ANI_CHANNEL_LISTEN);
        return;
    }
    if (strcmp(target_text, "push") == 0 || strcmp(target_text, "broadcast") == 0 ||
        strcmp(target_text, "broadcast!") == 0) {
        elixir_emit_second_arg(ctx, call, args, "phoenix_channel", ANI_CHANNEL_EMIT);
    }
}

/* Detect handle_in("event", payload, socket) — Phoenix Channel listener */
static void elixir_process_function_def(ANIExtractCtx *ctx, TSNode func_def) {
    ...
    if (val) {
        push_channel(ctx, val, "phoenix_channel", ANI_CHANNEL_LISTEN, func_def);
    }
}

static void extract_channels_elixir(ANIExtractCtx *ctx) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx, CHAN_STACK_CAP);
    ts_nstack_push(&stack, ctx->root);
...
static void rust_process_call(ANIExtractCtx *ctx, TSNode call) {
    TSNode func = ts_node_child_by_field_name(call, TS_FIELD("function"));
    if (ts_node_is_null(func)) {
        return;
    }
    ...
    char *recv = ani_node_text(ctx->arena, value, ctx->source);
    if (!recv) {
        return;
    }
    const char *tail = recv;
...
static void extract_channels_rust(ANIExtractCtx *ctx) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx, CHAN_STACK_CAP);
    ts_nstack_push(&stack, ctx->root);
