#ifndef ANI_PREPROCESSOR_H
#define ANI_PREPROCESSOR_H
...
// The returned string must be freed with ani_preprocess_free().
char *ani_preprocess(const char *source, int source_len, const char *filename,
...
// Free with ani_preprocessed_source_free().
ANIPreprocessedSource *ani_preprocess_with_map(const char *source, int source_len,
                                               const char *filename, const char **extra_defines,
                                               const char **include_paths, int cpp_mode);

// Free preprocessed source returned by ani_preprocess.
void ani_preprocess_free(char *expanded);

// Free preprocessed source and line maps returned by ani_preprocess_with_map.
void ani_preprocessed_source_free(ANIPreprocessedSource *pp);

#ifdef __cplusplus
}
#endif

#endif // ANI_PREPROCESSOR_H
