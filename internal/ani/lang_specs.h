#ifndef ANI_LANG_SPECS_H
#define ANI_LANG_SPECS_H

#include "ani.h"

// ANIEmbeddedLangSpec describes a sub-language embedded inside a host AST.
...
typedef struct {
    ...
    ANILanguage embedded_language; // grammar used to re-parse the content slice
} ANIEmbeddedLangSpec;

// ANILangSpec mirrors Go's lang.LanguageSpec with NULL-terminated string arrays.
typedef struct {
    ...
    const ANIEmbeddedLangSpec *embedded_imports;
} ANILangSpec;
...
// Kept out of ANILangSpec to avoid -Wmissing-field-initializers across 155
