...
// (ani_label_is_relation), so lineage joins dbt models to Table/View nodes
...
static char *dbt_unquote(ANIArena *a, char *s) {
    if (!s) {
        return NULL;
    }
    size_t n = strlen(s);
    if (n >= 2 && (s[0] == '\'' || s[0] == '"') && s[n - 1] == s[0]) {
        char *inner = ani_arena_strdup(a, s + 1);
        if (!inner) {
            return s;
        }
...
static void collect_dbt_refs(ANIExtractCtx *ctx, TSNode node, const char *enclosing_qn,
                             ANIUsageArray *out) {
    if (strcmp(ts_node_type(node), "fn_call") == 0) {
        ...
        if (!ts_node_is_null(fn)) {
            ...
            if (fname && (strcmp(fname, "ref") == 0 || strcmp(fname, "source") == 0)) {
                ...
                if (found) {
                    char *name =
                        dbt_unquote(ctx->arena, ani_node_text(ctx->arena, strn, ctx->source));
                    if (name && name[0]) {
                        ANIUsage usage = {0};
                        usage.ref_name = name;
                        ...
                        ani_usages_push(out, ctx->arena, usage);
                    }
                }
            }
        }
...
static char *dbt_name_from_path(ANIArena *a, const char *rel_path) {
    if (!rel_path) {
        return NULL;
    }
    ...
    char *out = ani_arena_strdup(a, base);
    if (!out) {
        return NULL;
    }
    out[n] = '\0';
...
void ani_extract_dbt(ANIExtractCtx *ctx) {
    if (!ctx || ctx->language != ANI_LANG_SQL || !ctx->source || ctx->source_len <= 0) {
        return;
    }
    if (!source_has_jinja_expr(ctx->source, ctx->source_len)) {
        return;
    }
    const TSLanguage *jl = ani_ts_language(ANI_LANG_JINJA2);
    if (!jl) {
        return;
    }
    ...
    const char *model_qn = ani_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, model_name);
    if (!model_qn) {
        return;
    }
    ...
    for (int i = 0; i < staged.count; i++) {
        ani_usages_push(&ctx->result->usages, ctx->arena, staged.items[i]);
    }
}
