/* Full declaration set for the same ANIArena, and it must precede ani.h:
 * internal/ani/arena.h declares a subset and the two share the ANI_ARENA_H
 * guard, so whichever is included first is the one this file sees. */
#include "foundation/arena.h" // ani_arena_init_sized
#include "ani.h"
#include "arena.h" // ANIArena, ani_arena_init/alloc/strdup/destroy
#include "helpers.h"
...
#include "foundation/compat_fs.h"  // ani_fopen — crash-supervisor per-file marker write
#include "foundation/hash_table.h" // ANIHashTable — crash-supervisor quarantine set
...
#if defined(ANI_BIND_TS_ALLOCATOR) && ANI_BIND_TS_ALLOCATOR
...
/* Use compat.h's ani_clock_gettime which accepts CLOCK_MONOTONIC (value
 * varies by platform: 1 on Linux/Windows, 6 on macOS). We pass the
 * platform value via the compat.h fallback. */
#if defined(CLOCK_MONOTONIC)
#define ANI_CLOCK_MONO CLOCK_MONOTONIC
#elif defined(__APPLE__)
#define ANI_CLOCK_MONO 6
#else
#define ANI_CLOCK_MONO 1
#endif

static uint64_t now_ns(void) {
    struct timespec ts;
    ani_clock_gettime(ANI_CLOCK_MONO, &ts);
    return ((uint64_t)ts.tv_sec * NSEC_PER_SEC) + (uint64_t)ts.tv_nsec;
}

// ani_get_profile returns accumulated parse/extract times and file count.
...
// ani_reset_profile zeros the profiling counters.
...
#define GROW_ARRAY(arr, arena)                                                                   \
    do {                                                                                         \
        if ((arr)->count >= (arr)->cap) {                                                        \
            int new_cap = (arr)->cap == 0 ? ANI_SZ_32 : (arr)->cap * PAIR_LEN;                   \
            void *new_items = ani_arena_alloc((arena), (size_t)new_cap * sizeof(*(arr)->items)); \
            if (!new_items)                                                                      \
                return;                                                                          \
...
static const char *ani_string_read(void *payload, uint32_t byte, TSPoint point,
                                   uint32_t *bytes_read) {
    (void)point;
    ANIStringInput *self = (ANIStringInput *)payload;
    if (byte >= self->length) {
        *bytes_read = 0;
        return "";
    }
...
static bool ani_timeout_cb(TSParseState *state) {
...
static ANI_TLS TSParser *tl_parser = NULL;
static ANI_TLS ANILanguage tl_parser_lang = ANI_LANG_COUNT; // invalid sentinel

// Get or create a thread-local parser configured for the given language.
static TSParser *get_thread_parser(const TSLanguage *ts_lang, ANILanguage lang) {
    if (!tl_parser) {
        tl_parser = ts_parser_new();
        if (!tl_parser) {
            return NULL;
        }
        tl_parser_lang = ANI_LANG_COUNT;
    }
 ...
 *
 * Guarded to the production build (ANI_BIND_TS_ALLOCATOR=1, which CFLAGS_PROD
 ...
 * (#581). */
static void *ani_sqlite_malloc(int n) {
    void *block = mi_malloc((size_t)n);
    return block;
}
static void ani_sqlite_free(void *p) {
    mi_free(p);
}
static void *ani_sqlite_realloc(void *p, int n) {
    if (p) {}
    void *grown = mi_realloc(p, (size_t)n);
    return grown;
}
static int ani_sqlite_size(void *p) {
    return (int)mi_usable_size(p);
}
static int ani_sqlite_roundup(int n) {
...
static void *ani_ts_malloc(size_t n) {
    void *block = mi_malloc(n);
    return block;
}
static void *ani_ts_calloc(size_t count, size_t size) {
    void *block = mi_calloc(count, size);
    return block;
}
static void *ani_ts_realloc(void *p, size_t n) {
    if (p) {}
    void *grown = mi_realloc(p, n);
    return grown;
}
static void ani_ts_free(void *p) {
    mi_free(p);
}

static int ani_sqlite_meminit(void *appdata) {
    (void)appdata;
    return SQLITE_OK;
}
static void ani_sqlite_memshutdown(void *appdata) {
    (void)appdata;
}
#endif /* ANI_BIND_TS_ALLOCATOR */
...
void ani_alloc_init(void) {
    ...
    /* tree-sitter runtime (was previously bound in ani_init; consolidated here). */
    ts_set_allocator(ani_ts_malloc, ani_ts_calloc, ani_ts_realloc, ani_ts_free);
     ...
     * silently and the binding is ignored. ani_alloc_init() runs as the very
     * first statement of main(), before ani_mcp_server_new → ani_store_open*. */
    static sqlite3_mem_methods ani_sqlite_mem = {
        ani_sqlite_malloc,      /* xMalloc */
        ani_sqlite_free,        /* xFree */
        ani_sqlite_realloc,     /* xRealloc */
        ani_sqlite_size,        /* xSize */
        ani_sqlite_roundup,     /* xRoundup */
        ani_sqlite_meminit,     /* xInit */
        ani_sqlite_memshutdown, /* xShutdown */
        NULL,                   /* pAppData */
    };
    int sqlite_rc = sqlite3_config(SQLITE_CONFIG_MALLOC, &ani_sqlite_mem);
...
static int ani_initialized = 0;

int ani_init(void) {
    if (ani_initialized) {
        return 0;
    }
    enum { ANI_INIT_DONE = 1 };
    ani_initialized = ANI_INIT_DONE;
    /* Defense-in-depth allocator binds (idempotent). main() calls ani_alloc_init
     * first; this covers non-main entry points (pipeline passes call ani_init).
...
void ani_reset_thread_parser(void) {
    ...
    // Must be called BEFORE ani_slab_reset_thread() to avoid corrupting
...
// (Go: `s` in `func (s *Store) save()`, from ANIDefinition.receiver). Matching
...
static void ani_index_mark(const char *rel_path, char event) {
    const char *mf = getenv("ANI_INDEX_MARKER_FILE");
    if (!mf || !mf[0] || !rel_path || !rel_path[0]) {
        return;
    }
    FILE *f = ani_fopen(mf, "ab");
    if (f) {
        (void)fprintf(f, "%c %s\n", event, rel_path);
        (void)fclose(f);
    }
 ...
 * (ani_extract_file): whether a pass re-extracts from disk on a cache miss
 ...
 * ani_index_is_quarantined() so the crasher surfaces in the response skipped[].
...
static ANIHashTable *g_quarantine_set = NULL;
enum { ANI_QSET_UNINIT = 0, ANI_QSET_INITING = 1, ANI_QSET_INITED = 2 };
static atomic_int g_quarantine_state = ANI_QSET_UNINIT;

static void ani_quarantine_load(void) {
    const char *qf = getenv("ANI_INDEX_QUARANTINE_FILE");
    if (!qf || !qf[0]) {
        return; /* normal path: empty set */
    }
    FILE *f = ani_fopen(qf, "rb");
    if (!f) {
        return;
    }
    ANIHashTable *set = ani_ht_create(16);
    if (!set) {
        (void)fclose(f);
        return;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
         ...
         * The value stores the phase so ani_index_quarantine_phase() can report
         * "crash" vs "hang"; membership (ani_index_is_quarantined) is value != NULL. */
        char *pval = ani_strdup(phase);
        if (!pval) {
            continue;
        }
        if (ani_ht_has(set, line)) {
             ...
             * value it displaces. */
            free(ani_ht_set(set, line, (void *)pval));
        } else {
            char *key = ani_strdup(line);
            if (key) {
                ani_ht_set(set, key, (void *)pval);
            } else {
...
bool ani_index_is_quarantined(const char *rel_path) {
    ...
    if (state != ANI_QSET_INITED) {
        /* First caller wins the CAS and loads; racers spin until INITED.
         * Same once-init pattern as ani_ui_log_init (http_server.c). */
        state = ANI_QSET_UNINIT;
        if (atomic_compare_exchange_strong(&g_quarantine_state, &state, ANI_QSET_INITING)) {
            ani_quarantine_load();
            atomic_store(&g_quarantine_state, ANI_QSET_INITED);
        } else {
            while (atomic_load(&g_quarantine_state) != ANI_QSET_INITED) {
                ani_usleep(1000); /* 1ms */
            }
        }
    }
    return g_quarantine_set && ani_ht_has(g_quarantine_set, rel_path);
}

const char *ani_index_quarantine_phase(const char *rel_path) {
    /* ani_index_is_quarantined drives the lazy once-load and returns true only
    ...
    if (!ani_index_is_quarantined(rel_path)) {
        return NULL;
    }
    return (const char *)ani_ht_get(g_quarantine_set, rel_path);
}
...
static void ani_test_fault_inject(const char *rel_path) {
    if (!rel_path || !rel_path[0]) {
        return;
    }
    const char *crash_on = getenv("ANI_TEST_CRASH_ON");
    if (crash_on && crash_on[0] && strstr(rel_path, crash_on)) {
        abort(); /* SIGABRT → WIFSIGNALED → classified as a crash */
    }
    const char *hang_on = getenv("ANI_TEST_HANG_ON");
    if (hang_on && hang_on[0] && strstr(rel_path, hang_on)) {
        for (;;) {
            /* Busy-spin: the supervisor's quiet-timeout kills + reports us. */
        }
    }
    const char *exit_on = getenv("ANI_TEST_EXIT_ON");
    if (exit_on && exit_on[0] && strstr(rel_path, exit_on)) {
        exit(1); /* Nonzero exit code → ANI_PROC_EXIT_NONZERO → classified as "error" */
    }
}
#endif
 ...
 *
 * ani_source_nesting_exceeds scans the raw bytes for the maximum bracket-nesting
...
#define ANI_PERL_MAX_PARSE_NESTING 128

static bool ani_source_nesting_exceeds(const char *source, int source_len, int cap) {
    int depth = 0;
 ...
 * Bounded by ANI_MAX_ERROR_REGIONS so pathological input can't blow up the
 * output. The ranges mark where constructs were dropped; they are a detection
 * aid, never a completeness proof. */
#define ANI_MAX_ERROR_REGIONS 64
typedef struct {
    uint32_t starts[ANI_MAX_ERROR_REGIONS];
    uint32_t ends[ANI_MAX_ERROR_REGIONS];
    int count;
} ani_error_regions_t;

static void ani_error_regions_push(ani_error_regions_t *acc, TSNode n) {
    if (acc->count >= ANI_MAX_ERROR_REGIONS) {
        return;
    }
 ...
 * a newline. */
static bool ani_is_blank_not_newline(char c) {
    return c == ' ' || c == '\t' || c == '\v' || c == '\f' || c == '\r';
}

static bool ani_is_eof_terminator_miss(TSNode n, const char *source, int source_len) {
    ...
    for (uint32_t i = end; i < (uint32_t)source_len; i++) {
        if (!ani_is_blank_not_newline(source[i])) {
            return false;
        }
    }
    return true;
...
static void ani_collect_error_regions(TSNode n, ani_error_regions_t *acc, const char *source,
                                      int source_len) {
    if (acc->count >= ANI_MAX_ERROR_REGIONS) {
        return;
    }
    uint32_t k = ts_node_child_count(n);
    for (uint32_t i = 0; i < k && acc->count < ANI_MAX_ERROR_REGIONS; i++) {
        TSNode c = ts_node_child(n, i);
        if (ts_node_is_missing(c) || strcmp(ts_node_type(c), "ERROR") == 0) {
            if (ani_is_eof_terminator_miss(c, source, source_len)) {
                continue; /* absent final newline only — nothing was dropped */
            }
            ani_error_regions_push(acc, c); /* top-most region; do not descend */
        } else if (ts_node_has_error(c)) {
            ani_collect_error_regions(c, acc, source, source_len);
        }
    }
}
...
static bool ani_region_is_recovered(uint32_t rs, uint32_t re, const ANIDefArray *defs) {
    ...
    for (int i = 0; i < defs->count && n < MAX_COVER_DEFS; i++) {
        const ANIDefinition *d = &defs->items[i];
...
static bool ani_line_contains(const char *src, int src_len, uint32_t line, const char *name) {
...
static bool ani_identifier_char(char c) {
...
static bool ani_span_contains_callable_def(const char *src, int src_len, uint32_t start_line,
    ...
    for (int pos = span_start; pos + (int)name_len <= span_end; pos++) {
        if (strncmp(src + pos, name, name_len) != 0 ||
            (pos > 0 && ani_identifier_char(src[pos - 1])) ||
            (pos + (int)name_len < src_len && ani_identifier_char(src[pos + name_len]))) {
            continue;
        }
        int before = pos;
...
static bool ani_remap_preprocessed_def(ANIDefinition *def, const ANIPreprocessedSource *pp) {
...
static void ani_subtract_recovered_regions(ani_error_regions_t *regs, const ANIDefArray *defs) {
    int kept = 0;
    for (int i = 0; i < regs->count; i++) {
        if (!ani_region_is_recovered(regs->starts[i], regs->ends[i], defs)) {
...
static bool ani_span_is_macro_invocation(const char *src, int src_len, uint32_t start_line,
                                         uint32_t end_line, const ANIDefArray *defs) {
    ...
    for (int di = 0; di < defs->count; di++) {
        const ANIDefinition *d = &defs->items[di];
        ...
        for (int pos = span_start; pos + nlen <= span_end; pos++) {
            if (strncmp(src + pos, d->name, (size_t)nlen) != 0 ||
                (pos > 0 && ani_identifier_char(src[pos - 1])) ||
                (pos + nlen < src_len && ani_identifier_char(src[pos + nlen]))) {
                continue;
            }
            int open = pos + nlen;
            while (open < span_end && isspace((unsigned char)src[open])) {
...
static bool ani_region_inside_callable(uint32_t rs, uint32_t re, const ANIDefArray *defs) {
...
static void ani_subtract_macro_invocation_regions(ani_error_regions_t *regs,
                                                  const ANIDefArray *defs, const char *src,
                                                  int src_len) {
    int kept = 0;
    for (int i = 0; i < regs->count; i++) {
        bool benign =
            ani_span_is_macro_invocation(src, src_len, regs->starts[i], regs->ends[i], defs) &&
            ani_region_inside_callable(regs->starts[i], regs->ends[i], defs);
        if (!benign) {
            regs->starts[kept] = regs->starts[i];
            regs->ends[kept] = regs->ends[i];
            kept++;
...
static const char *ani_error_ranges_str(ANIArena *a, const ani_error_regions_t *regs) {
    if (regs->count <= 0) {
        return NULL;
    }
    enum { RANGE_MAX = 24 }; /* "4294967295-4294967295," */
    char *buf = (char *)ani_arena_alloc(a, (size_t)regs->count * RANGE_MAX);
    if (!buf) {
        return NULL;
    }
    size_t off = 0;
...
ANIFileResult *ani_extract_file(const char *source, int source_len, ANILanguage language,
    ...
    ANIFileResult *r =
        ani_extract_file_ex(source, source_len, language, project, rel_path, timeout_micros,
...
enum { ANI_EXTRACT_SCRATCH_BLOCK = ANI_SZ_512 * ANI_SZ_1K };

static ANIFileResult *extract_file_ex_body(const char *source, int source_len, ANILanguage language,
    ...
    enum { SINGLE = 1 };
    ANIFileResult *result = (ANIFileResult *)calloc(SINGLE, sizeof(ANIFileResult));
    if (!result) {
        return NULL;
    }

    ani_arena_init(&result->arena);
    ANIArena *a = &result->arena;
    ...
    ani_index_mark_start(rel_path);
#ifdef ANI_ENABLE_TEST_SEAMS
    ani_test_fault_inject(rel_path);
#endif
    ...
    if (!spec) {
        ...
        result->error_msg = ani_arena_strdup(a, "unsupported language");
        ani_index_mark_done(rel_path);
        return result;
    }

    // Get tree-sitter language
    const TSLanguage *ts_lang = ani_ts_language(language);
    if (!ts_lang) {
        result->has_error = true;
        result->error_msg = ani_arena_strdup(a, "no tree-sitter grammar");
        ani_index_mark_done(rel_path);
        return result;
    }
    ...
    // ani_source_nesting_exceeds). Scoped to Perl: its ambiguous call grammar is
    // the only one that drives that recursion to the nesting depth.
    if (language == ANI_LANG_PERL &&
        ani_source_nesting_exceeds(source, source_len, ANI_PERL_MAX_PARSE_NESTING)) {
        result->has_error = true;
        result->error_msg = ani_arena_strdup(a, "perl source nesting too deep; skipped");
        return result;
    }
    ...
    if (!parser) {
        result->has_error = true;
        result->error_msg = ani_arena_strdup(a, "parser alloc failed");
        ani_index_mark_done(rel_path);
        return result;
    }
    ...
    ANIStringInput str_input = {source, (uint32_t)source_len};
    TSInput ts_input = {
        &str_input,
        ani_string_read,
        TSInputEncodingUTF8,
    ...
    if (timeout_micros > 0) {
        ...
        opts.progress_callback = ani_timeout_cb;
    }
    ...
    if (!tree) {
        result->has_error = true;
        result->error_msg =
            ani_arena_strdup(a, timeout_micros > 0 ? "parse timeout" : "parse failed");
        ani_index_mark_done(rel_path);
        return result;
    }
    ...
    result->module_qn = ani_fqn_module_source_lang(a, project, rel_path, language);
    result->is_test_file = ani_is_test_file(rel_path, language);
    ...
    ani_extract_definitions(&ctx);
    ani_extract_imports(&ctx);
    ani_extract_unified(&ctx);

    // Channel detection (Socket.IO / EventEmitter) — JS/TS only.
    ani_extract_channels(&ctx);

    // K8s / Kustomize semantic pass (additional structured extraction for YAML-based infra files).
    if (ctx.language == ANI_LANG_KUSTOMIZE || ctx.language == ANI_LANG_K8S) {
        ani_extract_k8s(&ctx);
    }
    ...
    {
        if (language == ANI_LANG_GO) {
            ani_run_go_lsp(a, result, source, source_len, root);
        }
        if (language == ANI_LANG_C || language == ANI_LANG_CPP || language == ANI_LANG_CUDA) {
            ani_run_c_lsp(a, result, source, source_len, root, language != ANI_LANG_C,
                          ANI_SOURCE_ORIGIN_RAW);
        }
        if (language == ANI_LANG_PHP) {
            ani_run_php_lsp(a, result, source, source_len, root);
        }
        if (language == ANI_LANG_PERL) {
            ani_run_perl_lsp(a, result, source, source_len, root);
        }
        if (language == ANI_LANG_PYTHON) {
            ani_run_py_lsp(a, result, source, source_len, root);
        }
        if (language == ANI_LANG_JAVASCRIPT || language == ANI_LANG_TYPESCRIPT ||
            language == ANI_LANG_TSX) {
            bool js_mode = (language == ANI_LANG_JAVASCRIPT);
            // jsx_mode: TSX always; .jsx in the JS bucket also enables it.
            bool jsx_mode = (language == ANI_LANG_TSX);
            if (language == ANI_LANG_JAVASCRIPT && rel_path) {
            ...
            bool dts_mode = false;
            if (language == ANI_LANG_TYPESCRIPT && rel_path) {
                size_t rl = strlen(rel_path);
                if (rl >= 5 && strcmp(rel_path + rl - 5, ".d.ts") == 0)
                    dts_mode = true;
            }
            ani_run_ts_lsp(a, result, source, source_len, root, js_mode, jsx_mode, dts_mode);
        }
        if (language == ANI_LANG_CSHARP) {
            ani_run_cs_lsp(a, result, source, source_len, root);
        }
    }
    if (language == ANI_LANG_JAVA) {
        ani_run_java_lsp(a, result, source, source_len, root);
    }
    if (language == ANI_LANG_KOTLIN) {
        ani_run_kotlin_lsp(a, result, source, source_len, root);
    }
    if (language == ANI_LANG_RUST) {
        ani_run_rust_lsp(a, result, source, source_len, root);
    }
    ...
    if (language == ANI_LANG_C || language == ANI_LANG_CPP || language == ANI_LANG_CUDA) {
        ...
        ANIPreprocessedSource *preprocessed = ani_preprocess_with_map(
            source, source_len, rel_path, extra_defines, include_paths, language != ANI_LANG_C);
        if (preprocessed && preprocessed->source) {
            ...
            if (pp_parser) {
                ...
                ANIStringInput pp_input = {expanded, (uint32_t)expanded_len};
                TSInput pp_ts_input = {
                    &pp_input,
                    ani_string_read,
                    TSInputEncodingUTF8,
                ...
                if (pp_tree) {
                    ...
                    for (int i = calls_before; i < result->calls.count; i++) {
                        result->calls.items[i].source_origin = ANI_SOURCE_ORIGIN_PREPROCESSED;
                    }
                    for (int i = usages_before; i < result->usages.count; i++) {
                        result->usages.items[i].source_origin = ANI_SOURCE_ORIGIN_PREPROCESSED;
                    }
                    ...
                    ani_run_c_lsp(a, result, expanded, expanded_len, pp_root,
                                  language != ANI_LANG_C, ANI_SOURCE_ORIGIN_PREPROCESSED);
                    ...
                    if (ts_node_has_error(root)) {
                        ani_error_regions_t raw_regs = {{0}, {0}, 0};
                        ani_collect_error_regions(root, &raw_regs, source, source_len);
                        if (raw_regs.count > 0) {
                            int defs_before = result->defs.count;
                            ani_extract_definitions(&pp_ctx);
                            int w = defs_before;
                            for (int i = defs_before; i < result->defs.count; i++) {
                                ANIDefinition *d = &result->defs.items[i];
                                bool adopt = false;
                                if (ani_remap_preprocessed_def(d, preprocessed)) {
                                ...
                                if (adopt && (!d->name ||
                                              !ani_line_contains(source, source_len, d->start_line,
                                                                 d->name) ||
                                              !ani_span_contains_callable_def(
            ...
            ani_preprocessed_source_free(preprocessed);
    ...
    // ANIFileResult share the same file). Range matching is used instead of
    ...
    for (int di = 0; di < def_count; di++) {
        ANIDefinition *d = &result->defs.items[di];
        int pc = 0;
        if (d->param_names) {
    ...
    for (int ci = 0; ci < orig_calls_count; ci++) {
        const ANICall *c = &result->calls.items[ci];
        ...
        int best_span = -1;
        for (int di = 0; di < def_count; di++) {
            const ANIDefinition *d = &result->defs.items[di];
            if (!d->name || !d->label ||
        ...
        ANIDefinition *d = &result->defs.items[best];
    ...
    if (ts_node_has_error(root)) {
        ani_error_regions_t regs = {{0}, {0}, 0};
        if (strcmp(ts_node_type(root), "ERROR") == 0) {
            ani_error_regions_push(&regs, root); /* whole file unparseable */
        } else {
            ani_collect_error_regions(root, &regs, source, source_len);
        }
        ani_subtract_recovered_regions(&regs, &result->defs);
        /* #1071: don't flag a benign function-like-macro call (defined in-file)
         * that tree-sitter can't parse without the preprocessor. */
        ani_subtract_macro_invocation_regions(&regs, &result->defs, source, source_len);
        if (regs.count > 0) {
            result->parse_incomplete = true;
            result->error_region_count = regs.count;
            result->error_ranges = ani_error_ranges_str(a, &regs);
        }
    }
    ...
    // Retain tree for cross-file LSP reuse (caller frees via ani_free_tree)
...
ANIFileResult *ani_extract_file_ex(const char *source, int source_len, ANILanguage language,
                                   const char *project, const char *rel_path,
                                   int64_t timeout_micros, const char **extra_defines,
                                   const char **include_paths, const ANIMacroTable *macro_table,
                                   const ANIReturnTypeTable *return_type_table) {
    ANIArena scratch;
    ani_arena_init_sized(&scratch, ANI_EXTRACT_SCRATCH_BLOCK);
    ANIFileResult *result = extract_file_ex_body(
...
void ani_free_result(ANIFileResult *result) {
    ...
    for (int i = 0; i < result->owned_result_count; i++) {
        ani_free_result(result->owned_results[i]);
    }
