...
static bool is_throw_node(TSNode node, const ANILangSpec *spec) {
    if (ani_kind_in_set(node, spec->throw_node_types)) {
        return true;
    }
    if (spec->language == ANI_LANG_KOTLIN && strcmp(ts_node_type(node), "jump_expression") == 0) {
...
static char *resolve_exception_name(ANIArena *a, TSNode throw_node, const char *source) {
...
static void extract_throws_clause(ANIExtractCtx *ctx, TSNode node, const ANILangSpec *spec,
                                  const char *func_qn) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ck, "type_identifier") == 0 || strcmp(ck, "identifier") == 0 ||
            ...
            char *exc = ani_node_text(ctx->arena, child, ctx->source);
            if (exc && exc[0]) {
                ANIThrow thr = {.exception_name = exc, .enclosing_func_qn = func_qn};
                ani_throws_push(&ctx->result->throws, ctx->arena, thr);
            }
        }
    }
}
...
static void process_throw_node(ANIExtractCtx *ctx, TSNode node, const ANILangSpec *spec) {
    if (is_throw_node(node, spec)) {
        ...
        if (exc_name && exc_name[0]) {
            ...
            thr.enclosing_func_qn = ani_enclosing_func_qn_cached(ctx, node);
            ani_throws_push(&ctx->result->throws, ctx->arena, thr);
        }
    }

    extract_throws_clause(ctx, node, spec, ani_enclosing_func_qn_cached(ctx, node));
}

// Iterative throw walker
static void walk_throws(ANIExtractCtx *ctx, TSNode root, const ANILangSpec *spec) {
    TSNodeStack stack;
...
static char *resolve_lhs_write_name(ANIExtractCtx *ctx, TSNode left, bool *is_member_out) {
    ...
    if (strcmp(lk, "identifier") == 0 || strcmp(lk, "simple_identifier") == 0) {
        return ani_node_text(ctx->arena, left, ctx->source);
    }
    ...
    if (strcmp(lk, "index_expression") == 0 || strcmp(lk, "subscript_expression") == 0) {
        ...
        if (!ts_node_is_null(base)) {
            ...
            if (strcmp(bk, "identifier") == 0 || strcmp(bk, "simple_identifier") == 0) {
                return ani_node_text(ctx->arena, base, ctx->source);
            }
        }
        return NULL;
    }
    ...
    if (strcmp(lk, "field_expression") == 0 || strcmp(lk, "member_access_expression") == 0 ||
        ...
        if (!ts_node_is_null(fld)) {
            ...
            return ani_node_text(ctx->arena, fld, ctx->source);
        }
        return NULL;
    }
    return NULL;
...
static void try_emit_assignment_write(ANIExtractCtx *ctx, TSNode node, const char *func_qn) {
    ...
    if (name && name[0] && !ani_is_keyword(name, ctx->language)) {
        ...
        ani_rw_push(&ctx->result->rw, ctx->arena, rw);
    }
}

static void walk_readwrites(ANIExtractCtx *ctx, TSNode root, const ANILangSpec *spec) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx, ANI_SZ_512);
    ts_nstack_push(&stack, root);
    while (stack.count > 0) {
        TSNode node = ts_nstack_pop(&stack);
        if (ani_kind_in_set(node, spec->assignment_node_types)) {
            try_emit_assignment_write(ctx, node, ani_enclosing_func_qn_cached(ctx, node));
        }
        uint32_t count = ts_node_child_count(node);
        for (int i = (int)count - LAST_IDX; i >= 0; i--) {
            ts_nstack_push(&stack, ts_node_child(node, (uint32_t)i));
