#include "ani.h"
#include "arena.h" // ANIArena, ani_arena_strndup
#include "helpers.h"
...
static const char *unquote(ANIArena *a, const char *s) {
    if (!s || !s[0]) {
        return NULL;
    }
    // Trim whitespace
...
static const char *extract_env_key_from_call(ANIExtractCtx *ctx, TSNode node,
    ...
    char *callee = ani_node_text(ctx->arena, func_node, ctx->source);
    if (!callee) {
        return NULL;
    }
    ...
    for (uint32_t i = 0; i < ts_node_child_count(args); i++) {
        ...
        char *arg_text = ani_node_text(ctx->arena, child, ctx->source);
...
static const char *extract_env_key_from_member(ANIExtractCtx *ctx, TSNode node,
                                               const ANILangSpec *spec) {
    ...
    for (const char **pat = spec->env_access_member_patterns; *pat; pat++) {
        ...
        if (strncmp(text, *pat, plen) == 0 && text[plen] == '.') {
            const char *key = text + plen + ANI_QUOTE_OFFSET;
        ...
        if (strncmp(text, *pat, plen) == 0 && text[plen] == '[') {
            const char *inner = text + plen + ANI_QUOTE_OFFSET;
            size_t ilen = strlen(inner);
            if (ilen > 0 && inner[ilen - ANI_QUOTE_OFFSET] == ']') {
                char *bracket_content =
                    ani_arena_strndup(ctx->arena, inner, ilen - ANI_QUOTE_OFFSET);
...
static void walk_env_accesses(ANIExtractCtx *ctx, TSNode root, const ANILangSpec *spec) {
    TSNodeStack stack;
    ...
    while (stack.count > 0) {
        ...
        if (ani_kind_in_set(node, spec->call_node_types)) {
        ...
        if (env_key && env_key[0] && is_env_var_name(env_key)) {
            ANIEnvAccess ea;
            ea.env_key = env_key;
            ea.enclosing_func_qn = ani_enclosing_func_qn_cached(ctx, node);
            ani_envaccess_push(&ctx->result->env_accesses, ctx->arena, ea);
...
void handle_env_accesses(ANIExtractCtx *ctx, TSNode node, const ANILangSpec *spec,
    ...
    if (has_funcs && spec->call_node_types && ani_kind_in_set(node, spec->call_node_types)) {
        env_key = extract_env_key_from_call(ctx, node, spec);
