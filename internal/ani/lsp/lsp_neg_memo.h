 ...
 * (reg->read_only, set at finalize; ani_registry_add_* hard-return then) and
 ...
 *
 * Per-language wiring is a few lines: add a ANINegMemo to the language ctx,
 * key each cascade entry with ani_negmemo_key (site tag + query strings),
 ...
 */
#ifndef ANI_LSP_NEG_MEMO_H
#define ANI_LSP_NEG_MEMO_H
...
enum {
    ANI_NEGMEMO_INIT_CAP = 1024,
    ANI_NEGMEMO_GROW = 2,
    ANI_NEGMEMO_LOAD_NUM = 7, /* grow at 70% load */
    ANI_NEGMEMO_LOAD_DEN = 10,
};
...
static inline uint64_t ani_negmemo_key(uint8_t site, const char *a, const char *b) {
...
static inline bool ani_negmemo_contains(const ANINegMemo *m, uint64_t key) {
...
static inline void ani_negmemo_insert_raw(uint64_t *slots, int cap, uint64_t key) {
...
static inline void ani_negmemo_insert(ANINegMemo *m, ANIArena *arena, uint64_t key) {
    if (!arena) {
        return; /* no arena — memo silently disabled */
    }
    if (!m->slots) {
        m->slots = ani_arena_alloc(arena, sizeof(uint64_t) * ANI_NEGMEMO_INIT_CAP);
        if (!m->slots) {
            return;
        }
        memset(m->slots, 0, sizeof(uint64_t) * ANI_NEGMEMO_INIT_CAP);
        m->cap = ANI_NEGMEMO_INIT_CAP;
        m->count = 0;
    }
    if ((int64_t)(m->count + 1) * ANI_NEGMEMO_LOAD_DEN >=
        (int64_t)m->cap * ANI_NEGMEMO_LOAD_NUM) {
        int new_cap = m->cap * ANI_NEGMEMO_GROW;
        uint64_t *ns = ani_arena_alloc(arena, sizeof(uint64_t) * (size_t)new_cap);
        if (!ns) {
        ...
        for (int i = 0; i < m->cap; i++) {
            if (m->slots[i]) {
                ani_negmemo_insert_raw(ns, new_cap, m->slots[i]);
            }
        }
        m->slots = ns;
        m->cap = new_cap;
    }
    if (!ani_negmemo_contains(m, key)) {
        ani_negmemo_insert_raw(m->slots, m->cap, key);
        m->count++;
    }
}

/* ── ANIIdxMemo: exact-match string-key → int index map ─────────────────────
...
static inline int32_t ani_idxmemo_get(const ANIIdxMemo *m, const char *key) {
    if (!m->slots || !key || m->count == 0) {
        return -1;
    }
    uint64_t h = ani_negmemo_key(0, key, NULL);
    uint64_t mask = (uint64_t)(m->cap - 1);
...
static inline void ani_idxmemo_put_raw(struct ani_idxmemo_slot *slots, int cap, uint64_t h,
...
static inline void ani_idxmemo_put_if_absent(ANIIdxMemo *m, ANIArena *arena, const char *key,
                                             int32_t val) {
    if (!arena || !key) {
        return;
    }
    if (!m->slots) {
        m->slots = ani_arena_alloc(arena,
                                   sizeof(struct ani_idxmemo_slot) * ANI_NEGMEMO_INIT_CAP);
        if (!m->slots) {
            return;
        }
        memset(m->slots, 0, sizeof(struct ani_idxmemo_slot) * ANI_NEGMEMO_INIT_CAP);
        m->cap = ANI_NEGMEMO_INIT_CAP;
        m->count = 0;
    }
    if ((int64_t)(m->count + 1) * ANI_NEGMEMO_LOAD_DEN >=
        (int64_t)m->cap * ANI_NEGMEMO_LOAD_NUM) {
        int new_cap = m->cap * ANI_NEGMEMO_GROW;
        struct ani_idxmemo_slot *ns =
            ani_arena_alloc(arena, sizeof(struct ani_idxmemo_slot) * (size_t)new_cap);
        if (!ns) {
            return;
        }
        memset(ns, 0, sizeof(struct ani_idxmemo_slot) * (size_t)new_cap);
        for (int i = 0; i < m->cap; i++) {
            if (m->slots[i].h) {
                ani_idxmemo_put_raw(ns, new_cap, m->slots[i].h, m->slots[i].key, m->slots[i].val);
            }
        }
        m->slots = ns;
        m->cap = new_cap;
    ...
    uint64_t h = ani_negmemo_key(0, key, NULL);
    if (ani_idxmemo_get(m, key) < 0) {
        ani_idxmemo_put_raw(m->slots, m->cap, h, key, val);
        m->count++;
    }
}

#endif /* ANI_LSP_NEG_MEMO_H */
