...
static void reg_method(ANITypeRegistry* reg, ANIArena* arena,
    const char* recv_qn, const char* method_name, const ANIType* ret_type) {
    ...
    char* qn = (char*)ani_arena_alloc(arena, rlen + 1 + mlen + 1);
    if (!qn) return;
    ...
    rf.signature = ani_type_func(arena, NULL, NULL,
        ret_type ? (const ANIType*[]){ret_type, NULL} : NULL);
    ani_registry_add_func(reg, rf);
}

// Helper: register a type
static void reg_type(ANITypeRegistry* reg, const char* qn, const char* short_name) {
...
static void reg_type_with_fields(ANITypeRegistry* reg, ANIArena* arena,
    const char* qn, const char* short_name,
    const char** fnames, const ANIType** ftypes) {
    ANIRegisteredType rt;
    memset(&rt, 0, sizeof(rt));
    rt.qualified_name = qn;
    rt.short_name = short_name;
...
static void reg_func(ANITypeRegistry* reg, ANIArena* arena,
    const char* qn, const char* short_name, const ANIType* ret_type) {
    ANIRegisteredFunc rf;
    memset(&rf, 0, sizeof(rf));
    rf.min_params = -1;
    rf.qualified_name = qn;
...
void ani_cpp_stdlib_register(ANITypeRegistry* reg, ANIArena* arena) {
    const ANIType* t_void = ani_type_builtin(arena, "void");
    const ANIType* t_void_ptr = ani_type_pointer(arena, t_void);
    const ANIType* t_bool = ani_type_builtin(arena, "bool");
    const ANIType* t_int = ani_type_builtin(arena, "int");
    const ANIType* t_size_t = ani_type_builtin(arena, "size_t");
    const ANIType* t_long = ani_type_builtin(arena, "long");
    const ANIType* t_char = ani_type_builtin(arena, "char");
    const ANIType* t_char_ptr = ani_type_pointer(arena, ani_type_builtin(arena, "const char"));
    ...
    const ANIType* t_string = ani_type_named(arena, string_qn);
    const ANIType* t_string_ref = ani_type_reference(arena, t_string);
    ...
    reg_method(reg, arena, string_qn, "front", ani_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "back", ani_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "at", ani_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "operator[]", ani_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "begin", ani_type_named(arena, "std.string.iterator"));
    reg_method(reg, arena, string_qn, "end", ani_type_named(arena, "std.string.iterator"));
    ...
    reg_method(reg, arena, sv_qn, "substr", ani_type_named(arena, sv_qn));
    ...
    const ANIType* t_T = ani_type_type_param(arena, "T");
    const ANIType* t_T_ref = ani_type_reference(arena, t_T);
    const ANIType* t_T_ptr = ani_type_pointer(arena, t_T);
    ...
    reg_method(reg, arena, vec_qn, "begin", ani_type_named(arena, "std.vector.iterator"));
    reg_method(reg, arena, vec_qn, "end", ani_type_named(arena, "std.vector.iterator"));
    reg_method(reg, arena, vec_qn, "erase", ani_type_named(arena, "std.vector.iterator"));
    reg_method(reg, arena, vec_qn, "insert", ani_type_named(arena, "std.vector.iterator"));
    ...
    const ANIType* t_V = ani_type_type_param(arena, "V");
    const ANIType* t_V_ref = ani_type_reference(arena, t_V);

    reg_method(reg, arena, map_qn, "operator[]", t_V_ref);
    reg_method(reg, arena, map_qn, "at", t_V_ref);
    reg_method(reg, arena, map_qn, "find", ani_type_named(arena, "std.map.iterator"));
    reg_method(reg, arena, map_qn, "insert", ani_type_named(arena, "std.pair"));
    reg_method(reg, arena, map_qn, "emplace", ani_type_named(arena, "std.pair"));
    ...
    reg_method(reg, arena, map_qn, "begin", ani_type_named(arena, "std.map.iterator"));
    reg_method(reg, arena, map_qn, "end", ani_type_named(arena, "std.map.iterator"));
    ...
    reg_method(reg, arena, umap_qn, "find", ani_type_named(arena, "std.unordered_map.iterator"));
    reg_method(reg, arena, umap_qn, "insert", ani_type_named(arena, "std.pair"));
    reg_method(reg, arena, umap_qn, "emplace", ani_type_named(arena, "std.pair"));
    ...
    reg_method(reg, arena, umap_qn, "begin", ani_type_named(arena, "std.unordered_map.iterator"));
    reg_method(reg, arena, umap_qn, "end", ani_type_named(arena, "std.unordered_map.iterator"));
    ...
    reg_method(reg, arena, set_qn, "insert", ani_type_named(arena, "std.pair"));
    reg_method(reg, arena, set_qn, "find", ani_type_named(arena, "std.set.iterator"));
    ...
    reg_method(reg, arena, set_qn, "begin", ani_type_named(arena, "std.set.iterator"));
    reg_method(reg, arena, set_qn, "end", ani_type_named(arena, "std.set.iterator"));

    const char* uset_qn = "std.unordered_set";
    reg_type(reg, uset_qn, "unordered_set");
    reg_method(reg, arena, uset_qn, "insert", ani_type_named(arena, "std.pair"));
    reg_method(reg, arena, uset_qn, "find", ani_type_named(arena, "std.unordered_set.iterator"));
    ...
    reg_method(reg, arena, wptr_qn, "lock", ani_type_named(arena, sptr_qn));
    ...
    reg_func(reg, arena, "std.make_unique", "make_unique",
        ani_type_named(arena, uptr_qn));
    reg_func(reg, arena, "std.make_shared", "make_shared",
        ani_type_named(arena, sptr_qn));
    ...
    const char* pair_qn = "std.pair";
    {
        ...
        const ANIType** pair_field_types = (const ANIType**)ani_arena_alloc(arena,
            3 * sizeof(const ANIType*));
        pair_field_types[0] = ani_type_type_param(arena, "T1");
        pair_field_types[1] = ani_type_type_param(arena, "T2");
    ...
    reg_func(reg, arena, "std.get", "get", ani_type_unknown());
    ...
    reg_method(reg, arena, "std.function", "operator()", ani_type_unknown());
    ...
    const ANIType* t_ostream_ref = ani_type_reference(arena, ani_type_named(arena, os_qn));
    ...
    const ANIType* t_istream_ref = ani_type_reference(arena, ani_type_named(arena, is_qn));
    ...
    reg_func(reg, arena, "std.cout", "cout", ani_type_named(arena, os_qn));
    reg_func(reg, arena, "std.cin", "cin", ani_type_named(arena, is_qn));
    reg_func(reg, arena, "std.cerr", "cerr", ani_type_named(arena, os_qn));
    reg_func(reg, arena, "std.clog", "clog", ani_type_named(arena, os_qn));
    ...
    const ANIType* t_path = ani_type_named(arena, fspath_qn);
    ...
    reg_method(reg, arena, fspath_qn, "operator/=", ani_type_reference(arena, t_path));
    ...
    reg_method(reg, arena, thread_qn, "get_id", ani_type_named(arena, "std.thread.id"));
    ...
    reg_func(reg, arena, "std.chrono.system_clock.now", "now",
        ani_type_named(arena, "std.chrono.time_point"));
    ...
    reg_func(reg, arena, "std.find", "find", ani_type_unknown());
    reg_func(reg, arena, "std.find_if", "find_if", ani_type_unknown());
    reg_func(reg, arena, "std.for_each", "for_each", ani_type_unknown());
    reg_func(reg, arena, "std.transform", "transform", ani_type_unknown());
    reg_func(reg, arena, "std.copy", "copy", ani_type_unknown());
    reg_func(reg, arena, "std.move", "move", ani_type_unknown());
    reg_func(reg, arena, "std.swap", "swap", t_void);
    reg_func(reg, arena, "std.min", "min", ani_type_unknown());
    reg_func(reg, arena, "std.max", "max", ani_type_unknown());
    reg_func(reg, arena, "std.accumulate", "accumulate", ani_type_unknown());
    reg_func(reg, arena, "std.count", "count", t_size_t);
    reg_func(reg, arena, "std.count_if", "count_if", t_size_t);
    reg_func(reg, arena, "std.remove", "remove", ani_type_unknown());
    reg_func(reg, arena, "std.remove_if", "remove_if", ani_type_unknown());
    reg_func(reg, arena, "std.reverse", "reverse", t_void);
    reg_func(reg, arena, "std.unique", "unique", ani_type_unknown());
    reg_func(reg, arena, "std.lower_bound", "lower_bound", ani_type_unknown());
    reg_func(reg, arena, "std.upper_bound", "upper_bound", ani_type_unknown());
    ...
    reg_func(reg, arena, "std.stof", "stof", ani_type_builtin(arena, "float"));
    reg_func(reg, arena, "std.stod", "stod", ani_type_builtin(arena, "double"));
    ...
    reg_method(reg, arena, "std.any", "type", ani_type_unknown()); // type_info
    reg_method(reg, arena, "std.any", "reset", t_void);
    reg_func(reg, arena, "std.any_cast", "any_cast", t_T);
    reg_func(reg, arena, "std.make_any", "make_any", ani_type_named(arena, "std.any"));
    ...
    reg_method(reg, arena, "std.smatch", "prefix", ani_type_unknown());
    reg_method(reg, arena, "std.smatch", "suffix", ani_type_unknown());
    ...
    reg_func(reg, arena, "std.partial_sum", "partial_sum", ani_type_unknown());
    reg_func(reg, arena, "std.adjacent_difference", "adjacent_difference", ani_type_unknown());
    ...
    reg_func(reg, arena, "std.distance", "distance", ani_type_builtin(arena, "ptrdiff_t"));
    reg_func(reg, arena, "std.next", "next", ani_type_unknown()); // iterator
    reg_func(reg, arena, "std.prev", "prev", ani_type_unknown());
    reg_func(reg, arena, "std.begin", "begin", ani_type_unknown());
    reg_func(reg, arena, "std.end", "end", ani_type_unknown());
    reg_func(reg, arena, "std.rbegin", "rbegin", ani_type_unknown());
    reg_func(reg, arena, "std.rend", "rend", ani_type_unknown());
    ...
    reg_func(reg, arena, "std.min_element", "min_element", ani_type_unknown());
    reg_func(reg, arena, "std.max_element", "max_element", ani_type_unknown());
    ...
    reg_func(reg, arena, "std.partition", "partition", ani_type_unknown());
    reg_func(reg, arena, "std.stable_partition", "stable_partition", ani_type_unknown());
    ...
    reg_func(reg, arena, "std.merge", "merge", ani_type_unknown());
    reg_func(reg, arena, "std.includes", "includes", t_bool);
    reg_func(reg, arena, "std.set_union", "set_union", ani_type_unknown());
    reg_func(reg, arena, "std.set_intersection", "set_intersection", ani_type_unknown());
    reg_func(reg, arena, "std.set_difference", "set_difference", ani_type_unknown());
    reg_func(reg, arena, "std.generate", "generate", t_void);
    reg_func(reg, arena, "std.generate_n", "generate_n", ani_type_unknown());
    ...
    reg_method(reg, arena, "std.bitset", "set", ani_type_named(arena, "std.bitset"));
    reg_method(reg, arena, "std.bitset", "reset", ani_type_named(arena, "std.bitset"));
    reg_method(reg, arena, "std.bitset", "flip", ani_type_named(arena, "std.bitset"));
    ...
    reg_method(reg, arena, "std.bitset", "to_ulong", ani_type_builtin(arena, "unsigned long"));
    ...
    reg_func(reg, arena, "std.filesystem.file_size", "file_size", ani_type_builtin(arena, "uintmax_t"));
    ...
    reg_func(reg, arena, "std.filesystem.remove_all", "remove_all", ani_type_builtin(arena, "uintmax_t"));
    ...
    reg_func(reg, arena, "std.filesystem.current_path", "current_path",
        ani_type_named(arena, fspath_qn));
    ...
    reg_method(reg, arena, direntry_qn, "path", ani_type_named(arena, fspath_qn));
    ...
    reg_method(reg, arena, direntry_qn, "file_size", ani_type_builtin(arena, "uintmax_t"));

    // =========================================================================
    // std.chrono — additional
    // =========================================================================
    reg_type(reg, "std.chrono.time_point", "time_point");
    reg_method(reg, arena, "std.chrono.time_point", "time_since_epoch", ani_type_unknown());
    reg_method(reg, arena, "std.chrono.system_clock", "now",
        ani_type_named(arena, "std.chrono.time_point"));
    reg_method(reg, arena, "std.chrono.steady_clock", "now",
        ani_type_named(arena, "std.chrono.time_point"));
    reg_func(reg, arena, "std.chrono.duration_cast", "duration_cast", ani_type_unknown());
    ...
    reg_method(reg, arena, bfspath_qn, "parent_path", ani_type_named(arena, bfspath_qn));
    reg_method(reg, arena, bfspath_qn, "filename", ani_type_named(arena, bfspath_qn));
    reg_method(reg, arena, bfspath_qn, "extension", ani_type_named(arena, bfspath_qn));
    reg_method(reg, arena, bfspath_qn, "stem", ani_type_named(arena, bfspath_qn));
    ...
    reg_method(reg, arena, absv_qn, "data", ani_type_pointer(arena, ani_type_builtin(arena, "char")));
    ...
    reg_method(reg, arena, absv_qn, "substr", ani_type_named(arena, absv_qn));

    const char* abstat_qn = "absl.Status";
    reg_type(reg, abstat_qn, "Status");
    reg_method(reg, arena, abstat_qn, "ok", t_bool);
    reg_method(reg, arena, abstat_qn, "code", ani_type_unknown());
    reg_method(reg, arena, abstat_qn, "message", ani_type_named(arena, absv_qn));
    reg_method(reg, arena, abstat_qn, "ToString", t_string);
    reg_func(reg, arena, "absl.OkStatus", "OkStatus", ani_type_named(arena, abstat_qn));

    const char* absor_qn = "absl.StatusOr";
    reg_type(reg, absor_qn, "StatusOr");
    reg_method(reg, arena, absor_qn, "ok", t_bool);
    reg_method(reg, arena, absor_qn, "status", ani_type_named(arena, abstat_qn));
    ...
    reg_method(reg, arena, abfhm_qn, "find", ani_type_unknown());
    ...
    reg_method(reg, arena, abfhm_qn, "insert", ani_type_unknown());
    ...
    reg_method(reg, arena, abfhs_qn, "insert", ani_type_unknown());
    ...
    reg_method(reg, arena, abspan_qn, "subspan", ani_type_named(arena, abspan_qn));
    ...
    reg_func(reg, arena, "absl.StrSplit", "StrSplit",
        ani_type_named(arena, "std.vector"));
    ...
    reg_method(reg, arena, grpc_status_qn, "error_code", ani_type_unknown());
    reg_method(reg, arena, grpc_status_qn, "error_message", t_string);
    reg_func(reg, arena, "grpc.Status.OK", "OK", ani_type_named(arena, grpc_status_qn));

    const char* grpc_sb_qn = "grpc.ServerBuilder";
    reg_type(reg, grpc_sb_qn, "ServerBuilder");
    reg_method(reg, arena, grpc_sb_qn, "AddListeningPort", ani_type_named(arena, grpc_sb_qn));
    reg_method(reg, arena, grpc_sb_qn, "RegisterService", ani_type_named(arena, grpc_sb_qn));
    reg_method(reg, arena, grpc_sb_qn, "BuildAndStart", ani_type_unknown());

    const char* grpc_chan_qn = "grpc.Channel";
    reg_type(reg, grpc_chan_qn, "Channel");
    reg_func(reg, arena, "grpc.CreateChannel", "CreateChannel",
        ani_type_named(arena, grpc_chan_qn));
    ...
    reg_func(reg, arena, "spdlog.get", "get",
        ani_type_named(arena, spdlog_qn));
    ...
    reg_method(reg, arena, qobj_qn, "parent", ani_type_pointer(arena, ani_type_named(arena, qobj_qn)));
    reg_method(reg, arena, qobj_qn, "children", ani_type_unknown());
    reg_method(reg, arena, qobj_qn, "objectName", ani_type_named(arena, "QString"));
    ...
    reg_method(reg, arena, qstr_qn, "toUtf8", ani_type_unknown());
    reg_method(reg, arena, qstr_qn, "toLatin1", ani_type_unknown());
    ...
    reg_method(reg, arena, qstr_qn, "trimmed", ani_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "simplified", ani_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "toLower", ani_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "toUpper", ani_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "arg", ani_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "split", ani_type_unknown());
    reg_method(reg, arena, qstr_qn, "replace", ani_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "mid", ani_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "left", ani_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "right", ani_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "toInt", t_int);
    reg_method(reg, arena, qstr_qn, "toDouble", ani_type_builtin(arena, "double"));
    reg_func(reg, arena, "QString.number", "number", ani_type_named(arena, qstr_qn));
    reg_func(reg, arena, "QString.fromStdString", "fromStdString", ani_type_named(arena, qstr_qn));
    reg_func(reg, arena, "QString.fromUtf8", "fromUtf8", ani_type_named(arena, qstr_qn));
