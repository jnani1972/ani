...
#define REG_FUNC(qn, short, ret_type) do { \
    ...
    rf.short_name = (short); \
    rf.signature = ani_type_func(arena, NULL, NULL, (const ANIType*[]){(ret_type), NULL}); \
...
void ani_c_stdlib_register(ANITypeRegistry* reg, ANIArena* arena) {
    ...
    const ANIType* t_double = ani_type_builtin(arena, "double");
    const ANIType* t_void = ani_type_builtin(arena, "void");
    const ANIType* t_char_ptr = ani_type_pointer(arena, ani_type_builtin(arena, "char"));
    const ANIType* t_void_ptr = ani_type_pointer(arena, t_void);

    // FILE type
    REG_TYPE("FILE", "FILE");
    const ANIType* t_file_ptr = ani_type_pointer(arena, ani_type_named(arena, "FILE"));
    ...
    REG_FUNC("ftell", "ftell", ani_type_builtin(arena, "long"));
    ...
    REG_FUNC("atol", "atol", ani_type_builtin(arena, "long"));
    REG_FUNC("strtol", "strtol", ani_type_builtin(arena, "long"));
    ...
    REG_FUNC("time", "time", ani_type_builtin(arena, "time_t"));
    REG_FUNC("clock", "clock", ani_type_builtin(arena, "clock_t"));
