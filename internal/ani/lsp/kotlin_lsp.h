 ...
 * scope, infers expression types, and emits ANIResolvedCall entries for
 ...
 * call-edge and method-dispatch attribution problems ANI cares about.
 *
 ...
 */
#ifndef ANI_LSP_KOTLIN_LSP_H
#define ANI_LSP_KOTLIN_LSP_H
...
typedef enum {
    ANI_KT_USE_UNKNOWN = 0,
    ANI_KT_USE_TYPE,     /* class / interface / object / typealias */
    ANI_KT_USE_FUNCTION, /* top-level fun, extension fun */
    ANI_KT_USE_PROPERTY, /* top-level val/var */
    ANI_KT_USE_WILDCARD, /* import a.b.* — local_name is a.b prefix */
} ANIKotlinUseKind;

typedef struct {
    const ANIScope *scope; /* exact lexical declaration frame, NULL for members */
    const char *owner_qn;  /* class/object QN for a member, NULL for lexical values */
    const char *name;
    const char *getter_qn;
    const char *setter_qn;
} ANIKotlinDelegateBinding;
...
typedef struct KotlinLSPContext {
    ...
    ANIKotlinDelegateBinding *delegate_bindings;
    int delegate_count;
    int delegate_cap;
     ...
     * overflow on deeply-nested/cyclic files; see ani_lsp_max_walk_depth).
     * Zero via memset. */
    int walk_depth;
     ...
     * QN, built from the project defs by ani_run_kotlin_lsp_cross. Consulted
     ...
     * ani_run_kotlin_lsp_cross; opaque pointer to avoid a foundation include
     * in this header. */
    const void *cross_type_short;
    /* Cross-file only (NULL per-file): receiver type QN -> KtCrossFieldList,
     * from ani_kotlin_register_lsp_defs. Property-reference emission reads
 ...
 * pass the package prefix as target_qn and ANI_KT_USE_WILDCARD as kind. */
...
/* Evaluate a Kotlin expression's type. Returns ani_type_unknown() on
 * failure — never NULL. */
 ...
 * Called from ani_extract_file() after definitions and imports have been
 * extracted. */
 ...
 * Mirrors ani_run_java_lsp_cross. `defs` carries the graph QNs of every
 ...
 * list map built during registration (opaque ANIHashTable; the caller frees
 * with ani_ht_free). Pass NULL to discard it. The forward declaration lives at
...
void ani_kotlin_register_lsp_defs(ANIArena *arena, ANITypeRegistry *reg, const ANILSPDef *defs,
                                  int def_count, struct ANIHashTable **out_field_map);
...
void ani_kotlin_stdlib_register(ANITypeRegistry *reg, ANIArena *arena);

/* Register Kotlin default-import targets — the prefixes auto-imported
 * by every Kotlin file. Used by the LSP context init. */
const char *const *ani_kotlin_default_import_packages(int *count_out);

#endif /* ANI_LSP_KOTLIN_LSP_H */
