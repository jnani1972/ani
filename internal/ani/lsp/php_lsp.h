#ifndef ANI_LSP_PHP_LSP_H
#define ANI_LSP_PHP_LSP_H

#include "type_rep.h"
#include "scope.h"
#include "type_registry.h"
#include "../ani.h"
#include "go_lsp.h"  /* ANILSPDef reused across languages */
...
typedef struct {
    ...
    enum { ANI_PHP_USE_CLASS = 0, ANI_PHP_USE_FUNCTION, ANI_PHP_USE_CONST } *use_kinds;
    int use_count;
    int use_cap;
    ...
    const ANIType **phpstan_alias_types;
...
/* Evaluate a PHP expression's type. May return NULL / ANI_TYPE_UNKNOWN. */
const ANIType *php_eval_expr_type(PHPLSPContext *ctx, TSNode node);

/* Parse a PHP type-AST node (named_type, primitive_type, union_type, ...) to ANIType. */
 ...
 * then run resolution. Called from ani_extract_file(). */
 ...
 *
 * Mirrors ani_run_py_lsp_cross / ani_run_ts_lsp_cross. Caller supplies the
 * combined ANILSPDef[] (file-local + cross-file) and a resolved import map
 ...
 * file_count pre-zeroed ANIResolvedCallArray structs. */
void ani_batch_php_lsp_cross(
    ANIArena *arena,
    ANIBatchPHPLSPFile *files, int file_count,
    ANIResolvedCallArray *out);

#endif /* ANI_LSP_PHP_LSP_H */
