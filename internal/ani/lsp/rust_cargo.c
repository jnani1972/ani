...
static int parse_key(ANIArena* a, const char* s, int len, int from,
    const char** out) {
    int start = from;
    if (from < len && s[from] == '"') {
        from++;
        ...
        *out = ani_arena_strndup(a, s + start, (size_t)(from - start));
...
static int parse_string(ANIArena* a, const char* s, int len, int from,
    const char** out) {
...
static int parse_section(ANIArena* a, const char* s, int len, int from,
    const char** out) {
 ...
 * matter for us. */
static int parse_dep_entry(ANIArena* a, const char* s, int len, int from,
    ...
    if (key && out->dep_count < ANI_CARGO_MAX_DEPS) {
...
static int parse_package_kv(ANIArena* a, const char* s, int len, int from,
    ANICargoManifest* out) {
    from = skip_ws_and_comment(s, len, from);
    if (from >= len || s[from] == '[') return from;
    const char* key = NULL;
...
static int parse_workspace_kv(ANIArena* a, const char* s, int len, int from,
    ANICargoManifest* out) {
    from = skip_ws_and_comment(s, len, from);
    if (from >= len || s[from] == '[') return from;
    ...
    if (key && strcmp(key, "members") == 0 && from < len && s[from] == '[') {
        ...
        while (from < len && s[from] != ']') {
            ...
            if (from < len && (s[from] == '"' || s[from] == '\'')) {
                ...
                if (mem && out->member_count < ANI_CARGO_MAX_MEMBERS) {
