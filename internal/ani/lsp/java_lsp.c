 ...
 *   - We populate a ANITypeRegistry from this file's ANIDefinitions plus a
 ...
 *     compatibility (ani_registry_lookup_method_by_types). Ambiguity ⇒
 ...
 *
 * Confidence policy (consumed by lsp_resolve.h with ANI_LSP_CONFIDENCE_FLOOR
 * = 0.6):
...
static const ANIType *eval_method_invocation(JavaLSPContext *ctx, TSNode node);
static const ANIType *eval_object_creation(JavaLSPContext *ctx, TSNode node);
static const ANIType *eval_field_access(JavaLSPContext *ctx, TSNode node);
static const ANIType *eval_array_access(JavaLSPContext *ctx, TSNode node);
static const ANIType *eval_cast(JavaLSPContext *ctx, TSNode node);
static const ANIType *eval_ternary(JavaLSPContext *ctx, TSNode node);
static const ANIType *eval_binary(JavaLSPContext *ctx, TSNode node);
static const ANIType *eval_unary(JavaLSPContext *ctx, TSNode node);
static const ANIType *eval_lambda(JavaLSPContext *ctx, TSNode node);
static const ANIType *eval_method_reference(JavaLSPContext *ctx, TSNode node);
static const ANIType *resolve_identifier_type(JavaLSPContext *ctx, const char *name);
static const ANIType *resolve_member_type(JavaLSPContext *ctx, const ANIType *recv_type,
                                          const char *member_name);
static void resolve_method_call(JavaLSPContext *ctx, TSNode call);
static void register_local_func_or_type_from_file(JavaLSPContext *ctx, ANITypeRegistry *reg,
                                                  ANIFileResult *result);
static const char *strip_generics(ANIArena *a, const char *type_text);
static const char *unwrap_array_text(ANIArena *a, const char *type_text, int *out_array_dim);
...
static const ANIType *box_primitive(ANIArena *a, const char *prim);
static int count_call_args(TSNode call_node);
static const ANIType *propagate_template(ANIArena *a, const char *recv_qn, const char *method_name,
                                         const ANIType *const *recv_targs, int recv_targ_count,
                                         const ANIType *return_t);
static bool method_implies_lambda_args(const char *recv_qn, const char *method_name,
                                       const ANIType *const *targs, int targ_count, int *out_arity,
                                       const ANIType **out_param0, const ANIType **out_param1);
static void resolve_method_reference(JavaLSPContext *ctx, TSNode mref,
                                     const ANIRegisteredFunc *outer_resolved, int arg_index,
                                     const ANIType *recv_type);
...
static const char *strip_generics(ANIArena *a, const char *type_text) {
    ...
    if (!lt)
        return type_text;
    return ani_arena_strndup(a, type_text, (size_t)(lt - type_text));
}

/* Strip array suffix (`int[]` → `int`, `String[][]` → `String`, dim=2). */
static const char *unwrap_array_text(ANIArena *a, const char *type_text, int *out_dim) {
    if (out_dim)
        *out_dim = 0;
    if (!type_text)
        return NULL;
    ...
    if (dim == 0)
        return type_text;
    return ani_arena_strndup(a, type_text, n);
}
...
static const ANIType *box_primitive(ANIArena *a, const char *prim) {
    ...
    for (size_t i = 0; i < sizeof(map) / sizeof(map[0]); i++) {
        if (strcmp(prim, map[i][0]) == 0)
            return ani_type_named(a, map[i][1]);
    }
    return ani_type_unknown();
}
...
void java_lsp_add_import(JavaLSPContext *ctx, const char *local_name, const char *target_qn,
    ...
    if (ctx->import_count >= ctx->import_cap) {
        ...
        const char **new_names =
            (const char **)ani_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*new_names));
        const char **new_qns =
            (const char **)ani_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*new_qns));
        int *new_kinds = (int *)ani_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*new_kinds));
    ...
    ctx->import_target_qns[ctx->import_count] = ani_arena_strdup(ctx->arena, target_qn);
...
static const ANIType *parse_type_arguments(JavaLSPContext *ctx, TSNode targs_node);
...
const ANIType *java_parse_type_node(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "scoped_type_identifier") == 0) {
        ...
        if (ani_registry_lookup_type(ctx->registry, full)) {
            return ani_type_named(ctx->arena, full);
        }
        /* Otherwise resolve the head segment and append the rest. */
        char *dot = strchr(full, '.');
        if (dot) {
            char *head = ani_arena_strndup(ctx->arena, full, (size_t)(dot - full));
            const char *head_qn = java_resolve_type_name(ctx, head);
            if (head_qn) {
                return ani_type_named(ctx->arena,
                                      ani_arena_sprintf(ctx->arena, "%s%s", head_qn, dot));
            }
        }
        return ani_type_named(ctx->arena, full);
    }
    ...
    if (strcmp(kind, "generic_type") == 0) {
        ...
        const ANIType *base = java_parse_type_node(ctx, raw);
        ...
        if (arg_count == 0) {
            arg_buf[arg_count++] = ani_type_unknown();
        }
        const ANIType **args =
            (const ANIType **)ani_arena_alloc(ctx->arena, (size_t)(arg_count + 1) * sizeof(*args));
        for (int i = 0; i < arg_count; i++)
            args[i] = arg_buf[i];
        args[arg_count] = NULL;
        return ani_type_template(ctx->arena, base_qn, args, arg_count);
    }
    ...
    if (strcmp(kind, "array_type") == 0) {
        ...
        return ani_type_slice(ctx->arena, java_parse_type_node(ctx, elem));
    }
...
const char *java_resolve_type_name(JavaLSPContext *ctx, const char *name) {
    ...
    for (int i = ctx->enclosing_class_depth - 1; i >= 0; i--) {
        ...
        if (n > 0 && (size_t)n < sizeof(buf)) {
            if (ani_registry_lookup_type(ctx->registry, buf)) {
                return ani_arena_strdup(ctx->arena, buf);
            }
        }
    }
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        if (ctx->import_kinds[i] != ANI_JAVA_IMPORT_TYPE)
            continue;
    ...
    /* 4. On-demand imports. */
    for (int i = 0; i < ctx->import_count; i++) {
        if (ctx->import_kinds[i] != ANI_JAVA_IMPORT_ON_DEMAND)
            continue;
        char buf[JAVA_LSP_BUF];
    ...
    if (ani_registry_lookup_type(ctx->registry, name)) {
    ...
    if (ctx->registry && ctx->registry->types) {
        ...
        for (int i = 0; i < ctx->registry->type_count && matches < 2; i++) {
            const ANIRegisteredType *t = &ctx->registry->types[i];
...
const ANIType *java_eval_expr_type(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "decimal_integer_literal") == 0 || strcmp(kind, "hex_integer_literal") == 0 ||
        ...
        if (txt) {
            ...
            if (len > 0 && (txt[len - 1] == 'l' || txt[len - 1] == 'L')) {
                result = ani_type_builtin(ctx->arena, "long");
                goto out;
            }
        }
        result = ani_type_builtin(ctx->arena, "int");
        goto out;
    }
    if (strcmp(kind, "decimal_floating_point_literal") == 0 ||
        ...
        if (txt) {
            ...
            if (len > 0 && (txt[len - 1] == 'f' || txt[len - 1] == 'F')) {
                result = ani_type_builtin(ctx->arena, "float");
                goto out;
            }
        }
        result = ani_type_builtin(ctx->arena, "double");
        goto out;
    }
    ...
    if (strcmp(kind, "character_literal") == 0) {
        result = ani_type_builtin(ctx->arena, "char");
        goto out;
    }
    if (strcmp(kind, "string_literal") == 0 || strcmp(kind, "string_fragment") == 0) {
        result = ani_type_named(ctx->arena, "java.lang.String");
        goto out;
    }
    ...
    if (strcmp(kind, "super") == 0) {
        if (ctx->enclosing_super_qn) {
            result = ani_type_named(ctx->arena, ctx->enclosing_super_qn);
        } else {
            result = ani_type_named(ctx->arena, "java.lang.Object");
        }
        goto out;
    }
    ...
    if (strcmp(kind, "class_literal") == 0) {
        result = ani_type_named(ctx->arena, "java.lang.Class");
        goto out;
    }
    ...
    if (strcmp(kind, "array_creation_expression") == 0) {
        ...
        const ANIType *elem = java_parse_type_node(ctx, type_node);
        result = ani_type_slice(ctx->arena, elem);
        goto out;
    }
...
static const ANIType *resolve_identifier_type(JavaLSPContext *ctx, const char *name) {
    ...
    if (ctx->enclosing_class_qn) {
        const ANIType *ft = java_lookup_field_type(ctx, ctx->enclosing_class_qn, name);
        if (ft && !ani_type_is_unknown(ft))
            return ft;
    }
    /* Outer classes. */
    for (int i = ctx->enclosing_class_depth - 2; i >= 0; i--) {
        ...
        if (!outer)
            continue;
        const ANIType *ft = java_lookup_field_type(ctx, outer, name);
        if (ft && !ani_type_is_unknown(ft))
            return ft;
    }
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        if (ctx->import_kinds[i] != ANI_JAVA_IMPORT_STATIC)
            continue;
        if (strcmp(ctx->import_local_names[i], name) == 0) {
            ...
            if (last_dot) {
                char *cls = ani_arena_strndup(ctx->arena, target, (size_t)(last_dot - target));
                const ANIType *ft = java_lookup_field_type(ctx, cls, name);
                if (ft && !ani_type_is_unknown(ft))
                    return ft;
            }
            return ani_type_unknown();
...
static const ANIType *eval_field_access(JavaLSPContext *ctx, TSNode node) {
    ...
    if (!ts_node_is_null(obj) && strcmp(ts_node_type(obj), "identifier") == 0) {
        ...
        if (oname && strcmp(oname, "System") == 0) {
            if (strcmp(fname, "out") == 0 || strcmp(fname, "err") == 0) {
                return ani_type_named(ctx->arena, "java.io.PrintStream");
            }
            if (strcmp(fname, "in") == 0) {
                return ani_type_named(ctx->arena, "java.io.InputStream");
            }
        }
    }

    /* Special-case: `length` on array types is `int`. */
    const ANIType *recv = ts_node_is_null(obj) ? ani_type_unknown() : java_eval_expr_type(ctx, obj);
    if (recv && recv->kind == ANI_TYPE_SLICE && strcmp(fname, "length") == 0) {
    ...
    if (!ts_node_is_null(obj) && strcmp(ts_node_type(obj), "this") == 0) {
        const ANIType *scope_t = ani_scope_lookup(ctx->current_scope, fname);
        if (scope_t && !ani_type_is_unknown(scope_t))
            return scope_t;
    }

    const ANIType *res = resolve_member_type(ctx, recv, fname);
    if (res && !ani_type_is_unknown(res))
        return res;
...
const ANIType *java_lookup_field_type(JavaLSPContext *ctx, const char *class_qn,
    ...
    for (int hops = 0; hops < JAVA_LSP_MAX_INHERIT_HOPS && cur; hops++) {
        const ANIRegisteredType *rt = ani_registry_lookup_type(ctx->registry, cur);
        if (!rt)
            break;
...
static const ANIType *resolve_member_type(JavaLSPContext *ctx, const ANIType *recv,
                                          const char *member_name) {
    if (!recv || !member_name)
        return ani_type_unknown();
...
const ANIRegisteredFunc *java_lookup_method(JavaLSPContext *ctx, const char *class_qn,
    ...
    if (!class_qn || !method_name)
        return NULL;
    const char *cur = class_qn;
    const ANIRegisteredFunc *fallback = NULL;
    for (int hops = 0; hops < JAVA_LSP_MAX_INHERIT_HOPS && cur; hops++) {
        /* Try arg-count-aware lookup first. */
        const ANIRegisteredFunc *m =
            ani_registry_lookup_method_by_args(ctx->registry, cur, method_name, arg_count);
        if (m)
            return m;
        /* Otherwise capture any name match as fallback. */
        if (!fallback) {
            fallback = ani_registry_lookup_method(ctx->registry, cur, method_name);
        }
        const ANIRegisteredType *rt = ani_registry_lookup_type(ctx->registry, cur);
        if (!rt)
            break;
...
static const ANIType *java_return_type_of(const ANIRegisteredFunc *f) {
    if (!f || !f->signature)
        return ani_type_unknown();
    if (f->signature->kind != ANI_TYPE_FUNC)
        return ani_type_unknown();
    const ANIType *const *rets = f->signature->data.func.return_types;
    if (!rets || !rets[0])
        return ani_type_unknown();
    return rets[0];
}
...
static const ANIType *substitute_generic_return(JavaLSPContext *ctx, const char *recv_qn,
                                                const char *method_name,
                                                const ANIType *const *arg_arr, int targ_count,
                                                const ANIType *fallback) {
    (void)ctx;
    ...
    if (fallback->kind != ANI_TYPE_NAMED)
        return fallback;
...
static const ANIType *eval_method_invocation(JavaLSPContext *ctx, TSNode node) {
    ...
    if (ts_node_is_null(obj)) {
        ...
        for (int i = 0; i < ctx->import_count; i++) {
            ...
            if (!last_dot)
                continue;
            char *cls = ani_arena_strndup(ctx->arena, target, (size_t)(last_dot - target));
            const ANIRegisteredFunc *f = java_lookup_method(ctx, cls, mname, arity);
            if (f)
                return java_return_type_of(f);
        }
        return ani_type_unknown();
    ...
    if (strcmp(ts_node_type(obj), "super") == 0) {
        ...
        const ANIRegisteredFunc *f = java_lookup_method(ctx, super_qn, mname, arity);
        if (f)
            return java_return_type_of(f);
        return ani_type_unknown();
    }
    ...
    if (strcmp(ts_node_type(obj), "identifier") == 0) {
        ...
        if (oname) {
            ...
            if (cls_qn) {
                ...
                if (ani_type_is_unknown(ani_scope_lookup(ctx->current_scope, oname))) {
                    const ANIRegisteredFunc *f = java_lookup_method(ctx, cls_qn, mname, arity);
                    if (f)
                        return java_return_type_of(f);
                }
            }
    ...
    /* Instance call. */
    const ANIType *recv = java_eval_expr_type(ctx, obj);
    const ANIType *base = recv;
    if (base && base->kind == ANI_TYPE_TEMPLATE) {
        const char *base_qn = base->data.template_type.template_name;
        if (base_qn) {
            const ANIRegisteredFunc *f = java_lookup_method(ctx, base_qn, mname, arity);
            if (f) {
                const ANIType *ret = java_return_type_of(f);
                /* Generic substitution + carrier propagation. */
                const ANIType *subst = substitute_generic_return(
...
static const ANIType *eval_array_access(JavaLSPContext *ctx, TSNode node) {
    ...
    if (ts_node_is_null(arr))
        return ani_type_unknown();
    const ANIType *t = java_eval_expr_type(ctx, arr);
    if (t && t->kind == ANI_TYPE_SLICE)
...
static const ANIType *eval_ternary(JavaLSPContext *ctx, TSNode node) {
    ...
    const ANIType *t = java_eval_expr_type(ctx, then_n);
    if (t && !ani_type_is_unknown(t))
        return t;
...
static bool is_string_concat(JavaLSPContext *ctx, TSNode node) {
    ...
    TSNode rhs = ts_node_child_by_field_name(node, "right", 5);
    const ANIType *l = java_eval_expr_type(ctx, lhs);
    const ANIType *r = java_eval_expr_type(ctx, rhs);
    if (l && l->kind == ANI_TYPE_NAMED &&
        strcmp(l->data.named.qualified_name, "java.lang.String") == 0)
        return true;
    if (r && r->kind == ANI_TYPE_NAMED &&
...
static const ANIType *eval_binary(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(op, "+") == 0 && is_string_concat(ctx, node)) {
        return ani_type_named(ctx->arena, "java.lang.String");
    }
...
void java_process_statement(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "local_variable_declaration") == 0) {
        ...
        if (rn) {
            const ANIType *rt = ani_type_unknown();
            bool is_var = false;
            ...
            if ((is_var || ani_type_is_unknown(rt)) && !ts_node_is_null(rvalue)) {
                rt = java_eval_expr_type(ctx, rvalue);
            }
            ani_scope_bind(ctx->current_scope, rn, rt ? rt : ani_type_unknown());
        }
        ...
        if (!ts_node_is_null(formal)) {
            ...
            if (pn) {
                ...
                ani_scope_bind(ctx->current_scope, pn, pt);
            }
        }
    }
...
static void process_local_var_decl(JavaLSPContext *ctx, TSNode node) {
    TSNode type_node = ts_node_child_by_field_name(node, "type", 4);
    const ANIType *static_type =
        ts_node_is_null(type_node) ? ani_type_unknown() : java_parse_type_node(ctx, type_node);
    bool is_var =
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (!vname)
            continue;
        const ANIType *bind_type = static_type;
        if ((is_var || ani_type_is_unknown(bind_type)) && !ts_node_is_null(value_node)) {
            bind_type = java_eval_expr_type(ctx, value_node);
        }
        ani_scope_bind(ctx->current_scope, vname, bind_type ? bind_type : ani_type_unknown());
    }
}
...
static void process_enhanced_for(JavaLSPContext *ctx, TSNode node) {
    ...
    const ANIType *t = (is_var || ts_node_is_null(type_node))
                           ? ani_type_unknown()
                           : java_parse_type_node(ctx, type_node);

    /* When type is `var` or unparseable, infer from the iterable. */
    if (ani_type_is_unknown(t) && !ts_node_is_null(value_node)) {
        const ANIType *iter_t = java_eval_expr_type(ctx, value_node);
        if (iter_t && iter_t->kind == ANI_TYPE_SLICE) {
            t = iter_t->data.slice.elem;
        } else if (iter_t && iter_t->kind == ANI_TYPE_TEMPLATE &&
    ...
    ani_scope_bind(ctx->current_scope, vname, t ? t : ani_type_unknown());
}
...
static void process_field_decl(JavaLSPContext *ctx, TSNode node) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (!fname)
            continue;
        ani_scope_bind(ctx->current_scope, fname, static_type);
    }
}
...
static void process_method_decl(JavaLSPContext *ctx, TSNode node, const char *class_qn,
    ...
    if (!mname)
        return;
    char *method_qn = ani_arena_sprintf(ctx->arena, "%s.%s", class_qn, mname);
    ...
    if (!ts_node_is_null(params)) {
        ...
        for (uint32_t i = 0; i < n; i++) {
            ...
            if (!pn)
                continue;
            const ANIType *pt =
                ts_node_is_null(ptype) ? ani_type_unknown() : java_parse_type_node(ctx, ptype);
            if (strcmp(pk, "spread_parameter") == 0 && pt) {
                pt = ani_type_slice(ctx->arena, pt);
            }
            ani_scope_bind(ctx->current_scope, pn, pt);
        }
    }
...
static void process_constructor_decl(JavaLSPContext *ctx, TSNode node, const char *class_qn,
    ...
    char *ctor_qn;
    if (cname) {
        ctor_qn = ani_arena_sprintf(ctx->arena, "%s.%s", class_qn, cname);
    } else {
        ctor_qn = ani_arena_sprintf(ctx->arena, "%s.<init>", class_qn);
    }
...
static const char *class_super_qn(JavaLSPContext *ctx, TSNode class_node) {
    ...
    if (ts_node_is_null(tnode))
        return NULL;
    const ANIType *t = java_parse_type_node(ctx, tnode);
...
static void java_process_class_decl(JavaLSPContext *ctx, TSNode node) {
    ...
    char *class_qn;
    if (ctx->enclosing_class_qn) {
        class_qn = ani_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, cname);
    } else if (ctx->module_qn && ctx->module_qn[0]) {
        class_qn = ani_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
    } else {
        class_qn = ani_arena_strdup(ctx->arena, cname);
    }
...
void java_lsp_process_file(JavaLSPContext *ctx, TSNode root) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (strcmp(k, "package_declaration") == 0) {
            ...
            if (path) {
                if (is_on_demand) {
                    int kind = is_static ? ANI_JAVA_IMPORT_STATIC_OD : ANI_JAVA_IMPORT_ON_DEMAND;
                    ...
                    java_lsp_add_import(ctx, local, path, ANI_JAVA_IMPORT_STATIC);
                } else {
                    const char *last_dot = strrchr(path, '.');
                    const char *local = last_dot ? last_dot + 1 : path;
                    java_lsp_add_import(ctx, local, path, ANI_JAVA_IMPORT_TYPE);
                }
            }
        }
    }
...
static void java_emit_unresolved_kind(JavaLSPContext *ctx, const char *expr_text,
                                      const char *reason, ANIResolvedKind kind) {
...
static void java_emit_unresolved(JavaLSPContext *ctx, const char *expr_text, const char *reason) {
    java_emit_unresolved_kind(ctx, expr_text, reason, ANI_RESOLVED_INVOCATION);
}

static void java_emit_unresolved_reference(JavaLSPContext *ctx, const char *expr_text,
                                           const char *reason) {
    java_emit_unresolved_kind(ctx, expr_text, reason, ANI_RESOLVED_CALL_REFERENCE);
}
...
static const char *java_find_sole_impl(JavaLSPContext *ctx, const char *iface_qn, const char *mname,
    ...
    for (int ti = 0; ti < ctx->registry->type_count && distinct < 2; ti++) {
         ...
         * name array. */
        bool has = ani_registry_lookup_method(ctx->registry, cand->qualified_name, mname) != NULL;
        ...
        for (int hops = 0; hops < JAVA_LSP_MAX_INHERIT_HOPS && cur && !subtype; hops++) {
            const ANIRegisteredType *ct = ani_registry_lookup_type(ctx->registry, cur);
...
static bool java_emit_interface_resolution(JavaLSPContext *ctx, const char *iface_qn,
    ...
    if (impl_count == 1 && sole_impl) {
        const ANIRegisteredFunc *cf = ani_registry_lookup_method(ctx->registry, sole_impl, mname);
        const char *target =
            cf ? cf->qualified_name : ani_arena_sprintf(ctx->arena, "%s.%s", sole_impl, mname);
        java_emit_resolved(ctx, target, "lsp_interface_resolve", 0.85f);
        return true;
    }
    if (impl_count >= 2) {
        java_emit_resolved(ctx, ani_arena_sprintf(ctx->arena, "%s.%s", iface_qn, mname),
...
static void resolve_method_call(JavaLSPContext *ctx, TSNode call) {
    ...
    if (ts_node_is_null(obj)) {
        ...
        for (int i = ctx->enclosing_class_depth - 2; i >= 0; i--) {
            ...
            if (!outer)
                continue;
            const ANIRegisteredFunc *f = java_lookup_method(ctx, outer, mname, arity);
            if (f) {
    ...
    if (strcmp(ts_node_type(obj), "identifier") == 0) {
        char *oname = java_node_text(ctx, obj);
        if (oname && ani_type_is_unknown(ani_scope_lookup(ctx->current_scope, oname))) {
            const char *cls_qn = java_resolve_type_name(ctx, oname);
            if (cls_qn) {
                const ANIRegisteredFunc *f = java_lookup_method(ctx, cls_qn, mname, arity);
                if (f) {
    ...
    if (recv_qn) {
        const ANIRegisteredFunc *f = java_lookup_method(ctx, recv_qn, mname, arity);
        if (f) {
            ...
            const ANIRegisteredType *rt0 = ani_registry_lookup_type(ctx->registry, recv_qn);
        ...
        if (rt && rt->is_interface) {
            ...
            java_emit_resolved(ctx, ani_arena_sprintf(ctx->arena, "%s.%s", recv_qn, mname),
                               "lsp_interface_dispatch", 0.80f);
            return;
        }
        java_emit_unresolved(ctx, ani_arena_sprintf(ctx->arena, "%s.%s", recv_qn, mname),
                             "no_method_match");
        return;
    }
 ...
 * `fi_qn` parameterized with `targs`. Returns ani_type_unknown() when the
 * parametric mapping isn't modeled (e.g. specialized primitive variants). */
static const ANIType *sam_param_type(ANIArena *a, const char *fi_qn, int param_idx,
                                     const ANIType *const *targs, int targ_count) {
    if (!fi_qn)
        return ani_type_unknown();
    ...
    for (int i = 0; single_t[i]; i++) {
        if (strcmp(single_t[i], fi_qn) == 0 && param_idx == 0 && targ_count >= 1) {
            return targs[0] ? targs[0] : ani_type_unknown();
        }
    }
    ...
    if (strcmp(fi_qn, "java.util.function.BiFunction") == 0 ||
        ...
        if (param_idx <= 1 && targ_count > param_idx) {
            return targs[param_idx] ? targs[param_idx] : ani_type_unknown();
        }
    }
 ...
 *
 * Used when the registry's ANIRegisteredFunc lacks param-type metadata
...
static const ANIType *propagate_template(ANIArena *a, const char *recv_qn, const char *method_name,
                                         const ANIType *const *recv_targs, int recv_targ_count,
                                         const ANIType *return_t) {
    if (!return_t || return_t->kind != ANI_TYPE_NAMED)
        return return_t;
    ...
    if (is_t0) {
        const ANIType **args = (const ANIType **)ani_arena_alloc(a, 2 * sizeof(*args));
        if (!args)
            return return_t;
        args[0] = recv_targs[0];
        args[1] = NULL;
        return ani_type_template(a, ret_qn, args, 1);
    }
    ...
    if (is_map_like(recv_qn) && recv_targ_count >= 2) {
        ...
        if (strcmp(method_name, "keySet") == 0) {
            args[0] = recv_targs[0];
            args[1] = NULL;
            return ani_type_template(a, "java.util.Set", args, 1);
        }
        if (strcmp(method_name, "values") == 0) {
            args[0] = recv_targs[1];
            args[1] = NULL;
            return ani_type_template(a, "java.util.Collection", args, 1);
        }
    }
    return return_t;
}

/* Given a method-invocation node and its resolved ANIRegisteredFunc, walk
...
static uint32_t bind_lambda_args(JavaLSPContext *ctx, TSNode call_node,
                                 const ANIRegisteredFunc *resolved, const ANIType *recv_type) {
    uint32_t handled_mask = 0;
    TSNode args_node = ts_node_child_by_field_name(call_node, "arguments", 9);
    if (ts_node_is_null(args_node))
        return handled_mask;
    const ANIType *const *param_types = NULL;
    if (resolved && resolved->signature && resolved->signature->kind == ANI_TYPE_FUNC) {
    ...
    const ANIType *const *recv_targs = NULL;
    ...
    } else if (recv_type && recv_type->kind == ANI_TYPE_NAMED) {
    ...
    for (uint32_t i = 0; i < n && i < 32; i++) {
        ...
        if (!expected && recv_qn && mname && recv_targ_count > 0) {
            ...
            int h_arity = 0;
            const ANIType *h_p0 = NULL;
            const ANIType *h_p1 = NULL;
                ...
                if (!ts_node_is_null(body_node)) {
                    ...
                    if (!ts_node_is_null(params_node)) {
                        ...
                        if (strcmp(pn_kind, "identifier") == 0) {
                            ...
                            if (pname)
                                ani_scope_bind(ctx->current_scope, pname, h_p0);
                            ...
                            for (uint32_t pi = 0; pi < pc; pi++) {
                                ...
                                const char *pk = ts_node_type(p);
                                char *pname = NULL;
                                const ANIType *pt = (idx == 0) ? h_p0 : h_p1;
        ...
        const ANIType *const *fi_targs = NULL;
        int fi_targ_count = 0;
        if (expected->kind == ANI_TYPE_NAMED) {
            fi_qn = expected->data.named.qualified_name;
        } else if (expected->kind == ANI_TYPE_TEMPLATE) {
         ...
         * cases below. */
        const ANIType *resolved_fi_targs[8] = {0};
        ...
        if (!ts_node_is_null(params_node)) {
            ...
            if (strcmp(pn_kind, "identifier") == 0) {
                ...
                for (uint32_t pi = 0; pi < pc; pi++) {
                    ...
                    if (pname) {
                        ani_scope_bind(ctx->current_scope, pname, pt ? pt : ani_type_unknown());
                        idx++;
                    }
                }
            }
 ...
 *
 * Emits a ANIResolvedCall edge for the referenced method when we can pin
...
static void resolve_method_reference(JavaLSPContext *ctx, TSNode mref,
    ...
    if (outer_resolved && outer_resolved->signature &&
        outer_resolved->signature->kind == ANI_TYPE_FUNC &&
        ...
        const ANIType *const *pts = outer_resolved->signature->data.func.param_types;
        int ptc = 0;
        while (pts[ptc]) {
            ptc++;
        }
        const ANIType *expected = (arg_index >= 0 && arg_index < ptc) ? pts[arg_index] : NULL;
        if (expected) {
            const char *fi_qn = NULL;
            if (expected->kind == ANI_TYPE_NAMED)
                fi_qn = expected->data.named.qualified_name;
    ...
    if (lhs_is_type) {
        ...
        if (lhs_t && lhs_t->kind == ANI_TYPE_NAMED)
            type_qn = lhs_t->data.named.qualified_name;
        else if (lhs_t && lhs_t->kind == ANI_TYPE_TEMPLATE)
    ...
    if (strcmp(mname, "new") == 0) {
        ...
        const ANIRegisteredFunc *cf =
            ani_registry_lookup_method(ctx->registry, type_qn, short_name);
    ...
    if (m) {
        ...
        java_emit_unresolved_reference_at(ctx,
                                          ani_arena_sprintf(ctx->arena, "%s.%s", type_qn, mname),
...
static const ANIRegisteredFunc *lookup_method_for_call(JavaLSPContext *ctx, TSNode call,
                                                       const ANIType **out_recv_type) {
    if (out_recv_type)
        *out_recv_type = NULL;
    TSNode obj = ts_node_child_by_field_name(call, "object", 6);
    TSNode name_node = ts_node_child_by_field_name(call, "name", 4);
    ...
    if (recv && recv->kind == ANI_TYPE_NAMED)
        recv_qn = recv->data.named.qualified_name;
    else if (recv && recv->kind == ANI_TYPE_TEMPLATE)
        recv_qn = recv->data.template_type.template_name;
    if (recv_qn)
...
static void java_resolve_calls_in_node_inner(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "method_invocation") == 0) {
        ...
        const ANIType *recv_type = NULL;
        const ANIRegisteredFunc *outer = lookup_method_for_call(ctx, node, &recv_type);
        if (outer) {
        ...
        TSNode type_node = ts_node_child_by_field_name(node, "type", 4);
        if (!ts_node_is_null(type_node)) {
            const ANIType *t = java_parse_type_node(ctx, type_node);
            const char *qn = NULL;
            if (t && t->kind == ANI_TYPE_NAMED)
            ...
            if (qn) {
                ...
                const ANIRegisteredFunc *cf =
                    ani_registry_lookup_method_by_args(ctx->registry, qn, short_name, arity);
                if (!cf)
                    cf = ani_registry_lookup_method(ctx->registry, qn, short_name);
                if (cf) {
...
static const char **split_generic_args(ANIArena *a, const char *inside, int *out_count) {
    *out_count = 0;
    if (!inside || !inside[0])
        return NULL;
    const char *args[16];
    ...
    for (const char *p = inside; *p; p++) {
        if (*p == '<')
            ...
            if (count < 16 && seg_end > seg_start) {
                args[count++] = ani_arena_strndup(a, seg_start, (size_t)(seg_end - seg_start));
            }
            seg_start = p + 1;
        }
    }
    ...
    if (count == 0)
        return NULL;
    const char **result = (const char **)ani_arena_alloc(a, (size_t)(count + 1) * sizeof(*result));
...
/* Parse a type-text into a ANIType, with full inner-class qualification.
...
static const ANIType *parse_param_text_full(ANIArena *a, const char *text, const char *parent_class,
    ...
    if (no_arr) {
        ...
        if (lt) {
            ...
            if (gt && gt > lt + 1) {
                gen_args_inner = ani_arena_strndup(a, lt + 1, (size_t)(gt - lt - 1));
            }
        }
    }

    const ANIType *base = NULL;
    if (no_gen && is_java_primitive(no_gen)) {
        base = ani_type_builtin(a, no_gen);
    } else if (no_gen) {
        if (strchr(no_gen, '.')) {
            base = ani_type_named(a, no_gen);
        } else {
            /* Try java.lang well-knowns. */
            for (int i = 0; JAVA_LANG_TYPES[i]; i++) {
                if (strcmp(JAVA_LANG_TYPES[i], no_gen) == 0) {
                    base = ani_type_named(a, ani_arena_sprintf(a, "java.lang.%s", no_gen));
                    break;
                }
            }
            ...
            if (!base && parent_class && reg) {
                ...
                while (cur) {
                    const char *cand = ani_arena_sprintf(a, "%s.%s", cur, no_gen);
                    if (ani_registry_lookup_type(reg, cand)) {
                        base = ani_type_named(a, cand);
                        break;
                    }
                    const char *last_dot = strrchr(cur, '.');
                    if (!last_dot)
                        break;
                    cur = ani_arena_strndup(a, cur, (size_t)(last_dot - cur));
                }
            }
            ...
            if (!base && reg) {
                for (int i = 0; JAVA_FALLBACK_PACKAGES[i]; i++) {
                    const char *cand =
                        ani_arena_sprintf(a, "%s.%s", JAVA_FALLBACK_PACKAGES[i], no_gen);
                    if (ani_registry_lookup_type(reg, cand)) {
                        base = ani_type_named(a, cand);
                        break;
                    }
            ...
            if (!base) {
                if (module_qn && module_qn[0]) {
                    base = ani_type_named(a, ani_arena_sprintf(a, "%s.%s", module_qn, no_gen));
                } else {
                    base = ani_type_named(a, no_gen);
                }
            }
    ...
    if (!base)
        base = ani_type_unknown();
    /* If we extracted generic args and the base is NAMED, wrap as TEMPLATE
     * with each arg recursively parsed. */
    if (gen_args_inner && base && base->kind == ANI_TYPE_NAMED) {
        int gc = 0;
        const char **arg_strs = split_generic_args(a, gen_args_inner, &gc);
        if (arg_strs && gc > 0) {
            const ANIType **gargs =
                (const ANIType **)ani_arena_alloc(a, (size_t)(gc + 1) * sizeof(*gargs));
            for (int i = 0; i < gc; i++) {
                gargs[i] = parse_param_text_full(a, arg_strs[i], parent_class, module_qn, reg);
            }
            gargs[gc] = NULL;
            base = ani_type_template(a, base->data.named.qualified_name, gargs, gc);
        }
    }
    while (dim-- > 0)
        base = ani_type_slice(a, base);
    return base;
}

/* Backwards-compatible wrapper for callers that don't know the parent_class
 * context. Used by ani_run_java_lsp_cross. */
static const ANIType *parse_param_text(ANIArena *a, const char *text, const char *module_qn) {
...
static const ANIType *java_signature_parse_type(ANIArena *arena, const char *text,
                                                void *parser_ctx) {
...
static void register_local_func_or_type_from_file(JavaLSPContext *ctx, ANITypeRegistry *reg,
     ...
     * registry entry in place via the type lookup (ANIRegisteredType *). */
    for (int i = 0; i < result->defs.count; i++) {
        ANIDefinition *d = &result->defs.items[i];
        if (!d->qualified_name || !d->name || !d->label)
            continue;
        ...
        const ANIRegisteredType *existing = ani_registry_lookup_type(reg, d->qualified_name);
        if (!existing)
            continue;
        ANIRegisteredType rt = *existing; /* Will be re-added; lookup_type returns the
                                             registry's stored copy. We instead build a
                                             local copy and let ani_registry_add_type's
        ...
        if (d->base_classes) {
            ...
            if (bc_count > 0) {
                const char **emb =
                    (const char **)ani_arena_alloc(a, (size_t)(bc_count + 1) * sizeof(*emb));
                    ...
                    if (!strchr(no_gen, '.')) {
                        ...
                        for (int k = 0; JAVA_LANG_TYPES[k]; k++) {
                            if (strcmp(JAVA_LANG_TYPES[k], no_gen) == 0) {
                                resolved = ani_arena_sprintf(a, "java.lang.%s", no_gen);
                                break;
                            }
                        }
                        ...
                        if (!parent_qn && d->qualified_name) {
                            ...
                            if (last_dot_qn) {
                                parent_qn =
                                    ani_arena_strndup(a, d->qualified_name,
                        ...
                        if (!resolved && module_qn && module_qn[0]) {
                            resolved = ani_arena_sprintf(a, "%s.%s", module_qn, no_gen);
                        }
                        emb[j] = resolved ? resolved : ani_arena_strdup(a, no_gen);
                    } else {
                        emb[j] = ani_arena_strdup(a, no_gen);
                    }
                }
                emb[bc_count] = NULL;
        ...
        if (!slot->embedded_types && !slot->is_interface) {
            const char **emb = (const char **)ani_arena_alloc(a, 2 * sizeof(*emb));
            if (emb) {
     ...
     * since ANIDefinition doesn't carry a per-field type for Java. See
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        if (d->signature_param_types || d->signature_param_count > 0) {
            ...
            ptypes = ani_type_materialize_signature_params(a, d->signature_param_types,
                                                           d->signature_param_count,
                                                           java_signature_parse_type, &parse_ctx);
        } else if (!d->signature_param_types && d->signature_param_count == 0) {
            /* Compatibility for callers that still construct ANIDefinition by
            ...
            if (d->param_types) {
                ...
                if (pc > 0) {
                    ptypes =
                        (const ANIType **)ani_arena_alloc(a, (size_t)(pc + 1) * sizeof(*ptypes));
                    for (int j = 0; j < pc; j++) {
                        ptypes[j] =
        ...
        const ANIType **rtypes = NULL;
        if (d->return_type && d->return_type[0]) {
            rtypes = (const ANIType **)ani_arena_alloc(a, 2 * sizeof(*rtypes));
        ...
        rf.signature = ani_type_func(a, param_names, ptypes, rtypes);
        if (d->parent_class) {
...
static void patch_method_signatures_from_ast(JavaLSPContext *ctx, ANITypeRegistry *reg,
                                             TSNode class_node, const char *enclosing_class_qn);

static void patch_one_method(JavaLSPContext *ctx, ANITypeRegistry *reg, TSNode method_node,
    ...
    const ANIType **ptypes = NULL;
    if (pn > 0) {
        ptypes = (const ANIType **)ani_arena_alloc(ctx->arena, (size_t)(pn + 1) * sizeof(*ptypes));
        if (!ptypes)
            return;
    }
    int idx = 0;
    ...
    ANIRegisteredFunc *slot = NULL;
    ANIRegisteredFunc *first_qn_match = NULL;
    for (int fi = 0; fi < reg->func_count; fi++) {
        ANIRegisteredFunc *candidate = &reg->funcs[fi];
        if (!candidate->qualified_name || strcmp(candidate->qualified_name, method_qn) != 0)
            continue;
        if (!first_qn_match)
            first_qn_match = candidate;
        if (!candidate->signature || candidate->signature->kind != ANI_TYPE_FUNC)
            continue;
        int candidate_arity = 0;
        const ANIType *const *candidate_params = candidate->signature->data.func.param_types;
        if (candidate_params) {
    ...
    if (!is_constructor) {
        ...
        if (!ts_node_is_null(rtype)) {
            rtypes = (const ANIType **)ani_arena_alloc(ctx->arena, 2 * sizeof(*rtypes));
    ...
    if (is_constructor) {
        rtypes = (const ANIType **)ani_arena_alloc(ctx->arena, 2 * sizeof(*rtypes));
        rtypes[0] = ani_type_named(ctx->arena, class_qn);
        rtypes[1] = NULL;
    }

    /* Rebuild the FUNC type with both param-types and return-types. */
    slot->signature = ani_type_func(ctx->arena, NULL, ptypes, rtypes);
}
...
static void patch_method_signatures_from_ast(JavaLSPContext *ctx, ANITypeRegistry *reg,
    ...
    if (enclosing_class_qn) {
        class_qn = ani_arena_sprintf(ctx->arena, "%s.%s", enclosing_class_qn, cname);
    } else if (ctx->module_qn && ctx->module_qn[0]) {
        class_qn = ani_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
    } else {
        class_qn = ani_arena_strdup(ctx->arena, cname);
...
static void append_field_to_class(ANITypeRegistry *reg, ANIArena *a, const char *class_qn,
                                  const char *field_name, const ANIType *ftype) {
    ANIRegisteredType *slot = NULL;
    for (int ti = 0; ti < reg->type_count; ti++) {
    ...
    const char **new_names =
        (const char **)ani_arena_alloc(a, (size_t)(existing + 2) * sizeof(*new_names));
    const ANIType **new_types =
        (const ANIType **)ani_arena_alloc(a, (size_t)(existing + 2) * sizeof(*new_types));
    ...
    new_names[existing] = ani_arena_strdup(a, field_name);
    new_types[existing] = ftype ? ftype : ani_type_unknown();
...
static void populate_class_fields_from_ast(JavaLSPContext *ctx, ANITypeRegistry *reg,
                                           TSNode class_node, const char *enclosing_class_qn) {
    if (ts_node_is_null(class_node))
        return;
    const char *kind = ts_node_type(class_node);
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (strcmp(ck, "field_declaration") == 0) {
            ...
            const ANIType *ftype =
                ts_node_is_null(tn) ? ani_type_unknown() : java_parse_type_node(ctx, tn);
...
/* ── Top-level entry: ani_run_java_lsp ────────────────────────────── */
...
void ani_run_java_lsp(ANIArena *arena, ANIFileResult *result, const char *source, int source_len,
    ...
    ani_java_stdlib_register(&reg, arena);
    ...
    for (int i = 0; i < result->imports.count; i++) {
        ...
        if (!imp->local_name || !imp->module_path)
            continue;
        java_lsp_add_import(&ctx, imp->local_name, imp->module_path, ANI_JAVA_IMPORT_TYPE);
    }
...
void ani_java_register_lsp_defs(ANIArena *arena, ANITypeRegistry *reg, const ANILSPDef *defs,
    ...
    for (int i = 0; i < def_count; i++) {
        ...
        if (strcmp(d->label, "Class") == 0 || strcmp(d->label, "Interface") == 0 ||
            ...
            const ANIType **ptypes = ani_type_materialize_signature_params(
            ...
            if (d->return_types && d->return_types[0]) {
                ...
                const char *first;
                if (bar) {
                    first = ani_arena_strndup(arena, no_pipe, (size_t)(bar - no_pipe));
                } else {
                    first = no_pipe;
                }
            ...
            const ANIType **rtypes = NULL;
            if (ret) {
                rtypes = (const ANIType **)ani_arena_alloc(arena, 2 * sizeof(*rtypes));
                rtypes[0] = ret;
                rtypes[1] = NULL;
            }
            rf.signature = ani_type_func(arena, NULL, ptypes, rtypes);
            ani_registry_add_func(reg, rf);
        }
    }
}
 ...
 * internal/ani layer free of src/pipeline includes. */
extern void ani_pxc_count_perfile_defs(uint64_t defs);
...
ANITypeRegistry *ani_java_build_cross_registry(ANIArena *arena, ANILSPDef *defs, int def_count) {
     ...
     * bare type names via ani_registry_lookup_type. Before finalize that lookup
    ...
    ANILSPDef *jvm = NULL;
    if (def_count > 0) {
        jvm = (ANILSPDef *)ani_arena_alloc(arena, (size_t)def_count * sizeof(*jvm));
        if (!jvm) {
            return NULL;
        }
    }
    ...
    for (int i = 0; i < def_count; i++) {
        ...
        if ((defs[i].lang == ANI_LANG_JAVA || defs[i].lang == ANI_LANG_KOTLIN) && lb &&
    ...
    for (int i = 0; i < def_count; i++) {
        const char *lb = defs[i].label;
        if ((defs[i].lang == ANI_LANG_JAVA || defs[i].lang == ANI_LANG_KOTLIN) &&
            !(lb && (strcmp(lb, "Class") == 0 || strcmp(lb, "Interface") == 0 ||
                     strcmp(lb, "Enum") == 0 || strcmp(lb, "Type") == 0))) {
            jvm[total++] = defs[i];
        }
    }
    ani_java_register_lsp_defs(arena, reg, jvm, type_count);
    ani_registry_finalize(reg);
    ani_java_register_lsp_defs(arena, reg, jvm + type_count, total - type_count);
    ani_registry_finalize(reg);
...
void ani_run_java_lsp_cross_with_registry(ANIArena *arena, ANIFileResult *result,
    ...
    ani_pxc_count_perfile_defs((uint64_t)result->defs.count);
    ...
    ani_registry_finalize_into(&overlay, &idx_arena);
    ...
    for (int i = 0; i < import_count; i++) {
        ...
        java_lsp_add_import(&ctx, import_names[i], import_qns[i], ANI_JAVA_IMPORT_TYPE);
    }
...
void ani_run_java_lsp_cross(ANIArena *arena, const char *source, int source_len,
    ...
    ani_java_register_lsp_defs(arena, &reg, defs, def_count);
...
void ani_batch_java_lsp_cross(ANIArena *arena, ANIBatchJavaLSPFile *files, int file_count,
    ...
    for (int i = 0; i < file_count; i++) {
        ani_run_java_lsp_cross(arena, files[i].source, files[i].source_len, files[i].module_qn,
