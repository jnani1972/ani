...
static void build_qn_index(ANITypeRegistry *reg, ANIArena *idx_arena, bool for_funcs) {
    ...
    int *buckets = (int *)ani_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    ANIRegistryHashEntry *entries = (ANIRegistryHashEntry *)ani_arena_alloc(
        idx_arena, (size_t)count * sizeof(ANIRegistryHashEntry));
    if (!buckets || !entries)
        return;
...
static void build_method_index(ANITypeRegistry *reg, ANIArena *idx_arena) {
    if (reg->func_count == 0)
        return;
    ...
    int *buckets = (int *)ani_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    ANIRegistryHashEntry *entries = (ANIRegistryHashEntry *)ani_arena_alloc(
        reg->arena, (size_t)reg->func_count * sizeof(ANIRegistryHashEntry));
    if (!buckets || !entries)
        return;
    for (int i = 0; i < bucket_count; i++)
        buckets[i] = -1;
    ...
    for (int i = 0; i < reg->func_count; i++) {
        const ANIRegisteredFunc *f = &reg->funcs[i];
...
static void build_type_embed_index(ANITypeRegistry *reg, ANIArena *idx_arena) {
    int ecount = 0;
    ...
    int *buckets = (int *)ani_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    ANIRegistryHashEntry *entries = (ANIRegistryHashEntry *)ani_arena_alloc(
        idx_arena, (size_t)ecount * sizeof(ANIRegistryHashEntry));
    if (!buckets || !entries)
        return;
    for (int i = 0; i < bucket_count; i++)
        buckets[i] = -1;
...
void ani_registry_build_type_short_index(ANITypeRegistry *reg) {
    if (!reg)
        return;
...
static void build_type_short_index(ANITypeRegistry *reg, ANIArena *idx_arena) {
    int tcount = 0;
    for (int i = 0; i < reg->type_count; i++) {
    ...
    int *buckets = (int *)ani_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    ANIRegistryHashEntry *entries = (ANIRegistryHashEntry *)ani_arena_alloc(
        idx_arena, (size_t)tcount * sizeof(ANIRegistryHashEntry));
    if (!buckets || !entries)
        return;
    for (int i = 0; i < bucket_count; i++)
        buckets[i] = -1;
    ...
    for (int i = reg->type_count - 1; i >= 0; i--) {
        const ANIRegisteredType *t = &reg->types[i];
        if (!t->short_name)
            continue;
...
static void build_ffunc_short_index(ANITypeRegistry *reg, ANIArena *idx_arena) {
    int fcount = 0;
    for (int i = 0; i < reg->func_count; i++) {
        const ANIRegisteredFunc *f = &reg->funcs[i];
        if (!f->receiver_type && f->short_name)
    ...
    int *buckets = (int *)ani_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    ANIRegistryHashEntry *entries = (ANIRegistryHashEntry *)ani_arena_alloc(
        idx_arena, (size_t)fcount * sizeof(ANIRegistryHashEntry));
    if (!buckets || !entries)
        return;
    for (int i = 0; i < bucket_count; i++)
        buckets[i] = -1;
...
void ani_registry_types_by_short_name(const ANITypeRegistry *reg, const char *short_name,
                                      ANITypeShortIter *out) {
    out->reg = reg;
...
int ani_type_short_iter_next(ANITypeShortIter *it) {
    const ANITypeRegistry *reg = it->reg;
    while (it->chain_idx >= 0) {
        const ANIRegistryHashEntry *e = &reg->type_short_entries[it->chain_idx];
...
void ani_registry_types_by_embedded_bare(const ANITypeRegistry *reg, const char *bare,
                                         ANITypeEmbedIter *out) {
    out->reg = reg;
    out->hash = fnv1a(bare);
    out->prev_type = -1;
    if (reg->type_qn_buckets && reg->type_qn_bucket_count > 0) {
...
int ani_type_embed_iter_next(ANITypeEmbedIter *it) {
    const ANITypeRegistry *reg = it->reg;
    while (it->chain_idx >= 0) {
        const ANIRegistryHashEntry *e = &reg->type_embed_entries[it->chain_idx];
        int p = e->payload_index;
        uint64_t h = e->hash;
        it->chain_idx = e->next_index;
        if (h != it->hash)
...
void ani_registry_free_funcs_by_short_name(const ANITypeRegistry *reg, const char *short_name,
                                           ANIFreeFuncIter *out) {
    out->reg = reg;
    out->hash = fnv1a(short_name);
...
int ani_free_func_iter_next(ANIFreeFuncIter *it) {
    const ANITypeRegistry *reg = it->reg;
    while (it->chain_idx >= 0) {
        const ANIRegistryHashEntry *e = &reg->ffunc_short_entries[it->chain_idx];
        int p = e->payload_index;
        uint64_t h = e->hash;
        it->chain_idx = e->next_index;
        if (h != it->hash)
...
void ani_registry_methods(const ANITypeRegistry *reg, const char *receiver_qn,
                          const char *method_name, ANIMethodIter *out) {
...
int ani_method_iter_next(ANIMethodIter *it) {
    if (!it || !it->reg || !it->receiver_qn || !it->method_name) {
        return -1;
    }
    const ANITypeRegistry *reg = it->reg;
    while (it->chain_idx >= 0) {
        const ANIRegistryHashEntry *e = &reg->method_entries[it->chain_idx];
        int p = e->payload_index;
        uint64_t h = e->hash;
        it->chain_idx = e->next_index;
        if (h != it->hash) {
        ...
        const ANIRegisteredFunc *f = &reg->funcs[p];
...
void ani_registry_finalize_into(ANITypeRegistry *reg, ANIArena *idx_arena) {
    if (!reg || !idx_arena)
        return;
...
void ani_registry_finalize(ANITypeRegistry *reg) {
    if (!reg)
        return;
    ani_registry_finalize_into(reg, reg->arena);
}

void ani_registry_init(ANITypeRegistry *reg, ANIArena *arena) {
    memset(reg, 0, sizeof(ANITypeRegistry));
    reg->arena = arena;
}

void ani_registry_add_func(ANITypeRegistry *reg, ANIRegisteredFunc func) {
    if (reg->read_only) {
    ...
    if (reg->func_count >= reg->func_cap) {
        ...
        ANIRegisteredFunc *new_items = (ANIRegisteredFunc *)ani_arena_alloc(
            reg->arena, (size_t)new_cap * sizeof(ANIRegisteredFunc));
        if (!new_items)
            return;
        if (reg->funcs && reg->func_count > 0) {
            memcpy(new_items, reg->funcs, (size_t)reg->func_count * sizeof(ANIRegisteredFunc));
        }
        reg->funcs = new_items;
...
void ani_registry_add_type(ANITypeRegistry *reg, ANIRegisteredType type) {
    if (reg->read_only) {
        return; /* sealed Tier-2 shared registry: refuse post-finalize mutation (O(n^2)+race guard)
                 */
    }
    if (reg->type_count >= reg->type_cap) {
        ...
        ANIRegisteredType *new_items = (ANIRegisteredType *)ani_arena_alloc(
            reg->arena, (size_t)new_cap * sizeof(ANIRegisteredType));
        if (!new_items)
            return;
        if (reg->types && reg->type_count > 0) {
            memcpy(new_items, reg->types, (size_t)reg->type_count * sizeof(ANIRegisteredType));
        }
        reg->types = new_items;
...
static const ANIRegisteredFunc *lookup_method_self(const ANITypeRegistry *reg,
    ...
    if (reg->method_buckets && reg->method_bucket_count > 0) {
        ...
        for (int idx = reg->method_buckets[slot]; idx >= 0;
            ...
            if (reg->method_entries[idx].hash != h)
                continue;
            const ANIRegisteredFunc *f = &reg->funcs[reg->method_entries[idx].payload_index];
...
const ANIRegisteredFunc *ani_registry_lookup_method(const ANITypeRegistry *reg,
                                                    const char *receiver_qn,
                                                    const char *method_name) {
    if (!reg || !receiver_qn || !method_name)
        return NULL;
    const ANIRegisteredFunc *r = lookup_method_self(reg, receiver_qn, method_name);
    if (!r && reg->fallback) {
        return ani_registry_lookup_method(reg->fallback, receiver_qn, method_name);
    }
    return r;
}

static const ANIRegisteredType *lookup_type_self(const ANITypeRegistry *reg,
...
const ANIRegisteredType *ani_registry_lookup_type(const ANITypeRegistry *reg,
                                                  const char *qualified_name) {
    if (!reg || !qualified_name)
        return NULL;
    const ANIRegisteredType *r = lookup_type_self(reg, qualified_name);
    if (!r && reg->fallback) {
        return ani_registry_lookup_type(reg->fallback, qualified_name);
    }
    return r;
}

static const ANIRegisteredFunc *lookup_func_self(const ANITypeRegistry *reg,
                                                 const char *qualified_name) {
    if (reg->func_qn_buckets && reg->func_qn_bucket_count > 0) {
        uint64_t h = fnv1a(qualified_name);
...
const ANIRegisteredFunc *ani_registry_lookup_func(const ANITypeRegistry *reg,
                                                  const char *qualified_name) {
    if (!reg || !qualified_name)
        return NULL;
    const ANIRegisteredFunc *r = lookup_func_self(reg, qualified_name);
    if (!r && reg->fallback) {
        return ani_registry_lookup_func(reg->fallback, qualified_name);
    }
    return r;
}

const ANIRegisteredType *ani_registry_resolve_alias(const ANITypeRegistry *reg,
                                                    const char *type_qn) {
    if (!reg || !type_qn)
        return NULL;
    const ANIRegisteredType *rt = ani_registry_lookup_type(reg, type_qn);
    for (int i = 0; i < 16 && rt && rt->alias_of; i++) {
        const ANIRegisteredType *next = ani_registry_lookup_type(reg, rt->alias_of);
        if (!next)
            return rt;
        rt = next;
    }
...
const ANIRegisteredFunc *ani_registry_lookup_method_aliased(const ANITypeRegistry *reg,
                                                            const char *receiver_qn,
                                                            const char *method_name) {
    if (!reg || !receiver_qn || !method_name)
        return NULL;
    ...
    // Direct lookup first
    const ANIRegisteredFunc *f = ani_registry_lookup_method(reg, receiver_qn, method_name);
    if (f)
        return f;

    // Follow alias chain
    const ANIRegisteredType *rt = ani_registry_lookup_type(reg, receiver_qn);
    for (int i = 0; i < 16 && rt && rt->alias_of; i++) {
        f = ani_registry_lookup_method(reg, rt->alias_of, method_name);
        if (f)
            return f;
        rt = ani_registry_lookup_type(reg, rt->alias_of);
    }
    return NULL;
}

const ANIRegisteredFunc *ani_registry_lookup_symbol(const ANITypeRegistry *reg,
    ...
    return ani_registry_lookup_func(reg, buf);
}

// Count parameters in a FUNC signature.
static int count_func_params(const ANIRegisteredFunc *f) {
...
const ANIRegisteredFunc *ani_registry_lookup_method_by_args(const ANITypeRegistry *reg,
                                                            const char *receiver_qn,
                                                            const char *method_name,
                                                            int arg_count) {
    if (!reg || !receiver_qn || !method_name)
    ...
    const ANIRegisteredFunc *first_match = NULL;
    const ANIRegisteredFunc *range_match = NULL;
    ...
    if (reg->method_buckets && reg->method_bucket_count > 0) {
        ...
        for (int idx = reg->method_buckets[slot]; idx >= 0;
            ...
            const ANIRegisteredFunc *f = &reg->funcs[pi];
            if (!f->receiver_type || !f->short_name || strcmp(f->receiver_type, receiver_qn) != 0 ||
                strcmp(f->short_name, method_name) != 0)
                continue;
        ...
        if (reg->fallback) {
            return ani_registry_lookup_method_by_args(reg->fallback, receiver_qn, method_name,
                                                      arg_count);
        }
        return NULL;
    }
    ...
    const ANIRegisteredFunc *res = range_match ? range_match : first_match;
    if (!res && reg->fallback) {
        return ani_registry_lookup_method_by_args(reg->fallback, receiver_qn, method_name,
                                                  arg_count);
    }
...
static const char *type_to_qn_simple(const ANIType *t) {
    if (!t)
        return NULL;
...
static int score_overload_match(const ANIRegisteredFunc *f, const ANIType **arg_types,
                                int arg_count) {
    int pc = count_func_params(f);
    int min_pc = (f->min_params >= 0) ? f->min_params : pc;
    if (arg_count < min_pc || arg_count > pc)
    ...
    for (int i = 0; i < arg_count; i++) {
        const ANIType *expected = f->signature->data.func.param_types[i];
        const ANIType *actual = arg_types[i];
        if (!expected || !actual || ani_type_is_unknown(actual))
            continue; // neutral
...
const ANIRegisteredFunc *ani_registry_lookup_method_by_types(const ANITypeRegistry *reg,
                                                             const char *receiver_qn,
                                                             const char *method_name,
    ...
    if (!arg_types)
        return ani_registry_lookup_method_by_args(reg, receiver_qn, method_name, arg_count);
    ...
    if (reg->method_buckets && reg->method_bucket_count > 0) {
        ...
        if (sel >= 0)
            return &reg->funcs[sel];
        if (reg->fallback) {
            return ani_registry_lookup_method_by_types(reg->fallback, receiver_qn, method_name,
                                                       arg_types, arg_count);
        }
        return NULL;
    }
    ...
    const ANIRegisteredFunc *res = best ? best : first_match;
    if (!res && reg->fallback) {
        return ani_registry_lookup_method_by_types(reg->fallback, receiver_qn, method_name,
                                                   arg_types, arg_count);
    }
...
const ANIRegisteredFunc *ani_registry_lookup_symbol_by_types(const ANITypeRegistry *reg,
    ...
    if (!reg || !package_qn || !name)
        return NULL;
    if (!arg_types)
        return ani_registry_lookup_symbol_by_args(reg, package_qn, name, arg_count);
    ...
    if (reg->func_qn_buckets && reg->func_qn_bucket_count > 0) {
        ...
        if (sel >= 0)
            return &reg->funcs[sel];
        if (reg->fallback) {
            return ani_registry_lookup_symbol_by_types(reg->fallback, package_qn, name, arg_types,
                                                       arg_count);
        }
        return NULL;
    }
...
const ANIRegisteredFunc *ani_registry_lookup_symbol_by_args(const ANITypeRegistry *reg,
                                                            const char *package_qn,
    ...
    // ani_registry_lookup_symbol_by_types). O(overloads) not O(func_count).
    if (reg->func_qn_buckets && reg->func_qn_bucket_count > 0) {
        int exact_pi = -1, range_pi = -1, first_pi = -1;
        uint64_t h = fnv1a(buf);
        int slot = (int)(h & (uint64_t)(reg->func_qn_bucket_count - 1));
        ...
        if (sel >= 0)
            return &reg->funcs[sel];
        if (reg->fallback) {
            return ani_registry_lookup_symbol_by_args(reg->fallback, package_qn, name, arg_count);
        }
        return NULL;
    }
...
const ANIRegisteredFunc *ani_registry_lookup_callable(const ANITypeRegistry *reg, ANIArena *arena,
                                                      const char *type_qn) {
    ...
    // Synthesise a ANIRegisteredFunc whose QN is "<type_qn>.__call".
    ...
    char *qn = (char *)ani_arena_alloc(arena, total);
    if (!qn)
        return NULL;
    memcpy(qn, type_qn, qn_len);
    memcpy(qn + qn_len, suffix, sizeof(suffix));

    ANIRegisteredFunc *f = (ANIRegisteredFunc *)ani_arena_alloc(arena, sizeof(ANIRegisteredFunc));
    if (!f)
        return NULL;
    memset(f, 0, sizeof(*f));
    f->qualified_name = qn;
...
const ANIType *ani_registry_lookup_index_signature(const ANITypeRegistry *reg, const char *type_qn,
                                                   const ANIType *key_type) {
    if (!reg || !type_qn)
        return NULL;
    ...
    if (rt->index_key_type->kind == ANI_TYPE_BUILTIN && key_type->kind == ANI_TYPE_BUILTIN) {
