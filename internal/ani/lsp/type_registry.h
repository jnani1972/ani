#ifndef ANI_LSP_TYPE_REGISTRY_H
#define ANI_LSP_TYPE_REGISTRY_H
...
typedef enum {
    ANI_FUNC_FLAG_NONE = 0,
    ANI_FUNC_FLAG_PROPERTY = 1 << 0,        // @property -> obj.attr returns getter return
    ANI_FUNC_FLAG_CLASSMETHOD = 1 << 1,     // @classmethod -> first arg is cls (the class)
    ANI_FUNC_FLAG_STATICMETHOD = 1 << 2,    // @staticmethod -> no implicit self/cls
    ANI_FUNC_FLAG_ABSTRACTMETHOD = 1 << 3,  // @abstractmethod -> still callable for resolution
    ANI_FUNC_FLAG_OVERLOAD = 1 << 4,        // @overload entry — non-implementation stub
    ANI_FUNC_FLAG_ASYNC = 1 << 5,           // async def — return is Coroutine[..., T]
    ANI_FUNC_FLAG_GENERATOR = 1 << 6,       // contains yield — return is Generator[T, ...]
    ANI_FUNC_FLAG_FINAL = 1 << 7,           // @final — overrides not allowed
    ANI_FUNC_FLAG_RUST_TRAIT_IMPL = 1 << 8, // exact method from impl Trait for Type
    ANI_FUNC_FLAG_RUST_ABSTRACT = 1 << 9,   // required trait method without a default body
    ...
    ANI_FUNC_FLAG_AMBIGUOUS_BINDING = 1 << 10,
} ANIFuncFlags;
...
typedef struct {
    ...
    const ANIType *signature;      // FUNC type with param/return types
    ...
    int flags;                     // ANI_FUNC_FLAG_* bitfield
...
typedef struct {
    ...
    const ANIType **field_types;   // NULL-terminated (parallel to field_names)
    ...
    const ANIType *index_key_type;   // BUILTIN("string"|"number") or NULL
    const ANIType *index_value_type; // V or NULL
    // Generic constraints, parallel to type_param_names. NULL or shorter array means "any".
    const ANIType **type_param_constraints; // NULL-terminated, parallel to type_param_names
} ANIRegisteredType;
...
} ANIRegistryHashEntry;

// Cross-file type/function registry.
typedef struct ANITypeRegistry {
    ANIRegisteredFunc *funcs;
    int func_count;
    int func_cap;

    ANIRegisteredType *types;
    int type_count;
    int type_cap;

    ANIArena *arena; // owns all string data
    ...
    const struct ANITypeRegistry *fallback;

    // Hash indexes (built lazily by ani_registry_finalize, NULL until then).
    ...
    ANIRegistryHashEntry *type_short_entries;
    ...
    ANIRegistryHashEntry *type_embed_entries;
    ...
    ANIRegistryHashEntry *ffunc_short_entries;
    int ffunc_short_bucket_count;
    int ffunc_short_entry_count;

    /* Sealed / read-only. Set true by the ani_X_build_cross_registry builders
     ...
     * ani_registry_add_func/_type no-op on a sealed registry, so a per-file
...
// Like ani_registry_finalize, but the hash-index allocations (buckets/entries)
...
//   ANITypeEmbedIter it; ani_registry_types_by_embedded_bare(reg, bare, &it);
//   int ti; while ((ti = ani_type_embed_iter_next(&it)) >= 0) { ... reg->types[ti] ... }
typedef struct {
    const ANITypeRegistry *reg;
    uint64_t hash;
...
// a synthesised ANIRegisteredFunc whose qualified_name is "<type_qn>.__call" and
