#ifndef ANI_LSP_PERL_LSP_H
#define ANI_LSP_PERL_LSP_H
...
/* Evaluate a Perl expression's type. May return NULL / ANI_TYPE_UNKNOWN. */
 ...
 * Called from ani_extract_file() via the language dispatch in ani.c. */
 ...
 *
 * Mirrors ani_run_php_lsp_cross / ani_run_py_lsp_cross. Stub-declared here so
 * a later plan (Phase 23, cross-file) can implement it without touching the
 * wiring. Caller supplies the combined ANILSPDef[] (file-local + cross-file)
 * and a resolved import map (use → target QN). */
void ani_run_perl_lsp_cross(ANIArena *arena, const char *source, int source_len,
                            const char *module_qn, ANILSPDef *defs, int def_count,
                            const char **import_names, const char **import_qns, int import_count,
                            TSTree *cached_tree, /* NULL = parse internally */
                            ANIResolvedCallArray *out);

#endif /* ANI_LSP_PERL_LSP_H */
