#ifndef ANI_LSP_PY_LSP_H
#define ANI_LSP_PY_LSP_H
...
typedef struct {
    ...
    const ANIType *result; // full-fidelity result, never NULL in a live entry
} ANIPyTypeCacheEntry;
...
typedef struct {
    ...
    // Built from ANIFileResult.imports.
    ...
    // non-NULL, py_emit_dunder_call injects a matching synthetic ANICall so the
    ...
    ANIPyTypeCacheEntry *type_cache; // open addressing, linear probe, arena-allocated
    ...
    struct {
        ...
        const ANIType *field_type;
    } *field_overlay;
    int field_overlay_count;
    int field_overlay_cap;

    // Debug mode (ANI_LSP_DEBUG env, shared across all language LSPs).
    bool debug;
} PyLSPContext;
...
void py_lsp_init(PyLSPContext *ctx, ANIArena *arena, const char *source, int source_len,
                 const ANITypeRegistry *registry, const char *module_qn, ANIResolvedCallArray *out);

// Add an import mapping (call once per ANIImport entry).
...
// Mirrors ani_run_go_lsp / ani_run_c_lsp.
