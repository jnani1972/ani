...
static const char* join_path(ANIArena* arena, yyjson_val* path_arr,
    const char* crate_qn) {
    ...
    return ani_arena_strndup(arena, buf, (size_t)off);
}

/* Convert a rustdoc Type JSON value into a ANIType. Best-effort —
 ...
 */
static const ANIType* parse_rustdoc_type(yyjson_val* t, ANIArena* arena,
    const char* crate_qn) {
    if (!t || !yyjson_is_obj(t)) return ani_type_unknown();

    yyjson_val* prim = yyjson_obj_get(t, "primitive");
    if (prim && yyjson_is_str(prim)) {
        return ani_type_builtin(arena, yyjson_get_str(prim));
    }
    ...
    if (resolved && yyjson_is_obj(resolved)) {
        ...
        if (name && yyjson_is_str(name)) {
            qn = ani_arena_strdup(arena, yyjson_get_str(name));
        }
        if (!qn) return ani_type_unknown();
        ...
        if (args && yyjson_is_obj(args)) {
            ...
            if (angle && yyjson_is_obj(angle)) {
                ...
                if (arr && yyjson_is_arr(arr) && yyjson_arr_size(arr) > 0) {
                    ...
                    if (ti > 0) {
                        return ani_type_template(arena, qn, targs, ti);
                    }
                }
            }
        }
    ...
    if (borrowed && yyjson_is_obj(borrowed)) {
        ...
        return ani_type_reference(arena,
    ...
    if (tuple && yyjson_is_arr(tuple)) {
        ...
        if (n == 0) return ani_type_builtin(arena, "()");
        const ANIType* elems[8];
        int ei = 0;
        yyjson_val* item;
        yyjson_arr_iter it;
        ...
        return ani_type_tuple(arena, elems, ei);
    }
...
static bool register_function(ANITypeRegistry* reg, ANIArena* arena,
     ...
     * (implicit unit). */
    const ANIType* ret = ani_type_builtin(arena, "()");
    ...
    const ANIType** ra = (const ANIType**)ani_arena_alloc(arena,
...
int ani_rust_rustdoc_ingest(ANITypeRegistry* reg, ANIArena* arena,
    ...
    while ((key = yyjson_obj_iter_next(&it))) {
        ...
        if (kind && (strcmp(kind, "struct") == 0 ||
            ...
            if (register_function(reg, arena, ani_arena_strdup(arena, short_name),
                                  qn, NULL, fn, crate_qn)) {
                count++;
            }
        }
