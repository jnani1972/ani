 ...
 * a ANIScope stack plus a CSLSPContext that tracks type/namespace/using
 * state that does not naturally live in the value scope.
 *
 * Resolution order for a bare identifier `X` inside member `M` in type
 * `T` in namespace `N`:
 *
 *    1. local variables / parameters in the current block          (ANIScope)
...
static const ANIType *cs_eval_invocation_type(CSLSPContext *ctx, TSNode call);
static const ANIType *cs_eval_member_access_type(CSLSPContext *ctx, TSNode node);
static const ANIType *cs_eval_object_creation_type(CSLSPContext *ctx, TSNode node);
static const ANIType *cs_eval_identifier_type(CSLSPContext *ctx, TSNode node);
static const ANIType *cs_substitute_type_params(ANIArena *arena, const ANIType *t,
                                                const char **param_names,
                                                const ANIType **param_args);
...
static void cs_register_type_decls(CSLSPContext *ctx, ANITypeRegistry *reg, TSNode root);
static char *cs_node_text_cached(CSLSPContext *ctx, TSNode node);
static const ANIType *cs_unwrap_task(CSLSPContext *ctx, const ANIType *t);
static const ANIType *cs_unwrap_nullable(const ANIType *t);
...
static char *cs_normalize_name(ANIArena *a, const char *name) {
    if (!name)
        return NULL;
    ...
    char *out = (char *)ani_arena_alloc(a, n + 1);
    if (!out)
        return NULL;
    size_t w = 0;
...
static char *cs_strip_generic_args(ANIArena *a, const char *name) {
    if (!name)
        return NULL;
    const char *lt = strchr(name, '<');
    if (!lt)
        return ani_arena_strdup(a, name);
    size_t len = (size_t)(lt - name);
    return ani_arena_strndup(a, name, len);
}
...
static const char *cs_predefined_alias(const char *name) {
    ...
    if (strcmp(name, "nint") == 0)
        return "System.IntPtr";
    if (strcmp(name, "nuint") == 0)
        return "System.UIntPtr";
    if (strcmp(name, "void") == 0)
        return "System.Void";
    if (strcmp(name, "dynamic") == 0)
        return "System.Object"; /* dynamic ≈ object */
    return NULL;
}
...
void cs_lsp_init(CSLSPContext *ctx, ANIArena *arena, const char *source, int source_len,
    ...
    cs_lsp_add_using(ctx, ANI_CS_USING_NAMESPACE, "", "System", false);
...
void cs_lsp_add_using(CSLSPContext *ctx, ANICSUsingKind kind, const char *local_name,
    ...
    for (int i = 0; i < ctx->using_count; i++) {
        ANICSUsing *u = &ctx->usings[i];
        if (u->kind != kind)
            continue;
        if (strcmp(u->target_qn, target_qn) != 0)
            continue;
    ...
    if (ctx->using_count >= ctx->using_cap) {
        ...
        ANICSUsing *nu = (ANICSUsing *)ani_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*nu));
        if (!nu)
            return;
        for (int i = 0; i < ctx->using_count; i++)
            nu[i] = ctx->usings[i];
    ...
    ANICSUsing *slot = &ctx->usings[ctx->using_count++];
    slot->kind = kind;
    slot->local_name = local_name ? ani_arena_strdup(ctx->arena, local_name) : "";
    slot->target_qn = ani_arena_strdup(ctx->arena, target_qn);
...
static void cs_namespace_push(CSLSPContext *ctx, const char *ns_name) {
    ...
    if (ctx->namespace_count >= ctx->namespace_cap) {
        ...
        const char **arr =
            (const char **)ani_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*arr));
        if (!arr)
            return;
        for (int i = 0; i < ctx->namespace_count; i++)
            arr[i] = ctx->namespace_stack[i];
        ctx->namespace_stack = arr;
        ctx->namespace_cap = new_cap;
    }
    ctx->namespace_stack[ctx->namespace_count++] = ani_arena_strdup(ctx->arena, ns_name);
}
...
static const char *cs_namespace_qn(CSLSPContext *ctx) {
    ...
    char *out = (char *)ani_arena_alloc(ctx->arena, total + 1);
    if (!out)
        return "";
    size_t w = 0;
    for (int i = 0; i < ctx->namespace_count; i++) {
...
static const ANIRegisteredType *cs_lookup_type_qn(CSLSPContext *ctx, const char *qn) {
    if (!ctx->registry || !qn)
        return NULL;
    return ani_registry_lookup_type(ctx->registry, qn);
}
...
const char *cs_resolve_type_name(CSLSPContext *ctx, const char *raw) {
    ...
    for (int i = 0; i < ctx->type_param_count; i++) {
        if (strcmp(ctx->type_param_names[i], bare) == 0) {
            const ANIType *arg = ctx->type_param_args[i];
            if (arg && arg->kind == ANI_TYPE_NAMED)
                return arg->data.named.qualified_name;
            /* Builtin or unknown — fall through. */
        }
    }

    /* 3. Exact registry hit. */
    if (cs_lookup_type_qn(ctx, bare))
        return bare;

    /* 4. Inside the current class? Try enclosing class's nested type. */
    if (ctx->enclosing_class_qn) {
        const char *try_qn = ani_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, bare);
        if (cs_lookup_type_qn(ctx, try_qn))
            return try_qn;
    }
    ...
    if (ctx->namespace_count > 0) {
        ...
        for (;;) {
            if (!ns || !*ns)
                break;
            const char *try_qn = ani_arena_sprintf(ctx->arena, "%s.%s", ns, bare);
            if (cs_lookup_type_qn(ctx, try_qn))
                return try_qn;
            const char *dot = strrchr(ns, '.');
            if (!dot)
                break;
            char *trim = ani_arena_strndup(ctx->arena, ns, (size_t)(dot - ns));
            ns = trim;
        }
    }
    ...
    if (ctx->module_qn && ctx->module_qn[0]) {
        const char *try_qn = ani_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, bare);
        if (cs_lookup_type_qn(ctx, try_qn))
            return try_qn;
    }
    ...
    for (int i = 0; i < ctx->using_count; i++) {
        const ANICSUsing *u = &ctx->usings[i];
        if (u->kind != ANI_CS_USING_NAMESPACE)
            continue;
        const char *try_qn = ani_arena_sprintf(ctx->arena, "%s.%s", u->target_qn, bare);
        if (cs_lookup_type_qn(ctx, try_qn))
            return try_qn;
    }
        ...
        if (strncmp(bare, u->local_name, alias_len) == 0) {
            if (bare[alias_len] == '\0') {
                ...
                if (target_bare && cs_lookup_type_qn(ctx, target_bare))
                    return target_bare;
                return ani_arena_strdup(ctx->arena, u->target_qn);
            }
            if (bare[alias_len] == '.') {
                /* `Alias.Sub` -> `Target.Sub` */
                char *target_bare = cs_strip_generic_args(ctx->arena, u->target_qn);
                const char *base = target_bare ? target_bare : u->target_qn;
                const char *try_qn = ani_arena_sprintf(ctx->arena, "%s%s", base, bare + alias_len);
                if (cs_lookup_type_qn(ctx, try_qn))
                    return try_qn;
            }
        }
     ...
     * `namespace` declaration doesn't line up with the directory tree. */
    {
        ...
        if (the_short && *the_short) {
            const ANIRegisteredType *best = NULL;
            int best_score = -1;
            int best_idx = -1;
            ...
            ANITypeShortIter ts_it;
            int i;
            if (ctx->registry) {
                ani_registry_types_by_short_name(ctx->registry, the_short, &ts_it);
            }
            while (ctx->registry && (i = ani_type_short_iter_next(&ts_it)) >= 0) {
                const ANIRegisteredType *cand = &ctx->registry->types[i];
...
const ANIRegisteredFunc *cs_lookup_method(CSLSPContext *ctx, const char *type_qn,
    ...
    if (!type_qn || !method_name)
        return NULL;

    const ANIRegisteredFunc *f = ani_registry_lookup_method(ctx->registry, type_qn, method_name);
    if (f)
        return f;

    /* Walk inheritance chain (base + interfaces + transitive bases). */
    const ANIRegisteredType *t = cs_lookup_type_qn(ctx, type_qn);
    if (!t)
        return NULL;
    ...
    while (frontier_count > 0 && visited_count < CS_LSP_PARENT_WALK_MAX) {
        ...
        f = ani_registry_lookup_method(ctx->registry, parent, method_name);
        if (f)
            return f;

        const ANIRegisteredType *next = cs_lookup_type_qn(ctx, parent);
        if (!next)
            continue;
    ...
    if (strcmp(type_qn, "System.Object") != 0) {
        f = ani_registry_lookup_method(ctx->registry, "System.Object", method_name);
        if (f)
            return f;
    }
    return NULL;
 ...
 * dependent. */
static const ANIRegisteredFunc *cs_lookup_using_static_method(CSLSPContext *ctx,
                                                              const char *method_name,
                                                              bool *ambiguous) {
    if (ambiguous)
        *ambiguous = false;
    ...
    const ANIRegisteredFunc *only = NULL;
    for (int i = 0; i < ctx->using_count; i++) {
        const ANICSUsing *u = &ctx->usings[i];
        if (u->kind != ANI_CS_USING_STATIC)
            continue;
...
static const ANIRegisteredFunc *cs_lookup_extension(CSLSPContext *ctx, const char *receiver_qn,
                                                    const char *method_name) {
    if (!ctx->registry || !receiver_qn || !method_name)
        return NULL;
     ...
     * per-file resolve cost proportional to the whole corpus (#1669 follow-up).
     * The iterator degrades to the same full scan on an unfinalized registry,
     * and the min-index selection below preserves the scan's first-match-in-
     * registration-order tie-break exactly (bucket chains are not in funcs[]
     * order). */
    ANIFreeFuncIter ext_it;
    ani_registry_free_funcs_by_short_name(ctx->registry, method_name, &ext_it);
    const ANIRegisteredFunc *best = NULL;
    int best_idx = -1;
    int i;
    while ((i = ani_free_func_iter_next(&ext_it)) >= 0) {
        const ANIRegisteredFunc *cand = &ctx->registry->funcs[i];
        if (!cand->short_name || strcmp(cand->short_name, method_name) != 0)
            continue;
        if (cand->receiver_type)
            continue; /* must be a free static method */
        if (!cand->signature || cand->signature->kind != ANI_TYPE_FUNC)
            continue;
        const ANIType *sig = cand->signature;
        if (!sig->data.func.param_types)
            continue;
        const ANIType *first = sig->data.func.param_types[0];
        if (!first)
            continue;
        /* Only consider candidates marked as extensions (param[0] is named
         * "this" in our convention). param_names[0] == "this self" or "this". */
        if (!sig->data.func.param_names || !sig->data.func.param_names[0])
            continue;
        const char *p0 = sig->data.func.param_names[0];
        if (strncmp(p0, "this", 4) != 0)
            continue;
        ...
        bool match = false;
        if (first->kind == ANI_TYPE_NAMED && first->data.named.qualified_name) {
            if (strcmp(first->data.named.qualified_name, receiver_qn) == 0)
                match = true;
            if (!match && strcmp(first->data.named.qualified_name, "System.Object") == 0)
                match = true;
            /* Walk receiver's bases. */
            const ANIRegisteredType *rt = cs_lookup_type_qn(ctx, receiver_qn);
            if (rt && rt->embedded_types && !match) {
                for (int j = 0; rt->embedded_types[j]; j++) {
                    if (strcmp(rt->embedded_types[j], first->data.named.qualified_name) == 0) {
                        match = true;
                        break;
                    }
                }
            }
        } else if (first->kind == ANI_TYPE_TEMPLATE) {
            ...
            if (!match && strcmp(receiver_qn, tn) == 0)
                match = true;
        }
        if (!match)
            continue;
...
const ANIType *cs_parse_type_node(CSLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "predefined_type") == 0) {
        ...
        if (aliased)
            return ani_type_named(ctx->arena, aliased);
        return ani_type_builtin(ctx->arena, t);
    }
    ...
    if (strcmp(kind, "array_type") == 0) {
        ...
        if (ts_node_is_null(inner))
            inner = cs_first_named_child(node);
        const ANIType *elem = cs_parse_type_node(ctx, inner);
        return ani_type_template(ctx->arena, "System.Array", (const ANIType *[]){elem, NULL}, 1);
    }
    ...
    if (strcmp(kind, "tuple_type") == 0) {
        ...
        const ANIType **elems =
            (const ANIType **)ani_arena_alloc(ctx->arena, (size_t)(nc + 1) * sizeof(*elems));
        if (!elems)
            return ani_type_unknown();
        int count = 0;
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (strcmp(ck, "tuple_element") != 0)
                continue;
            TSNode te = ts_node_child_by_field_name(c, "type", 4);
            if (ts_node_is_null(te))
                te = cs_first_named_child(c);
    ...
    if (strcmp(kind, "generic_name") == 0) {
        ...
        if (ts_node_is_null(name_node))
            name_node = cs_child_named_kind(node, "identifier");
        ...
        if (ts_node_is_null(args_node)) {
            return ani_type_named(ctx->arena, resolved);
        }
        /* Parse args. */
        uint32_t nc = ts_node_child_count(args_node);
        const ANIType **args =
            (const ANIType **)ani_arena_alloc(ctx->arena, (size_t)(nc + 1) * sizeof(*args));
        if (!args)
            return ani_type_named(ctx->arena, resolved);
        int count = 0;
        for (uint32_t i = 0; i < nc; i++) {
        ...
        args[count] = NULL;
        return ani_type_template(ctx->arena, resolved, args, count);
    }
    if (strcmp(kind, "qualified_name") == 0 || strcmp(kind, "identifier") == 0 ||
        ...
        if (pre)
            return ani_type_named(ctx->arena, pre);
        return ani_type_named(ctx->arena, resolved);
    }
    if (strcmp(kind, "ref_type") == 0) {
        ...
        if (ts_node_is_null(inner))
            inner = cs_first_named_child(node);
        const ANIType *t = cs_parse_type_node(ctx, inner);
        return ani_type_reference(ctx->arena, t);
    }
...
const ANIType *cs_eval_expr_type(CSLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "identifier") == 0) {
        ...
        if (ctx->enclosing_base_qn) {
            result = ani_type_named(ctx->arena, ctx->enclosing_base_qn);
        }
        ...
        result = ani_type_named(ctx->arena, "System.String");
    } else if (strcmp(kind, "character_literal") == 0) {
        result = ani_type_named(ctx->arena, "System.Char");
    } else if (strcmp(kind, "integer_literal") == 0) {
        result = ani_type_named(ctx->arena, "System.Int32");
    } else if (strcmp(kind, "real_literal") == 0) {
        result = ani_type_named(ctx->arena, "System.Double");
    } else if (strcmp(kind, "boolean_literal") == 0) {
        result = ani_type_named(ctx->arena, "System.Boolean");
        ...
        if (!ts_node_is_null(rhs))
            result = cs_parse_type_node(ctx, rhs);
        ...
        if (!ts_node_is_null(inner)) {
            const ANIType *t = cs_eval_expr_type(ctx, inner);
            result = cs_unwrap_task(ctx, t);
        }
    } else if (strcmp(kind, "binary_expression") == 0 ||
               strcmp(kind, "prefix_unary_expression") == 0 ||
        ...
        if (opt && (strcmp(opt, "==") == 0 || strcmp(opt, "!=") == 0 || strcmp(opt, "<") == 0 ||
            ...
            if (ts_node_is_null(left))
                left = cs_first_named_child(node);
            if (!ts_node_is_null(left))
                result = cs_eval_expr_type(ctx, left);
        }
    } else if (strcmp(kind, "assignment_expression") == 0) {
        TSNode rhs = ts_node_child_by_field_name(node, "right", 5);
        if (!ts_node_is_null(rhs))
            result = cs_eval_expr_type(ctx, rhs);
        ...
        if (ts_node_is_null(b))
            b = ts_node_child_by_field_name(node, "alternative", 11);
        if (ts_node_is_null(b)) {
            uint32_t nc = ts_node_child_count(node);
            for (uint32_t i = 0; i < nc; i++) {
                TSNode c = ts_node_child(node, i);
                if (!ts_node_is_null(c) && ts_node_is_named(c)) {
                    const ANIType *t = cs_eval_expr_type(ctx, c);
                    if (t && t->kind != ANI_TYPE_UNKNOWN) {
                        result = t;
                        break;
                    }
                }
        ...
        elems[count] = NULL;
        result = ani_type_tuple(ctx->arena, elems, count);
        ...
        result = ani_type_named(ctx->arena, "System.Type");
        ...
        if (ts_node_is_null(obj))
            obj = cs_first_named_child(node);
        if (!ts_node_is_null(obj)) {
            const ANIType *recv = cs_eval_expr_type(ctx, obj);
            if (recv && recv->kind == ANI_TYPE_TEMPLATE) {
                const ANIType *const *args = recv->data.template_type.template_args;
                if (args) {
                    int n = 0;
                    while (args[n])
                        n++;
        ...
        TSNode tnode = ts_node_child_by_field_name(node, "type", 4);
        if (!ts_node_is_null(tnode)) {
            const ANIType *elem = cs_parse_type_node(ctx, tnode);
            result =
                ani_type_template(ctx->arena, "System.Array", (const ANIType *[]){elem, NULL}, 1);
        }
    } else if (strcmp(kind, "implicit_array_creation_expression") == 0) {
        result = ani_type_template(ctx->arena, "System.Array",
                                   (const ANIType *[]){ani_type_unknown(), NULL}, 1);
    }
...
static const ANIType *cs_eval_identifier_type(CSLSPContext *ctx, TSNode node) {
    ...
    /* Local / parameter scope. ani_scope_lookup returns ani_type_unknown()
     * (a non-NULL singleton) when the name isn't bound, so we need to
     * check the kind, not just the pointer. */
    const ANIType *bound = ani_scope_lookup(ctx->current_scope, name);
    if (bound && bound->kind != ANI_TYPE_UNKNOWN)
        return bound;

    /* Implicit `this` member. Try to find a field on the enclosing class. */
    if (ctx->enclosing_class_qn) {
        const ANIRegisteredType *rt = cs_lookup_type_qn(ctx, ctx->enclosing_class_qn);
    ...
    if (resolved) {
        const ANIRegisteredType *rt = cs_lookup_type_qn(ctx, resolved);
        if (rt)
...
static const ANIType *cs_eval_invocation_type(CSLSPContext *ctx, TSNode call) {
    ...
    if (strcmp(fk, "member_access_expression") == 0 ||
        ...
        if (ts_node_is_null(name)) {
            ...
            for (uint32_t i = 0; i < fnc; i++) {
                ...
                if (ts_node_is_null(c) || !ts_node_is_named(c))
                    continue;
                const char *k = ts_node_type(c);
                if (strcmp(k, "identifier") == 0 || strcmp(k, "generic_name") == 0)
                    name = c;
            }
        }
        if (ts_node_is_null(name))
        ...
        const ANIType *recv_type = ani_type_unknown();
        if (!ts_node_is_null(recv))
            recv_type = cs_eval_expr_type(ctx, recv);
        recv_type = cs_unwrap_nullable(recv_type);

        const char *type_qn = NULL;
        if (recv_type && recv_type->kind == ANI_TYPE_NAMED) {
            type_qn = recv_type->data.named.qualified_name;
        } else if (recv_type && recv_type->kind == ANI_TYPE_TEMPLATE) {
            type_qn = recv_type->data.template_type.template_name;
        }
        if (!type_qn)
            return ani_type_unknown();

        const ANIRegisteredFunc *f = cs_lookup_method(ctx, type_qn, bare_name);
        if (!f)
            f = cs_lookup_extension(ctx, type_qn, bare_name);
        if (!f || !f->signature)
            return ani_type_unknown();
        if (f->signature->kind == ANI_TYPE_FUNC && f->signature->data.func.return_types &&
            f->signature->data.func.return_types[0]) {
            const ANIType *ret = f->signature->data.func.return_types[0];
            /* If receiver is templated and method's return type contains type
             * parameters, substitute. */
            if (recv_type && recv_type->kind == ANI_TYPE_TEMPLATE) {
                const ANIRegisteredType *rt = cs_lookup_type_qn(ctx, type_qn);
    ...
    if (strcmp(fk, "identifier") == 0 || strcmp(fk, "generic_name") == 0) {
        ...
        if (strcmp(bare, "nameof") == 0) {
            return ani_type_named(ctx->arena, "System.String");
        }
        if (strcmp(bare, "typeof") == 0) {
            return ani_type_named(ctx->arena, "System.Type");
        }

        /* Try enclosing class member. */
        if (ctx->enclosing_class_qn) {
            const ANIRegisteredFunc *f = cs_lookup_method(ctx, ctx->enclosing_class_qn, bare);
        ...
        const char *ns = cs_namespace_qn(ctx);
        if (ns && *ns) {
            const char *qn = ani_arena_sprintf(ctx->arena, "%s.%s", ns, bare);
        ...
        const ANIRegisteredFunc *f = cs_lookup_using_static_method(ctx, bare, &ambiguous);
        if (ambiguous)
            return ani_type_unknown();
        if (f && f->signature && f->signature->data.func.return_types &&
            f->signature->data.func.return_types[0]) {
...
static const ANIType *cs_eval_member_access_type(CSLSPContext *ctx, TSNode node) {
    ...
    if (ts_node_is_null(obj) || ts_node_is_null(name))
        return ani_type_unknown();
    ...
    if (obj_text) {
        ...
        if (type_qn && cs_lookup_type_qn(ctx, type_qn)) {
            ...
            const ANIRegisteredFunc *f = cs_lookup_method(ctx, type_qn, fname);
            if (f && f->signature && f->signature->kind == ANI_TYPE_FUNC &&
                f->signature->data.func.return_types && f->signature->data.func.return_types[0]) {
                return f->signature->data.func.return_types[0];
            }
    ...
    const char *type_qn = NULL;
    if (recv->kind == ANI_TYPE_NAMED) {
        type_qn = recv->data.named.qualified_name;
    } else if (recv->kind == ANI_TYPE_TEMPLATE) {
    ...
    if (rt && rt->field_names) {
        for (int i = 0; rt->field_names[i]; i++) {
            if (strcmp(rt->field_names[i], fname) == 0) {
                if (rt->field_types && rt->field_types[i]) {
                    const ANIType *ft = rt->field_types[i];
                    if (recv->kind == ANI_TYPE_TEMPLATE && rt->type_param_names &&
                        recv->data.template_type.template_args) {
                        return cs_substitute_type_params(ctx->arena, ft, rt->type_param_names,
                                                         recv->data.template_type.template_args);
                    }
                    return ft;
                }
    ...
    if (f && f->signature && f->signature->kind == ANI_TYPE_FUNC &&
        f->signature->data.func.return_types && f->signature->data.func.return_types[0]) {
        const ANIType *ret = f->signature->data.func.return_types[0];
        if (recv->kind == ANI_TYPE_TEMPLATE && rt && rt->type_param_names &&
            recv->data.template_type.template_args) {
            return cs_substitute_type_params(ctx->arena, ret, rt->type_param_names,
                                             recv->data.template_type.template_args);
        }
...
static const ANIType *cs_substitute_type_params(ANIArena *arena, const ANIType *t,
    ...
    if (t->kind == ANI_TYPE_TEMPLATE) {
        const ANIType *const *old_args = t->data.template_type.template_args;
        if (!old_args)
            return t;
        int n = 0;
        while (old_args[n])
        ...
        const ANIType **new_args =
            (const ANIType **)ani_arena_alloc(arena, (size_t)(n + 1) * sizeof(*new_args));
        if (!new_args)
            return t;
        for (int i = 0; i < n; i++) {
    ...
    if (t->kind == ANI_TYPE_TYPE_PARAM) {
        const char *p = t->data.type_param.name;
        if (!p)
            return t;
        for (int i = 0; param_names[i]; i++) {
            if (strcmp(param_names[i], p) == 0)
                return param_args[i] ? param_args[i] : t;
        }
    }
    return t;
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (ts_node_is_null(tnode) || ts_node_is_null(nnode)) {
            ...
            for (uint32_t j = 0; j < pc; j++) {
                ...
                if (strcmp(ck, "identifier") == 0 && ts_node_is_null(nnode))
                    nnode = cc;
                else if (ts_node_is_null(tnode) && strcmp(ck, "identifier") != 0)
                    tnode = cc;
            }
        }
        ...
        const ANIType *ptype = ani_type_unknown();
        if (!ts_node_is_null(tnode))
            ptype = cs_parse_type_node(ctx, tnode);
...
static const char *cs_resolve_callable_name(CSLSPContext *ctx, const char *name) {
    ...
    if (ani_scope_contains(ctx->current_scope, name)) {
        return ani_scope_lookup_callable(ctx->current_scope, name);
    }

    if (ctx->enclosing_class_qn) {
        const ANIRegisteredFunc *f = cs_lookup_method(ctx, ctx->enclosing_class_qn, name);
        if (f && f->qualified_name)
            return f->qualified_name;
    }
    if (ctx->enclosing_base_qn) {
        const ANIRegisteredFunc *f = cs_lookup_method(ctx, ctx->enclosing_base_qn, name);
        if (f && f->qualified_name)
            return f->qualified_name;
    }

    bool using_static_ambiguous = false;
    const ANIRegisteredFunc *using_static =
    ...
    const char *ns = cs_namespace_qn(ctx);
    if (ns && ns[0]) {
        const char *qn = ani_arena_sprintf(ctx->arena, "%s.%s", ns, name);
        const ANIRegisteredFunc *f = ani_registry_lookup_func(ctx->registry, qn);
        if (f && f->qualified_name)
            return f->qualified_name;
    }
    ...
    for (int i = 0; ctx->registry && i < ctx->registry->func_count; i++) {
        const ANIRegisteredFunc *candidate = &ctx->registry->funcs[i];
...
static const char *cs_resolve_callable_value(CSLSPContext *ctx, TSNode node,
    ...
    if (receiver_text) {
        ...
        if (type_qn && cs_lookup_type_qn(ctx, type_qn)) {
            const ANIRegisteredFunc *f = cs_lookup_method(ctx, type_qn, member_name);
            if (f && callable_leaf)
    ...
    const ANIType *receiver_type = cs_unwrap_nullable(cs_eval_expr_type(ctx, receiver));
    const char *type_qn = NULL;
    if (receiver_type && receiver_type->kind == ANI_TYPE_NAMED) {
        type_qn = receiver_type->data.named.qualified_name;
    } else if (receiver_type && receiver_type->kind == ANI_TYPE_TEMPLATE) {
...
static void cs_process_local_decl(CSLSPContext *ctx, TSNode node) {
    ...
    const ANIType *declared_type =
        ts_node_is_null(tnode) ? ani_type_unknown() : cs_parse_type_node(ctx, tnode);
    for (uint32_t i = 0; i < nc; i++) {
        TSNode c = ts_node_child(vd, i);
        if (ts_node_is_null(c) || !ts_node_is_named(c))
            continue;
        const char *k = ts_node_type(c);
        if (strcmp(k, "variable_declarator") != 0)
            continue;
        ...
        if (!rhs_t || rhs_t->kind == ANI_TYPE_UNKNOWN) {
            ...
            for (uint32_t j = 0; j < cc; j++) {
                ...
                if (ts_node_is_null(cn) || !ts_node_is_named(cn))
                    continue;
                ...
                if (strcmp(ck, "equals_value_clause") == 0) {
                    ...
                    if (!ts_node_is_null(rhs)) {
                        rhs_node = rhs;
                        const ANIType *t = cs_eval_expr_type(ctx, rhs);
                        if (t && t->kind != ANI_TYPE_UNKNOWN)
                            rhs_t = t;
                    }
                    break;
                ...
                if (seen_named >= 1) {
                    rhs_node = cn;
                    const ANIType *t = cs_eval_expr_type(ctx, cn);
                    if (t && t->kind != ANI_TYPE_UNKNOWN) {
                        rhs_t = t;
                        break;
                    }
        ...
        const ANIType *bind = is_var ? (rhs_t ? rhs_t : declared_type) : declared_type;
        if (!bind)
            bind = ani_type_unknown();
        const char *callable_qn =
            ts_node_is_null(rhs_node) ? NULL : cs_resolve_callable_value(ctx, rhs_node, NULL);
        if (callable_qn) {
            ani_scope_bind_callable(ctx->current_scope, vname, bind, callable_qn);
        } else {
            ani_scope_bind(ctx->current_scope, vname, bind);
        }
    }
}
...
static void cs_process_foreach(CSLSPContext *ctx, TSNode node) {
    ...
    if (ts_node_is_null(nnode))
        nnode = cs_child_named_kind(node, "identifier");
    ...
    if (ts_node_is_null(iter)) {
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (ts_node_is_null(c) || !ts_node_is_named(c))
                continue;
            const char *k = ts_node_type(c);
            if (strcmp(k, "identifier") == 0 && !past_id) {
                past_id = true;
                continue;
            }
            if (past_id) {
    ...
    const ANIType *element_t = ani_type_unknown();
    if (!ts_node_is_null(iter)) {
        const ANIType *iter_t = cs_eval_expr_type(ctx, iter);
        if (iter_t && iter_t->kind == ANI_TYPE_TEMPLATE) {
            const ANIType *const *args = iter_t->data.template_type.template_args;
            if (args && args[0])
                element_t = args[0];
        }
    }
    if (!ts_node_is_null(tnode)) {
        const ANIType *declared = cs_parse_type_node(ctx, tnode);
        char *tt = cs_node_text(ctx, tnode);
        bool is_var = (tt && (strcmp(tt, "var") == 0));
        if (!is_var && declared && declared->kind != ANI_TYPE_UNKNOWN)
            element_t = declared;
    }
    ani_scope_bind(ctx->current_scope, vname, element_t);
}
...
static void cs_process_assignment(CSLSPContext *ctx, TSNode node) {
    ...
    if (callable_qn) {
        ani_scope_update_callable(ctx->current_scope, vname, callable_qn);
    } else {
        ...
        ani_scope_update_callable(ctx->current_scope, vname, NULL);
    }
}
...
static void cs_invalidate_conditionally_assigned_callables(CSLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "assignment_expression") == 0) {
        ...
        if (cs_node_is(lhs, "identifier")) {
            ...
            if (name)
                ani_scope_update_callable(ctx->current_scope, name, NULL);
        }
        return;
    }
    uint32_t nc = ts_node_child_count(node);
...
static void cs_emit_resolved_kind_reason(CSLSPContext *ctx, const char *callee_qn,
                                         ...
                                         ANIResolvedKind kind, const char *reason) {
...
static void cs_emit_resolved_kind(CSLSPContext *ctx, const char *callee_qn, const char *strategy,
                                  float confidence, ANIResolvedKind kind) {
...
static void cs_emit_resolved(CSLSPContext *ctx, const char *callee_qn, const char *strategy,
                             float confidence) {
    cs_emit_resolved_kind(ctx, callee_qn, strategy, confidence, ANI_RESOLVED_INVOCATION);
}
...
static void cs_emit_resolved_reason(CSLSPContext *ctx, const char *callee_qn, const char *strategy,
    ...
    cs_emit_resolved_kind_reason(ctx, callee_qn, strategy, confidence, ANI_RESOLVED_INVOCATION,
                                 reason);
}
...
static void cs_emit_resolved_kind_reason_at(CSLSPContext *ctx, const char *callee_qn,
                                            const char *strategy, float confidence,
                                            ANIResolvedKind kind, const char *reason, TSNode site) {
...
static void cs_retarget_callable_carrier(CSLSPContext *ctx, TSNode site, const char *source_name,
    ...
    for (int i = 0; i < ctx->call_carriers->count; i++) {
        ANICall *call = &ctx->call_carriers->items[i];
        ...
        call->callee_name = ani_arena_strdup(ctx->arena, target_name);
...
static void cs_resolve_callable_arguments(CSLSPContext *ctx, TSNode call) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        cs_emit_resolved_kind_reason_at(ctx, target, "lsp_callable_value_reference", 0.95f,
                                        ANI_RESOLVED_CALL_REFERENCE, source_name, callable_leaf);
    }
}
...
static void cs_resolve_invocation(CSLSPContext *ctx, TSNode call) {
    ...
    if (strcmp(fk, "member_access_expression") == 0 ||
        ...
        if (recv_text) {
            ...
            if (type_qn && cs_lookup_type_qn(ctx, type_qn)) {
                const ANIRegisteredFunc *f = cs_lookup_method(ctx, type_qn, bare);
                if (f) {
                    cs_emit_resolved(ctx, f->qualified_name, "cs_static_typed", 0.95f);
                    return;
                }
                /* Type known, method not in registry — synth a call. */
                cs_emit_resolved(ctx, ani_arena_sprintf(ctx->arena, "%s.%s", type_qn, bare),
        ...
        const ANIRegisteredFunc *f = cs_lookup_method(ctx, type_qn, bare);
        if (f) {
            const char *strategy = (f->receiver_type && strcmp(f->receiver_type, type_qn) == 0)
                                       ? "cs_method_typed"
                                       : "cs_method_inherited";
    ...
    if (strcmp(fk, "identifier") == 0 || strcmp(fk, "generic_name") == 0) {
        ...
        if (ani_scope_contains(ctx->current_scope, bare)) {
            const char *target = ani_scope_lookup_callable(ctx->current_scope, bare);
            if (target) {
        ...
        if (ctx->enclosing_base_qn) {
            const ANIRegisteredFunc *f = cs_lookup_method(ctx, ctx->enclosing_base_qn, bare);
            if (f) {
...
static void cs_resolve_object_creation(CSLSPContext *ctx, TSNode call) {
    ...
    if (ts_node_is_null(tnode))
        return;
    const ANIType *t = cs_parse_type_node(ctx, tnode);
    const char *tqn = NULL;
    if (t && t->kind == ANI_TYPE_NAMED)
        tqn = t->data.named.qualified_name;
    else if (t && t->kind == ANI_TYPE_TEMPLATE)
    ...
    const ANIRegisteredFunc *f = cs_lookup_method(ctx, tqn, short_name);
    if (f) {
...
static void cs_resolve_calls_in_node(CSLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "block") == 0) {
        ANIScope *prev = ctx->current_scope;
        ctx->current_scope = ani_scope_push(ctx->arena, prev);
    ...
    do {
        ...
        if (strcmp(ck, "class_declaration") == 0 || strcmp(ck, "struct_declaration") == 0 ||
            ...
            strcmp(ck, "destructor_declaration") == 0 || strcmp(ck, "operator_declaration") == 0 ||
            strcmp(ck, "conversion_operator_declaration") == 0 ||
            strcmp(ck, "indexer_declaration") == 0 || strcmp(ck, "property_declaration") == 0 ||
            strcmp(ck, "event_declaration") == 0 || strcmp(ck, "local_function_statement") == 0 ||
...
static void cs_collect_type_params(CSLSPContext *ctx, TSNode node, const char ***out_names,
    ...
    *out_count = 0;
    ...
    if (ts_node_is_null(tplist))
        tplist = cs_child_named_kind(node, "type_parameter_list");
    if (ts_node_is_null(tplist))
        return;
    uint32_t nc = ts_node_child_count(tplist);
    int cap = 4;
    const char **arr = (const char **)ani_arena_alloc(ctx->arena, (size_t)cap * sizeof(*arr));
    if (!arr)
        return;
    int n = 0;
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (ts_node_is_null(c) || !ts_node_is_named(c))
            continue;
        if (strcmp(ts_node_type(c), "type_parameter") != 0)
            continue;
        ...
        if (n + 1 >= cap) {
            int new_cap = cap * 2;
            const char **ne =
                (const char **)ani_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*ne));
            if (!ne)
                break;
            for (int j = 0; j < n; j++)
                ne[j] = arr[j];
...
static void cs_process_function_like(CSLSPContext *ctx, TSNode node) {
    const char *kind = ts_node_type(node);
    bool is_method =
        (strcmp(kind, "method_declaration") == 0 || strcmp(kind, "local_function_statement") == 0);
    ...
    const ANIType **saved_tp_args = ctx->type_param_args;
    ...
    if (is_method) {
        ...
        if (!ts_node_is_null(nm))
            short_name = cs_node_text(ctx, nm);
    } else if (is_ctor) {
        short_name = ".ctor";
    } else if (is_dtor) {
        short_name = ".dtor";
        ...
        if (!ts_node_is_null(op))
            short_name = cs_node_text(ctx, op);
    }

    if (short_name) {
        if (ctx->enclosing_class_qn) {
            ctx->enclosing_func_qn =
                ani_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, short_name);
        } else if (ctx->module_qn) {
            ctx->enclosing_func_qn =
                ani_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, short_name);
        } else {
            ctx->enclosing_func_qn = ani_arena_strdup(ctx->arena, short_name);
        }
    }
    ...
    if (is_method) {
        ...
        if (tp_count > 0) {
            const ANIType **args = (const ANIType **)ani_arena_alloc(
                ctx->arena, (size_t)(tp_count + 1) * sizeof(*args));
            if (args) {
                for (int i = 0; i < tp_count; i++) {
                    args[i] = ani_type_type_param(ctx->arena, tp_names[i]);
                }
                args[tp_count] = NULL;
    ...
    if (is_method) {
        ...
        if (ts_node_is_null(params))
            params = cs_child_named_kind(node, "parameter_list");
        if (!ts_node_is_null(params)) {
            ...
            for (uint32_t i = 0; i < nc; i++) {
                ...
                if (ts_node_is_null(c) || !ts_node_is_named(c))
                    continue;
                if (strcmp(ts_node_type(c), "parameter") != 0)
                    continue;
                /* Look for "this" keyword child. */
                uint32_t pc = ts_node_child_count(c);
                for (uint32_t j = 0; j < pc; j++) {
                    TSNode cc = ts_node_child(c, j);
                    if (ts_node_is_null(cc))
                        continue;
        ...
        if (!ts_node_is_null(params))
            cs_bind_parameters(ctx, params, false);
    ...
    if (ts_node_is_null(body)) {
        ...
        if (!ts_node_is_null(arrow)) {
            ...
            if (!ts_node_is_null(expr))
                cs_resolve_calls_in_node(ctx, expr);
        }
    } else {
        ...
        if (!ts_node_is_null(accessors)) {
            ...
            for (uint32_t i = 0; i < nc; i++) {
                ...
                if (ts_node_is_null(a) || !ts_node_is_named(a))
                    continue;
                if (strcmp(ts_node_type(a), "accessor_declaration") != 0)
                    continue;
                TSNode abody = ts_node_child_by_field_name(a, "body", 4);
                if (ts_node_is_null(abody))
                    abody = cs_child_named_kind(a, "block");
                if (!ts_node_is_null(abody))
                    cs_resolve_calls_in_node(ctx, abody);
                else {
...
static void cs_process_type_decl(CSLSPContext *ctx, TSNode node) {
    ...
    bool is_record =
        strcmp(kind, "record_declaration") == 0 || strcmp(kind, "record_struct_declaration") == 0;
    ...
    if (ctx->module_qn) {
        ...
        ctx->enclosing_class_qn = ani_arena_strdup(ctx->arena, cname);
    }
    ...
    if (tp_count > 0) {
        const ANIType **args =
            (const ANIType **)ani_arena_alloc(ctx->arena, (size_t)(tp_count + 1) * sizeof(*args));
        if (args) {
            for (int i = 0; i < tp_count; i++) {
                args[i] = ani_type_type_param(ctx->arena, tp_names[i]);
            }
    ...
    /* Base list -> enclosing_base_qn + enclosing_iface_qns. */
    {
        ...
        if (!ts_node_is_null(bl)) {
            ...
            int icap = 4, icount = 0;
            const char **ifs =
                (const char **)ani_arena_alloc(ctx->arena, (size_t)icap * sizeof(*ifs));
            for (uint32_t i = 0; i < nc; i++) {
                ...
                const ANIRegisteredType *rt = cs_lookup_type_qn(ctx, qn);
                if (rt && rt->is_interface) {
                    if (icount + 1 >= icap) {
                        int nc2 = icap * 2;
                        const char **ne =
                            (const char **)ani_arena_alloc(ctx->arena, (size_t)nc2 * sizeof(*ne));
                        if (!ne)
                            continue;
                    ...
                    if (!ctx->enclosing_base_qn)
                        ctx->enclosing_base_qn = qn;
                    else {
                        if (icount + 1 >= icap) {
                            int nc2 = icap * 2;
                            const char **ne = (const char **)ani_arena_alloc(
    ...
    if (!is_enum) {
        TSNode body = ts_node_child_by_field_name(node, "body", 4);
        if (ts_node_is_null(body))
            body = cs_child_named_kind(node, "declaration_list");
        if (!ts_node_is_null(body)) {
            ...
            for (uint32_t i = 0; i < nc; i++) {
                ...
                if (strcmp(k, "method_declaration") == 0 ||
                    ...
                    strcmp(k, "property_declaration") == 0 || strcmp(k, "event_declaration") == 0) {
        ...
        if (!ts_node_is_null(pctor)) {
            ...
            ctx->enclosing_func_qn =
                ani_arena_sprintf(ctx->arena, "%s..ctor", ctx->enclosing_class_qn);
    ...
    while (top > 0) {
        ...
        if (strcmp(k, "using_directive") == 0) {
            ...
            for (uint32_t i = 0; i < nc; i++) {
                ...
                if (strcmp(ck, "global") == 0) {
                    is_global = true;
                    continue;
                }
                if (strcmp(ck, "static") == 0) {
                    is_static = true;
                    continue;
                }
                if (strcmp(ck, "=") == 0) {
                ...
                if (strcmp(ck, "name_equals") == 0) {
                    ...
                    TSNode id = cs_first_named_child(c);
                    if (!ts_node_is_null(id))
                        alias_name = cs_node_text(ctx, id);
                    continue;
                }
            ...
            if (seen_equals && !ts_node_is_null(pre_eq) && !ts_node_is_null(post_eq)) {
                ...
                if (t)
                    target = cs_normalize_name(ctx->arena, t);
            ...
            if (target) {
                if (is_alias) {
                    cs_lsp_add_using(ctx, ANI_CS_USING_ALIAS, alias_name ? alias_name : "", target,
                                     is_global);
                } else if (is_static) {
                    cs_lsp_add_using(ctx, ANI_CS_USING_STATIC, "", target, is_global);
                } else {
                    cs_lsp_add_using(ctx, ANI_CS_USING_NAMESPACE, "", target, is_global);
                }
            }
        }
...
static void cs_collect_namespace(CSLSPContext *ctx, TSNode ns_node, bool file_scoped) {
    ...
    if (ts_node_is_null(body))
        body = cs_child_named_kind(ns_node, "declaration_list");
    if (!ts_node_is_null(body)) {
        uint32_t nc = ts_node_child_count(body);
        for (uint32_t i = 0; i < nc; i++) {
            TSNode c = ts_node_child(body, i);
...
void cs_lsp_process_file(CSLSPContext *ctx, TSNode root) {
    ...
    for (uint32_t i = 0; i < kn; i++) {
        ...
        if (strcmp(k, "file_scoped_namespace_declaration") == 0) {
            ...
            cs_collect_namespace(ctx, c, false);
        } else if (strcmp(k, "class_declaration") == 0 || strcmp(k, "struct_declaration") == 0 ||
                   strcmp(k, "record_declaration") == 0 ||
            ...
            cs_process_type_decl(ctx, c);
                   ...
                   strcmp(k, "foreach_statement") == 0 || strcmp(k, "for_each_statement") == 0 ||
    ...
    if (file_scoped_active && ctx->namespace_count > 0)
        cs_namespace_pop(ctx);
}
...
static void cs_parse_signature(ANIArena *arena, const char *signature, CSLSPContext *ctx,
                               const char ***out_names, const ANIType ***out_types) {
    *out_names = NULL;
    *out_types = NULL;
    if (!signature)
        return;
    ...
    int cap = 8;
    int count = 0;
    const char **names = (const char **)ani_arena_alloc(arena, (size_t)cap * sizeof(*names));
    const ANIType **types = (const ANIType **)ani_arena_alloc(arena, (size_t)cap * sizeof(*types));
    if (!names || !types)
        return;
    ...
    while (*p && *p != ')') {
        ...
        while (name_end > type_start && (name_end[-1] == ' ' || name_end[-1] == '\t'))
            name_end--;
        if (!last_space) {
            /* No name — treat the whole token as the type with synthetic name. */
            char *type_text = ani_arena_strndup(arena, type_start, (size_t)(name_end - type_start));
            const ANIType *t = ani_type_unknown();
            if (ctx)
                t = cs_resolve_type_name(ctx, type_text)
                        ? ani_type_named(ctx->arena, cs_resolve_type_name(ctx, type_text))
                        : ani_type_unknown();
            (void)t;
            if (count + 1 >= cap)
                break;
            names[count] = ani_arena_sprintf(arena, "_arg%d", count);
            types[count] = t;
            count++;
        } else {
            char *type_text =
                ani_arena_strndup(arena, type_start, (size_t)(last_space - type_start));
            char *pname =
                ani_arena_strndup(arena, last_space + 1, (size_t)(name_end - last_space - 1));
            const ANIType *t = ani_type_unknown();
            if (ctx) {
                const char *resolved = cs_resolve_type_name(ctx, type_text);
                if (resolved)
                    t = ani_type_named(ctx->arena, resolved);
            }
            if (count + 1 >= cap)
                break;
            names[count] = pname;
...
static void cs_register_type_decls(CSLSPContext *ctx, ANITypeRegistry *reg, TSNode root) {
    /* We rely on ANIFileResult.defs entries already being filled by the
...
static cs_fields_t *cs_fields_get(ANIArena *arena, cs_fields_table_t *tab, const char *class_qn) {
    ...
    if (tab->count + 1 >= tab->cap) {
        ...
        cs_fields_t *ne = (cs_fields_t *)ani_arena_alloc(arena, (size_t)new_cap * sizeof(*ne));
        if (!ne)
            return NULL;
        for (int i = 0; i < tab->count; i++)
            ne[i] = tab->items[i];
    ...
    slot->cap = 8;
    slot->field_names =
        (const char **)ani_arena_alloc(arena, (size_t)slot->cap * sizeof(*slot->field_names));
    slot->field_types =
        (const ANIType **)ani_arena_alloc(arena, (size_t)slot->cap * sizeof(*slot->field_types));
    return slot;
}

static void cs_fields_add_debug(ANIArena *arena, cs_fields_t *f, const char *name,
                                const ANIType *type, bool debug);

static void cs_fields_add(ANIArena *arena, cs_fields_t *f, const char *name, const ANIType *type) {
...
static void cs_fields_add_debug(ANIArena *arena, cs_fields_t *f, const char *name,
    ...
    if (f->count + 2 >= f->cap) {
        int new_cap = f->cap * 2;
        const char **nn = (const char **)ani_arena_alloc(arena, (size_t)new_cap * sizeof(*nn));
        const ANIType **nt =
            (const ANIType **)ani_arena_alloc(arena, (size_t)new_cap * sizeof(*nt));
        if (!nn || !nt)
            return;
        for (int i = 0; i < f->count; i++) {
    ...
    f->field_names[f->count] = ani_arena_strdup(arena, name);
    f->field_types[f->count] = type ? type : ani_type_unknown();
    f->count++;
    f->field_names[f->count] = NULL;
    f->field_types[f->count] = NULL;
}

static void cs_collect_class_fields(CSLSPContext *ctx, ANITypeRegistry *reg, TSNode root,
    ...
    while (top > 0) {
        ...
        if (strcmp(k, "field_declaration") == 0 || strcmp(k, "property_declaration") == 0 ||
            ...
            while (!ts_node_is_null(p)) {
                ...
                if (strcmp(pk, "class_declaration") == 0 || strcmp(pk, "struct_declaration") == 0 ||
            ...
            if (cls_short) {
                const char *cls_qn = ctx->module_qn ? ani_arena_sprintf(ctx->arena, "%s.%s",
                                                                        ctx->module_qn, cls_short)
                                                    : ani_arena_strdup(ctx->arena, cls_short);
                ...
                if (strcmp(k, "field_declaration") == 0 ||
                    ...
                    if (!ts_node_is_null(vd)) {
                        ...
                        if (ts_node_is_null(tnode))
                            tnode = cs_first_named_child(vd);
                        const ANIType *t = ts_node_is_null(tnode) ? ani_type_unknown()
                        ...
                        for (uint32_t i = 0; i < vc; i++) {
                            ...
                            if (ts_node_is_null(id))
                                id = ts_node_child_by_field_name(c, "name", 4);
                            if (ts_node_is_null(id))
                                continue;
                            char *fn = cs_node_text(ctx, id);
                            if (fn)
                                cs_fields_add_debug(ctx->arena, f, fn, t, ctx->debug);
                        }
                    }
                } else {
                    /* property */
        ...
        if (strcmp(k, "record_declaration") == 0 || strcmp(k, "record_struct_declaration") == 0 ||
            strcmp(k, "class_declaration") == 0 || strcmp(k, "struct_declaration") == 0) {
            ...
            if (!ts_node_is_null(params) && !ts_node_is_null(nm)) {
                ...
                if (cls_short) {
                    const char *cls_qn =
                        ctx->module_qn
                            ? ani_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cls_short)
                            : ani_arena_strdup(ctx->arena, cls_short);
                    cs_fields_t *f = cs_fields_get(ctx->arena, tab, cls_qn);
                    ...
                    for (uint32_t i = 0; i < pc; i++) {
                        ...
                        if (ts_node_is_null(tn) || ts_node_is_null(pn))
                            continue;
                        const ANIType *t = cs_parse_type_node(ctx, tn);
                        char *pname = cs_node_text(ctx, pn);
                        if (pname)
                            cs_fields_add(ctx->arena, f, pname, t);
                    }
                }
            }
        }
 ...
 *
 * The unified extractor leaves ANIDefinition.return_type NULL for C#
...
static void cs_method_rt_add(ANIArena *arena, cs_method_rt_table_t *tab, const char *qn,
                             const ANIType *rt) {
    if (!qn || !rt)
        return;
    for (int i = 0; i < tab->count; i++) {
        if (strcmp(tab->items[i].qn, qn) == 0)
            return;
    }
    if (tab->count + 1 >= tab->cap) {
        int new_cap = tab->cap ? tab->cap * 2 : 16;
        cs_method_rt_entry_t *ne =
            (cs_method_rt_entry_t *)ani_arena_alloc(arena, (size_t)new_cap * sizeof(*ne));
        if (!ne)
            return;
        for (int i = 0; i < tab->count; i++)
            ne[i] = tab->items[i];
...
static void cs_collect_method_return_types(CSLSPContext *ctx, TSNode root,
    ...
    while (top > 0) {
        ...
        if (is_method || is_property || is_indexer) {
            ...
            const char *cls_qn =
                ctx->module_qn ? ani_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cls_short)
                               : ani_arena_strdup(ctx->arena, cls_short);
            TSNode tnode = ts_node_child_by_field_name(n, "type", 4);
            TSNode nm = ts_node_child_by_field_name(n, "name", 4);
            ...
            if (ts_node_is_null(tnode)) {
                ...
                for (uint32_t i = 0; i < cn; i++) {
                    ...
                    if (strcmp(ck, "modifier") == 0 || strcmp(ck, "attribute_list") == 0 ||
                        strcmp(ck, "type_parameter_list") == 0)
                        continue;
                    /* The name comes after the type; skip it. */
                    if (!ts_node_is_null(nm) && ts_node_eq(c, nm))
                        continue;
                    if (strcmp(ck, "parameter_list") == 0 || strcmp(ck, "block") == 0 ||
                        strcmp(ck, "arrow_expression_clause") == 0 ||
                        strcmp(ck, "type_parameter_constraints_clause") == 0)
                        break;
            ...
            if (is_indexer) {
                ...
                if (!ts_node_is_null(tnode)) {
                    const ANIType *rt = cs_parse_type_node(ctx, tnode);
                    cs_method_rt_add(ctx->arena, tab,
                                     ani_arena_sprintf(ctx->arena, "%s.%s", cls_qn, short_name),
                                     rt);
                }
...
void ani_cs_refine_ast_return_types(CSLSPContext *ctx, ANITypeRegistry *reg, TSNode root) {
    ...
    for (int i = 0; i < mtab.count; i++) {
        ...
        const ANIType *rt = mtab.items[i].rt;
        bool patched = false;
            ...
            if (strcmp(reg->funcs[j].qualified_name, qn) == 0) {
                ...
                if (rets) {
                    ...
                    reg->funcs[j].signature =
                        ani_type_func_replace_returns(ctx->arena, reg->funcs[j].signature, rets);
                }
                patched = true;
                break;
            }
...
/* ── parse return type from ANIDefinition.return_type ──────────── */

static const ANIType *cs_parse_return_type_text(CSLSPContext *ctx, const char *text) {
    if (!text || !*text)
        return ani_type_unknown();
    const char *p = text;
    while (*p == ' ' || *p == ':')
    ...
    if (n == 0)
        return ani_type_unknown();
    char *trimmed = ani_arena_strndup(ctx->arena, p, n);
...
static const ANIType *cs_signature_param_type_adapter(ANIArena *arena, const char *text,
                                                      void *parser_ctx) {
    (void)arena;
    CSLSPContext *ctx = (CSLSPContext *)parser_ctx;
    return ctx ? cs_parse_return_type_text(ctx, text) : ani_type_unknown();
}

/* ── single-file entry: ani_run_cs_lsp ──────────────────────────── */
...
void ani_run_cs_lsp(ANIArena *arena, ANIFileResult *result, const char *source, int source_len,
    ...
    /* Phase A: stdlib. */
    ani_csharp_stdlib_register(&reg, arena);
    ...
    for (int i = 0; i < result->defs.count; i++) {
                    ...
                    if (emb) {
                        for (int j = 0; j < bc; j++) {
                            ...
                            if (base[0] != '.' && !strchr(base, '.') && module_qn) {
                                q = ani_arena_sprintf(arena, "%s.%s", module_qn, base);
                            }
                            emb[j] = q;
                        }
                        emb[bc] = NULL;
        ...
        if (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0) {
            ...
            const ANIType *rt = d->return_type ? ani_type_unknown() /* will refine below with ctx */
                                               : ani_type_unknown();
            const ANIType **rets =
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        for (int j = 0; j < reg.func_count; j++) {
            if (strcmp(reg.funcs[j].qualified_name, d->qualified_name) != 0)
                continue;
            const ANIType **return_types = NULL;
            if (reg.funcs[j].signature && reg.funcs[j].signature->kind == ANI_TYPE_FUNC) {
                return_types = reg.funcs[j].signature->data.func.return_types;
            }
            reg.funcs[j].signature = ani_type_func(arena, NULL, param_types, return_types);
            break;
        }
    }
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ANIDefinition *d = &result->defs.items[i];
        if (!d->qualified_name || !d->return_type)
            continue;
        if (strcmp(d->label, "Function") != 0 && strcmp(d->label, "Method") != 0)
            continue;
        const ANIType *rt = cs_parse_return_type_text(&ctx, d->return_type);
        ...
        for (int j = 0; j < reg.func_count; j++) {
            if (strcmp(reg.funcs[j].qualified_name, d->qualified_name) == 0) {
                ...
                if (rets) {
                    ...
                    rets[1] = NULL;
                    reg.funcs[j].signature =
                        ani_type_func_replace_returns(arena, reg.funcs[j].signature, rets);
                }
                break;
            }
        }
     ...
     * leaves ANIDefinition.return_type NULL for C# methods, so we walk the
     * AST ourselves to recover declared return types. */
    ani_cs_refine_ast_return_types(&ctx, &reg, root);
    ...
    if (ctx.debug) {
        ...
        for (int i = 0; i < result->resolved_calls.count; i++) {
            const ANIResolvedCall *rc = &result->resolved_calls.items[i];
...
static void cs_register_lsp_defs(ANIArena *arena, ANITypeRegistry *reg, ANILSPDef *defs,
                                 int def_count) {
    for (int i = 0; i < def_count; i++) {
            ...
            if (d->embedded_types && *d->embedded_types) {
                ...
                for (const char *p = d->embedded_types; *p; p++)
                    if (*p == '|')
                        n++;
                const char **arr =
                    (const char **)ani_arena_alloc(arena, (size_t)(n + 1) * sizeof(*arr));
                if (arr) {
                    int idx = 0;
                    ...
                    for (const char *p = d->embedded_types;; p++) {
                        if (*p == '|' || *p == '\0') {
                            ...
                            if (len > 0) {
                                arr[idx++] = ani_arena_strndup(arena, start, len);
                            }
                            if (*p == '\0')
                                break;
                            start = p + 1;
        ...
        if (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0) {
            ...
            rf.signature = ani_type_func(arena, NULL, param_types, rets);
            ani_registry_add_func(reg, rf);
        }
    }
}
...
/* True iff a CS def registers as a TYPE — must mirror cs_register_lsp_defs'
 * own label branch exactly, or the two-phase split below silently mis-buckets. */
static bool cs_def_is_type(const ANILSPDef *d) {
...
ANITypeRegistry *ani_cs_build_cross_registry(ANIArena *arena, ANILSPDef *defs, int def_count) {
     ...
     * parses signatures, and type-name qualification does registry lookups
     * that are LINEAR scans until finalize builds the hash buckets — one
     * mixed pass over a 963k-def corpus measured ~280 s of lsp_cross_prepare.
     * Register all TYPES (no lookups in their registration), finalize once so
    ...
    ANILSPDef *cs = NULL;
    if (def_count > 0) {
        cs = (ANILSPDef *)ani_arena_alloc(arena, (size_t)def_count * sizeof(*cs));
        if (!cs)
            return NULL;
    }
    int total = 0;
    for (int i = 0; i < def_count; i++) {
        if (defs[i].lang == ANI_LANG_CSHARP && cs_def_is_type(&defs[i]))
            cs[total++] = defs[i];
    }
...
void ani_run_cs_lsp_cross_with_registry(ANIArena *arena, const char *source, int source_len,
    ...
    for (int i = 0; i < using_count; i++) {
        if (using_targets[i]) {
            cs_lsp_add_using(&ctx, ANI_CS_USING_NAMESPACE, "", using_targets[i], false);
        }
    }
    cs_lsp_process_file(&ctx, root);
...
void ani_batch_cs_lsp_cross(ANIArena *arena, ANIBatchCSLSPFile *files, int file_count,
    ...
    for (int i = 0; i < file_count; i++) {
        ANIBatchCSLSPFile *f = &files[i];
        ani_run_cs_lsp_cross(arena, f->source, f->source_len, f->module_qn, f->defs, f->def_count,
