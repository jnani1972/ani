 ...
 *
 * Architecture mirrors go_lsp / py_lsp: build a `ANITypeRegistry` from the
 ...
 *
 * Public entry points:
 *   - `ani_run_rust_lsp` — single-file resolution invoked from
 *     `ani_extract_file()`.
 *   - `ani_run_rust_lsp_cross` — cross-file resolution given a list of
 *     `ANILSPDef`s gathered by the pipeline.
 *   - `ani_batch_rust_lsp_cross` — batch wrapper that processes several
 *     files in one CGo call (per-file arenas + result copy).
 */
#ifndef ANI_LSP_RUST_LSP_H
#define ANI_LSP_RUST_LSP_H
...
#include "../ani.h"
#include "go_lsp.h" // for ANILSPDef (pipeline def), used by the Tier-2 builder
 ...
 * scoring >= ANI_LSP_CONFIDENCE_FLOOR (0.6). Numbers here mirror the Go
 * LSP so the call-edge mix from a polyglot project stays comparable. */
#define ANI_RUST_CONF_DIRECT 0.95f      /* path::to::func or alias hit  */
#define ANI_RUST_CONF_METHOD 0.95f      /* inherent method dispatch     */
#define ANI_RUST_CONF_TRAIT_SOLE 0.92f  /* trait method, single impl    */
#define ANI_RUST_CONF_TRAIT_AMB 0.85f   /* trait method, many impls     */
#define ANI_RUST_CONF_UFCS 0.93f        /* T::method() / Self::new()    */
#define ANI_RUST_CONF_PROMOTED 0.90f    /* through Deref / blanket impl */
#define ANI_RUST_CONF_MACRO_KNOWN 0.85f /* known std macro mapped to fn */
#define ANI_RUST_CONF_OPERATOR 0.88f    /* a+b → T::add (operator trait) */

/* Rust-flavoured LSP context: one per file, lifetime tied to a single
 * `ani_extract_file()` invocation (or the cross-file caller's arena). */
typedef struct {
    ANIArena *arena;
    const char *source;
    int source_len;
     ...
     * to every emitted ANIResolvedCall as `caller_qn`. */
    ...
    const ANIType *pending_closure_param_type;
    ...
    const struct ANICargoManifest *cargo_manifest;
     ...
     * into a CALLS edge when a *syntactic* ANICall with the same
    ...
    /* ANI_LSP_DEBUG=1 in env enables verbose stderr trace. */
    bool debug;
 ...
 * `ani_type_unknown()` for anything we cannot type. */
 ...
 * reference_type, primitive_type, …) into a ANIType. */
...
/* Populate the same per-file registry used by ani_run_rust_lsp, including
 ...
 * `ANILSPDef` in `go_lsp.h` for compatibility with the pipeline plumbing
...
/* Same as `ani_run_rust_lsp_cross`, plus an optional parsed Cargo manifest
