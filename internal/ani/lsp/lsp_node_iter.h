 ...
 * run for ~133 minutes). ani_lsp_collect_children() enumerates the children in
 * a single O(n) cursor pass into an arena array; callers then index the array.
 */
#ifndef ANI_LSP_NODE_ITER_H
#define ANI_LSP_NODE_ITER_H
...
static inline TSNode* ani_lsp_collect_children(ANIArena* arena, TSNode node, uint32_t* out_n) {
    uint32_t nc = ts_node_child_count(node);
    *out_n = 0;
    if (nc == 0) return NULL;
