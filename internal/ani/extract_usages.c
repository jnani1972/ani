...
// Forward declaration
static void walk_usages(ANIExtractCtx *ctx, TSNode root, const ANILangSpec *spec);
...
static bool is_reference_node(TSNode node, ANILanguage lang) {
    ...
    if (lang == ANI_LANG_PYTHON && strcmp(kind, "attribute") == 0 &&
    ...
    if ((lang == ANI_LANG_PUPPET || lang == ANI_LANG_VIMSCRIPT) &&
        strcmp(kind, "identifier") == 0) {
        TSNode parent = ts_node_parent(node);
        if (!ts_node_is_null(parent) &&
            ((lang == ANI_LANG_PUPPET && strcmp(ts_node_type(parent), "variable") == 0) ||
             (lang == ANI_LANG_VIMSCRIPT && strcmp(ts_node_type(parent), "argument") == 0))) {
            return false;
        }
    }
    ...
    case ANI_LANG_LLVM_IR:
        return strcmp(kind, "local_var") == 0 || strcmp(kind, "global_var") == 0;
    case ANI_LANG_PUPPET:
    case ANI_LANG_POWERSHELL:
    ...
    case ANI_LANG_CLOJURE:
    case ANI_LANG_COMMONLISP:
        return strcmp(kind, "sym_lit") == 0;
    case ANI_LANG_VIMSCRIPT:
    ...
    case ANI_LANG_EMACSLISP:
    case ANI_LANG_SCHEME:
    case ANI_LANG_FENNEL:
    case ANI_LANG_RACKET:
    case ANI_LANG_CHIALISP:
    case ANI_LANG_LINKERSCRIPT:
        return strcmp(kind, "symbol") == 0;
    case ANI_LANG_MAKEFILE:
        return strcmp(kind, "variable_reference") == 0;
    case ANI_LANG_CMAKE:
        /* `${watched}` contains a named `variable` leaf spanning only `watched`. */
        return strcmp(kind, "variable") == 0;
    case ANI_LANG_WOLFRAM:
    ...
    case ANI_LANG_RESCRIPT:
        return strcmp(kind, "value_identifier") == 0;
    case ANI_LANG_PURESCRIPT:
        return strcmp(kind, "variable") == 0;
    case ANI_LANG_NICKEL:
        return strcmp(kind, "ident") == 0;
    case ANI_LANG_JSONNET:
        return strcmp(kind, "id") == 0;
    case ANI_LANG_CFML:
        return strcmp(kind, "property_identifier") == 0;
    case ANI_LANG_OBJECTSCRIPT_UDL:
    case ANI_LANG_OBJECTSCRIPT_ROUTINE:
...
static bool is_forward_sibling_callee(ANILanguage language, TSNode node) {
    if (language == ANI_LANG_DART && strcmp(ts_node_type(node), "identifier") == 0) {
...
typedef enum {
    ANI_OCCURRENCE_STANDARD = 0,
    ANI_OCCURRENCE_LISP_DEF,
    ANI_OCCURRENCE_COMMONLISP_DEFUN,
    ANI_OCCURRENCE_FENNEL_FN,
    ANI_OCCURRENCE_ELIXIR_DEF,
    ANI_OCCURRENCE_JULIA_FUNCTION,
    ANI_OCCURRENCE_WOLFRAM_SET,
    ANI_OCCURRENCE_TYPST_LET,
    ANI_OCCURRENCE_AGDA_FUNCTION,
    ANI_OCCURRENCE_TLAPLUS_OPERATOR,
    ANI_OCCURRENCE_COBOL_MOVE,
    ANI_OCCURRENCE_HCL_ATTRIBUTE,
    ANI_OCCURRENCE_ELM_VALUE,
    ANI_OCCURRENCE_RESCRIPT_LET,
    ANI_OCCURRENCE_PURESCRIPT_LHS,
    ANI_OCCURRENCE_NICKEL_LET,
    ANI_OCCURRENCE_ERLANG_CLAUSE,
    ANI_OCCURRENCE_NIX_FUNCTION,
    ANI_OCCURRENCE_MATLAB_ARGUMENTS,
    ANI_OCCURRENCE_LEAN_BINDER,
    ANI_OCCURRENCE_PASCAL_PROC,
    ANI_OCCURRENCE_TEAL_FUNCTION,
    ANI_OCCURRENCE_VHDL_INTERFACE,
    ANI_OCCURRENCE_PINE_FUNCTION,
    ANI_OCCURRENCE_LLVM_FUNCTION,
    ANI_OCCURRENCE_PKL_DECLARATION,
} ANIOccurrencePolicy;

typedef struct {
    const char *const *whole_binding_nodes;
    const char *const *write_nodes;
    ANIOccurrencePolicy policy;
 ...
 * the positional ANILangSpec initializer used by every language. */
static const ANIOccurrenceSpec occurrence_specs[ANI_LANG_COUNT] = {
    [ANI_LANG_SQL] = {sql_binding_nodes, NULL, ANI_OCCURRENCE_STANDARD, false},
    [ANI_LANG_CLOJURE] = {NULL, NULL, ANI_OCCURRENCE_LISP_DEF, false},
    [ANI_LANG_SCHEME] = {NULL, NULL, ANI_OCCURRENCE_LISP_DEF, false},
    [ANI_LANG_RACKET] = {NULL, NULL, ANI_OCCURRENCE_LISP_DEF, false},
    [ANI_LANG_CHIALISP] = {NULL, NULL, ANI_OCCURRENCE_LISP_DEF, false},
    [ANI_LANG_COMMONLISP] = {NULL, NULL, ANI_OCCURRENCE_COMMONLISP_DEFUN, false},
    [ANI_LANG_FENNEL] = {NULL, NULL, ANI_OCCURRENCE_FENNEL_FN, false},
    [ANI_LANG_ELIXIR] = {NULL, NULL, ANI_OCCURRENCE_ELIXIR_DEF, false},
    [ANI_LANG_JULIA] = {NULL, NULL, ANI_OCCURRENCE_JULIA_FUNCTION, false},
    [ANI_LANG_WOLFRAM] = {NULL, NULL, ANI_OCCURRENCE_WOLFRAM_SET, false},
    [ANI_LANG_TYPST] = {NULL, NULL, ANI_OCCURRENCE_TYPST_LET, false},
    [ANI_LANG_AGDA] = {NULL, NULL, ANI_OCCURRENCE_AGDA_FUNCTION, false},
    [ANI_LANG_TLAPLUS] = {NULL, NULL, ANI_OCCURRENCE_TLAPLUS_OPERATOR, false},
    [ANI_LANG_COBOL] = {NULL, NULL, ANI_OCCURRENCE_COBOL_MOVE, false},
    [ANI_LANG_HCL] = {NULL, NULL, ANI_OCCURRENCE_HCL_ATTRIBUTE, false},
    [ANI_LANG_ELM] = {NULL, NULL, ANI_OCCURRENCE_ELM_VALUE, false},
    [ANI_LANG_RESCRIPT] = {rescript_binding_nodes, NULL, ANI_OCCURRENCE_RESCRIPT_LET, false},
    [ANI_LANG_PURESCRIPT] = {purescript_binding_nodes, NULL, ANI_OCCURRENCE_PURESCRIPT_LHS, false},
    [ANI_LANG_NICKEL] = {nickel_binding_nodes, NULL, ANI_OCCURRENCE_NICKEL_LET, false},
    [ANI_LANG_JSONNET] = {jsonnet_binding_nodes, NULL, ANI_OCCURRENCE_STANDARD, false},
    [ANI_LANG_HASKELL] = {haskell_binding_nodes, NULL, ANI_OCCURRENCE_STANDARD, false},
    [ANI_LANG_ERLANG] = {NULL, NULL, ANI_OCCURRENCE_ERLANG_CLAUSE, false},
    [ANI_LANG_FSHARP] = {fsharp_binding_nodes, NULL, ANI_OCCURRENCE_STANDARD, false},
    [ANI_LANG_NIX] = {NULL, NULL, ANI_OCCURRENCE_NIX_FUNCTION, false},
    [ANI_LANG_MATLAB] = {NULL, NULL, ANI_OCCURRENCE_MATLAB_ARGUMENTS, false},
    [ANI_LANG_LEAN] = {NULL, NULL, ANI_OCCURRENCE_LEAN_BINDER, false},
    [ANI_LANG_PASCAL] = {NULL, NULL, ANI_OCCURRENCE_PASCAL_PROC, false},
    [ANI_LANG_VERILOG] = {systemverilog_binding_nodes, NULL, ANI_OCCURRENCE_STANDARD, false},
    [ANI_LANG_AWK] = {awk_binding_nodes, NULL, ANI_OCCURRENCE_STANDARD, false},
    [ANI_LANG_CRYSTAL] = {crystal_binding_nodes, NULL, ANI_OCCURRENCE_STANDARD, false},
    [ANI_LANG_TEAL] = {teal_binding_nodes, NULL, ANI_OCCURRENCE_TEAL_FUNCTION, false},
    [ANI_LANG_VHDL] = {NULL, NULL, ANI_OCCURRENCE_VHDL_INTERFACE, false},
    [ANI_LANG_SYSTEMVERILOG] = {systemverilog_binding_nodes, NULL, ANI_OCCURRENCE_STANDARD, false},
    [ANI_LANG_PINE] = {NULL, NULL, ANI_OCCURRENCE_PINE_FUNCTION, false},
    [ANI_LANG_PUPPET] = {NULL, NULL, ANI_OCCURRENCE_STANDARD, true},
    [ANI_LANG_LLVM_IR] = {llvm_binding_nodes, NULL, ANI_OCCURRENCE_LLVM_FUNCTION, false},
    [ANI_LANG_PKL] = {NULL, NULL, ANI_OCCURRENCE_PKL_DECLARATION, false},
    [ANI_LANG_MESON] = {NULL, meson_write_nodes, ANI_OCCURRENCE_STANDARD, true},
    [ANI_LANG_GN] = {NULL, gn_write_nodes, ANI_OCCURRENCE_STANDARD, true},
    [ANI_LANG_LINKERSCRIPT] = {NULL, linkerscript_write_nodes, ANI_OCCURRENCE_STANDARD, true},
    [ANI_LANG_OBJECTSCRIPT_UDL] = {objectscript_binding_nodes, objectscript_write_nodes,
                                   ANI_OCCURRENCE_STANDARD, true},
    [ANI_LANG_OBJECTSCRIPT_ROUTINE] = {objectscript_binding_nodes, objectscript_write_nodes,
                                       ANI_OCCURRENCE_STANDARD, true},
};

static bool text_equals(ANIExtractCtx *ctx, TSNode node, const char *expected) {
...
static bool is_lisp_def_binding(ANIExtractCtx *ctx, TSNode node) {
    ...
    for (TSNode form = ts_node_parent(node); !ts_node_is_null(form); form = ts_node_parent(form)) {
        ...
        TSNode head_node =
            chialisp ? ani_lisp_named_child_skip_comments(form, 0) : ts_node_named_child(form, 0);
        if (ts_node_is_null(head_node)) {
            continue;
        }
        char *head = ani_node_text(ctx->arena, head_node, ctx->source);
        if (!(chialisp ? ani_chialisp_is_def_head(head) : lisp_def_head(head))) {
            continue;
        }
        if (node_contains(head_node, node) || named_child_contains(form, 1, node)) {
            return true;
        }
        if ((ctx->language == ANI_LANG_CLOJURE ||
...
static bool is_fennel_fn_binding(ANIExtractCtx *ctx, TSNode node) {
    for (TSNode form = ts_node_parent(node); !ts_node_is_null(form); form = ts_node_parent(form)) {
...
static bool is_elixir_def_binding(ANIExtractCtx *ctx, TSNode node) {
    for (TSNode form = ts_node_parent(node); !ts_node_is_null(form); form = ts_node_parent(form)) {
...
static bool fish_argument_marker(ANIExtractCtx *ctx, TSNode node) {
    char *text = ani_node_text(ctx->arena, node, ctx->source);
    return text && (strcmp(text, "-a") == 0 || strcmp(text, "--argument") == 0 ||
                    strcmp(text, "--argument-names") == 0);
}

static bool is_fish_function_parameter(ANIExtractCtx *ctx, TSNode node, WalkState *state) {
    ...
    for (TSNode previous = ts_node_prev_named_sibling(node); !ts_node_is_null(previous);
        ...
        char *text = ani_node_text(ctx->arena, previous, ctx->source);
        if (!text || text[0] != '-') {
            continue;
        }
...
static bool cfml_argument_tag_name_binding(ANIExtractCtx *ctx, TSNode node) {
    ...
    TSNode attribute_name = ani_find_child_by_kind(attribute, "cf_attribute_name");
    char *name_text = ani_node_text(ctx->arena, attribute_name, ctx->source);
    ...
    char *tag_text = ani_node_text(ctx->arena, tag, ctx->source);
...
static bool is_exact_language_binding(ANIExtractCtx *ctx, TSNode node, WalkState *state) {
    const char *kind = ts_node_type(node);
    switch (ctx->language) {
    case ANI_LANG_OCAML: {
...
static bool is_policy_binding(ANIExtractCtx *ctx, TSNode node,
                              const ANIOccurrenceSpec *occurrence) {
    switch (occurrence->policy) {
    case ANI_OCCURRENCE_LISP_DEF:
        return is_lisp_def_binding(ctx, node);
    case ANI_OCCURRENCE_COMMONLISP_DEFUN:
        return is_commonlisp_defun_binding(node);
    case ANI_OCCURRENCE_FENNEL_FN:
        return is_fennel_fn_binding(ctx, node);
    case ANI_OCCURRENCE_ELIXIR_DEF:
        return is_elixir_def_binding(ctx, node);
    case ANI_OCCURRENCE_JULIA_FUNCTION:
        return is_first_named_part_of(node, "function_definition");
    case ANI_OCCURRENCE_WOLFRAM_SET:
        return is_wolfram_lhs(node);
    case ANI_OCCURRENCE_TYPST_LET:
        return is_first_named_part_of(node, "let");
    case ANI_OCCURRENCE_AGDA_FUNCTION:
        for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
             parent = ts_node_parent(parent)) {
    ...
    case ANI_OCCURRENCE_LLVM_FUNCTION:
        ...
        return false;
    case ANI_OCCURRENCE_STANDARD:
    default:
        return false;
    }
}

static bool elixir_binary_operator_binds(ANIExtractCtx *ctx, TSNode node) {
    if (ctx->language != ANI_LANG_ELIXIR || strcmp(ts_node_type(node), "binary_operator") != 0) {
        return false;
    }
...
static bool is_binding_occurrence(ANIExtractCtx *ctx, TSNode node, const ANILangSpec *spec,
                                  WalkState *state) {
    const ANIOccurrenceSpec *occurrence = &occurrence_specs[ctx->language];
    ...
    while (!ts_node_is_null(current)) {
         ...
         * value usages. */
        if (ctx->language == ANI_LANG_PLSQL && strcmp(kind, "parameter") == 0) {
            current = parent;
            continue;
        }
        ...
        bool variable_container =
            spec->variable_node_types && ani_kind_in_set(parent, spec->variable_node_types);
        if (ctx->language == ANI_LANG_ELIXIR && strcmp(kind, "binary_operator") == 0) {
        ...
        bool declared_container =
            ...
            (spec->function_node_types && ani_kind_in_set(parent, spec->function_node_types)) ||
            (spec->class_node_types && ani_kind_in_set(parent, spec->class_node_types)) ||
            (spec->field_node_types && ani_kind_in_set(parent, spec->field_node_types)) ||
            variable_container;
...
static bool is_write_occurrence(ANIExtractCtx *ctx, TSNode node, const ANILangSpec *spec,
                                WalkState *state) {
    const ANIOccurrenceSpec *occurrence = &occurrence_specs[ctx->language];
    TSNode current = node;
    TSTreeCursor *cursor = reset_occurrence_cursor(state, node);
    while (!ts_node_is_null(current)) {
        ...
        bool assignment =
            (spec->assignment_node_types && ani_kind_in_set(parent, spec->assignment_node_types)) ||
        ...
        if (assignment) {
            ...
            if (ctx->language == ANI_LANG_LINKERSCRIPT &&
...
static bool language_may_stamp_exact_callable_value_candidate(ANILanguage language) {
    switch (language) {
    case ANI_LANG_JAVASCRIPT:
    case ANI_LANG_TYPESCRIPT:
    case ANI_LANG_TSX:
...
static TSNode call_reference_candidate_site(ANIExtractCtx *ctx, TSNode node, const char *name,
                                            WalkState *state) {
    ...
    bool ts_family = ctx->language == ANI_LANG_JAVASCRIPT || ctx->language == ANI_LANG_TYPESCRIPT ||
    ...
    if (ctx->language == ANI_LANG_PYTHON && strcmp(kind, "identifier") == 0 &&
    ...
    if (ctx->language == ANI_LANG_GO && strcmp(kind, "field_identifier") == 0 &&
    ...
    if (ctx->language == ANI_LANG_RUST && strcmp(kind, "scoped_identifier") == 0) {
     ...
     * as before. */
    if (ctx->language == ANI_LANG_KOTLIN && !ts_node_is_null(parent) &&
    ...
    if (ctx->language == ANI_LANG_PYTHON && !ts_node_is_null(parent) &&
        strcmp(ts_node_type(parent), "assignment") == 0) {
...
static char *reference_name(ANIExtractCtx *ctx, TSNode node) {
    char *name = ani_node_text(ctx->arena, node, ctx->source);
    if (!name || ctx->language != ANI_LANG_MAKEFILE ||
    ...
    return parenthesized ? ani_arena_strndup(ctx->arena, name + 2, length - 3) : name;
}

static bool lexical_identifier_case_insensitive(ANILanguage language) {
    switch (language) {
    case ANI_LANG_POWERSHELL:
    case ANI_LANG_FORTRAN:
    case ANI_LANG_ADA:
    case ANI_LANG_PASCAL:
    case ANI_LANG_COBOL:
    case ANI_LANG_VHDL:
        return true;
    default:
...
static const char *lexical_binding_key(ANIExtractCtx *ctx, WalkState *state, const char *name) {
    if (!ctx || !name) {
        return name;
    }
    ...
    if (ctx->language == ANI_LANG_TCL && name[0] == '$') {
        name++;
    }
    ...
    char *folded = (char *)ani_arena_alloc(ctx->arena, n + 1U);
    if (!folded) {
        if (state) {
...
static void stamp_usage_site(ANIExtractCtx *ctx, ANIUsage *usage, TSNode node, const char *name,
                             WalkState *state) {
...
static const ANILexicalScope *usage_lexical_scope(const WalkState *state, uint32_t id) {
...
static uint32_t usage_lexical_scope_id_for_node(ANIExtractCtx *ctx, const WalkState *state,
                                                TSNode node) {
    uint32_t scope_id = active_lexical_scope_id(state);
    if (!ctx || ctx->language != ANI_LANG_PYTHON || !python_default_value_reference(node)) {
        return scope_id;
    }
    uint32_t function_id = lexical_ancestor_of_kind(state, scope_id, true, false);
    const ANILexicalScope *function_scope = usage_lexical_scope(state, function_id);
...
static uint32_t lexical_ancestor_of_kind(const WalkState *state, uint32_t start_id,
    ...
    while (id != 0 && remaining-- > 0) {
        const ANILexicalScope *scope = usage_lexical_scope(state, id);
        if (!scope) {
            return 0;
        }
        bool function_scope = scope->kind == ANI_LEXICAL_SCOPE_FUNCTION ||
                              scope->kind == ANI_LEXICAL_SCOPE_COMPREHENSION;
        bool block_scope = scope->kind == ANI_LEXICAL_SCOPE_BLOCK;
...
static uint32_t python_nearest_namespace(const WalkState *state, uint32_t start_id) {
    ...
    while (id != 0 && remaining-- > 0) {
        ...
        if (scope->kind == ANI_LEXICAL_SCOPE_FUNCTION ||
            scope->kind == ANI_LEXICAL_SCOPE_COMPREHENSION ||
            scope->kind == ANI_LEXICAL_SCOPE_CLASS || scope->kind == ANI_LEXICAL_SCOPE_MODULE) {
            return id;
        }
        id = scope->parent_id;
    }
...
static bool ensure_lexical_binding_capacity(WalkState *state) {
    ...
    ANILexicalBinding *grown =
        (ANILexicalBinding *)ani_arena_alloc(state->arena, (size_t)new_capacity * sizeof(*grown));
    if (!grown) {
        state->lexical_binding_tracking_failed = true;
        return false;
    }
...
static TSNode declared_function_name_owner(TSNode node, const ANILangSpec *spec) {
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
         parent = ts_node_parent(parent)) {
        if (spec->function_node_types && ani_kind_in_set(parent, spec->function_node_types)) {
...
static TSNode declared_class_name_owner(TSNode node, const ANILangSpec *spec) {
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
         parent = ts_node_parent(parent)) {
        if (spec->class_node_types && ani_kind_in_set(parent, spec->class_node_types)) {
            return field_contains_node(parent, "name", node) ? parent : (TSNode){0};
        }
    }
    return (TSNode){0};
...
static bool ensure_python_directive_capacity(WalkState *state) {
    ...
    ANIPythonDirective *grown =
        (ANIPythonDirective *)ani_arena_alloc(state->arena, (size_t)new_capacity * sizeof(*grown));
    if (!grown) {
        state->lexical_binding_tracking_failed = true;
        return false;
    }
...
static void record_python_directive(WalkState *state, uint32_t function_scope_id, const char *name,
    ...
    ANIPythonDirective *directive = &state->python_directives[state->python_directive_count++];
    directive->function_scope_id = function_scope_id;
    directive->name = name;
    directive->kind = (uint8_t)kind;
}

static ANIPythonDirectiveKind python_directive_for(const WalkState *state,
                                                   uint32_t function_scope_id, const char *name) {
    for (int i = state ? state->python_directive_count - 1 : -1; i >= 0; i--) {
        const ANIPythonDirective *directive = &state->python_directives[i];
        if (directive->function_scope_id == function_scope_id && directive->name &&
            strcmp(directive->name, name) == 0) {
            return (ANIPythonDirectiveKind)directive->kind;
        }
    }
    return 0;
}
...
static uint32_t python_enclosing_function_namespace(const WalkState *state,
    ...
    const ANILexicalScope *inner = usage_lexical_scope(state, inner_function_id);
...
static bool binding_is_parameter(ANIExtractCtx *ctx, TSNode node, const ANILangSpec *spec) {
    const ANIOccurrenceSpec *occurrence = &occurrence_specs[ctx->language];
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
         parent = ts_node_parent(parent)) {
        const char *kind = ts_node_type(parent);
...
static bool import_kind_matches(TSNode node, const ANILangSpec *spec) {
    return (spec->import_node_types && ani_kind_in_set(node, spec->import_node_types)) ||
           (spec->import_from_types && ani_kind_in_set(node, spec->import_from_types));
}

static TSNode nearest_import_ancestor(TSNode node, const ANILangSpec *spec) {
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
         parent = ts_node_parent(parent)) {
 ...
 * name. */
static bool is_import_binding_occurrence(ANIExtractCtx *ctx, TSNode node, const ANILangSpec *spec) {
...
static uint32_t lexical_import_scope(const WalkState *state, uint32_t start_id) {
    ...
    while (id != 0 && remaining-- > 0) {
        ...
        if (scope->kind == ANI_LEXICAL_SCOPE_MODULE || scope->kind == ANI_LEXICAL_SCOPE_FUNCTION ||
            scope->kind == ANI_LEXICAL_SCOPE_COMPREHENSION ||
            scope->kind == ANI_LEXICAL_SCOPE_BLOCK) {
            return id;
        }
        id = scope->parent_id;
    }
...
static void record_lexical_binding(ANIExtractCtx *ctx, WalkState *state, TSNode node,
                                   const ANILangSpec *spec, const char *raw_name,
                                   bool import_binding) {
    if (!ctx || !state || !raw_name || !raw_name[0] || state->lexical_binding_tracking_failed) {
        return;
    }
    bool parameter = binding_is_parameter(ctx, node, spec);
    if (ctx->language == ANI_LANG_VIMSCRIPT && parameter && !strchr(raw_name, ':')) {
        raw_name = ani_arena_sprintf(ctx->arena, "a:%s", raw_name);
        if (!raw_name) {
            state->lexical_binding_tracking_failed = true;
            return;
        }
    ...
    if (ctx->language == ANI_LANG_PYTHON && (lexical_ancestor_kind(node, "global_statement") ||
                                             lexical_ancestor_kind(node, "nonlocal_statement"))) {
        uint32_t function_id = lexical_ancestor_of_kind(state, current_id, true, false);
        ANIPythonDirectiveKind directive = lexical_ancestor_kind(node, "global_statement")
                                               ? ANI_PYTHON_DIRECTIVE_GLOBAL
    ...
    if (!ts_node_is_null(function_declaration)) {
        ...
        if (ctx->language == ANI_LANG_PYTHON) {
            scope_id = python_nearest_namespace(state, parent_id);
            const ANILexicalScope *owner = usage_lexical_scope(state, scope_id);
            if (!owner || (owner->kind != ANI_LEXICAL_SCOPE_FUNCTION &&
                           owner->kind != ANI_LEXICAL_SCOPE_COMPREHENSION)) {
                return;
            }
        } else {
        ...
        whole_scope = true;
    } else if (ctx->language == ANI_LANG_PYTHON && !ts_node_is_null(class_declaration)) {
        uint32_t class_id = python_nearest_namespace(state, current_id);
        const ANILexicalScope *class_scope = usage_lexical_scope(state, class_id);
        ...
        whole_scope = true;
    } else if (ctx->language == ANI_LANG_PYTHON) {
        uint32_t namespace_id = python_nearest_namespace(state, current_id);
        const ANILexicalScope *namespace_scope = usage_lexical_scope(state, namespace_id);
        uint32_t function_id =
            namespace_scope && (namespace_scope->kind == ANI_LEXICAL_SCOPE_FUNCTION ||
                                namespace_scope->kind == ANI_LEXICAL_SCOPE_COMPREHENSION)
                ? namespace_id
                : lexical_ancestor_of_kind(state, current_id, true, false);
        ANIPythonDirectiveKind directive = python_directive_for(state, function_id, name);
        if (directive == ANI_PYTHON_DIRECTIVE_GLOBAL) {
            scope_id = state->root_lexical_scope_id;
        } else if (directive == ANI_PYTHON_DIRECTIVE_NONLOCAL) {
        ...
        whole_scope = directive == 0 && (owner->kind == ANI_LEXICAL_SCOPE_FUNCTION ||
                                         owner->kind == ANI_LEXICAL_SCOPE_COMPREHENSION);
    } else if (ctx->language == ANI_LANG_POWERSHELL) {
        scope_id = lexical_ancestor_of_kind(state, current_id, true, false);
        whole_scope = true;
    } else if (ctx->language == ANI_LANG_JAVASCRIPT || ctx->language == ANI_LANG_TYPESCRIPT ||
               ctx->language == ANI_LANG_TSX || ctx->language == ANI_LANG_ARKTS) {
    ...
    } else if (ctx->language == ANI_LANG_RUST && import_binding) {
    ...
    const ANILexicalScope *scope = usage_lexical_scope(state, scope_id);
    if (!scope || !ensure_lexical_binding_capacity(state)) {
        return;
    }
    ANILexicalBinding *binding = &state->lexical_bindings[state->lexical_binding_count++];
...
static int compare_lexical_bindings(const void *left_ptr, const void *right_ptr) {
    const ANILexicalBinding *left = (const ANILexicalBinding *)left_ptr;
    const ANILexicalBinding *right = (const ANILexicalBinding *)right_ptr;
...
static int compare_binding_key(const ANILexicalBinding *binding, uint32_t scope_id,
                               const char *name) {
...
static bool lexical_binding_active_at(const WalkState *state, uint32_t scope_id, const char *name,
    ...
    while (index < state->lexical_binding_count) {
        const ANILexicalBinding *binding = &state->lexical_bindings[index++];
...
static const ANILexicalScope *lexical_binding_scope_for_usage(const WalkState *state,
                                                              const ANIUsage *usage,
                                                              const char *name) {
...
void ani_finalize_lexical_usages(ANIExtractCtx *ctx, WalkState *state) {
    ...
    for (int i = state->usage_start_index; i < ctx->result->usages.count; i++) {
        ANIUsage *usage = &ctx->result->usages.items[i];
        ...
        const ANILexicalScope *binding_scope = lexical_binding_scope_for_usage(state, usage, name);
        if (!binding_scope) {
            continue;
        }
        usage->semantic_reference_blocked = true;
        usage->semantic_reference_local_shadow = binding_scope->kind != ANI_LEXICAL_SCOPE_MODULE;
    }
    ...
    if (state->lexical_binding_tracking_failed) {
        for (int i = state->usage_start_index; i < ctx->result->usages.count; i++) {
            ANIUsage *usage = &ctx->result->usages.items[i];
            usage->semantic_reference_blocked = true;
            const ANILexicalScope *scope = usage_lexical_scope(state, usage->lexical_scope_id);
            usage->semantic_reference_local_shadow =
                scope && scope->kind != ANI_LEXICAL_SCOPE_MODULE;
        }
    }
}

static char *perl_direct_coderef_name(ANIExtractCtx *ctx, TSNode node) {
    if (!ctx || ctx->language != ANI_LANG_PERL ||
    ...
    return end > start ? ani_arena_strndup(ctx->arena, start, (size_t)(end - start)) : NULL;
}

static bool inside_direct_perl_coderef(ANIExtractCtx *ctx, TSNode node) {
    if (!ctx || ctx->language != ANI_LANG_PERL) {
        return false;
    }
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
...
static bool emit_direct_perl_coderef_usage(ANIExtractCtx *ctx, TSNode node,
...
static void try_emit_usage(ANIExtractCtx *ctx, TSNode node, const ANILangSpec *spec,
                           bool inside_call, bool inside_import) {
    if (emit_direct_perl_coderef_usage(ctx, node, ani_enclosing_func_qn_cached(ctx, node), 0)) {
        return;
    }
    ...
    if (name && name[0] && !ani_is_keyword(name, ctx->language)) {
        ANIUsage usage = {0};
        usage.ref_name = name;
        usage.enclosing_func_qn = ani_enclosing_func_qn_cached(ctx, node);
...
static void walk_usages(ANIExtractCtx *ctx, TSNode root, const ANILangSpec *spec) {
    ...
    int cap = 256;
    UsageFrame *frames = (UsageFrame *)ani_arena_alloc(ctx->arena, (size_t)cap * sizeof(*frames));
    if (!frames) {
        return;
    }
    ...
    while (top > 0) {
        ...
        if (entering) {
            ...
            f->counts_call = ani_kind_in_set(f->node, spec->call_node_types);
            f->counts_import =
                (has_imports && ani_kind_in_set(f->node, spec->import_node_types)) ||
                (has_from_imports && ani_kind_in_set(f->node, spec->import_from_types));
            if (f->counts_call) {
                call_depth++;
            }
            if (f->counts_import) {
        ...
        if (f->next_child < count) {
            ...
            if (top == cap) {
                int new_cap = cap * 2;
                UsageFrame *grown =
                    (UsageFrame *)ani_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*grown));
                if (!grown) {
                    return;
                }
...
void handle_usages(ANIExtractCtx *ctx, TSNode node, const ANILangSpec *spec, WalkState *state) {
    ...
    if (exact_callee) {
        if (reference_node && state->invocation_kind == ANI_INVOCATION_CALLABLE_REFERENCE) {
            char *name = reference_name(ctx, node);
            if (name && name[0] && !ani_is_keyword(name, ctx->language)) {
                ANIUsage usage = {0};
                usage.ref_name = name;
    ...
    if (state->inside_import) {
        if (is_import_binding_occurrence(ctx, node, spec)) {
            ...
            if (binding_name && binding_name[0] && !ani_is_keyword(binding_name, ctx->language)) {
    ...
    bool python_scope_directive =
        ctx->language == ANI_LANG_PYTHON && (lexical_ancestor_kind(node, "global_statement") ||
                                             lexical_ancestor_kind(node, "nonlocal_statement"));
