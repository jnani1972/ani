...
# machine). Config: ~/.claude/ani-vm/config (ANI_VM_HOST, ANI_VM_USER),
# key: ~/.claude/ani-vm/id_ed25519. Provision first: provision-windows.sh.
#
# Usage:
...
temp root). Config: ~/.claude/ani-vm/config (ANI_VM_HOST, ANI_VM_USER,
ANI_VM_HOST_KEY_SHA256); key: ~/.claude/ani-vm/id_ed25519; first-time setup:
...
Environment:
  ANI_VM_BRANCH          branch for update (default: current local branch)
  ANI_VM_MIN_FREE_GB     preflight free-disk floor (default 14, runner spec)
  ANI_VM_SKIP_PREFLIGHT=1  bootstrap-only escape (checkout predates the script)
...
CONFIG="${HOME}/.claude/ani-vm/config"
KEY="${HOME}/.claude/ani-vm/id_ed25519"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# The fixed host-local config is intentionally outside this repository.
# shellcheck source=/dev/null
[ -f "$CONFIG" ] && . "$CONFIG"
HOST="${ANI_VM_HOST:?set ANI_VM_HOST in ~/.claude/ani-vm/config}"
USER_="${ANI_VM_USER:-test}"

# ── Per-run checkout (the VM's last shared-state hazard) ─────────────────────
# The VM holds ONE working tree at /c/ani, and `update`/`sync` REPLACE it
...
#
# Setting ANI_CI_RUN_ID gives this run its own checkout, cloned from /c/ani.
...
VM_BASE_REPO="/c/ani"
if [ -n "${ANI_CI_RUN_ID:-}" ]; then
    if [[ ! "$ANI_CI_RUN_ID" =~ ^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$ ]]; then
        echo "FATAL: ANI_CI_RUN_ID must match [A-Za-z0-9][A-Za-z0-9._-]{0,63}." >&2
        exit 2
    fi
    VM_REPO="/c/ani-run-${ANI_CI_RUN_ID}"
else
    VM_REPO="$VM_BASE_REPO"
fi
VM_REPO_WIN="C:\\${VM_REPO#/c/}"
HOST_KEY="${ANI_VM_HOST_KEY_SHA256:?set ANI_VM_HOST_KEY_SHA256 in ~/.claude/ani-vm/config}"
LOCAL_BRANCH="$(git -C "$ROOT" branch --show-current)"
BRANCH="${ANI_VM_BRANCH:-${LOCAL_BRANCH:-main}}"
...
ani_vm_require_safe_branch "$BRANCH"
ani_vm_prepare_known_hosts "$HOST" "$HOST_KEY"
WIN_MANIFEST=""
WIN_ARCHIVE=""
WIN_PATCH=""
win_cleanup() {
    ...
    ani_vm_cleanup_known_hosts
}
trap win_cleanup EXIT
SSH_OPTIONS=(-i "$KEY" -o IdentitiesOnly=yes -o HostKeyAlgorithms=ssh-ed25519 \
             -o StrictHostKeyChecking=yes -o UserKnownHostsFile="$ANI_VM_KNOWN_HOSTS" \
...
vm_assert_head() {
    ...
    if [ "$got" != "$want" ]; then
        ...
        echo "       Re-run with ANI_CI_RUN_ID set to get an isolated tree." >&2
        return 1
    fi
}
...
# ANI_VM_SKIP_PREFLIGHT=1 exists for ONE case: bootstrapping a VM whose
...
vm_preflight() {
    [ "${ANI_VM_SKIP_PREFLIGHT:-0}" = "1" ] && {
        echo "=== win.sh: preflight SKIPPED (ANI_VM_SKIP_PREFLIGHT=1) ==="
        return 0
    }
    vm_cmd "powershell -NoProfile -ExecutionPolicy Bypass -File \
${VM_REPO_WIN}\\scripts\\ci\\clean-test-residue.ps1 -MinFreeGB ${ANI_VM_MIN_FREE_GB:-14}"
...
status)
    ...
    vm clangarm64 "cd $VM_REPO 2>/dev/null && git log --oneline -1 && ls -la build/c/ani.exe build/c/test-runner.exe 2>/dev/null || echo 'repo/build missing — run provision-windows.sh'"
    ;;
update)
...
sync)
    ...
    WIN_MANIFEST="$(mktemp "${TMPDIR:-/tmp}/ani-vm-manifest.XXXXXX")"
    WIN_ARCHIVE="$(mktemp "${TMPDIR:-/tmp}/ani-vm-worktree.XXXXXX.tar")"
    WIN_PATCH="$(mktemp "${TMPDIR:-/tmp}/ani-vm-worktree.XXXXXX.patch")"
    git -C "$ROOT" diff --binary --full-index HEAD -- >"$WIN_PATCH"
    ani_vm_write_untracked_manifest "$ROOT" "$WIN_MANIFEST"
...
test)
    ...
    # ANI_VM_EXPECT_HEAD=<sha> asserts the tree is the one the caller means
    # before spending a leg on it — cheap insurance against a concurrent
    # session having re-synced the shared checkout.
    [ -z "${ANI_VM_EXPECT_HEAD:-}" ] || vm_assert_head "$ANI_VM_EXPECT_HEAD"
...
guards)
    ...
    vm_cmd "cd /d ${VM_REPO_WIN} && set PATH=C:\\msys64\\clangarm64\\bin;C:\\msys64\\usr\\bin;%PATH%&& powershell -NoProfile -ExecutionPolicy Bypass -File scripts\\test-windows.ps1 -GuardsOnly -Binary build\\guards\\ani.exe -Make C:\\msys64\\usr\\bin\\make.exe"
    ;;
smoke-install)
...
soak)
    ...
    vm clangarm64 "cd $VM_REPO && ANI_VM_TEST_LOG=/tmp/win-soak.log bash \
...
ubsan-build)
    ...
    vm clang64 "cd $VM_REPO && make -j${JOBS} -f Makefile.ani CC=clang CXX=clang++ SANITIZE='-fsanitize=undefined -fno-omit-frame-pointer' build/c/test-runner > /tmp/win-ubsan-build.log 2>&1 && echo UBSAN_BUILD_OK || (echo UBSAN_BUILD_FAIL; tail -20 /tmp/win-ubsan-build.log; exit 1)"
    ;;
ubsan-test)
    [ $# -ge 1 ] || { echo "usage: win.sh ubsan-test <suite...>" >&2; exit 2; }
    vm clang64 "cd $VM_REPO && ANI_VM_TEST_LOG=/tmp/win-ubsan-test.log bash test-infrastructure/vm/vm-run-tests.sh $*"
    ;;
trap-ubsan-build)
...
trap-ubsan-test)
    ...
    vm clangarm64 "cd $VM_REPO && ANI_VM_RUNNER=build/trap-ubsan/test-runner ANI_VM_TEST_LOG=/tmp/win-trap-ubsan-test.log bash test-infrastructure/vm/vm-run-tests.sh $*"
    ;;
pageheap)
...
test-par)
    ...
    vm clangarm64 "cd $VM_REPO && ANI_VM_TEST_LOG=/tmp/win-test-par.log bash test-infrastructure/vm/vm-run-tests.sh --par"
    ;;
help | -h | --help)
    print_help
    ;;
