...
Host-local config (never committed): `~/.claude/ani-vm/config` with
`ANI_VM_HOST=<ip>`, `ANI_VM_USER=<user>`, and the
`ANI_VM_HOST_KEY_SHA256=<SHA256:...>` fingerprint printed by the bootstrap.
`ANI_VM_BRANCH=<branch>` is optional; both drivers otherwise use the current
...
`~/.claude/ani-vm/id_ed25519` (generate: `ssh-keygen -t ed25519 -N "" -C
claude-ani-vm -f ~/.claude/ani-vm/id_ed25519`; include the public half beside
...
7. Get `windows-bootstrap.ps1` into the VM via the ISO trick in its header
   ...
   run it, note the printed ip/user, write `~/.claude/ani-vm/config`,
...
vm/win.sh sh "cd /c/ani && gdb ..."   # anything, interactively
vm/win.sh push-file src/cli/cli.c /c/ani/src/cli/cli.c   # WIP iteration
```
