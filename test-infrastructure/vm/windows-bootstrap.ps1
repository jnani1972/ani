...
#   mkdir /tmp/aniiso
#   cp windows-bootstrap.ps1 /tmp/aniiso/
#   cp ~/.claude/ani-vm/id_ed25519.pub /tmp/aniiso/host-key.pub
#   printf '@echo off\r\npowershell -NoProfile -ExecutionPolicy Bypass -File "%%~dp0windows-bootstrap.ps1" -SshPublicKeyPath "%%~dp0host-key.pub"\r\n' > /tmp/aniiso/RUN-ME.bat
#   hdiutil makehybrid -iso -joliet -default-volume-name ANISETUP -o ~/ani-setup.iso /tmp/aniiso
...
New-NetFirewallRule -Name ani-sshd -DisplayName "OpenSSH (ani test VM)" `
...
Write-Host "On the host, write ~/.claude/ani-vm/config with:"
Write-Host "  ANI_VM_HOST=$ip"
Write-Host ("  ANI_VM_USER=" + $env:USERNAME)
Write-Host "  ANI_VM_HOST_KEY_SHA256=<copy the SHA256:... value above>"
