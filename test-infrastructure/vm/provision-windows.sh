...
#   2. Host config in ~/.claude/ani-vm/config (KEY=VALUE), values printed by
#      windows-bootstrap.ps1 at the end of its run:
#        ANI_VM_HOST=<vm ip>
#        ANI_VM_USER=<vm user>
#        ANI_VM_HOST_KEY_SHA256=<SHA256:... host-key fingerprint>
#      Key at ~/.claude/ani-vm/id_ed25519 (see README.md to generate your own;
...
REPO_URL="${ANI_VM_REPO:-https://github.com/jnani1972/ani.git}"
...
trap ani_vm_cleanup_known_hosts EXIT
...
vm_powershell() { ani_vm_run_powershell "$1" "${SSH[@]}"; }
...
step "4/6 repo clone/update -> /c/ani @ ${BRANCH}"
if vm "clangarm64" "test -d /c/ani/.git && echo REPO_PRESENT" | grep -q REPO_PRESENT; then
    vm "clangarm64" "cd /c/ani && git fetch origin ${BRANCH} && git reset --hard FETCH_HEAD && git clean -fdx && git log --oneline -1"
else
    vm "clangarm64" "git clone --branch ${BRANCH} --single-branch --depth 200 ${REPO_URL} /c/ani && cd /c/ani && git log --oneline -1"
fi

step "5/6 build: native ARM64 binary + test-runner (no ASan on arm64)"
vm "clangarm64" "cd /c/ani && make -j${JOBS} -f Makefile.ani CC=clang CXX=clang++ SANITIZE= ani build/c/test-runner > /tmp/provision-build.log 2>&1 && echo BUILD_OK || (echo BUILD_FAIL; tail -15 /tmp/provision-build.log; exit 1)"

step "6/6 smoke: binary + test-runner start"
vm "clangarm64" "cd /c/ani && ./build/c/ani.exe --version && ./build/c/test-runner --list-suites | head -3"
