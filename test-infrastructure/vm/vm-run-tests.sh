...
Environment:
  ANI_VM_RUNNER      sanitizer-variant runner path (ubsan/trap-ubsan builds);
                     switches to direct-runner mode for those iteration tools.
  ANI_VM_SOAK_BINARY product binary for --soak (default build/c/ani.exe)
  ANI_VM_TEST_LOG    VM-side log path (default /tmp/win-test.log)
...
RUNNER="${ANI_VM_RUNNER:-}"

# Per-run identity. The VM holds ONE checkout (/c/ani), so two concurrent runs
...
CALLER_RUN_ID="${ANI_CI_RUN_ID:-}"
RUN_ID="${ANI_CI_RUN_ID:-$$-$(date +%s)}"
export ANI_CI_RUN_ID="$RUN_ID"
LOG="${ANI_VM_TEST_LOG:-/tmp/win-test-${RUN_ID}.log}"

# A caller that sets ANI_CI_RUN_ID is declaring concurrency, so give that run
...
if [ "$1" = "--soak" ]; then
    ...
    binary="${ANI_VM_SOAK_BINARY:-build/c/ani.exe}"
    [ -x "$binary" ] || { echo "ERROR: binary '$binary' missing — build first" >&2; exit 2; }
    artifact="$binary"
elif [ -n "$RUNNER" ]; then
    # Explicit ANI_VM_RUNNER = the sanitizer-variant iteration mode (ubsan /
...
# profile carry no Authenticated-Users ACE, but C:\ani (like CI's workspace
...
# Only an explicit ANI_VM_RUNNER (sanitizer-variant builds) bypasses test.sh.
if [ -n "$RUNNER" ]; then
    if [ "$1" = "--par" ]; then
...
if [ -n "$CALLER_RUN_ID" ] && [ "${ANI_CI_KEEP:-0}" != "1" ]; then
    if [ "${rc:-1}" -eq 0 ]; then
