...
# reason to throttle it. The ANI_LOCAL_CI_CPUS knob survives only for the
...
#
#   ANI_CI_RUN_ID=<id>       name/reuse a run (default: pid + epoch, unique)
#   ANI_CI_SHARED_BUILD=1    opt back into the single shared `ani-build` volume
#   ANI_CI_KEEP=1            keep this run's build volume even on success
RUN_ID="${ANI_CI_RUN_ID:-$$-$(date +%s)}"
if [ "${ANI_CI_SHARED_BUILD:-0}" = "1" ]; then
    ANI_CI_BUILD_VOLUME="ani-build"
else
    ANI_CI_BUILD_VOLUME="ani-build-${RUN_ID}"
fi
export ANI_CI_BUILD_VOLUME
...
ci_cleanup() {
    local rc=$?
    if [ "${ANI_CI_BUILD_VOLUME}" = "ani-build" ] || [ "${ANI_CI_KEEP:-0}" = "1" ]; then
        return $rc
    fi
    if [ $rc -eq 0 ]; then
        docker volume rm -f "${ANI_CI_BUILD_VOLUME}" >/dev/null 2>&1 || true
    else
        echo "run.sh: kept ${ANI_CI_BUILD_VOLUME} for post-mortem (run failed)" >&2
        echo "        inspect: docker run --rm -v ${ANI_CI_BUILD_VOLUME}:/b alpine ls -R /b" >&2
        echo "        drop:    docker volume rm ${ANI_CI_BUILD_VOLUME}" >&2
    fi
    return $rc
}
...
first — ANI_SKIP_PREFLIGHT=1 skips it, ANI_PREFLIGHT_ARGS=--deep widens it.
...
Legs:
  ...
  perf            arm64 incremental-perf leg (ANI_SKIP_PERF unset)
  ...
  soak-linux      both CI soak legs (quick + #581 query-leak) via
                  scripts/soak-legs.sh; ANI_SOAK_MINUTES per leg (default 10)
  soak-windows    delegates to the real-Windows VM (win.sh soak 10)

Environment:
  ANI_SOAK_MINUTES     soak-linux duration per leg (default 10)
  ANI_LOCAL_CI_CPUS    opt-in CPU cap for smoke/soak starvation checks only
                       (the regular suite always runs full-parallel)
  ANI_SKIP_PREFLIGHT=1 / ANI_PREFLIGHT_ARGS=--deep   preflight controls
...
# product failure deep inside an install path. ANI_PREFLIGHT_ARGS=--deep also
# reclaims the build cache; ANI_SKIP_PREFLIGHT=1 opts out for a quick re-run.
if [ "${ANI_SKIP_PREFLIGHT:-0}" != "1" ]; then
    # shellcheck disable=SC2086  # deliberate word-splitting of opt-in flags
    "$ROOT/scripts/ci/preflight-docker.sh" ${ANI_PREFLIGHT_ARGS:-}
fi

case "${1:-full}" in
    full)
        echo "=== Linux arm64: test + build ==="
        $COMPOSE run --rm -e ANI_SKIP_PERF=1 test
    ...
    smoke)
        ...
        ;;
    soak-linux)
        # Both CI legs (quick + #581 query-leak). Duration via ANI_SOAK_MINUTES,
        ...
        # the fast ladder.
        echo "=== Linux arm64: soak (quick + query-leak, ${ANI_SOAK_MINUTES:-10}m each) ==="
        $COMPOSE run --rm soak
        ;;
    portable)
        echo "=== Linux portable: Alpine static build + smoke ==="
    ...
    portable-test)
        ...
        $COMPOSE run --rm -e ANI_SKIP_PERF=1 test-portable
        ;;
    windows)
    ...
    amd64 | test-amd64)
        ...
        $COMPOSE run --rm -e ANI_SKIP_PERF=1 test-amd64
