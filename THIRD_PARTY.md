...
The tree-sitter C runtime is vendored in `internal/ani/vendored/ts_runtime/`.

- **Project:** [tree-sitter](https://github.com/tree-sitter/tree-sitter)
- **License:** MIT
- **Copyright:** (c) 2018–2024 Max Brunsfeld

**Local modification** (`internal/ani/vendored/ts_runtime/src/stack.c`, #913): a
single ANI patch bounds the recursive ambiguity-merge in `stack_node_add_link`
at `ANI_TS_STACK_MERGE_MAX_DEPTH` (512). Deeply nested grammar-ambiguous input
...
`MAX_LINK_COUNT` bail-out. The change is clearly marked `// ANI patch:` inline.
**On re-vendor (e.g. ts_runtime → 0.26.x): re-apply this bound.**

The shared scanner helpers in `internal/ani/vendored/common/` (`scanner.h`,
...
`internal/ani/vendored/common/`.

The core runtime headers in `internal/ani/vendored/common/tree_sitter/`
...
160 pre-generated parsers are vendored in `internal/ani/vendored/grammars/<lang>/`
...
[`internal/ani/vendored/grammars/MANIFEST.md`](internal/ani/vendored/grammars/MANIFEST.md).
...
- The grammars authored in-house for this project (`chialisp`, `cobol`, `form`,
  ...
  license, (c) the project authors. Each ships the repository's own LICENSE, byte-identical
...
- Seven further grammars (`arkts`, `assembly`, `cfml`,
  ...
  base, MIT, (c) 2014 Max Brunsfeld) with (c) 2026 ani project contributors' ArkTS additions;
...
- **Vendored at:** `internal/ani/vendored/grammars/plsql/`
...
- **Notes:** Community-maintained grammar for Oracle PL/SQL; not in the
  ...
  `internal/ani/vendored/grammars/MANIFEST.md`. PL/SQL support was originally
...
- **Vendored at:** `internal/ani/vendored/grammars/objectscript_udl/`, `internal/ani/vendored/grammars/objectscript_routine/`
- **Pinned commit:** `a7ffcdf`
- **Notes:** InterSystems-maintained grammar for the ObjectScript language (InterSystems IRIS / Caché). Vendor-maintained; not in nvim-treesitter or Helix registries. Each `scanner.c`'s upstream `#include "../../common/scanner.h"` is repointed to a per-directory `objectscript_common.h` copied from upstream `common/scanner.h`; two loop counters in that copy are widened from `uint8_t` to `int` as documented in `internal/ani/vendored/grammars/MANIFEST.md`.
...
| LZ4 | `internal/ani/vendored/lz4/` | BSD-2-Clause (library files) | [lz4/lz4](https://github.com/lz4/lz4) |
| Zstandard | `internal/ani/vendored/zstd/` | BSD-3-Clause (dual BSD / GPLv2 — BSD selected) | [facebook/zstd](https://github.com/facebook/zstd) |
| simplecpp | `internal/ani/vendored/simplecpp/` | 0BSD | [danmar/simplecpp](https://github.com/danmar/simplecpp) |
| Verstable | `internal/ani/vendored/verstable/` | MIT | [JacksonAllan/Verstable](https://github.com/JacksonAllan/Verstable) |
| wyhash | `internal/ani/vendored/wyhash/` | Unlicense (public domain) | [wangyi-fudan/wyhash](https://github.com/wangyi-fudan/wyhash) |
...
ANI's 4 KiB path support). Patches must be reapplied on every upstream
...
The Hybrid LSP layer (`internal/ani/lsp/`) is an original C implementation
...
The stdlib type registries in `internal/ani/lsp/generated/` were produced as
follows:
