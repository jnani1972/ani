/*
 * application.c — Daemon-owned ANI application sessions and thin-client wire.
 */
...
enum {
    ...
    APPLICATION_UI_READINESS_REQUEST_SIZE = 1 + ANI_SHA256_DIGEST_LEN,
...
typedef struct ani_daemon_application_watch ani_daemon_application_watch_t;
typedef struct ani_daemon_application_session ani_daemon_application_session_t;
typedef struct ani_daemon_application_job ani_daemon_application_job_t;
typedef struct ani_daemon_application_mutation ani_daemon_application_mutation_t;
typedef struct ani_daemon_application_watch_job_subscription
    ani_daemon_application_watch_job_subscription_t;
...
struct ani_daemon_application_watch {
    char *project;
    char *root;
    size_t subscribers;
    ani_daemon_application_watch_t *next;
};

struct ani_daemon_application_session {
    ani_daemon_application_t *application;
    ani_mcp_server_t *mcp;
    ...
    ani_daemon_runtime_application_token_t request_cancel_token;
    ani_daemon_application_watch_t *watch;
    ani_daemon_application_job_t *active_job;
    bool active_job_subscribed;
    ani_daemon_application_job_t *auto_index_job;
    ...
    ani_daemon_application_session_t *next;
};

struct ani_daemon_application_job {
    ani_daemon_application_t *application;
    char *project_key;
    char *root_path;
    char *args_json;
    char *response;
    ani_daemon_application_worker_t worker;
    ani_thread_t thread;
    size_t subscribers;
    ...
    ani_daemon_application_job_t *next;
};
...
struct ani_daemon_application_watch_job_subscription {
    ani_daemon_application_session_t *session;
    ani_daemon_application_job_t *job;
    ani_daemon_application_watch_job_subscription_t *next;
};

struct ani_daemon_application_mutation {
    char *project_key;
    ani_project_lock_lease_t *project_lock_lease;
    bool releasing;
    ani_daemon_application_mutation_t *next;
};

struct ani_daemon_application {
    ani_mutex_t mutex;
    struct ani_watcher *watcher;
    struct ani_config *config;
    ani_daemon_application_session_t *sessions;
    ani_daemon_application_watch_t *watches;
    ani_daemon_application_job_t *jobs;
    ani_daemon_application_watch_job_subscription_t *watch_job_subscriptions;
    ani_daemon_application_mutation_t *mutations;
    ani_daemon_application_worker_ops_t worker_ops;
    ani_daemon_application_update_ops_t update_ops;
    ...
    ani_daemon_application_update_worker_t update_worker;
    ani_thread_t update_thread;
    ...
    bool stopping;
    /* See ani_daemon_application_set_permanent. */
    bool permanent;
    uint8_t ui_readiness_secret[ANI_SHA256_DIGEST_LEN];
    bool ui_readiness_secret_set;
};

static void application_job_unsubscribe_locked(ani_daemon_application_job_t *job);
...
static bool application_watch_job_subscribe_late_session_locked(
    ani_daemon_application_session_t *session, ani_daemon_application_watch_t *watch);
static bool application_unique_recovery_file(char out[APPLICATION_PATH_CAP], const char *kind);
static bool application_update_reap(ani_daemon_application_t *application, bool wait,
                                    uint32_t timeout_ms);
static void *application_job_thread(void *opaque);
static char *application_auto_index_args(const char *root_path);
static ani_daemon_application_job_t *application_job_subscribe_locked(
    ani_daemon_application_t *application, const char *project_key, const char *root_path,
...
void ani_daemon_application_fail_next_job_thread_start_for_test(void) {
...
int ani_daemon_application_background_initializes_for_test(void) {
...
int ani_daemon_application_busy_queue_waits_for_test(void) {
    return atomic_load_explicit(&g_application_busy_queue_waits_for_test, memory_order_acquire);
}

void ani_daemon_application_hold_job_before_start_for_test(bool hold) {
    atomic_store_explicit(&g_application_hold_job_before_start_for_test, hold,
                          memory_order_release);
}

static int application_job_thread_create(ani_thread_t *thread, void *context) {
    if (atomic_exchange_explicit(&g_application_fail_next_job_thread_start_for_test, false,
                                 memory_order_acq_rel)) {
        return -1;
    }
    return ani_thread_create(thread, APPLICATION_JOB_THREAD_STACK, application_job_thread, context);
}

static bool application_request_cancelled_locked(const ani_daemon_application_session_t *session) {
    return session &&
           (session->session_cancelled ||
            (session->request_active &&
             session->active_request_token != ANI_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID &&
...
static void application_project_lock_release_fully(ani_project_lock_lease_t **lease) {
...
static int application_worker_start_default(void *context, const char *args_json,
    ...
    (void)context;
    ani_index_worker_handle_t *worker = NULL;
    int result = ani_index_worker_start(args_json, memory_budget_bytes, false, marker_file,
                                        quarantine_file, &worker);
    *worker_out = worker;
    return result;
}

static ani_index_worker_poll_t application_worker_poll_default(
    void *context, ani_daemon_application_worker_t worker,
    const ani_index_worker_result_t **result_out) {
    (void)context;
    return ani_index_worker_poll((ani_index_worker_handle_t *)worker, result_out);
}

static bool application_worker_cancel_default(void *context,
                                              ani_daemon_application_worker_t worker) {
    (void)context;
    return ani_index_worker_request_cancel((ani_index_worker_handle_t *)worker);
}

static const char *application_worker_log_path_default(void *context,
                                                       ani_daemon_application_worker_t worker) {
    (void)context;
    return ani_index_worker_log_path((ani_index_worker_handle_t *)worker);
}

static void application_worker_destroy_default(void *context,
                                               ani_daemon_application_worker_t worker) {
    (void)context;
    ani_index_worker_destroy((ani_index_worker_handle_t *)worker);
}
...
static bool application_canonical_directory_exists(const char *path) {
    ani_path_info_t info = {0};
    return ani_path_info_utf8(path, &info) == 0 && info.is_directory;
}

static ani_daemon_application_watch_t *application_find_watch_locked(
    ani_daemon_application_t *application, const char *project) {
    for (ani_daemon_application_watch_t *watch = application->watches; watch; watch = watch->next) {
...
static void application_remove_watch_entry_locked(ani_daemon_application_t *application,
                                                  ani_daemon_application_watch_t *watch,
                                                  bool unregister_physical_watch) {
    ani_daemon_application_watch_t **cursor = &application->watches;
    ...
    *cursor = watch->next;
    for (ani_daemon_application_session_t *session = application->sessions; session;
    ...
    if (unregister_physical_watch && application->watcher) {
        ani_watcher_unwatch(application->watcher, watch->project);
    }
    free(watch->project);
    free(watch->root);
    free(watch);
...
static void application_remove_watch_locked(ani_daemon_application_t *application,
                                            ani_daemon_application_watch_t *watch) {
    application_remove_watch_entry_locked(application, watch, true);
}

static void application_release_session_watch_locked(ani_daemon_application_session_t *session) {
    ani_daemon_application_watch_t *watch = session->watch;
    if (!watch) {
        return;
    }
    application_watch_job_unsubscribe_session_locked(session);
...
static bool application_session_workspace_allowed(const ani_daemon_application_session_t *session,
                                                  const char *operation) {
    const char *root = session ? ani_mcp_server_session_root(session->mcp) : NULL;
    char boundary_error[ANI_SZ_1K];
    bool allowed =
        root && root[0] &&
        ani_workspace_root_allowed(root, ani_workspace_home_dir(), ani_workspace_cache_dir(),
                                   ani_mcp_server_allowed_root(session->mcp), boundary_error,
                                   sizeof(boundary_error));
    if (!allowed) {
        ani_log_warn("daemon.workspace.skipped", "operation", operation, "detail",
...
static void application_refresh_watch_locked(ani_daemon_application_session_t *session) {
    ani_daemon_application_t *application = session->application;
    if (!application->watcher || !session->context_set ||
        session->tool_profile != ANI_MCP_TOOL_PROFILE_ALL || session->hook_event ||
        session->hook_dialect || session->auto_index_subscribed) {
        return;
    }
    const char *project = ani_mcp_server_session_project(session->mcp);
    const char *root = ani_mcp_server_session_root(session->mcp);
    ...
    bool enabled = !application->config ||
                   ani_config_get_bool(application->config, ANI_CONFIG_AUTO_WATCH, true);
    ...
    ani_daemon_application_watch_t *watch = application_find_watch_locked(application, project);
    ...
    if (watch) {
        if (strcmp(watch->root, root) == 0) {
            if (!application_watch_job_subscribe_late_session_locked(session, watch)) {
                ani_log_warn("daemon.watch.late_owner_allocation_failed", "project", project,
                             "action", "retry");
                return;
            }
            watch->subscribers++;
            ...
            ani_log_warn("daemon.watch.project_collision", "project", project, "existing_root",
    ...
    if (!ani_watcher_watch(application->watcher, project, root)) {
        ani_log_warn("daemon.watch.registration_failed", "project", project, "action", "retry");
        free(watch->project);
        free(watch->root);
        free(watch);
        return;
...
static void application_refresh_watch(ani_daemon_application_session_t *session) {
    if (!session || !session->application) {
        return;
    }
    ani_mutex_lock(&session->application->mutex);
    application_refresh_watch_locked(session);
    ani_mutex_unlock(&session->application->mutex);
}

static void application_job_free(ani_daemon_application_job_t *job) {
    if (!job) {
        return;
    }
...
static void application_jobs_reap_completed(ani_daemon_application_t *application) {
    for (;;) {
        ani_daemon_application_job_t *reap = NULL;
        ani_mutex_lock(&application->mutex);
        ani_daemon_application_job_t **cursor = &application->jobs;
        while (*cursor) {
        ...
        ani_mutex_unlock(&application->mutex);
        if (!reap) {
            return;
        }
        if (reap->thread_started) {
            (void)ani_thread_join(&reap->thread);
        }
...
static bool application_cache_dir(char out[APPLICATION_PATH_CAP]) {
    ...
    int written = snprintf(out, APPLICATION_PATH_CAP, "%s/.cache/ani", home);
    if (written <= 0 || written >= APPLICATION_PATH_CAP) {
        return false;
    }
    ani_normalize_path_sep(out);
...
static void application_recovery_files_remove(const char *marker_path,
    ...
    if (quarantine_path && quarantine_path[0]) {
        (void)ani_unlink(quarantine_path);
    }
}

static bool application_truncate_file(const char *path) {
    FILE *file = ani_fopen(path, "wb");
    return file && fclose(file) == 0;
}

static bool application_job_cancel_requested(ani_daemon_application_job_t *job);

static char **application_read_suspects(ani_daemon_application_job_t *job, const char *path,
                                        int *count_out, bool *cancelled_out) {
    *count_out = 0;
    *cancelled_out = false;
    int64_t marker_size = ani_file_size(path);
...
static int application_max_restarts(void) {
    char value[32] = {0};
    if (!ani_safe_getenv("ANI_INDEX_MAX_RESTARTS", value, sizeof(value), NULL) || !value[0]) {
...
static void application_attempt_init(application_attempt_t *attempt) {
    ...
    attempt->result.outcome = ANI_PROC_SPAWN_FAILED;
...
static bool application_job_cancel_requested(ani_daemon_application_job_t *job) {
    ani_daemon_application_t *application = job->application;
    ani_mutex_lock(&application->mutex);
...
static bool application_mutation_conflicts_locked(ani_daemon_application_t *application,
                                                  const char *project_key) {
    for (ani_daemon_application_mutation_t *mutation = application->mutations; mutation;
...
static bool application_job_reserves_project_locked(ani_daemon_application_t *application,
                                                    const char *project_key) {
    for (ani_daemon_application_job_t *job = application->jobs; job; job = job->next) {
...
static bool application_mutation_begin_internal(ani_daemon_application_t *application,
                                                ani_daemon_application_session_t *session,
                                                const char *project_key, bool wait) {
    if (!application || !project_key || !project_key[0]) {
        return false;
    }
    ani_daemon_application_mutation_t *reserved = NULL;
    for (;;) {
        ani_mutex_lock(&application->mutex);
        bool cancelled =
        ...
        if (!cancelled && !busy) {
            ani_daemon_application_mutation_t *mutation = calloc(1, sizeof(*mutation));
            if (mutation) {
                mutation->project_key = ani_strdup(project_key);
            }
        ...
        ani_usleep(APPLICATION_JOB_POLL_US);
    }
    ...
    for (;;) {
        ...
        ani_private_file_lock_status_t status =
            wait ? ani_project_lock_acquire(application->project_locks, project_key, deadline, NULL,
                                            &lease)
                 : ani_project_lock_try_acquire(application->project_locks, project_key, &lease);
        if (status == ANI_PRIVATE_FILE_LOCK_OK && lease) {
            ...
            if (cancelled) {
                ...
                status = ANI_PRIVATE_FILE_LOCK_BUSY;
            } else {
        ...
        bool cancelled =
            application->stopping || (session && application_request_cancelled_locked(session));
        if (status != ANI_PRIVATE_FILE_LOCK_BUSY || cancelled || !wait) {
            ani_daemon_application_mutation_t **cursor = &application->mutations;
            ...
            if (status != ANI_PRIVATE_FILE_LOCK_BUSY) {
                ani_log_error("daemon.project_lock_failed", "project", project_key, "action",
                              "refuse_mutation");
            }
            return false;
        }
...
bool ani_daemon_application_project_mutation_try_begin(ani_daemon_application_t *application,
                                                       const char *project) {
    return application_mutation_begin_internal(application, NULL, project, false);
}

void ani_daemon_application_project_mutation_end(ani_daemon_application_t *application,
                                                 const char *project) {
    if (!application || !project || !project[0]) {
        return;
    }
    ani_mutex_lock(&application->mutex);
    ani_daemon_application_mutation_t *mutation = application->mutations;
...
static bool application_watcher_mutation_begin(void *context, const char *project) {
    return ani_daemon_application_project_mutation_try_begin(context, project);
}

static void application_watcher_mutation_end(void *context, const char *project) {
    ani_daemon_application_project_mutation_end(context, project);
}

static void application_watcher_project_pruned(void *context, const char *project) {
    ani_daemon_application_t *application = context;
...
static bool application_session_mutation_begin(void *context, const char *project) {
    ani_daemon_application_session_t *session = context;
    return session &&
...
static void application_session_mutation_end(void *context, const char *project) {
    ani_daemon_application_session_t *session = context;
    if (session) {
        ani_daemon_application_project_mutation_end(session->application, project);
    }
}

static bool application_job_wait_for_mutations(ani_daemon_application_job_t *job) {
    ani_daemon_application_t *application = job->application;
    for (;;) {
        ani_mutex_lock(&application->mutex);
        bool cancelled = job->cancel_requested || application->stopping;
...
static void application_cancelled_result(ani_index_worker_result_t *result) {
    memset(result, 0, sizeof(*result));
    result->outcome = ANI_PROC_KILLED;
    result->exit_code = -1;
    result->cancellation_requested = true;
    result->tree_quiesced = true;
}

static application_attempt_status_t application_job_run_attempt(ani_daemon_application_job_t *job,
    ...
    ani_daemon_application_worker_t worker = NULL;
    application_tmp_lock();
    ...
    const ani_index_worker_result_t *borrowed = NULL;
    for (;;) {
        ani_index_worker_poll_t state =
        ...
        ani_mutex_unlock(&application->mutex);
        if (cancel_pending || state == ANI_INDEX_WORKER_POLL_ERROR) {
            (void)application->worker_ops.cancel(application->worker_ops.context, worker);
        }
        ani_usleep(APPLICATION_JOB_POLL_US);
    }
    ...
    if (borrowed) {
        ...
        attempt->result.response = borrowed->response ? ani_strdup(borrowed->response) : NULL;
...
static char *application_job_failure_response(const ani_index_worker_result_t *result,
                                              const char *log_path) {
    char message[1024];
    if (result && result->cancellation_requested) {
        ...
        (void)snprintf(message, sizeof(message),
                       ...
                       ani_proc_outcome_str(result->outcome), log_path ? log_path : "unavailable");
    } else if (result) {
        (void)snprintf(message, sizeof(message),
                       "index worker ended with %s (exit=%d, signal=%d); inspect log: %s",
                       ani_proc_outcome_str(result->outcome), result->exit_code,
...
static ani_mcp_supervised_result_disposition_t application_attempt_disposition(
    ani_daemon_application_job_t *job, application_attempt_t *attempt) {
    if (application_job_cancel_requested(job)) {
        if (!attempt->has_result) {
            application_cancelled_result(&attempt->result);
            attempt->has_result = true;
    ...
    return ani_mcp_supervised_result_disposition(0, attempt->has_result ? &attempt->result : NULL);
}

static void application_record_attempt(const application_attempt_t *attempt,
                                       ani_index_worker_result_t *last_result,
...
static void application_record_cancelled(ani_index_worker_result_t *last_result,
                                         bool *have_last_result,
                                         char last_log[APPLICATION_PATH_CAP]) {
...
static bool application_result_is_attributable_failure(
    const application_attempt_t *attempt, ani_mcp_supervised_result_disposition_t disposition) {
    return disposition == ANI_MCP_SUPERVISED_RESULT_CONTAINED_FAILURE && attempt->has_result &&
           (attempt->result.outcome == ANI_PROC_CRASH || attempt->result.outcome == ANI_PROC_HANG);
}
...
static application_attempt_decision_t application_consume_attempt(
    ani_daemon_application_job_t *job, application_attempt_t *attempt,
    application_job_execution_t *execution, ani_proc_outcome_t *failure_outcome) {
...
static bool application_recovery_record_suspects(
    ani_daemon_application_job_t *job, const char *marker_path, const char *quarantine_path,
    ani_proc_outcome_t outcome, int recovery_index, char ***previous_suspects, int *previous_count,
    ...
    if (!suspects || suspect_count == 0) {
        application_free_suspects(suspects, suspect_count);
        ani_log_warn("daemon.index.recovery_unattributable", "action", "stop");
        return false;
    }
    if (*previous_suspects) {
        ...
        const char *phase = outcome == ANI_PROC_HANG ? "hang" : "crash";
        if (!application_append_quarantine(quarantine_path, pick, phase)) {
            ani_log_warn("daemon.index.quarantine_write_fail", "path", pick);
            application_free_suspects(suspects, suspect_count);
            return false;
        }
        (*quarantined)++;
        ...
        ani_log_warn("daemon.index.file_quarantined", "path", pick, "outcome", phase, "attempt",
                     attempt_text);
    }
...
static void application_job_try_partial(ani_daemon_application_job_t *job,
    ...
    ani_proc_outcome_t failure_outcome = ANI_PROC_SPAWN_FAILED;
    ...
    if (decision == APPLICATION_ATTEMPT_DECISION_SUCCESS) {
        ...
        ani_log_warn("daemon.index.recovery_partial", "quarantined", quarantined_text);
    }
}

static void application_job_recover(ani_daemon_application_job_t *job,
                                    application_job_execution_t *execution) {
    if (application_job_cancel_requested(job)) {
        application_job_execution_cancel(execution);
        return;
...
static void application_auto_index_retry_pending_locked(ani_daemon_application_t *application) {
    ...
    for (ani_daemon_application_session_t *session = application->sessions; session;
        ...
        const char *project = ani_mcp_server_session_project(session->mcp);
        const char *root_path = ani_mcp_server_session_root(session->mcp);
        ...
        ani_daemon_application_job_t *retry = application_job_subscribe_locked(
        ...
        if (retry) {
            ...
            session->auto_index_retry_pending = false;
            ani_log_info("daemon.autoindex.admission_retried", "project", project);
            continue;
        }
...
static void application_job_publish(ani_daemon_application_job_t *job,
                                    application_job_execution_t *execution) {
...
static void *application_job_thread(void *opaque) {
    ani_daemon_application_job_t *job = opaque;
    application_job_execution_t execution;
...
static ani_daemon_application_job_t *application_find_job_locked(
    ani_daemon_application_t *application, const char *project_key) {
    for (ani_daemon_application_job_t *job = application->jobs; job; job = job->next) {
...
static ani_daemon_application_job_t *application_find_active_job_locked(
    ani_daemon_application_t *application, const char *project_key) {
    for (ani_daemon_application_job_t *job = application->jobs; job; job = job->next) {
...
static char *application_index_project_key(const char *root_path, const char *args_json) {
    char *override = ani_mcp_get_string_arg(args_json, "name");
    char *key = ani_project_name_from_path(override && override[0] ? override : root_path);
    free(override);
    return key;
}

static size_t application_active_job_count_locked(ani_daemon_application_t *application) {
    size_t count = 0;
    for (ani_daemon_application_job_t *job = application->jobs; job; job = job->next) {
        if (!job->terminal) {
            count++;
...
static ani_daemon_application_job_t *application_job_subscribe_locked(
    ...
    *status_out = APPLICATION_JOB_SUBSCRIBE_UNAVAILABLE;
    ...
    if (application_active_job_count_locked(application) >= application->physical_job_limit) {
        ...
        ani_log_warn("daemon.index.admission_busy", "limit", limit, "project", project_key);
    ...
    if (application_job_thread_create(&job->thread, job) == 0) {
        ...
        application_job_free(job);
        *status_out = APPLICATION_JOB_SUBSCRIBE_UNAVAILABLE;
        ani_log_warn("daemon.index.thread_start_failed", "action", "retry");
        return NULL;
    }
    *status_out = APPLICATION_JOB_SUBSCRIBE_OK;
    return job;
...
static ani_daemon_application_job_t *application_job_subscribe(
    ani_daemon_application_t *application, const char *project_key, const char *root_path,
    const char *args_json, application_job_subscribe_status_t *status_out) {
    application_jobs_reap_completed(application);
    ani_mutex_lock(&application->mutex);
    ani_daemon_application_job_t *job = application_job_subscribe_locked(
...
static ani_daemon_application_job_t *application_auto_index_release_locked(
    ani_daemon_application_session_t *session) {
    if (!session || !session->auto_index_job || !session->auto_index_subscribed) {
        return NULL;
    }
    ani_daemon_application_job_t *job = session->auto_index_job;
    session->auto_index_job = NULL;
    session->auto_index_subscribed = false;
...
static void application_auto_index_cancel_join(ani_daemon_application_t *application,
                                               ani_daemon_application_job_t *job) {
    if (!application || !job) {
        return;
    }
...
static void application_update_cancel_locked(ani_daemon_application_t *application) {
...
static bool application_update_owner_release_locked(ani_daemon_application_session_t *session) {
...
static void application_update_publish_terminal_locked(ani_daemon_application_t *application,
                                                       const char *latest_version,
                                                       bool completed_generation) {
    if (!application->update_cancel_requested && application_update_version_valid(latest_version) &&
        ani_compare_versions(latest_version, ani_cli_get_version()) > 0) {
        (void)snprintf(application->update_notice, sizeof(application->update_notice),
                       "Update available: %s -> %s -- run: ani update  |  "
                       "Enjoying ani? Please leave a star: "
                       "https://github.com/jnani1972/ani",
                       ani_cli_get_version(), latest_version);
        ani_log_info("update.available", "current", ani_cli_get_version(), "latest",
                     latest_version);
    }
    for (ani_daemon_application_session_t *session = application->sessions; session;
         session = session->next) {
...
static void *application_update_thread(void *opaque) {
    ani_daemon_application_t *application = opaque;
    ani_daemon_application_update_worker_t worker = NULL;
    ...
    for (;;) {
        ani_daemon_application_update_poll_t status =
            application->update_ops.poll(application->update_ops.context, worker, &latest_version);
        if (status != ANI_DAEMON_APPLICATION_UPDATE_POLL_RUNNING) {
            if (status == ANI_DAEMON_APPLICATION_UPDATE_POLL_ERROR) {
                latest_version = NULL;
            }
            break;
        }
        ...
        ani_mutex_unlock(&application->mutex);
        ani_usleep(APPLICATION_UPDATE_POLL_US);
    }
...
static void application_update_subscribe_locked(ani_daemon_application_session_t *session) {
    ani_daemon_application_t *application = session->application;
    ...
    if (ani_thread_create(&application->update_thread, APPLICATION_JOB_THREAD_STACK,
    ...
    application->update_thread_done = false;
    ani_log_warn("daemon.update.thread_start_failed", "action", "retry");
}
...
static bool application_update_reap(ani_daemon_application_t *application, bool wait,
    ...
    for (;;) {
        ...
        ani_mutex_unlock(&application->mutex);
        if (join) {
            bool joined = ani_thread_join(&application->update_thread) == 0;
            ani_mutex_lock(&application->mutex);
            if (joined) {
...
static void application_background_initialize_impl(ani_daemon_application_session_t *session) {
    ...
    bool auto_index = application->config &&
                      ani_config_get_bool(application->config, ANI_CONFIG_AUTO_INDEX, false);
    int auto_index_limit =
        application->config ? ani_config_get_int(application->config, ANI_CONFIG_AUTO_INDEX_LIMIT,
    ...
    bool within_auto_index_limit =
        ...
        ani_mcp_auto_index_within_file_limit(root_path, auto_index_limit, &tracked_files);
    if (auto_index_candidate && !within_auto_index_limit) {
        char files[32];
        (void)snprintf(files, sizeof(files), "%d", tracked_files);
        ani_log_warn("daemon.autoindex.skipped", "project", project, "reason",
    ...
    if (attempt_auto_index && args) {
        ...
        if (job) {
            ...
            ani_log_warn("daemon.autoindex.admission_failed", "project", project, "action",
...
static void application_background_initialize(ani_daemon_application_session_t *session) {
...
static void application_update_notice_inject(ani_daemon_application_session_t *session,
                                             char **response_io) {
    ani_daemon_application_t *application = session->application;
    ani_mutex_lock(&application->mutex);
    if (session->background_eligible && !session->update_notice_delivered &&
        ...
        ani_mcp_jsonrpc_response_prepend_notice(response_io, application->update_notice)) {
...
static bool application_watch_job_subscription_exists_locked(
    ani_daemon_application_t *application, ani_daemon_application_session_t *session,
    ani_daemon_application_job_t *job) {
    for (ani_daemon_application_watch_job_subscription_t *subscription =
...
static bool application_watch_job_subscribe_late_session_locked(
    ...
    ani_daemon_application_watch_job_subscription_t *subscription =
...
static bool application_watch_job_subscribe_sessions_locked(ani_daemon_application_t *application,
                                                            ani_daemon_application_watch_t *watch,
                                                            ani_daemon_application_job_t *job,
                                                            size_t *matched_out) {
    *matched_out = 0;
    ...
    ani_daemon_application_watch_job_subscription_t *pending = NULL;
    for (ani_daemon_application_session_t *session = application->sessions; session;
         session = session->next) {
        ...
        if (!subscription) {
            while (pending) {
                ani_daemon_application_watch_job_subscription_t *next = pending->next;
                free(pending);
                pending = next;
            }
            return false;
    ...
    while (pending) {
        ani_daemon_application_watch_job_subscription_t *subscription = pending;
...
static void application_watch_job_unsubscribe_session_locked(
    ...
    ani_daemon_application_watch_job_subscription_t **cursor =
        &session->application->watch_job_subscriptions;
    while (*cursor) {
        ani_daemon_application_watch_job_subscription_t *subscription = *cursor;
...
static void application_watch_job_unsubscribe_job_locked(ani_daemon_application_t *application,
                                                         ani_daemon_application_job_t *job) {
    ani_daemon_application_watch_job_subscription_t **cursor =
        &application->watch_job_subscriptions;
    while (*cursor) {
...
static char *application_job_wait_for_session(ani_daemon_application_session_t *session,
                                              ani_daemon_application_job_t *job) {
    ani_daemon_application_t *application = session->application;
    for (;;) {
        ani_mutex_lock(&application->mutex);
        if (session->active_job != job || !session->active_job_subscribed) {
            ani_mutex_unlock(&application->mutex);
            return ani_mcp_text_result("index operation cancelled for this session", true);
        }
        if (job->terminal) {
            ...
            return response ? response
                            : ani_mcp_text_result("index coordinator lost its result", true);
        }
        ani_mutex_unlock(&application->mutex);
        ani_usleep(APPLICATION_JOB_POLL_US);
    }
...
static char *application_index_execute(void *context, const char *root_path,
    ...
    if (!project_key) {
        return ani_mcp_text_result("failed to derive index project identity", true);
    }
    application_job_subscribe_status_t subscribe_status = APPLICATION_JOB_SUBSCRIBE_UNAVAILABLE;
    ani_daemon_application_job_t *job = NULL;
    for (;;) {
    ...
    if (session->active_job) {
        application_job_unsubscribe_locked(job);
        ani_mutex_unlock(&session->application->mutex);
        return ani_mcp_text_result("this session already has an active index operation", true);
    }
...
static ani_daemon_runtime_application_session_t *application_session_open(
    void *context, ani_daemon_client_id_t client_id, uint64_t authenticated_process_id) {
    ani_daemon_application_t *application = context;
    if (!application || client_id == ANI_DAEMON_CLIENT_ID_INVALID ||
        authenticated_process_id == 0) {
        return NULL;
    }
    ani_daemon_application_session_t *session = calloc(1, sizeof(*session));
    if (!session) {
        return NULL;
    }
    session->mcp = ani_mcp_server_new(NULL);
    if (!session->mcp || !ani_mcp_server_release_pristine_memory_store(session->mcp)) {
        ani_mcp_server_free(session->mcp);
        free(session);
        return NULL;
    }
    ani_mcp_server_set_background_tasks(session->mcp, false);
    ani_mcp_server_set_config(session->mcp, application->config);
    ani_mcp_server_set_index_executor(session->mcp, application_index_execute, session);
    ani_mcp_server_set_project_mutation_guard(session->mcp, application_session_mutation_begin,
                                              application_session_mutation_end, session);
    ani_mcp_server_set_project_mutation_try_guard(session->mcp,
                                                  application_session_mutation_try_begin);
    session->tool_profile = ANI_MCP_TOOL_PROFILE_ALL;
...
static ani_daemon_runtime_application_status_t application_set_context(
    ani_daemon_application_session_t *session, const uint8_t *request, uint32_t request_length) {
    if (session->context_set || request_length < APPLICATION_CONTEXT_HEADER_SIZE) {
        return ANI_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
    ...
    if (request[5] > 1 || root_length == 0 || expected != request_length ||
        ...
        profile_value > (uint8_t)ANI_MCP_TOOL_PROFILE_SCOUT ||
        (profile_value != (uint8_t)ANI_MCP_TOOL_PROFILE_ALL &&
         (event_length != 0 || dialect_length != 0))) {
        return ANI_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
    ani_mcp_tool_profile_t tool_profile = (ani_mcp_tool_profile_t)profile_value;
    ...
    if (!root || (allowed_present && !allowed) || (event_length && !hook_event) ||
        ...
        !ani_hook_augment_invocation_supported(hook_event, hook_dialect)) {
        free(root);
        free(allowed);
        free(hook_event);
        free(hook_dialect);
    ...
    bool canonical = ani_canonical_path(root, canonical_root, sizeof(canonical_root));
    if (canonical && allowed_present) {
        canonical = ani_canonical_path(allowed, canonical_allowed, sizeof(canonical_allowed));
    }
    canonical = canonical && application_canonical_directory_exists(canonical_root);
    bool set =
        canonical && ani_mcp_server_set_session_context(session->mcp, canonical_root,
    ...
    ani_mcp_server_set_tool_profile(session->mcp, tool_profile);
...
static char *application_oversized_response_error(const ani_jsonrpc_request_t *request,
    ...
    (void)snprintf(message, sizeof(message),
                   ...
                   response_length, (unsigned)ANI_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX);
...
static char *application_framable_response(char *response, const ani_jsonrpc_request_t *request) {
    if (!response || strlen(response) <= ANI_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX) {
        return response;
    }
...
char *ani_daemon_application_framable_response_for_test(char *response,
                                                        const ani_jsonrpc_request_t *request) {
    return application_framable_response(response, request);
}

static ani_daemon_runtime_application_status_t application_mcp_request(
    ani_daemon_application_session_t *session, const uint8_t *request, uint32_t request_length,
    ...
    ani_jsonrpc_request_t parsed = {0};
    bool parsed_ok = ani_jsonrpc_parse(message, &parsed) == 0;
    ...
    char *response = ani_mcp_server_handle(session->mcp, message);
    free(message);
    ...
    if (response) {
        ...
        if (response_length > UINT32_MAX) {
            if (parsed_ok) {
                ani_jsonrpc_request_free(&parsed);
            }
            free(response);
...
static ani_daemon_runtime_application_status_t application_tool_request(
    ani_daemon_application_session_t *session, const uint8_t *request, uint32_t request_length,
    uint8_t **response_out, uint32_t *response_length_out) {
    ...
    char *response = ani_mcp_handle_tool(session->mcp, tool, args);
    free(tool);
    free(args);
    if (!response) {
        return ANI_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR;
...
static ani_daemon_runtime_application_status_t application_set_ui_config(
    ani_daemon_application_t *application, ani_daemon_application_session_t *session,
    const uint8_t *request, uint32_t request_length) {
    const uint8_t valid_mask =
        ANI_DAEMON_APPLICATION_UI_CONFIG_ENABLED | ANI_DAEMON_APPLICATION_UI_CONFIG_PORT;
    if (!session || !session->context_set || session->tool_profile != ANI_MCP_TOOL_PROFILE_ALL ||
    ...
    bool enabled_present = (update_mask & ANI_DAEMON_APPLICATION_UI_CONFIG_ENABLED) != 0;
    bool port_present = (update_mask & ANI_DAEMON_APPLICATION_UI_CONFIG_PORT) != 0;
    ...
    if (application->stopping) {
        ani_mutex_unlock(&application->mutex);
        return ANI_DAEMON_RUNTIME_APPLICATION_UNAVAILABLE;
    }
    ani_ui_config_t config;
    ani_ui_config_load(&config);
    if (enabled_present) {
    ...
    bool saved = ani_ui_config_save(&config);
    ani_mutex_unlock(&application->mutex);
    return saved ? ANI_DAEMON_RUNTIME_APPLICATION_OK : ANI_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR;
}

static ani_daemon_runtime_application_status_t application_ui_readiness_proof(
    ani_daemon_application_t *application, const uint8_t *request, uint32_t request_length,
    uint8_t **response_out, uint32_t *response_length_out) {
    if (!application->ui_readiness_secret_set ||
        request_length != APPLICATION_UI_READINESS_REQUEST_SIZE) {
        return ANI_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
    uint8_t *proof = malloc(ANI_SHA256_DIGEST_LEN);
    if (!proof) {
        return ANI_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR;
    }
    ani_hmac_sha256(application->ui_readiness_secret, sizeof(application->ui_readiness_secret),
                    request + 1, ANI_SHA256_DIGEST_LEN, proof);
    *response_out = proof;
    *response_length_out = ANI_SHA256_DIGEST_LEN;
    return ANI_DAEMON_RUNTIME_APPLICATION_OK;
}

static ani_daemon_runtime_application_status_t application_request_dispatch(
    ani_daemon_application_t *application, ani_daemon_application_session_t *session,
    const uint8_t *request, uint32_t request_length, uint8_t **response_out,
    uint32_t *response_length_out) {
    switch ((ani_daemon_application_request_kind_t)request[0]) {
    case ANI_DAEMON_APPLICATION_REQUEST_SET_CONTEXT:
        return application_set_context(session, request, request_length);
    case ANI_DAEMON_APPLICATION_REQUEST_MCP:
        return application_mcp_request(session, request, request_length, response_out,
                                       response_length_out);
    case ANI_DAEMON_APPLICATION_REQUEST_TOOL:
        return application_tool_request(session, request, request_length, response_out,
                                        response_length_out);
    case ANI_DAEMON_APPLICATION_REQUEST_SET_UI_CONFIG:
        return application_set_ui_config(application, session, request, request_length);
    case ANI_DAEMON_APPLICATION_REQUEST_UI_READINESS_PROOF:
        return application_ui_readiness_proof(application, request, request_length, response_out,
                                              response_length_out);
    case ANI_DAEMON_APPLICATION_REQUEST_HOOK_AUGMENT: {
        if (!session->context_set || request_length <= 1) {
            return ANI_DAEMON_RUNTIME_APPLICATION_REJECTED;
        }
        ...
        char *response = ani_hook_augment_process_for(session->mcp, input, session->hook_event,
                                                      session->hook_dialect);
        free(input);
        if (response) {
            size_t response_length = strlen(response);
...
static ani_daemon_runtime_application_status_t application_request(
    void *context, ani_daemon_runtime_application_session_t *opaque_session,
    ...
    ani_daemon_application_t *application = context;
    ani_daemon_application_session_t *session = (ani_daemon_application_session_t *)opaque_session;
    if (response_out) {
        *response_out = NULL;
    }
    ...
    if (!application || !session || session->application != application ||
        request_token == ANI_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID || !request ||
    ...
    if (session->request_cancel_token != request_token) {
        session->request_cancel_token = ANI_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    }
    session->request_active = true;
    session->active_request_token = request_token;
    bool mcp_scope_started = ani_mcp_server_request_scope_begin(session->mcp);
    ...
    ani_daemon_runtime_application_status_t status =
        cancelled_before_entry
            ? ANI_DAEMON_RUNTIME_APPLICATION_CANCELLED
    ...
    ani_mcp_server_request_scope_end(session->mcp);
    session->request_active = false;
    session->active_request_token = ANI_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    if (session->request_cancel_token == request_token) {
        session->request_cancel_token = ANI_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    ...
    bool activate_background =
        !cancelled && status == ANI_DAEMON_RUNTIME_APPLICATION_OK &&
    ...
    if (!cancelled && status == ANI_DAEMON_RUNTIME_APPLICATION_OK &&
...
static void application_cancel_jobs_locked(ani_daemon_application_t *application) {
    for (ani_daemon_application_job_t *job = application->jobs; job; job = job->next) {
        if (!job->terminal) {
            job->cancel_requested = true;
        }
...
static void application_request_cancel(void *context,
                                       ani_daemon_runtime_application_session_t *opaque_session,
    ...
    if (!session->session_cancelled &&
        ...
        if (active_match && session->active_job && session->active_job_subscribed) {
            ani_daemon_application_job_t *job = session->active_job;
            session->active_job = NULL;
            session->active_job_subscribed = false;
            application_job_unsubscribe_locked(job);
        }
        if (active_match) {
            ...
            (void)ani_mcp_server_cancel_active(session->mcp);
        }
    }
    ani_mutex_unlock(&application->mutex);
}
...
static void application_session_cancel(void *context,
    ...
    if (application && session && session->application == application) {
        ...
        ani_daemon_application_job_t *join_auto_index = NULL;
        ani_mutex_lock(&application->mutex);
        ...
        for (ani_daemon_application_session_t *other = application->sessions;
...
static void application_session_close(void *context,
    ...
    ani_daemon_application_job_t *join_auto_index = NULL;
    ani_mutex_lock(&application->mutex);
    ani_daemon_application_session_t **cursor = &application->sessions;
...
void ani_daemon_application_set_permanent(ani_daemon_application_t *application, bool permanent) {
    if (!application) {
        return;
    }
    ani_mutex_lock(&application->mutex);
...
ani_daemon_application_t *ani_daemon_application_new(
    const ani_daemon_application_config_t *config) {
    ani_daemon_application_t *application = calloc(1, sizeof(*application));
    if (!application) {
        return NULL;
    }
    ani_mutex_init(&application->mutex);
    application->physical_job_limit = APPLICATION_DEFAULT_PHYSICAL_JOB_LIMIT;
    size_t aggregate_memory_budget_bytes = ani_mem_budget();
    if (config) {
        if ((config->ui_readiness_secret != NULL || config->ui_readiness_secret_length != 0) &&
            ...
            ani_mutex_destroy(&application->mutex);
            ani_secure_zero(application->ui_readiness_secret,
    ...
    if (!application->worker_ops.start) {
        application->worker_ops = (ani_daemon_application_worker_ops_t){
            .context = NULL,
    ...
    if (!application->worker_ops.poll || !application->worker_ops.cancel ||
        ...
        ani_mutex_destroy(&application->mutex);
        ani_secure_zero(application->ui_readiness_secret, sizeof(application->ui_readiness_secret));
        free(application);
        return NULL;
    }
    ...
    if (application->watcher) {
        ani_watcher_set_project_mutation_guard(
...
bool ani_daemon_application_shutdown(ani_daemon_application_t *application, uint32_t timeout_ms) {
    if (!application) {
        return false;
    }
    uint64_t deadline = application_deadline_after(timeout_ms);
    ...
    for (;;) {
        ...
        for (ani_daemon_application_session_t *session = application->sessions; all_done && session;
             session = session->next) {
            if (session->request_active) {
                all_done = false;
            }
...
bool ani_daemon_application_free_with_timeout(ani_daemon_application_t *application,
                                              uint32_t timeout_ms) {
    if (!application) {
        return true;
    }
    if (!ani_daemon_application_shutdown(application, timeout_ms)) {
         ...
         * resolved. */
        ani_log_error("daemon.application.free_busy", "action", "retain");
        return false;
    }
    if (application->watcher) {
        ...
        ani_watcher_set_project_mutation_guard(application->watcher, NULL, NULL, NULL, NULL);
    }
    ani_mutex_lock(&application->mutex);
    ani_daemon_application_session_t *sessions = application->sessions;
    application->sessions = NULL;
    ani_daemon_application_watch_t *watches = application->watches;
    application->watches = NULL;
    ani_daemon_application_job_t *jobs = application->jobs;
    application->jobs = NULL;
    ani_daemon_application_watch_job_subscription_t *watch_job_subscriptions =
        application->watch_job_subscriptions;
    application->watch_job_subscriptions = NULL;
    ani_daemon_application_mutation_t *mutations = application->mutations;
    application->mutations = NULL;
    ani_mutex_unlock(&application->mutex);
    while (sessions) {
        ani_daemon_application_session_t *next = sessions->next;
        ani_mcp_server_free(sessions->mcp);
        free(sessions);
        sessions = next;
    }
    while (watches) {
        ani_daemon_application_watch_t *next = watches->next;
        if (application->watcher) {
            ani_watcher_unwatch(application->watcher, watches->project);
        }
    ...
    while (watch_job_subscriptions) {
        ani_daemon_application_watch_job_subscription_t *next = watch_job_subscriptions->next;
        free(watch_job_subscriptions);
        watch_job_subscriptions = next;
    }
    while (jobs) {
        ani_daemon_application_job_t *next = jobs->next;
        if (jobs->thread_started) {
            (void)ani_thread_join(&jobs->thread);
        }
        application_job_free(jobs);
        jobs = next;
    }
    while (mutations) {
        ani_daemon_application_mutation_t *next = mutations->next;
...
bool ani_daemon_application_free(ani_daemon_application_t *application) {
    return ani_daemon_application_free_with_timeout(application, 3000);
}

ani_daemon_runtime_application_callbacks_t ani_daemon_application_runtime_callbacks(
    ani_daemon_application_t *application) {
...
static ani_daemon_runtime_application_status_t application_client_exchange_tagged(
    ...
    ani_daemon_runtime_application_status_t status =
        request_token == ANI_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID
            ? ani_daemon_runtime_client_application_request(client, request, request_length,
                                                            &response, &response_length, timeout_ms)
            : ani_daemon_runtime_client_application_request_tagged(client, request_token, request,
                                                                   request_length, &response,
                                                                   &response_length, timeout_ms);
    free(request);
    if (status != ANI_DAEMON_RUNTIME_APPLICATION_OK) {
        free(response);
        return status;
    }
    if (response) {
...
static ani_daemon_runtime_application_status_t application_client_exchange(
    ani_daemon_runtime_client_t *client, uint8_t *request, uint32_t request_length,
    uint8_t **response_out, uint32_t *response_length_out, uint32_t timeout_ms) {
    return application_client_exchange_tagged(client, ANI_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID,
                                              request, request_length, response_out,
                                              response_length_out, timeout_ms);
}

ani_daemon_runtime_application_status_t ani_daemon_application_client_set_context(
    ani_daemon_runtime_client_t *client, const char *session_root, const char *allowed_root,
    ani_mcp_tool_profile_t tool_profile, const char *hook_event, const char *hook_dialect,
    uint32_t timeout_ms) {
    ...
    uint64_t total = (uint64_t)APPLICATION_CONTEXT_HEADER_SIZE + root_length + allowed_length +
                     event_length + dialect_length;
    if (tool_profile < ANI_MCP_TOOL_PROFILE_ALL || tool_profile > ANI_MCP_TOOL_PROFILE_SCOUT ||
        (tool_profile != ANI_MCP_TOOL_PROFILE_ALL && (event_length != 0 || dialect_length != 0)) ||
        !ani_hook_augment_invocation_supported(hook_event, hook_dialect) ||
        root_length > UINT32_MAX || allowed_length > UINT32_MAX || event_length > UINT32_MAX ||
        dialect_length > UINT32_MAX || total > ANI_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX) {
        return ANI_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
    ...
    ani_daemon_runtime_application_status_t status = application_client_exchange(
        client, request, (uint32_t)total, &unexpected, &unexpected_length, timeout_ms);
    if (status == ANI_DAEMON_RUNTIME_APPLICATION_OK && (unexpected || unexpected_length != 0)) {
...
ani_daemon_runtime_application_status_t ani_daemon_application_client_set_ui_config(
    ani_daemon_runtime_client_t *client, uint8_t update_mask, bool ui_enabled, int ui_port,
    uint32_t timeout_ms) {
    const uint8_t valid_mask =
        ANI_DAEMON_APPLICATION_UI_CONFIG_ENABLED | ANI_DAEMON_APPLICATION_UI_CONFIG_PORT;
    bool enabled_present = (update_mask & ANI_DAEMON_APPLICATION_UI_CONFIG_ENABLED) != 0;
    ...
    request[0] = ANI_DAEMON_APPLICATION_REQUEST_SET_UI_CONFIG;
...
ani_daemon_runtime_application_status_t ani_daemon_application_client_ui_readiness_proof(
    ani_daemon_runtime_client_t *client, const uint8_t challenge[ANI_SHA256_DIGEST_LEN],
    uint8_t proof_out[ANI_SHA256_DIGEST_LEN], uint32_t timeout_ms) {
    if (!client || !challenge || !proof_out) {
        return ANI_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
    ani_secure_zero(proof_out, ANI_SHA256_DIGEST_LEN);
    uint8_t *request = malloc(APPLICATION_UI_READINESS_REQUEST_SIZE);
    if (!request) {
        return ANI_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    }
    request[0] = ANI_DAEMON_APPLICATION_REQUEST_UI_READINESS_PROOF;
    memcpy(request + 1, challenge, ANI_SHA256_DIGEST_LEN);
    uint8_t *response = NULL;
    uint32_t response_length = 0;
    ani_daemon_runtime_application_status_t status =
        application_client_exchange(client, request, APPLICATION_UI_READINESS_REQUEST_SIZE,
                                    &response, &response_length, timeout_ms);
    if (status == ANI_DAEMON_RUNTIME_APPLICATION_OK) {
        if (!response || response_length != ANI_SHA256_DIGEST_LEN) {
            status = ANI_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR;
        } else {
            memcpy(proof_out, response, ANI_SHA256_DIGEST_LEN);
        }
    }
    if (response) {
        ani_secure_zero(response, response_length);
    }
    free(response);
    return status;
}

static ani_daemon_runtime_application_status_t application_client_text_request_tagged(
    ani_daemon_runtime_client_t *client, ani_daemon_runtime_application_token_t request_token,
    ani_daemon_application_request_kind_t kind, const char *text, uint8_t **response_out,
    uint32_t *response_length_out, uint32_t timeout_ms) {
    if (!client || !text || !text[0]) {
        return ANI_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
    size_t text_length = strlen(text);
    if (text_length + 1U > ANI_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX) {
        return ANI_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
...
static ani_daemon_runtime_application_status_t application_client_text_request(
    ani_daemon_runtime_client_t *client, ani_daemon_application_request_kind_t kind,
    ...
    return application_client_text_request_tagged(
        client, ANI_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID, kind, text, response_out,
        response_length_out, timeout_ms);
}

ani_daemon_runtime_application_status_t ani_daemon_application_client_mcp(
    ani_daemon_runtime_client_t *client, const char *message, uint8_t **response_out,
    uint32_t *response_length_out, uint32_t timeout_ms) {
    return application_client_text_request(client, ANI_DAEMON_APPLICATION_REQUEST_MCP, message,
                                           response_out, response_length_out, timeout_ms);
}

ani_daemon_runtime_application_status_t ani_daemon_application_client_mcp_tagged(
    ani_daemon_runtime_client_t *client, ani_daemon_runtime_application_token_t request_token,
    ...
    return application_client_text_request_tagged(client, request_token,
                                                  ANI_DAEMON_APPLICATION_REQUEST_MCP, message,
                                                  response_out, response_length_out, timeout_ms);
}

ani_daemon_runtime_application_status_t ani_daemon_application_client_tool(
    ani_daemon_runtime_client_t *client, const char *tool_name, const char *args_json,
    uint8_t **response_out, uint32_t *response_length_out, uint32_t timeout_ms) {
    ...
    if (tool_length > UINT32_MAX || total > ANI_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX) {
        return ANI_DAEMON_RUNTIME_APPLICATION_REJECTED;
    }
...
ani_daemon_runtime_application_status_t ani_daemon_application_client_hook_augment(
    ani_daemon_runtime_client_t *client, const char *input_json, uint8_t **response_out,
    uint32_t *response_length_out, uint32_t timeout_ms) {
    return application_client_text_request(client, ANI_DAEMON_APPLICATION_REQUEST_HOOK_AUGMENT,
                                           input_json, response_out, response_length_out,
                                           timeout_ms);
}

static int application_background_index(ani_daemon_application_t *application,
    ...
    if (!ani_canonical_path(root_path, canonical_root, sizeof(canonical_root)) ||
    ...
    char *default_project = ani_project_name_from_path(canonical_root);
    bool custom_project =
...
int ani_daemon_application_index(ani_daemon_application_t *application, const char *project_name,
                                 const char *root_path) {
    return application_background_index(application, project_name, root_path, false);
}

int ani_daemon_application_watcher_index(const char *project_name, const char *root_path,
                                         void *context) {
    return application_background_index(context, project_name, root_path, true);
}

size_t ani_daemon_application_active_jobs(ani_daemon_application_t *application) {
    if (!application) {
        return 0;
    }
    size_t count = 0;
...
size_t ani_daemon_application_job_subscribers(ani_daemon_application_t *application,
                                              const char *project_key) {
    if (!application || !project_key) {
        return 0;
    }
    ani_mutex_lock(&application->mutex);
    ani_daemon_application_job_t *job = application_find_job_locked(application, project_key);
    size_t subscribers = job ? job->subscribers : 0;
    ani_mutex_unlock(&application->mutex);
    return subscribers;
}

size_t ani_daemon_application_physical_job_limit(ani_daemon_application_t *application) {
    if (!application) {
        return 0;
    }
    ani_mutex_lock(&application->mutex);
...
size_t ani_daemon_application_worker_memory_budget_bytes(ani_daemon_application_t *application) {
    if (!application) {
        return 0;
    }
    ani_mutex_lock(&application->mutex);
...
bool ani_daemon_application_session_retains_store_for_test(
    const ani_daemon_runtime_application_session_t *opaque_session) {
    const ani_daemon_application_session_t *session =
        (const ani_daemon_application_session_t *)opaque_session;
    return session && session->mcp && ani_mcp_server_store(session->mcp) != NULL;
}
