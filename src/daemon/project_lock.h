/* project_lock.h — Shared daemon/local-CLI project mutation leases. */
#ifndef ANI_DAEMON_PROJECT_LOCK_H
#define ANI_DAEMON_PROJECT_LOCK_H

#include "daemon/ipc.h"
#include "foundation/lock_registry.h"

#include <stdint.h>

typedef struct ani_project_lock_manager ani_project_lock_manager_t;
typedef struct ani_project_lock_lease ani_project_lock_lease_t;

/* Each manager is an independent process-local registry over the endpoint's
 * owner-only runtime directory. Separate ANI processes therefore coordinate
 * through the same native lock files without sharing memory. */
ani_project_lock_manager_t *ani_project_lock_manager_new(const ani_daemon_ipc_endpoint_t *endpoint);

/* Normal projects hold SH(project-set) + EX(project). "*" holds
 * EX(project-set), blocking every named project. Project lock keys are ASCII
 * case-folded to cover filename aliases on case-insensitive filesystems. */
ani_private_file_lock_status_t ani_project_lock_acquire(ani_project_lock_manager_t *manager,
                                                        const char *project, uint64_t deadline_ms,
                                                        const ani_lock_cancel_token_t *cancel_token,
                                                        ani_project_lock_lease_t **lease_out);

/* One fair, nonblocking attempt for UI/watcher paths. */
ani_private_file_lock_status_t ani_project_lock_try_acquire(ani_project_lock_manager_t *manager,
                                                            const char *project,
                                                            ani_project_lock_lease_t **lease_out);

ani_private_file_lock_status_t ani_project_lock_lease_release(ani_project_lock_lease_t **lease_io);

ani_private_file_lock_status_t ani_project_lock_request_cancel(ani_project_lock_manager_t *manager,
                                                               ani_lock_cancel_token_t *token);

/* Refuses teardown while any lease/cleanup state remains. */
ani_private_file_lock_status_t ani_project_lock_manager_free(
    ani_project_lock_manager_t **manager_io);

#endif /* ANI_DAEMON_PROJECT_LOCK_H */
