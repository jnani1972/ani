/*
 * ipc.h — Authenticated local transport for the ANI daemon.
 *
 ...
 */
#ifndef ANI_DAEMON_IPC_H
#define ANI_DAEMON_IPC_H
...
typedef struct ani_daemon_ipc_endpoint ani_daemon_ipc_endpoint_t;
typedef struct ani_daemon_ipc_listener ani_daemon_ipc_listener_t;
typedef struct ani_daemon_ipc_connection ani_daemon_ipc_connection_t;
typedef struct ani_daemon_ipc_startup_lock ani_daemon_ipc_startup_lock_t;
typedef struct ani_daemon_ipc_local_transition ani_daemon_ipc_local_transition_t;
typedef struct ani_daemon_ipc_participant_guard ani_daemon_ipc_participant_guard_t;
typedef struct ani_daemon_ipc_lifetime_reservation ani_daemon_ipc_lifetime_reservation_t;

/* A receive wait with no wall-clock expiry. The wait remains interruptible by
 * peer EOF and ani_daemon_ipc_connection_interrupt(). This is intentionally a
 ...
 * arriving. */
#define ANI_DAEMON_IPC_WAIT_FOREVER UINT32_MAX
 ...
 * receive_frame also accepts ANI_DAEMON_IPC_WAIT_FOREVER. */
...
int ani_daemon_ipc_local_transition_try_acquire(const ani_daemon_ipc_endpoint_t *endpoint,
...
int ani_daemon_ipc_local_transition_lifetime_probe(
    const ani_daemon_ipc_endpoint_t *endpoint, const ani_daemon_ipc_local_transition_t *transition);
