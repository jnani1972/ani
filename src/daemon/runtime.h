/*
 * runtime.h — Mandatory per-account ANI daemon runtime.
 *
 ...
 */
#ifndef ANI_DAEMON_RUNTIME_H
#define ANI_DAEMON_RUNTIME_H
...
#define ANI_DAEMON_SEMVER_SIZE 12U
...
#define ANI_DAEMON_RUNTIME_WIRE_ABI 1U
...
#define ANI_DAEMON_RENDEZVOUS_ABI 1U
#define ANI_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP 64U
#define ANI_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP 65U
#define ANI_DAEMON_RENDEZVOUS_MESSAGE_CAP 512U
#define ANI_DAEMON_RENDEZVOUS_REQUEST_SIZE 133U
#define ANI_DAEMON_RENDEZVOUS_RESPONSE_SIZE 798U
...
#define ANI_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE 137U
#define ANI_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE 24U
...
#define ANI_DAEMON_CONTROL_REQUEST_SIZE 65U
#define ANI_DAEMON_CONTROL_CLIENT_CAP 8U
#define ANI_DAEMON_STATUS_RESPONSE_SIZE 121U
#define ANI_DAEMON_STOP_RESPONSE_SIZE 40U
#define ANI_DAEMON_RUNTIME_PROJECT_KEY_MAX 4096U
#define ANI_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX (ANI_DAEMON_MAX_FRAME_SIZE - 16U)
...
typedef enum {
    ...
    ANI_DAEMON_RUNTIME_OP_HEARTBEAT = 2,
    ANI_DAEMON_RUNTIME_OP_JOB_SUBSCRIBE = 3,
    ANI_DAEMON_RUNTIME_OP_JOB_UNSUBSCRIBE = 4,
    ANI_DAEMON_RUNTIME_OP_DISCONNECT = 5,
    ANI_DAEMON_RUNTIME_OP_APPLICATION_REQUEST = 6,
    ANI_DAEMON_RUNTIME_OP_APPLICATION_CANCEL = 7,
    ...
    ANI_DAEMON_RUNTIME_OP_CLOSE_INTENT = 9,
    ...
    ANI_DAEMON_RUNTIME_OP_STATUS = 10,
    ...
    ANI_DAEMON_RUNTIME_OP_STOP = 11,
} ani_daemon_runtime_operation_t;

typedef enum {
    ANI_DAEMON_RUNTIME_ACTIVATION_INSTALL = 1,
    ANI_DAEMON_RUNTIME_ACTIVATION_UPDATE = 2,
    ANI_DAEMON_RUNTIME_ACTIVATION_UNINSTALL = 3,
} ani_daemon_runtime_activation_action_t;

typedef struct {
    bool accepted;
    uint64_t active_clients;
    uint64_t active_connections;
} ani_daemon_runtime_activation_result_t;

typedef struct ani_daemon_runtime_service ani_daemon_runtime_service_t;
typedef struct ani_daemon_runtime_client ani_daemon_runtime_client_t;

typedef enum {
    ANI_DAEMON_RUNTIME_SERVICE_STARTING = 1,
    ANI_DAEMON_RUNTIME_SERVICE_RUNNING = 2,
    ANI_DAEMON_RUNTIME_SERVICE_STOPPING = 3,
    ANI_DAEMON_RUNTIME_SERVICE_EXITED = 4,
} ani_daemon_runtime_service_state_t;
...
typedef enum {
    ANI_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR = 0,
    ANI_DAEMON_RUNTIME_APPLICATION_OK = 1,
    ANI_DAEMON_RUNTIME_APPLICATION_BUSY = 2,
    ANI_DAEMON_RUNTIME_APPLICATION_UNAVAILABLE = 3,
    ANI_DAEMON_RUNTIME_APPLICATION_REJECTED = 4,
    ANI_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR = 5,
    ANI_DAEMON_RUNTIME_APPLICATION_CANCELLED = 6,
} ani_daemon_runtime_application_status_t;

typedef uint64_t ani_daemon_runtime_application_token_t;
#define ANI_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID UINT64_C(0)

typedef enum {
    ANI_DAEMON_RUNTIME_CANCEL_ERROR = -1,
    ANI_DAEMON_RUNTIME_CANCEL_STALE = 0,
    ANI_DAEMON_RUNTIME_CANCEL_ACCEPTED = 1,
} ani_daemon_runtime_cancel_result_t;

typedef void ani_daemon_runtime_application_session_t;

typedef ani_daemon_runtime_application_session_t *(*ani_daemon_runtime_application_session_open_fn)(
...
typedef ani_daemon_runtime_application_status_t (*ani_daemon_runtime_application_request_fn)(
...
typedef void (*ani_daemon_runtime_application_request_cancel_fn)(
    void *context, ani_daemon_runtime_application_session_t *session,
...
typedef void (*ani_daemon_runtime_application_session_cancel_fn)(
    void *context, ani_daemon_runtime_application_session_t *session);
typedef void (*ani_daemon_runtime_application_session_close_fn)(
    void *context, ani_daemon_runtime_application_session_t *session);

typedef struct {
    void *context;
    ani_daemon_runtime_application_session_open_fn session_open;
    ani_daemon_runtime_application_request_fn request;
    ani_daemon_runtime_application_request_cancel_fn request_cancel;
    ani_daemon_runtime_application_session_cancel_fn session_cancel;
    ani_daemon_runtime_application_session_close_fn session_close;
} ani_daemon_runtime_application_callbacks_t;
...
typedef struct {
    ...
    bool permanent;
} ani_daemon_runtime_service_config_t;

typedef enum {
    ANI_DAEMON_RUNTIME_CONNECT_ERROR = 0,
    ANI_DAEMON_RUNTIME_CONNECT_ACCEPTED = 1,
    ANI_DAEMON_RUNTIME_CONNECT_CONFLICT = 2,
    ANI_DAEMON_RUNTIME_CONNECT_REJECTED = 3,
} ani_daemon_runtime_connect_status_t;

typedef struct {
    ani_daemon_runtime_connect_status_t status;
    ani_daemon_hello_status_t hello_status;
...
} ani_daemon_runtime_connect_result_t;
...
typedef struct {
    ...
    uint32_t client_pids[ANI_DAEMON_CONTROL_CLIENT_CAP];
    char build_fingerprint[ANI_DAEMON_BUILD_FINGERPRINT_SIZE];
    char semantic_version[ANI_DAEMON_SEMVER_SIZE];
...
typedef struct {
    ...
    uint32_t client_pids[ANI_DAEMON_CONTROL_CLIENT_CAP];
} ani_daemon_runtime_stop_result_t;
 ...
 * Client exchange timeouts must be finite, not ANI_DAEMON_IPC_WAIT_FOREVER. */
...
typedef void (*ani_daemon_runtime_containment_hook_t)(const char *component);
