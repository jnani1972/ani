 ...
 */
#ifndef ANI_GRAPH_BUFFER_H
#define ANI_GRAPH_BUFFER_H
...
typedef struct ani_gbuf ani_gbuf_t;
 ...
 * If id_source is NULL, behaves like ani_gbuf_new(). */
...
typedef void (*ani_gbuf_node_visitor_fn)(const ani_gbuf_node_t *node, void *userdata);
void ani_gbuf_foreach_node(const ani_gbuf_t *gb, ani_gbuf_node_visitor_fn fn, void *userdata);

/* Iterate all edges. */
typedef void (*ani_gbuf_edge_visitor_fn)(const ani_gbuf_edge_t *edge, void *userdata);
 ...
 * Vectors are carried through to ani_write_db during the dump phase. */
