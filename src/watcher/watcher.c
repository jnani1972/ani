...
typedef struct {
    ...
    ani_subprocess_t *active_git;
    /* Room for Git's 64-hex SHA-256 object ID plus newline/NUL while reading. */
    char last_head[ANI_SZ_128]; /* git HEAD hash (committed baseline) */
    ...
    uint64_t first_missing_ms;  /* ani_now_ms() of the streak's first miss (0 = no streak) */
    ...
    char pending_head[ANI_SZ_128]; /* HEAD observed at check time */
...
struct ani_watcher {
    ani_store_t *store;
    ani_index_fn index_fn;
    void *user_data;
    ANIHashTable *projects; /* name → project_state_t* */
    ani_mutex_t projects_lock;
    ...
    ani_mutex_t coordination_lock;
    ani_watcher_project_mutation_begin_fn mutation_begin;
    ani_watcher_project_mutation_end_fn mutation_end;
    ani_watcher_project_pruned_fn project_pruned;
...
#define PRUNE_GRACE_DEFAULT_S 600 /* 10 min; override: ANI_WATCHER_PRUNE_GRACE_S */
...
int ani_watcher_poll_interval_ms(int file_count) {
    int ms = POLL_BASE_MS + ((file_count / POLL_FILE_STEP) * ANI_MSEC_PER_SEC);
    if (ms > POLL_MAX_MS) {
        ms = POLL_MAX_MS;
    }
    return ms;
...
static bool watcher_git_output_create(watcher_git_output_t *output) {
    ...
    int written =
        snprintf(output->path, sizeof(output->path), "%s/ani-watcher-git-XXXXXX", ani_tmpdir());
    if (written <= 0 || written >= (int)sizeof(output->path)) {
        output->path[0] = '\0';
        return false;
    }
    int descriptor = ani_mkstemp(output->path);
    if (descriptor < 0) {
        output->path[0] = '\0';
        return false;
    }
...
static bool watcher_windows_git_candidate(const wchar_t *entry, size_t entry_length,
    ...
    if (entry_length == 0U || entry_length >= ANI_SZ_4K) {
        return false;
    }
    wchar_t directory[ANI_SZ_4K];
    ...
    wchar_t candidate[ANI_SZ_4K];
    int written =
        swprintf(candidate, ANI_SZ_4K, separator ? L"%lsgit.exe" : L"%ls\\git.exe", directory);
    if (written <= 0 || written >= ANI_SZ_4K) {
        return false;
    }
    ...
    char *utf8 = regular ? ani_wide_to_utf8(normalized) : NULL;
    size_t utf8_length = utf8 ? strlen(utf8) : 0U;
    bool valid = utf8 && utf8_length > 0U && utf8_length < ANI_SZ_4K;
    if (valid) {
...
static bool watcher_resolve_git_executable(char output[ANI_SZ_4K]) {
    output[0] = '\0';
...
static watcher_git_status_t watcher_git_run(ani_watcher_t *w, project_state_t *state,
    ...
    ani_mutex_lock(&w->projects_lock);
    if (state->active_git) {
        publication_conflict = true;
        cancelled = ani_subprocess_request_cancel(process);
    } else {
    ...
    ani_mutex_unlock(&w->projects_lock);
    ...
    for (;;) {
        ...
        if (!cancelled && output_limit > 0) {
            int64_t observed_size = ani_file_size(output->path);
            if (observed_size > 0 && (uint64_t)observed_size > output_limit) {
                output_exceeded = true;
                cancelled = ani_subprocess_request_cancel(process);
            }
        }
        ani_proc_poll_t polled = ani_subprocess_poll(process, &result);
        if (polled == ANI_PROC_POLL_TERMINAL) {
            break;
        }
        if (polled == ANI_PROC_POLL_ERROR) {
            /* Valid owned handles have no ERROR transition. Fail closed, but
             * continue polling so the contained tree is never abandoned. */
            cancelled = ani_subprocess_request_cancel(process) || cancelled;
        }
        ani_usleep(WATCHER_GIT_POLL_US);
    }
    ...
    } else if (result.outcome != ANI_PROC_CLEAN) {
        status = WATCHER_GIT_COMMAND_FAILED;
    }

    if (status == WATCHER_GIT_OK && output_limit > 0) {
        int64_t final_size = ani_file_size(output->path);
        if (final_size < 0) {
            status = WATCHER_GIT_SUPERVISION_FAILED;
    ...
    if (status == WATCHER_GIT_DEADLINE || status == WATCHER_GIT_OUTPUT_LIMIT ||
        status == WATCHER_GIT_SUPERVISION_FAILED) {
        ani_log_warn("watcher.git.failed", "project", state->project_name, "reason",
                     status == WATCHER_GIT_DEADLINE
                         ? "deadline"
...
static watcher_git_status_t git_repo_status(ani_watcher_t *w, project_state_t *state) {
...
static watcher_git_status_t git_tracks_anything_here(ani_watcher_t *w, project_state_t *state,
                                                     bool *tracked_out) {
    *tracked_out = false;
    ...
    FILE *fp = ani_fopen(output.path, "rb");
    if (fp) {
...
static watcher_git_status_t git_repo_cdup(ani_watcher_t *w, project_state_t *state, char *out,
                                          size_t out_size) {
    if (!out || out_size < 2) {
        return WATCHER_GIT_SUPERVISION_FAILED;
    }
    ...
    FILE *file = ani_fopen(output.path, "rb");
...
static watcher_git_status_t git_head(ani_watcher_t *w, project_state_t *state, char *out,
                                     size_t out_size) {
    if (!out || out_size < 2) {
        return WATCHER_GIT_SUPERVISION_FAILED;
    }
...
static watcher_git_status_t git_dirty_signature(ani_watcher_t *w, project_state_t *state,
...
static watcher_git_status_t git_file_count(ani_watcher_t *w, project_state_t *state,
                                           int *count_out) {
    if (!count_out) {
        return WATCHER_GIT_SUPERVISION_FAILED;
    }
...
static project_state_t *state_new(const char *name, const char *root_path) {
    project_state_t *s = calloc(ANI_ALLOC_ONE, sizeof(*s));
    if (!s) {
        return NULL;
    }
...
static bool defer_state_free(ani_watcher_t *w, project_state_t *s) {
    if (w->pending_free_count >= w->pending_free_cap) {
        ...
        if (!tmp) {
            ani_log_warn("watcher.unwatch.oom", "project", s->project_name);
            return false;
        }
        w->pending_free = tmp;
...
bool ani_watcher_root_missing_errno(int err) {
...
static root_status_t root_status(const char *root_path, int *out_errno) {
    ...
    *out_errno = errno;
    return ani_watcher_root_missing_errno(errno) ? ROOT_MISSING : ROOT_UNCERTAIN;
}

/* Sustained-absence window (seconds) before a missing root may be pruned.
 * Generous default: 10 minutes. Override with ANI_WATCHER_PRUNE_GRACE_S
 ...
 * same convention as ani_max_file_bytes in limits.c. */
static long prune_grace_s(void) {
    const char *raw = getenv("ANI_WATCHER_PRUNE_GRACE_S");
    if (raw && raw[0]) {
        errno = 0;
        char *end = NULL;
...
static const char *itoa_buf(int v) {
    static ANI_TLS char buf[ANI_SZ_32];
...
static bool watcher_unlink_cached_file(const char *project_name, const char *path,
    ...
    char errno_text[ANI_SZ_32];
    snprintf(errno_text, sizeof(errno_text), "%d", unlink_errno);
    ani_log_warn("watcher.root_prune_delete_failed", "project", project_name, "artifact", artifact,
                 "path", path, "errno", errno_text);
    return false;
}

static bool delete_cached_project_db(const char *project_name) {
    if (!ani_validate_project_name(project_name)) {
        return false;
    }
...
ani_watcher_t *ani_watcher_new(ani_store_t *store, ani_index_fn index_fn, void *user_data) {
    ani_watcher_t *w = calloc(ANI_ALLOC_ONE, sizeof(*w));
    if (!w) {
        return NULL;
    }
    w->store = store;
    ...
    w->projects = ani_ht_create(ANI_SZ_32);
    if (!w->projects) {
        free(w);
        return NULL;
    }
    ani_mutex_init(&w->projects_lock);
    ani_mutex_init(&w->coordination_lock);
    atomic_init(&w->stopped, 0);
    return w;
}

void ani_watcher_free(ani_watcher_t *w) {
    if (!w) {
        return;
    }
    /* Safety net: ensure stopped is set before draining pending_free.
     * In production the caller should ani_watcher_stop() + join first. */
    atomic_store(&w->stopped, 1);
    ani_mutex_lock(&w->coordination_lock);
    ani_mutex_lock(&w->projects_lock);
    ani_ht_foreach(w->projects, free_state_entry, NULL);
    ani_ht_free(w->projects);
    for (int i = 0; i < w->pending_free_count; i++) {
        state_free(w->pending_free[i]);
    }
    free(w->pending_free);
    ani_mutex_unlock(&w->projects_lock);
    ani_mutex_unlock(&w->coordination_lock);
    ani_mutex_destroy(&w->projects_lock);
    ani_mutex_destroy(&w->coordination_lock);
    free(w);
}

void ani_watcher_set_project_mutation_guard(ani_watcher_t *w,
                                            ani_watcher_project_mutation_begin_fn begin,
                                            ani_watcher_project_mutation_end_fn end,
                                            ani_watcher_project_pruned_fn pruned, void *context) {
    if (!w) {
        return;
    }
    ani_mutex_lock(&w->coordination_lock);
...
bool ani_watcher_watch(ani_watcher_t *w, const char *project_name, const char *root_path) {
    ...
    if (!s) {
        ani_log_warn("watcher.watch.oom", "project", project_name, "path", root_path);
        return false;
    }
    ...
    project_state_t *old = ani_ht_get(w->projects, project_name);
    ...
    if (old) {
        ...
        ani_ht_delete(w->projects, project_name);
    }
    ani_ht_set(w->projects, s->project_name, s);
    bool registered = ani_ht_get(w->projects, project_name) == s;
    if (!registered && old) {
        ...
        ani_ht_set(w->projects, old->project_name, old);
        if (ani_ht_get(w->projects, project_name) == old) {
            w->pending_free[--w->pending_free_count] = NULL;
        } else {
            atomic_store_explicit(&old->registered, false, memory_order_release);
            if (old->active_git) {
                (void)ani_subprocess_request_cancel(old->active_git);
            }
        }
    }
    if (registered && old) {
    ...
    if (!registered) {
        state_free(s);
        ani_log_warn("watcher.watch.failed", "project", project_name, "reason", "registration");
        return false;
    }
    ani_log_info("watcher.watch", "project", project_name, "path", root_path);
    return true;
}

void ani_watcher_unwatch(ani_watcher_t *w, const char *project_name) {
    if (!w || !project_name) {
        return;
    }
    bool removed = false;
    ani_mutex_lock(&w->projects_lock);
    project_state_t *s = ani_ht_get(w->projects, project_name);
    if (s && defer_state_free(w, s)) {
        ...
        if (s->active_git) {
            (void)ani_subprocess_request_cancel(s->active_git);
        }
        ani_ht_delete(w->projects, project_name);
        removed = true;
    }
    ani_mutex_unlock(&w->projects_lock);
    if (removed) {
        ani_log_info("watcher.unwatch", "project", project_name);
    }
}

void ani_watcher_touch(ani_watcher_t *w, const char *project_name) {
    if (!w || !project_name) {
        return;
    }
    ani_mutex_lock(&w->projects_lock);
...
int ani_watcher_watch_count(ani_watcher_t *w) {
    if (!w) {
        return 0;
    }
    ani_mutex_lock(&w->projects_lock);
    int count = (int)ani_ht_count(w->projects);
    ani_mutex_unlock(&w->projects_lock);
    return count;
}
...
static bool init_baseline(ani_watcher_t *w, project_state_t *s) {
    struct stat st;
    if (stat(s->root_path, &st) != 0) {
        ani_log_warn("watcher.root_gone", "project", s->project_name, "path", s->root_path);
    ...
    if (s->is_git && !git_has_own_dot_git(s->root_path)) {
        ...
        if (!tracked) {
            s->is_git = false;
            ani_log_info("watcher.nested_non_git", "project", s->project_name, "path",
                         s->root_path);
        }
    }
    ...
    if (s->is_git) {
        ...
        s->interval_ms = ani_watcher_poll_interval_ms(s->file_count);
        ani_log_info("watcher.baseline", "project", s->project_name, "strategy", "git", "files",
                     s->file_count > 0 ? "yes" : "0");
    } else {
        ani_log_info("watcher.baseline", "project", s->project_name, "strategy", "none");
    }
...
static bool check_changes(ani_watcher_t *w, project_state_t *s, bool *changed_out) {
    if (!changed_out) {
        return false;
    }
    *changed_out = false;
...
static void prune_missing_project(ani_watcher_t *w, project_state_t *s) {
    if (!w || !s || !s->project_name) {
        return;
    }

    char *project_name = ani_strdup(s->project_name);
    char *root_path = ani_strdup(s->root_path);
    ...
    ani_mutex_lock(&w->projects_lock);
    project_state_t *current = ani_ht_get(w->projects, project_name);
    /* Deferred free (same discipline as ani_watcher_unwatch): this state
    ...
    bool still_eligible =
        ...
        now_ms - current->first_missing_ms >= (uint64_t)prune_grace_s() * ANI_MSEC_PER_SEC &&
    ...
    if (removed) {
        ani_log_info("watcher.root_pruned", "project", project_name);
    }
    free(project_name);
    free(root_path);
}
...
static void poll_project(const char *key, void *val, void *ud) {
    ...
    if (rs == ROOT_UNCERTAIN) {
        ...
        ani_log_warn("watcher.root_stat_error", "project", s->project_name, "path", s->root_path,
    ...
    if (rs == ROOT_MISSING) {
        ...
        ani_log_warn("watcher.root_missing", "project", s->project_name, "path", s->root_path,
                     "polls", itoa_buf(s->missing_root_count));
        if (s->missing_root_count >= MISSING_ROOT_DELETE_AFTER &&
            now_ms - s->first_missing_ms >= (uint64_t)prune_grace_s() * ANI_MSEC_PER_SEC) {
            prune_missing_project(ctx->w, s);
        }
        return;
    }
    if (s->missing_root_count > 0) {
        ani_log_info("watcher.root_restored", "project", s->project_name, "path", s->root_path);
        s->missing_root_count = 0;
        s->first_missing_ms = 0;
    }
    ...
    ani_log_info("watcher.changed", "project", s->project_name, "strategy", "git");
    if (ctx->w->index_fn) {
        ...
        if (rc == 0) {
            ...
            ani_log_info("watcher.index.retry", "project", s->project_name);
        } else {
            ani_log_warn("watcher.index.err", "project", s->project_name);
        }
    }
...
int ani_watcher_poll_once(ani_watcher_t *w) {
    if (!w) {
        return 0;
    }
    ...
    ani_ht_foreach(w->projects, snapshot_project, &sc);
    ani_mutex_unlock(&w->projects_lock);
...
static void cancel_active_git_entry(const char *key, void *value, void *user_data) {
    ...
    if (state && state->active_git) {
        (void)ani_subprocess_request_cancel(state->active_git);
    }
}

void ani_watcher_stop(ani_watcher_t *w) {
    if (w) {
        atomic_store_explicit(&w->stopped, 1, memory_order_release);
        ani_mutex_lock(&w->projects_lock);
        ani_ht_foreach(w->projects, cancel_active_git_entry, NULL);
        for (int i = 0; i < w->pending_free_count; i++) {
...
int ani_watcher_run(ani_watcher_t *w, int base_interval_ms) {
    if (!w) {
    ...
    ani_log_info("watcher.start", "interval_ms", base_interval_ms > 999 ? "multi-sec" : "fast");
    ...
    while (!atomic_load(&w->stopped)) {
        ...
        while (slept < base_interval_ms && !atomic_load(&w->stopped)) {
            ...
            ani_usleep((unsigned)chunk * ANI_MSEC_PER_SEC);
            slept += chunk;
        }
    }

    ani_log_info("watcher.stop");
    return 0;
}
