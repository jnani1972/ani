...
struct ani_activation_transaction {
...
static ani_activation_transaction_before_absent_publish_for_test_fn
...
#endif
    int written =
        snprintf(path, needed, "%s%s.ani-%s-%lu-%" PRIu64, directory,
...
static bool activation_posix_acl_empty(int descriptor) {
    return ani_macos_extended_acl_fd_is_empty(descriptor);
}
            ...
            if (next_ok && *remaining) {
                if (!activation_posix_intermediate_secure(&next_status)) {
                    ...
                    (void)snprintf(mode_text, sizeof(mode_text), "%04o",
                                   (unsigned)(next_status.st_mode & 07777));
                    ani_log_warn("activation.ancestor_group_writable", "path", walk_path, "mode",
                                 mode_text);
                }
            }
            if (next_ok) {
...
static bool activation_directory_still_valid(const ani_activation_transaction_t *transaction) {
#ifdef _WIN32
    int ignored = 0;
...
static activation_create_status_t activation_private_file_create(
    const ani_activation_transaction_t *transaction, const char *path, const char *name,
...
static ani_activation_transaction_status_t activation_create_unique(
    const ani_activation_transaction_t *transaction, const char *tag, char **path_out,
    ...
    *file_out = ACTIVATION_INVALID_FILE;
    for (unsigned int attempt = 0; attempt < 1024U; attempt++) {
        ...
        if (!candidate) {
            return ANI_ACTIVATION_TRANSACTION_NO_MEMORY;
        }
        ...
        if (created == ACTIVATION_CREATE_OK) {
            *path_out = candidate;
            *name_out = name;
            return ANI_ACTIVATION_TRANSACTION_OK;
        }
        free(candidate);
        free(name);
        if (created != ACTIVATION_CREATE_EXISTS) {
            return ANI_ACTIVATION_TRANSACTION_IO;
        }
    }
    return ANI_ACTIVATION_TRANSACTION_IO;
}
...
static bool activation_posix_entry_snapshot_with_links(
    const ani_activation_transaction_t *transaction, const char *name, nlink_t required_links,
...
static bool activation_entry_snapshot(const ani_activation_transaction_t *transaction,
...
static bool activation_path_matches(const ani_activation_transaction_t *transaction,
...
static bool activation_sync_directory(const ani_activation_transaction_t *transaction) {
#ifdef _WIN32
...
static bool activation_rename(const ani_activation_transaction_t *transaction, const char *source,
...
static void activation_transaction_destroy(ani_activation_transaction_t *transaction) {
    if (!transaction) {
        return;
    }
#ifndef _WIN32
...
static ani_activation_transaction_status_t activation_discard_staged_assets(
    ...
    return ok ? ANI_ACTIVATION_TRANSACTION_OK : ANI_ACTIVATION_TRANSACTION_IO;
}

static ani_activation_transaction_status_t activation_transaction_prepare(
    const char *target_path, activation_action_t action,
    ani_activation_transaction_t **transaction_out) {
    *transaction_out = NULL;
    activation_refusal_clear();
    ani_activation_transaction_t *transaction = calloc(1, sizeof(*transaction));
    if (!transaction) {
        return ANI_ACTIVATION_TRANSACTION_NO_MEMORY;
    }
#ifndef _WIN32
    ...
    if (!activation_target_parts(target_path, &transaction->directory_path,
        ...
        activation_transaction_destroy(transaction);
        return ANI_ACTIVATION_TRANSACTION_INVALID_ARGUMENT;
    }
#ifdef _WIN32
    int ignored = 0;
    ...
    if (transaction->target_existed) {
        ...
        ani_activation_transaction_status_t status = activation_create_unique(
...
static void activation_failed_stage_cleanup(ani_activation_transaction_t *transaction) {
    if (transaction) {
        (void)activation_discard_staged_assets(transaction);
        activation_transaction_destroy(transaction);
    }
...
#ifdef _WIN32
static bool activation_windows_copy_target_to_backup(ani_activation_transaction_t *transaction) {
    if (!activation_directory_still_valid(transaction)) {
        return false;
    }
...
#ifndef _WIN32
static bool activation_linked_backup_pair_valid(const ani_activation_transaction_t *transaction) {
    activation_file_identity_t target_identity;
    activation_file_identity_t backup_identity;
    bool target_exists = false;
...
static bool activation_remove_linked_backup(ani_activation_transaction_t *transaction) {
...
static bool activation_target_still_original(const ani_activation_transaction_t *transaction) {
    activation_file_identity_t current;
    bool exists = false;
    if (!activation_entry_snapshot(transaction, transaction->target_path, transaction->target_name,
                                   &exists, &current) ||
...
static bool activation_absent_target_snapshot(const ani_activation_transaction_t *transaction,
...
static bool activation_remove_absent_published_target(ani_activation_transaction_t *transaction,
                                                      bool linked_pair) {
    if (!linked_pair) {
...
static ani_activation_transaction_status_t activation_rollback_internal(
    ani_activation_transaction_t *transaction) {
    if (transaction->target_existed) {
        bool backup_exists = false;
        if (!transaction->backup_contains_target ||
            ...
            return ANI_ACTIVATION_TRANSACTION_ROLLBACK_FAILED;
        }
        bool target_exists = false;
    ...
    return activation_sync_directory(transaction) ? ANI_ACTIVATION_TRANSACTION_OK
...
ani_activation_transaction_status_t ani_activation_transaction_commit(
    ...
    if (transaction->state != ACTIVATION_STAGED) {
        return ANI_ACTIVATION_TRANSACTION_INVALID_STATE;
    }
    ...
    if (transaction->target_existed) {
        if (transaction->action == ACTIVATION_REPLACE) {
            ...
            if (published == ACTIVATION_PUBLISH_CHANGED_ERROR) {
                transaction->state = ACTIVATION_RECOVERY_NEEDED;
                ani_activation_transaction_status_t restored =
                    activation_rollback_internal(transaction);
                return restored == ANI_ACTIVATION_TRANSACTION_OK
                           ? ANI_ACTIVATION_TRANSACTION_IO
                           : ANI_ACTIVATION_TRANSACTION_ROLLBACK_FAILED;
            }
        } else {
    ...
    if (!activation_directory_still_valid(transaction) || !activation_sync_directory(transaction)) {
        ani_activation_transaction_status_t restored = activation_rollback_internal(transaction);
        return restored == ANI_ACTIVATION_TRANSACTION_OK
                   ? ANI_ACTIVATION_TRANSACTION_IO
                   : ANI_ACTIVATION_TRANSACTION_ROLLBACK_FAILED;
    }
...
ani_activation_transaction_status_t ani_activation_transaction_finalize(
    ...
    if (transaction->backup_contains_target) {
        ...
        if (removed == ACTIVATION_UNLINK_DEFERRED) {
            ...
            return ANI_ACTIVATION_TRANSACTION_DEFERRED;
        }
        transaction->backup_exists = false;
        transaction->backup_contains_target = false;
    }
...
ani_activation_transaction_status_t ani_activation_transaction_close(
    ...
    ani_activation_transaction_t *transaction = *transaction_io;
    if (transaction->state == ACTIVATION_COMMITTED ||
        transaction->state == ACTIVATION_RECOVERY_NEEDED) {
        ani_activation_transaction_status_t status = activation_rollback_internal(transaction);
        if (status != ANI_ACTIVATION_TRANSACTION_OK) {
            return status;
        }
    }
    if (transaction->state == ACTIVATION_STAGED || transaction->state == ACTIVATION_ROLLED_BACK) {
        ani_activation_transaction_status_t status = activation_discard_staged_assets(transaction);
        if (status != ANI_ACTIVATION_TRANSACTION_OK) {
            return status;
        }
    }
...
const char *ani_activation_transaction_target_path(
    const ani_activation_transaction_t *transaction) {
    return transaction ? transaction->target_path : NULL;
}

const char *ani_activation_transaction_staged_path(
    const ani_activation_transaction_t *transaction) {
    return transaction ? transaction->staged_path : NULL;
}

const char *ani_activation_transaction_backup_path(
    const ani_activation_transaction_t *transaction) {
