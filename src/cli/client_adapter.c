...
static void sb_append(adapter_sb_t *sb, const char *s) {
    ...
    if (sb->len + add + 1 > sb->cap) {
        size_t want = sb->cap ? sb->cap : ANI_SZ_2K;
 ...
 * ani_client_adapter_escape_js imposes, so long registry descriptions fit. */
...
static void emit_header(adapter_sb_t *sb, const char *client) {
    /* No markers here: ani_text_upsert_managed_block adds them and REJECTS
     * content that already carries them, so the body must stay marker-free. */
    sb_append(sb, "// Generated by ani for ");
    sb_append(sb, client);
    sb_append(sb,
              ".\n"
...
char *ani_client_adapter_pi(const char *binary_path) {
    ...
    char bin[ANI_SZ_1K];
    if (!ani_client_adapter_escape_js(binary_path, bin, sizeof(bin))) {
        return NULL;
    }
    ...
    sb_append(
        &sb, "async function call(tool, args, signal) {\n"
             ...
             "      env: { ...process.env, ANI_LOG_LEVEL: 'error' },\n"
             "    });\n"
             "    let out = '';\n"
             "    let stdinError;\n"
             ...
             "      }\n"
             "      resolve({ error: 'no JSON response from ani' });\n"
             "    });\n"
    ...
    for (int i = 0; i < count; i++) {
        ...
        const char *title = ani_mcp_tool_title(name);
        const char *description = ani_mcp_tool_description(name);
