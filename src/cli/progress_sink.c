...
static ani_mutex_t s_sink_mutex;
...
static void progress_tool_name(const char *tool_name, char out[ANI_SZ_64]) {
    size_t offset = 0;
    if (tool_name) {
        for (; tool_name[offset] && offset + 1 < ANI_SZ_64; offset++) {
    ...
    if (offset == 0) {
        (void)snprintf(out, ANI_SZ_64, "%s", "tool");
    } else {
        out[offset] = '\0';
    }
}
...
void ani_progress_sink_init(FILE *out) {
    ...
    ani_mutex_lock(&s_sink_mutex);
    s_out = out ? out : stderr;
    atomic_store(&s_needs_newline, 0);
    s_gbuf_nodes = NOT_SET;
    s_gbuf_edges = NOT_SET;
    ani_log_set_sink(ani_progress_sink_fn);
    ani_mutex_unlock(&s_sink_mutex);
}
...
static void on_gbuf_dump(const char *line) {
    ...
    if (extract_kv(line, "nodes", n, (int)sizeof(n))) {
        s_gbuf_nodes = (int)strtol(n, NULL, ANI_DECIMAL_BASE);
    }
    if (extract_kv(line, "edges", e, (int)sizeof(e))) {
        s_gbuf_edges = (int)strtol(e, NULL, ANI_DECIMAL_BASE);
    }
}
...
static void on_extract_progress(const char *line) {
    char done[ANI_SZ_32] = {0};
    char total[ANI_SZ_32] = {0};
    if (extract_kv(line, "done", done, (int)sizeof(done)) &&
        extract_kv(line, "total", total, (int)sizeof(total))) {
        long d = strtol(done, NULL, ANI_DECIMAL_BASE);
        long t = strtol(total, NULL, ANI_DECIMAL_BASE);
