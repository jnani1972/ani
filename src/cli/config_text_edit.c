...
static ANI_TLS ani_text_precommit_test_hook_t text_precommit_test_hook = NULL;
static ANI_TLS void *text_precommit_test_context = NULL;
static ANI_TLS ani_text_precommit_test_hook_t text_prepublish_test_hook = NULL;
static ANI_TLS void *text_prepublish_test_context = NULL;
static ANI_TLS ani_text_precommit_test_hook_t text_temp_closed_test_hook = NULL;
static ANI_TLS void *text_temp_closed_test_context = NULL;
#endif
...
#ifdef _WIN32
    intptr_t native_handle = _get_osfhandle(ani_fileno(file));
    return native_handle == -1
               ? TEXT_ERROR
               : text_snapshot_from_handle((HANDLE)(uintptr_t)native_handle, snapshot);
#else
    struct stat state;
    return fstat(ani_fileno(file), &state) == 0 ? text_snapshot_from_stat(&state, snapshot)
                                                : TEXT_ERROR;
#endif
}
...
static int text_write_atomic_mode(const char *path, const char *new_data, size_t new_len,
    ...
    for (unsigned attempt = 0U; attempt < TEXT_TEMP_ATTEMPTS; attempt++) {
        ...
        int written = snprintf(temp_path, capacity, "%s.ani-text-%ld-%u.tmp", path,
...
#endif
    if (!failed && TEXT_SYNC(ani_fileno(file)) != 0) {
        failed = 1;
    }
