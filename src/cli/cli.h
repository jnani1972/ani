 ...
 *
 * Port of Go cmd/ani/ install/update/config logic.
 *
...
#define ANI_SKILL_COUNT 1

/* Skill name/content pair. */
typedef struct {
    const char *name;    /* e.g. "ani" */
 ...
 * Format: { "mcpServers": { "ani": { "command": binary_path } } }
 ...
 * Format: { "servers": { "ani": { "type": "stdio", "command": binary_path } } }
 * Returns 0 on success. */
 ...
 * Format: { "context_servers": { "ani": { "command": path, "args": [] } } }
 * Returns 0 on success. */
...
bool ani_cli_clients_apply_selection_for_testing(const char *spec, ani_detected_agents_t *detected);
...
void ani_hook_sanitize_metadata_for_testing(const char *input, char *output, size_t output_size);
char *ani_hook_augment_format_context_for_testing(const char *envelope, const char *token,
                                                  bool *is_error);
...
char *ani_hook_augment_lifecycle_json_for_dialect(const char *input, const char *forced_event,
                                                  const char *dialect);
char *ani_hook_augment_tool_json_for_testing(const char *input, const char *dialect,
                                             const char *context, char *path, size_t path_size);
bool ani_hook_augment_invocation_supported_for_testing(const char *dialect,
                                                       const char *forced_event);
bool ani_hook_path_contains_for_testing(const char *root, const char *candidate,
                                        bool case_insensitive);
const char *ani_hook_no_project_index_guidance_for_testing(const char *event);
bool ani_hook_augment_parse_bash_pattern_for_testing(const char *cmd, char *out, size_t out_sz);
...
/* Upsert a ani instruction section in a markdown file.
 * Uses <!-- ani:start --> / <!-- ani:end --> markers.
...
/* Remove the ani instruction section from a markdown file.
...
typedef void (*ani_hook_json_prewrite_test_hook_t)(const char *settings_path, void *context);
...
/* ── Codex instructions (legacy, wraps ani_get_agent_instructions) ── */
...
/* Extract a binary named "ani*" from a tar.gz buffer.
 * Returns malloc'd binary content and sets *out_len.
 * Returns NULL on error. Caller must free. */
unsigned char *ani_extract_binary_from_targz(const unsigned char *data, int data_len, int *out_len);

/* Extract the ani binary from a zip archive in memory.
 * Returns malloc'd binary content and sets *out_len.
 * Returns NULL on error. Caller must free. */
...
/* List .db files in the cache directory (~/.cache/ani/).
...
typedef struct ani_config ani_config_t;
...
#define ANI_CONFIG_AUTO_INDEX "auto_index"
#define ANI_CONFIG_AUTO_INDEX_LIMIT "auto_index_limit"
#define ANI_CONFIG_AUTO_WATCH "auto_watch"
#define ANI_CONFIG_UI_LANG "ui-lang"
#define ANI_CONFIG_WATCHER_ENABLED "watcher_enabled"
...
#define ANI_CONFIG_UI_ENABLED "ui_enabled"
#define ANI_CONFIG_UI_PORT "ui_port"
...
typedef void *ani_cli_activation_lock_t;
typedef int (*ani_cli_activation_mutation_fn)(void *context);
 ...
 * quiescence of active modern ANI processes before returning an exclusive
...
typedef int (*ani_cli_activation_reserve_fn)(void *context, ani_cli_activation_lock_t *lease_out);
typedef void (*ani_cli_activation_release_fn)(void *context, ani_cli_activation_lock_t lease);
...
typedef struct {
    void *context;
    ani_cli_activation_reserve_fn reserve_for_mutation;
    ani_cli_activation_release_fn mutation_lease_release;
    void (*visible_diagnostic)(void *context, const char *message);
} ani_cli_activation_ops_t;
...
int ani_cmd_hook_augment(int argc, char **argv);
...
char *ani_hook_augment_lifecycle_json(const char *input);
...
char *ani_hook_augment_lifecycle_json_for(const char *input, const char *forced_event,
...
void ani_hook_augment_arm_deadline(void);
char *ani_hook_augment_read_stdin(void);
/* Pure no-op gate for the hook-client fast path (see hook_augment.c). */
bool ani_hook_augment_input_is_noop_bash(const char *input);
/* Hand back stdin bytes main() consumed early; the next read returns them. */
void ani_hook_augment_prefetch_stdin(char *owned);
...
typedef enum {
    ANI_HOOK_ADMISSION_DAEMON_ABSENT = 0, /* no daemon running: `daemon start` heals it */
    ANI_HOOK_ADMISSION_BUILD_CONFLICT     /* daemon runs another build: needs `daemon stop` */
} ani_hook_admission_t;
...
const char *ani_hook_admission_notice(ani_hook_admission_t reason, const char *hook_dialect);
...
char *ani_hook_augment_process(ani_mcp_server_t *srv, const char *input_json);
...
bool ani_hook_augment_invocation_supported(const char *forced_event, const char *dialect_name);
char *ani_hook_augment_process_for(ani_mcp_server_t *srv, const char *input_json,
 ...
 * no-op (#618). */
bool ani_hook_path_is_abs(const char *path);
