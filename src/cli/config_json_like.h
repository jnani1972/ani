 ...
 */
#ifndef ANI_CONFIG_JSON_LIKE_H
#define ANI_CONFIG_JSON_LIKE_H
 ...
 * members produce ANI_JSON_LIKE_OBJECT_MISMATCH. STRING fields may require an
...
typedef enum {
    ...
    ANI_JSON_LIKE_VALUE_EMPTY_ARRAY,
    ANI_JSON_LIKE_VALUE_SINGLE_STRING_ARRAY,
} ani_json_like_value_shape_t;

enum {
    ANI_JSON_LIKE_FIELD_REQUIRED = 1U << 0U,
    ANI_JSON_LIKE_FIELD_CAPTURE_STRING = 1U << 1U,
};

typedef struct {
    const char *key;
    ani_json_like_value_shape_t shape;
    const char *expected_string;
    unsigned flags;
} ani_json_like_object_field_t;

enum {
    ANI_JSON_LIKE_OBJECT_MATCH = 0,
    ANI_JSON_LIKE_OBJECT_MISSING = 1,
    ANI_JSON_LIKE_OBJECT_MISMATCH = 2,
    ...
    ANI_JSON_LIKE_OBJECT_MATCH_WITH_EXTRAS = 3,
};

/* captured_string_out receives malloc-owned decoded content only on MATCH.
 * Returns one of ANI_JSON_LIKE_OBJECT_* or -1 for malformed input. */
...
/* Top-level convenience wrapper for ani_json_like_add_unique_string_at_path. */
int ani_json_like_add_unique_string(const char *file_path, const char *array_key,
                                    const char *string_value);

/* Top-level convenience wrapper for ani_json_like_remove_string_at_path. */
...
typedef void (*ani_json_like_precommit_test_hook_t)(const char *file_path, void *context);
