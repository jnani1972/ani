 ...
 * (ani_mcp_tool_count / ani_mcp_tool_name), so it cannot drift: adding a tool to
 ...
 */
#ifndef ANI_CLI_CLIENT_ADAPTER_H
#define ANI_CLI_CLIENT_ADAPTER_H
...
#define ANI_ADAPTER_MARKER_START "// ani:start"
#define ANI_ADAPTER_MARKER_END "// ani:end"
 ...
 * path such as C:\Users\x\bin\ani.exe must not corrupt the module or, worse,
...
char *ani_client_adapter_pi(const char *binary_path);

/* Emit an OpenCode plugin that augments grep/glob results from the graph.
 *
 * OpenCode HAS MCP (see ani_upsert_opencode_mcp), so this adds no tools — it
 ...
 *
 * Returns NULL on allocation failure or when binary_path is NULL/empty. */
char *ani_client_adapter_opencode(const char *binary_path);
...
bool ani_client_adapter_escape_js(const char *in, char *out, size_t out_sz);

#endif /* ANI_CLI_CLIENT_ADAPTER_H */
