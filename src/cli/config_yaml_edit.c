...
#ifdef ANI_YAML_ENABLE_TEST_API
static ani_yaml_precommit_test_hook_t yaml_precommit_test_hook = NULL;
static void *yaml_precommit_test_context = NULL;
static ani_yaml_precommit_test_hook_t yaml_prepublish_test_hook = NULL;
static void *yaml_prepublish_test_context = NULL;
static ani_yaml_lock_postcreate_test_hook_t yaml_lock_postcreate_test_hook = NULL;
...
static int yaml_build_lock_path(const char *path, char **out_path) {
    static const char suffix[] = ".ani-yaml.lock";
    size_t path_len = 0U;
...
#ifdef _WIN32
    wchar_t *wide_path = ani_utf8_to_wide(lock->path);
    if (!wide_path) {
        free(lock->path);
        lock->path = NULL;
        return YAML_ERROR;
...
static int yaml_write_atomic(const char *path, const char *data, size_t len,
    ...
    for (unsigned attempt = 0U; attempt < YAML_TMP_ATTEMPTS; attempt++) {
        ...
        int written = snprintf(temp_path, capacity, "%s.ani-yaml-%ld-%u.tmp", path,
...
#endif
    if (!failed && YAML_SYNC(ani_fileno(file)) != 0) {
        failed = true;
    }
    if (fclose(file) != 0) {
        failed = true;
...
#ifdef ANI_YAML_ENABLE_TEST_API
void ani_yaml_set_precommit_hook_for_testing(ani_yaml_precommit_test_hook_t hook, void *context) {
...
int ani_yaml_encode_double_quoted_scalar(const char *value, char **encoded_out) {
    if (!encoded_out) {
        return YAML_ERROR;
    }
    *encoded_out = NULL;
    ...
    if (target.identity_found) {
        ...
        if (existing_len != rendered.len ||
            ...
            if (yaml_splice(&out, &doc, target.identity_start, target.identity_end, NULL, 0U) !=
                ...
                result = ANI_YAML_IDENTITY_EDIT_ERROR;
            }
            yaml_buf_free(&out);
        }
    } else if (!remove) {
 ...
 *   basename must be ani[.exe]. Anything else stays FOREIGN
 *   (fail-closed). */
static bool yaml_owned_value_is_our_binary(const char *v, size_t vlen) {
    ...
    static const char plain[] = "ani";
    static const char exe[] = "ani.exe";
...
static bool yaml_owned_entry_is_prior_shape(const char *data, size_t len, const char *entry_key,
    ...
    static const char goose_name[] = "    name: ani";
...
static int yaml_edit_owned_mapping_entry_locked(const char *file_path, const char *section_key,
    ...
    if (target.entry_found) {
        ...
        if (!canonical_match &&
            ...
            yaml_doc_free(&doc);
            free(data);
            return ANI_YAML_IDENTITY_EDIT_FOREIGN;
        }
    ...
    yaml_doc_free(&doc);
    free(data);
    return result == 0 ? ANI_YAML_IDENTITY_EDIT_OK : ANI_YAML_IDENTITY_EDIT_ERROR;
}
...
int ani_yaml_upsert_mapping_entry(const char *file_path, const char *section_key,
                                  const char *entry_key, const char *entry_block) {
...
int ani_yaml_remove_mapping_entry(const char *file_path, const char *section_key,
                                  const char *entry_key) {
    yaml_config_lock_t lock;
    if (yaml_lock_acquire(file_path, &lock) != 0) {
        return YAML_ERROR;
...
int ani_yaml_upsert_owned_mapping_entry(const char *file_path, const char *section_key,
    ...
    int release_result = yaml_lock_release(&lock);
    return release_result == 0 ? result : ANI_YAML_IDENTITY_EDIT_ERROR;
}

int ani_yaml_remove_owned_mapping_entry(const char *file_path, const char *section_key,
                                        const char *entry_key, const char *canonical_entry_block) {
    yaml_config_lock_t lock;
    if (yaml_lock_acquire(file_path, &lock) != 0) {
        return ANI_YAML_IDENTITY_EDIT_ERROR;
...
int ani_yaml_upsert_mapping_sequence_item(const char *file_path, const char *const *sequence_path,
...
int ani_yaml_remove_mapping_sequence_item(const char *file_path, const char *const *sequence_path,
                                          size_t sequence_path_len, const char *identity_key,
                                          const char *identity_scalar, const char *canonical_item) {
    yaml_config_lock_t lock;
    if (yaml_lock_acquire(file_path, &lock) != 0) {
...
int ani_yaml_upsert_string_list_item(const char *file_path, const char *key, const char *item) {
    yaml_config_lock_t lock;
    if (yaml_lock_acquire(file_path, &lock) != 0) {
        return YAML_ERROR;
    }
...
int ani_yaml_remove_string_list_item(const char *file_path, const char *key, const char *item) {
    yaml_config_lock_t lock;
    if (yaml_lock_acquire(file_path, &lock) != 0) {
        return YAML_ERROR;
    }
