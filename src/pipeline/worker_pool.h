 ...
 */
#ifndef ANI_WORKER_POOL_H
#define ANI_WORKER_POOL_H

#include <stdbool.h>

/* Worker callback: called once per iteration with index [0..count-1]. */
typedef void (*ani_parallel_fn)(int idx, void *ctx);

/* Options for parallel dispatch. */
typedef struct {
    int max_workers;     /* 0 = auto-detect from ani_default_worker_count */
    bool force_pthreads; /* unused, kept for API compat */
} ani_parallel_for_opts_t;
