 ...
 * supported)
 *
 * Depends on: pass_infrascan.c (ani_is_kustomize_file, ani_is_k8s_manifest, ani_infra_qn),
 *             extraction layer (ani.h), graph_buffer, pipeline internals.
 */
...
static void handle_kustomize(ani_pipeline_ctx_t *ctx, const char *path, const char *rel_path,
                             ANIFileResult *result) {
    /* Emit Module node for this kustomize overlay file */
    char *mod_qn = ani_infra_qn(ctx->project_name, rel_path, "kustomize", NULL);
    if (!mod_qn) {
        return;
    }

    int64_t mod_id = ani_gbuf_upsert_node(ctx->gbuf, "Module", k8s_basename(rel_path), mod_qn,
    ...
    if (!res) {
        ...
        if (source) {
            res = ani_extract_file(source, src_len, ANI_LANG_KUSTOMIZE, ctx->project_name, rel_path,
                                   ANI_EXTRACT_BUDGET, NULL, NULL);
            free(source);
            allocated = true;
        }
    }
    ...
    if (res) {
        for (int j = 0; j < res->imports.count; j++) {
            ANIImport *imp = &res->imports.items[j];
            if (!imp->module_path) {
                continue;
            }
            ...
            char *target_qn =
                ani_pipeline_fqn_compute(ctx->project_name, imp->module_path, "__file__");
            if (!target_qn) {
                continue;
            }
            ...
            if (target) {
                ani_gbuf_insert_edge(ctx->gbuf, mod_id, target->id, "IMPORTS",
                                     "{\"via\":\"kustomize\"}");
                import_count++;
            }
        }
    ...
    ani_log_info("pass.k8s.kustomize", "file", rel_path, "imports", itoa_k8s(import_count));
}
...
static void k8s_link_selectors(ani_pipeline_ctx_t *ctx, const k8s_record_array_t *recs) {
    int edges = 0;
        ...
        for (int j = 0; j < recs->count; j++) {
            ...
            if (k8s_selector_matches(svc, wl)) {
                ...
                snprintf(props, sizeof(props),
                         ...
                         svc->name, wl->name);
                ani_gbuf_insert_edge(ctx->gbuf, svc->node_id, wl->node_id, "INFRA_MAPS", props);
                edges++;
            }
        }
    }
    if (edges > 0) {
        ani_log_info("pass.k8s.selectors", "linked", itoa_k8s(edges));
    }
}
...
static void handle_k8s_manifest(ani_pipeline_ctx_t *ctx, const char *path, const char *rel_path,
                                const char *source, int src_len, k8s_record_t *rec) {
    (void)path; /* retained for symmetry; source is always provided now */
    int resource_count = 0;

    ANIFileResult *res = ani_extract_file(source, src_len, ANI_LANG_K8S, ctx->project_name,
                                          rel_path, ANI_EXTRACT_BUDGET, NULL, NULL);
    if (!res) {
        return;
    }
    ...
    const ani_gbuf_node_t *file_node = file_qn ? ani_gbuf_find_by_qn(ctx->gbuf, file_qn) : NULL;
    free(file_qn);

    for (int d = 0; d < res->defs.count; d++) {
        ANIDefinition *def = &res->defs.items[d];
        ...
        int64_t node_id =
            ani_gbuf_upsert_node(ctx->gbuf, "Resource", def->name, def->qualified_name, rel_path,
    ...
    ani_log_info("pass.k8s.manifest", "file", rel_path, "resources", itoa_k8s(resource_count));
}
...
static void handle_helm_chart(ani_pipeline_ctx_t *ctx, const char *rel_path, const char *source) {
    ani_helm_chart_t hc;
    if (ani_parse_helm_chart(source, &hc) != 0) {
        return;
    }

    const char *cname = hc.chart_name[0] ? hc.chart_name : k8s_basename(rel_path);
    char *chart_qn = ani_infra_qn(ctx->project_name, rel_path, "helm-chart", NULL);
    if (!chart_qn) {
        return;
    }
    int64_t chart_id = ani_gbuf_upsert_node(ctx->gbuf, "Chart", cname, chart_qn, rel_path, SKIP_ONE,
                                            0, "{\"source\":\"helm\"}");
    free(chart_qn);
    ...
    if (file_node && chart_id > 0) {
        ani_gbuf_insert_edge(ctx->gbuf, file_node->id, chart_id, "DEFINES", "{}");
    }
    free(file_qn);
    ...
    for (int i = 0; i < hc.dep_count && chart_id > 0; i++) {
        ...
        char dep_qn[ANI_SZ_512];
        snprintf(dep_qn, sizeof(dep_qn), "%s.__helm_dep__.%s", ctx->project_name, hc.deps[i]);
        int64_t dep_id =
            ani_gbuf_upsert_node(ctx->gbuf, "Chart", hc.deps[i], dep_qn, rel_path, SKIP_ONE, 0,
                                 "{\"source\":\"helm\",\"external\":true}");
        if (dep_id > 0) {
            ani_gbuf_insert_edge(ctx->gbuf, chart_id, dep_id, "DEPENDS_ON", "{}");
            dep_edges++;
        }
    }
    ani_log_info("pass.k8s.helm", "file", rel_path, "deps", itoa_k8s(dep_edges));
}
...
static int emit_dep_edge(ani_pipeline_ctx_t *ctx, const ani_gbuf_node_t *src, const char *rel_path,
    ...
    char dep_props[ANI_SZ_256];
    snprintf(dep_props, sizeof(dep_props), "{\"source\":\"%s\",\"external\":true}", ecosystem);
    int64_t dep_id =
        ani_gbuf_upsert_node(ctx->gbuf, "Package", name, dep_qn, rel_path, SKIP_ONE, 0, dep_props);
    if (dep_id > 0 && dep_id != src->id) {
        ani_gbuf_insert_edge(ctx->gbuf, src->id, dep_id, "DEPENDS_ON", "{}");
        return 1;
    }
    return 0;
}
...
static int parse_gomod_deps(ani_pipeline_ctx_t *ctx, const ani_gbuf_node_t *src,
...
static int parse_requirements_deps(ani_pipeline_ctx_t *ctx, const ani_gbuf_node_t *src,
                                   const char *rel_path, const char *source) {
    int edges = 0;
    const char *p = source;
    while (p && *p) {
...
static void handle_dep_manifest(ani_pipeline_ctx_t *ctx, const char *rel_path, const char *source,
                                const char *ecosystem) {
    if (!source) {
        return;
    }
    char *file_qn = ani_pipeline_fqn_compute(ctx->project_name, rel_path, "__file__");
    const ani_gbuf_node_t *src = file_qn ? ani_gbuf_find_by_qn(ctx->gbuf, file_qn) : NULL;
    free(file_qn);
    if (!src) {
        return;
    }
    ...
    ani_log_info("pass.k8s.depmanifest", "file", rel_path, "deps", itoa_k8s(dep_edges));
}

/* ── Pass entry point ────────────────────────────────────────────── */

int ani_pipeline_pass_k8s(ani_pipeline_ctx_t *ctx, const ani_file_info_t *files, int file_count) {
    ani_log_info("pass.start", "pass", "k8s", "files", itoa_k8s(file_count));
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        ANIFileResult *cached =
            (ctx->result_cache && ctx->result_cache[i]) ? ctx->result_cache[i] : NULL;

        if (is_gomod_file(base) || lang == ANI_LANG_GOMOD || is_requirements_file(base)) {
            int dep_len = 0;
        ...
        } else if (ani_is_kustomize_file(base)) {
            handle_kustomize(ctx, path, rel, cached);
            kustomize_count++;
        } else if (lang == ANI_LANG_YAML || lang == ANI_LANG_K8S) {
            ...
            if (source) {
                if (is_helm_chart_file(base)) {
                    ...
                    helm_count++;
                } else if (ani_is_k8s_manifest(base, source)) {
                    /* Always re-extract with ANI_LANG_K8S regardless of any cached
    ...
    ani_log_info("pass.done", "pass", "k8s", "kustomize", itoa_k8s(kustomize_count), "manifests",
