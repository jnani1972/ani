...
static void deferred_buf_push(deferred_edge_buf_t *buf, int64_t src, int64_t tgt, float score,
    ...
    if (buf->count >= buf->cap) {
        int nc = buf->cap < ANI_SZ_256 ? ANI_SZ_256 : buf->cap * GROW;
...
static int inject_calls_pattern_tokens(const ani_gbuf_node_t *n, const ani_gbuf_t *gbuf,
    ...
    int ec = 0;
    if (ani_gbuf_find_edges_by_source_type(gbuf, n->id, "CALLS", &edges, &ec) != 0) {
        return count;
    }
    for (int e = 0; e < ec && count < max_tokens; e++) {
        const ani_gbuf_node_t *t = ani_gbuf_find_by_id(gbuf, edges[e]->target_id);
        if (!t || !t->name) {
            continue;
        }
...
static int inject_pattern_tokens(const ani_gbuf_node_t *n, const ani_gbuf_t *gbuf, char **tokens,
                                 int count, int max_tokens) {
    if (!n || count >= max_tokens) {
        return count;
    }

    char bt_buf[ANI_SZ_512];
    const char *bt = n->properties_json
                         ? json_str_value(n->properties_json, "bt", bt_buf, sizeof(bt_buf))
                         : NULL;
    char dec_buf[ANI_SZ_256];
...
static int tokenize_json_string_field(const char *json, const char *key, char **tokens, int count,
    ...
    count += ani_sem_tokenize(buf, tokens + count, max_tokens - count);
    return count;
}
...
static int tokenize_json_array_field(const char *json, const char *key, char **tokens, int count,
    ...
    char *arr[ANI_SZ_16];
    int n = json_str_array(json, key, arr, ANI_SZ_16);
    for (int p = 0; p < n; p++) {
        if (count < max_tokens) {
            count += ani_sem_tokenize(arr[p], tokens + count, max_tokens - count);
        }
        free(arr[p]);
    }
    return count;
...
static const char **collect_sorted_call_neighbors(const ani_gbuf_t *gbuf, int64_t node_id,
                                                  bool outbound, int *out_n) {
    *out_n = 0;
    const ani_gbuf_edge_t **edges = NULL;
    int ec = 0;
    int rc = outbound ? ani_gbuf_find_edges_by_source_type(gbuf, node_id, "CALLS", &edges, &ec)
                      : ani_gbuf_find_edges_by_target_type(gbuf, node_id, "CALLS", &edges, &ec);
    if (rc != 0 || ec <= 0) {
        return NULL;
    }
    ...
    for (int e = 0; e < ec; e++) {
        ...
        const ani_gbuf_node_t *neighbor = ani_gbuf_find_by_id(gbuf, id);
...
static int tokenize_call_neighbors(const ani_gbuf_node_t *n, const ani_gbuf_t *gbuf, bool outbound,
                                   char **tokens, int count, int max_tokens) {
    ...
    for (int e = 0; e < nn && e < MAX_CALLEES && count < max_tokens; e++) {
        count += ani_sem_tokenize(names[e], tokens + count, max_tokens - count);
    }
    free((void *)names);
    return count;
}

static int tokenize_node(const ani_gbuf_node_t *n, const ani_gbuf_t *gbuf, char **tokens,
                         int max_tokens) {
    int count = 0;
    count += ani_sem_tokenize(n->name, tokens + count, max_tokens - count);
    if (n->qualified_name && count < max_tokens) {
        count += ani_sem_tokenize(n->qualified_name, tokens + count, max_tokens - count);
    }
    if (n->file_path && count < max_tokens) {
        count += ani_sem_tokenize(n->file_path, tokens + count, max_tokens - count);
    }
...
static void build_api_vec(const ani_gbuf_t *gbuf, int64_t node_id, ani_sem_vec_t *out) {
    ...
    for (int i = 0; i < n && i < MAX_CALLEES; i++) {
        ani_sem_vec_t callee_ri;
        ani_sem_random_index(names[i], &callee_ri);
        ani_sem_vec_add_scaled(out, &callee_ri, PSE_UNIT_POS);
    }
    free((void *)names);
    ani_sem_normalize(out);
}

static void build_type_vec(const char *props_json, ani_sem_vec_t *out) {
    memset(out, 0, sizeof(*out));
    if (!props_json) {
        return;
    }
    /* Extract param_types and return_type */
    char rt_buf[ANI_SZ_128];
    if (json_str_value(props_json, "return_type", rt_buf, sizeof(rt_buf))) {
        ani_sem_vec_t ri;
        ani_sem_random_index(rt_buf, &ri);
        ani_sem_vec_add_scaled(out, &ri, PSE_UNIT_POS);
    }
    char *ptypes[ANI_SZ_16];
    int pt_count = json_str_array(props_json, "param_types", ptypes, ANI_SZ_16);
    for (int i = 0; i < pt_count; i++) {
        ani_sem_vec_t ri;
        ani_sem_random_index(ptypes[i], &ri);
        ani_sem_vec_add_scaled(out, &ri, PSE_UNIT_POS);
        free(ptypes[i]);
    }
    ani_sem_normalize(out);
...
static void build_deco_vec(const char *props_json, ani_sem_vec_t *out) {
    memset(out, 0, sizeof(*out));
    if (!props_json) {
        return;
    }
    char *decos[ANI_SZ_16];
    int dc = json_str_array(props_json, "decorators", decos, ANI_SZ_16);
    for (int i = 0; i < dc; i++) {
        ani_sem_vec_t ri;
        ani_sem_random_index(decos[i], &ri);
        ani_sem_vec_add_scaled(out, &ri, PSE_UNIT_POS);
        free(decos[i]);
    }
    ani_sem_normalize(out);
...
static void decode_struct_profile(const char *props_json, float *out) {
    memset(out, 0, sizeof(float) * ANI_AST_PROFILE_DIMS);
    if (!props_json) {
        return;
    }
    ...
    if (json_str_value(props_json, "sp", sp_buf, sizeof(sp_buf))) {
        ...
        if (ani_ast_profile_from_str(sp_buf, &profile)) {
            ani_ast_profile_to_vector(&profile, out);
        }
    }
}

static void decode_minhash(const char *props_json, ani_sem_func_t *func) {
    ...
    if (!end || (int)(end - hex) != ANI_MINHASH_HEX_LEN) {
        return;
    }
    char hex_buf[ANI_MINHASH_HEX_BUF];
    memcpy(hex_buf, hex, ANI_MINHASH_HEX_LEN);
    hex_buf[ANI_MINHASH_HEX_LEN] = '\0';
    ani_minhash_t mh;
    if (ani_minhash_from_hex(hex_buf, &mh)) {
...
typedef struct {
    const ani_gbuf_node_t **node_ptrs; /* node pointer per function index */
    ani_gbuf_t *gbuf;                  /* read-only during tokenization */
...
static void tokenize_worker(int worker_id, void *ctx_ptr) {
    ...
    while (true) {
        ...
        const ani_gbuf_node_t *n = tc->node_ptrs[f];
        ...
        char **dst = &tc->all_tokens[(ptrdiff_t)f * ANI_SEM_MAX_TOKENS];
        int count = tokenize_node(n, tc->gbuf, dst, ANI_SEM_MAX_TOKENS);
        count = inject_pattern_tokens(n, tc->gbuf, dst, count, ANI_SEM_MAX_TOKENS);
        if (tc->pools && tc->pools[worker_id]) {
            ANIHashTable *pool = tc->pools[worker_id];
            for (int t = 0; t < count; t++) {
                char *canon = ani_ht_get(pool, dst[t]);
                if (canon) {
                    free(dst[t]);
                    dst[t] = canon;
                } else {
                    ani_ht_set(pool, dst[t], dst[t]); /* key borrows the value */
                }
            }
        }
...
typedef struct {
    ani_sem_func_t *funcs;
    char **all_tokens;
    int *token_counts;
    ani_sem_corpus_t *corpus;
    uint8_t *qvecs; /* output: pre-quantized int8 vectors [func_count * ANI_SEM_DIM] */
    int func_count;
    _Atomic int next_idx;
} vec_build_ctx_t;
...
static void vec_build_worker(int worker_id, void *ctx_ptr) {
    ...
    while (true) {
        ...
        char **tokens = &vc->all_tokens[(ptrdiff_t)f * ANI_SEM_MAX_TOKENS];
        ...
        int tfidf_len = 0;
        for (int t = 0; t < tc; t++) {
            float idf = ani_sem_corpus_idf(vc->corpus, tokens[t]);
            if (idf > 0.0F) {
                indices[tfidf_len] = t;
        ...
        ani_sem_vec_t ri_dense;
        memset(&ri_dense, 0, sizeof(ani_sem_vec_t));
        for (int t = 0; t < tc; t++) {
            const ani_sem_vec_t *ri = ani_sem_corpus_ri_vec(vc->corpus, tokens[t]);
            if (ri) {
                float idf = ani_sem_corpus_idf(vc->corpus, tokens[t]);
                ani_sem_vec_add_scaled(&ri_dense, ri, idf);
            }
        }
        ani_sem_normalize(&ri_dense);
        ani_rsq_encode(ri_dense.v, &vc->funcs[f].ri_code);

        /* Int8 quantize into pre-allocated output array (parallel-safe) */
        uint8_t *qv = &vc->qvecs[(ptrdiff_t)f * ANI_SEM_DIM];
...
typedef float hyperplane_row_t[ANI_RSQ_DIM];
...
static void sig_build_worker(int worker_id, void *ctx_ptr) {
    ...
    while (true) {
        ...
        float dec[ANI_RSQ_DIM];
        ani_rsq_decode(&sc->funcs[f].ri_code, dec);
        uint64_t sig = 0;
        for (int h = 0; h < NUM_HYPERPLANES; h++) {
            float dot = 0.0F;
            for (int d = 0; d < ANI_RSQ_DIM; d++) {
...
static void score_try_emit(score_ctx_t *sc, int i, int j, int c, deferred_edge_buf_t *my_buf) {
    ...
    float score = ani_sem_combined_score(&sc->funcs[i], &sc->funcs[j], &sc->cfg);
...
typedef struct {
    ani_sem_func_t *funcs;
    const ani_gbuf_node_t **node_ptrs;
...
static void collect_worker(int worker_id, void *ctx_ptr) {
    ...
    while (true) {
        ...
        for (int i = f; i < end; i++) {
            const ani_gbuf_node_t *n = cc->node_ptrs[i];
            decode_minhash(n->properties_json, &cc->funcs[i]);
            decode_struct_profile(n->properties_json, cc->funcs[i].struct_profile);
            ani_sem_vec_t tmp_vec;
            build_api_vec(cc->gbuf, n->id, &tmp_vec);
            ani_rsq_encode(tmp_vec.v, &cc->funcs[i].api_code);
            build_type_vec(n->properties_json, &tmp_vec);
            ani_rsq_encode(tmp_vec.v, &cc->funcs[i].type_code);
            build_deco_vec(n->properties_json, &tmp_vec);
            ani_rsq_encode(tmp_vec.v, &cc->funcs[i].deco_code);
        }
    }
}

/* ── Phase helpers (keep ani_pipeline_pass_semantic_edges complexity low) ── */
...
static int cmp_node_ptr_by_qn(const void *pa, const void *pb) {
    const ani_gbuf_node_t *a = *(const ani_gbuf_node_t *const *)pa;
    const ani_gbuf_node_t *b = *(const ani_gbuf_node_t *const *)pb;
...
static int phase1_scan_functions(ani_gbuf_t *gbuf, ani_sem_func_t **out_funcs,
                                 const ani_gbuf_node_t ***out_nodes) {
    *out_funcs = NULL;
    *out_nodes = NULL;
    ani_sem_func_t *funcs = NULL;
    const ani_gbuf_node_t **node_ptrs = NULL;
    int func_count = 0;
    int func_cap = 0;
    ...
    for (int li = 0; labels[li]; li++) {
        ...
        int node_count = 0;
        if (ani_gbuf_find_by_label(gbuf, labels[li], &nodes, &node_count) != 0) {
            continue;
        }
        for (int i = 0; i < node_count; i++) {
            if (func_count >= func_cap) {
                int new_cap = func_cap < MAX_FUNCS_INIT ? MAX_FUNCS_INIT : func_cap * GROW;
                ani_sem_func_t *grown = realloc(funcs, (size_t)new_cap * sizeof(ani_sem_func_t));
                if (!grown) {
                    break;
                }
                funcs = grown;
                const ani_gbuf_node_t **np_grown =
                    realloc(node_ptrs, (size_t)new_cap * sizeof(ani_gbuf_node_t *));
                if (!np_grown) {
                    break;
                }
                node_ptrs = np_grown;
            ...
            memset(&funcs[func_count], 0, sizeof(ani_sem_func_t));
...
static int phase6b_merge_edges(ani_gbuf_t *gbuf, deferred_edge_buf_t *worker_bufs, int worker_count,
    ...
    for (int e = 0; e < n; e++) {
        ...
        ani_gbuf_insert_edge(gbuf, de->source_id, de->target_id, "SEMANTICALLY_RELATED", props);
        edge_counts[de->i]++;
        edge_counts[de->j]++;
        total_edges++;
    }
...
static void phase3c_export_token_vectors(ani_gbuf_t *gbuf, ani_sem_corpus_t *corpus) {
    int tv_count = ani_sem_corpus_token_count(corpus);
    for (int t = 0; t < tv_count; t++) {
        const ani_sem_vec_t *vec = NULL;
        float idf = 0.0F;
        const char *tok = ani_sem_corpus_token_at(corpus, t, &vec, &idf);
        if (!tok || !vec || idf <= PSE_FLOW_WEIGHT) {
            continue;
        }
        uint8_t qvec[ANI_SEM_DIM];
        for (int d = 0; d < ANI_SEM_DIM; d++) {
        ...
        ani_gbuf_store_token_vector(gbuf, tok, qvec, ANI_SEM_DIM, idf);
    }
    ani_log_info("pass.semantic.token_vectors", "count", itoa_log(tv_count));
}

/* Phase 5a: generate NUM_HYPERPLANES × ANI_SEM_DIM deterministic random
...
static hyperplane_row_t *phase5a_build_hyperplanes(void) {
    ...
    for (int h = 0; h < NUM_HYPERPLANES; h++) {
        for (int d = 0; d < ANI_RSQ_DIM; d++) {
            uint64_t seed = XXH3_64bits_withSeed(&d, sizeof(d), (uint64_t)h * ANI_RSQ_DIM);
 ...
 * parallel.  Must be called after ani_sem_ensure_ready() so the pretrained
 * token map is initialized. */
static void phase1b_decode_and_build(ani_sem_func_t *funcs, const ani_gbuf_node_t **node_ptrs,
                                     const ani_gbuf_t *gbuf, int func_count, int worker_count) {
    if (func_count <= 0) {
        return;
    }
    collect_ctx_t cc = {
    ...
    ani_parallel_for(worker_count, collect_worker, &cc, opts);
}

/* Phase 2: tokenize each function's metadata in parallel, filling
 * all_tokens[] and token_counts[].  Caller allocates the arrays. */
static void phase2_tokenize(const ani_gbuf_node_t **node_ptrs, ani_gbuf_t *gbuf, char **all_tokens,
    ...
    ani_parallel_for_opts_t opts = {.max_workers = worker_count, .force_pthreads = false};
    ani_parallel_for(worker_count, tokenize_worker, &tc, opts);
}
...
static void phase4_build_and_store_vectors(ani_gbuf_t *gbuf, ani_sem_func_t *funcs,
                                           char **all_tokens, int *token_counts,
                                           ani_sem_corpus_t *corpus, int func_count,
                                           int worker_count) {
    uint8_t *qvecs = malloc((size_t)func_count * ANI_SEM_DIM);
    if (!qvecs) {
        return;
    }
    vec_build_ctx_t vc = {
    ...
    ani_parallel_for_opts_t opts = {.max_workers = worker_count, .force_pthreads = false};
    ani_parallel_for(worker_count, vec_build_worker, &vc, opts);
    for (int f = 0; f < func_count; f++) {
        ani_gbuf_store_vector(gbuf, funcs[f].node_id, &qvecs[(ptrdiff_t)f * ANI_SEM_DIM],
                              ANI_SEM_DIM);
    }
    free(qvecs);
}
 ...
 * caller frees both. */
static void phase5_lsh_build(ani_sem_func_t *funcs, int func_count, int worker_count,
    ...
    if (hyperplanes && signatures) {
        ...
        ani_parallel_for_opts_t opts = {.max_workers = worker_count, .force_pthreads = false};
        ani_parallel_for(worker_count, sig_build_worker, &sc, opts);
    }
    free(hyperplanes);
...
static void phase6a_score_candidates(ani_sem_func_t *funcs, uint64_t *signatures, int *edge_counts,
                                     sem_bucket_t **band_buckets, ani_sem_config_t cfg,
    ...
    atomic_init(&sc.next_idx, 0);
    ani_parallel_for_opts_t opts = {.max_workers = worker_count, .force_pthreads = false};
    ani_parallel_for(worker_count, score_worker, &sc, opts);
}
 ...
 * the caller must ani_sem_corpus_free() later. */
static ani_sem_corpus_t *run_corpus_phase(ani_gbuf_t *gbuf, char **all_tokens, int *token_counts,
                                          int func_count) {
    ANI_PROF_START(t_phase3a);
    ani_sem_corpus_t *corpus = ani_sem_corpus_new();
    ani_sem_corpus_add_docs_batch(corpus, all_tokens, token_counts, func_count, ANI_SEM_MAX_TOKENS);
    ANI_PROF_END_N("semantic_edges", "3a_corpus_batch", t_phase3a, func_count);

    ANI_PROF_START(t_phase3b);
    ani_sem_corpus_finalize(corpus);
    ANI_PROF_END_N("semantic_edges", "3b_corpus_finalize_seq", t_phase3b,
                   ani_sem_corpus_token_count(corpus));

    ANI_PROF_START(t_phase3c);
    phase3c_export_token_vectors(gbuf, corpus);
    ANI_PROF_END_N("semantic_edges", "3c_token_vec_export_seq", t_phase3c,
                   ani_sem_corpus_token_count(corpus));
    return corpus;
}
...
static int run_scoring_phase(ani_gbuf_t *gbuf, ani_sem_func_t *funcs, uint64_t *signatures,
                             sem_bucket_t **band_buckets, ani_sem_config_t cfg, int func_count,
                             int worker_count) {
...
static void free_funcs_and_tokens(ani_sem_func_t *funcs, int func_count, char **all_tokens,
                                  const int *token_counts, ANIHashTable **pools, int worker_count) {
    (void)token_counts;
    for (int f = 0; f < func_count; f++) {
    ...
    if (pools) {
        for (int w = 0; w < worker_count; w++) {
            if (pools[w]) {
                ani_ht_foreach(pools[w], free_token_pool_entry, NULL);
                ani_ht_free(pools[w]);
            }
        }
        free(pools);
...
int ani_pipeline_pass_semantic_edges(ani_pipeline_ctx_t *ctx) {
    ...
    ani_log_info("pass.start", "pass", "semantic_edges");

    ani_gbuf_t *gbuf = ctx->gbuf;
    ani_sem_config_t cfg = ani_sem_get_config();

    ANI_PROF_START(t_phase1a);
    ani_sem_func_t *funcs = NULL;
    const ani_gbuf_node_t **node_ptrs = NULL;
    int func_count = phase1_scan_functions(gbuf, &funcs, &node_ptrs);
    ANI_PROF_END_N("semantic_edges", "1a_scan_seq", t_phase1a, func_count);

    /* Phase 1b: Decode minhash + profile + build api/type/deco vectors (PARALLEL). */
    ani_sem_ensure_ready();
    ANI_PROF_START(t_phase1b);
    phase1b_decode_and_build(funcs, node_ptrs, gbuf, func_count, ani_default_worker_count(false));
    ANI_PROF_END_N("semantic_edges", "1b_decode_build_parallel", t_phase1b, func_count);
    ani_log_info("pass.semantic.collected", "functions", itoa_log(func_count));

    if (func_count < PSE_MIN_FUNCS_FOR_PAIR) {
        free(funcs);
        free(node_ptrs);
        ani_log_info("pass.done", "pass", "semantic_edges", "edges", "0");
        return 0;
    }

    /* Phase 2: Tokenize all nodes (PARALLEL) */
    int worker_count = ani_default_worker_count(false);
    char **all_tokens = malloc((size_t)func_count * sizeof(char *) * ANI_SEM_MAX_TOKENS);
    int *token_counts = calloc((size_t)func_count, sizeof(int));

    ANI_PROF_START(t_phase2);
    ANIHashTable **token_pools = calloc((size_t)worker_count, sizeof(ANIHashTable *));
    if (token_pools) {
        for (int w = 0; w < worker_count; w++) {
            token_pools[w] = ani_ht_create(ANI_SZ_1K);
        }
    }
    phase2_tokenize(node_ptrs, gbuf, all_tokens, token_counts, func_count, worker_count,
                    token_pools);
    ANI_PROF_END_N("semantic_edges", "2_tokenize_parallel", t_phase2, func_count);
    free(node_ptrs);

    /* Phase 3: Build corpus (batch add), finalize, export enriched token vectors. */
    ani_sem_corpus_t *corpus = run_corpus_phase(gbuf, all_tokens, token_counts, func_count);

    /* Phase 4: Build per-function TF-IDF + RI vectors (PARALLEL) and store them. */
    ANI_PROF_START(t_phase4);
    phase4_build_and_store_vectors(gbuf, funcs, all_tokens, token_counts, corpus, func_count,
                                   worker_count);
    ANI_PROF_END_N("semantic_edges", "4_build_and_store_vec", t_phase4, func_count);

    ani_log_info("pass.semantic.vectors_stored", "count", itoa_log(func_count));

    /* Phase 5: LSH hyperplanes → signatures → buckets. */
    ANI_PROF_START(t_phase5);
    ...
    ANI_PROF_END_N("semantic_edges", "5_lsh_build", t_phase5, func_count);

    ani_log_info("pass.semantic.lsh_built", "functions", itoa_log(func_count), "bands",
    ...
    /* Phase 7: Cleanup */
    ANI_PROF_START(t_phase7);
    free_lsh_buckets(band_buckets);
    free(signatures);
    ani_log_info("pass.done", "pass", "semantic_edges", "edges", itoa_log(total_edges));
    free_funcs_and_tokens(funcs, func_count, all_tokens, token_counts, token_pools, worker_count);
    free(token_counts);
    ani_sem_corpus_free(corpus);
    ANI_PROF_END("semantic_edges", "7_cleanup", t_phase7);
