#ifndef ANI_PATH_ALIAS_H
#define ANI_PATH_ALIAS_H
 ...
 *
 * Lookup is two-step:
 *   1. ani_path_alias_find_for_file picks the nearest ancestor scope for
 *      a given source file (longest matching dir_prefix wins).
 *   2. ani_path_alias_resolve runs the chosen scope's alias map against
...
typedef struct {
    ...
    ani_path_alias_map_t *map;
} ani_path_alias_scope_t;
 ...
 * linear scan. */
typedef struct {
    ani_path_alias_scope_t *scopes;
    int count;
} ani_path_alias_collection_t;
 ...
 * ani_path_alias_collection_free. */
...
/* Free a collection produced by ani_load_path_aliases. NULL-safe. */
