...
#define ANI_ARTIFACT_SCHEMA_VERSION 2

#define ANI_ARTIFACT_FILENAME "graph.db.zst"
#define ANI_ARTIFACT_META "artifact.json"
#define ANI_ARTIFACT_DIR ".ani"

/* Export quality levels */
enum {
    ANI_ARTIFACT_FAST = 0, /* zstd -3, no index stripping (watcher path) */
    ANI_ARTIFACT_BEST = 1, /* zstd -9 + drop indexes + VACUUM INTO (explicit index) */
};

/* Export DB to .ani/graph.db.zst artifact.
 * quality: ANI_ARTIFACT_FAST or ANI_ARTIFACT_BEST.
 * Creates .ani/ dir, .gitattributes, and artifact.json.
...
/* Why the most recent ani_artifact_export on THIS thread did NOT write the
 ...
 * command (as artifact.c does via ani_popen). Rejects quote / backslash / shell
 * substitution metacharacters (ani_validate_shell_arg); on Windows also rejects the
