...
static bool pu_module_is_dir(ANILanguage lang) {
...
static int build_import_map(ani_pipeline_ctx_t *ctx, const char *rel_path,
                            ...
                            const char ***out_vals, int *out_count, ANILanguage lang) {
    return ani_pxc_build_import_map(ctx->gbuf, ctx->project_name, rel_path, lang, result, out_keys,
                                    out_vals, out_count);
}

static void free_import_map(const char **keys, const char **vals, int count) {
    ani_pxc_free_import_map(keys, vals, count);
}

/* Find the graph buffer node for an enclosing function QN, falling back to file node. */
static const ani_gbuf_node_t *find_enclosing_node(ani_pipeline_ctx_t *ctx, const char *func_qn,
                                                  const char *rel_path) {
    const ani_gbuf_node_t *node = NULL;
    if (func_qn && func_qn[0]) {
        node = ani_gbuf_find_by_qn(ctx->gbuf, func_qn);
        ...
        if (ani_pipeline_node_is_dir_container(node)) {
            node = NULL;
        }
    }
    if (!node) {
...
static int resolve_usage_edges(ani_pipeline_ctx_t *ctx, const ANIFileResult *result,
    ...
    for (int u = 0; u < result->usages.count; u++) {
        ...
        const ani_gbuf_node_t *src = find_enclosing_node(ctx, usage->enclosing_func_qn, rel);
        if (!src) {
            continue;
        }
        ...
        if (ani_pipeline_usage_semantic_reference_candidate(usage)) {
            ...
            if (semantic_reference) {
                tgt = ani_pipeline_lsp_target_node(ctx->gbuf, ctx->project_name,
        ...
        if (!tgt) {
            ...
            ani_resolution_t res =
                (lang == ANI_LANG_SQL)
                    ? ani_registry_resolve_lineage(ctx->registry, usage->ref_name, module_qn,
                                                   imp_keys, imp_vals, imp_count)
                    : ani_registry_resolve(ctx->registry, usage->ref_name, module_qn, imp_keys,
                                           imp_vals, imp_count);
            ...
            tgt = ani_gbuf_find_by_qn(ctx->gbuf, res.qualified_name);
        ...
        ani_gbuf_insert_edge(ctx->gbuf, src->id, tgt->id, edge_type, uprops);
        resolved++;
    }
...
static int resolve_throw_edges(ani_pipeline_ctx_t *ctx, const ANIFileResult *result,
                               const char *rel, const char *module_qn, const char **imp_keys,
    ...
    for (int t = 0; t < result->throws.count; t++) {
        ...
        const ani_gbuf_node_t *src = find_enclosing_node(ctx, thr->enclosing_func_qn, rel);
        if (!src) {
            continue;
        }

        const char *edge_type = is_checked_exception(thr->exception_name) ? "THROWS" : "RAISES";
        ani_resolution_t res = ani_registry_resolve(ctx->registry, thr->exception_name, module_qn,
                                                    imp_keys, imp_vals, imp_count);
        ...
        ani_gbuf_insert_edge(ctx->gbuf, src->id, tgt->id, edge_type, "{}");
        resolved++;
    }
    return resolved;
}
...
static int resolve_rw_edges(ani_pipeline_ctx_t *ctx, const ANIFileResult *result, const char *rel,
    ...
    for (int r = 0; r < result->rw.count; r++) {
        ...
        const ani_gbuf_node_t *src = find_enclosing_node(ctx, rw->enclosing_func_qn, rel);
        if (!src) {
            continue;
        }

        ani_resolution_t res = ani_registry_resolve(ctx->registry, rw->var_name, module_qn,
                                                    imp_keys, imp_vals, imp_count);
        if (!res.qualified_name || res.qualified_name[0] == '\0') {
            continue;
        }

        const ani_gbuf_node_t *tgt = ani_gbuf_find_by_qn(ctx->gbuf, res.qualified_name);
        if (!tgt || src->id == tgt->id) {
            continue;
        }
...
int ani_pipeline_pass_usages(ani_pipeline_ctx_t *ctx, const ani_file_info_t *files,
                             int file_count) {
    ani_log_info("pass.start", "pass", "usages", "files", itoa_log(file_count));
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        if (!result) {
            ...
            result = ani_extract_file(source, source_len, files[i].language, ctx->project_name, rel,
    ...
    ani_log_info("pass.done", "pass", "usages", "usage", itoa_log(usage_resolved), "throws",
