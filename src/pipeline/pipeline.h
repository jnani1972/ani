 ...
 */
#ifndef ANI_PIPELINE_H
#define ANI_PIPELINE_H

#include <stdbool.h>
#include <stdint.h>
#include <stdatomic.h>

#include "discover/discover.h"    /* ani_ignored_file_t (#963) */
#include "foundation/constants.h" /* ANI_SZ_512 */
...
typedef struct ani_pipeline ani_pipeline_t;
...
typedef enum {
    ...
    ANI_MODE_FULL = 0,     /* Full: everything including SIMILAR_TO + SEMANTICALLY_RELATED */
    ANI_MODE_MODERATE = 1, /* Moderate: fast discovery + SIMILAR_TO + SEMANTICALLY_RELATED */
    ANI_MODE_FAST = 2,     /* Fast: skip non-essential files, no similarity/semantic edges */
} ani_index_mode_t;
#endif
...
ani_pipeline_t *ani_pipeline_new(const char *repo_path, const char *db_path, ani_index_mode_t mode);

/* Enable persistent artifact export (.ani/graph.db.zst).
 ...
 * existing database in place. Those codes (ANI_PIPELINE_ABORT_PRESERVE_DB and
 * ANI_PIPELINE_PERSIST_FAILED) are defined in pipeline_internal.h alongside the
 ...
 * owned by the pipeline. Valid until ani_pipeline_free(). */
...
/* Get the index mode (ANI_MODE_FULL, ANI_MODE_MODERATE, ANI_MODE_FAST). */
 ...
 * valid until ani_pipeline_free()); *count receives its length. Both are set
 ...
 * the pipeline (copied on record, freed in ani_pipeline_free). A skip is the
 ...
 * ani_pipeline_free()). out and count are set to NULL and 0 when p is NULL or
...
char *ani_pipeline_fqn_compute(const char *project, const char *rel_path, const char *name);

/* Module QN: project.dir.parts (no name). Caller must free(). */
char *ani_pipeline_fqn_module(const char *project, const char *rel_path);
 ...
 * it is exactly ani_pipeline_fqn_module(). Caller must free(). */
char *ani_pipeline_fqn_module_dir(const char *project, const char *rel_path, bool module_is_dir);

/* Folder QN: project.dir.parts. Caller must free(). */
char *ani_pipeline_fqn_folder(const char *project, const char *rel_dir);
 ...
 * (e.g. "src/api/helpers") suitable for passing to ani_pipeline_fqn_module,
...
char *ani_pipeline_resolve_relative_import(const char *source_rel, const char *module_path);

/* Derive project name from an absolute path.
 * Replaces / and : with -, collapses --, trims leading -.
 * Caller must free() the returned string. */
char *ani_project_name_from_path(const char *abs_path);

/* ── Function Registry ──────────────────────────────────────────── */

typedef struct ani_registry ani_registry_t;
...
/* Per-file full-result cache for ani_registry_resolve. The same
...
int ani_parse_hunks(const char *output, ani_changed_hunk_t *out, int max_out);

#endif /* ANI_PIPELINE_H */
