...
#define ANI_WORKER_STACK_SIZE ((size_t)8 * ANI_SZ_1K * ANI_SZ_1K)

/* ── Serial fallback ─────────────────────────────────────────────── */

static void run_serial(int count, ani_parallel_fn fn, void *ctx) {
...
static void run_pthreads(int count, ani_parallel_fn fn, void *ctx, int nworkers) {
    ...
    ani_thread_t *threads = (ani_thread_t *)malloc((size_t)nworkers * sizeof(ani_thread_t));
    if (!threads) {
    ...
    for (int i = 0; i < nworkers; i++) {
        if (ani_thread_create(&threads[i], ANI_WORKER_STACK_SIZE, pthread_worker, &wa) != 0) {
...
void ani_parallel_for(int count, ani_parallel_fn fn, void *ctx, ani_parallel_for_opts_t opts) {
    if (count <= 0 || !fn) {
        return;
    }
    ...
    if (nworkers <= 0) {
        nworkers = ani_default_worker_count(true);
    }
    if (nworkers < WP_MIN) {
        nworkers = SKIP_ONE;
    }
