 ...
 *
 * Each file is read and parsed ONCE (Phase 3A). The ANIFileResult is cached
 * and reused for resolution (Phase 4), eliminating 3x redundant I/O + parsing.
 *
 * Depends on: worker_pool, graph_buffer (shared IDs + merge), extraction (ani.h)
 */
 ...
 * (source_reread fallback). Both caps are env-overridable — ANI_RETAIN_TOTAL_MB
 * / ANI_RETAIN_PER_FILE_MB raise or lower the auto-derived defaults directly
...
#include "pipeline/pass_lsp_cross.h" /* ani_pxc_* helpers for fused cross-file LSP */
#include "pipeline/lsp_resolve.h"
#include "lsp/rust_cargo.h"
#include "helpers.h" /* ani_kind_in_set_free_cache — per-worker-thread cache teardown */
...
/* Parse a positive MB-valued retention env knob (ANI_RETAIN_*_MB) into bytes.
 * Follows the limits.c strtol convention: unset / unparseable / non-positive
 * → return 0 so the caller keeps its derived default. */
static size_t ani_retain_env_bytes(const char *name) {
 ...
 * the ceiling. */
static size_t ani_parallel_extract_default_total_cap(void) {
    size_t cap = ani_mem_budget();
    if (cap > 0) {
        cap /= 8;
    } else {
...
static size_t ani_parallel_extract_default_per_file_cap(size_t total_cap) {
 ...
 * explicit caller opts > ANI_RETAIN_*_MB env knobs > RAM-derived defaults.
...
static ani_parallel_extract_opts_t ani_parallel_extract_resolve_opts(
    const ani_parallel_extract_opts_t *opts) {
    ani_parallel_extract_opts_t resolved = {
        .retain_sources = true,
        .retain_sources_set = true,
        .retain_total_budget_bytes = ani_parallel_extract_default_total_cap(),
        .retain_per_file_max_bytes = 0,
    };

    size_t env_total = ani_retain_env_bytes("ANI_RETAIN_TOTAL_MB");
    if (env_total > 0) {
        resolved.retain_total_budget_bytes = env_total;
    }

    resolved.retain_per_file_max_bytes =
        ani_parallel_extract_default_per_file_cap(resolved.retain_total_budget_bytes);
    size_t env_per_file = ani_retain_env_bytes("ANI_RETAIN_PER_FILE_MB");
    if (env_per_file > 0) {
...
static uint64_t extract_now_ns(void) {
    struct timespec ts;
    ani_clock_gettime(CLOCK_MONOTONIC, &ts);
 ...
 * ani_file_error_t array in the existing sequential merge loop. */
typedef struct {
    ani_file_error_t *items;
    int count;
    int cap;
} pp_err_list_t;
...
static void pp_err_add(pp_err_list_t *list, const char *path, const char *reason,
    ...
    if (list->count >= list->cap) {
        ...
        ani_file_error_t *grown =
            (ani_file_error_t *)realloc(list->items, (size_t)ncap * sizeof(*grown));
        if (!grown) {
...
static const char *itoa_log(int val) {
    static ANI_TLS char bufs[PP_RING][ANI_SZ_32];
...
static bool pp_module_is_dir(ANILanguage lang) {
...
static const char *resolve_as_class(const ani_registry_t *reg, const char *name,
                                    const char *module_qn, const char **imp_keys,
                                    const char **imp_vals, int imp_count) {
    ani_resolution_t res =
        ani_registry_resolve(reg, name, module_qn, imp_keys, imp_vals, imp_count);
    ...
    const char *label = ani_registry_label_of(reg, res.qualified_name);
    if (!ani_label_is_type_like(label)) {
        return NULL;
    }
...
#define ANI_CACHE_LINE ANI_SZ_128

typedef struct __attribute__((aligned(ANI_CACHE_LINE))) {
    ani_gbuf_t *local_gbuf;
    int nodes_created;
    int errors;
    char _pad[ANI_CACHE_LINE - sizeof(ani_gbuf_t *) - (PP_ESC_SPACE * sizeof(int))];
...
typedef struct {
    ...
    ani_pkg_entries_t *pkg_entries; /* per-worker manifest arrays (separate allocation) */
    ...
    const ANIMacroTable *macro_table;            /* ObjectScript $$$macros (NULL if none) */
    const ANIReturnTypeTable *return_type_table; /* ObjectScript return types (NULL if none) */

    /* Superlinearity probe — see profile.h. Ticked on each file claim below. */
    ani_scale_probe_t scale;
} extract_ctx_t;
...
static void insert_def_into_gbuf(extract_worker_state_t *ws, const ani_file_info_t *fi,
                                 ANIDefinition *def) {
    char props[ANI_SZ_2K];
    build_def_props(props, sizeof(props), def);
    int64_t func_id =
        ani_gbuf_upsert_node(ws->local_gbuf, def->label ? def->label : "Function", def->name,
    ...
    if (def->route_path && def->route_path[0] != '\0') {
        ...
        snprintf(route_qn, sizeof(route_qn), "__route__%s__%s", rm,
                 ani_route_canon_path(def->route_path, cpath, sizeof(cpath)));
        char rprops[ANI_SZ_256];
        snprintf(rprops, sizeof(rprops), "{\"method\":\"%s\",\"source\":\"decorator\"}", rm);
        int64_t route_id =
            ani_gbuf_upsert_node(ws->local_gbuf, "Route", def->route_path, route_qn,
                                 def->file_path ? def->file_path : fi->rel_path, 0, 0, rprops);
        char hprops[ANI_SZ_512];
        char esc_h[ANI_SZ_512];
        ani_json_escape(esc_h, sizeof(esc_h), def->qualified_name);
        snprintf(hprops, sizeof(hprops), "{\"handler\":\"%s\"}", esc_h);
        ani_gbuf_insert_edge(ws->local_gbuf, func_id, route_id, "HANDLES", hprops);
    }
}

static void log_extract_fail(int pos, uint64_t ms, const char *path) {
    if (pos < PP_LOG_THRESH) {
        ani_log_warn("parallel.extract.file.fail", "pos", itoa_log(pos), "elapsed_ms",
...
static void log_extract_done(int pos, uint64_t ms, int defs, const char *path) {
    if (pos < PP_LOG_THRESH || ms > PP_TIMER_THRESH) {
        ani_log_info("parallel.extract.file.done", "pos", itoa_log(pos), "elapsed_ms",
...
static void extract_worker(int worker_id, void *ctx_ptr) {
    ...
    /* Lazy gbuf creation */
    if (!ws->local_gbuf) {
        ws->local_gbuf = ani_gbuf_new_shared_ids(ec->project_name, ec->repo_path, ec->shared_ids);
    }
    ...
    while (SKIP_ONE) {
        ...
        ani_scale_tick(&ec->scale, sort_pos);
        ...
        if (ani_mem_budget() > 0) {
            ...
            if (over && !futile) {
                ...
                int bp = 0;
                for (; bp < PP_BACKPRESSURE_MAX_SPINS && ani_mem_over_budget() &&
                       !atomic_load_explicit(ec->cancelled, memory_order_relaxed);
                     bp++) {
                    struct timespec nap = {0, PP_BACKPRESSURE_NAP_NS};
                    ani_nanosleep(&nap, NULL);
                }
                if (bp == PP_BACKPRESSURE_MAX_SPINS && ani_mem_over_budget()) {
                    ...
                    if (atomic_exchange_explicit(&ec->bp_futile, 1, memory_order_relaxed) == 0) {
                        ani_log_warn("mem.backpressure.futile", "action", "soft_overshoot");
                    }
                }
            } else if (!over && futile) {
        ...
        const ani_file_info_t *fi = &ec->files[file_idx];
         ...
         * ANI_INDEX_QUARANTINE_FILE. Covers the parallel path; the supervisor's
         * single-threaded recovery run instead takes the sequential path
         * (pass_definitions.c), and ani_extract_file's hard guard backstops both. */
        if (ani_index_is_quarantined(fi->rel_path)) {
            const char *phase = ani_index_quarantine_phase(fi->rel_path);
            if (!phase) {
                phase = "crash";
            }
        ...
        if (!source) {
            ...
            if (rst == ANI_READ_OVERSIZED) {
                ...
                if (atomic_fetch_add_explicit(&ec->oversized_warned, SKIP_ONE,
                    ...
                    ani_log_warn("index.file_oversized", "path", fi->rel_path, "size_mb",
            ...
            /* ANI_READ_EMPTY: benign 0-byte file — not reported. */
            continue;
        }
        ...
        if (sort_pos < PP_LOG_THRESH || /* first 2 rounds of workers = most interesting */
            ani_log_crash_durable()) {
            ani_log_info("parallel.extract.file.start", "pos", itoa_log(sort_pos), "size_kb",
                         itoa_log(source_len / ANI_SZ_1K), "path", fi->rel_path);
        }
        ...
        ANIFileResult *result =
            fi->language == ANI_LANG_OBJECTSCRIPT_EXPORT
                ? ani_pipeline_extract_objectscript_export(source, source_len, ec->project_name,
                                                           fi->rel_path, ec->macro_table,
                                                           ec->return_type_table)
                : ani_extract_file_ex(source, source_len, fi->language, ec->project_name,
                                      fi->rel_path, ANI_EXTRACT_BUDGET, NULL, NULL, ec->macro_table,
                                      ec->return_type_table);
        ...
        ani_free_tree(result);

        /* Detect and parse manifest files for package map */
        {
            const char *bn = strrchr(fi->rel_path, '/');
            ani_pkgmap_try_parse(bn ? bn + SKIP_ONE : fi->rel_path, fi->rel_path, source,
        ...
        if (ec->retain_sources && source_len > 0) {
            ...
            if ((size_t)source_len > ec->retain_per_file_max_bytes) {
                ...
                if ((size_t)(prior + (int64_t)source_len) <= ec->retain_total_budget_bytes) {
                    char *copy = (char *)ani_arena_alloc(&result->arena, (size_t)source_len + 1);
                    if (copy) {
            ...
            if (dropped &&
                ...
                ani_log_warn("index.retain_capped", "path", fi->rel_path ? fi->rel_path : "?",
        ...
        if ((sort_pos + SKIP_ONE) % PP_LOG_INTERVAL == 0 || sort_pos + SKIP_ONE == ec->file_count) {
            ani_log_info("parallel.extract.progress", "done", itoa_log(sort_pos + SKIP_ONE),
                         "total", itoa_log(ec->file_count));
        }

        /* Reclaim all slab + tier2 memory between files.
         *
         * After ani_free_tree(result), all tree nodes are on free lists.
         ...
         *
         * get_thread_parser() in ani_extract_file will create a fresh
    ...
    ani_slab_destroy_thread();
    ani_kind_in_set_free_cache(); /* free this worker thread's node-type bitset cache */
}

static void merge_pkg_entries(ani_pipeline_ctx_t *ctx, ani_pkg_entries_t *pkg_entries,
                              int worker_count) {
    if (!pkg_entries) {
        return;
    }
    ...
    ani_pkgmap_scan_repo(ctx->repo_path, &pkg_entries[0], ctx->excluded_dirs, ctx->excluded_count);
    ani_pipeline_set_pkgmap(ani_pkgmap_build(pkg_entries, worker_count, ctx->project_name));
    for (int i = 0; i < worker_count; i++) {
        ani_pkg_entries_free(&pkg_entries[i]);
    }
    free(pkg_entries);
}
...
static void log_extract_mem_stats(int worker_count) {
    if (ani_mem_budget() > 0) {
        size_t mb = (size_t)ANI_SZ_1K * ANI_SZ_1K;
        ani_log_info("parallel.extract.mem", "rss_mb", itoa_log((int)(ani_mem_rss() / mb)),
                     "peak_mb", itoa_log((int)(ani_mem_peak_rss() / mb)), "budget_mb",
                     itoa_log((int)(ani_mem_budget() / mb)), "per_worker_mb",
                     itoa_log((int)(ani_mem_worker_budget(worker_count) / mb)));
    }
}
...
int ani_parallel_extract_ex(ani_pipeline_ctx_t *ctx, const ani_file_info_t *files, int file_count,
    ...
    ani_parallel_extract_opts_t resolved_opts = ani_parallel_extract_resolve_opts(opts);

    if (file_count == 0) {
        return 0;
    }

    ani_log_info("parallel.extract.start", "files", itoa_log(file_count), "workers",
                 itoa_log(worker_count));
    {
        size_t mb = (size_t)ANI_SZ_1K * ANI_SZ_1K;
        ani_log_info("parallel.extract.retention", "retain_sources",
    ...
    if (ani_mem_budget() > 0) {
        size_t worker_budget = ani_mem_worker_budget(worker_count);
        ani_log_info("parallel.mem.budget", "total_mb",
                     itoa_log((int)(ani_mem_budget() / ((size_t)ANI_SZ_1K * ANI_SZ_1K))),
                     "per_worker_mb",
                     itoa_log((int)(worker_budget / ((size_t)ANI_SZ_1K * ANI_SZ_1K))));
    }

    /* Sub-phase: Ensure extraction library is initialized */
    ANI_PROF_START(t_init);
    ani_init();
    ...
    ani_destroy_thread_parser();
    ani_slab_install();
    ANI_PROF_END("parallel_extract", "1_init_libs", t_init);

    /* Sub-phase: Sort files by descending size for tail-latency reduction */
    ANI_PROF_START(t_sort);
    ...
    ANI_PROF_END_N("parallel_extract", "2_sort_files", t_sort, file_count);

    /* Allocate per-worker state (cache-line aligned via posix_memalign) */
    extract_worker_state_t *workers = NULL;
    if (ani_aligned_alloc((void **)&workers, ANI_CACHE_LINE,
    ...
    ani_pkg_entries_t *pkg_entries = calloc((size_t)worker_count, sizeof(ani_pkg_entries_t));
    if (!pkg_entries) {
        ani_aligned_free(workers);
        free(sorted);
        return ANI_NOT_FOUND;
    }
    ...
    ANIMacroTable *pp_macro_table =
        ani_build_macro_table_from_files(files, file_count, ctx->repo_path);
    ...
    ANI_PROF_START(t_dispatch);
    ani_parallel_for_opts_t parallel_opts = {.max_workers = worker_count, .force_pthreads = false};
    ani_scale_begin(&ec.scale, "parallel_extract", (long)file_count);
    ani_parallel_for(worker_count, extract_worker, &ec, parallel_opts);
    ani_scale_end(&ec.scale);
    ANI_PROF_END_N("parallel_extract", "3_dispatch_workers_parallel", t_dispatch, file_count);

    /* Sub-phase: Merge all local gbufs into main gbuf (SEQUENTIAL, gbuf not thread-safe) */
    ANI_PROF_START(t_merge);
    int total_nodes = 0;
    int total_errors = 0;
    for (int i = 0; i < worker_count; i++) {
        if (workers[i].local_gbuf) {
            ani_gbuf_merge(ctx->gbuf, workers[i].local_gbuf);
            total_nodes += workers[i].nodes_created;
            total_errors += workers[i].errors;
            ani_gbuf_free(workers[i].local_gbuf);
        }
    }
    ANI_PROF_END_N("parallel_extract", "4_merge_gbufs_seq", t_merge, total_nodes);
    ...
    if (err_lists) {
        for (int i = 0; i < worker_count; i++) {
            for (int j = 0; j < err_lists[i].count; j++) {
                ani_pipeline_add_file_error(ctx->pipeline, err_lists[i].items[j].path,
    ...
    ani_aligned_free(workers);
    free(sorted);
    ani_macro_table_free(pp_macro_table); /* ObjectScript macro table (NULL-safe) */
    ...
    ani_log_info("parallel.extract.done", "nodes", itoa_log(total_nodes), "errors",
...
int ani_parallel_extract(ani_pipeline_ctx_t *ctx, const ani_file_info_t *files, int file_count,
                         ANIFileResult **result_cache, _Atomic int64_t *shared_ids,
                         int worker_count) {
    return ani_parallel_extract_ex(ctx, files, file_count, result_cache, shared_ids, worker_count,
                                   NULL);
}
...
static int register_and_link_def(ani_pipeline_ctx_t *ctx, const ANIDefinition *def, const char *rel,
                                 int *reg_entries) {
    int edges = 0;
    ...
    if (ani_label_is_registry_symbol(def->label)) {
    ...
    const ani_gbuf_node_t *def_node = ani_gbuf_find_by_qn(ctx->gbuf, def->qualified_name);
    if (file_node && def_node) {
        ani_gbuf_insert_edge(ctx->gbuf, file_node->id, def_node->id, "DEFINES", "{}");
        edges++;
    }
    free(file_qn);
    if (def->parent_class && strcmp(def->label, "Method") == 0) {
        ...
        if (parent && def_node) {
            ani_gbuf_insert_edge(ctx->gbuf, parent->id, def_node->id, "DEFINES_METHOD", "{}");
        }
    }
    return edges;
}
...
static int create_imports_edges(ani_pipeline_ctx_t *ctx, const ANIFileResult *result,
    ...
    for (int j = 0; j < result->imports.count; j++) {
        ANIImport *imp = &result->imports.items[j];
        if (!imp->module_path) {
            continue;
        }
...
static const ani_gbuf_node_t *find_channel_src(ani_pipeline_ctx_t *ctx, const ANIChannel *ch,
                                               const char *rel) {
...
static void create_channel_edges(ani_pipeline_ctx_t *ctx, const ANIFileResult *result,
                                 const char *rel) {
    for (int j = 0; j < result->channels.count; j++) {
        ANIChannel *ch = &result->channels.items[j];
        ...
        char esc_cn[ANI_SZ_256];
        ani_json_escape(esc_cn, sizeof(esc_cn), ch->channel_name);
        ...
        const ani_gbuf_node_t *src_node = find_channel_src(ctx, ch, rel);
...
int ani_build_registry_from_cache(ani_pipeline_ctx_t *ctx, const ani_file_info_t *files,
    ...
    ani_log_info("parallel.registry.start", "files", itoa_log(file_count));
    ...
    ANIHashTable *namespace_map =
        ani_pipeline_namespace_map_build(ctx->project_name, result_cache, rels, file_count);
    free(rels);
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        ANIFileResult *result = result_cache[i];
        if (!result) {
            continue;
        }
    ...
    ani_log_info("parallel.registry.done", "entries", itoa_log(reg_entries), "defines",
...
typedef struct __attribute__((aligned(ANI_CACHE_LINE))) {
    ...
    int lsp_overrides;
    char _pad[ANI_CACHE_LINE - sizeof(ani_gbuf_t *) - ((PP_RING + 1) * sizeof(int))];
...
typedef struct {
    ...
    ANIFileResult **result_cache;
    const ani_gbuf_t *main_gbuf;    /* READ-ONLY during Phase 4 */
    const ani_registry_t *registry; /* READ-ONLY during Phase 4 */
    _Atomic int64_t *shared_ids;
    _Atomic int *cancelled;
    _Atomic int next_file_idx;
     ...
     * match the existing ani_run_X_lsp_cross callee signatures the
     ...
     * When non-NULL, the fused worker calls ani_pxc_filter_defs_for_file
     ...
     * ani_run_X_lsp_cross_with_registry — skip per-file build entirely.
     * Stored as ANICrossLspRegistries* (typedef from pass_lsp_cross.h). */
    ANICrossLspRegistries *cross_registries;
    ...
    const ANICargoManifest *rust_manifest;
    ...
    _Atomic(ANITypeRegistry *) rust_shared_reg;
    ani_mutex_t rust_shared_mu;
    ANIArena rust_shared_arena;
     ...
     * ani_parallel_for synchronization barrier at the end. */
...
#define ANI_ARG_JSON_GUARD ANI_SZ_32
...
static int format_call_arg(char *buf, size_t bufsize, const ANICallArg *a, const char *expr) {
    char esc_k[ANI_SZ_128];
    char esc_e[ANI_SZ_128];
    char esc_v[ANI_SZ_128];
    ani_json_escape(esc_e, sizeof(esc_e), expr);
    if (a->keyword && a->value) {
        ani_json_escape(esc_k, sizeof(esc_k), a->keyword);
...
static size_t append_args_json(char *buf, size_t bufsize, size_t pos, const ANICall *call) {
...
static const char *find_route_path_in_args(const ANICall *call, const char **out_handler) {
    *out_handler = NULL;
    ...
    for (int ai = 0; ai < call->arg_count && !found; ai++) {
        const ANICallArg *ca = &call->args[ai];
...
static void finalize_and_emit(ani_gbuf_t *gbuf, int64_t src_id, int64_t tgt_id,
                              const char *edge_type, char *props, int n, const ANICall *call) {
    if (n > 0 && (size_t)n < ANI_SZ_2K - PP_ESC_SPACE) {
        size_t pos = append_args_json(props, ANI_SZ_2K, (size_t)n, call);
        if (call->start_line > 0 && strcmp(edge_type, "CALLS") == 0 &&
            pos < ANI_SZ_2K - PP_LINE_MARGIN) {
            int ln = snprintf(props + pos, ANI_SZ_2K - pos, ",\"line\":%d", call->start_line);
            if (ln > 0) {
                pos += (size_t)ln;
            }
        }
        if (pos < ANI_SZ_2K - SKIP_ONE) {
            props[pos] = '}';
            props[pos + SKIP_ONE] = '\0';
        }
    }
    ani_gbuf_insert_edge(gbuf, src_id, tgt_id, edge_type, props);
}

/* Build Route node QN and properties for HTTP/async service edges. */
static int64_t build_service_route(ani_gbuf_t *gbuf, const char *arg, const char *method,
                                   const char *broker, ani_svc_kind_t svc) {
    char route_qn[ANI_ROUTE_QN_SIZE];
    const char *prefix;
    char cpath[ANI_SZ_256];
    ...
    if (svc == ANI_SVC_HTTP) {
        ...
        qpath = ani_route_canon_path(arg, cpath, sizeof(cpath));
    } else {
    ...
    return ani_gbuf_upsert_node(gbuf, "Route", arg, route_qn, "", 0, 0, route_props);
}

/* Emit HTTP_CALLS or ASYNC_CALLS edge via Route node. */
static void emit_http_async_service_edge(ani_gbuf_t *gbuf, const ani_gbuf_node_t *source,
                                         const ANICall *call, const ani_resolution_t *res,
                                         ani_svc_kind_t svc, const char *arg) {
    ...
    char esc_a[ANI_SZ_256];
    ani_json_escape(esc_c, sizeof(esc_c), call->callee_name);
    ani_json_escape(esc_a, sizeof(esc_a), arg);
    char props[ANI_SZ_2K];
...
static void emit_config_edge(ani_gbuf_t *gbuf, const ani_gbuf_node_t *source,
                             const ani_gbuf_node_t *target, const ANICall *call,
                             const ani_resolution_t *res, const char *arg) {
    ...
    ani_json_escape(esc_c, sizeof(esc_c), call->callee_name);
    ani_json_escape(esc_k, sizeof(esc_k), arg ? arg : "");
    char props[ANI_SZ_2K];
...
static void emit_normal_calls_edge(ani_gbuf_t *gbuf, const ani_gbuf_node_t *source,
                                   const ani_gbuf_node_t *target, const ANICall *call,
                                   const ani_resolution_t *res) {
...
static void emit_route_registration(ani_gbuf_t *gbuf, const ani_gbuf_node_t *source,
                                    const ANICall *call, const char *route_path,
                                    const char *handler_ref, const char *module_qn,
                                    const ani_registry_t *registry, const ani_gbuf_t *main_gbuf,
                                    const char **ik, const char **iv, int ic) {
    const char *method = ani_service_pattern_route_method(call->callee_name);
    char rqn[ANI_ROUTE_QN_SIZE];
    char cpath[ANI_SZ_256];
    snprintf(rqn, sizeof(rqn), "__route__%s__%s", method ? method : "ANY",
             ani_route_canon_path(route_path, cpath, sizeof(cpath)));
    char rp[ANI_SZ_256];
    snprintf(rp, sizeof(rp), "{\"method\":\"%s\"}", method ? method : "ANY");
    int64_t rid = ani_gbuf_upsert_node(gbuf, "Route", route_path, rqn, "", 0, 0, rp);
    char esc_cn[ANI_SZ_256]; /* sliced source text: escape quotes/newlines */
    char esc_rp[ANI_SZ_512];
    ani_json_escape(esc_cn, sizeof(esc_cn), call->callee_name);
    ani_json_escape(esc_rp, sizeof(esc_rp), route_path);
    char props[ANI_SZ_1K];
    snprintf(props, sizeof(props),
             "{\"callee\":\"%s\",\"url_path\":\"%s\",\"via\":\"route_registration\"}", esc_cn,
             esc_rp);
    ani_gbuf_insert_edge(gbuf, source->id, rid, "CALLS", props);
    if (handler_ref && handler_ref[0] != '\0') {
        ani_resolution_t hres = ani_registry_resolve(registry, handler_ref, module_qn, ik, iv, ic);
        if (hres.qualified_name && hres.qualified_name[0] != '\0') {
            const ani_gbuf_node_t *h = ani_gbuf_find_by_qn(main_gbuf, hres.qualified_name);
            if (h) {
                char hp[ANI_SZ_1K]; /* must exceed escaped value + wrapper or snprintf cuts the
                                       closing brace */
                char esc_h2[ANI_SZ_512];
                ani_json_escape(esc_h2, sizeof(esc_h2), hres.qualified_name);
                snprintf(hp, sizeof(hp), "{\"handler\":\"%s\"}", esc_h2);
                ani_gbuf_insert_edge(gbuf, h->id, rid, "HANDLES", hp);
            }
        }
    }
}
...
static void detect_url_in_args(ani_gbuf_t *gbuf, const ani_gbuf_node_t *source,
                               const ANICall *call) {
    for (int ai = 0; ai < call->arg_count; ai++) {
        const ANICallArg *ca = &call->args[ai];
        ...
        char norm[ANI_SZ_256];
        if (!normalize_url_arg(url, norm, (int)sizeof(norm))) {
            continue;
        }
        if (!ani_service_pattern_is_http_route_literal(norm, call->callee_name)) {
            continue;
        }
        char route_qn[ANI_ROUTE_QN_SIZE];
        char cpath[ANI_SZ_256];
        snprintf(route_qn, sizeof(route_qn), "__route__ANY__%s",
                 ani_route_canon_path(norm, cpath, sizeof(cpath)));
        int64_t route_id = ani_gbuf_upsert_node(gbuf, "Route", norm, route_qn, "", 0, 0,
                                                "{\"source\":\"arg_url\"}");
        char esc_c[ANI_SZ_256];
        char esc_n[ANI_SZ_256];
        ani_json_escape(esc_c, sizeof(esc_c), call->callee_name);
        ani_json_escape(esc_n, sizeof(esc_n), norm);
        char eprops[ANI_SZ_512];
        snprintf(eprops, sizeof(eprops),
                 "{\"callee\":\"%s\",\"url_path\":\"%s\",\"via\":\"arg_url\"}", esc_c, esc_n);
        ani_gbuf_insert_edge(gbuf, source->id, route_id, "HTTP_CALLS", eprops);
        break;
    }
}
...
bool extract_grpc_service_method(const char *callee, char *service, size_t srv_sz, char *method,
    ...
    const char *s = raw;
    if (strncmp(s, "pb.New", ANI_SZ_6) == 0) {
        s += ANI_SZ_6;
    } else if (strncmp(s, "pb.", ANI_SZ_3) == 0 || strncmp(s, "New", ANI_SZ_3) == 0) {
        s += ANI_SZ_3;
    }
...
static void emit_grpc_edge(ani_gbuf_t *gbuf, const ani_gbuf_node_t *source, const ANICall *call,
                           const ani_resolution_t *res) {
    char service[ANI_SZ_256];
    char method[ANI_SZ_256];
    ...
    char route_name[ANI_SZ_256];
    snprintf(route_name, sizeof(route_name), "%s/%s", service, method);

    int64_t route_id = ani_gbuf_upsert_node(gbuf, "Route", route_name, route_qn, "", 0, 0,
                                            "{\"source\":\"grpc\"}");
    ...
    ani_gbuf_insert_edge(gbuf, source->id, route_id, "GRPC_CALLS", props);
}

/* Emit GRAPHQL_CALLS edge. Extract operation from first string arg if available. */
static void emit_graphql_edge(ani_gbuf_t *gbuf, const ani_gbuf_node_t *source, const ANICall *call,
                              const ani_resolution_t *res) {
    ...
    if (strncmp(p, "query ", ANI_SZ_6) == 0) {
        p += ANI_SZ_6;
    } else if (strncmp(p, "mutation ", ANI_SZ_8) == 0) {
        p += ANI_SZ_8;
    }
    ...
    int64_t route_id =
        ani_gbuf_upsert_node(gbuf, "Route", p, route_qn, "", 0, 0, "{\"source\":\"graphql\"}");

    char esc_c[ANI_SZ_256];
    char esc_op[ANI_SZ_512];
    ani_json_escape(esc_c, sizeof(esc_c), call->callee_name);
    ani_json_escape(esc_op, sizeof(esc_op), p);
    char props[ANI_SZ_1K];
    snprintf(props, sizeof(props), "{\"callee\":\"%s\",\"operation\":\"%s\",\"confidence\":%.2f}",
             esc_c, esc_op, res->confidence);
    ani_gbuf_insert_edge(gbuf, source->id, route_id, "GRAPHQL_CALLS", props);
}

/* Emit TRPC_CALLS edge. Extract procedure path from callee chain. */
static void emit_trpc_edge(ani_gbuf_t *gbuf, const ani_gbuf_node_t *source, const ANICall *call,
                           const ani_resolution_t *res) {
    ...
    int64_t route_id =
        ani_gbuf_upsert_node(gbuf, "Route", p, route_qn, "", 0, 0, "{\"source\":\"trpc\"}");
    ...
    ani_gbuf_insert_edge(gbuf, source->id, route_id, "TRPC_CALLS", props);
}
...
static void emit_service_edge(ani_gbuf_t *gbuf, const ani_gbuf_node_t *source,
                              const ani_gbuf_node_t *target, const ANICall *call,
    ...
    if (svc == ANI_SVC_NONE && ani_service_pattern_route_method(call->callee_name) != NULL) {
        svc = ANI_SVC_ROUTE_REG;
    }
    ...
    if (svc == ANI_SVC_NONE && res->qualified_name) {
    ...
    bool has_topic = (arg && arg[0] != '\0' && svc == ANI_SVC_ASYNC && strlen(arg) > PP_ESC_SPACE);

    if ((svc == ANI_SVC_HTTP || svc == ANI_SVC_ASYNC) && (has_url || has_topic)) {
        emit_http_async_service_edge(gbuf, source, call, res, svc, arg);
    } else if (svc == ANI_SVC_GRPC) {
        emit_grpc_edge(gbuf, source, call, res);
    } else if (svc == ANI_SVC_GRAPHQL) {
        emit_graphql_edge(gbuf, source, call, res);
    } else if (svc == ANI_SVC_TRPC) {
        emit_trpc_edge(gbuf, source, call, res);
    } else if (svc == ANI_SVC_CONFIG) {
...
static const ani_gbuf_node_t *find_source_node(const ani_gbuf_t *gbuf, const char *project,
                                               const char *rel, const char *enclosing_qn) {
    const ani_gbuf_node_t *src = NULL;
    if (enclosing_qn) {
        src = ani_gbuf_find_by_qn(gbuf, enclosing_qn);
    ...
    if (!src) {
        ...
        src = ani_gbuf_find_by_qn(gbuf, file_qn);
        free(file_qn);
    }
    return src;
}
...
static void try_field_type_hint(resolve_ctx_t *rc, ani_resolution_t *res, const char *callee_name,
                                int64_t source_id) {
...
static ANIResolvedCall *const LSP_IDX_AMBIGUOUS = (ANIResolvedCall *)(uintptr_t)1;

static bool lsp_idx_keep_best(ANIHashTable *index, const char *key, ANIResolvedCall *candidate,
                              const ani_gbuf_t *gbuf, const char *project_name,
                              bool allow_tail_match) {
    if (!index || !key || !candidate) {
        return false;
    }
    ANIResolvedCall *existing = (ANIResolvedCall *)ani_ht_get(index, key);
    if (!existing) {
        char *owned_key = strdup(key);
        if (owned_key) {
            ani_ht_set(index, owned_key, candidate);
            if (ani_ht_get(index, key) == candidate) {
                return true;
            }
            free(owned_key);
        }
    ...
    if (!ani_pipeline_invocation_targets_equal(existing->callee_qn, candidate->callee_qn, gbuf,
                                               project_name, allow_tail_match)) {
        const char *stored_key = ani_ht_get_key(index, key);
        if (stored_key) {
            ani_ht_set(index, stored_key, LSP_IDX_AMBIGUOUS);
        }
        return ani_ht_get(index, key) != NULL;
    }
    if (candidate->confidence > existing->confidence) {
        const char *stored_key = ani_ht_get_key(index, key);
        if (stored_key) {
            ani_ht_set(index, stored_key, candidate);
        }
    }
    return ani_ht_get(index, key) != NULL;
}
...
static bool lsp_idx_insert_leaf(ANIHashTable *index, ANIResolvedCall *candidate, const char *leaf,
                                bool exact_site, const ani_gbuf_t *gbuf, const char *project_name,
                                bool allow_tail_match) {
...
static const ANIResolvedCall *lsp_idx_lookup(const ANIHashTable *index, const ANICall *call,
    ...
    const char *leaf = ani_pipeline_call_callee_leaf(call->callee_name);
    if (!leaf || !leaf[0]) {
        return NULL;
    }
    ...
    const ANIResolvedCall *candidate = (const ANIResolvedCall *)ani_ht_get(index, key);
    free(key);
    ...
    if (!candidate || candidate->kind != ANI_RESOLVED_INVOCATION || !candidate->caller_qn ||
        strcmp(candidate->caller_qn, call->enclosing_func_qn) != 0 ||
        candidate->confidence < ANI_LSP_CONFIDENCE_FLOOR) {
        return NULL;
    }
    int site_rank = ani_pipeline_invocation_site_rank(candidate, call);
    int expected_rank = exact_site ? 2 : 1;
    return site_rank == expected_rank &&
                   ani_pipeline_invocation_leaf_matches(candidate, call, site_rank)
               ? candidate
               : NULL;
}

/* Resolve calls for one file and emit CALLS/HTTP_CALLS/ASYNC_CALLS edges. */
static void resolve_file_calls(resolve_ctx_t *rc, resolve_worker_state_t *ws, ANIFileResult *result,
                               const char *rel, const char *module_qn, const char **imp_keys,
                               const char **imp_vals, int imp_count, ANILanguage lang) {
    ...
    ANIHashTable *lsp_exact_idx = NULL;
    ANIHashTable *lsp_legacy_idx = NULL;
    ...
    if (result->calls.count > 0 && result->resolved_calls.count > 0) {
        ...
        lsp_exact_idx = ani_ht_create(capacity);
        lsp_legacy_idx = ani_ht_create(capacity);
        if (!lsp_exact_idx) {
        ...
        if (lsp_exact_idx || lsp_legacy_idx) {
            for (int i = 0; i < result->resolved_calls.count; i++) {
                ANIResolvedCall *rc_e = &result->resolved_calls.items[i];
                if (rc_e->kind != ANI_RESOLVED_INVOCATION || !rc_e->caller_qn || !rc_e->callee_qn ||
                    rc_e->confidence < ANI_LSP_CONFIDENCE_FLOOR) {
                    continue;
                }
                bool exact_site =
                    ani_pipeline_source_site_present(rc_e->site_start_byte, rc_e->site_end_byte);
                bool legacy_site =
                    ani_pipeline_source_site_legacy(rc_e->site_start_byte, rc_e->site_end_byte);
                if (!exact_site && !legacy_site) {
                    continue;
                }
                ANIHashTable *index = exact_site ? lsp_exact_idx : lsp_legacy_idx;
                bool inserted =
                    lsp_idx_insert_leaf(index, rc_e, ani_lsp_bare_segment(rc_e->callee_qn),
                ...
                if (rc_e->reason && ani_pipeline_invocation_reason_join_strategy(rc_e->strategy)) {
                    inserted = lsp_idx_insert_leaf(index, rc_e, ani_lsp_bare_segment(rc_e->reason),
    ...
    for (int c = 0; c < result->calls.count; c++) {
        ...
        const ani_gbuf_node_t *source_node =
         ...
         * Same helper + same ANI_LSP_CONFIDENCE_FLOOR as the sequential
         ...
         * fallbacks are JVM-only (see ani_pipeline_lsp_allow_tail_match). */
        ani_resolution_t res = {0};
        const ANIResolvedCall *lsp = NULL;
        bool exact_key_built = true;
        bool exact_key_ambiguous = false;
        bool legacy_key_built = true;
        _rc_t0 = extract_now_ns();
        if (ani_pipeline_source_site_present(call->site_start_byte, call->site_end_byte)) {
        ...
        if (!lsp && !exact_index_authoritative) {
            ...
            lsp = ani_pipeline_find_lsp_resolution_in_graph(
        ...
        atomic_fetch_add_explicit(&rc->time_ns_rc_lsp_lookup, extract_now_ns() - _rc_t0,
                                  memory_order_relaxed);
        _rc_t0 = extract_now_ns();
        const ani_gbuf_node_t *lsp_target = NULL;
        if (lsp) {
            ...
            lsp_target = exact_external_target
                             ? ani_pipeline_lsp_target_node_strict(rc->main_gbuf, rc->project_name,
                                                                   lsp->callee_qn, allow_tail)
                             : ani_pipeline_lsp_target_node(rc->main_gbuf, rc->project_name,
                                                            lsp->callee_qn, allow_tail);
            if (lsp_target) {
        ...
        if ((!res.qualified_name || !res.qualified_name[0]) && !call->requires_lsp_resolution) {
            res = ani_registry_resolve(rc->registry, call->callee_name, module_qn, imp_keys,
                                       imp_vals, imp_count);
        }
        ...
        if (ani_perl_suppress_generic_match(lang == ANI_LANG_PERL, call->is_method,
        ...
        if (csvc == ANI_SVC_HTTP || csvc == ANI_SVC_ASYNC) {
            ...
            bool chas_url = cu && cu[0] != '\0' &&
                            (cu[0] == '/' || strstr(cu, "://") != NULL ||
                             (csvc == ANI_SVC_ASYNC && strlen(cu) > PP_ESC_SPACE));
            if (chas_url) {
        ...
        if (!res.qualified_name || res.qualified_name[0] == '\0') {
            if (ani_service_pattern_route_method(call->callee_name) != NULL) {
                ani_resolution_t fake_res = {.qualified_name = call->callee_name,
                                             .confidence = PP_HALF_CONF,
                                             .strategy = "callee_suffix"};
                emit_service_edge(ws->local_edge_buf, source_node, source_node, call, &fake_res,
                                  module_qn, rc->registry, rc->main_gbuf, imp_keys, imp_vals,
                                  imp_count, false);
            } else if (ani_service_pattern_is_global_fetch(call->callee_name)) {
                 ...
                 * from res->qualified_name via ani_service_pattern_match, which
         ...
         * second ani_gbuf_find_by_qn lookup. try_field_type_hint may have
        ...
        if (lsp_target && res.qualified_name == lsp_target->qualified_name) {
            ...
            target_node = ani_gbuf_find_by_qn(rc->main_gbuf, res.qualified_name);
        }
        atomic_fetch_add_explicit(&rc->time_ns_rc_target, extract_now_ns() - _rc_t0,
                                  memory_order_relaxed);
        if (target_node && source_node->id != target_node->id &&
            ani_suppress_cross_language_suffix_match(lang, target_node->file_path, res.strategy)) {
        ...
        if (!target_node || source_node->id == target_node->id) {
            ...
            ani_svc_kind_t psvc = ani_service_pattern_match(res.qualified_name);
            if ((psvc == ANI_SVC_HTTP || psvc == ANI_SVC_ASYNC) && !target_node) {
                const char *u = call->first_string_arg;
                bool url_or_topic = u && u[0] != '\0' &&
                                    (u[0] == '/' || strstr(u, "://") != NULL ||
                                     (psvc == ANI_SVC_ASYNC && strlen(u) > PP_ESC_SPACE));
                if (url_or_topic) {
    ...
    if (lsp_exact_idx) {
        ani_ht_foreach(lsp_exact_idx, lsp_idx_free_key, NULL);
        ani_ht_free(lsp_exact_idx);
    }
    if (lsp_legacy_idx) {
        ani_ht_foreach(lsp_legacy_idx, lsp_idx_free_key, NULL);
        ani_ht_free(lsp_legacy_idx);
    }
}

/* Resolve usages for one file. */
static void resolve_file_usages(resolve_ctx_t *rc, resolve_worker_state_t *ws,
                                ANIFileResult *result, const char *rel, const char *module_qn,
                                const char **imp_keys, const char **imp_vals, int imp_count,
                                ANILanguage lang) {
    ani_pipeline_lsp_reference_index_t reference_index = {0};
    bool reference_index_ready =
        ani_pipeline_lsp_reference_index_build(&result->resolved_calls, &reference_index);
    for (int u = 0; u < result->usages.count; u++) {
        ANIUsage *usage = &result->usages.items[u];
        if (!usage->ref_name) {
            continue;
        }
        ...
        const ani_gbuf_node_t *tgt = NULL;
        bool precise_call_reference = false;
        const ANIResolvedCall *semantic_reference = NULL;
        if (ani_pipeline_usage_semantic_reference_candidate(usage)) {
            bool allow_tail = ani_pipeline_lsp_allow_tail_match(lang);
            semantic_reference = ani_pipeline_find_lsp_reference_indexed_in_graph(
            ...
            if (semantic_reference &&
                !ani_pipeline_usage_allows_semantic_reference(usage, semantic_reference)) {
                semantic_reference = NULL;
            }
            if (semantic_reference) {
                tgt = ani_pipeline_lsp_target_node(rc->main_gbuf, rc->project_name,
                                                   semantic_reference->callee_qn, allow_tail);
                precise_call_reference = ani_pipeline_node_is_callable_target(tgt);
            }
        }
        if (!tgt) {
             ...
             * targets (ani_registry_resolve_lineage); every other language
            ...
            ani_resolution_t res =
                (lang == ANI_LANG_SQL)
                    ? ani_registry_resolve_lineage(rc->registry, usage->ref_name, module_qn,
                                                   imp_keys, imp_vals, imp_count)
                    : ani_registry_resolve(rc->registry, usage->ref_name, module_qn, imp_keys,
                                           imp_vals, imp_count);
            if (!res.qualified_name || res.qualified_name[0] == '\0') {
                continue;
            }
            if (!ani_pipeline_reference_candidate_fallback_allowed(lang, usage, res.strategy,
                                                                   imp_keys, imp_count)) {
                continue;
            }
            tgt = ani_gbuf_find_by_qn(rc->main_gbuf, res.qualified_name);
            ...
            if (tgt && ani_suppress_cross_language_ref(lang, tgt->file_path)) {
                continue;
            }
            /* #1942/#1962: a bare Go reference can never denote a struct
             * field; the member half of a selector may. */
            if (tgt && ani_go_suppress_bare_field_ref(lang == ANI_LANG_GO, usage->is_member_access,
                                                      tgt->label)) {
                continue;
            }
            if (usage->semantic_reference_blocked && (usage->semantic_reference_local_shadow ||
                                                      ani_pipeline_node_is_callable_target(tgt))) {
                continue;
            }
        }
        ...
        char uprops[ANI_SZ_512];
        char esc_ref[ANI_SZ_256]; /* sliced source text: escape quotes/newlines */
        ani_json_escape(esc_ref, sizeof(esc_ref), usage->ref_name);
        snprintf(uprops, sizeof(uprops), "{\"callee\":\"%s\"}", esc_ref);
        const char *edge_type = precise_call_reference ? "CALL_REFERENCE" : "USAGE";
        ani_gbuf_insert_edge(ws->local_edge_buf, src->id, tgt->id, edge_type, uprops);
        ws->usages_resolved++;
    }
    ani_pipeline_lsp_reference_index_free(&reference_index);
}
...
static void resolve_file_throws(resolve_ctx_t *rc, resolve_worker_state_t *ws,
    ...
    for (int t = 0; t < result->throws.count; t++) {
        ANIThrow *thr = &result->throws.items[t];
        ...
        ani_resolution_t res = ani_registry_resolve(rc->registry, thr->exception_name, module_qn,
                                                    imp_keys, imp_vals, imp_count);
        if (!res.qualified_name || res.qualified_name[0] == '\0') {
            continue;
        }
        const ani_gbuf_node_t *tgt = ani_gbuf_find_by_qn(rc->main_gbuf, res.qualified_name);
        if (!tgt || src->id == tgt->id) {
            continue;
        }
        ani_gbuf_insert_edge(ws->local_edge_buf, src->id, tgt->id, edge_type, "{}");
    }
}

/* Resolve reads/writes for one file. */
static void resolve_file_rw(resolve_ctx_t *rc, resolve_worker_state_t *ws, ANIFileResult *result,
                            const char *rel, const char *module_qn, const char **imp_keys,
                            const char **imp_vals, int imp_count, ANILanguage lang) {
    for (int r = 0; r < result->rw.count; r++) {
        ANIReadWrite *rw = &result->rw.items[r];
        if (!rw->var_name) {
            continue;
        }
        const ani_gbuf_node_t *src =
        ...
        ani_resolution_t res = ani_registry_resolve(rc->registry, rw->var_name, module_qn, imp_keys,
                                                    imp_vals, imp_count);
        if (!res.qualified_name || res.qualified_name[0] == '\0') {
            continue;
        }
         ...
         * sequential twin (pass_usages.c). */
        if (ani_suppress_cross_language_ref(lang, tgt->file_path)) {
            continue;
        }
        /* #1942/#1962: a bare Go reference can never denote a struct field;
         * a selector-LHS write (`t.err = x`) may bind it. */
        if (ani_go_suppress_bare_field_ref(lang == ANI_LANG_GO, rw->is_member_access, tgt->label)) {
            continue;
        }
        const char *etype = rw->is_write ? "WRITES" : "READS";
        ani_gbuf_insert_edge(ws->local_edge_buf, src->id, tgt->id, etype, "{}");
    }
}
...
static void resolve_def_inherits(resolve_ctx_t *rc, resolve_worker_state_t *ws,
                                 const ANIDefinition *def, const ani_gbuf_node_t *node,
    ...
    for (int b = 0; def->base_classes[b]; b++) {
        ...
        const ani_gbuf_node_t *bn = ani_gbuf_find_by_qn(rc->main_gbuf, bqn);
        if (bn && node->id != bn->id) {
            ...
            ani_gbuf_insert_edge(ws->local_edge_buf, node->id, bn->id,
                                 ani_semantic_base_edge_type(bn), "{}");
...
static void resolve_def_decorators(resolve_ctx_t *rc, resolve_worker_state_t *ws,
    ...
    for (int dc = 0; def->decorators[dc]; dc++) {
        ...
        ani_resolution_t res = ani_registry_resolve(rc->registry, fn, mq, ik, iv, ic);
        if ((!res.qualified_name || res.qualified_name[0] == '\0') && !strchr(fn, '.')) {
            ...
            char with_suffix[ANI_SZ_256];
            int wn = snprintf(with_suffix, sizeof(with_suffix), "%sAttribute", fn);
            if (wn > 0 && (size_t)wn < sizeof(with_suffix)) {
                res = ani_registry_resolve(rc->registry, with_suffix, mq, ik, iv, ic);
            }
        }
        const ani_gbuf_node_t *dn = NULL;
        if (res.qualified_name && res.qualified_name[0] != '\0') {
            dn = ani_gbuf_find_by_qn(rc->main_gbuf, res.qualified_name);
        }
        ...
        if (dn) {
            ...
            char syn_qn[ANI_SZ_512];
            snprintf(syn_qn, sizeof(syn_qn), "<decorator:%s>", fn);
            dn_id =
                ani_gbuf_upsert_node(ws->local_edge_buf, "Decorator", fn, syn_qn, "", 0, 0, "{}");
        }
        if (dn_id != 0 && node->id != dn_id) {
            ...
            char esc_dec[ANI_SZ_256];
            ani_json_escape(esc_dec, sizeof(esc_dec), def->decorators[dc]);
            char dp[ANI_SZ_512];
            snprintf(dp, sizeof(dp), "{\"decorator\":\"%s\"}", esc_dec);
            ani_gbuf_insert_edge(ws->local_edge_buf, node->id, dn_id, "DECORATES", dp);
            ...
            ani_gbuf_insert_edge(ws->local_edge_buf, node->id, dn_id, "USAGE", "{}");
            ws->semantic_resolved++;
        }
    }
}
...
static void resolve_file_semantic(resolve_ctx_t *rc, resolve_worker_state_t *ws,
                                  ANIFileResult *result, const char *module_qn,
                                  const char **imp_keys, const char **imp_vals, int imp_count) {
    for (int d = 0; d < result->defs.count; d++) {
        ANIDefinition *def = &result->defs.items[d];
        ...
        const ani_gbuf_node_t *node = ani_gbuf_find_by_qn(rc->main_gbuf, def->qualified_name);
        if (!node) {
            continue;
        }
    ...
    for (int t = 0; t < result->impl_traits.count; t++) {
        ANIImplTrait *it = &result->impl_traits.items[t];
        ...
        const ani_gbuf_node_t *tn = ani_gbuf_find_by_qn(rc->main_gbuf, tqn);
        const ani_gbuf_node_t *sn = ani_gbuf_find_by_qn(rc->main_gbuf, sqn);
        if (tn && sn && tn->id != sn->id) {
            ani_gbuf_insert_edge(ws->local_edge_buf, sn->id, tn->id, "IMPLEMENTS", "{}");
            ws->semantic_resolved++;
        }
    }
}
...
static ANITypeRegistry *pp_rust_shared_registry(resolve_ctx_t *rc) {
    ANITypeRegistry *p = atomic_load_explicit(&rc->rust_shared_reg, memory_order_acquire);
    if (p)
        return p;
    if (!rc->all_defs || rc->def_count <= 0)
        return NULL;
    ani_mutex_lock(&rc->rust_shared_mu);
    p = atomic_load_explicit(&rc->rust_shared_reg, memory_order_relaxed);
    if (!p) {
        ani_arena_init(&rc->rust_shared_arena);
        rc->rust_shared_arena_live = true;
        p = ani_rust_build_cross_registry(&rc->rust_shared_arena, rc->all_defs, rc->def_count);
        if (p) {
            char sb[96];
            snprintf(sb, sizeof(sb), "types=%d funcs=%d", p->type_count, p->func_count);
            ani_log_info("cross_lsp.rust_registry", "scale", sb);
        }
        atomic_store_explicit(&rc->rust_shared_reg, p, memory_order_release);
    }
    ani_mutex_unlock(&rc->rust_shared_mu);
    return p;
}
...
static ANITypeRegistry *pp_rust_shared_registry_get(void *ctx) {
    return pp_rust_shared_registry((resolve_ctx_t *)ctx);
}

static int pp_call_reference_site_count(const ANIFileResult *result, ANILanguage lang) {
    int count = 0;
    for (int i = 0; i < result->usages.count; i++) {
        if (ani_pipeline_usage_semantic_reference_candidate(&result->usages.items[i])) {
            count++;
        }
    }
    return count;
...
static int pp_qualified_lsp_site_count(const ANIFileResult *result) {
    int count = 0;
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const ANIResolvedCall *resolved = &result->resolved_calls.items[i];
        if ((resolved->kind == ANI_RESOLVED_INVOCATION ||
             resolved->kind == ANI_RESOLVED_CALL_REFERENCE) &&
            resolved->caller_qn && resolved->callee_qn &&
            resolved->confidence >= ANI_LSP_CONFIDENCE_FLOOR) {
            count++;
        }
    }
    return count;
 ...
 * syntax extractor cannot represent as a ANICall yet: Python operators and
...
static bool pp_has_pending_lsp_site(const ANIFileResult *result) {
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const ANIResolvedCall *resolved = &result->resolved_calls.items[i];
        if (resolved->kind != ANI_RESOLVED_INVOCATION &&
            resolved->kind != ANI_RESOLVED_CALL_REFERENCE) {
        ...
        if (resolved->reason || resolved->confidence < ANI_LSP_CONFIDENCE_FLOOR) {
            return true;
        }
    }
    return false;
...
static void resolve_worker(int worker_id, void *ctx_ptr) {
    ...
    ani_pxc_set_rust_manifest(rc->rust_manifest);

    if (!ws->local_edge_buf) {
        ws->local_edge_buf =
            ani_gbuf_new_shared_ids(rc->project_name, rc->repo_path, rc->shared_ids);
    }
    ...
    while (SKIP_ONE) {
        ...
        ani_scale_tick(&rc->scale, file_idx);
        ...
        ANIFileResult *result = rc->result_cache[file_idx];
        if (!result) {
        ...
        ANILanguage lang = rc->files[file_idx].language;
        ...
        bool jvm_cross_lsp = (lang == ANI_LANG_JAVA || lang == ANI_LANG_KOTLIN);
        bool rust_workspace_cross_lsp =
            (lang == ANI_LANG_RUST && rc->rust_manifest && rc->rust_manifest->member_count > 0);
        ...
        bool cross_lsp_eligible =
            (rc->all_defs && rc->def_count > 0 && ani_pxc_has_cross_lsp(lang) &&
        ...
        ani_pxc_build_import_map(rc->main_gbuf, rc->project_name, rel, lang, result, &imp_keys,
                                 &imp_vals, &imp_count);
         ...
         * through ani_registry_resolve. Same callee_name appears in
        ...
        ani_registry_reach_cache_begin(result->calls.count + result->usages.count + 64);
        ...
        ani_registry_import_map_cache_begin(imp_keys, imp_vals, imp_count);

        /* THE BIG ONE: per-file cache of ani_registry_resolve results.
        ...
        ani_registry_resolve_cache_begin(result->calls.count + result->usages.count + 64);

        char *module_qn =
            ani_pipeline_fqn_module_dir(rc->project_name, rel, pp_module_is_dir(lang));

        /* ── Cross-file LSP (FUSED) ─────────────────────────────
         * Runs BEFORE resolve_file_calls so its additions to
         * result->resolved_calls are picked up by
         * ani_pipeline_find_lsp_resolution when calls become CALLS
        ...
        if (cross_lsp_eligible) {
            ...
            if ((!lsp_source || lsp_source_len <= 0) && rc->files[file_idx].path) {
                 ...
                 * by read_file's ani_max_file_bytes cap), freed below. */
            ...
            if (lsp_source && lsp_source_len > 0) {
                ...
                ani_index_mark_start(rel);
                ani_pxc_dispatch_file(lang, result, lsp_source, lsp_source_len, rel, def_module,
                 ...
                 * ani_extract_file on this thread will recreate the parser. */
                ani_destroy_thread_parser();
                ani_slab_reclaim();
                ...
                if (lsp_elapsed_ms > PP_TIMER_THRESH) {
                    ani_log_info("parallel.resolve.lsp_cross.slow", "elapsed_ms",
                 ...
                 * unaffected. Deliberately NOT recorded as a ani_file_error —
        ...
        ani_registry_reach_cache_end();
        ani_registry_import_map_cache_end();
        ani_registry_resolve_cache_end();

        free(module_qn);
        ani_pxc_free_import_map(imp_keys, imp_vals, imp_count);
    ...
    ani_pxc_set_rust_manifest(NULL);
    ani_destroy_thread_parser();
    ani_slab_destroy_thread();
    ani_service_pattern_cache_end();
}

int ani_parallel_resolve(ani_pipeline_ctx_t *ctx, const ani_file_info_t *files, int file_count,
                         ANIFileResult **result_cache, _Atomic int64_t *shared_ids,
                         int worker_count, ANILSPDef *all_defs, int def_count,
                         char *const *def_modules, struct ANIModuleDefIndex *module_def_index,
                         void *cross_registries_v) {
    /* See header: typed as void* across the TU boundary; cast back here. */
    ANICrossLspRegistries *cross_registries = (ANICrossLspRegistries *)cross_registries_v;
    if (file_count == 0) {
        return 0;
    }

    ani_log_info("parallel.resolve.start", "files", itoa_log(file_count), "workers",
                 itoa_log(worker_count));
    ...
    for (int i = 0; i < file_count; i++) {
        if (result_cache[i] && files[i].language == ANI_LANG_RUST) {
            have_rust = true;
            break;
        }
    }
    ANIArena rust_manifest_arena;
    ANICargoManifest rust_manifest;
    bool rust_manifest_arena_live = false;
    const ANICargoManifest *rust_manifest_ptr = NULL;
    if (have_rust) {
        ani_arena_init(&rust_manifest_arena);
        rust_manifest_arena_live = true;
        if (ani_pxc_build_rust_manifest(ctx, &rust_manifest_arena, &rust_manifest)) {
    ...
    ani_mutex_init(&rc.rust_shared_mu);
    rc.rust_shared_arena_live = false;

    /* Sub-phase: Dispatch resolve workers (per-file call/usage resolution, PARALLEL) */
    ANI_PROF_START(t_resolve_dispatch);
    ani_parallel_for_opts_t opts = {.max_workers = worker_count, .force_pthreads = false};
    ani_scale_begin(&rc.scale, "parallel_resolve", (long)file_count);
    ani_parallel_for(worker_count, resolve_worker, &rc, opts);
    ani_scale_end(&rc.scale);
    ANI_PROF_END_N("parallel_resolve", "1_dispatch_workers_parallel", t_resolve_dispatch,
                   file_count);
    ...
    if (rc.rust_shared_arena_live) {
        ani_arena_destroy(&rc.rust_shared_arena);
        rc.rust_shared_arena_live = false;
    }
    ani_mutex_destroy(&rc.rust_shared_mu);
    if (rust_manifest_arena_live) {
        ani_arena_destroy(&rust_manifest_arena);
    }

    /* Sub-phase: Merge all local edge bufs into main gbuf (SEQUENTIAL) */
    ANI_PROF_START(t_resolve_merge);
    int total_calls = 0;
    int total_usages = 0;
    ...
    for (int i = 0; i < worker_count; i++) {
        if (workers[i].local_edge_buf) {
            ani_gbuf_merge(ctx->gbuf, workers[i].local_edge_buf);
            ...
            ani_gbuf_free(workers[i].local_edge_buf);
        }
    }
    ANI_PROF_END_N("parallel_resolve", "2_merge_edge_bufs_seq", t_resolve_merge,
                   total_calls + total_usages);

    ani_aligned_free(workers);

    /* Go-style implicit interface satisfaction (needs full graph, serial) */
    int go_impl = ani_pipeline_implements_go(ctx);
    ...
    total_lsp_overrides += ani_pipeline_override_explicit(ctx);
    ...
    if (cross_files > 0) {
        char cf_buf[ANI_SZ_32];
        char nf_buf[ANI_SZ_32];
        char cu_buf[ANI_SZ_32];
        char pk_buf[ANI_SZ_32];
        ...
        snprintf(pk_buf, sizeof(pk_buf), "%llu",
                 (unsigned long long)(def_count > 0
                                          ...
                                          : 0ULL));
        ani_log_info("parallel.resolve.cross_lsp_cost", "cross_lsp_ms", cu_buf, "files", nf_buf,
        ...
        ani_pxc_filter_stats(&reg_defs, &reg_files, &flt_files, &flt_failed);
        if (reg_files > 0) {
            char rd_buf[ANI_SZ_32];
            char ff_buf[ANI_SZ_32];
            char fp_buf[ANI_SZ_32];
            ...
            ani_log_info("parallel.resolve.perfile_registry", "defs_per_file", rd_buf, "defs_total",
                         itoa_log(def_count), "filtered_files", fp_buf, "filter_failed", ff_buf);
        }
    }

    ani_log_info("parallel.resolve.done", "calls", itoa_log(total_calls), "usages",
    ...
    ani_log_info("parallel.resolve.phase_summary", "total_loop_cpu_ms", loop_buf, "files_visited",
                 visits_buf);
    ...
    snprintf(cal_buf, sizeof(cal_buf), "%llu",
             (unsigned long long)(atomic_load_explicit(&rc.time_ns_calls, memory_order_relaxed) /
                                  1000000ULL));
    ani_log_info("parallel.resolve.phase_ms_a", "import_map", imp_buf, "cross_lsp", xls_buf,
    ...
    snprintf(sem_buf, sizeof(sem_buf), "%llu",
             (unsigned long long)(atomic_load_explicit(&rc.time_ns_semantic, memory_order_relaxed) /
                                  1000000ULL));
    ani_log_info("parallel.resolve.phase_ms_b", "resolve_usages", use_buf, "resolve_throws",
    ...
    snprintf(emt_buf, sizeof(emt_buf), "%llu",
             (unsigned long long)(atomic_load_explicit(&rc.time_ns_rc_emit, memory_order_relaxed) /
                                  1000000ULL));
    ani_log_info("parallel.resolve.calls_breakdown", "find_source", src_buf, "lsp_lookup", lsp_buf,
                 "resolve", rsv_buf);
    ani_log_info("parallel.resolve.calls_breakdown2", "field_hint", hnt_buf, "find_target", tgt_buf,
                 "emit_edge", emt_buf);
    ...
    char tl_buf[ANI_SZ_32];
    char tc_buf[ANI_SZ_32];
    char tp_buf[ANI_SZ_32];
    char fb_buf[ANI_SZ_32];
    ...
    snprintf(fb_buf, sizeof(fb_buf), "%llu",
             (unsigned long long)atomic_load_explicit(&g_lsp_linear_fallback_rows,
                                                      memory_order_relaxed));
    ani_log_info("parallel.resolve.scan_cost", "tail_lookups", tl_buf, "tail_candidates", tc_buf,
