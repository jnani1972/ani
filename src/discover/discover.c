...
#include "ani.h" // ANILanguage, ANI_LANG_COUNT, ANI_LANG_JSON
...
int ani_gitignore_match_result(const ani_gitignore_t *gi, const char *rel_path, bool is_dir);
...
static const char *ALWAYS_SKIP_DIRS[] = {
    ...
    /* Misc */
    ".ani", ".qdrant_code_embeddings", ".tmp", "vendor", "vendored", NULL};
...
static char *strip_matching_quotes(char *s) {
    size_t len = strlen(s);
    if (len >= ANI_QUOTE_PAIR && ((s[0] == '"' && s[len - SKIP_ONE] == '"') ||
                                  (s[0] == '\'' && s[len - SKIP_ONE] == '\''))) {
        s[len - SKIP_ONE] = '\0';
        return s + SKIP_ONE;
    }
...
static bool expand_git_path(const char *path, char *out, size_t out_sz) {
    ...
    char normalized[ANI_SZ_4K];
    snprintf(normalized, sizeof(normalized), "%s", path);
    ani_normalize_path_sep(normalized);
...
static bool read_core_excludes_file(const char *config_path, char *out, size_t out_sz) {
    FILE *f = ani_fopen(config_path, "r");
    if (!f) {
        return false;
    }
...
static bool resolve_xdg_git_config_dir(char *out, size_t out_sz) {
    char env[ANI_SZ_4K];
    if (ani_safe_getenv("XDG_CONFIG_HOME", env, sizeof(env), NULL) && env[0] != '\0') {
...
bool ani_matches_fast_pattern(const char *filename, ani_index_mode_t mode) {
    if (!filename || mode == ANI_MODE_FULL) {
        return false;
    }
...
static bool file_list_should_stop(file_list_t *fl) {
    ...
    if (!fl->failed && fl->deadline_ms != 0 && ani_now_ms() >= fl->deadline_ms) {
        fl->failed = true;
    }
...
static void file_list_add_excluded(file_list_t *fl, const char *rel_path) {
    ...
    if (fl->excluded_count >= fl->excluded_cap) {
        int new_cap = fl->excluded_cap ? fl->excluded_cap * PAIR_LEN : ANI_SZ_64;
...
static void file_list_add_ignored(file_list_t *fl, const char *rel_path, const char *reason) {
    ...
    fl->ignored_total++;
    if (fl->ignored_count >= ANI_DISCOVER_IGNORED_CAP) {
        return; /* counted above — truncation stays visible via the total */
    }
    if (fl->ignored_count >= fl->ignored_cap) {
        int new_cap = fl->ignored_cap ? fl->ignored_cap * PAIR_LEN : ANI_SZ_64;
        ani_ignored_file_t *grown = realloc(fl->ignored, new_cap * sizeof(ani_ignored_file_t));
        if (!grown) {
            fl->failed = true;
            return;
        }
...
static void fl_add(file_list_t *fl, const char *abs_path, const char *rel_path, ANILanguage lang,
                   int64_t size) {
    ...
    if (fl->count >= fl->capacity) {
        int new_cap = fl->capacity ? fl->capacity * PAIR_LEN : ANI_SZ_256;
        ani_file_info_t *new_files = realloc(fl->files, new_cap * sizeof(ani_file_info_t));
        if (!new_files) {
            fl->failed = true;
            return;
        }
    ...
    ani_file_info_t *fi = &fl->files[fl->count++];
    fi->path = path_copy;
...
/* Non-negatable safety core: built-in skip dirs that a .aniignore negation
 * can NEVER un-skip. A repo-committed .aniignore must not be able to defeat
 ...
 *
 * ani_resolve_cache_dir() returns a pointer to a static thread-local buffer, so
...
static _Thread_local char g_walk_cache_dir[ANI_SZ_4K];

static void walk_cache_dir_snapshot(void) {
    const char *cache = ani_workspace_cache_dir();
 ...
 * ANI_CACHE_DIR sits inside a repository — which happens in tests and is legal in
...
static bool should_skip_directory(const char *entry_name, const char *rel_path,
                                  const ani_discover_opts_t *opts, const ani_gitignore_t *gitignore,
                                  const ani_gitignore_t *global_gi,
                                  const ani_gitignore_t *aniignore, const ani_gitignore_t *local_gi,
                                  const char *local_gi_prefix) {
    if (ani_should_skip_dir(entry_name, opts ? opts->mode : ANI_MODE_FULL)) {
        /* #500: a .aniignore negation (e.g. "!obj/") whose rule is the last
        ...
        bool unskipped = aniignore && !is_safety_core_dir(entry_name) &&
                         ani_gitignore_match_result(aniignore, rel_path, true) < 0;
        if (!unskipped) {
            return true;
        }
    }
    if (gitignore && ani_gitignore_matches(gitignore, rel_path, true)) {
        return true;
    }
    bool global_ignored = global_gi && ani_gitignore_matches(global_gi, rel_path, true);
    if (local_gi) {
        const char *lrel = local_rel_path(rel_path, local_gi_prefix);
        if (ani_gitignore_matches(local_gi, lrel, true)) {
            return true;
        }
    }
    if (aniignore) {
        int ani_result = ani_gitignore_match_result(aniignore, rel_path, true);
        if (ani_result > 0) {
            return true;
        }
        if (ani_result < 0 && global_ignored) {
            return false;
        }
    }
 ...
 * semantics — including the .aniignore negation un-ignoring a global-gitignore
...
static const char *file_skip_reason(const char *entry_name, const char *rel_path,
                                    const ani_discover_opts_t *opts,
                                    const ani_gitignore_t *gitignore,
                                    const ani_gitignore_t *global_gi,
                                    const ani_gitignore_t *aniignore,
                                    const ani_gitignore_t *local_gi, const char *local_gi_prefix,
                                    off_t file_size) {
    ani_index_mode_t mode = opts ? opts->mode : ANI_MODE_FULL;
    if (ani_has_ignored_suffix(entry_name, mode)) {
        return "ignored-suffix";
    }
    if (ani_should_skip_filename(entry_name, mode)) {
        return "skip-list";
    }
    if (ani_matches_fast_pattern(entry_name, mode)) {
        return "fast-pattern";
    }
    if (gitignore && ani_gitignore_matches(gitignore, rel_path, false)) {
        return "gitignore";
    }
    bool global_ignored = global_gi && ani_gitignore_matches(global_gi, rel_path, false);
    if (local_gi) {
        const char *lrel = local_rel_path(rel_path, local_gi_prefix);
        if (ani_gitignore_matches(local_gi, lrel, false)) {
            return "gitignore";
        }
    }
    if (aniignore) {
        int ani_result = ani_gitignore_match_result(aniignore, rel_path, false);
        if (ani_result > 0) {
            return "aniignore";
        }
        if (ani_result < 0 && global_ignored) {
...
static ANILanguage detect_file_language(const char *entry_name, const char *abs_path) {
    ANILanguage lang = ani_language_for_filename(entry_name);
...
#ifdef _WIN32
    wchar_t *wpath = ani_path_to_wide(path);
    if (!wpath) {
...
static void walk_dir_process_file(const char *abs_path, const char *rel_path, const char *name,
    ...
    const char *skip_reason = file_skip_reason(name, rel_path, opts, gitignore, global_gi,
                                               aniignore, local_gi, local_gi_prefix, size);
    if (skip_reason) {
    ...
    ANILanguage lang = detect_file_language(name, abs_path);
    if (lang == ANI_LANG_COUNT) {
        return;
    }
...
typedef struct {
    char dir[ANI_SZ_4K];
    char prefix[ANI_SZ_4K];
    ani_gitignore_t *local_gi;       /* nested .gitignore for this subtree */
    char local_gi_prefix[ANI_SZ_4K]; /* rel_prefix when local_gi was loaded */
} walk_frame_t;
...
static void walk_push_subdir(walk_stack_t *ws, const char *abs_path, const char *rel_path,
    ...
    int directory_length = snprintf(slot->dir, ANI_SZ_4K, "%s", abs_path);
    int prefix_length = snprintf(slot->prefix, ANI_SZ_4K, "%s", rel_path);
    if (directory_length <= 0 || directory_length >= ANI_SZ_4K || prefix_length < 0 ||
        prefix_length >= ANI_SZ_4K) {
        out->failed = true;
        return;
    }
    slot->local_gi = parent->local_gi;
    int local_prefix_length =
        snprintf(slot->local_gi_prefix, ANI_SZ_4K, "%s", parent->local_gi_prefix);
    if (local_prefix_length < 0 || local_prefix_length >= ANI_SZ_4K) {
        out->failed = true;
        return;
    }
    ws->top++;
...
static void walk_dir_process_entry(ani_dirent_t *entry, const walk_frame_t *frame,
                                   const ani_discover_opts_t *opts,
                                   const ani_gitignore_t *gitignore,
                                   const ani_gitignore_t *global_gi,
                                   const ani_gitignore_t *aniignore, walk_stack_t *ws,
                                   file_list_t *out) {
    ...
    if (S_ISDIR(st.st_mode)) {
        if (!dir_is_cache_tree(abs_path) &&
            !should_skip_directory(entry->name, rel_path, opts, gitignore, global_gi, aniignore,
        ...
        walk_dir_process_file(abs_path, rel_path, entry->name, opts, gitignore, global_gi,
                              aniignore, frame->local_gi, frame->local_gi_prefix, st.st_size, out);
    }
}

static bool walk_owned_gitignore_append(ani_gitignore_t ***owned, size_t *count, size_t *capacity,
                                        ani_gitignore_t *gitignore) {
    ...
    if (*count == *capacity) {
        ...
        ani_gitignore_t **grown = realloc(*owned, next_capacity * sizeof(*grown));
        if (!grown) {
            return false;
        }
        *owned = grown;
...
static void walk_dir(const char *dir_path, const char *rel_prefix, const ani_discover_opts_t *opts,
                     const ani_gitignore_t *gitignore, const ani_gitignore_t *global_gi,
                     const ani_gitignore_t *aniignore, file_list_t *out) {
    walk_stack_t ws = {
    ...
    ani_gitignore_t **owned_gis = NULL;
    size_t owned_count = 0;
    size_t owned_capacity = 0;

    int initial_directory_length = snprintf(ws.frames[0].dir, ANI_SZ_4K, "%s", dir_path);
    int initial_prefix_length = snprintf(ws.frames[0].prefix, ANI_SZ_4K, "%s", rel_prefix);
    if (initial_directory_length <= 0 || initial_directory_length >= ANI_SZ_4K ||
        initial_prefix_length < 0 || initial_prefix_length >= ANI_SZ_4K) {
        out->failed = true;
        free(ws.frames);
        return;
    }
    ...
    while (ws.top > 0 && !file_list_should_stop(out)) {
        ...
        ani_gitignore_t *loaded = try_load_nested_gitignore(&frame);
        if (loaded) {
            int local_prefix_length =
        ...
        ani_dir_t *d = ani_opendir(frame.dir);
        if (!d) {
            if (out->count_only) {
                out->failed = true;
            }
        ...
        ani_dirent_t *entry;
        while (!file_list_should_stop(out) && (entry = ani_readdir(d)) != NULL) {
            walk_dir_process_entry(entry, &frame, opts, gitignore, global_gi, aniignore, &ws, out);
        }
        ani_closedir(d);
    }
    for (size_t i = 0; i < owned_count; i++) {
        ani_gitignore_free(owned_gis[i]);
    }
    free(owned_gis);
    free(ws.frames);
}
...
static bool resolve_git_common_dir(const char *repo_path, char *common_dir, size_t cd_sz) {
    ...
    if (S_ISDIR(st.st_mode)) {
        ...
        ani_normalize_path_sep(common_dir);
        return true;
    }
     ...
     * ani_fopen (not raw fopen) so non-ASCII repo paths open on Windows. */
    FILE *f = ani_fopen(dot_git, "r");
    if (!f) {
        return false;
    }
    char git_dir[ANI_SZ_4K];
        ...
        if (strncmp(gs, "gitdir:", 7) == 0) {
            ...
            if (val[0] != '\0') {
                if (discover_path_is_absolute(val)) {
                    ...
                    ani_normalize_path_sep(git_dir);
                } else {
...
int ani_discover(const char *repo_path, const ani_discover_opts_t *opts, ani_file_info_t **out,
                 int *count) {
    return ani_discover_ex(repo_path, opts, out, count, NULL, NULL);
}
...
int ani_discover_ex(const char *repo_path, const ani_discover_opts_t *opts, ani_file_info_t **out,
    ...
    return ani_discover_ex2(repo_path, opts, out, count, excluded_out, excluded_count_out, NULL,
                            NULL, NULL);
}

static ani_discover_status_t discover_impl(const char *repo_path, const ani_discover_opts_t *opts,
                                           ani_file_info_t **out, int *count, char ***excluded_out,
    ...
    *count = 0;
    ...
    ani_gitignore_t *gitignore = NULL;
    char gi_path[ANI_SZ_4K];
    struct stat gi_stat;
    ...
    if (is_git_repo) {
        ...
        if (git_exclude) {
            if (!gitignore) {
                ...
                (void)ani_gitignore_merge(gitignore, git_exclude);
                ani_gitignore_free(git_exclude);
            }
        }
    }

    ani_gitignore_t *global_gi = NULL;
    if (has_git_config && resolve_global_excludes_path(gi_path, sizeof(gi_path))) {
        global_gi = ani_gitignore_load(gi_path);
    }

    /* Load aniignore if specified or exists at repo root */
    ani_gitignore_t *aniignore = NULL;
    if (opts && opts->ignore_file) {
        aniignore = ani_gitignore_load(opts->ignore_file);
    } else {
        snprintf(gi_path, sizeof(gi_path), "%s/.aniignore", repo_path);
        aniignore = ani_gitignore_load(gi_path);
    }
    ...
    walk_dir(repo_path, "", opts, gitignore, global_gi, aniignore, &fl);

    /* Cleanup */
    ani_gitignore_free(gitignore);
    ani_gitignore_free(global_gi);
    ani_gitignore_free(aniignore);

    if (count_only) {
        ani_discover_free(fl.files, fl.count);
        ani_discover_free_excluded(fl.excluded, fl.excluded_count);
        ani_discover_free_ignored(fl.ignored, fl.ignored_count);
        *count = fl.count;
        if (fl.failed) {
            return ANI_DISCOVER_ERROR;
        }
        return fl.limit_exceeded ? ANI_DISCOVER_LIMIT_EXCEEDED : ANI_DISCOVER_OK;
    }
    if (fl.failed) {
        ani_discover_free(fl.files, fl.count);
        ani_discover_free_excluded(fl.excluded, fl.excluded_count);
...
ani_discover_status_t ani_discover_count_bounded(const char *repo_path,
    ...
    int count = 0;
    ani_discover_status_t status = discover_impl(repo_path, opts, &files, &count, NULL, NULL, NULL,
                                                 NULL, NULL, true, max_files, deadline_ms);
    ani_discover_free(files, count);
    *count_out = status == ANI_DISCOVER_ERROR ? -1 : count;
    return status;
}
