 ...
 * number of groups. Patterns come from a committed .gitignore / .aniignore and
...
static void gi_add_pattern(ani_gitignore_t *gi, const char *line, int len) {
...
ani_gitignore_t *ani_gitignore_parse(const char *content) {
    ...
    ani_gitignore_t *gi = calloc(ANI_ALLOC_ONE, sizeof(ani_gitignore_t));
    if (!gi) {
        return NULL;
    }
...
ani_gitignore_t *ani_gitignore_load(const char *path) {
    ...
    ani_gitignore_t *gi = ani_gitignore_parse(buf);
    free(buf);
    return gi;
}
...
bool ani_gitignore_matches(const ani_gitignore_t *gi, const char *rel_path, bool is_dir) {
    return ani_gitignore_match_result(gi, rel_path, is_dir) > 0;
}
...
char *(*ani_gitignore_merge_dup_hook_for_test)(const char *) = NULL;
...
bool ani_gitignore_merge(ani_gitignore_t *dst, const ani_gitignore_t *src) {
    ...
    for (int i = 0; i < src->count; i++) {
        char *pat = ani_gitignore_merge_dup_hook_for_test
                        ? ani_gitignore_merge_dup_hook_for_test(src->patterns[i].pattern)
