 ...
 *     query parameter VALUES are decoded (ani_http_query_param), and
 *     decoded values containing NUL are rejected.
 */
#ifndef ANI_UI_HTTPD_H
#define ANI_UI_HTTPD_H
...
#define ANI_HTTP_MAX_HEAD (16 * 1024)
/* Maximum request body accepted via Content-Length. */
#define ANI_HTTP_MAX_BODY (1024 * 1024)
/* Default per-connection receive deadline. */
#define ANI_HTTP_RECV_DEADLINE_MS 5000
typedef struct ani_httpd ani_httpd_t;         /* listener */
typedef struct ani_http_conn ani_http_conn_t; /* accepted connection */

/* Observable transport phase used by deterministic lifecycle tests. */
typedef enum {
    ANI_HTTPD_ACTIVITY_IDLE = 0,
    ANI_HTTPD_ACTIVITY_READING_REQUEST = 1,
    ANI_HTTPD_ACTIVITY_RESPONDING = 2,
} ani_httpd_activity_t;
...
typedef struct {
    char method[ANI_SZ_16];
    char path[ANI_SZ_2K];
    char query[ANI_SZ_2K];
    unsigned char http_minor; /* 0 for HTTP/1.0, 1 for HTTP/1.1 */
    char origin[ANI_SZ_256];
    char host[ANI_SZ_256];
    char content_type[ANI_SZ_128];
    char accept_language[ANI_SZ_256];
    char *body;
    size_t body_len;
} ani_http_req_t;
