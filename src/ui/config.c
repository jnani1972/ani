 ...
 *
 * Config file: ~/.cache/ani/config.json
...
void ani_ui_config_load(ani_ui_config_t *cfg) {
    ...
    cfg->ui_enabled = ANI_UI_DEFAULT_ENABLED;
    cfg->ui_port = ANI_UI_DEFAULT_PORT;
    ...
    free(buffer);
    if (!doc) {
        ani_log_warn("ui.config.corrupt", "path", path);
...
bool ani_ui_config_save(const ani_ui_config_t *cfg) {
    if (!cfg || cfg->ui_port <= 0 || cfg->ui_port > 65535) {
        ani_log_error("ui.config.write_fail", "reason", "invalid_config");
        return false;
    }
    char path[ANI_SZ_1K];
    ani_ui_config_path(path, (int)sizeof(path));
    ...
    if (directory_ready && !ani_is_dir(dir)) {
        directory_ready = ani_mkdir_p(dir, 0750) || ani_is_dir(dir);
    }
    if (!directory_ready) {
        ani_log_error("ui.config.write_fail", "path", path, "reason", "create_directory");
        return false;
    }
    ...
    if (!json) {
        ani_log_error("ui.config.write_fail", "reason", "serialize");
        return false;
    }
    ...
    free(json);
    if (!saved) {
        ani_log_error("ui.config.write_fail", "path", path, "reason", "atomic_publish");
        return false;
    }

    ani_log_debug("ui.config.saved", "path", path);
    return true;
}
