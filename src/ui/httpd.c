...
#include <winsock2.h>
#include <ws2tcpip.h>
typedef SOCKET ani_sock_t;
#define ANI_SOCK_BAD INVALID_SOCKET
#define ani_sock_close closesocket
#else
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
...
#include <time.h>
#include <unistd.h>
typedef int ani_sock_t;
#define ANI_SOCK_BAD (-1)
#define ani_sock_close close
#endif

/* Suppress SIGPIPE on send: Linux has the per-call flag, macOS has the
 * per-socket option (set at accept), Windows has no signals. */
#ifdef MSG_NOSIGNAL
#define ANI_SEND_FLAGS MSG_NOSIGNAL
#else
#define ANI_SEND_FLAGS 0
#endif

/* A non-reading peer may occupy the sequential event loop only briefly. */
#define ANI_HTTP_SEND_DEADLINE_MS 1000
#define ANI_HTTP_SEND_SLICE_BYTES (64U * 1024U)
#ifdef _WIN32
...
#define ANI_HTTP_WAIT_SLICE_MS 50
#endif
...
struct ani_httpd {
    ...
    ani_mutex_t active_mutex;
    struct ani_http_conn *active;
    bool interrupted;
    int send_buffer_for_test;
    int send_deadline_for_test_ms;
};

struct ani_http_conn {
    ani_sock_t fd;
    ani_httpd_t *owner;
    int recv_deadline_ms;
...
static int wait_ready(ani_sock_t fd, bool writing, int timeout_ms) {
#ifdef _WIN32
    fd_set ready;
    fd_set errors;
    FD_ZERO(&ready);
...
static int wait_readable(ani_sock_t fd, int timeout_ms) {
...
static bool socket_set_nonblocking(ani_sock_t fd) {
#ifdef _WIN32
    u_long enabled = 1;
...
static void socket_shutdown(ani_sock_t fd) {
#ifdef _WIN32
...
static bool connection_interrupted(ani_http_conn_t *connection) {
    ani_httpd_t *owner = connection->owner;
    if (!owner)
        return false;
    ani_mutex_lock(&owner->active_mutex);
    bool interrupted = owner->interrupted;
    ani_mutex_unlock(&owner->active_mutex);
    return interrupted;
}
...
static int wait_connection_ready(ani_http_conn_t *connection, bool writing, int64_t deadline) {
    for (;;) {
...
#ifdef _WIN32
        if (timeout_ms > ANI_HTTP_WAIT_SLICE_MS)
            timeout_ms = ANI_HTTP_WAIT_SLICE_MS;
#endif
...
static int send_all(ani_http_conn_t *connection, const void *data, size_t len, int64_t deadline) {
    const char *p = data;
    size_t off = 0;
    while (off < len) {
        ...
        if (available > ANI_HTTP_SEND_SLICE_BYTES)
            available = ANI_HTTP_SEND_SLICE_BYTES;
#ifdef _WIN32
        int n = send(connection->fd, p + off, (int)available, ANI_SEND_FLAGS);
#else
        ssize_t n = send(connection->fd, p + off, available, ANI_SEND_FLAGS);
#endif
...
ani_httpd_t *ani_httpd_listen(int port) {
#ifdef _WIN32
    ...
    ani_sock_t fd = socket(AF_INET, SOCK_STREAM, 0);
    if (fd == ANI_SOCK_BAD)
        return NULL;
    ...
    ani_httpd_t *d = calloc(1, sizeof(*d));
    if (!d) {
        ani_sock_close(fd);
        return NULL;
    }
    ...
    d->recv_deadline_ms = ANI_HTTP_RECV_DEADLINE_MS;
    ani_mutex_init(&d->active_mutex);
    return d;
}

int ani_httpd_port(const ani_httpd_t *d) {
    return d ? d->port : -1;
}

void ani_httpd_set_recv_deadline_ms(ani_httpd_t *d, int ms) {
    if (d && ms > 0) {
        ani_mutex_lock(&d->active_mutex);
        d->recv_deadline_ms = ms;
        ani_mutex_unlock(&d->active_mutex);
    }
}

void ani_httpd_interrupt(ani_httpd_t *d) {
    if (!d)
        return;
    ani_mutex_lock(&d->active_mutex);
    d->interrupted = true;
...
bool ani_httpd_close(ani_httpd_t *d) {
    if (!d)
        return true;
    ani_mutex_lock(&d->active_mutex);
    d->interrupted = true;
    ...
    ani_mutex_unlock(&d->active_mutex);
    ani_sock_close(d->fd);
    ani_mutex_destroy(&d->active_mutex);
    free(d);
    return true;
}

void ani_httpd_set_send_deadline_for_test(ani_httpd_t *d, int ms) {
...
void ani_httpd_set_send_buffer_for_test(ani_httpd_t *d, int bytes) {
    if (!d)
        return;
    ani_mutex_lock(&d->active_mutex);
...
ani_httpd_activity_t ani_httpd_activity_for_test(ani_httpd_t *d) {
    if (!d)
        return ANI_HTTPD_ACTIVITY_IDLE;
    ani_mutex_lock(&d->active_mutex);
    ani_httpd_activity_t activity = ANI_HTTPD_ACTIVITY_IDLE;
    if (d->active) {
        activity = atomic_load_explicit(&d->active->response_started, memory_order_acquire)
                       ? ANI_HTTPD_ACTIVITY_RESPONDING
                       : ANI_HTTPD_ACTIVITY_READING_REQUEST;
    }
    ani_mutex_unlock(&d->active_mutex);
    return activity;
}
...
ani_http_conn_t *ani_httpd_accept(ani_httpd_t *d, int timeout_ms) {
    if (!d)
        return NULL;
    ani_mutex_lock(&d->active_mutex);
    ...
    ani_sock_t cfd = accept(d->fd, NULL, NULL);
    if (cfd == ANI_SOCK_BAD)
        return NULL;
    ...
    ani_http_conn_t *c = calloc(1, sizeof(*c));
    if (!c) {
        ani_sock_close(cfd);
        return NULL;
    }
...
void ani_httpd_conn_close(ani_http_conn_t *c) {
    if (!c)
        return;
    ani_httpd_t *owner = c->owner;
    if (owner) {
        ani_mutex_lock(&owner->active_mutex);
        if (owner->active == c)
            owner->active = NULL;
...
int ani_http_conn_status(const ani_http_conn_t *c) {
    return c ? c->response_status : 0;
}

size_t ani_http_conn_response_bytes(const ani_http_conn_t *c) {
...
int ani_http_parse_head(const char *data, size_t len, ani_http_req_t *req, size_t *body_offset,
    ...
    *content_length = 0;
    ...
    size_t scan = len < ANI_HTTP_MAX_HEAD ? len : ANI_HTTP_MAX_HEAD;
    ...
    if (head_end == 0)
        return len >= ANI_HTTP_MAX_HEAD ? 431 : ANI_HTTP_NEED_MORE;
    ...
    while (p < head_stop) {
        ...
        if (header_name_is(p, nlen, "origin")) {
            ...
            for (const char *v = val; *v; v++) {
                ...
                if (cl > (uint64_t)ANI_HTTP_MAX_BODY)
                    return 413;
            }
...
int ani_httpd_read_request(ani_http_conn_t *c, ani_http_req_t *req) {
    if (!c || !req)
        return -1;

    char *head = malloc(ANI_HTTP_MAX_HEAD);
    if (!head)
        return -1;
    ...
    for (;;) {
...
#ifdef _WIN32
        int n = recv(c->fd, head + have, (int)(ANI_HTTP_MAX_HEAD - have), 0);
#else
        ssize_t n = recv(c->fd, head + have, ANI_HTTP_MAX_HEAD - have, 0);
#endif
        if (n < 0 && socket_would_block())
            continue;
        if (n <= 0) {
        ...
        rc = ani_http_parse_head(head, have, req, &body_off, &clen);
        if (rc == 0)
            break;
        if (rc != ANI_HTTP_NEED_MORE) {
            free(head);
            return rc;
        }
        if (have >= ANI_HTTP_MAX_HEAD) {
            free(head);
            return 431;
        }
    }
...
void ani_http_req_free(ani_http_req_t *req) {
    if (req && req->body) {
        free(req->body);
        req->body = NULL;
        req->body_len = 0;
...
void ani_http_reply_buf(ani_http_conn_t *c, int status, const char *extra_headers, const void *data,
                        size_t len) {
    if (!c)
        return;
...
void ani_http_replyf(ani_http_conn_t *c, int status, const char *extra_headers, const char *fmt,
                     ...) {
    va_list ap;
    va_start(ap, fmt);
    va_list ap2;
    ...
    ani_http_reply_buf(c, status, extra_headers, body, (size_t)need);
    if (body != stack_buf)
        free(body);
}
...
bool ani_http_path_match(const char *str, const char *pattern) {
    if (!str || !pattern)
        return false;
...
bool ani_http_query_param(const char *query, const char *name, char *buf, int bufsz) {
