 ...
 * read-only ani_mcp_server_t instance.
 *
 * Runs in a background pthread, same pattern as the watcher thread.
 */
#ifndef ANI_UI_HTTP_SERVER_H
#define ANI_UI_HTTP_SERVER_H
...
typedef struct ani_http_server ani_http_server_t;
struct ani_watcher;
...
typedef int (*ani_http_index_executor_fn)(void *context, const char *root_path,
                                          const char *project_name);
typedef bool (*ani_http_project_mutation_begin_fn)(void *context, const char *project);
typedef void (*ani_http_project_mutation_end_fn)(void *context, const char *project);

/* Create an HTTP server on the given port.
 * Creates its own ani_mcp_server_t with a separate read-only SQLite connection.
 ...
 * Blocks until ani_http_server_stop() is called. */
 ...
 * before its run loop starts. GET /__ani/ui-readiness returns only a
