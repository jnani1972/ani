 ...
 */
#ifndef ANI_INDEX_SUPERVISOR_H
#define ANI_INDEX_SUPERVISOR_H

#include <stdbool.h>
#include <stddef.h>
#include <stdatomic.h>

#include "foundation/subprocess.h" /* ani_proc_outcome_t */
...
#define ANI_INDEX_WORKER_SINGLE_THREAD_ARG "--index-worker-single-thread"
#define ANI_INDEX_WORKER_MARKER_ARG "--index-worker-marker"
#define ANI_INDEX_WORKER_QUARANTINE_ARG "--index-worker-quarantine"
#define ANI_INDEX_WORKER_MEMORY_BUDGET_ARG "--index-worker-memory-budget-bytes"
#define ANI_INDEX_WORKER_BUILD_ARG "--index-worker-build"
#define ANI_INDEX_WORKER_BUILD_FINGERPRINT_LENGTH 64U
#define ANI_INDEX_WORKER_BUILD_FINGERPRINT_SIZE 65U
...
#define ANI_INDEX_WORKER_LOG_START_EVENT "index.worker.start"
 ...
 *   1. the stream becomes crash-durable (see ani_log_set_crash_durable), so a
...
typedef struct {
    ...
    size_t memory_budget_bytes;
} ani_index_worker_invocation_t;

typedef enum {
    ANI_INDEX_WORKER_ARGV_NOT_WORKER = 0,
    ANI_INDEX_WORKER_ARGV_VALID,
    ANI_INDEX_WORKER_ARGV_INVALID,
    ANI_INDEX_WORKER_ARGV_BUILD_UNAVAILABLE,
    ANI_INDEX_WORKER_ARGV_BUILD_MISMATCH,
} ani_index_worker_argv_status_t;
 ...
 * ani binary calls this (first thing in main(), before any
 * subcommand dispatch, so MCP server + CLI + HTTP paths are all covered).
 * EMBEDDERS of ani_mcp_handle_tool (test binaries, future library users) never
 ...
 * this process called ani_index_supervisor_mark_host() (i.e. it IS the real
 ...
 * entry to ani_index_worker_start (including calls made by the synchronous
...
typedef struct {
    ani_proc_outcome_t outcome; /* how the worker ended */
...
} ani_index_worker_result_t;

/* Daemon-owned, nonblocking supervisor for one contained worker process tree. */
typedef struct ani_index_worker_handle ani_index_worker_handle_t;

typedef enum {
    ANI_INDEX_WORKER_POLL_ERROR = -1,
    ANI_INDEX_WORKER_POLL_RUNNING = 0,
    ANI_INDEX_WORKER_POLL_TERMINAL = 1,
} ani_index_worker_poll_t;
 ...
 * and applied after exec by ani_index_set_worker_role_options(). */
 ...
 *   - single_thread   → ANI_INDEX_SINGLE_THREAD=1
 *   - marker_file     → ANI_INDEX_MARKER_FILE
 *   - quarantine_file → ANI_INDEX_QUARANTINE_FILE
