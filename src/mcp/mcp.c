...
/* Write through the descriptor ani_mkstemp returned rather than reopening its
...
static const tool_def_t TOOLS[] = {
    {"index_repository", "Index repository",
     ...
     "deliberately NOT indexed (gitignore/.aniignore/skip-lists) — by design, not failures.",
     ...
     "\"Write compressed artifact to .ani/graph.db.zst for team sharing. "
    ...
    {"index_status", "Index status",
     ...
     "BY DESIGN (gitignore/.aniignore/skip-lists): deliberate and deterministic, not failures; "
...
static bool mcp_tool_allowed(ani_mcp_tool_profile_t profile, const char *name) {
...
static const char *mcp_tool_profile_name(ani_mcp_tool_profile_t profile) {
    return profile == ANI_MCP_TOOL_PROFILE_SCOUT ? "scout" : "analysis";
}
...
int ani_mcp_parse_tool_profile_args(int argc, const char *const argv[const],
    ...
    *profile_out = ANI_MCP_TOOL_PROFILE_ALL;
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(arg, "--tool-profile=analysis") == 0) {
            *profile_out = ANI_MCP_TOOL_PROFILE_ANALYSIS;
            continue;
        }
        if (strcmp(arg, "--tool-profile=scout") == 0) {
            *profile_out = ANI_MCP_TOOL_PROFILE_SCOUT;
            continue;
        }
...
bool ani_mcp_tool_profile_allows_http(ani_mcp_tool_profile_t profile) {
    return profile == ANI_MCP_TOOL_PROFILE_ALL;
}

static int mcp_allowed_tool_count(ani_mcp_tool_profile_t profile) {
    int count = 0;
...
static char *ani_mcp_tools_list_range(ani_mcp_tool_profile_t profile, int offset, int limit,
...
char *ani_mcp_tools_list(void) {
    return ani_mcp_tools_list_range(ANI_MCP_TOOL_PROFILE_ALL, 0, TOOL_COUNT, false);
}
...
static char *ani_mcp_tools_list_page(ani_mcp_tool_profile_t profile, const char *params_json) {
    bool has_cursor = false;
    int offset = mcp_tools_cursor_offset(params_json, &has_cursor);
    if (!has_cursor) {
        return ani_mcp_tools_list_range(profile, 0, TOOL_COUNT, false);
    }
    return ani_mcp_tools_list_range(profile, offset, MCP_TOOLS_PAGE_SIZE, true);
}
...
static char *ani_mcp_prompts_list(void) {
    yyjson_mut_doc *doc = yyjson_mut_doc_new(NULL);
    yyjson_mut_val *root = yyjson_mut_obj(doc);
    yyjson_mut_doc_set_root(doc, root);
...
static char *ani_mcp_prompt_get(const char *params_json, char **error_json) {
    *error_json = NULL;
...
static char *ani_mcp_initialize_response_for_profile(const char *params_json,
                                                     ani_mcp_tool_profile_t profile) {
    ...
    yyjson_mut_obj_add_str(doc, impl, "name", "ani");
    yyjson_mut_obj_add_str(doc, impl, "version", ani_cli_get_version());
...
char *ani_mcp_initialize_response(const char *params_json) {
    return ani_mcp_initialize_response_for_profile(params_json, ANI_MCP_TOOL_PROFILE_ALL);
}
...
static char *canonicalize_repo_path_if_exists(char *repo_path) {
    ...
    if (ani_canonical_path(repo_path, real, sizeof(real))) {
        ani_normalize_path_sep(real);
...
static char *normalize_project_arg(char *project) {
    ...
    char *normalized = ani_project_name_from_path(project);
    if (normalized) {
        free(project);
        return normalized;
    }
...
bool ani_validate_project_name(const char *project);

/* #1025: agents naturally pass the repo FOLDER name ("ani"),
...
static char *resolve_project_tail(char *project) {
    if (!project || !ani_validate_project_name(project)) {
        return project;
    }
    char dir[ANI_SZ_1K];
    cache_dir(dir, sizeof(dir));
    char exact[ANI_SZ_2K];
    snprintf(exact, sizeof(exact), "%s/%s.db", dir, project);
    if (ani_file_exists(exact)) {
        return project; /* exact name — untouched fast path */
    }
    size_t plen = strlen(project);
    char match[ANI_SZ_1K] = "";
    int matches = 0;
    ani_dir_t *d = ani_opendir(dir);
    if (!d) {
        return project;
    }
    ani_dirent_t *entry;
    while ((entry = ani_readdir(d)) != NULL) {
    ...
    ani_closedir(d);
    if (matches == 1) {
        ani_log_info("mcp.project_tail_resolved", "passed", project, "resolved", match);
        free(project);
...
static char *get_project_arg(const char *args_json) {
    char *p = ani_mcp_get_string_arg(args_json, "project");
    if (!p) {
        p = ani_mcp_get_string_arg(args_json, "project_name");
    }
    if (!p) {
        p = ani_mcp_get_string_arg(args_json, "project_id");
    }
    if (!p) {
        p = ani_mcp_get_string_arg(args_json, "projectName");
    }
...
struct ani_mcp_server {
    ani_store_t *store;     /* currently open project store (or NULL) */
    ...
    char readonly_corrupt_project[ANI_SZ_1K];

    /* Session + auto-index state */
    char session_root[ANI_SZ_1K];     /* detected project root path */
    char session_project[ANI_SZ_256]; /* derived project name */
    ...
    struct ani_watcher *watcher;      /* external watcher ref (not owned) */
    struct ani_config *config;        /* external config ref (not owned) */
    ani_mcp_index_executor_fn index_executor;
    void *index_executor_context;
    ani_proc_log_cb index_log_callback;
    void *index_log_context;
    ani_mcp_project_mutation_begin_fn mutation_begin;
    ani_mcp_project_mutation_try_begin_fn mutation_try_begin;
    ani_mcp_project_mutation_end_fn mutation_end;
    void *mutation_context;
    ani_mcp_quarantine_test_hook_fn quarantine_test_hook;
    void *quarantine_test_context;
    ani_mcp_command_test_hook_fn command_test_hook;
    void *command_test_context;
#ifdef ANI_ENABLE_TEST_SEAMS
    ani_mcp_auto_index_count_test_hook_fn auto_index_count_test_hook;
    ...
    ani_mutex_t request_scope_mutex;
    unsigned int request_scope_depth;
    atomic_int pipeline_cancel_requested;
    ani_pipeline_t *active_pipeline; /* non-NULL while index_repository runs */
...
ani_mcp_server_t *ani_mcp_server_new(const char *store_path) {
    ani_mcp_server_t *srv = calloc(ANI_ALLOC_ONE, sizeof(*srv));
    if (!srv) {
        return NULL;
    }
    ani_mutex_init(&srv->request_scope_mutex);
    ...
    if (store_path) {
        srv->store = ani_store_open(store_path);
        srv->current_project = heap_strdup(store_path);
    } else {
        srv->store = ani_store_open_memory();
    }
    srv->owns_store = true;
    srv->tool_profile = ANI_MCP_TOOL_PROFILE_ALL;
...
#ifdef ANI_ENABLE_TEST_SEAMS
void ani_mcp_server_set_auto_index_count_test_hook(ani_mcp_server_t *srv,
                                                   ani_mcp_auto_index_count_test_hook_fn hook,
                                                   void *context) {
    if (srv) {
...
bool ani_mcp_server_set_session_context(ani_mcp_server_t *srv, const char *session_root,
    ...
    char *project = ani_project_name_from_path(session_root);
...
void ani_mcp_server_set_project_mutation_try_guard(
    ani_mcp_server_t *srv, ani_mcp_project_mutation_try_begin_fn try_begin) {
    if (srv && srv->mutation_begin) {
        srv->mutation_try_begin = try_begin;
    }
}

static bool mcp_project_mutation_begin(ani_mcp_server_t *srv, const char *project) {
    return !srv->mutation_begin || srv->mutation_begin(srv->mutation_context, project);
}

static bool mcp_project_mutation_try_begin(ani_mcp_server_t *srv, const char *project) {
    return !srv->mutation_begin ||
           (srv->mutation_try_begin && srv->mutation_try_begin(srv->mutation_context, project));
}

static void mcp_project_mutation_end(ani_mcp_server_t *srv, const char *project) {
    if (srv->mutation_end) {
...
void ani_mcp_server_free(ani_mcp_server_t *srv) {
    ...
    if (srv->autoindex_active) {
        ani_thread_join(&srv->autoindex_tid);
    }
    if (srv->owns_store && srv->store) {
        ani_store_close(srv->store);
    }
    free(srv->current_project);
    free(srv->allowed_root);
    free(srv->active_request_id_str);
    ani_mutex_destroy(&srv->request_scope_mutex);
    free(srv);
}
...
bool ani_mcp_server_release_pristine_memory_store(ani_mcp_server_t *srv) {
    const char *db_path = srv && srv->store ? ani_store_db_path(srv->store) : NULL;
...
ani_pipeline_t *ani_mcp_server_active_pipeline(ani_mcp_server_t *srv) {
...
static bool mcp_request_cancelled(const ani_mcp_server_t *srv) {
    return srv && atomic_load_explicit(&srv->pipeline_cancel_requested, memory_order_acquire) != 0;
}

void ani_mcp_server_set_quarantine_test_hook(ani_mcp_server_t *srv,
                                             ani_mcp_quarantine_test_hook_fn hook, void *context) {
    if (!srv) {
        return;
    }
...
void ani_mcp_server_set_command_test_hook(ani_mcp_server_t *srv, ani_mcp_command_test_hook_fn hook,
                                          void *context) {
    if (!srv) {
        return;
    }
...
void ani_mcp_server_set_search_output_limit_for_test(ani_mcp_server_t *srv, size_t limit) {
    if (srv) {
        srv->search_output_limit_override = limit;
    }
}

void ani_mcp_server_set_search_scan_command_for_test(ani_mcp_server_t *srv, const char *command) {
    if (srv) {
        srv->search_scan_command_override = command;
    }
}

void ani_mcp_server_set_search_scan_timeout_for_test(ani_mcp_server_t *srv, uint64_t timeout_ms,
                                                     bool override_set) {
    if (srv) {
...
static const char *cache_dir(char *buf, size_t bufsz) {
    const char *dir = ani_resolve_cache_dir();
    if (!dir) {
        dir = ani_tmpdir();
    }
...
static const char *project_db_path(const char *project, char *buf, size_t bufsz) {
    if (!ani_validate_project_name(project)) {
        buf[0] = '\0';
        return buf;
    }
    char dir[ANI_SZ_1K];
 ...
 * handle is transferred to the caller (who must ani_store_close it). On failure
...
static ani_store_t *resolve_store_fallback_scan(const char *project);
...
static bool reserve_unique_corrupt_pending(const char *path, char *pending, size_t pending_size,
    ...
    for (unsigned int attempt = 0; attempt < 128; attempt++) {
        uint64_t token = ani_now_ns() ^ ((uint64_t)(unsigned int)getpid() << 32) ^
        ...
        if (ani_file_exists(backup)) {
            continue;
        }
#ifdef _WIN32
        wchar_t *wide = ani_path_to_wide(pending);
...
static void discard_corrupt_pending(const char *pending) {
    ...
    (void)ani_remove_db_sidecars(pending);
    (void)ani_unlink(pending);
}

#ifndef _WIN32
static bool sync_parent_directory(const char *path) {
    char directory[ANI_SZ_2K];
...
#ifdef _WIN32
    wchar_t *wide_pending = ani_path_to_wide(pending);
    wchar_t *wide_backup = ani_path_to_wide(backup);
...
static bool quarantine_step_allowed(ani_mcp_server_t *srv, const char *step) {
...
static bool quarantine_corrupt_store(ani_mcp_server_t *srv, const char *project, const char *path,
                                     char *backup_out, size_t backup_out_size) {
    char backup[ANI_SZ_2K];
    char pending[ANI_SZ_2K];
    ...
    if (!path || !path[0]) {
        ani_log_error("store.auto_clean_failed", "project", project, "path", "", "reason",
                      "empty store path");
        return false;
    }
    if (!reserve_unique_corrupt_pending(path, pending, sizeof(pending), backup, sizeof(backup))) {
        ani_log_error("store.auto_clean_failed", "project", project, "path", path, "reason",
                      "cannot reserve unique backup");
        return false;
    }

    if (ani_store_backup_path(path, pending) != ANI_STORE_OK ||
        ani_store_prepare_path_for_replace(pending) != ANI_STORE_OK) {
    ...
    ani_store_t *snapshot = ani_store_open_path_query(pending);
    if (!snapshot) {
        discard_corrupt_pending(pending);
        ani_log_error("store.auto_clean_failed", "project", project, "path", path, "reason",
    ...
    if (ani_unlink(path) != 0 && errno != ENOENT) {
        ani_log_error("store.auto_clean_failed", "project", project, "path", path, "reason",
                      "backup complete; live database removal failed", "backup", backup);
        return false;
    }
    if (ani_remove_db_sidecars(path) != 0) {
        ani_log_error("store.auto_clean_sidecars", "project", project, "path", path, "reason",
...
static ani_store_t *resolve_store_internal(ani_mcp_server_t *srv, const char *project,
    ...
    srv->store = path[0] ? ani_store_open_path_query(path) : NULL;
    if (srv->store) {
        ...
        if (!allow_autorecovery && !ani_store_check_integrity(srv->store)) {
            ani_store_close(srv->store);
            srv->store = NULL;
            srv->store = ani_store_open_path_query(path);
            ani_integrity_verdict_t verdict = srv->store
                                                  ? ani_store_check_integrity_verdict(srv->store)
                                                  : ANI_INTEGRITY_TRANSIENT;
            if (srv->store) {
                ani_store_close(srv->store);
                srv->store = NULL;
            }
            if (verdict == ANI_INTEGRITY_CORRUPT) {
                srv->readonly_resolve_hit_corrupt = true;
                snprintf(srv->readonly_corrupt_project, sizeof(srv->readonly_corrupt_project), "%s",
                         project);
                ani_log_warn("store.corrupt_readonly", "project", project, "path", path, "action",
        ...
        if (!ani_store_check_integrity(srv->store)) {
            ani_store_close(srv->store);
            srv->store = NULL;
            ...
            ani_integrity_verdict_t verdict = srv->store
                                                  ? ani_store_check_integrity_verdict(srv->store)
                                                  : ANI_INTEGRITY_TRANSIENT;
            bool current_valid = (verdict == ANI_INTEGRITY_OK);
            if (verdict == ANI_INTEGRITY_TRANSIENT) {
            ...
            if (!current_valid) {
                ani_store_close(srv->store);
                srv->store = NULL;
                char backup[ANI_SZ_2K] = {0};
                bool quarantined =
                    quarantine_corrupt_store(srv, project, path, backup, sizeof(backup));
                ani_log_error("store.auto_clean", "project", project, "path", path, "action",
        ...
        ani_project_t proj_verify = {0};
        if (ani_store_get_project(srv->store, project, &proj_verify) == ANI_STORE_OK) {
            ani_project_free_fields(&proj_verify);
            srv->owns_store = true;
            free(srv->current_project);
    ...
    ani_store_t *scanned = resolve_store_fallback_scan(project);
    if (scanned) {
        srv->store = scanned;
        srv->owns_store = true;
        free(srv->current_project);
...
static ani_store_t *resolve_store(ani_mcp_server_t *srv, const char *project) {
...
static void free_node_contents(ani_node_t *n);
...
static int collect_db_project_names(const char *dir_path, char *out, size_t out_sz) {
    int count = 0;
    int offset = 0;
    ani_dir_t *d = ani_opendir(dir_path);
    if (!d) {
        return 0;
    }
    ani_dirent_t *entry;
...
static void add_git_context_json(yyjson_mut_doc *doc, yyjson_mut_val *obj, const char *root_path) {
    ...
    (void)ani_git_context_resolve(root_path, &ctx);
...
static char *build_project_list_error(const char *reason) {
    char dir_path[ANI_SZ_1K];
    cache_dir(dir_path, sizeof(dir_path));

    char projects[ANI_SZ_4K] = "";
...
static char *build_no_store_error_checked(ani_mcp_server_t *srv, const char *project) {
...
#define REQUIRE_STORE(store, project)                                \
    do {                                                             \
        if (!(store)) {                                              \
            ...
            char *_res = ani_mcp_text_result(_err, true);            \
            free(_err);                                              \
            free(project);                                           \
            return _res;                                             \
        }                                                            \
...
static bool project_has_adr(ani_store_t *store, const char *project, const char *root_path) {
    if (store && project) {
        ani_adr_t adr;
        memset(&adr, 0, sizeof(adr));
        if (ani_store_adr_get(store, project, &adr) == ANI_STORE_OK) {
    ...
    char adr_path[ANI_SZ_4K];
    snprintf(adr_path, sizeof(adr_path), "%s/.ani/adr.md", root_path);
    struct stat adr_st;
 ...
 * "tmp-" (ani_project_name_from_path: "/tmp/bench/..." → "tmp-bench-...";
...
static bool db_internal_project_name(const char *full_path, char *name_out, size_t name_sz,
    ...
    ani_store_t *st = ani_store_open_path_query(full_path);
    if (!st) {
        return false; /* nonexistent / unreadable */
    }
    ani_project_t *projs = NULL;
    int n = 0;
    bool ok = false;
    if (ani_store_list_projects(st, &projs, &n) == ANI_STORE_OK) {
    ...
    ani_store_free_projects(projs, n);
    if (ok && out_store) {
...
static void build_project_json_entry(yyjson_mut_doc *doc, yyjson_mut_val *arr, const char *dir_path,
    ...
    char project_name[ANI_SZ_1K];
    ani_store_t *pstore = NULL;
    ...
    int edges = 0;
    if (include_details) {
        nodes = ani_store_count_nodes(pstore, project_name);
        edges = ani_store_count_edges(pstore, project_name);
    }
    char root_path_buf[ANI_SZ_1K] = "";
    ani_project_t proj = {0};
    if (ani_store_get_project(pstore, project_name, &proj) == ANI_STORE_OK) {
        if (proj.root_path) {
            snprintf(root_path_buf, sizeof(root_path_buf), "%s", proj.root_path);
        }
        ani_project_free_fields(&proj);
    }
    ani_store_close(pstore);
    ...
    if (include_details && root_path_buf[0]) {
        ani_git_context_t gctx = {0};
        (void)ani_git_context_resolve(root_path_buf, &gctx);
        if (gctx.is_git && gctx.branch) {
            yyjson_mut_obj_add_strcpy(doc, p, "branch", gctx.branch);
        }
        ani_git_context_free(&gctx);
    }
    if (include_details) {
...
static char *handle_list_projects(ani_mcp_server_t *srv, const char *args) {
    (void)srv;
    int offset = ani_mcp_get_int_arg(args, "offset", 0);
    int limit = ani_mcp_get_int_arg(args, "limit", 50);
    bool include_details = ani_mcp_get_bool_arg(args, "include_details");
    ...
    if (!d) {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 dir_path);
        yyjson_mut_doc_free(doc);
        return ani_mcp_text_result(msg, true);
    }
    ...
    for (size_t i = start; i < end; i++) {
        ...
        char full_path[ANI_SZ_2K];
        snprintf(full_path, sizeof(full_path), "%s/%s", dir_path, name);
        int64_t size_bytes = ani_file_size(full_path);
        if (size_bytes < 0) {
            continue;
        }
    ...
    char *result = ani_mcp_text_result(json, false);
    free(json);
    return result;
}
 ...
 * resolve_store uses ani_store_open_path_query (no SQLITE_OPEN_CREATE), so
...
static char *verify_project_indexed(ani_store_t *store, const char *project) {
    ani_project_t proj_check = {0};
    if (ani_store_get_project(store, project, &proj_check) != ANI_STORE_OK) {
        char *err = build_project_list_error("project not indexed — run index_repository first");
        char *res = ani_mcp_text_result(err, true);
        free(err);
        return res;
    }
    ani_project_free_fields(&proj_check);
    return NULL;
}
...
static ani_store_t *compare_open_project_store(const char *project) {
    char path[ANI_SZ_1K];
    project_db_path(project, path, sizeof(path));
    ani_store_t *store = path[0] ? ani_store_open_path_query(path) : NULL;
    if (store) {
        ani_project_t row = {0};
        if (ani_store_get_project(store, project, &row) == ANI_STORE_OK) {
            ani_project_free_fields(&row);
            return store;
        }
...
static char *compare_graphs_error(const char *code, const char *message) {
    ...
    if (!doc || !root) {
        yyjson_mut_doc_free(doc);
        return ani_mcp_text_result("compare_graphs failed: out of memory", true);
    }
    yyjson_mut_doc_set_root(doc, root);
    ...
    char *result = ani_mcp_text_result(json, true);
    free(json);
    return result;
}
...
static yyjson_mut_val *compare_project_json(yyjson_mut_doc *doc, const char *project,
                                            const ani_graph_compare_project_t *metadata) {
    yyjson_mut_val *object = yyjson_mut_obj(doc);
...
static char *handle_compare_graphs(ani_mcp_server_t *server, const char *args) {
    ...
    ani_store_t *base_store = compare_open_project_store(base_project);
    if (!base_store) {
        free(base_project);
        free(target_project);
        return compare_graphs_error("project_not_indexed", "base project is not indexed");
    }
    ani_store_t *target_store = compare_open_project_store(target_project);
    if (!target_store) {
    ...
    ani_graph_compare_result_t comparison = {0};
    int compare_rc = ani_store_compare_graphs(
    ...
    if (compare_rc != ANI_STORE_OK) {
        yyjson_mut_doc_free(doc);
        free(base_project);
        free(target_project);
        if (compare_rc == ANI_STORE_CANCELLED) {
            return compare_graphs_error("cancelled", "compare_graphs cancelled for this request");
        }
        if (compare_rc == ANI_STORE_NOT_FOUND) {
            return compare_graphs_error("project_not_indexed", "project is not indexed");
        }
        if (compare_rc == ANI_STORE_SCAN_LIMIT) {
            return compare_graphs_error("scan_limit_exceeded",
                                        "combined graph rows exceed scan_limit");
        }
        if (compare_rc == ANI_STORE_CALLBACK_ERR) {
...
static char *handle_get_graph_schema(ani_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    ani_store_t *store = resolve_store(srv, project);
    ...
    ani_schema_info_t schema = {0};
    ani_store_get_schema(store, project, &schema);
    ...
    /* Check ADR presence */
    ani_project_t proj_info = {0};
    if (ani_store_get_project(store, project, &proj_info) == 0 && proj_info.root_path) {
        ...
        ani_project_free_fields(&proj_info);
    }

    char *json = yy_doc_to_str(doc);
    yyjson_mut_doc_free(doc);
    ani_store_schema_free(&schema);
    free(project);
...
static void enrich_add_bfs(yyjson_mut_doc *doc, yyjson_mut_val *arr, ani_traverse_result_t *tr) {
...
static yyjson_mut_val *enrich_connected(yyjson_mut_doc *doc, ani_store_t *store, int64_t node_id,
    ...
    ani_traverse_result_t tr_in = {0};
    ani_store_bfs(store, node_id, "inbound", et, SKIP_ONE, SKIP_ONE, MCP_DEFAULT_LIMIT, &tr_in);
    enrich_add_bfs(doc, conn, &tr_in);
    ani_store_traverse_free(&tr_in);

    ani_traverse_result_t tr_out = {0};
    ani_store_bfs(store, node_id, "outbound", et, SKIP_ONE, SKIP_ONE, MCP_DEFAULT_LIMIT, &tr_out);
    enrich_add_bfs(doc, conn, &tr_out);
    ani_store_traverse_free(&tr_out);
...
static void enrich_connected_joined(ani_store_t *store, int64_t node_id, const char *relationship,
    ...
    for (int d = 0; d < 2; d++) {
        ani_traverse_result_t tr = {0};
        ani_store_bfs(store, node_id, dirs[d], et, SKIP_ONE, SKIP_ONE, MCP_DEFAULT_LIMIT, &tr);
...
static char *bm25_file_pattern_like(const char *file_pattern) {
    ...
    char *like = ani_glob_to_like(file_pattern);
...
static char *bm25_search(ani_store_t *store, const char *project, const char *query,
    ...
    if (toon) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            ani_tree_row_begin(&rows);
            ani_tree_cell_str(&rows, (const char *)sqlite3_column_text(stmt, BM25_COL_QN), true);
            ani_tree_cell_str(&rows, (const char *)sqlite3_column_text(stmt, BM25_COL_LABEL),
                              false);
            ani_tree_cell_str(&rows, (const char *)sqlite3_column_text(stmt, BM25_COL_FILE), false);
            ani_tree_cell_str(&rows, lines, false);
            ani_tree_cell_real(&rows, sqlite3_column_double(stmt, BM25_COL_RANK), false);
            ani_tree_row_end(&rows);
            emitted++;
        }
        ...
        ani_sb_t sb;
        ani_sb_init(&sb);
        ani_tree_scalar_int(&sb, "total", total);
        ani_tree_scalar_str(&sb, "search_mode", "bm25");
        static const char *const cols[] = {"qn", "label", "file", "lines", "rank"};
        ani_tree_table_header(&sb, "results", emitted, cols, 5);
        char *rows_text = ani_sb_finish(&rows);
        ani_sb_append(&sb, rows_text ? rows_text : "");
        free(rows_text);
        ani_tree_scalar_bool(&sb, "has_more", total > offset + emitted);
        return ani_sb_finish(&sb);
    }
...
static void emit_semantic_results(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                  ani_vector_result_t *vresults, int vcount) {
 ...
 * *out_vcount (caller frees via ani_store_free_vector_results when vcount>0).
 * Returns true if semantic_query was provided as a non-array (type error —
 * caller should surface to the user). */
static bool run_semantic_query_core(const char *args, ani_store_t *store, const char *project,
                                    int limit, ani_vector_result_t **out_vresults, int *out_vcount,
                                    bool *out_present) {
    ...
    *out_vcount = 0;
    ...
    if (sq_val && !yyjson_is_arr(sq_val)) {
        ...
        ani_vector_result_t *vresults = NULL;
        int vcount = 0;
        int sem_limit = limit > 0 ? limit : ANI_SZ_16;
        if (ani_store_vector_search(store, project, keywords, ki, sem_limit, &vresults, &vcount) ==
                ANI_STORE_OK &&
            vcount > 0) {
...
static void sg_toon_property_cell(ani_sb_t *sb, yyjson_val *v) {
    if (v && yyjson_is_str(v)) {
        ani_tree_cell_str(sb, yyjson_get_str(v), false);
    } else if (v && yyjson_is_bool(v)) {
        ani_tree_cell_bool(sb, yyjson_get_bool(v), false);
    } else if (v && yyjson_is_int(v)) {
        ani_tree_cell_int(sb, yyjson_get_int(v), false);
    } else if (v && yyjson_is_real(v)) {
        ani_tree_cell_real(sb, yyjson_get_real(v), false);
    } else if (v && !yyjson_is_null(v)) {
        char *json = yyjson_val_write(v, 0, NULL);
        ani_tree_cell_str(sb, json ? json : "", false);
        free(json);
    } else {
        ani_tree_cell_str(sb, "", false);
    }
}

/* Append one row's extra-field cells, pulled from the node's properties. */
static void sg_toon_extra_cells(ani_sb_t *sb, const char *props_json, const char *const *fields,
                                int nfields) {
    yyjson_doc *pd =
...
static void emit_search_results_toon(ani_sb_t *sb, const ani_search_output_t *out, int offset,
                                     const char *const *fields, int nfields, bool detail_ids) {
    ani_tree_scalar_int(sb, "total", out->total);
    if (detail_ids) {
        ...
        ani_tree_table_header(sb, "results", out->count, id_cols, 1);
        for (int i = 0; i < out->count; i++) {
            ani_tree_row_begin(sb);
            ani_tree_cell_str(sb, out->results[i].node.qualified_name, true);
            ani_tree_row_end(sb);
        }
        ani_tree_scalar_bool(sb, "has_more", out->total > offset + out->count);
        return;
    }
    ...
    ani_tree_table_header(sb, "results", out->count, cols, ncols);
    for (int i = 0; i < out->count; i++) {
        const ani_search_result_t *sr = &out->results[i];
        char lines[ANI_SZ_32];
        sg_lines_str(lines, sizeof(lines), sr->node.start_line, sr->node.end_line);
        ani_tree_row_begin(sb);
        ani_tree_cell_str(sb, sr->node.qualified_name, true);
        ani_tree_cell_str(sb, sr->node.label, false);
        ani_tree_cell_str(sb, sr->node.file_path, false);
        ani_tree_cell_str(sb, lines, false);
        ani_tree_cell_int(sb, sr->in_degree, false);
        ani_tree_cell_int(sb, sr->out_degree, false);
...
static int sg_cmp_by_qn(const void *pa, const void *pb) {
    const ani_search_result_t *a = (const ani_search_result_t *)pa;
    const ani_search_result_t *b = (const ani_search_result_t *)pb;
    const char *qa = a->node.qualified_name ? a->node.qualified_name : "";
    const char *qb = b->node.qualified_name ? b->node.qualified_name : "";
    return strcmp(qa, qb);
}

static void emit_search_results_tree(ani_sb_t *sb, ani_search_output_t *out, int offset,
                                     const char *const *fields, int nfields, ani_store_t *store,
                                     const char *relationship, bool include_connected) {
    char buf[ANI_SZ_512];
    char extra_cols[ANI_SZ_256] = "";
    for (int f = 0; f < nfields; f++) {
    ...
    ani_sb_append(sb, buf);
    /* Sort by qn so same-prefix rows are adjacent (module clustering). */
    if (out->count > 1) {
        qsort(out->results, (size_t)out->count, sizeof(ani_search_result_t), sg_cmp_by_qn);
    }
    char cur_group[ANI_SZ_1K] = "";
    for (int i = 0; i < out->count; i++) {
        const ani_search_result_t *sr = &out->results[i];
...
static void emit_search_results_tree_json(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                          ani_search_output_t *out, int offset,
                                          const char *const *fields, int nfields,
                                          ani_store_t *store, const char *relationship,
...
static void emit_semantic_results_toon(ani_sb_t *sb, const ani_vector_result_t *vresults,
                                       int vcount) {
    static const char *const cols[] = {"qn", "label", "file", "score"};
    ani_tree_table_header(sb, "semantic", vcount, cols, 4);
    for (int v = 0; v < vcount; v++) {
        ani_tree_row_begin(sb);
        ani_tree_cell_str(sb, vresults[v].qualified_name, true);
        ani_tree_cell_str(sb, vresults[v].label, false);
        ani_tree_cell_str(sb, vresults[v].file_path, false);
        ani_tree_cell_real(sb, vresults[v].score, false);
        ani_tree_row_end(sb);
    }
}

static char *handle_search_graph(ani_mcp_server_t *srv, const char *args) {
    ...
    ani_mem_phase_mark("handler.args");
    char *project = get_project_arg(args);
    ani_mem_phase_mark("handler.resolve_store");
    ani_store_t *store = resolve_store(srv, project);
    ani_mem_phase_mark("handler.body");
    REQUIRE_STORE(store, project);
    ...
    char *format_arg = ani_mcp_get_string_arg(args, "format");
    ...
    char *query = ani_mcp_get_string_arg(args, "query");
    if (query && query[0]) {
        int q_limit = ani_mcp_get_int_arg(args, "limit", BM25_DEFAULT_LIMIT);
        int q_offset = ani_mcp_get_int_arg(args, "offset", 0);
        char *q_file_pattern = ani_mcp_get_string_arg(args, "file_pattern");
        char *bm25_json =
        ...
        if (bm25_json) {
            free(query);
            free(project);
            char *result = ani_mcp_text_result(bm25_json, false);
            free(bm25_json);
            return result;
        }
    }
    ...
    char *label = ani_mcp_get_string_arg(args, "label");
    char *name_pattern = ani_mcp_get_string_arg(args, "name_pattern");
    char *qn_pattern = ani_mcp_get_string_arg(args, "qn_pattern");
    char *file_pattern = ani_mcp_get_string_arg(args, "file_pattern");
    char *relationship = ani_mcp_get_string_arg(args, "relationship");
    bool exclude_entry_points = ani_mcp_get_bool_arg(args, "exclude_entry_points");
    bool include_connected = ani_mcp_get_bool_arg(args, "include_connected");
    int limit = ani_mcp_get_int_arg(args, "limit", ANI_DEFAULT_SEARCH_LIMIT);
    int offset = ani_mcp_get_int_arg(args, "offset", 0);
    int min_degree = ani_mcp_get_int_arg(args, "min_degree", ANI_NOT_FOUND);
    int max_degree = ani_mcp_get_int_arg(args, "max_degree", ANI_NOT_FOUND);
    ...
    if (relationship && !validate_edge_type(relationship)) {
        ...
        free(file_pattern);
        free(relationship);
        return ani_mcp_text_result("relationship must be uppercase letters and underscores", true);
    }

    ani_search_params_t params = {
        .project = project,
        .label = label,
    ...
    bool has_structural_filters = label || name_pattern || qn_pattern || file_pattern ||
                                  ...
                                  min_degree != ANI_NOT_FOUND || max_degree != ANI_NOT_FOUND;

    if (!legacy_json) {
        const char *fields[SG_MAX_EXTRA_FIELDS];
        yyjson_doc *fields_owner = NULL;
        char *sg_detail = ani_mcp_get_string_arg(args, "detail");
        ...
        if (!sq_type_error) {
            ...
            ani_sb_t sb;
            ani_sb_init(&sb);
            ani_search_output_t tout = {0};
            if (!semantic_only) {
                ani_store_search(store, &params, &tout);
            ...
            free(file_pattern);
            free(relationship);
            char *text = ani_sb_finish(&sb);
            char *result = ani_mcp_text_result(text ? text : "out of memory", text == NULL);
            free(text);
            return result;
        }
    ...
    if (!semantic_only) {
        ani_store_search(store, &params, &out);
    }
...
static char *handle_query_graph(ani_mcp_server_t *srv, const char *args) {
    char *query = ani_mcp_get_string_arg(args, "query");
    char *project = get_project_arg(args);
    ani_store_t *store = resolve_store(srv, project);
    int max_rows = ani_mcp_get_int_arg(args, "max_rows", 0);
    ...
    char *graph_arg = ani_mcp_get_string_arg(args, "graph");
    bool missed_graph = graph_arg && strcmp(graph_arg, "missed") == 0;
    free(graph_arg);

    if (!query) {
        free(project);
        return ani_mcp_text_result("query is required", true);
    }
    if (missed_graph && !project) {
        free(query);
        return ani_mcp_text_result("project is required when graph=\"missed\"", true);
    }
    if (!store) {
    ...
    char covproj[ANI_SZ_512];
    const char *cypher_project = project;
    if (missed_graph) {
        ani_store_coverage_shadow_project(covproj, sizeof(covproj), project);
        cypher_project = covproj;
    }

    ani_cypher_result_t result = {0};
    int rc = ani_cypher_execute(store, query, cypher_project, max_rows, &result);

    if (rc < 0) {
        char *err_msg = result.error ? result.error : "query execution failed";
        char *resp = ani_mcp_text_result(err_msg, true);
        ani_cypher_result_free(&result);
        free(query);
        free(project);
        return resp;
    }
    ...
    char *qg_format = ani_mcp_get_string_arg(args, "format");
    ...
    if (!qg_legacy_json) {
        ani_sb_t sb;
        ani_sb_init(&sb);
        ani_tree_table_header(&sb, "rows", result.row_count, (const char *const *)result.columns,
                              result.col_count);
        for (int r = 0; r < result.row_count; r++) {
            ani_tree_row_begin(&sb);
            for (int c = 0; c < result.col_count; c++) {
                ani_tree_cell_str(&sb, result.rows[r][c], c == 0);
            }
            ani_tree_row_end(&sb);
        }
        ani_tree_scalar_int(&sb, "total", result.row_count);
        if (result.warning) {
            ani_tree_scalar_str(&sb, "warning", result.warning);
        }
    ...
    char *res = ani_mcp_text_result(json ? json : "out of memory", json == NULL);
    free(json);
    return res;
}
...
static void add_coverage_report(yyjson_mut_doc *doc, yyjson_mut_val *root, ani_store_t *store,
                                const char *project) {
    ani_coverage_row_t *rows = NULL;
    int count = 0;
    (void)ani_store_coverage_get(store, project, &rows, &count);
    ...
    ani_store_free_coverage(rows, count);
    ...
    if (ni_dir_n > 0 || ni_file_n > 0) {
        yyjson_mut_obj_add_str(doc, ni, "note",
                               ...
                               "gitignore/.aniignore/skip-lists (see each file's reason). Not an "
...
bool ani_path_within_root(const char *root_path, const char *abs_path); /* defined below */
...
#ifdef __APPLE__
    return ((int64_t)st->st_mtimespec.tv_sec * (int64_t)ANI_NSEC_PER_SEC) +
           (int64_t)st->st_mtimespec.tv_nsec;
#elif defined(_WIN32)
    return (int64_t)st->st_mtime * (int64_t)ANI_NSEC_PER_SEC;
#else
    return ((int64_t)st->st_mtim.tv_sec * (int64_t)ANI_NSEC_PER_SEC) + (int64_t)st->st_mtim.tv_nsec;
#endif
}

static const char *coverage_path_freshness(ani_store_t *store, const char *project,
    ...
    *outside = false;
    ...
    if (!ani_path_within_root(root_path, abs_path)) {
        *outside = true;
        return "outside_project";
    }

    ani_file_hash_t hash = {0};
    int rc = ani_store_get_file_hash(store, project, rel_path, &hash);
    if (rc == ANI_STORE_NOT_FOUND) {
        return "not_tracked";
    }
    if (rc != ANI_STORE_OK) {
        return "unavailable";
    ...
    ani_store_clear_file_hash(&hash);
...
static void coverage_add_row_json(yyjson_mut_doc *doc, yyjson_mut_val *array,
                                  const ani_coverage_row_t *row, const char *requested_path) {
    yyjson_mut_val *item = yyjson_mut_obj(doc);
...
static const char *coverage_status(const ani_coverage_row_t *rows, int count,
...
static char *handle_check_index_coverage(ani_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    ani_store_t *store = resolve_store(srv, project);
    REQUIRE_STORE(store, project);
    ...
    ani_project_t proj = {0};
    bool have_project = ani_store_get_project(store, project, &proj) == ANI_STORE_OK;
    ani_coverage_meta_t meta = {0};
    bool have_meta = ani_store_coverage_meta_get(store, project, &meta) == ANI_STORE_OK;
    ...
    if (paths) {
        yyjson_arr_foreach(paths, idx, max, value) {
            ...
            ani_coverage_row_t *rows = NULL;
            int row_count = 0;
            int cov_rc = ani_store_coverage_get_path(store, project, rel, &rows, &row_count);
            bool lookup_ok = cov_rc == ANI_STORE_OK || cov_rc == ANI_STORE_NOT_FOUND;
            if (!lookup_ok) {
                row_count = 0;
            ...
            ani_store_free_coverage(rows, row_count);
            yyjson_mut_arr_add_val(path_results, item);
        }
    }
    yyjson_mut_obj_add_val(doc, root, "paths", path_results);

    int scope_limit = ani_mcp_get_int_arg(args, "scope_limit", COVERAGE_SCOPE_DEFAULT_LIMIT);
    int scope_offset = ani_mcp_get_int_arg(args, "scope_offset", 0);
    if (scope_limit < 1) {
        scope_limit = 1;
    ...
    if (scopes) {
        yyjson_arr_foreach(scopes, idx, max, value) {
            ...
            ani_coverage_row_t *rows = NULL;
            int row_count = 0;
            int cov_rc = ani_store_coverage_get_scope(store, project, scope, &rows, &row_count);
            bool lookup_ok = cov_rc == ANI_STORE_OK || cov_rc == ANI_STORE_NOT_FOUND;
            if (!lookup_ok) {
                row_count = 0;
                yyjson_mut_obj_add_str(doc, item, "coverage_lookup", "error");
...
static char *handle_index_status(ani_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    ani_store_t *store = resolve_store(srv, project);
    REQUIRE_STORE(store, project);
     ...
     * stays lean. */
    bool verbose = ani_mcp_get_bool_arg(args, "verbose");
    ...
    if (project) {
        int nodes = ani_store_count_nodes(store, project);
        int edges = ani_store_count_edges(store, project);
        yyjson_mut_obj_add_str(doc, root, "project", project);
        yyjson_mut_obj_add_int(doc, root, "nodes", nodes);
        yyjson_mut_obj_add_int(doc, root, "edges", edges);
        yyjson_mut_obj_add_str(doc, root, "status", nodes > 0 ? "ready" : "empty");
        ani_project_t proj_info = {0};
        if (ani_store_get_project(store, project, &proj_info) == ANI_STORE_OK) {
...
static char *handle_delete_project(ani_mcp_server_t *srv, const char *args) {
    char *name = get_project_arg(args);
    if (!name) {
        return ani_mcp_text_result("project is required", true);
    }
    if (!mcp_project_mutation_begin(srv, name)) {
        free(name);
        return ani_mcp_text_result("project operation cancelled or blocked by an active index",
                                   true);
    }
    ...
    if (srv->watcher) {
        ani_watcher_unwatch(srv->watcher, name);
    }

    ani_mem_collect(); /* return freed pages to OS after closing database */
    ...
    char *result = ani_mcp_text_result(json, is_error);
    free(json);
    return result;
}
 ...
 * ani_store_arch_aspect_in_overview in store.c), or the aspect name itself. */
...
static bool aspect_wanted(yyjson_doc *aspects_doc, yyjson_val *aspects_arr, const char *name) {
    ...
    while ((val = yyjson_arr_iter_next(&iter)) != NULL) {
        ...
        if (strcmp(s, "overview") == 0 && ani_store_arch_aspect_in_overview(name)) {
            return true;
        }
        if (strcmp(s, name) == 0) {
            return true;
...
static void append_cross_repo_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                      const ani_schema_info_t *schema) {
 ...
 * ANI_STORE_OK, or ANI_STORE_ERR on failure (all outs zeroed). */
enum { ARCH_SCC_MAX_EDGES = 400000, ARCH_SCC_MAX_CYCLES = 100, ARCH_SCC_MEMBERS_SHOWN = 20 };

static int arch_compute_cycles(ani_store_t *store, const char *project, int64_t ***out_members,
    ...
    *edges_truncated = false;
    ...
    int64_t *tgt = NULL;
    int ecount = 0;
    if (ani_store_fetch_call_edges(store, project, ARCH_SCC_MAX_EDGES, &src, &tgt, &ecount,
                                   edges_truncated) != ANI_STORE_OK) {
        return ANI_STORE_ERR;
    }
    *scanned_edges = ecount;
    ...
    if (!scc_build(src, tgt, ecount, &g)) {
        free(src);
        free(tgt);
        return ANI_STORE_OK; /* no edges = no cycles, not an error */
    }
    free(src);
    free(tgt);
...
static void arch_node_qn(ani_store_t *store, int64_t id, char *out, size_t outsz) {
    ani_node_t n = {0};
    if (ani_store_find_node_by_id(store, id, &n) == ANI_STORE_OK && n.qualified_name) {
...
static char *handle_get_architecture(ani_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    ani_store_t *store = resolve_store(srv, project);
    ...
    REQUIRE_STORE(store, project);
    char *scope_path = ani_mcp_get_string_arg(args, "path");
    ...
    /* Build a C string array from aspects for ani_store_get_architecture.
    ...
    for (int i = 0; i < aspects_strs_count; i++) {
        if (!aspect_is_valid(aspects_strs[i])) {
            ...
            snprintf(msg, sizeof(msg), "Unknown aspect '%s'. Valid: %s.", aspects_strs[i],
                     valid_list);
            char *err = ani_mcp_text_result(msg, true);
            free(project);
            free(scope_path);
            if (aspects_doc) {
    ...
    ani_store_get_schema_counts_scoped(store, project, scope_path, &schema);

    ani_architecture_info_t arch = {0};
    ani_store_get_architecture(store, project, scope_path, aspects_strs, aspects_strs_count, &arch);

    int node_count = ani_store_count_nodes_scoped(store, project, scope_path);
    int edge_count = ani_store_count_edges_scoped(store, project, scope_path);
    char norm_path[ANI_SZ_512];
    bool path_scoped = ani_store_normalize_arch_path(scope_path, norm_path, sizeof(norm_path));

    /* Response encoding: tree tables by default; format:"json" emits the
     * same model as structured JSON ({cols, rows} per section). */
    char *arch_format = ani_mcp_get_string_arg(args, "format");
    ...
    if (!arch_legacy_json) {
        ...
        ani_sb_init(&sb);
        if (project) {
            ani_tree_scalar_str(&sb, "project", project);
        }
        if (default_summary) {
            ani_tree_scalar_str(&sb, "aspects_hint",
        ...
        if (path_scoped) {
            ani_tree_scalar_str(&sb, "path", norm_path);
            ani_tree_scalar_int(&sb, "root_total_nodes", ani_store_count_nodes(store, project));
            ani_tree_scalar_int(&sb, "root_total_edges", ani_store_count_edges(store, project));
            ani_tree_scalar_int(&sb, "scoped_total_nodes", node_count);
            ani_tree_scalar_int(&sb, "scoped_total_edges", edge_count);
        }
        ani_tree_scalar_int(&sb, "total_nodes", node_count);
        ani_tree_scalar_int(&sb, "total_edges", edge_count);

        if (aspect_wanted(aspects_doc, aspects_arr, "structure") && schema.node_label_count > 0) {
            static const char *const lcols[] = {"label", "count"};
            ani_tree_table_header(&sb, "node_labels", schema.node_label_count, lcols, 2);
            for (int i = 0; i < schema.node_label_count; i++) {
                ani_tree_row_begin(&sb);
                ani_tree_cell_str(&sb, schema.node_labels[i].label, true);
                ani_tree_cell_int(&sb, schema.node_labels[i].count, false);
                ani_tree_row_end(&sb);
            }
        }
        if (aspect_wanted(aspects_doc, aspects_arr, "dependencies") && schema.edge_type_count > 0) {
            static const char *const tcols[] = {"type", "count"};
            ani_tree_table_header(&sb, "edge_types", schema.edge_type_count, tcols, 2);
            for (int i = 0; i < schema.edge_type_count; i++) {
                ani_tree_row_begin(&sb);
                ani_tree_cell_str(&sb, schema.edge_types[i].type, true);
                ani_tree_cell_int(&sb, schema.edge_types[i].count, false);
                ani_tree_row_end(&sb);
            }
        }
        if (aspect_wanted(aspects_doc, aspects_arr, "routes") && schema.rel_pattern_count > 0) {
            static const char *const pcols[] = {"pattern"};
            ani_tree_table_header(&sb, "relationship_patterns", schema.rel_pattern_count, pcols, 1);
            for (int i = 0; i < schema.rel_pattern_count; i++) {
                ani_tree_row_begin(&sb);
                ani_tree_cell_str(&sb, schema.rel_patterns[i], true);
                ani_tree_row_end(&sb);
            }
        }
        if (arch.language_count > 0) {
            static const char *const gcols[] = {"language", "files"};
            ani_tree_table_header(&sb, "languages", arch.language_count, gcols, 2);
            for (int i = 0; i < arch.language_count; i++) {
                ani_tree_row_begin(&sb);
                ani_tree_cell_str(&sb, arch.languages[i].language, true);
                ani_tree_cell_int(&sb, arch.languages[i].file_count, false);
                ani_tree_row_end(&sb);
            }
        }
        if (arch.package_count > 0) {
            static const char *const kcols[] = {"name", "nodes", "fan_in", "fan_out"};
            ani_tree_table_header(&sb, "packages", arch.package_count, kcols, 4);
            for (int i = 0; i < arch.package_count; i++) {
                ani_tree_row_begin(&sb);
                ani_tree_cell_str(&sb, arch.packages[i].name, true);
                ani_tree_cell_int(&sb, arch.packages[i].node_count, false);
                ani_tree_cell_int(&sb, arch.packages[i].fan_in, false);
                ani_tree_cell_int(&sb, arch.packages[i].fan_out, false);
                ani_tree_row_end(&sb);
            }
        }
        if (arch.entry_point_count > 0) {
            ...
            ani_tree_table_header(&sb, "entry_points", arch.entry_point_count, ecols, 2);
            for (int i = 0; i < arch.entry_point_count; i++) {
                ani_tree_row_begin(&sb);
                ani_tree_cell_str(&sb, arch.entry_points[i].qualified_name, true);
                ani_tree_cell_str(&sb, arch.entry_points[i].file, false);
                ani_tree_row_end(&sb);
            }
        }
        if (arch.route_count > 0) {
            static const char *const rcols[] = {"method", "path", "handler"};
            ani_tree_table_header(&sb, "routes", arch.route_count, rcols, 3);
            for (int i = 0; i < arch.route_count; i++) {
                ani_tree_row_begin(&sb);
                ani_tree_cell_str(&sb, arch.routes[i].method, true);
                ani_tree_cell_str(&sb, arch.routes[i].path, false);
                ani_tree_cell_str(&sb, arch.routes[i].handler, false);
                ani_tree_row_end(&sb);
            }
        }
        if (arch.hotspot_count > 0) {
            static const char *const hcols[] = {"qn", "fan_in"};
            ani_tree_table_header(&sb, "hotspots", arch.hotspot_count, hcols, 2);
            for (int i = 0; i < arch.hotspot_count; i++) {
                ani_tree_row_begin(&sb);
                ani_tree_cell_str(&sb, arch.hotspots[i].qualified_name, true);
                ani_tree_cell_int(&sb, arch.hotspots[i].fan_in, false);
                ani_tree_row_end(&sb);
            }
        }
        if (arch.boundary_count > 0) {
            static const char *const bcols[] = {"from", "to", "calls"};
            ani_tree_table_header(&sb, "boundaries", arch.boundary_count, bcols, 3);
            for (int i = 0; i < arch.boundary_count; i++) {
                ani_tree_row_begin(&sb);
                ani_tree_cell_str(&sb, arch.boundaries[i].from, true);
                ani_tree_cell_str(&sb, arch.boundaries[i].to, false);
                ani_tree_cell_int(&sb, arch.boundaries[i].call_count, false);
                ani_tree_row_end(&sb);
            }
        }
        if (arch.service_count > 0) {
            static const char *const scols[] = {"from", "to", "type", "count"};
            ani_tree_table_header(&sb, "services", arch.service_count, scols, 4);
            for (int i = 0; i < arch.service_count; i++) {
                ani_tree_row_begin(&sb);
                ani_tree_cell_str(&sb, arch.services[i].from, true);
                ani_tree_cell_str(&sb, arch.services[i].to, false);
                ani_tree_cell_str(&sb, arch.services[i].type, false);
                ani_tree_cell_int(&sb, arch.services[i].count, false);
                ani_tree_row_end(&sb);
            }
        }
        if (arch.layer_count > 0) {
            static const char *const ycols[] = {"name", "layer", "reason"};
            ani_tree_table_header(&sb, "layers", arch.layer_count, ycols, 3);
            for (int i = 0; i < arch.layer_count; i++) {
                ani_tree_row_begin(&sb);
                ani_tree_cell_str(&sb, arch.layers[i].name, true);
                ani_tree_cell_str(&sb, arch.layers[i].layer, false);
                ani_tree_cell_str(&sb, arch.layers[i].reason, false);
                ani_tree_row_end(&sb);
            }
        }
        if (arch.cluster_count > 0) {
            ...
            ani_tree_table_header(&sb, "clusters", arch.cluster_count, ccols, 7);
            for (int i = 0; i < arch.cluster_count; i++) {
                const ani_cluster_info_t *c = &arch.clusters[i];
                char joined[ANI_SZ_1K];
                ani_tree_row_begin(&sb);
                ani_tree_cell_int(&sb, c->id, true);
                ani_tree_cell_str(&sb, c->label, false);
                ani_tree_cell_int(&sb, c->members, false);
                ani_tree_cell_real(&sb, c->cohesion, false);
                arch_join_list(joined, sizeof(joined), c->top_nodes, c->top_node_count);
                ani_tree_cell_str(&sb, joined, false);
        ...
        if (arch.file_tree_count > 0) {
            ...
            ani_tree_table_header(&sb, "file_tree", arch.file_tree_count, fcols, 3);
            for (int i = 0; i < arch.file_tree_count; i++) {
                ani_tree_row_begin(&sb);
                ani_tree_cell_str(&sb, arch.file_tree[i].path, true);
                ani_tree_cell_str(&sb, arch.file_tree[i].type, false);
                ani_tree_cell_int(&sb, arch.file_tree[i].children, false);
                ani_tree_row_end(&sb);
            }
        }
        /* Cross-repo edge summary (mirrors append_cross_repo_summary). */
        {
            ...
            if (cross_total > 0) {
                ani_tree_scalar_int(&sb, "cross_repo_links_total", cross_total);
            }
        }
        ...
        if (aspect_explicitly_named(aspects_arr, "cycles")) {
            ...
            if (arch_compute_cycles(store, project, &members, &sizes, &ncyc, &total_cyc, &scanned,
                                    &etrunc) == ANI_STORE_OK) {
                ani_tree_scalar_int(&sb, "call_edges_scanned", scanned);
                ani_tree_scalar_int(&sb, "cycles_total", total_cyc);
                if (etrunc) {
                    ani_tree_scalar_bool(&sb, "cycles_partial", true);
                    ani_tree_scalar_str(&sb, "cycles_hint",
                                        "call graph exceeded the scan budget; cycle list may be "
                                        "incomplete");
                }
                if (total_cyc > ncyc) {
                    char omit[ANI_SZ_128];
                    snprintf(omit, sizeof(omit), "cycles_omitted: %d  (showing the first %d)\n",
                             total_cyc - ncyc, ncyc);
                    ani_sb_append(&sb, omit);
                }
                char hdr[ANI_SZ_128];
                snprintf(hdr, sizeof(hdr),
                         "cycles: %d  (rows: size members; circular CALLS dependencies)\n", ncyc);
                ani_sb_append(&sb, hdr);
        ...
        ani_store_architecture_free(&arch);
        ani_store_schema_free(&schema);
        if (aspects_doc) {
            yyjson_doc_free(aspects_doc);
        }
    ...
    if (path_scoped) {
        ...
        int root_nodes = ani_store_count_nodes(store, project);
        int root_edges = ani_store_count_edges(store, project);
...
static const char *bfs_edge_args_for_hop(ani_traverse_result_t *tr, int64_t hop_node_id,
                                         size_t *out_len) {
...
const char *ani_mcp_edge_strategy_class(const char *strategy) {
...
static bool bfs_edge_evidence_for_hop(ani_traverse_result_t *tr, int64_t hop_node_id,
    ...
    for (int e = 0; e < tr->edge_count; e++) {
        ...
        memcpy(raw, open, len);
        raw[len] = '\0';
        const char *cls = ani_mcp_edge_strategy_class(raw);
        if (!cls) {
            continue;
        }
        *class_out = cls;
...
static void bfs_to_toon_table(ani_sb_t *sb, const char *key, ani_traverse_result_t *tr,
    ...
    ani_tree_table_header(sb, key, visible, cols, ncols);
    for (int i = 0; i < tr->visited_count; i++) {
        ...
        ani_tree_row_begin(sb);
        ani_tree_cell_str(sb, tr->visited[i].node.qualified_name, true);
        ani_tree_cell_int(sb, tr->visited[i].hop, false);
        if (risk_labels) {
            ani_tree_cell_str(sb, ani_risk_label(ani_hop_to_risk(tr->visited[i].hop)), false);
        }
        if (include_tests) {
            ani_tree_cell_bool(sb, test, false);
        }
        if (include_evidence) {
            const char *ev_class = NULL;
            double ev_conf = -1.0;
            if (bfs_edge_evidence_for_hop(tr, tr->visited[i].node.id, &ev_class, &ev_conf)) {
                ani_tree_cell_str(sb, ev_class ? ev_class : "", false);
                if (ev_conf >= 0.0) {
                    ani_tree_cell_real(sb, ev_conf, false);
                } else {
                    ani_tree_cell_str(sb, "-", false);
                }
            } else {
        ...
        if (data_flow) {
            ...
            if (ea && alen > 0 && alen < ANI_SZ_1K) {
                char abuf[ANI_SZ_1K];
                memcpy(abuf, ea, alen);
                abuf[alen] = '\0';
                ani_tree_cell_str(sb, abuf, false);
            } else {
                ani_tree_cell_str(sb, "", false);
            }
        }
...
static char *snippet_suggestions(const char *input, ani_node_t *nodes, int count);
...
static long node_resolution_score(const ani_node_t *n) {
...
static bool node_is_real_callable_def(const ani_node_t *n) {
    if (!n->label) {
        return false;
    }
    if (strcmp(n->label, "Function") != 0 && strcmp(n->label, "Method") != 0) {
...
static int pick_resolved_node(const ani_node_t *nodes, int count, bool *ambiguous) {
    *ambiguous = false;
    if (count <= 1) {
        return 0;
    }
...
static int node_hop_cmp_hop_id(const void *pa, const void *pb) {
    const ani_node_hop_t *a = (const ani_node_hop_t *)pa;
    const ani_node_hop_t *b = (const ani_node_hop_t *)pb;
    if (a->hop != b->hop) {
 ...
 * transfers into *out, freed by ani_store_traverse_free. */
static void bfs_union_same_name(ani_store_t *store, const ani_node_t *nodes, int node_count,
                                const char *direction, const char **edge_types, int edge_type_count,
                                int depth, int limit, ani_traverse_result_t *out) {
    memset(out, 0, sizeof(*out));
    int vcap = 0, ecap = 0;
    for (int k = 0; k < node_count; k++) {
        ani_traverse_result_t tr = {0};
        ani_store_bfs(store, nodes[k].id, direction, edge_types, edge_type_count, depth, limit,
                      &tr);
        for (int i = 0; i < tr.visited_count; i++) {
            bool dup = false;
            ...
            if (out->visited_count >= vcap) {
                vcap = vcap ? vcap * 2 : 8;
                out->visited = safe_realloc(out->visited, vcap * sizeof(ani_node_hop_t));
            }
        ...
        for (int i = 0; i < tr.edge_count; i++) {
            ...
            if (out->edge_count >= ecap) {
                ecap = ecap ? ecap * 2 : 8;
                out->edges = safe_realloc(out->edges, ecap * sizeof(ani_edge_info_t));
            }
            out->edges[out->edge_count++] = tr.edges[i];
            memset(&tr.edges[i], 0, sizeof(tr.edges[i])); /* ownership moved */
        }
        ani_store_traverse_free(&tr); /* frees only the un-moved (root + dup) fields */
    }
    ...
    if (out->visited_count > 1) {
        qsort(out->visited, (size_t)out->visited_count, sizeof(ani_node_hop_t),
              node_hop_cmp_hop_id);
    }
}
...
static int trace_watermark_index(const ani_traverse_result_t *tr, int hop, int64_t node_id) {
    for (int i = 0; i < tr->visited_count; i++) {
...
static yyjson_mut_val *bfs_to_tree_json(yyjson_mut_doc *doc, ani_traverse_result_t *tr,
                                        bool risk_labels, bool include_tests, bool data_flow,
                                        bool include_evidence) {
    ...
    for (int i = 0; i < tr->visited_count; i++) {
        ...
        if (risk_labels) {
            yyjson_mut_arr_add_str(doc, row, ani_risk_label(ani_hop_to_risk(tr->visited[i].hop)));
        }
...
static void bfs_to_tree_table(ani_sb_t *sb, const char *key, ani_traverse_result_t *tr,
    ...
    if (tr->visited_count > 1) {
        qsort(tr->visited, (size_t)tr->visited_count, sizeof(ani_node_hop_t), tree_hop_cmp_qn);
    }
    char cur_group[ANI_SZ_1K] = "";
    for (int i = 0; i < tr->visited_count; i++) {
        if (!include_tests && is_test_file(tr->visited[i].node.file_path)) {
...
/* Clamp a client-supplied traversal depth to the MCP ceiling (ani_mcp_max_depth),
 * WARN-logging when it does so — never a silent truncation (#887). An unclamped
 * `depth` would drive the shared ani_store_bfs to an arbitrary hop count. */
static int clamp_mcp_depth(int depth, const char *tool) {
    int cap = ani_mcp_max_depth();
    if (depth > cap) {
        char req_buf[16];
        char cap_buf[16];
        snprintf(req_buf, sizeof(req_buf), "%d", depth);
        snprintf(cap_buf, sizeof(cap_buf), "%d", cap);
        ani_log_warn("mcp.depth_capped", "tool", tool, "requested", req_buf, "cap", cap_buf);
        return cap;
    }
    return depth;
}

static char *handle_trace_call_path(ani_mcp_server_t *srv, const char *args) {
    char *func_name = ani_mcp_get_string_arg(args, "function_name");
    char *project = get_project_arg(args);
    ani_store_t *store = resolve_store(srv, project);
    char *direction = ani_mcp_get_string_arg(args, "direction");
    char *mode = ani_mcp_get_string_arg(args, "mode");
    char *param_name = ani_mcp_get_string_arg(args, "parameter_name");
    int depth = ani_mcp_get_int_arg(args, "depth", MCP_DEFAULT_DEPTH);
     ...
     * depth clamp, #887). */
    int trace_limit = ani_mcp_get_int_arg(args, "limit", MCP_BFS_LIMIT);
    if (trace_limit < 1) {
        trace_limit = 1;
    }
    ...
    bool risk_labels = ani_mcp_get_bool_arg(args, "risk_labels");
    bool include_tests = ani_mcp_get_bool_arg(args, "include_tests");
    ...
    bool include_evidence = ani_mcp_get_bool_arg(args, "include_evidence");
    ...
    if (!func_name) {
        ...
        free(mode);
        free(param_name);
        return ani_mcp_text_result("function_name is required", true);
    }
    if (!store) {
        char *_err = build_project_list_error("project not found or not indexed");
        char *_res = ani_mcp_text_result(_err, true);
    ...
    char generation[96];
    (void)ani_store_generation(store, generation, sizeof(generation));
    char *cursor_arg = ani_mcp_get_string_arg(args, "cursor");
    ...
    if (cursor_arg && cursor_arg[0]) {
        uint64_t qh =
            trace_params_hash(project, func_name, direction ? direction : "both", mode,
                              ani_mcp_get_int_arg(args, "depth", MCP_DEFAULT_DEPTH), include_tests,
                              ani_mcp_get_int_arg(args, "limit", MCP_BFS_LIMIT));
        ...
        if (cerr) {
            ...
            free(mode);
            free(param_name);
            return ani_mcp_text_result(cerr, true);
        }
        have_cursor = true;
    }
    free(cursor_arg);
    ...
    if (strcmp(direction, "inbound") != 0 && strcmp(direction, "outbound") != 0 &&
        ...
        free(mode);
        free(param_name);
        return ani_mcp_text_result(errbuf, true);
    }
     ...
     * fields cleanly and ani_store_free_nodes will free them. */
    ani_node_t *nodes = NULL;
    int node_count = 0;
    ani_store_find_nodes_by_name(store, project, func_name, &nodes, &node_count);

    if (node_count == 0) {
        ani_node_t qn_node = {0};
        if (ani_store_find_node_by_qn(store, project, func_name, &qn_node) == ANI_STORE_OK) {
            nodes = malloc(sizeof(ani_node_t));
            if (nodes) {
                nodes[0] = qn_node;
                node_count = 1;
            } else {
    ...
    if (node_count == 0) {
        ...
        free(mode);
        free(param_name);
        ani_store_free_nodes(nodes, 0);
        return ani_mcp_text_result(hint, true);
    }
    ...
    if (trace_ambiguous) {
        ...
        free(mode);
        free(param_name);
        ani_store_free_nodes(nodes, node_count);
        return result;
    }

    /* Response encoding: tree tables by default; format:"json" emits the
     * same grouped model as structured JSON. */
    char *trace_format = ani_mcp_get_string_arg(args, "format");
    ...
    if (!trace_legacy_json) {
        ani_sb_t sb;
        ani_sb_init(&sb);
        ani_tree_scalar_str(&sb, "function", func_name);
        ani_tree_scalar_str(&sb, "direction", direction);
        if (mode) {
            ani_tree_scalar_str(&sb, "mode", mode);
        }
        ...
        if (do_outbound) {
            ani_tree_scalar_int(&sb, "callees_total", out_total);
            if (flat_trace) {
        ...
        if (do_inbound) {
            ani_tree_scalar_int(&sb, "callers_total", in_total);
            if (flat_trace) {
        ...
        if (more_rows) {
            ani_tree_scalar_bool(&sb, "truncated", true);
            if (next_tok[0]) {
                ani_tree_scalar_str(&sb, "next", next_tok);
                ani_tree_scalar_str(&sb, "hint",
...
static char *project_root_from_store(ani_store_t *store, const char *project) {
    if (!store || !project) {
        return NULL;
    }
    ani_project_t proj = {0};
    if (ani_store_get_project(store, project, &proj) != ANI_STORE_OK) {
        return NULL;
    }
...
static char *get_project_root(ani_mcp_server_t *srv, const char *project) {
...
static char *handle_cross_repo_mode(ani_mcp_server_t *srv, const char *repo_path,
                                    const char *name_override, const char *args) {
    if (name_override && name_override[0] && !ani_validate_project_name(name_override)) {
        return ani_mcp_text_result("invalid project name", true);
    }
    char *project = name_override && name_override[0] ? heap_strdup(name_override)
                                                      : ani_project_name_from_path(repo_path);
    if (!project) {
        return ani_mcp_text_result("cannot derive project name", true);
    }
    ...
    if (target_count > MCP_MAX_CROSS_REPO_TARGETS) {
        yyjson_doc_free(jdoc);
        free(project);
        return ani_mcp_text_result("too many cross-repo target projects", true);
    }
    ...
    if (!targets || !lease_keys) {
        ...
        yyjson_doc_free(jdoc);
        free(project);
        return ani_mcp_text_result("failed to allocate cross-repo project leases", true);
    }
    size_t idx;
    size_t max;
    yyjson_val *val;
    ...
    yyjson_arr_foreach(tp_arr, idx, max, val) {
        ...
        if (!target || !target[0] || strlen(target) >= ANI_SZ_256 ||
            (strcmp(target, "*") != 0 && !ani_validate_project_name(target))) {
            invalid_target = true;
            break;
        }
        targets[ti++] = target;
    ...
    if (invalid_target || ti != tp_count) {
        ...
        yyjson_doc_free(jdoc);
        free(project);
        return ani_mcp_text_result("target_projects must contain valid project names or '*'", true);
    }
    if (all_projects && tp_count != 1) {
        ...
        yyjson_doc_free(jdoc);
        free(project);
        return ani_mcp_text_result("target_projects wildcard '*' must be the only entry", true);
    }
    if (!all_projects) {
    ...
    if (held_count != lease_count || cancelled) {
        ...
        yyjson_doc_free(jdoc);
        free(project);
        return ani_mcp_text_result("cross-repo operation cancelled or blocked by active indexing",
                                   true);
    }

    ani_cross_repo_result_t result = ani_cross_repo_match_cancellable(
    ...
    yyjson_mut_doc_free(doc);
    free(project);
    char *out = ani_mcp_text_result(json, result.cancelled);
    free(json);
    return out;
}
...
static void try_artifact_bootstrap(const char *project_name, const char *repo_path) {
    char db_buf[ANI_SZ_1K];
    project_db_path(project_name, db_buf, sizeof(db_buf));
    if (ani_file_size(db_buf) < 0 && ani_artifact_exists(repo_path)) {
        ani_log_info("index.artifact_bootstrap", "project", project_name);
         ...
         * and self-healing.
         *
         * ani_artifact_reconcile_hashes deliberately trades part of that away
        ...
        if (ani_artifact_import(repo_path, db_buf) == 0) {
            (void)ani_artifact_reconcile_hashes(repo_path, db_buf, project_name);
        }
    }
}
...
static void add_not_indexed_files_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                          ani_pipeline_t *p) {
    ani_ignored_file_t *ignored = NULL;
    int stored = 0;
    int total = 0;
    ani_pipeline_get_ignored(p, &ignored, &stored, &total);
    ...
    yyjson_mut_obj_add_str(doc, ni, "note",
                           "Excluded by design (gitignore/.aniignore/skip-lists); examples only — "
...
static bool is_parse_partial(const ani_file_error_t *e) {
...
static void add_skipped_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                const ani_file_error_t *errs, int count, const char *logfile) {
    int skips = 0;
    for (int i = 0; i < count; i++) {
...
static void add_parse_partial_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                      const ani_file_error_t *errs, int count) {
    int partials = 0;
    for (int i = 0; i < count; i++) {
        if (is_parse_partial(&errs[i])) {
            partials++;
...
static bool add_persisted_failure_summaries(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                            ani_store_t *store, const char *project,
                                            const char *logfile) {
    ani_coverage_row_t *rows = NULL;
    int row_count = 0;
    if (ani_store_coverage_get(store, project, &rows, &row_count) != ANI_STORE_OK) {
        return false;
    }
 ...
 *   $ANI_INDEX_LOG (override) else <cache_dir>/logs/<project>-<epoch>.log
 * Returns true and fills out_path on success. */
static bool write_skip_logfile(const char *project, const ani_file_error_t *errs, int count,
                               char *out_path, size_t out_sz) {
    if (!errs || count <= 0) {
        return false;
    }
    char path[ANI_SZ_1K];
    const char *override = getenv("ANI_INDEX_LOG");
    if (override && override[0]) {
        snprintf(path, sizeof(path), "%s", override);
    } else {
        const char *cdir = ani_resolve_cache_dir();
        if (!cdir) {
            return false;
        }
        char logdir[ANI_SZ_1K];
        snprintf(logdir, sizeof(logdir), "%s/logs", cdir);
        ani_mkdir_p(logdir, 0755);
        snprintf(path, sizeof(path), "%s/%s-%lld.log", logdir, project ? project : "index",
                 (long long)time(NULL));
    }
    FILE *f = ani_fopen(path, "wb");
    if (!f) {
        ani_log_warn("index.logfile_open_fail", "path", path);
        return false;
    }
    int partials = 0;
    for (int i = 0; i < count; i++) {
    ...
    (void)fprintf(f, "# ani index coverage report\n");
...
static bool build_index_success_response(ani_mcp_server_t *srv, yyjson_mut_doc *doc,
                                         yyjson_mut_val *root, const char *project_name,
                                         const char *repo_path, bool persistence, ani_pipeline_t *p,
                                         char **excluded_dirs, int excluded_count,
                                         const ani_file_error_t *file_errors, int file_error_count,
                                         const char *logfile) {
    ...
    int exp_nodes = -1;
    int exp_edges = -1;
    ani_pipeline_get_committed_counts(p, &exp_nodes, &exp_edges);

    const double ratio = ani_dump_verify_min_ratio();
    const int min_floor = ANI_DUMP_VERIFY_MIN_FLOOR;

    ani_store_t *store = resolve_store(srv, project_name);
    ...
    if (!store) {
        ...
        nodes = ani_store_count_nodes(store, project_name);
        edges = ani_store_count_edges(store, project_name);
        if (nodes < 0) {
            degraded = true;
            nodes = 0;
            edges = edges >= 0 ? edges : 0;
        } else if (ani_dump_verify_is_degraded(exp_nodes, nodes, ratio, min_floor)) {
            (void)ani_store_checkpoint(store);
            int nodes2 = ani_store_count_nodes(store, project_name);
            int edges2 = ani_store_count_edges(store, project_name);
            if (nodes2 >= 0) {
                nodes = nodes2;
            }
            if (edges2 >= 0) {
            ...
            degraded = ani_dump_verify_is_degraded(exp_nodes, nodes, ratio, min_floor);
        }
    }
    ...
    if (degraded) {
        if (!store) {
            ...
            ani_log_warn("dump.verify", "reason", "store_missing", "expected_nodes",
            ...
            ani_log_warn("dump.verify", "expected_nodes", exp_buf, "persisted_nodes", got_buf);
        }
    }
    ...
    bool has_artifact = ani_artifact_exists(repo_path);
    ...
    if (persistence && has_artifact) {
        yyjson_mut_obj_add_str(doc, root, "artifact_hint",
                               "Persistent artifact written to .ani/graph.db.zst. "
...
static char *build_worker_failure_response(const char *args, ani_proc_outcome_t outcome) {
    char *repo_path = ani_mcp_get_string_arg(args, "repo_path");
    yyjson_mut_doc *doc = yyjson_mut_doc_new(NULL);
    yyjson_mut_val *root = yyjson_mut_obj(doc);
    yyjson_mut_doc_set_root(doc, root);
    yyjson_mut_obj_add_str(doc, root, "status", "error");
    yyjson_mut_obj_add_str(doc, root, "outcome", ani_proc_outcome_str(outcome));
    const char *hint = NULL;
    if (outcome == ANI_PROC_SPAWN_FAILED) {
        hint = "Indexing worker could not be started. Supervision is mandatory in a ANI "
               "host, so no in-process fallback was attempted.";
    } else if (outcome == ANI_PROC_HANG) {
        hint = "Indexing worker timed out (a file made no progress). The worker was "
               "terminated and the server survived. Re-run to retry.";
    } else if (outcome == ANI_PROC_CRASH) {
    ...
    } else if (outcome == ANI_PROC_CLEAN) {
...
static char *build_worker_unsafe_terminal_response(const char *args, ani_proc_outcome_t outcome,
    ...
    yyjson_mut_doc_free(doc);
    free(repo_path);
    char *response = ani_mcp_text_result(json, true);
    free(json);
    return response;
}
 ...
 * to invalidate. */
static void invalidate_cached_store(ani_mcp_server_t *srv) {
    if (!srv) {
        return;
    }
    if (srv->owns_store && srv->store) {
 ...
 * per line — see ani_index_mark_start/done) into the crash/hang SUSPECT set:
    ...
    *out_n = 0;
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (line[0] == 'S') {
            ...
            open_paths[open_n++] = ani_strdup(rel);
        } else {
...
ani_mcp_supervised_result_disposition_t ani_mcp_supervised_result_disposition(
    ...
    if (spawn_result != 0 || !worker_result || worker_result->outcome == ANI_PROC_SPAWN_FAILED) {
        return ANI_MCP_SUPERVISED_RESULT_FALLBACK;
    }
    if (worker_result->cancellation_requested || !worker_result->tree_quiesced ||
        worker_result->supervision_failed) {
        return ANI_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL;
    }
    if (worker_result->outcome == ANI_PROC_CLEAN) {
        return worker_result->response ? ANI_MCP_SUPERVISED_RESULT_SUCCESS
                                       : ANI_MCP_SUPERVISED_RESULT_FALLBACK;
    }
    return ANI_MCP_SUPERVISED_RESULT_CONTAINED_FAILURE;
}
 ...
 * A physical ANI host never falls back to its in-process pipeline: an initial
 * start/protocol failure is returned as an explicit error response. */
static char *index_run_supervised(ani_mcp_server_t *srv, const char *args) {
    invalidate_cached_store(srv);

    /* First attempt: normal parallel run. */
    ani_index_worker_result_t wr;
    int rc = ani_index_spawn_worker_with_log_cancel(
    ...
    ani_mcp_supervised_result_disposition_t disposition =
        ani_mcp_supervised_result_disposition(rc, &wr);

    if (disposition == ANI_MCP_SUPERVISED_RESULT_FALLBACK) {
        ani_proc_outcome_t outcome = wr.outcome;
        ani_index_worker_result_free(&wr);
        invalidate_cached_store(srv);
        return build_worker_failure_response(args, outcome);
    }
    if (disposition == ANI_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL) {
        char *failure =
    ...
    if (disposition == ANI_MCP_SUPERVISED_RESULT_SUCCESS) {
     ...
     * time. */
    ani_proc_outcome_t last_outcome = wr.outcome;
    ani_index_worker_result_free(&wr);

    char marker_path[ANI_SZ_1K];
    char quarantine_path[ANI_SZ_1K];
    ...
    FILE *qinit = ani_fopen(quarantine_path, "wb");
    if (qinit) {
        (void)fclose(qinit);
    }

    int cap = 100;
    const char *cap_env = getenv("ANI_INDEX_MAX_RESTARTS");
    ...
    for (int i = 0; i < cap; i++) {
        ani_index_worker_result_t wr2;
        int rc2 = ani_index_spawn_worker_with_log_cancel(
        ...
        ani_mcp_supervised_result_disposition_t recovery_disposition =
            ani_mcp_supervised_result_disposition(rc2, &wr2);
        if (recovery_disposition == ANI_MCP_SUPERVISED_RESULT_FALLBACK) {
            last_outcome = wr2.outcome;
            ani_index_worker_result_free(&wr2);
            break; /* spawn failed mid-recovery — give up */
        }
        if (recovery_disposition == ANI_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL) {
            last_outcome = wr2.outcome;
        ...
        if (recovery_disposition == ANI_MCP_SUPERVISED_RESULT_SUCCESS) {
            resp = wr2.response; /* transfer ownership to caller */
            wr2.response = NULL;
            ani_index_worker_result_free(&wr2);
            break; /* good files indexed; quarantined files reported as crash/hang */
        }
        if (wr2.outcome == ANI_PROC_CRASH || wr2.outcome == ANI_PROC_HANG ||
            wr2.outcome == ANI_PROC_EXIT_NONZERO) {
            last_outcome = wr2.outcome;
            ani_index_worker_result_free(&wr2);
            ...
            const char *phase;
            if (last_outcome == ANI_PROC_HANG) {
                phase = "hang";
            } else if (last_outcome == ANI_PROC_EXIT_NONZERO) {
                phase = "error";
            } else {
                phase = "crash";
            }
            ...
            if (!suspects || sus_n == 0) {
                ...
                ani_log_warn("index.supervisor.unattributable", "action", "give_up");
                break;
            }
            if (prev_suspects) {
                ...
                if (!supervisor_append_quarantine(quarantine_path, pick, phase)) {
                    ani_log_warn("index.supervisor.quarantine_write_fail", "path", pick);
                    supervisor_free_suspects(suspects, sus_n);
                    break;
                }
                quarantined++;
                ...
                ani_log_warn("index.file_quarantined", "path", pick, "outcome", phase, "attempt",
                             attempt_buf);
            }
    ...
    if (!resp && !unsafe_terminal && quarantined > 0) {
        ani_index_worker_result_t wrp;
        int rcp = ani_index_spawn_worker_with_log_cancel(
            args, /*single_thread=*/false, NULL, quarantine_path,
            srv ? srv->index_log_callback : NULL, srv ? srv->index_log_context : NULL,
            srv ? &srv->pipeline_cancel_requested : NULL, &wrp);
        ani_mcp_supervised_result_disposition_t partial_disposition =
            ani_mcp_supervised_result_disposition(rcp, &wrp);
        if (partial_disposition == ANI_MCP_SUPERVISED_RESULT_SUCCESS) {
            resp = wrp.response; /* transfer ownership to caller */
            wrp.response = NULL;
            char qn[MCP_FIELD_SIZE];
            snprintf(qn, sizeof(qn), "%d", quarantined);
            ani_log_error("index.supervisor.partial", "quarantined", qn, "outcome",
                          ani_proc_outcome_str(last_outcome));
        } else if (partial_disposition == ANI_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL) {
            last_outcome = wrp.outcome;
            unsafe_terminal = true;
            terminal_cancelled = wrp.cancellation_requested;
        }
        ani_index_worker_result_free(&wrp);
    }
...
static char *index_run_supervised_path(ani_mcp_server_t *srv, const char *root_path) {
    if (!root_path || !root_path[0]) {
        return NULL;
    }
    yyjson_mut_doc *doc = yyjson_mut_doc_new(NULL);
...
static bool resolve_session_repo_path(ani_mcp_server_t *srv, char **repo_path) {
...
static char *resolved_repo_path_from_project_arg(const char *args) {
    ...
    ani_store_t *store = db_path[0] ? ani_store_open_path_query(db_path) : NULL;
    char *root_path = NULL;
    if (store) {
        ani_project_t proj = {0};
        if (ani_store_get_project(store, project, &proj) == ANI_STORE_OK) {
...
static char *handle_index_repository(ani_mcp_server_t *srv, const char *args) {
    char *repo_path = ani_mcp_get_string_arg(args, "repo_path");
    char *mode_str = ani_mcp_get_string_arg(args, "mode");
    char *name_override = ani_mcp_get_string_arg(args, "name");
    ani_normalize_path_sep(repo_path);
    ...
    if (!repo_path) {
        free(mode_str);
        free(name_override);
        return ani_mcp_text_result("repo_path is required", true);
    }
    ...
    if (!resolve_session_repo_path(srv, &repo_path)) {
        ...
        free(name_override);
        free(repo_path);
        return ani_mcp_text_result("failed to resolve repo_path", true);
    }
     ...
     * back to the process-wide ANI_ALLOWED_ROOT. The decision itself lives in one
    ...
    const char *allowed_root =
        srv->allowed_root_policy_set ? srv->allowed_root : getenv("ANI_ALLOWED_ROOT");
    ...
    char boundary_err[ANI_SZ_1K];
    if (repo_path && repo_path[0] &&
        !ani_workspace_root_allowed(repo_path, ani_workspace_home_dir(), ani_workspace_cache_dir(),
                                    allowed_root, boundary_err, sizeof(boundary_err))) {
        free(mode_str);
        free(name_override);
        free(repo_path);
        return ani_mcp_text_result(boundary_err, true);
    }
    ...
    char *mutation_project =
        ani_project_name_from_path(name_override && name_override[0] ? name_override : repo_path);
    if (!mutation_project) {
        free(repo_path);
        free(mode_str);
        free(name_override);
        return ani_mcp_text_result("could not resolve index project name", true);
    }
    ...
    if (ani_index_supervisor_should_wrap()) {
        char *worker_args = index_args_with_repo_path(args, repo_path);
        if (!worker_args) {
            ...
            free(mode_str);
            free(name_override);
            return ani_mcp_text_result("failed to prepare supervised index request", true);
        }
    ...
    if (!mcp_project_mutation_begin(srv, mutation_project)) {
        ...
        free(mode_str);
        free(name_override);
        return ani_mcp_text_result("index operation blocked by another mutation for this project",
                                   true);
    }
    if (mcp_request_cancelled(srv)) {
        ...
        free(mode_str);
        free(name_override);
        return ani_mcp_text_result("index operation cancelled for this request", true);
    }

    ani_index_mode_t mode = ANI_MODE_FULL;
    if (mode_str && strcmp(mode_str, "fast") == 0) {
        mode = ANI_MODE_FAST;
    } else if (mode_str && strcmp(mode_str, "moderate") == 0) {
        mode = ANI_MODE_MODERATE;
    }
    free(mode_str);

    bool persistence = ani_mcp_get_bool_arg(args, "persistence");

    ani_pipeline_t *p = ani_pipeline_new(repo_path, NULL, mode);
    if (!p) {
        mcp_project_mutation_end(srv, mutation_project);
        free(mutation_project);
        free(name_override);
        free(repo_path);
        return ani_mcp_text_result("failed to create pipeline", true);
    }
    if (name_override && name_override[0] && !ani_pipeline_set_project_name(p, name_override)) {
        ani_pipeline_free(p);
        mcp_project_mutation_end(srv, mutation_project);
        free(mutation_project);
        free(name_override);
    ...
    free(name_override);
    ani_pipeline_set_persistence(p, persistence);

    char *project_name = heap_strdup(ani_pipeline_project_name(p));
    ...
    ani_pipeline_lock();
    ani_pipeline_bind_cancel_flag(p, &srv->pipeline_cancel_requested);
     ...
     * JSON doc, so they need only outlive that call, not ani_pipeline_free. */
    char **excluded_dirs = NULL;
    int excluded_count = 0;
    ani_pipeline_get_excluded(p, &excluded_dirs, &excluded_count);
    ...
    ani_file_error_t *file_errors = NULL;
    int file_error_count = 0;
    ani_pipeline_get_file_errors(p, &file_errors, &file_error_count);

    ani_mem_collect(); /* return mimalloc pages to OS after large indexing */
    ...
    if (rc == 0) {
        ...
        if (ani_pipeline_had_format_migration(p)) {
    ...
    if (ani_index_worker_active()) {
        ani_log_info("index.worker.fast_exit", "skip", "pipeline_free");
    } else {
        ani_pipeline_free(p);
    }
    free(project_name);
    ...
    char *result = ani_mcp_text_result(json, rc != 0);
    free(json);
    return result;
}
...
static void copy_node(const ani_node_t *src, ani_node_t *dst) {
    dst->id = src->id;
 ...
 * (>= ANI_SZ_4K bytes); returns true on success. Isolating the per-OS resolver
 * keeps ani_path_within_root's control flow unconditional: the previous `#ifdef`
...
static bool resolve_canonical_path(const char *path, char *out, size_t out_sz) {
    /* ani_canonical_path: realpath on POSIX; an opened handle plus
    ...
    if (!ani_canonical_path(path, out, out_sz)) {
        return false;
    }
#ifdef _WIN32
...
#ifdef _WIN32
    wchar_t *wide_root = ani_utf8_to_wide(root_path);
    wchar_t *wide_candidate = ani_utf8_to_wide(candidate_path);
...
bool ani_path_within_root(const char *root_path, const char *abs_path) {
    if (!root_path || !abs_path) {
        return false;
    }
    char real_root[ANI_SZ_4K];
    char real_file[ANI_SZ_4K];
...
static void add_snippet_coverage_note(yyjson_mut_doc *doc, yyjson_mut_val *root_obj,
                                      ani_store_t *store, const ani_node_t *node) {
    if (!node->file_path || !node->file_path[0] || !node->project) {
        return;
    }
    ani_coverage_row_t *rows = NULL;
    int count = 0;
    if (ani_store_coverage_get_path(store, node->project, node->file_path, &rows, &count) !=
        ANI_STORE_OK) {
        return;
    }
    for (int i = 0; i < count; i++) {
...
static char *build_snippet_response(ani_mcp_server_t *srv, ani_node_t *node,
                                    const char *match_method, bool include_neighbors,
                                    ani_node_t *alternatives, int alt_count) {
    ...
    ani_store_t *store = srv->store;
    int in_deg = 0;
    int out_deg = 0;
    ani_store_node_degree(store, node->id, &in_deg, &out_deg);
    ...
    if (include_neighbors) {
        ani_store_node_neighbor_names(store, node->id, MCP_DEFAULT_LIMIT, &nb_callers,
...
static bool file_outline_request_cancelled(void *context) {
    return mcp_request_cancelled((const ani_mcp_server_t *)context);
}

static char *handle_get_file_outline(ani_mcp_server_t *srv, const char *args) {
    ...
    if (!root || !yyjson_is_obj(root)) {
        yyjson_doc_free(args_doc);
        return ani_mcp_text_result("get_file_outline arguments must be a JSON object", true);
    }
    ...
    if (!file_path || !file_path[0]) {
        free(project);
        yyjson_doc_free(args_doc);
        return ani_mcp_text_result("file_path is required", true);
    }

    char normalized_path[ANI_SZ_4K];
    if (coverage_normalize_rel(file_path, false, normalized_path, sizeof(normalized_path)) !=
        COVERAGE_PATH_OK) {
        free(project);
        yyjson_doc_free(args_doc);
        return ani_mcp_text_result("file_path must be a repository-relative path without '..'",
                                   true);
    }
    ...
    if (limit_value) {
        if (!yyjson_is_int(limit_value)) {
            free(project);
            yyjson_doc_free(args_doc);
            return ani_mcp_text_result("limit must be an integer", true);
        }
        int64_t parsed = yyjson_get_sint(limit_value);
        if (parsed < 1 || parsed > ANI_STORE_FILE_OUTLINE_MAX_LIMIT) {
            free(project);
            yyjson_doc_free(args_doc);
            return ani_mcp_text_result("limit must be between 1 and 200", true);
        }
        limit = (int)parsed;
    }
    ...
    if (offset_value) {
        if (!yyjson_is_int(offset_value)) {
            free(project);
            yyjson_doc_free(args_doc);
            return ani_mcp_text_result("offset must be a non-negative integer", true);
        }
    ...
    if (format_value) {
        ...
        if (!format || (strcmp(format, "tree") != 0 && strcmp(format, "json") != 0)) {
            free(project);
            yyjson_doc_free(args_doc);
            return ani_mcp_text_result("format must be either 'tree' or 'json'", true);
        }
        json_format = strcmp(format, "json") == 0;
    }

    const char *labels[ANI_STORE_FILE_OUTLINE_MAX_LABELS];
    int label_count = 0;
    yyjson_val *labels_value = yyjson_obj_get(root, "labels");
    if (labels_value) {
        if (!yyjson_is_arr(labels_value) ||
            yyjson_arr_size(labels_value) > ANI_STORE_FILE_OUTLINE_MAX_LABELS) {
            free(project);
            yyjson_doc_free(args_doc);
            return ani_mcp_text_result("labels must be an array of at most 16 strings", true);
        }
        size_t index = 0;
        size_t max = 0;
        yyjson_val *item;
        yyjson_arr_foreach(labels_value, index, max, item) {
            ...
            if (!label || !label[0] || strlen(label) >= ANI_SZ_128) {
                free(project);
                yyjson_doc_free(args_doc);
                return ani_mcp_text_result("labels must contain only non-empty bounded strings",
                                           true);
            }
    ...
    if (!store) {
        ...
        char *result = ani_mcp_text_result(error, true);
        free(error);
        free(project);
        yyjson_doc_free(args_doc);
        return result;
    ...
    ani_file_outline_row_t *rows = NULL;
    int row_count = 0;
    int total = 0;
    int rc = ani_store_get_file_outline(store, project, normalized_path, labels, label_count, limit,
                                        offset, file_outline_request_cancelled, srv, &rows,
                                        &row_count, &total);
    if (rc != ANI_STORE_OK) {
        free(project);
        yyjson_doc_free(args_doc);
        if (rc == ANI_STORE_CANCELLED) {
            return ani_mcp_text_result("get_file_outline cancelled for this request", true);
        }
        if (rc == ANI_STORE_SCAN_LIMIT) {
            return ani_mcp_text_result("get_file_outline exceeded its fixed output safety limit",
                                       true);
        }
        return ani_mcp_text_result("get_file_outline query failed", true);
    }
    ...
    if (!json_format) {
        ani_sb_t sb;
        ani_sb_init(&sb);
        ani_tree_scalar_str(&sb, "file_path", normalized_path);
        static const char *const columns[] = {"name", "label", "lines", "qn"};
        ani_tree_table_header(&sb, "results", row_count, columns, 4);
        for (int i = 0; i < row_count; i++) {
            char lines[ANI_SZ_32];
            ...
            ani_tree_row_begin(&sb);
            ani_tree_cell_str(&sb, rows[i].name, true);
            ani_tree_cell_str(&sb, rows[i].label, false);
            ani_tree_cell_str(&sb, lines, false);
            ani_tree_cell_str(&sb, rows[i].qualified_name, false);
            ani_tree_row_end(&sb);
        }
        ani_tree_scalar_int(&sb, "total", total);
        ani_tree_scalar_int(&sb, "offset", offset);
        ani_tree_scalar_int(&sb, "limit", limit);
        ani_tree_scalar_int(&sb, "returned", row_count);
        ani_tree_scalar_bool(&sb, "has_more", (int64_t)offset + row_count < total);
        payload = ani_sb_finish(&sb);
    } else {
        yyjson_mut_doc *doc = yyjson_mut_doc_new(NULL);
        yyjson_mut_val *object = yyjson_mut_obj(doc);
    ...
    ani_store_free_file_outline(rows, row_count);
    free(project);
    yyjson_doc_free(args_doc);
    if (!payload) {
        return ani_mcp_text_result("get_file_outline output allocation failed", true);
    }
    if (strlen(payload) > MCP_FILE_OUTLINE_OUTPUT_MAX) {
        free(payload);
        return ani_mcp_text_result("get_file_outline exceeded its fixed output safety limit", true);
    }
    char *result = ani_mcp_text_result(payload, false);
    free(payload);
    return result;
}

static char *handle_get_code_snippet(ani_mcp_server_t *srv, const char *args) {
    char *qn = ani_mcp_get_string_arg(args, "qualified_name");
    char *project = get_project_arg(args);
    bool include_neighbors = ani_mcp_get_bool_arg(args, "include_neighbors");

    if (!qn) {
        free(project);
        return ani_mcp_text_result("qualified_name is required", true);
    }
    ...
    ani_node_t node = {0};
    int rc = ani_store_find_node_by_qn(store, effective_project, qn, &node);
    if (rc == ANI_STORE_OK) {
    ...
    ani_node_t *suffix_nodes = NULL;
    int suffix_count = 0;
    ani_store_find_nodes_by_qn_suffix(store, effective_project, qn, &suffix_nodes, &suffix_count);

    if (suffix_count == SKIP_ONE) {
        copy_node(&suffix_nodes[0], &node);
        ani_store_free_nodes(suffix_nodes, suffix_count);
...
typedef struct {
    char file[ANI_SZ_512];
    int line;
    char content[ANI_SZ_1K];
} grep_match_t;

/* Deduped result: one per containing graph node */
typedef struct {
    int64_t node_id; /* 0 = raw match (no containing node) */
    char node_name[ANI_SZ_256];
    char qualified_name[ANI_SZ_512];
    char label[ANI_SZ_64];
    char file[ANI_SZ_512];
    int start_line;
    int end_line;
    int in_degree;
...
bool ani_search_code_file_pattern_can_prefilter(const char *file_pattern) {
...
bool ani_search_code_windows_path_matches_prefilter(const char *path, const char *file_pattern) {
    if (!path || !ani_search_code_file_pattern_can_prefilter(file_pattern)) {
        return false;
    }
 ...
 * defaults.) */
#define ANI_PS_UTF8_PRELUDE "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; "

void ani_search_code_build_grep_cmd(char *cmd, size_t cmd_sz, bool use_regex, bool scoped,
    ...
    if (scoped) {
        if (file_pattern) {
            snprintf(
                cmd, cmd_sz,
                "powershell -Command \"" ANI_PS_UTF8_PRELUDE
...
static int aggregate_search_dirs(search_result_t *sr, int sr_count, char dir_names[][ANI_SZ_128],
    ...
    for (int di = 0; di < sr_count; di++) {
        ...
        if (found >= 0) {
            ...
            snprintf(dir_names[dir_n], ANI_SZ_128, "%s", top);
...
static yyjson_mut_val *build_dir_distribution(yyjson_mut_doc *doc, search_result_t *sr,
    ...
    char dir_names[ANI_SZ_64][ANI_SZ_128];
    int dir_counts[ANI_SZ_64];
    int dir_n = aggregate_search_dirs(sr, sr_count, dir_names, dir_counts, ANI_SZ_64);
    for (int d = 0; d < dir_n; d++) {
...
static char *assemble_search_output_toon(search_result_t *sr, int sr_count, grep_match_t *raw,
    ...
    ani_tree_table_header(&sb, "results", output_count, cols, 7);
    for (int ri = 0; ri < output_count; ri++) {
        ...
        ani_tree_row_begin(&sb);
        ani_tree_cell_str(&sb, r->qualified_name, true);
        ani_tree_cell_str(&sb, r->label, false);
        ani_tree_cell_str(&sb, r->file, false);
        ani_tree_cell_str(&sb, lines, false);
        ani_tree_cell_str(&sb, matches, false);
        ani_tree_cell_int(&sb, r->in_degree, false);
        ani_tree_cell_int(&sb, r->out_degree, false);
        ani_tree_row_end(&sb);
    }
    ...
    if (raw_output > 0) {
        ...
        ani_tree_table_header(&sb, "raw", raw_output, rcols, 3);
        for (int ri = 0; ri < raw_output; ri++) {
            ani_tree_row_begin(&sb);
            ani_tree_cell_str(&sb, raw[ri].file, true);
            ani_tree_cell_int(&sb, raw[ri].line, false);
            ani_tree_cell_str(&sb, raw[ri].content, false);
            ani_tree_row_end(&sb);
        }
    }
    ...
    if (dir_n > 0) {
        ...
        ani_tree_table_header(&sb, "dirs", dir_n, dcols, 2);
        for (int d = 0; d < dir_n; d++) {
            ani_tree_row_begin(&sb);
            ani_tree_cell_str(&sb, dir_names[d], true);
            ani_tree_cell_int(&sb, dir_counts[d], false);
            ani_tree_row_end(&sb);
        }
    }

    ani_tree_scalar_int(&sb, "total_grep_matches", gm_count);
    ani_tree_scalar_int(&sb, "total_results", sr_count);
    ani_tree_scalar_int(&sb, "raw_match_count", raw_count);
    if (metrics->include_phase_timings) {
        ani_tree_scalar_int(&sb, "scope_ms", (long long)metrics->scope_ms);
        ani_tree_scalar_int(&sb, "scan_ms", (long long)metrics->scan_ms);
        ani_tree_scalar_int(&sb, "enrich_ms", (long long)metrics->enrich_ms);
    }
    ani_tree_scalar_int(&sb, "elapsed_ms", (long long)metrics->elapsed_ms);
    if (warn_literal_pipe) {
        ani_tree_scalar_str(&sb, "warning",
    ...
    if (metrics->elapsed_ms >= SEARCH_SLOW_MS) {
        ani_tree_scalar_str(&sb, "warning_slow",
...
static char *assemble_search_output(search_result_t *sr, int sr_count, grep_match_t *raw,
    ...
    if (metrics->elapsed_ms >= SEARCH_SLOW_MS) {
        ...
        ani_log_warn("search.slow", "elapsed_ms", ems); /* visibility in logs */
    }
...
static grep_match_t *collect_grep_matches(FILE *fp, const char *root_path, size_t root_len,
                                          bool has_path_filter, ani_regex_t *path_regex,
                                          int grep_limit, int *out_count) {
    int gm_cap = ANI_SZ_64;
    int gm_count = 0;
...
#ifdef _WIN32
        ani_normalize_path_sep(line);
#endif
        const char *path = line;
        const char *file = strip_root_prefix(path, root_path, root_len);

        if (has_path_filter && ani_regexec(path_regex, file, 0, NULL, 0) != ANI_REG_OK) {
            continue;
        }

        safe_grow(gm, gm_count, gm_cap, PAIR_LEN);
        snprintf(gm[gm_count].file, sizeof(gm[0].file), "%s", file);
        gm[gm_count].line = (int)strtol(sep1 + SKIP_ONE, NULL, ANI_DECIMAL_BASE);
...
static int find_tightest_node(ani_node_t *nodes, int count, int line) {
    int best = ANI_NOT_FOUND;
...
static void add_to_search_results(search_result_t **sr, int *sr_count, int *sr_cap, ani_node_t *n,
                                  int line) {
    for (int j = 0; j < *sr_count; j++) {
        if ((*sr)[j].node_id == n->id) {
            if ((*sr)[j].match_count < ANI_SZ_64) {
...
static void classify_grep_hit(grep_match_t *hit, ani_node_t *file_nodes, int file_node_count,
    ...
    if (best >= 0) {
        ...
        if (*raw_count >= *raw_cap) {
            *raw_cap = (*raw_cap == 0) ? ANI_SZ_32 : *raw_cap * PAIR_LEN;
...
/* Free a file_nodes array returned from ani_store_find_nodes_by_file. */
static void free_file_nodes(ani_node_t *nodes, int count) {
    for (int j = 0; j < count; j++) {
...
static void classify_all_grep_hits(grep_match_t *gm, int gm_count, ani_store_t *store,
    ...
    while (i < gm_count) {
        ...
        if (store) {
            ani_store_find_nodes_by_file(store, project, cur_file, &file_nodes, &file_node_count);
        }
 ...
 *
 * `fl` is the caller's already-open binary stream on the descriptor ani_mkstemp
...
static bool write_scoped_filelist(ani_mcp_server_t *srv, const char *project, const char *root_path,
                                  FILE *fl, const char *file_pattern, bool has_path_filter,
                                  ani_regex_t *path_regex, int *out_written) {
    *out_written = 0;
    ani_store_t *pre_store = resolve_store(srv, project);
    if (!pre_store) {
        return false;
    }
    char **indexed_files = NULL;
    int indexed_count = 0;
    int list_rc = ani_store_list_files(pre_store, project, &indexed_files, &indexed_count);
    if (list_rc != ANI_STORE_OK || indexed_count == 0) {
    ...
    if (fl) {
        ...
        for (int fi = 0; fi < indexed_count; fi++) {
...
#ifdef _WIN32
                ani_normalize_path_sep(indexed_files[fi]);
#endif
                if (ani_regexec(path_regex, indexed_files[fi], 0, NULL, 0) != ANI_REG_OK) {
                    continue;
                }
            }
#ifdef _WIN32
            if (ani_search_code_file_pattern_can_prefilter(file_pattern) &&
                !ani_search_code_windows_path_matches_prefilter(indexed_files[fi], file_pattern)) {
                continue;
            }
#endif
            size_t root_len = strlen(root_path);
            ...
            ani_path_info_t path_info;
            if (ani_path_info_utf8(scan_path, &path_info) != 0 || !path_info.is_regular) {
                free(scan_path);
                continue;
            }
 ...
 * "<tmp>/ani_search_<pid>.pat" and its ".files" companion — opened with a plain
 ...
 * them. Now both live inside a directory created by ani_mkdtemp (0700 on POSIX,
 * an explicit owner-only DACL on Windows) under an unguessable XXXXXX suffix,
 * and each file is created by ani_mkstemp — O_CREAT|O_EXCL at mode 0600, so the
 * create fails rather than following anything already at the name. Every write
 * goes through the descriptor ani_mkstemp returned; neither path is ever
 * reopened by name.
 *
 * Sizing: ani_mkdtemp copies its expanded result back into `dir`, and its own
 * internal buffer is ANI_SZ_512, so `dir` must be at least that big to receive
 * it. The two file paths are `dir` plus a short basename. */
typedef struct {
    char dir[ANI_SZ_512];
    char pattern_path[ANI_SZ_1K];
    char filelist_path[ANI_SZ_1K];
...
static FILE *search_scratch_file(const char *dir, const char *basename, char *path_out,
    ...
    int descriptor = ani_mkstemp(path_out);
    if (descriptor < 0) {
        path_out[0] = '\0';
        return NULL;
    }
...
static void search_scratch_close(search_scratch_t *scratch) {
    ...
    if (scratch->pattern_path[0] != '\0') {
        (void)ani_unlink(scratch->pattern_path);
        scratch->pattern_path[0] = '\0';
    }
    if (scratch->filelist_path[0] != '\0') {
        (void)ani_unlink(scratch->filelist_path);
        scratch->filelist_path[0] = '\0';
    }
    if (scratch->dir[0] != '\0') {
        (void)ani_rmdir(scratch->dir);
        scratch->dir[0] = '\0';
    }
}
...
static bool search_scratch_open(search_scratch_t *scratch, const char *pattern) {
    ...
    int written =
        snprintf(scratch->dir, sizeof(scratch->dir), "%s/ani-search-XXXXXX", ani_tmpdir());
    if (written <= 0 || (size_t)written >= sizeof(scratch->dir) || !ani_mkdtemp(scratch->dir)) {
        scratch->dir[0] = '\0';
        return false;
    }
...
static bool compile_path_filter(const char *filter, ani_regex_t *re) {
    if (!filter || !filter[0]) {
        return false;
    }
    return ani_regcomp(re, filter, ANI_REG_EXTENDED | ANI_REG_NOSUB) == ANI_REG_OK;
}

static mcp_scan_cause_t mcp_run_shell_command_cancellable_bounded(
    ani_mcp_server_t *srv, const char *command, char output_path[ANI_SZ_2K], size_t output_limit,
    uint64_t deadline_ms, bool deadline_enabled, bool deadline_latched, bool exit_one_is_no_match,
    ani_proc_result_t *result_out);
...
static char *search_code_scan_error(search_scratch_t *scratch, const char *output_path,
                                    bool has_path_filter, ani_regex_t *path_regex, char *root_path,
                                    char *pattern, char *project, char *file_pattern,
                                    mcp_scan_cause_t cause, const char *message) {
    if (output_path && output_path[0]) {
        (void)ani_unlink(output_path);
    }
    search_scratch_close(scratch);
    if (has_path_filter) {
        ani_regfree(path_regex);
    }
    free(root_path);
    free(pattern);
    free(project);
    ...
    return ani_mcp_text_result(message, true);
}

static char *handle_search_code(ani_mcp_server_t *srv, const char *args) {
    char *pattern = ani_mcp_get_string_arg(args, "pattern");
    char *project = get_project_arg(args);
    char *file_pattern = ani_mcp_get_string_arg(args, "file_pattern");
    char *path_filter = ani_mcp_get_string_arg(args, "path_filter");
    char *mode_str = ani_mcp_get_string_arg(args, "mode");
    int limit = ani_mcp_get_int_arg(args, "limit", MCP_DEFAULT_LIMIT);
    ...
    int context_lines = ani_mcp_get_int_arg(args, "context", 0);
    bool use_regex = ani_mcp_get_bool_arg(args, "regex");
    uint64_t search_t0 = ani_now_ms();
    search_metrics_t metrics = {0};
    metrics.include_phase_timings = ani_mcp_get_bool_arg(args, "debug");
    ...
    if (!pattern) {
        free(project);
        free(file_pattern);
        return ani_mcp_text_result("pattern is required", true);
    }
    ...
    if (!validate_search_args(root_path, file_pattern)) {
        ...
        free(project);
        free(file_pattern);
        return ani_mcp_text_result("path or file_pattern contains invalid characters", true);
    }
    ...
    if (use_regex) {
        ani_regex_t probe;
        if (ani_regcomp(&probe, pattern, ANI_REG_EXTENDED | ANI_REG_NOSUB) != ANI_REG_OK) {
            if (has_path_filter) {
                ani_regfree(&path_regex);
            }
            free(root_path);
    ...
    uint64_t scan_started_ms = ani_now_ms();
    ...
    if (!search_scratch_open(&scratch, pattern)) {
        ...
        bool scan_timed_out = ani_now_ms() >= scan_deadline_ms;
        char errmsg[ANI_SZ_256];
        ...
        free(file_pattern);
        if (scan_cancelled) {
            return ani_mcp_text_result("search_code cancelled for this request", true);
        }
        if (scan_timed_out) {
            return search_code_timeout_result();
        }
        return ani_mcp_text_result(errmsg, true);
    }
    scan_deadline_latched = ani_now_ms() >= scan_deadline_ms;
    ...
    uint64_t scope_t0 = metrics.include_phase_timings ? ani_now_ms() : 0;
    ...
    scan_deadline_latched = scan_deadline_latched || ani_now_ms() >= scan_deadline_ms;
    if (metrics.include_phase_timings) {
        metrics.scope_ms = ani_now_ms() - scope_t0;
    }

    /* Collect grep matches into array */
    int gm_count = 0;
    grep_match_t *gm = NULL;
    uint64_t scan_t0 = metrics.include_phase_timings ? ani_now_ms() : 0;
    if (scoped && scoped_written == 0 && !scan_cancellation_latched && !scan_deadline_latched) {
        ...
        char cmd[ANI_SZ_4K];
        ani_search_code_build_grep_cmd(cmd, sizeof(cmd), use_regex, scoped, file_pattern, tmpfile,
                                       filelist, root_path);

        char output_path[ANI_SZ_2K] = {0};
        ani_proc_result_t scan_result = {0};
        ...
        FILE *fp = ani_fopen(output_path, "rb");
        if (!fp) {
            return search_code_scan_error(&scratch, output_path, has_path_filter, &path_regex,
    ...
    if (metrics.include_phase_timings) {
        metrics.scan_ms = ani_now_ms() - scan_t0;
    }
    ...
    uint64_t enrich_t0 = metrics.include_phase_timings ? ani_now_ms() : 0;
    ani_store_t *store = resolve_store(srv, project);

    int sr_cap = ANI_SZ_32;
    int sr_count = 0;
    search_result_t *sr = calloc(sr_cap, sizeof(search_result_t));

    int raw_cap = ANI_SZ_32;
    int raw_count = 0;
    ...
    if (store && sr_count > 0) {
        ...
        if (ani_store_batch_count_degrees(store, ids, sr_count, "CALLS", in_degs, out_degs) ==
            ANI_STORE_OK) {
            for (int j = 0; j < sr_count; j++) {
    ...
    if (metrics.include_phase_timings) {
        metrics.enrich_ms = ani_now_ms() - enrich_t0;
    }
    metrics.elapsed_ms = ani_now_ms() - search_t0;
    ...
    char *sc_format = ani_mcp_get_string_arg(args, "format");
    ...
    if (mode == 0 && !sc_legacy_json) {
        ...
        result = ani_mcp_text_result(toon_text ? toon_text : "out of memory", toon_text == NULL);
        free(toon_text);
    } else {
        result =
...
static bool mcp_command_output_path(char out[ANI_SZ_2K]) {
    char directory[ANI_SZ_1K];
    ...
    if (cache && cache[0]) {
        ...
        written = snprintf(directory, sizeof(directory), "%s", ani_tmpdir());
        if (written <= 0 || written >= (int)sizeof(directory)) {
            return false;
        }
    }
    written = snprintf(out, ANI_SZ_2K, "%s/.mcp-command-XXXXXX", directory);
    if (written <= 0 || written >= ANI_SZ_2K) {
        out[0] = '\0';
        return false;
    }
 ...
 * COMSPEC. ani_subprocess receives this as lpApplicationName and validates the
 * same absolute cmd.exe path before using the dedicated payload serializer. */
static bool mcp_resolve_windows_cmd(char out[ANI_SZ_4K]) {
    if (!out) {
        return false;
    }
    out[0] = '\0';
    ...
    char *candidate = ani_wide_to_utf8(system_directory);
    if (!candidate) {
        return false;
    }
    bool resolved = ani_canonical_path(candidate, out, ANI_SZ_4K) != 0;
    free(candidate);
    return resolved;
}
#endif

static mcp_scan_cause_t mcp_scan_pre_spawn_cause(ani_mcp_server_t *srv, const char *output_path,
    ...
    *deadline_latched = *deadline_latched || (deadline_enabled && ani_now_ms() >= deadline_ms);
    if (!*output_limit_latched && output_limit > 0 && output_path && output_path[0]) {
        int64_t output_size = ani_file_size(output_path);
...
static mcp_scan_cause_t mcp_run_shell_command_cancellable_bounded(
    ...
    ani_proc_opts_t options = {
        ...
        .quiet_timeout_ms = 0,
        .cancel_grace_ms = ANI_SUBPROCESS_DEFAULT_CANCEL_GRACE_MS,
    ...
    if (ani_subprocess_spawn(&options, &process) != 0) {
        pre_spawn_cause = mcp_scan_pre_spawn_cause(srv, output_path, output_limit, deadline_ms,
                                                   deadline_enabled, &cancellation_latched,
                                                   &deadline_latched, &output_limit_latched);
        (void)ani_unlink(output_path);
    ...
    for (;;) {
        ...
        state = ani_subprocess_poll(process, result_out);
        cancellation_latched = cancellation_latched || mcp_request_cancelled(srv);
        deadline_latched = deadline_latched || (deadline_enabled && ani_now_ms() >= deadline_ms);
        if (!output_limit_latched && output_limit > 0) {
            int64_t output_size = ani_file_size(output_path);
    ...
    bool contained = state == ANI_PROC_POLL_TERMINAL && result_out->tree_quiesced &&
                     !result_out->supervision_failed;
    ani_subprocess_destroy(process);
    if (!output_limit_latched && output_limit > 0) {
        int64_t final_size = ani_file_size(output_path);
...
#ifndef _WIN32
    if (exit_one_is_no_match && result_out->outcome == ANI_PROC_EXIT_NONZERO &&
        result_out->exit_code == 1) {
        return MCP_SCAN_SUCCESS; /* ordinary direct grep no-match */
    }
#endif
    return result_out->outcome == ANI_PROC_CLEAN ? MCP_SCAN_SUCCESS
                                                 : MCP_SCAN_CONTAINED_COMMAND_FAILURE;
}

static int mcp_run_shell_command_cancellable(ani_mcp_server_t *srv, const char *command,
                                             char output_path[ANI_SZ_2K],
                                             ani_proc_result_t *result_out) {
...
bool ani_detect_node_in_hunks(const ani_node_t *node, const ani_changed_hunk_t *hunks,
...
static void detect_collect_seeds(ani_store_t *store, const char *project, const char *file,
                                 const ani_changed_hunk_t *hunks, int hunk_count, int64_t **seeds,
                                 int *n, int *cap) {
    ani_node_t *nodes = NULL;
    int ncount = 0;
    ani_store_find_nodes_by_file(store, project, file, &nodes, &ncount);
    ...
    if (scope_to_hunks) {
        ...
        for (int i = 0; i < ncount && !any_overlap; i++) {
            any_overlap = detect_is_seedable_label(nodes[i].label) &&
                          ani_detect_node_in_hunks(&nodes[i], hunks, hunk_count, file);
        }
        scope_to_hunks = any_overlap;
    }
    for (int i = 0; i < ncount; i++) {
        if (detect_is_seedable_label(nodes[i].label)) {
            if (scope_to_hunks && !ani_detect_node_in_hunks(&nodes[i], hunks, hunk_count, file)) {
                continue;
            }
            if (*n >= *cap) {
                *cap = *cap ? *cap * 2 : 16;
    ...
    ani_store_free_nodes(nodes, ncount);
}
...
static int detect_module_rollup(const ani_traverse_result_t *impact, char mods[][ANI_SZ_128],
    ...
    for (int i = 0; i < impact->visited_count; i++) {
        ...
        if (j == nmods) {
            if (nmods < DETECT_MODCAP) {
                snprintf(mods[nmods], ANI_SZ_128, "%s", m);
                mcnt[nmods] = 1;
                nmods++;
            } else {
                (*overflow)++;
...
static void detect_emit_impacted_tree(ani_sb_t *sb, ani_traverse_result_t *tr, int limit) {
    ani_tree_scalar_int(sb, "impacted_total", tr->visited_count);
    ...
    ani_sb_append(sb, hdr);
    if (shown > 1) {
        qsort(tr->visited, (size_t)shown, sizeof(ani_node_hop_t), tree_hop_cmp_qn);
    }
    char cur_group[ANI_SZ_1K] = "";
    for (int i = 0; i < shown; i++) {
        const char *qn =
    ...
    if (shown < tr->visited_count) {
        ...
        ani_sb_append(sb, more);
    }
}

static char *handle_detect_changes(ani_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    char *base_branch = ani_mcp_get_string_arg(args, "base_branch");
    char *since = ani_mcp_get_string_arg(args, "since");
    char *scope = ani_mcp_get_string_arg(args, "scope");
    int depth = ani_mcp_get_int_arg(args, "depth", MCP_DEFAULT_BFS_DEPTH);
    ...
    if (!ani_validate_shell_arg(base_branch) || base_branch[0] == '-' ||
        !validate_windows_cmd_interpolation_arg(base_branch)) {
        free(project);
        free(base_branch);
        free(scope);
        return ani_mcp_text_result("base_branch contains invalid characters", true);
    }
    ...
    if (!validate_search_path_arg(root_path) ||
        ...
        free(base_branch);
        free(scope);
        return ani_mcp_text_result("project path contains invalid characters", true);
    }
    ...
    char output_path[ANI_SZ_2K] = {0};
    ani_proc_result_t git_result = {0};
    ...
    if (git_cancelled) {
        ...
        free(base_branch);
        free(scope);
        return ani_mcp_text_result("detect_changes cancelled for this request", true);
    }
    if (git_run != 0) {
        (void)ani_unlink(output_path);
        char errmsg[ANI_SZ_256];
    ...
    if (!fp) {
        ...
        free(base_branch);
        free(scope);
        return ani_mcp_text_result("git diff failed: contained output could not be read", true);
    }
    ...
    char *fmt = ani_mcp_get_string_arg(args, "format");
    ...
    int imp_limit = ani_mcp_get_int_arg(args, "limit", MCP_DEFAULT_IMPACT_LIMIT);
    if (imp_limit < 1) {
        imp_limit = 1;
    }
    ...
    ani_changed_hunk_t *hunks = NULL;
    int hunk_count = 0;
    if (want_symbols) {
        char hunk_cmd[ANI_SZ_2K];
#ifdef _WIN32
        ...
        snprintf(hunk_cmd, sizeof(hunk_cmd),
                 ...
                 root_path, base_branch, root_path);
#endif
        char hunk_output_path[ANI_SZ_2K] = {0};
        ani_proc_result_t hunk_result = {0};
        int hunk_run =
        ...
        FILE *hfp = (!hunk_cancelled && hunk_run == 0) ? ani_fopen(hunk_output_path, "rb") : NULL;
        if (hfp) {
            ...
            if (hsz > 0) {
                ...
                if (hbuf) {
                    ...
                    hbuf[hread] = '\0';
                    enum { HUNK_CAP = 4096 };
                    hunks = safe_realloc(NULL, (size_t)HUNK_CAP * sizeof(ani_changed_hunk_t));
                    hunk_count = ani_parse_hunks(hbuf, hunks, HUNK_CAP);
                    ...
                    if (hunk_count >= HUNK_CAP) {
                        ani_log_info("detect_changes.hunks", "action", "scoping_disabled", "reason",
                                     "hunk_cap_reached");
                        free(hunks);
                        hunks = NULL;
                        hunk_count = 0;
        ...
        if (hunk_output_path[0]) {
            (void)ani_unlink(hunk_output_path);
        }
    }
    ...
    char merge_base[64] = "";
    {
        ...
        snprintf(mbcmd, sizeof(mbcmd), "git -C '%s' merge-base '%s' HEAD 2>/dev/null", root_path,
                 base_branch);
#endif
        char mb_output_path[ANI_SZ_2K] = {0};
        ani_proc_result_t mb_result = {0};
        ...
        FILE *mbfp =
            mb_run == 0 && mb_result.exit_code == 0 ? ani_fopen(mb_output_path, "rb") : NULL;
        ...
        if (mb_output_path[0]) {
            (void)ani_unlink(mb_output_path);
        }
    ...
    ani_traverse_result_t impact = {0};
    bool truncated = false;
    if (want_symbols && seed_count > 0) {
        (void)ani_store_bfs_multi(store, seeds, seed_count, direction, NULL, 0, depth,
    ...
    if (!legacy_json) {
        ani_sb_t sb;
        ani_sb_init(&sb);
        ani_tree_scalar_str(&sb, "base", base_branch);
        if (merge_base[0]) {
            ani_tree_scalar_str(&sb, "merge_base", merge_base);
        }
        ani_tree_scalar_str(&sb, "direction", direction);
        if (is_error) {
            char hint_buf[ANI_SZ_256];
            snprintf(hint_buf, sizeof(hint_buf),
                     "git diff exited with status %d. Check that branch '%s' exists.", git_status,
                     base_branch);
            ani_tree_scalar_str(&sb, "hint", hint_buf);
        }
    ...
    ani_store_traverse_free(&impact);
    for (int i = 0; i < file_count; i++) {
        free(files[i]);
    }
    free(files);
    ...
    char *result = ani_mcp_text_result(out_str, is_error);
    free(out_str);
    return result;
}
...
static void adr_sections_cb(void *ctx, const ani_adr_heading_t *h) {
 ...
 *
 * This uses ani_adr_scan_headings(), the SAME classifier the section-write
...
static void adr_list_sections_from_content(yyjson_mut_doc *doc, yyjson_mut_val *root_obj,
    ...
    if (content && ani_adr_scan_headings(content, adr_sections_cb, &ctx) != ANI_STORE_OK) {
...
/* Read the legacy file-based ADR (<root>/.ani/adr.md), used by
...
static ani_store_t *open_adr_store_for_write(ani_mcp_server_t *srv, ani_store_t *resolved,
                                             ani_store_t **owned_rw) {
    if (!srv || !resolved || !owned_rw) {
        return NULL;
    }
    const char *resolved_db_path = ani_store_db_path(resolved);
    if (!resolved_db_path) {
        return resolved;
    }
    ...
    invalidate_cached_store(srv);
    *owned_rw = ani_store_open_path(rw_path);
    free(rw_path);
    return *owned_rw;
}
...
static char *handle_manage_adr(ani_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    char *mode_str = ani_mcp_get_string_arg(args, "mode");
    char *content = ani_mcp_get_string_arg(args, "content");
    ...
    char section_key_err[ANI_SZ_256] = "";
    if (set_sections_mode) {
        ...
        if (!updates.status && ani_adr_validate_section_keys(
                                   (const char **)updates.keys, updates.count, section_key_err,
                                   (int)sizeof(section_key_err)) != ANI_STORE_OK) {
    ...
    if (write_request && project) {
        ...
        if (mcp_request_cancelled(srv)) {
            ...
            free(mode_str);
            free(content);
            return ani_mcp_text_result("project operation cancelled for this request", true);
        }
    }
    ...
    if (!resolved) {
        ...
        if (recovery_status == STORE_RECOVERY_BUSY) {
            res = ani_mcp_text_result("project is busy; retry after indexing", true);
        } else if (recovery_status == STORE_RECOVERY_TRY_GUARD_UNAVAILABLE) {
            res =
                ani_mcp_text_result("project recovery requires a nonblocking mutation guard", true);
        } else {
            char *err = build_no_store_error_checked(srv, project);
            res = ani_mcp_text_result(err, true);
            free(err);
        }
        if (mutation_held) {
            mcp_project_mutation_end(srv, project);
    ...
    ani_store_t *store = resolved;
    ani_store_t *owned_rw = NULL;
    if (write_request) {
        ...
        if (!store) {
            ...
            free(mode_str);
            free(content);
            return ani_mcp_text_result("failed to open writable ADR store", true);
        }
    }

    /* One-time migration: older versions wrote ADRs to a file at
     * <root>/.ani/adr.md. A read never waits for the project lease:
     * it returns the legacy content immediately and attempts migration only if
     * a nonblocking acquire succeeds. */
    ani_adr_t adr;
    memset(&adr, 0, sizeof(adr));
    bool have_adr = (ani_store_adr_get(store, project, &adr) == ANI_STORE_OK);
    char *legacy = NULL;
        ...
        if (legacy && mcp_project_mutation_try_begin(srv, project)) {
            if (!mcp_request_cancelled(srv)) {
                ...
                if (resolved) {
                    store = open_adr_store_for_write(srv, resolved, &owned_rw);
                    if (store) {
                        have_adr = (ani_store_adr_get(store, project, &adr) == ANI_STORE_OK);
                        if (!have_adr &&
                            ani_store_adr_store(store, project, legacy) == ANI_STORE_OK) {
                            have_adr = (ani_store_adr_get(store, project, &adr) == ANI_STORE_OK);
                        }
                    }
                }
    ...
    if (set_sections_mode) {
        /* ani_store_adr_update_sections requires an existing row — its
        ...
        if (!base_present) {
            ...
            if (ani_store_adr_store(store, project, seed) == ANI_STORE_OK) {
                base_present = true;
        ...
        if (section_rc == ANI_STORE_OK) {
            ...
            if (seeded_empty) {
                (void)ani_store_adr_delete(store, project);
            }
            yyjson_mut_obj_add_str(doc, root_obj, "status", "write_error");
            const char *store_err = ani_store_error(store);
        ...
        if (ani_store_adr_store(store, project, content) == ANI_STORE_OK) {
...
static char *handle_ingest_traces(ani_mcp_server_t *srv, const char *args) {
    (void)srv;
...
static char *dispatch_tool(ani_mcp_server_t *srv, const char *tool_name, const char *args_json) {
    if (!tool_name) {
        return ani_mcp_text_result("missing tool name", true);
    }
    if (srv && !mcp_tool_allowed(srv->tool_profile, tool_name)) {
        char message[ANI_SZ_256];
 ...
 * process lifetime. */
static void release_request_store(ani_mcp_server_t *srv) {
    if (!srv || !srv->owns_store || !srv->store || !ani_store_db_path(srv->store)) {
        return;
    }
    ani_store_close(srv->store);
    srv->store = NULL;
...
char *ani_mcp_handle_tool(ani_mcp_server_t *srv, const char *tool_name, const char *args_json) {
     ...
     * cannot hide between them (ANI_MEM_PHASES=1; see foundation/mem.h). The
    ...
    ani_mem_phase_mark("request.scope_begin");
    bool request_scope = !srv || ani_mcp_server_request_scope_begin(srv);
    if (!request_scope) {
        release_request_store(srv);
        ani_mem_phase_mark("idle");
        return ani_mcp_text_result("request cancellation scope unavailable", true);
    }
    ani_mem_phase_mark("request.dispatch_tool");
    char *result = dispatch_tool(srv, tool_name, args_json);
    ani_mem_phase_mark("request.scope_end");
    if (srv) {
        ani_mcp_server_request_scope_end(srv);
    }
    ani_mem_phase_mark("request.release_store");
    release_request_store(srv);
    ani_mem_phase_mark("idle");
    ...
    ani_mem_census_log("mcp.request");
    return result;
}
...
static void detect_session(ani_mcp_server_t *srv) {
    ...
    if (getcwd(cwd, sizeof(cwd)) != NULL) {
        ...
        if (strcmp(cwd, "/") != 0 && (home == NULL || strcmp(cwd, home) != 0)) {
            ...
            ani_log_info("session.root.cwd", "path", cwd);
        }
    }

    /* Derive project name from path — must match ani_project_name_from_path
    ...
    if (srv->session_root[0]) {
        char *pname = ani_project_name_from_path(srv->session_root);
        if (pname) {
...
static bool auto_watch_enabled(ani_mcp_server_t *srv) {
    if (!srv->config) {
        return true; /* default on */
    }
    return ani_config_get_bool(srv->config, ANI_CONFIG_AUTO_WATCH, true);
}

/* Register the session project with the background watcher for ongoing
 * change detection — unless auto_watch is disabled. */
static void register_watcher_if_enabled(ani_mcp_server_t *srv) {
    if (!srv->watcher || srv->session_project[0] == '\0' || srv->session_root[0] == '\0') {
        return;
    }
    if (!auto_watch_enabled(srv)) {
        ani_log_info("watcher.register.skipped", "reason", "auto_watch_off", "project",
                     srv->session_project);
        return;
    }
    ani_watcher_watch(srv->watcher, srv->session_project, srv->session_root);
}

/* Background auto-index thread function */
static void *autoindex_thread(void *arg) {
    ani_mcp_server_t *srv = (ani_mcp_server_t *)arg;

    ani_log_info("autoindex.start", "project", srv->session_project, "path", srv->session_root);
    ...
    if (ani_index_supervisor_should_wrap()) {
        ...
        if (resp) {
            free(resp);
            ani_log_info("autoindex.done", "project", srv->session_project, "mode", "supervised");
        ...
        ani_log_error("autoindex.supervision_failed", "project", srv->session_project, "action",
                      "fail_closed");
        return NULL;
    }

    ani_pipeline_t *p = ani_pipeline_new(srv->session_root, NULL, ANI_MODE_FULL);
    if (!p) {
        ani_log_warn("autoindex.err", "msg", "pipeline_create_failed");
        return NULL;
    }
    ...
    ani_pipeline_free(p);
    ani_mem_collect(); /* return mimalloc pages to OS after indexing (in-process only) */

    if (rc == 0) {
        ani_log_info("autoindex.done", "project", srv->session_project);
        register_watcher_if_enabled(srv);
    } else {
        ani_log_warn("autoindex.err", "msg", "pipeline_run_failed");
    }
    return NULL;
}

bool ani_mcp_auto_index_within_file_limit(const char *root_path, int file_limit,
                                          int *file_count_out) {
    if (file_count_out) {
        *file_count_out = -1;
    }
    ...
    ani_discover_opts_t options = {
        .mode = ANI_MODE_FULL,
        .ignore_file = NULL,
        .max_file_size = 0,
    };
    int count = -1;
    ani_discover_status_t status = ani_discover_count_bounded(
        root_path, &options, file_limit, ani_now_ms() + AUTO_INDEX_COUNT_TIMEOUT_MS, &count);
    if (file_count_out) {
        *file_count_out = status == ANI_DISCOVER_LIMIT_EXCEEDED
                              ? (file_limit < INT_MAX ? file_limit + 1 : INT_MAX)
                              : count;
    }
    return status == ANI_DISCOVER_OK;
}

/* Start auto-indexing if configured and project not yet indexed. */
static void maybe_auto_index(ani_mcp_server_t *srv) {
    ...
    const char *allowed_root =
        srv->allowed_root_policy_set ? srv->allowed_root : getenv("ANI_ALLOWED_ROOT");
    char boundary_err[ANI_SZ_1K];
    if (!ani_workspace_root_allowed(srv->session_root, ani_workspace_home_dir(),
                                    ani_workspace_cache_dir(), allowed_root, boundary_err,
                                    sizeof(boundary_err))) {
        ani_log_warn("autoindex.skip", "reason", "workspace_boundary", "detail", boundary_err);
        return;
    }
    ...
    const char *home = ani_get_home_dir();
    if (home) {
        char db_check[ANI_SZ_1K];
        snprintf(db_check, sizeof(db_check), "%s/%s.db", ani_resolve_cache_dir(),
                 srv->session_project);
        if (ani_file_size(db_check) >= 0) {
            /* Already indexed → register watcher for change detection */
            ani_log_info("autoindex.skip", "reason", "already_indexed", "project",
                         srv->session_project);
            register_watcher_if_enabled(srv);
            return;
        }
    ...
    int file_limit = ANI_MCP_DEFAULT_AUTO_INDEX_LIMIT;
    if (srv->config) {
        auto_index = ani_config_get_bool(srv->config, ANI_CONFIG_AUTO_INDEX, false);
        file_limit = ani_config_get_int(srv->config, ANI_CONFIG_AUTO_INDEX_LIMIT,
                                        ANI_MCP_DEFAULT_AUTO_INDEX_LIMIT);
    }

    if (!auto_index) {
        ani_log_info("autoindex.skip", "reason", "disabled", "hint",
                     "run: ani config set auto_index true");
        return;
    }
...
#endif
    if (!ani_mcp_auto_index_within_file_limit(srv->session_root, file_limit, &file_count)) {
        char files[32];
        char limit[32];
        (void)snprintf(files, sizeof(files), "%d", file_count);
        (void)snprintf(limit, sizeof(limit), "%d", file_limit);
        ani_log_warn("autoindex.skip", "reason",
    ...
    if (ani_thread_create(&srv->autoindex_tid, 0, autoindex_thread, srv) == 0) {
...
bool ani_mcp_jsonrpc_response_prepend_notice(char **response_io, const char *notice) {
...
char *ani_mcp_server_handle(ani_mcp_server_t *srv, const char *line) {
    ...
    if (ani_jsonrpc_parse(line, &req) < 0) {
        return ani_jsonrpc_format_error(0, JSONRPC_PARSE_ERROR, "Parse error");
    }

    /* Notifications (no id) → handle cancellation, then no response */
    if (!req.has_id) {
        if (req.method && strcmp(req.method, "notifications/cancelled") == 0) {
            if (ani_mcp_cancel_request_matches(req.params_raw, srv->active_request_id,
                                               srv->active_request_id_str) &&
                ani_mcp_server_cancel_active(srv)) {
                ani_log_info("mcp.cancelled", "match", "true");
            }
        }
    ...
    if (!ani_mcp_server_request_scope_begin(srv)) {
        int64_t request_id = req.id;
        ani_jsonrpc_request_free(&req);
        return ani_jsonrpc_format_error(request_id, JSONRPC_INTERNAL_ERROR,
                                        "Request cancellation scope unavailable");
    }

    struct timespec req_t0;
    ani_clock_gettime(CLOCK_MONOTONIC, &req_t0);
    char *result_json = NULL;
    char *request_error_json = NULL;
    bool request_logged = false;

    if (strcmp(req.method, "initialize") == 0) {
        result_json = ani_mcp_initialize_response_for_profile(req.params_raw, srv->tool_profile);
        detect_session(srv);
        if (srv->background_tasks && srv->tool_profile == ANI_MCP_TOOL_PROFILE_ALL) {
        ...
        result_json = ani_mcp_prompts_list();
    } else if (strcmp(req.method, "prompts/get") == 0) {
        result_json = ani_mcp_prompt_get(req.params_raw, &request_error_json);
    } else if (strcmp(req.method, "tools/list") == 0) {
        result_json = ani_mcp_tools_list_page(srv->tool_profile, req.params_raw);
    } else if (strcmp(req.method, "tools/call") == 0) {
        char *tool_name = req.params_raw ? ani_mcp_get_tool_name(req.params_raw) : NULL;
        char *tool_args =
            req.params_raw ? ani_mcp_get_arguments(req.params_raw) : heap_strdup("{}");
        ...
        result_json = ani_mcp_handle_tool(srv, tool_name, tool_args);
        srv->active_request_id = ANI_NOT_FOUND;
        free(srv->active_request_id_str);
        srv->active_request_id_str = NULL;
        struct timespec t1;
        ani_clock_gettime(CLOCK_MONOTONIC, &t1);
        ...
        ani_jsonrpc_response_t err_resp = {
            .id = req.id,
            .id_str = req.id_str,
            .error_json = err_obj,
        };
        char *err = ani_jsonrpc_format_response(&err_resp);
        struct timespec t1;
        ani_clock_gettime(CLOCK_MONOTONIC, &t1);
        long long dur_us = ((long long)(t1.tv_sec - req_t0.tv_sec) * MCP_S_TO_US) +
                           ((long long)(t1.tv_nsec - req_t0.tv_nsec) / MCP_MS_TO_US);
        ani_log_mcp_request(req.method, NULL, true, dur_us);
        ani_mcp_server_request_scope_end(srv);
        ani_jsonrpc_request_free(&req);
        return err;
    }
    ...
    if (!request_logged) {
        ...
        long long dur_us = ((long long)(t1.tv_sec - req_t0.tv_sec) * MCP_S_TO_US) +
                           ((long long)(t1.tv_nsec - req_t0.tv_nsec) / MCP_MS_TO_US);
        ani_log_mcp_request(req.method, NULL, false, dur_us);
    }
    ...
    char *out = ani_jsonrpc_format_response(&resp);
    free(result_json);
    ani_mcp_server_request_scope_end(srv);
    ani_jsonrpc_request_free(&req);
    return out;
...
static bool parse_content_length(const char *line, size_t *out) {
    ...
    errno = 0;
    char *end = NULL;
    unsigned long long parsed = strtoull(cursor, &end, ANI_DECIMAL_BASE);
...
static int poll_for_input_unix(ani_mcp_server_t *srv, int fd, FILE *in) {
    ...
    if (saved_flags < 0) {
        ...
        if (pr == 0) {
            ani_mcp_server_evict_idle(srv, STORE_IDLE_TIMEOUT_S);
            return 0;
        }
        return SKIP_ONE;
    }
...
int ani_mcp_server_run(ani_mcp_server_t *srv, FILE *in, FILE *out) {
    ...
    _setmode(ani_fileno(in), _O_BINARY);
    _setmode(ani_fileno(out), _O_BINARY);
#endif
    ...
    for (;;) {
        ...
        if (ani_mcp_read_message(in, &message, &content_length_framed) <= 0) {
            break;
        }

        char *resp = ani_mcp_server_handle(srv, message);
        free(message);
        if (resp) {
