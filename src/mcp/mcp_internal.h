#ifndef ANI_MCP_INTERNAL_H
#define ANI_MCP_INTERNAL_H

#include "mcp/mcp.h"
#include "pipeline/pipeline.h" /* ani_changed_hunk_t */
#include "store/store.h"       /* ani_node_t */
...
typedef bool (*ani_mcp_quarantine_test_hook_fn)(void *context, const char *step);
typedef bool (*ani_mcp_command_test_hook_fn)(void *context, const char *command);
#ifdef ANI_ENABLE_TEST_SEAMS
typedef void (*ani_mcp_auto_index_count_test_hook_fn)(void *context);
#endif
 ...
 * ani_mcp_server_new(NULL) semantics remain unchanged; daemon sessions use
...
enum { ANI_MCP_DEFAULT_AUTO_INDEX_LIMIT = 50000 };
