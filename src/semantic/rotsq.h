 ...
 */
#ifndef ANI_SEMANTIC_ROTSQ_H
#define ANI_SEMANTIC_ROTSQ_H

#include <stdint.h>

enum {
    ANI_RSQ_IN_DIM = 768,                 /* input dimension (ANI_SEM_DIM) */
    ANI_RSQ_DIM = 1024,                   /* padded pow2 rotation dimension */
    ANI_RSQ_BITS = 4,                     /* bits per coordinate */
    ANI_RSQ_LEVELS = 15,                  /* (1 << ANI_RSQ_BITS) - 1 */
    ANI_RSQ_CODE_BYTES = ANI_RSQ_DIM / 2, /* two 4-bit codes per byte */
};

typedef struct {
    uint8_t codes[ANI_RSQ_CODE_BYTES]; /* packed 4-bit codes, little nibble first */
    float scale;                       /* per-vector dequant scale  */
    float offset;                      /* per-vector dequant offset */
    int32_t code_sum;                  /* Σ codes (for the IP expansion) */
} ani_rsq_code_t;

/* Encode a ANI_RSQ_IN_DIM float vector (zero-padded to ANI_RSQ_DIM, rotated,
 * quantized). Deterministic: the rotation is fixed at build time. */
void ani_rsq_encode(const float *v, ani_rsq_code_t *out);

/* Estimated inner product of the two ORIGINAL vectors from their codes.
 * Deterministic pure function of the codes. */
float ani_rsq_ip(const ani_rsq_code_t *a, const ani_rsq_code_t *b);

/* Dequantize a code into the ROTATED space (ANI_RSQ_DIM floats). Note: this
 ...
 * coordinates. */
void ani_rsq_decode(const ani_rsq_code_t *c, float *out);

#endif /* ANI_SEMANTIC_ROTSQ_H */
