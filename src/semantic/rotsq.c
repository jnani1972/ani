...
static float g_rsq_diag[ANI_RSQ_DIM];
enum {
...
static void rsq_fwht(float *v) {
    for (int len = 1; len < ANI_RSQ_DIM; len <<= 1) {
        for (int i = 0; i < ANI_RSQ_DIM; i += len << 1) {
...
void ani_rsq_encode(const float *v, ani_rsq_code_t *out) {
    ...
    float rot[ANI_RSQ_DIM];
    for (int d = 0; d < ANI_RSQ_IN_DIM; d++) {
        rot[d] = v[d] * g_rsq_diag[d];
    }
    for (int d = ANI_RSQ_IN_DIM; d < ANI_RSQ_DIM; d++) {
        rot[d] = 0.0F;
    }
    rsq_fwht(rot);
    ...
    /* Per-vector scalar quantization over [lo, hi] at ANI_RSQ_BITS. */
    float range = hi - lo;
    float step = range > 0.0F ? range / (float)ANI_RSQ_LEVELS : 1.0F;
    out->offset = lo;
    out->scale = step;
...
float ani_rsq_ip(const ani_rsq_code_t *a, const ani_rsq_code_t *b) {
    ...
    int64_t dot = 0;
    for (int i = 0; i < ANI_RSQ_CODE_BYTES; i++) {
