 ...
 */
#ifndef ANI_SEMANTIC_H
#define ANI_SEMANTIC_H
 ...
 * but ~80% precision). */
#define ANI_SEM_EDGE_THRESHOLD 0.75

/* Maximum SEMANTICALLY_RELATED edges per node. */
enum { ANI_SEM_MAX_EDGES = 10 };

/* AST structural profile: 25 float features per function (control flow,
 * nesting, expression types, literals, data flow, Halstead). */
enum { ANI_SEM_AST_PROFILE_DIMS = 25 };

/* MinHash fingerprint length (must match simhash/minhash.h ANI_MINHASH_K). */
enum { ANI_SEM_MINHASH_K = 64 };
 ...
 * Call this BEFORE dispatching parallel work that invokes ani_sem_random_index,
...
typedef struct {
    ...
    ani_rsq_code_t ri_code;
    ani_rsq_code_t api_code;
    ani_rsq_code_t type_code;
    ani_rsq_code_t deco_code;

    /* AST profile as float vector (decoded from "sp" property). */
    float struct_profile[ANI_SEM_AST_PROFILE_DIMS];

    /* MinHash fingerprint (decoded from "fp" property). */
    bool has_minhash;
    uint32_t minhash[ANI_SEM_MINHASH_K];
} ani_sem_func_t;

/* ── Corpus-level data ───────────────────────────────────────────── */

/* Opaque corpus handle for IDF and Random Indexing state. */
typedef struct ani_sem_corpus ani_sem_corpus_t;
 ...
 * This replaces a loop of ani_sem_corpus_add_doc() calls. */
