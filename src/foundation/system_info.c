...
enum { DEFAULT_CORES = 1, MIN_WORKERS = 1, ANI_WORKERS_MAX = 256 };
...
static ani_system_info_t detect_system_macos(void) {
    ani_system_info_t info;
...
static ani_system_info_t detect_system_bsd(void) {
    ani_system_info_t info;
    memset(&info, 0, sizeof(info));
...
int ani_detect_cgroup_cpus(const char *cgroup_root) {
    ...
    long quota = strtol(buf, NULL, ANI_DECIMAL_BASE);
    if (quota <= 0) {
        return -1;
    }
    ...
    long period = strtol(buf, NULL, ANI_DECIMAL_BASE);
    if (period <= 0) {
        return -1;
    }
...
size_t ani_detect_cgroup_mem(const char *cgroup_root) {
    char path[ANI_PATH_MAX];
    char buf[ANI_SZ_64];
    ...
    if (read_small_file(path, buf, sizeof(buf)) > 0) {
        ...
        char *end = NULL;
        unsigned long long n = strtoull(buf, &end, ANI_DECIMAL_BASE);
        if (end == buf || n == 0) {
            return 0;
        }
        return (size_t)n;
...
static ani_system_info_t detect_system_linux(void) {
    ani_system_info_t info;
    memset(&info, 0, sizeof(info));
    ...
    int cg_cpus = ani_detect_cgroup_cpus("/sys/fs/cgroup");
    ...
    size_t cg_ram = ani_detect_cgroup_mem("/sys/fs/cgroup");
...
#ifdef _WIN32
static ani_system_info_t detect_system_windows(void) {
    ani_system_info_t info;
    memset(&info, 0, sizeof(info));
...
static ani_system_info_t cached_info;

ani_system_info_t ani_system_info(void) {
    if (!info_cached) {
#ifdef _WIN32
...
int ani_default_worker_count(bool initial) {
    /* ANI_WORKERS env override (clamped to [1, ANI_WORKERS_MAX]).
     ...
     * Same precedence shape as other ANI_* env overrides:
     * explicit override > implicit detection. */
    char buf[ANI_SZ_32];
    if (ani_safe_getenv("ANI_WORKERS", buf, sizeof(buf), NULL) != NULL) {
        long n = strtol(buf, NULL, ANI_DECIMAL_BASE);
        if (n >= MIN_WORKERS && n <= ANI_WORKERS_MAX) {
            return (int)n;
        }
        ani_log_warn("workers.env.invalid", "value", buf, "fallback", "sysconf");
    }
