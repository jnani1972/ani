...
ANIInternPool *ani_intern_create(void) {
    ANIInternPool *p = (ANIInternPool *)calloc(ANI_ALLOC_ONE, sizeof(*p));
    if (!p) {
        return NULL;
    }
    ani_arena_init(&p->arena);
    p->capacity = ANI_SZ_256;
    p->mask = p->capacity - SKIP_ONE;
    p->buckets = (InternEntry *)calloc(p->capacity, sizeof(InternEntry));
    if (!p->buckets) {
        ani_arena_destroy(&p->arena);
        free(p);
        return NULL;
    }
    return p;
...
void ani_intern_free(ANIInternPool *pool) {
    ...
    ani_arena_destroy(&pool->arena);
    free(pool->buckets);
    free(pool);
}

static void intern_resize(ANIInternPool *p) {
...
const char *ani_intern_n(ANIInternPool *pool, const char *s, size_t len) {
    ...
    /* Resize at 70% load */
    if (pool->count * ANI_DECIMAL_BASE >= pool->capacity * INTERN_LOAD_NUM) {
        intern_resize(pool);
    ...
    char *copy = ani_arena_strndup(&pool->arena, s, len);
    if (!copy) {
        return NULL;
    }
...
const char *ani_intern(ANIInternPool *pool, const char *s) {
    ...
    return ani_intern_n(pool, s, strlen(s));
}
