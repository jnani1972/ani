...
#include "compat.h" /* ani_nanosleep */
#include "compat_fs.h"
#include "log.h"
#include "platform.h"  /* ani_now_ms */
#include "sanitized.h" /* ANI_SANITIZED — spawn-retry budget */
...
#ifdef _WIN32
#include <windows.h>
#include "win_utf8.h" /* ani_utf8_to_wide — spawn the worker with a wide command line so a
...
#define ANI_WIN_CRASH_CODE_MIN 0xC0000000u
#define ANI_WIN_CONTROL_C_EXIT 0xC000013Au

#ifndef _WIN32
static bool ani_is_fault_signal(int sig) {
    switch (sig) {
    case SIGSEGV:
    case SIGBUS:
    case SIGILL:
...
ani_proc_outcome_t ani_proc_classify(bool exited_normally, int exit_code, int term_signal,
                                     bool timed_out) {
    if (timed_out) {
        return ANI_PROC_HANG;
    }
    if (!exited_normally) {
        /* POSIX signal death. */
#ifndef _WIN32
        if (ani_is_fault_signal(term_signal)) {
            return ANI_PROC_CRASH;
        }
#else
        (void)term_signal;
#endif
        return ANI_PROC_KILLED;
    }
    ...
    if ((unsigned)exit_code == ANI_WIN_CONTROL_C_EXIT) {
        return ANI_PROC_KILLED;
    }
    if ((unsigned)exit_code >= ANI_WIN_CRASH_CODE_MIN) {
        return ANI_PROC_CRASH;
    }
    return (exit_code == 0) ? ANI_PROC_CLEAN : ANI_PROC_EXIT_NONZERO;
}

const char *ani_proc_outcome_str(ani_proc_outcome_t o) {
    switch (o) {
    case ANI_PROC_CLEAN:
        return "clean";
    case ANI_PROC_EXIT_NONZERO:
        return "exit_nonzero";
    case ANI_PROC_CRASH:
        return "crash";
    case ANI_PROC_HANG:
        return "hang";
    case ANI_PROC_KILLED:
        return "killed";
    case ANI_PROC_SPAWN_FAILED:
    default:
        return "spawn_failed";
    }
}
...
static ani_tail_result_t ani_tail_log(const char *log_file, long *tail_pos, ani_proc_log_cb cb,
                                      void *ud, bool final) {
    ani_tail_result_t result = {.status = ANI_TAIL_ERROR, .progressed = false};
    if (!log_file || !tail_pos) {
        return result;
    }
#ifdef _WIN32
    FILE *lf = ani_fopen(log_file, "r");
#else
    ...
    if (fseek(lf, *tail_pos, SEEK_SET) == 0) {
        ...
        enum {
            ANI_TAIL_MAX_LINES_PER_POLL = 64,
            ANI_TAIL_MAX_BYTES_PER_POLL = 64 * 1024,
        };
        while (delivered_lines < ANI_TAIL_MAX_LINES_PER_POLL &&
               delivered_bytes < ANI_TAIL_MAX_BYTES_PER_POLL) {
            long before = ftell(lf);
            if (!fgets(line, sizeof(line), lf)) {
                result.status = ferror(lf) ? ANI_TAIL_ERROR : ANI_TAIL_CAUGHT_UP;
                break;
            }
            ...
            if (complete) {
                ...
                *tail_pos = ftell(lf);
                ...
                result.status = ANI_TAIL_CAUGHT_UP;
                break;
            }
        }
        if (result.status == ANI_TAIL_ERROR && !ferror(lf)) {
...
static size_t ani_cmdline_put(char *buf, size_t cap, size_t pos, char c, bool *ovf) {
    if (pos + 1 >= cap) {
        *ovf = true;
        return pos;
    }
...
static size_t ani_cmdline_append_arg(char *buf, size_t cap, size_t pos, const char *arg, bool first,
                                     bool *ovf) {
    if (!first) {
        pos = ani_cmdline_put(buf, cap, pos, ' ', ovf);
    }
    pos = ani_cmdline_put(buf, cap, pos, '"', ovf);
    for (const char *p = arg; *p;) {
        ...
        if (*p == '"') {
            ...
            pos = ani_cmdline_put(buf, cap, pos, *p, ovf);
            p++;
        }
    }
    pos = ani_cmdline_put(buf, cap, pos, '"', ovf);
 ...
 * only runs on Windows. Shared by ani_subprocess_spawn_win AND the UI http_server
...
bool ani_build_win_cmdline(char *buf, size_t cap, const char *const *argv) {
    if (!buf || cap == 0 || !argv) {
        return false;
    }
    size_t pos = 0;
    ...
    for (int i = 0; argv[i]; i++) {
        pos = ani_cmdline_append_arg(buf, cap, pos, argv[i], i == 0, &ovf);
        if (ovf) {
...
static bool ani_ascii_case_equal(const char *left, const char *right) {
    if (!left || !right) {
        return false;
    }
    while (*left && *right) {
...
static bool ani_win_cmd_path_is_absolute(const char *path) {
    if (!path) {
        return false;
    }
    ...
    return ani_ascii_case_equal(basename, "cmd.exe");
}

bool ani_build_win_cmd_payload(char *buf, size_t cap, const char *cmd_executable,
                               const char *payload) {
    if (!buf || cap == 0) {
        return false;
    }
    buf[0] = '\0';
    if (!ani_win_cmd_path_is_absolute(cmd_executable) || !payload || !payload[0]) {
        return false;
    }
...
enum { ANI_SUBPROCESS_ARGV_LIMIT = 4096 };

typedef enum {
    ANI_SUBPROCESS_ACTIVE = 0,
    ANI_SUBPROCESS_CANCEL_REQUESTED,
    ANI_SUBPROCESS_DRAINING,
    ANI_SUBPROCESS_TERMINAL,
} ani_subprocess_lifecycle_t;

struct ani_subprocess {
    char *bin;
    char **argv;
    size_t argc;
    char *windows_cmd_payload;
    char *log_file;
    ani_proc_log_cb on_log_line;
    void *log_ud;
...
static void ani_subprocess_result_init(ani_proc_result_t *result) {
...
static void ani_subprocess_free_config(ani_subprocess_t *process) {
    if (!process) {
        return;
    }
    if (process->argv) {
...
static ani_subprocess_t *ani_subprocess_copy_opts(const ani_proc_opts_t *opts) {
    if (!opts || !opts->bin || !opts->bin[0]) {
        return NULL;
    }
#ifdef _WIN32
    if (opts->windows_cmd_payload &&
        (!opts->windows_cmd_payload[0] || opts->argv || !ani_win_cmd_path_is_absolute(opts->bin))) {
        return NULL;
    }
#else
    if (opts->windows_cmd_payload) {
    ...
    if (opts->argv) {
        argc = 0;
        while (argc < ANI_SUBPROCESS_ARGV_LIMIT && opts->argv[argc]) {
            argc++;
        }
        if (argc == 0 || argc == ANI_SUBPROCESS_ARGV_LIMIT) {
            return NULL;
        }
    }

    ani_subprocess_t *process = (ani_subprocess_t *)calloc(1, sizeof(*process));
    if (!process) {
        return NULL;
    }
    process->bin = ani_strdup(opts->bin);
    process->windows_cmd_payload =
        opts->windows_cmd_payload ? ani_strdup(opts->windows_cmd_payload) : NULL;
    ...
    if (!process->bin || !process->argv ||
        ...
        ani_subprocess_free_config(process);
        return NULL;
    }
    for (size_t i = 0; i < argc; i++) {
        const char *arg = opts->argv ? opts->argv[i] : opts->bin;
        process->argv[i] = ani_strdup(arg);
        if (!process->argv[i]) {
            ani_subprocess_free_config(process);
            return NULL;
        }
    ...
    if (opts->log_file) {
        process->log_file = ani_strdup(opts->log_file);
        if (!process->log_file) {
            ani_subprocess_free_config(process);
            return NULL;
        }
    ...
    process->cancel_grace_ms =
        opts->cancel_grace_ms > 0 ? opts->cancel_grace_ms : ANI_SUBPROCESS_DEFAULT_CANCEL_GRACE_MS;
    if (process->cancel_grace_ms > ANI_SUBPROCESS_MAX_CANCEL_GRACE_MS) {
        process->cancel_grace_ms = ANI_SUBPROCESS_MAX_CANCEL_GRACE_MS;
    }
    process->delete_log_on_exit = opts->delete_log_on_exit;
    atomic_init(&process->lifecycle, ANI_SUBPROCESS_ACTIVE);
    ani_subprocess_result_init(&process->result);
    return process;
}

static ani_tail_result_t ani_subprocess_poll_log(ani_subprocess_t *process, bool final) {
    ani_tail_result_t result = ani_tail_log(process->log_file, &process->tail_pos,
                                            process->on_log_line, process->log_ud, final);
    if (result.progressed) {
        process->last_activity_ms = ani_now_ms();
    }
    return result;
}

static bool ani_subprocess_cancellation_requested(const ani_subprocess_t *process) {
    return atomic_load_explicit(&process->lifecycle, memory_order_acquire) ==
           ANI_SUBPROCESS_CANCEL_REQUESTED;
}

static void ani_subprocess_delete_log(ani_subprocess_t *process) {
    if (!process->log_file || !process->delete_log_on_exit) {
        return;
    }
#ifdef _WIN32
    wchar_t *path = ani_path_to_wide(process->log_file);
    if (path) {
...
static bool ani_subprocess_begin_terminal_transition(ani_subprocess_t *process) {
    int lifecycle = atomic_load_explicit(&process->lifecycle, memory_order_acquire);
    for (;;) {
        if (lifecycle == ANI_SUBPROCESS_DRAINING) {
            return true;
        }
        if (lifecycle == ANI_SUBPROCESS_TERMINAL) {
            return false;
        }
        int desired = ANI_SUBPROCESS_DRAINING;
        if (atomic_compare_exchange_weak_explicit(&process->lifecycle, &lifecycle, desired,
                                                  memory_order_acq_rel, memory_order_acquire)) {
            process->result.cancellation_requested = lifecycle == ANI_SUBPROCESS_CANCEL_REQUESTED;
            return true;
        }
    }
}

static ani_proc_poll_t ani_subprocess_publish_terminal(ani_subprocess_t *process,
                                                       ani_proc_result_t *out, bool delete_log) {
    if (delete_log) {
        ani_subprocess_delete_log(process);
    }
    atomic_store_explicit(&process->lifecycle, ANI_SUBPROCESS_TERMINAL, memory_order_release);
    if (out) {
        *out = process->result;
    }
    return ANI_PROC_POLL_TERMINAL;
}

static ani_proc_poll_t ani_subprocess_finish(ani_subprocess_t *process, ani_proc_result_t *out) {
    if (!ani_subprocess_begin_terminal_transition(process)) {
        return ANI_PROC_POLL_ERROR;
    }
    process->result.tree_quiesced = true;
    process->result.supervision_failed = false;
    if (process->log_file && process->on_log_line) {
        return ANI_PROC_POLL_RUNNING;
    }
    return ani_subprocess_publish_terminal(process, out, true);
}

static ani_proc_poll_t ani_subprocess_finish_failed(ani_subprocess_t *process,
                                                    ani_proc_result_t *out) {
    if (!ani_subprocess_begin_terminal_transition(process)) {
        return ANI_PROC_POLL_ERROR;
    }
    ...
    if (process->timed_out) {
        process->result.outcome = ANI_PROC_HANG;
    } else if (process->result.outcome == ANI_PROC_SPAWN_FAILED) {
        process->result.outcome = ANI_PROC_KILLED;
    }
    process->containment_failed = true;
    return ani_subprocess_publish_terminal(process, out, false);
}

#ifdef _WIN32

static void ani_win_close_spawn_handles(HANDLE nul, HANDLE log, LPPROC_THREAD_ATTRIBUTE_LIST attrs,
                                        bool attrs_init) {
    if (attrs) {
        if (attrs_init) {
...
static int ani_subprocess_spawn_win(ani_subprocess_t *process) {
    char cmdline[8192];
    bool built =
        process->windows_cmd_payload
            ? ani_build_win_cmd_payload(cmdline, sizeof(cmdline), process->bin,
                                        process->windows_cmd_payload)
            : ani_build_win_cmdline(cmdline, sizeof(cmdline), (const char *const *)process->argv);
    if (!built) {
        return -1;
    }
    wchar_t *wbin = ani_utf8_to_wide(process->bin);
    wchar_t *wcmdline = ani_utf8_to_wide(cmdline);
    if (!wbin || !wcmdline) {
        free(wbin);
        free(wcmdline);
        return -1;
    ...
    if (process->log_file) {
        wchar_t *wlog = ani_path_to_wide(process->log_file);
        if (wlog) {
    ...
    if (!attrs_ready) {
        ani_win_close_spawn_handles(nul, log, attrs, attrs_init);
        CloseHandle(job);
        free(wbin);
        free(wcmdline);
        return -1;
...
static bool ani_win_job_active(ani_subprocess_t *process, bool *known) {
...
static void ani_win_begin_termination(ani_subprocess_t *process, uint64_t now) {
...
static void ani_win_force_tree(ani_subprocess_t *process, uint64_t now) {
...
static void ani_win_capture_root(ani_subprocess_t *process, DWORD code) {
    ...
    if (process->timed_out) {
        process->result.outcome = ANI_PROC_HANG;
               ...
               (ani_subprocess_cancellation_requested(process) && code == ANI_WIN_CONTROL_C_EXIT)) {
        process->result.outcome = ANI_PROC_KILLED;
    } else {
        process->result.outcome = ani_proc_classify(true, (int)code, 0, false);
    }
}

static ani_proc_poll_t ani_subprocess_poll_win(ani_subprocess_t *process, ani_proc_result_t *out) {
    ...
    if (!process->root_reaped) {
        ...
        if (waited == WAIT_OBJECT_0) {
            ...
            ani_win_capture_root(process, code);
        } else if (waited == WAIT_FAILED) {
            DWORD code = STILL_ACTIVE;
            if (GetExitCodeProcess(process->process, &code) && code != STILL_ACTIVE) {
                ani_win_capture_root(process, code);
            } else {
                ani_win_begin_termination(process, now);
                ani_win_force_tree(process, now);
            }
        }
    }

    bool job_known = false;
    bool job_active = ani_win_job_active(process, &job_known);
    if (!process->termination_started) {
        if (ani_subprocess_cancellation_requested(process)) {
            ani_win_begin_termination(process, now);
    ...
    if (process->force_started_ms != 0 &&
        now - process->force_started_ms >= ANI_SUBPROCESS_FORCE_SETTLE_MS &&
        (!job_known || job_active || !process->root_reaped)) {
        return ani_subprocess_finish_failed(process, out);
    }
    if (process->root_reaped && !job_active) {
        return ani_subprocess_finish(process, out);
    }
    return ANI_PROC_POLL_RUNNING;
}
...
/* Transient spawn-failure retry (see the EAGAIN note in ani_posix_spawn_apple):
 ...
 * source of truth for both — see ani_spawn_backoff for the resulting waits.
 *
...
#if ANI_SANITIZED
enum { ANI_SPAWN_RETRY = 2, ANI_SPAWN_RETRY_ATTEMPTS = 9 };
#else
enum { ANI_SPAWN_RETRY = 2, ANI_SPAWN_RETRY_ATTEMPTS = 6 };
#endif
 ...
 * ANI_SPAWN_RETRY_ATTEMPTS above and from the per-wait ceiling at
 * ani_spawn_backoff, rather than being listed separately here, so changing the
...
void ani_subprocess_force_spawn_eagain_for_testing(int attempts) {
    g_force_spawn_eagain = attempts > 0 ? attempts : 0;
}
int ani_subprocess_pending_spawn_eagain_for_testing(void) {
    return g_force_spawn_eagain;
}
static bool ani_spawn_eagain_injected(void) {
 ...
 *
 * The clamp it replaces bounded `attempt` by ANI_SPAWN_RETRY_ATTEMPTS, which no
 ...
 * clamped. */
enum { ANI_SPAWN_BACKOFF_BASE_MS = 10, ANI_SPAWN_BACKOFF_MAX_SHIFT = 8 };

static void ani_spawn_backoff(int attempt) {
    int shift = attempt < ANI_SPAWN_BACKOFF_MAX_SHIFT ? attempt : ANI_SPAWN_BACKOFF_MAX_SHIFT;
    long ms = (long)ANI_SPAWN_BACKOFF_BASE_MS << shift;
    struct timespec delay = {ms / 1000L, (ms % 1000L) * 1000L * 1000L};
    (void)ani_nanosleep_full(&delay);
}
...
static pid_t ani_fork_with_retry(void) {
    /* ANI_SPAWN_RETRY_ATTEMPTS backoffs means ATTEMPTS+1 tries. Every try goes
...
#ifdef ANI_ENABLE_TEST_SEAMS
        if (ani_spawn_eagain_injected()) {
            if (attempt >= ANI_SPAWN_RETRY_ATTEMPTS) {
                errno = EAGAIN;
                return -1;
            }
            ani_spawn_backoff(attempt);
            continue;
        }
#endif
        pid_t pid = fork();
...
static void ani_posix_reset_child_signals(void) {
 ...
 * fallback (see ani_posix_spawn_apple), which preserves the documented
 * "bogus binary => child exits 127" contract across platforms. */
static void ani_posix_child_exec(ani_subprocess_t *process, int input, int output, long max_fd) {
    if (setpgid(0, 0) < 0) {
        _exit(127);
    }
    ani_posix_reset_child_signals();
...
static int ani_posix_fd_at_least_three(int fd) {
...
static int ani_posix_spawn_apple(ani_subprocess_t *process, int input, int output, pid_t *pid_out) {
...
static int ani_subprocess_spawn_posix(ani_subprocess_t *process) {
    int input_flags = O_RDONLY;
#ifdef O_CLOEXEC
    input_flags |= O_CLOEXEC;
#endif
    int input = ani_posix_fd_at_least_three(open("/dev/null", input_flags));
...
#endif
    int output = ani_posix_fd_at_least_three(open(target, output_flags, 0600));
    if (input < 0 || output < 0) {
        if (input >= 0) {
            (void)close(input);
        }
    ...
    pid_t pid = -1;
#ifdef __APPLE__
    int spawn_rc = ani_posix_spawn_apple(process, input, output, &pid);
    for (int attempt = 0; spawn_rc == ANI_SPAWN_RETRY && attempt < ANI_SPAWN_RETRY_ATTEMPTS;
         attempt++) {
        ani_spawn_backoff(attempt);
        spawn_rc = ani_posix_spawn_apple(process, input, output, &pid);
    }
    if (spawn_rc == ANI_SPAWN_RETRY) {
    ...
    if (spawn_rc > 0) { /* exec-class failure: reproduce the fork+exec 127 */
        ...
        if (pid == 0) {
            ani_posix_child_exec(process, input, output, max_fd);
        }
    }
#else
    pid = ani_fork_with_retry();
...
static bool ani_posix_group_active(ani_subprocess_t *process) {
...
static void ani_posix_begin_termination(ani_subprocess_t *process, uint64_t now) {
    if (process->termination_started) {
        return;
    }
    process->termination_started = true;
...
static void ani_posix_force_tree(ani_subprocess_t *process, uint64_t now) {
    if (process->force_sent) {
        return;
    }
    if (process->force_started_ms == 0) {
...
static void ani_posix_capture_root(ani_subprocess_t *process, int status) {
    process->root_reaped = true;
    if (WIFEXITED(status)) {
        process->result.exit_code = WEXITSTATUS(status);
        process->result.term_signal = 0;
        process->result.outcome =
            ani_proc_classify(true, process->result.exit_code, 0, process->timed_out);
    } else if (WIFSIGNALED(status)) {
        process->result.exit_code = -1;
        process->result.term_signal = WTERMSIG(status);
        process->result.outcome =
            ani_proc_classify(false, -1, process->result.term_signal, process->timed_out);
    } else {
        process->result.exit_code = -1;
        process->result.term_signal = 0;
        process->result.outcome = process->timed_out ? ANI_PROC_HANG : ANI_PROC_KILLED;
    }
}

static ani_proc_poll_t ani_subprocess_poll_posix(ani_subprocess_t *process,
                                                 ani_proc_result_t *out) {
    uint64_t now = ani_now_ms();
    ...
    if (!process->root_reaped) {
        ...
        if (waited == process->pid) {
            ani_posix_capture_root(process, status);
            ...
            process->result.exit_code = -1;
            process->result.term_signal = 0;
            ani_posix_begin_termination(process, now);
        }
    }

    bool group_active = ani_posix_group_active(process);
    if (!process->termination_started) {
        if (ani_subprocess_cancellation_requested(process)) {
            ani_posix_begin_termination(process, now);
        } else if (!process->root_reaped && process->quiet_timeout_ms > 0 &&
    ...
    if (process->termination_started && group_active && !process->force_sent &&
        now - process->termination_started_ms >= (uint64_t)process->cancel_grace_ms) {
        ani_posix_force_tree(process, now);
    }
    group_active = ani_posix_group_active(process);
...
int ani_subprocess_spawn(const ani_proc_opts_t *opts, ani_subprocess_t **out) {
    if (!out) {
        return -1;
    }
    *out = NULL;
    ani_subprocess_t *process = ani_subprocess_copy_opts(opts);
    if (!process) {
        return -1;
    }
#ifdef _WIN32
    int spawn_rc = ani_subprocess_spawn_win(process);
#else
    int spawn_rc = ani_subprocess_spawn_posix(process);
#endif
    if (spawn_rc != 0) {
        ani_subprocess_free_config(process);
        return -1;
...
ani_proc_poll_t ani_subprocess_poll(ani_subprocess_t *process, ani_proc_result_t *out) {
    if (!process) {
        return ANI_PROC_POLL_ERROR;
    }
    int lifecycle = atomic_load_explicit(&process->lifecycle, memory_order_acquire);
    ...
    if (lifecycle == ANI_SUBPROCESS_DRAINING) {
        ani_tail_result_t tail = ani_subprocess_poll_log(process, true);
        if (tail.status == ANI_TAIL_MORE) {
            return ANI_PROC_POLL_RUNNING;
        }
        if (tail.status == ANI_TAIL_ERROR) {
            ani_log_error("subprocess.log_drain_failed", "reason", "io_error");
            return ani_subprocess_publish_terminal(process, out, false);
        }
        return ani_subprocess_publish_terminal(process, out, true);
    }
    ...
    (void)ani_subprocess_poll_log(process, false);
#ifdef _WIN32
    return ani_subprocess_poll_win(process, out);
#else
    return ani_subprocess_poll_posix(process, out);
#endif
}

bool ani_subprocess_request_cancel(ani_subprocess_t *process) {
    if (!process) {
        return false;
    }
    int lifecycle = atomic_load_explicit(&process->lifecycle, memory_order_acquire);
    for (;;) {
        if (lifecycle == ANI_SUBPROCESS_CANCEL_REQUESTED) {
            return true;
        }
        if (lifecycle != ANI_SUBPROCESS_ACTIVE) {
            return false;
        }
        int desired = ANI_SUBPROCESS_CANCEL_REQUESTED;
        if (atomic_compare_exchange_weak_explicit(&process->lifecycle, &lifecycle, desired,
                                                  memory_order_acq_rel, memory_order_acquire)) {
            return true;
        }
...
void ani_subprocess_destroy(ani_subprocess_t *process) {
...
int ani_subprocess_run(const ani_proc_opts_t *opts, ani_proc_result_t *out) {
    ani_proc_result_t local;
    if (!out) {
        out = &local;
    }
    ani_subprocess_result_init(out);

    ani_subprocess_t *process = NULL;
    if (ani_subprocess_spawn(opts, &process) != 0) {
        return -1;
    }
    for (;;) {
