 ...
 * The ANI_MEM_BUDGET_MB env var, when set to a positive integer, overrides
 ...
 * ANI_MEM_BUDGET_MB wins, while a larger/default budget is capped. */
void ani_mem_init_with_cap(double ram_fraction, size_t hard_cap_bytes);

/* Result of ani_mem_resolve_budget: the resolved budget plus the metadata
 * ani_mem_init logs — so the parse/clamp logic lives in exactly ONE place and
 * the caller never re-parses the env string. */
typedef struct {
    size_t budget;      /* resolved budget in bytes */
    const char *source; /* "ram_fraction" | "ANI_MEM_BUDGET_MB" | "daemon_worker_cap" */
...
/* Pure budget resolver shared by ani_mem_init (exposed for testing).
 * Returns ram_fraction * total_ram, unless `budget_mb` is a STRICTLY valid
 * positive integer string (the ANI_MEM_BUDGET_MB override) — then it returns
...
/* TEST HOOK: overwrite the budget directly, bypassing ani_mem_init's
 ...
 * guard: a later ani_mem_init still initializes normally. Callers must
 * save ani_mem_budget() first and restore it before their assertions.
 ...
 */
enum { ANI_MEM_MAP_BUCKETS = 8 };

typedef struct {
    /* Is plain malloc actually served by the configured allocator on this
     * build? If false, every allocator tuning in ani_mem_init (immediate
    ...
    size_t bucket_bytes[ANI_MEM_MAP_BUCKETS];
    size_t bucket_blocks[ANI_MEM_MAP_BUCKETS];
} ani_mem_map_t;
 ...
 */
enum { ANI_MEM_OWNERSHIP_CLASSES = 6 };

typedef struct {
    size_t probe_bytes[ANI_MEM_OWNERSHIP_CLASSES]; /* size probed per class */
    bool owned[ANI_MEM_OWNERSHIP_CLASSES];         /* predicate said "ours" */
    bool allocated[ANI_MEM_OWNERSHIP_CLASSES];     /* the probe itself worked */
    int owned_count;
    int probed_count;
    bool all_owned; /* every successfully probed class came back owned */
} ani_mem_ownership_audit_t;
 ...
 *   - Off unless ANI_MEM_PHASES=1, so the hot path pays one atomic load.
 */
 ...
 */
enum { ANI_MEM_TRACKED_REGIONS = 6 };
...
typedef struct {
    ...
    void *large_base[ANI_MEM_TRACKED_REGIONS];
    size_t large_size[ANI_MEM_TRACKED_REGIONS];
 ...
 * read straight out of the daemon log. No-op unless ANI_MEM_CENSUS=1. */
void ani_mem_census_log(const char *tag);

/* Census logging gate: ANI_MEM_CENSUS=1. Off by default — HeapWalk locks each
