...
ani_query_stats_t g_query_stats = {0};
static atomic_bool g_diag_stop = false;
static atomic_bool g_diag_done = false;
static ani_thread_t g_diag_thread;
static bool g_diag_started = false;
static bool g_diag_abandoned = false;
static time_t g_start_time = 0;
static ani_private_lock_directory_t *g_diag_directory = NULL;
#ifndef _WIN32
static int g_diag_directory_fd = -1;
#endif
static char g_diag_directory_path[ANI_PATH_MAX] = "";
static char g_diag_path[ANI_PATH_MAX] = "";
static char g_diag_ndjson_path[ANI_PATH_MAX] = "";
 ...
 * committed bytes cannot tell apart. Off unless ANI_MEM_STATS=1. */
...
#ifdef _WIN32
    /* _wopen mirrors ani_mkstemp's Windows contract — the ANSI CRT interprets
...
static void diag_write_allocator_stats(void) {
    char flag[ANI_SZ_16];
    if (ani_safe_getenv("ANI_MEM_STATS", flag, sizeof(flag), NULL) == NULL || flag[0] != '1') {
        return;
    }
    char path[ANI_PATH_MAX];
    ...
    int written =
        snprintf(path, sizeof(path), "%s/ani-allocator-stats-%d.txt", ani_tmpdir(), (int)getpid());
...
#endif
    ani_private_lock_directory_close(g_diag_directory);
    g_diag_directory = NULL;
    if (remove_directory && g_diag_directory_path[0] != '\0') {
        (void)ani_rmdir(g_diag_directory_path);
    }
}

/* Diagnostics placement honors the documented contract ($TMPDIR, /tmp
 * fallback) without touching ani_tmpdir(), whose many other callers keep
...
static const char *diag_tmp_base(char *buffer, size_t size) {
    ...
    return ani_tmpdir();
#else
    ani_safe_getenv("TMPDIR", buffer, size, NULL);
    if (buffer[0] != '\0') {
        return buffer;
    }
    return "/tmp";
...
static bool diag_directory_prepare(void) {
    char tmp_base[ANI_PATH_MAX];
    char directory_template[ANI_PATH_MAX];
    int written =
        snprintf(directory_template, sizeof(directory_template), "%s/ani-diagnostics-%d-XXXXXX",
                 diag_tmp_base(tmp_base, sizeof(tmp_base)), (int)getpid());
    if (written <= 0 || (size_t)written >= sizeof(directory_template) ||
        !ani_mkdtemp(directory_template)) {
        return false;
    }
    written =
    ...
    if (written <= 0 || (size_t)written >= sizeof(g_diag_directory_path)) {
        (void)ani_rmdir(directory_template);
        return false;
    }

#ifdef _WIN32
    wchar_t *wide_directory = ani_path_to_wide(g_diag_directory_path);
    HANDLE handle =
        wide_directory
    ...
    free(wide_directory);
    ani_private_file_lock_status_t status = ANI_PRIVATE_FILE_LOCK_IO;
    if (handle != INVALID_HANDLE_VALUE) {
        status = ani_private_lock_directory_adopt_windows(handle, g_diag_directory_path,
                                                          &g_diag_directory);
    ...
    ani_private_file_lock_status_t status = ANI_PRIVATE_FILE_LOCK_IO;
    if (same_directory) {
        status = ani_private_lock_directory_adopt_posix(validation_fd, g_diag_directory_path,
                                                        &g_diag_directory);
        if (status == ANI_PRIVATE_FILE_LOCK_OK) {
            validation_fd = -1;
    ...
    if (status != ANI_PRIVATE_FILE_LOCK_OK || !diag_set_output_paths()) {
...
static bool diag_write_file(const char *base_name, const char *data, size_t length, bool append) {
    ...
    wchar_t *wide_path = ani_path_to_wide(path);
...
static bool diag_posix_file_valid(int descriptor) {
    ...
    return descriptor >= 0 && fstat(descriptor, &state) == 0 && S_ISREG(state.st_mode) &&
           ...
           ani_macos_extended_acl_fd_is_empty(descriptor);
}
...
static void diag_wait_for_interval(void) {
    uint64_t deadline = ani_now_ms() + DIAG_INTERVAL_MS;
...
static void write_diagnostics(void) {
     ...
     * "leak"). ani_mem_rss()/ani_mem_peak_rss() already encode the whole
    ...
    current_rss = ani_mem_rss();
    peak_rss = ani_mem_peak_rss();
    ...
    ani_mem_map_t map;
    (void)ani_mem_map_collect_os(&map);
    size_t residual =
        map.os_committed_bytes > map.live_bytes ? map.os_committed_bytes - map.live_bytes : 0;
    char buckets[ANI_SZ_512];
    int bucket_length = 0;
    for (int i = 0;
         i < ANI_MEM_MAP_BUCKETS && bucket_length >= 0 && (size_t)bucket_length < sizeof(buckets);
         i++) {
        int written =
            snprintf(buckets + bucket_length, sizeof(buckets) - (size_t)bucket_length,
                     "%s{\"limit\": %zu, \"bytes\": %zu, \"blocks\": %zu}", i == 0 ? "" : ", ",
                     ani_mem_map_bucket_limit(i), map.bucket_bytes[i], map.bucket_blocks[i]);
        if (written < 0) {
            break;
        }
...
bool ani_diag_start(void) {
    ...
    ani_safe_getenv("ANI_DIAGNOSTICS", env_buf, sizeof(env_buf), NULL);
    ...
    if (ani_thread_create(&g_diag_thread, 0, diag_thread_fn, NULL) != 0) {
        diag_directory_close(true);
        return false;
    }

    g_diag_started = true;
    char interval[ANI_SZ_32];
    (void)snprintf(interval, sizeof(interval), "%d", DIAG_INTERVAL_MS / 1000);
    /* Discovery must survive ANI_LOG_LEVEL suppression and paths containing
     * spaces: a control record is the only announced copy of these paths. */
    ani_log_control("diagnostics.start", "snapshot", g_diag_path, "trajectory", g_diag_ndjson_path,
                    "interval_s", interval);
    return true;
}
...
void ani_diag_stop(void) {
    ...
    uint64_t deadline = ani_now_ms() + DIAG_STOP_TIMEOUT_MS;
    while (!atomic_load_explicit(&g_diag_done, memory_order_acquire) && ani_now_ms() < deadline) {
        diag_sleep_ms(DIAG_WAIT_SLICE_MS);
    }
    ...
    if (completed) {
        ...
        (void)ani_thread_detach(&g_diag_thread);
