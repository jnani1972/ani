...
static ani_mutex_t g_profile_mutex;
static profile_site_t g_sites[PROFILE_SITES];
static profile_pointer_t g_pointers[PROFILE_POINTERS];
static ani_mem_profile_totals_t g_totals;
static atomic_int g_profile_enabled = -1;
static size_t g_profile_min = PROFILE_DEFAULT_MIN;

size_t ani_mem_profile_threshold(void) {
    (void)ani_mem_profile_enabled();
    return g_profile_min;
}

bool ani_mem_profile_enabled(void) {
    int state = atomic_load_explicit(&g_profile_enabled, memory_order_acquire);
    if (state >= 0) {
        return state == 1;
    }
    char buf[ANI_SZ_32];
    bool on = ani_safe_getenv("ANI_MEM_PROFILE", buf, sizeof(buf), NULL) != NULL && buf[0] == '1';
    if (on) {
        char min_buf[ANI_SZ_32];
        if (ani_safe_getenv("ANI_MEM_PROFILE_MIN", min_buf, sizeof(min_buf), NULL) != NULL) {
            long parsed = strtol(min_buf, NULL, 10);
            if (parsed > 0) {
                g_profile_min = (size_t)parsed;
            }
        }
        ani_mutex_init(&g_profile_mutex);
    }
...
void ani_mem_profile_alloc(void *block, size_t size) {
    ani_mem_profile_alloc_at(block, size, NULL);
}

void ani_mem_profile_alloc_at(void *block, size_t size, void *caller) {
    if (!block || g_profile_reentrant) {
        return;
    }
    g_profile_reentrant = true;
    if (!ani_mem_profile_enabled() || size < g_profile_min) {
    ...
    if (count <= 0) {
        ani_mutex_lock(&g_profile_mutex);
        g_totals.capture_failed++;
        ani_mutex_unlock(&g_profile_mutex);
        g_profile_reentrant = false;
        return;
    }
    ani_mutex_lock(&g_profile_mutex);
...
void ani_mem_profile_free(void *block) {
    if (!block || g_profile_reentrant) {
        return;
    }
    g_profile_reentrant = true;
    if (!ani_mem_profile_enabled()) {
        g_profile_reentrant = false;
        return;
    }
    ani_mutex_lock(&g_profile_mutex);
...
void ani_mem_profile_totals(ani_mem_profile_totals_t *out) {
    if (!out) {
        return;
    }
...
bool ani_mem_profile_dump(const char *path, const char *label) {
    if (!path || !ani_mem_profile_enabled()) {
        return false;
    }
    FILE *out = ani_fopen(path, "ab");
    if (!out) {
        return false;
    }
    ani_mutex_lock(&g_profile_mutex);
