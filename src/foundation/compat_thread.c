...
#include "foundation/sanitized.h" /* ANI_SANITIZED — diagnostic stack floor */
...
#define ANI_DEFAULT_STACK_SIZE ((size_t)8 * ANI_SZ_1K * ANI_SZ_1K)
...
static size_t ani_thread_stack_floor(size_t requested) {
#if ANI_SANITIZED
    const char *env = getenv("ANI_THREAD_STACK_MB");
    if (env && env[0]) {
        char *end = NULL;
        unsigned long mb = strtoul(env, &end, 10);
        if (end && *end == '\0' && mb > 0 && mb <= 1024) {
            size_t floor_bytes = (size_t)mb * ANI_SZ_1K * ANI_SZ_1K;
...
static size_t ani_thread_default_stack_size(void) {
    return ani_thread_stack_floor(ANI_DEFAULT_STACK_SIZE);
}
 ...
 *
 * ANI_MI_THREAD_DONE=0 disables it, so one binary can demonstrate both
...
static bool thread_release_heap_enabled(void) {
    ...
    if (state < 0) {
        char buf[ANI_SZ_16];
        state =
            (ani_safe_getenv("ANI_MI_THREAD_DONE", buf, sizeof(buf), NULL) != NULL && buf[0] == '0')
                ? 0
                : 1;
    }
    return state == 1;
...
static void NTAPI ani_thread_detach_callback(PVOID handle, DWORD reason, PVOID reserved) {
    (void)handle;
    (void)reserved;
...
static const void *const ani_tls_anchor __attribute__((used)) = &_tls_used;
__attribute__((section(".CRT$XLB"), used)) PIMAGE_TLS_CALLBACK ani_thread_detach_tls_cb =
    ani_thread_detach_callback;
...
int ani_thread_create(ani_thread_t *t, size_t stack_size, void *(*fn)(void *), void *arg) {
    if (stack_size == 0) {
        stack_size = ani_thread_default_stack_size();
    } else {
        stack_size = ani_thread_stack_floor(stack_size);
    }
