...
static void check_pressure(size_t allocated) {
    ...
    if (over && !was) {
        ...
        snprintf(pct_str, sizeof(pct_str), "%zu",
                 g_budget > 0 ? (allocated * ANI_PERCENT) / g_budget : 0);
        ani_log_warn("mem.pressure.warn", "allocated_mb", alloc_mb, "budget_mb", budget_mb, "pct",
                     pct_str);
    } else if (!over && was) {
        ...
        snprintf(pct_str, sizeof(pct_str), "%zu",
                 g_budget > 0 ? (allocated * ANI_PERCENT) / g_budget : 0);
        ani_log_info("mem.pressure.ok", "allocated_mb", alloc_mb, "budget_mb", budget_mb, "pct",
                     pct_str);
    }
}
...
void ani_vmem_init(double ram_fraction) {
    ...
    ani_log_info("vmem.init", "budget_mb", budget_mb, "total_ram_mb", ram_mb);
}

void *ani_vmem_alloc(size_t size) {
    if (size == 0) {
        return NULL;
    }
    ...
    if (!ptr) {
        ani_log_error("vmem.alloc.fail", "size_mb", size > MB_DIVISOR ? "large" : "small");
        return NULL;
    }
...
void ani_vmem_free(void *ptr, size_t size) {
    if (!ptr || size == 0) {
        return;
    }
...
size_t ani_vmem_allocated(void) {
    return atomic_load(&g_allocated);
}

size_t ani_vmem_peak(void) {
    return atomic_load(&g_peak);
}

size_t ani_vmem_budget(void) {
    return g_budget;
}

bool ani_vmem_over_budget(void) {
    return atomic_load(&g_allocated) > g_budget;
}

size_t ani_vmem_worker_budget(int num_workers) {
    if (num_workers <= 0) {
