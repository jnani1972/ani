 ...
 */
#ifndef ANI_STR_UTIL_H
#define ANI_STR_UTIL_H
 ...
 * Everything ani_validate_shell_arg rejects, plus the cmd.exe expansion
 ...
 *
 * Use this — not ani_validate_shell_arg — for every path that crosses into a
 ...
 *
 * Usage: ANI_SNPRINTF_APPEND(buf, sizeof(buf), off, "fmt %s", arg);
 * Requires: <stdio.h> included by caller. */
#define ANI_SNPRINTF_APPEND(buf, sz, off, ...)                                       \
    do {                                                                             \
        if ((off) >= 0 && (off) < (int)(sz)) {                                       \
            int _ani_r = snprintf((buf) + (off), (sz) - (size_t)(off), __VA_ARGS__); \
            if (_ani_r > 0)                                                          \
                (off) += _ani_r;                                                     \
            if ((off) >= (int)(sz))                                                  \
                (off) = (int)(sz) - 1;                                               \
