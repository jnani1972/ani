...
static _Atomic ANILogLevel g_log_level = ANI_LOG_INFO;
static _Atomic ANILogFormat g_log_format = ANI_LOG_FORMAT_TEXT;
...
static _Atomic ani_log_sink_fn g_log_sink = (ani_log_sink_fn)NULL;
static _Atomic ANILogSinkMode g_log_sink_mode = ANI_LOG_SINK_REPLACE;

/* See ani_log_set_crash_durable in log.h. Read on every emitted line, so it
 * follows the same relaxed-atomic discipline as the four above. */
static _Atomic bool g_log_crash_durable = false;

void ani_log_set_crash_durable(bool enabled) {
    if (enabled) {
        ...
        if (raw[i] == '\0') { /* fully consumed — candidate textual match */
            for (size_t lvl = 0; lvl < sizeof(names) / sizeof(names[0]); lvl++) {
                if (strcmp(lower, names[lvl]) == 0) {
                    ani_log_set_level((ANILogLevel)lvl);
                    goto parse_format;
                }
            }
        }

        /* Numeric form: 0=debug .. 4=none, matching ANILogLevel. */
        char *end = NULL;
        long n = strtol(raw, &end, ANI_DECIMAL_BASE);
        if (end != raw && *end == '\0' && n >= ANI_LOG_DEBUG && n <= ANI_LOG_NONE) {
            ani_log_set_level((ANILogLevel)n);
        }
    }
...
parse_format:;
    const char *fmt = getenv("ANI_LOG_FORMAT");
    if (fmt && fmt[0] != '\0') {
        char lower_fmt[8];
        size_t i = 0;
...
void ani_log_set_sink(ani_log_sink_fn fn) {
    ani_log_set_sink_ex(fn, ANI_LOG_SINK_REPLACE);
}

void ani_log_set_sink_ex(ani_log_sink_fn fn, ANILogSinkMode mode) {
...
static void emit_line(const char *line) {
     ...
     * let a concurrent ani_log_set_sink turn a checked pointer into a NULL
     * call. */
    ani_log_sink_fn sink = atomic_load_explicit(&g_log_sink, memory_order_relaxed);
    if (sink) {
        sink(line);
        if (atomic_load_explicit(&g_log_sink_mode, memory_order_relaxed) == ANI_LOG_SINK_REPLACE) {
            return;
        }
    }
...
void ani_log(ANILogLevel level, const char *msg, ...) {
    if (level < atomic_load_explicit(&g_log_level, memory_order_relaxed)) {
        return;
    }

    char line_buf[ANI_SZ_4K];
    size_t pos = 0;
    va_list args;
    va_start(args, msg);

    if (atomic_load_explicit(&g_log_format, memory_order_relaxed) == ANI_LOG_FORMAT_JSON) {
...
void ani_log_control_record(const char *msg, ...) {
    /* No threshold check: a control record's whole purpose is surviving
     * ANI_LOG_LEVEL suppression. Always JSON, independent of ANI_LOG_FORMAT,
...
void ani_log_int(ANILogLevel level, const char *msg, const char *key, int64_t value) {
    char value_buf[ANI_SZ_32];
    snprintf(value_buf, sizeof(value_buf), "%" PRId64, value);
    ani_log(level, msg, key ? key : "?", value_buf, NULL);
}
...
void ani_log_mcp_request(const char *method, const char *tool_name, bool is_error,
                         int64_t duration_us) {
    char duration_ms[ANI_SZ_32];
    snprintf(duration_ms, sizeof(duration_ms), "%" PRId64, duration_us / 1000);
    if (tool_name && tool_name[0] != '\0') {
        ani_log(is_error ? ANI_LOG_WARN : ANI_LOG_INFO, "mcp.request", "protocol", "jsonrpc",
...
void ani_log_http_request(const char *component, const char *method, const char *path, int status,
                          int64_t duration_ms, size_t request_bytes, size_t response_bytes) {
    char safe_path[ANI_SZ_1K];
    char status_buf[ANI_SZ_16];
    char duration_buf[ANI_SZ_32];
    char request_buf[ANI_SZ_32];
    char response_buf[ANI_SZ_32];
    ...
    ANILogLevel level = ANI_LOG_INFO;
    if (status >= 500) {
        level = ANI_LOG_ERROR;
    } else if (status >= 400) {
        level = ANI_LOG_WARN;
    ...
    ani_log(level, "http.request", "component", component ? component : "", "method",
