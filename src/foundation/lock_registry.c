...
struct ani_lock_registry {
    ani_private_lock_directory_t *directory;
    ani_private_fork_condition_t *condition;
    ani_mutex_t mutex;
    ...
    uint64_t owner_pid;
    bool closing;
    ani_lock_registry_stage_hook_fn stage_hook;
    void *stage_context;
    ani_lock_registry_release_handle_t test_release_fault_handle;
    ani_private_file_lock_release_step_t test_release_fault_step;
    bool test_release_fault_armed;
    ani_lock_registry_abort_failure_t test_abort_failure;
    bool test_abort_failure_armed;
    atomic_uint_fast64_t test_condition_wait_calls;
    atomic_size_t test_condition_waiters_now;
    struct ani_lock_registry *next_live;
    struct ani_lock_registry *next_retired;
};

struct ani_lock_lease {
    ani_lock_registry_t *registry;
    lock_registry_entry_t *entry;
    lock_registry_waiter_t waiter;
    ani_private_file_lock_mode_t mode;
    ani_private_file_lock_t *turn;
    ani_private_file_lock_t *rw;
    uint64_t owner_pid;
    bool active;
    bool cleanup_only;
...
/* Protected by ani_private_file_lock_fork_guard_enter(). Besides serializing
 ...
 * freed mutex. */
static ani_lock_registry_t *lock_registry_live;
...
static ani_lock_registry_t *lock_registry_retired;
...
static bool lock_registry_is_live_unlocked(const ani_lock_registry_t *registry) {
    for (const ani_lock_registry_t *cursor = lock_registry_live; cursor;
...
static bool lock_registry_lock(ani_lock_registry_t *registry) {
    if (!registry || !ani_private_file_lock_fork_guard_enter()) {
        return false;
    }
    if (!lock_registry_is_live_unlocked(registry)) {
        ani_private_file_lock_fork_guard_leave();
        return false;
    }
    ani_mutex_lock(&registry->mutex);
    if (registry->closing || registry->owner_pid != lock_registry_current_pid()) {
        ani_mutex_unlock(&registry->mutex);
        ani_private_file_lock_fork_guard_leave();
        return false;
    }
    return true;
...
static void lock_registry_unlock(ani_lock_registry_t *registry) {
    ani_mutex_unlock(&registry->mutex);
    ani_private_file_lock_fork_guard_leave();
}

/* Requires the global fork guard and registry mutex, in that order. */
static void lock_registry_broadcast_locked(ani_lock_registry_t *registry) {
    ani_private_fork_condition_broadcast_while_guarded(registry->condition);
}
...
static ani_private_fork_wait_status_t lock_registry_wait_locked(ani_lock_registry_t *registry,
    ...
    ani_mutex_unlock(&registry->mutex);
    ani_private_fork_wait_status_t status =
        ani_private_fork_condition_wait_until_while_guarded(registry->condition, deadline_ms);
    ani_mutex_lock(&registry->mutex);
...
static bool lock_registry_park_native(ani_lock_registry_t *registry, uint64_t deadline_ms,
                                      const ani_lock_cancel_token_t *cancel_token) {
    if (!lock_registry_lock(registry)) {
        return false;
    }
    ani_private_fork_wait_status_t status = ANI_PRIVATE_FORK_WAIT_SIGNALED;
    ...
    return status != ANI_PRIVATE_FORK_WAIT_ERROR;
}
...
static bool lock_registry_should_stop(uint64_t deadline_ms,
    ...
    return deadline_ms != UINT64_MAX && ani_now_ms() >= deadline_ms;
}

static void lock_registry_notify(ani_lock_registry_t *registry, ani_private_file_lock_mode_t mode,
                                 ani_lock_registry_stage_t stage) {
    if (!lock_registry_lock(registry)) {
        return;
    }
    ani_lock_registry_stage_hook_fn hook = registry->stage_hook;
    if (!hook) {
        lock_registry_unlock(registry);
        return;
    }
...
static lock_registry_entry_t *lock_registry_find_entry(ani_lock_registry_t *registry,
...
static void lock_registry_waiter_push(ani_lock_registry_t *registry, lock_registry_entry_t *entry,
                                      lock_registry_waiter_t *waiter) {
    waiter->next = NULL;
    waiter->queued = true;
...
static bool lock_registry_waiter_remove(ani_lock_registry_t *registry, lock_registry_entry_t *entry,
                                        lock_registry_waiter_t *waiter) {
    if (!waiter->queued) {
        return false;
    }
...
static void lock_registry_apply_release_fault(ani_lock_registry_t *registry,
    ...
    ani_private_file_lock_release_step_t step = registry->test_release_fault_step;
    if (inject) {
        registry->test_release_fault_armed = false;
    }
    lock_registry_unlock(registry);
    if (inject) {
        (void)ani_private_file_lock_fail_next_release_step_for_test(lock, step);
    }
}

/* Rollback and writer release are deliberately rw-before-turn. */
static ani_private_file_lock_status_t lock_registry_release_native(
    ani_lock_registry_t *registry, ani_private_file_lock_t **rw_io,
    ani_private_file_lock_t **turn_io) {
    ani_private_file_lock_status_t result = ANI_PRIVATE_FILE_LOCK_OK;
    lock_registry_apply_release_fault(registry, ANI_LOCK_REGISTRY_RELEASE_RW, *rw_io);
    if (*rw_io && ani_private_file_lock_release(rw_io) != ANI_PRIVATE_FILE_LOCK_OK) {
        result = ANI_PRIVATE_FILE_LOCK_IO;
        if (*rw_io && !ani_private_file_lock_unlock_complete(*rw_io)) {
            return result;
        }
    }
    lock_registry_apply_release_fault(registry, ANI_LOCK_REGISTRY_RELEASE_TURN, *turn_io);
    if (*turn_io && ani_private_file_lock_release(turn_io) != ANI_PRIVATE_FILE_LOCK_OK) {
        result = ANI_PRIVATE_FILE_LOCK_IO;
    }
    return result;
}
...
bool ani_lock_registry_resource_names(const char *resource_key,
    ...
    char digest[ANI_SHA256_HEX_LEN + 1];
    ani_sha256_hex(resource_key, length, digest);
    int turn_written = snprintf(turn_out, ANI_LOCK_REGISTRY_NAME_CAP, "ani-%s.turn", digest);
    int rw_written = snprintf(rw_out, ANI_LOCK_REGISTRY_NAME_CAP, "ani-%s.rw", digest);
    return turn_written > 0 && turn_written < (int)ANI_LOCK_REGISTRY_NAME_CAP && rw_written > 0 &&
           rw_written < (int)ANI_LOCK_REGISTRY_NAME_CAP;
}
...
ani_lock_registry_t *ani_lock_registry_new(ani_private_lock_directory_t *directory) {
    ...
    ani_lock_registry_t *registry = calloc(1, sizeof(*registry));
    if (!registry) {
        return NULL;
    }
    registry->directory = directory;
    registry->owner_pid = lock_registry_current_pid();
    ani_mutex_init(&registry->mutex);
    registry->condition = ani_private_fork_condition_new();
    if (!registry->condition) {
        ani_mutex_destroy(&registry->mutex);
        free(registry);
        return NULL;
    }
    if (!ani_private_file_lock_fork_guard_enter()) {
        ani_private_fork_condition_free(registry->condition);
        ani_mutex_destroy(&registry->mutex);
        free(registry);
        return NULL;
    }
...
static void lock_registry_retain_waiter_cleanup(ani_lock_lease_t *lease,
                                                ani_private_file_lock_t **rw_io,
                                                ani_private_file_lock_t **turn_io,
...
static bool lock_registry_abort_lock(ani_lock_registry_t *registry) {
    if (!lock_registry_lock(registry)) {
        return false;
    }
    bool fail_lock = registry->test_abort_failure_armed &&
                     registry->test_abort_failure == ANI_LOCK_REGISTRY_ABORT_FAIL_LOCK;
    if (fail_lock) {
...
static ani_private_file_lock_status_t lock_registry_abort_attempt(
    ani_lock_registry_t *registry, lock_registry_entry_t *entry, lock_registry_waiter_t *waiter,
    ani_private_file_lock_t **rw_io, ani_private_file_lock_t **turn_io,
    ani_private_file_lock_status_t requested_status, ani_lock_lease_t *lease,
    ani_lock_lease_t **lease_out) {
    ...
    bool fail_remove = registry->test_abort_failure_armed &&
                       registry->test_abort_failure == ANI_LOCK_REGISTRY_ABORT_FAIL_REMOVE;
    if (fail_remove) {
        registry->test_abort_failure_armed = false;
    }
    ...
    if (!has_native) {
        free(lease);
        return owns_attempt ? requested_status : ANI_PRIVATE_FILE_LOCK_IO;
    }
    ani_lock_lease_t *cleanup = lease;
    if (ani_lock_lease_release(&cleanup) != ANI_PRIVATE_FILE_LOCK_OK) {
        *lease_out = cleanup;
        return ANI_PRIVATE_FILE_LOCK_IO;
    }
    return owns_attempt ? requested_status : ANI_PRIVATE_FILE_LOCK_IO;
...
static ani_private_file_lock_status_t lock_registry_attempt_native(
    ani_lock_registry_t *registry, lock_registry_entry_t *entry, lock_registry_waiter_t *waiter,
    uint64_t deadline_ms, const ani_lock_cancel_token_t *cancel_token, bool try_once,
    ani_lock_lease_t *lease, ani_lock_lease_t **lease_out) {
    ani_private_file_lock_t *turn = NULL;
    ani_private_file_lock_t *rw = NULL;
    ...
    while (!rw) {
        ...
        if (!turn) {
            ani_private_file_lock_mode_t turn_mode = waiter->mode == ANI_PRIVATE_FILE_LOCK_SH
                                                         ? ANI_PRIVATE_FILE_LOCK_EX
                                                         : ANI_PRIVATE_FILE_LOCK_SH;
            status = ani_private_file_lock_try_acquire(registry->directory, entry->turn_name,
                                                       turn_mode, &turn);
            if (status == ANI_PRIVATE_FILE_LOCK_BUSY) {
                lock_registry_notify(registry, waiter->mode, ANI_LOCK_REGISTRY_STAGE_TURN_BUSY);
                if (try_once) {
                    break;
                }
            ...
            lock_registry_notify(registry, waiter->mode, ANI_LOCK_REGISTRY_STAGE_TURN_HELD);
        }
        ...
        status = ani_private_file_lock_try_acquire(registry->directory, entry->rw_name,
                                                   waiter->mode, &rw);
        if (status == ANI_PRIVATE_FILE_LOCK_BUSY) {
            lock_registry_notify(registry, waiter->mode, ANI_LOCK_REGISTRY_STAGE_RW_BUSY);
            if (try_once) {
                break;
            }
            if (!lock_registry_park_native(registry, deadline_ms, cancel_token)) {
        ...
        if (waiter->mode == ANI_PRIVATE_FILE_LOCK_SH) {
            status = ani_private_file_lock_release(&turn);
            if (status != ANI_PRIVATE_FILE_LOCK_OK) {
                break;
            }
        }
        lock_registry_notify(registry, waiter->mode, ANI_LOCK_REGISTRY_STAGE_NATIVE_READY);
    }

    if (!rw || status != ANI_PRIVATE_FILE_LOCK_OK) {
    ...
    for (;;) {
        if (!try_once && lock_registry_should_stop(deadline_ms, cancel_token)) {
            return lock_registry_abort_attempt(registry, entry, waiter, &rw, &turn,
                                               ANI_PRIVATE_FILE_LOCK_BUSY, lease, lease_out);
        }
        if (!lock_registry_lock(registry)) {
            return lock_registry_abort_attempt(registry, entry, waiter, &rw, &turn,
                                               ANI_PRIVATE_FILE_LOCK_IO, lease, lease_out);
        }
        bool owns_attempt = entry->attempt_waiter == waiter && entry->waiter_head == waiter;
        bool local_state_ready = waiter->mode == ANI_PRIVATE_FILE_LOCK_SH
                                     ? !entry->writer_active
        ...
        ani_private_fork_wait_status_t wait_status = ANI_PRIVATE_FORK_WAIT_SIGNALED;
        if (removed) {
            entry->attempt_waiter = NULL;
            if (waiter->mode == ANI_PRIVATE_FILE_LOCK_SH) {
        ...
        if (wait_status == ANI_PRIVATE_FORK_WAIT_ERROR) {
            return lock_registry_abort_attempt(registry, entry, waiter, &rw, &turn,
                                               ANI_PRIVATE_FILE_LOCK_IO, lease, lease_out);
        }
    }
...
static ani_private_file_lock_status_t lock_registry_acquire_internal(
    ...
    if (mode != ANI_PRIVATE_FILE_LOCK_SH && mode != ANI_PRIVATE_FILE_LOCK_EX) {
        return ANI_PRIVATE_FILE_LOCK_UNSAFE;
    }
    char turn_name[ANI_LOCK_REGISTRY_NAME_CAP];
    char rw_name[ANI_LOCK_REGISTRY_NAME_CAP];
    if (!ani_lock_registry_resource_names(resource_key, turn_name, rw_name)) {
        return ANI_PRIVATE_FILE_LOCK_UNSAFE;
    }
    if (!try_once && lock_registry_should_stop(deadline_ms, cancel_token)) {
        return ANI_PRIVATE_FILE_LOCK_BUSY;
    }

    lock_registry_entry_t *candidate = calloc(1, sizeof(*candidate));
    ani_lock_lease_t *lease = calloc(1, sizeof(*lease));
    if (!candidate || !lease) {
        free(candidate);
        free(lease);
        return ANI_PRIVATE_FILE_LOCK_IO;
...
static ani_private_file_lock_status_t lock_registry_cleanup_lease_release(
    ani_lock_lease_t **lease_io) {
    ani_lock_lease_t *lease = *lease_io;
    ani_lock_registry_t *registry = lease->registry;
...
ani_private_file_lock_status_t ani_lock_lease_release(ani_lock_lease_t **lease_io) {
    ...
    } else if (lease->mode == ANI_PRIVATE_FILE_LOCK_SH) {
        native_state_valid = lease->rw && !lease->turn;
    } else if (lease->mode == ANI_PRIVATE_FILE_LOCK_EX) {
    ...
    if (valid && lease->mode == ANI_PRIVATE_FILE_LOCK_SH) {
        valid = entry->active_readers > 0;
    } else if (valid && lease->mode == ANI_PRIVATE_FILE_LOCK_EX) {
    ...
    if (!lease->critical_released) {
        ...
        if (lease->rw && !ani_private_file_lock_unlock_complete(lease->rw)) {
            return status;
        }
    ...
    if (lease->mode == ANI_PRIVATE_FILE_LOCK_SH) {
        entry->active_readers--;
    } else {
    ...
    ani_private_file_lock_status_t result =
        lease->release_error ? ANI_PRIVATE_FILE_LOCK_IO : ANI_PRIVATE_FILE_LOCK_OK;
    *lease_io = NULL;
    free(lease);
    return result;
}
...
ani_private_file_lock_status_t ani_lock_registry_free(ani_lock_registry_t **registry_io) {
    if (!registry_io || !*registry_io || !ani_private_file_lock_fork_guard_enter()) {
        return ANI_PRIVATE_FILE_LOCK_IO;
    }
    ani_lock_registry_t *registry = *registry_io;
    ani_lock_registry_t **live_cursor = &lock_registry_live;
...
bool ani_lock_registry_is_retired_for_test(const ani_lock_registry_t *registry) {
    ...
    bool retired = false;
    for (const ani_lock_registry_t *cursor = lock_registry_retired; cursor;
...
bool ani_lock_lease_fail_next_release_step_for_test(ani_lock_lease_t *lease,
    ...
    ani_private_file_lock_t *lock = handle == ANI_LOCK_REGISTRY_RELEASE_RW     ? lease->rw
                                    : handle == ANI_LOCK_REGISTRY_RELEASE_TURN ? lease->turn
                                                                               : NULL;
    return ani_private_file_lock_fail_next_release_step_for_test(lock, step);
}
...
bool ani_lock_lease_has_release_handle_for_test(const ani_lock_lease_t *lease,
    ...
    if (handle == ANI_LOCK_REGISTRY_RELEASE_RW) {
        return lease->rw != NULL;
    }
    if (handle == ANI_LOCK_REGISTRY_RELEASE_TURN) {
...
bool ani_lock_lease_fail_close_after_consuming_for_test(ani_lock_lease_t *lease,
    ...
    ani_private_file_lock_t *lock = handle == ANI_LOCK_REGISTRY_RELEASE_RW     ? lease->rw
                                    : handle == ANI_LOCK_REGISTRY_RELEASE_TURN ? lease->turn
                                                                               : NULL;
    return ani_private_file_lock_fail_close_after_consuming_for_test(lock);
}
#endif
...
bool ani_lock_registry_fail_next_native_release_step_for_test(
    ...
    ani_private_file_lock_release_step_t step) {
    if ((handle != ANI_LOCK_REGISTRY_RELEASE_RW && handle != ANI_LOCK_REGISTRY_RELEASE_TURN) ||
        (step != ANI_PRIVATE_FILE_LOCK_RELEASE_UNLOCK &&
         step != ANI_PRIVATE_FILE_LOCK_RELEASE_CLOSE) ||
        !lock_registry_lock(registry)) {
        return false;
    }
...
bool ani_lock_registry_fail_next_abort_bookkeeping_for_test(
    ...
    if ((failure != ANI_LOCK_REGISTRY_ABORT_FAIL_LOCK &&
         failure != ANI_LOCK_REGISTRY_ABORT_FAIL_REMOVE) ||
        !lock_registry_lock(registry)) {
        return false;
    }
    bool idle = registry->waiter_count == 0 && registry->active_lease_count == 0 &&
...
bool ani_lock_registry_set_stage_hook_for_test(ani_lock_registry_t *registry,
                                               ani_lock_registry_stage_hook_fn hook,
                                               void *context) {
    if (!lock_registry_lock(registry)) {
        return false;
    }
