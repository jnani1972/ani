 ...
 *
 * Restructured from internal/ani/arena.c with additions:
 *   - ani_arena_init_sized() for custom block sizes
 *   - ani_arena_calloc() for zero-initialized allocations
 *   - ani_arena_reset() for reuse without full destroy
 *   - ani_arena_total() for allocation tracking
 *
...
void ani_arena_init(ANIArena *a) {
    ani_arena_init_sized(a, ANI_ARENA_DEFAULT_BLOCK_SIZE);
}

void ani_arena_init_sized(ANIArena *a, size_t block_size) {
    memset(a, 0, sizeof(*a));
    if (block_size < ANI_SZ_64) {
        block_size = ANI_SZ_64; /* minimum sanity */
    }
...
static int arena_grow(ANIArena *a, size_t min_size) {
    if (a->nblocks >= ANI_ARENA_MAX_BLOCKS) {
        return 0;
    }
...
char *ani_arena_strdup(ANIArena *a, const char *s) {
    ...
    size_t len = strlen(s);
    char *dst = (char *)ani_arena_alloc(a, len + SKIP_ONE);
    if (dst) {
...
char *ani_arena_sprintf(ANIArena *a, const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    ...
    char *dst = (char *)ani_arena_alloc(a, (size_t)needed + SKIP_ONE);
    if (!dst) {
        return NULL;
    }
...
void ani_arena_reset(ANIArena *a) {
     ...
     * block_size grew during previous allocations (e.g., ANI_SZ_128 → ANI_SZ_256). */
    if (a->nblocks == SKIP_ONE) {
