 ...
 */
#ifndef ANI_CONSTANTS_H
#define ANI_CONSTANTS_H

/* ── Allocation counts ───────────────────────────────────────── */
enum { ANI_ALLOC_ONE = 1 }; /* calloc(ANI_ALLOC_ONE, sizeof(T)) */

/* ── Byte / character constants ──────────────────────────────── */
enum {
    ANI_BYTE_RANGE = 256, /* full byte range 0x00–0xFF */
    ANI_QUOTE_PAIR = 2,   /* two quote characters (open + close) */
    ANI_QUOTE_OFFSET = 1, /* skip opening quote */
};
...
/* ── Sentinel values ─────────────────────────────────────────── */
enum {
    ANI_NOT_FOUND = -1, /* search miss, invalid index */
    ANI_INIT_DONE = 1,  /* initialization flag */
};
...
enum { ANI_DEFAULT_SEARCH_LIMIT = 50 };

/* ── Time conversion factors ─────────────────────────────────── */
#define ANI_NSEC_PER_SEC 1000000000ULL
#define ANI_USEC_PER_SEC 1000000ULL
#define ANI_MSEC_PER_SEC 1000ULL
#define ANI_NSEC_PER_USEC 1000ULL
#define ANI_NSEC_PER_MSEC 1000000ULL

/* ── Common string/buffer sizes ──────────────────────────────── */
enum {
    ANI_SMALL_BUF = 3,   /* small scratch buffers */
    ANI_NAME_BUF = 4,    /* name buffer slots */
    ANI_PATH_MAX = 1024, /* path buffer size */
    ANI_LINE_BUF = 512,  /* line read buffer */
};
 ...
 * SQL mirror of ani_label_is_type_like() (internal/ani/helpers.c). That
 ...
 * them against ani_label_is_type_like(), so adding a type-like label there
 * without updating these fails CI rather than quietly shrinking query results. */
#define ANI_SQL_TYPE_LIKE_LABELS "'Class','Struct','Interface','Enum','Type','Trait'"
#define ANI_SQL_CALLABLE_LABELS "'Function','Method'"
#define ANI_SQL_CALLABLE_OR_TYPE_LABELS ANI_SQL_CALLABLE_LABELS "," ANI_SQL_TYPE_LIKE_LABELS
/* SQL mirror of ani_label_is_relation() (Table/View/Model — data-lineage
 * nodes), pinned by tests/test_store_nodes.c the same way as the sets above. */
#define ANI_SQL_RELATION_LABELS "'Table','View','Model'"

#endif /* ANI_CONSTANTS_H */
