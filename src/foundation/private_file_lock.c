...
struct ani_private_lock_directory {
    int fd;
    char *path;
    dev_t device;
    ino_t inode;
...
struct ani_private_file_lock {
    int fd;
    pid_t owner_pid;
    ani_private_file_lock_mode_t mode;
    struct ani_private_file_lock *next_tracked;
    bool unlocked;
...
struct ani_private_fork_condition {
...
static ani_private_file_lock_t *private_tracked_locks;
...
static void private_atfork_child(void) {
    ani_private_file_lock_t *lock = private_tracked_locks;
    while (lock) {
        if (lock->fd >= 0) {
...
ani_private_fork_condition_t *ani_private_fork_condition_new(void) {
    ani_private_fork_condition_t *condition = calloc(1, sizeof(*condition));
...
ani_private_fork_wait_status_t ani_private_fork_condition_wait_until_while_guarded(
    ...
    if (!condition) {
        return ANI_PRIVATE_FORK_WAIT_ERROR;
    }
    int status;
    ...
    if (status == 0) {
        return ANI_PRIVATE_FORK_WAIT_SIGNALED;
    }
    return status == ETIMEDOUT ? ANI_PRIVATE_FORK_WAIT_TIMEOUT : ANI_PRIVATE_FORK_WAIT_ERROR;
}
...
static bool private_directory_revalidate(const ani_private_lock_directory_t *directory) {
    ...
    return directory && directory->owner_pid == getpid() && directory->fd >= 0 &&
           ...
           ani_macos_extended_acl_fd_is_empty(directory->fd);
}
...
static bool private_file_revalidate(const ani_private_lock_directory_t *directory,
    ...
    return private_directory_revalidate(directory) && fstat(fd, &by_handle) == 0 &&
           ...
           by_path.st_ino == by_handle.st_ino && ani_macos_extended_acl_fd_is_empty(fd) &&
           (!expected ||
            (expected->st_dev == by_handle.st_dev && expected->st_ino == by_handle.st_ino));
}

static ani_private_file_lock_status_t private_open_failure_status(
    const ani_private_lock_directory_t *directory, const char *base_name) {
    struct stat status;
    if (fstatat(directory->fd, base_name, &status, AT_SYMLINK_NOFOLLOW) == 0) {
        return ANI_PRIVATE_FILE_LOCK_UNSAFE;
    }
    return errno == ENOENT ? ANI_PRIVATE_FILE_LOCK_IO : ANI_PRIVATE_FILE_LOCK_UNSAFE;
}
...
static int private_release_unlock(ani_private_file_lock_t *lock) {
...
static int private_release_close(ani_private_file_lock_t *lock, bool *attempted_out) {
    *attempted_out = false;
...
ani_private_file_lock_status_t ani_private_lock_directory_adopt_posix(
    ...
    if (fstat(directory_fd, &status) != 0 || lstat(stable_path, &by_path) != 0 ||
        ...
        status.st_ino != by_path.st_ino || !ani_macos_extended_acl_fd_is_empty(directory_fd)) {
        return ANI_PRIVATE_FILE_LOCK_UNSAFE;
    }
    ani_private_lock_directory_t *directory = calloc(1, sizeof(*directory));
    if (directory) {
...
ani_private_file_lock_status_t ani_private_file_lock_try_acquire(
    ani_private_lock_directory_t *directory, const char *base_name,
    ani_private_file_lock_mode_t mode, ani_private_file_lock_t **lock_out) {
    if (lock_out) {
        *lock_out = NULL;
    }
    if (!directory || !lock_out || !private_base_name_valid(base_name) ||
        (mode != ANI_PRIVATE_FILE_LOCK_SH && mode != ANI_PRIVATE_FILE_LOCK_EX)) {
        return ANI_PRIVATE_FILE_LOCK_UNSAFE;
    }
    ...
    if (fd < 0) {
        ani_private_file_lock_status_t status = private_open_failure_status(directory, base_name);
    ...
    ani_private_file_lock_t *lock = calloc(1, sizeof(*lock));
    if (!lock) {
        (void)close(fd);
        ani_private_file_lock_fork_guard_leave();
        return ANI_PRIVATE_FILE_LOCK_IO;
    ...
    int operation = mode == ANI_PRIVATE_FILE_LOCK_SH ? LOCK_SH : LOCK_EX;
    if (private_flock_set(fd, operation | LOCK_NB) != 0) {
        int lock_error = errno;
        (void)close(fd);
        free(lock);
        ani_private_file_lock_fork_guard_leave();
        return lock_error == EWOULDBLOCK || lock_error == EAGAIN ? ANI_PRIVATE_FILE_LOCK_BUSY
    ...
    if (forced_cleanup || !private_file_revalidate(directory, base_name, fd, &initial)) {
        ani_private_file_lock_status_t failure_status =
            forced_cleanup ? ANI_PRIVATE_FILE_LOCK_IO : ANI_PRIVATE_FILE_LOCK_UNSAFE;
        lock->test_fail_unlock_once = fail_cleanup_unlock;
        lock->test_fail_close_once = fail_cleanup_close;
        *lock_out = lock;
        ani_private_file_lock_fork_guard_leave();
        ani_private_file_lock_status_t cleanup_status = ani_private_file_lock_release(lock_out);
        return cleanup_status == ANI_PRIVATE_FILE_LOCK_OK ? failure_status
                                                          : ANI_PRIVATE_FILE_LOCK_IO;
    }
    *lock_out = lock;
    ani_private_file_lock_fork_guard_leave();
...
static bool private_lock_is_tracked(const ani_private_file_lock_t *lock) {
    for (const ani_private_file_lock_t *cursor = private_tracked_locks; cursor;
...
static bool private_payload_fd_valid(const ani_private_file_lock_t *lock, struct stat *status_out) {
    struct stat status;
    bool valid = lock && lock->fd >= 0 && !lock->unlocked && lock->owner_pid == getpid() &&
                 ...
                 ani_macos_extended_acl_fd_is_empty(lock->fd);
    if (valid && status_out) {
        *status_out = status;
    }
    return valid;
...
ani_private_file_lock_status_t ani_private_file_lock_payload_write(ani_private_file_lock_t *lock,
    ...
    if (!lock || !buffer || length == 0 || length > PRIVATE_FILE_LOCK_PAYLOAD_CAP ||
        lock->mode != ANI_PRIVATE_FILE_LOCK_EX) {
        return ANI_PRIVATE_FILE_LOCK_UNSAFE;
    }
    if (!ani_private_file_lock_fork_guard_enter()) {
        return ANI_PRIVATE_FILE_LOCK_IO;
    ...
    return valid ? ANI_PRIVATE_FILE_LOCK_OK : ANI_PRIVATE_FILE_LOCK_IO;
}

ani_private_file_lock_status_t ani_private_file_lock_release(ani_private_file_lock_t **lock_io) {
    if (!lock_io || !*lock_io) {
        return ANI_PRIVATE_FILE_LOCK_IO;
    }
    ani_private_file_lock_t *lock = *lock_io;
    ...
    ani_private_file_lock_t **cursor = &private_tracked_locks;
    ...
    *lock_io = NULL;
    free(lock);
    return close_status == 0 ? ANI_PRIVATE_FILE_LOCK_OK : ANI_PRIVATE_FILE_LOCK_IO;
}

void ani_private_lock_directory_close(ani_private_lock_directory_t *directory) {
    if (!directory) {
        return;
    }
    if (directory->fd >= 0) {
...
const char *ani_private_lock_directory_path(const ani_private_lock_directory_t *directory) {
...
bool ani_private_file_lock_fail_next_release_step_for_test(
    ...
    if (step == ANI_PRIVATE_FILE_LOCK_RELEASE_UNLOCK) {
        lock->test_fail_unlock_once = true;
        return true;
    }
    if (step == ANI_PRIVATE_FILE_LOCK_RELEASE_CLOSE) {
...
static ani_private_file_lock_status_t private_win_path_from_utf8(const char *path,
                                                                 wchar_t **wide_out) {
    *wide_out = NULL;
...
static bool private_win_directory_handle_valid(ani_private_lock_directory_t *directory,
...
static bool private_win_directory_path_matches(ani_private_lock_directory_t *directory) {
    HANDLE probe =
...
static bool private_win_directory_revalidate(ani_private_lock_directory_t *directory) {
    BY_HANDLE_FILE_INFORMATION information;
...
static wchar_t *private_win_file_path(const ani_private_lock_directory_t *directory,
...
static bool private_win_file_handle_valid(ani_private_lock_directory_t *directory, HANDLE handle,
                                          BY_HANDLE_FILE_INFORMATION *information_out) {
    BY_HANDLE_FILE_INFORMATION information;
...
static bool private_win_file_revalidate(ani_private_lock_directory_t *directory,
...
static ani_private_file_lock_status_t private_win_open_failure_status(const wchar_t *path) {
    DWORD attributes = GetFileAttributesW(path);
    return attributes == INVALID_FILE_ATTRIBUTES ? ANI_PRIVATE_FILE_LOCK_IO
                                                 : ANI_PRIVATE_FILE_LOCK_UNSAFE;
}
...
ani_private_file_lock_status_t ani_private_lock_directory_adopt_windows(
    ...
    ani_private_file_lock_status_t path_status =
        private_win_path_from_utf8(stable_path, &wide_path);
    if (path_status != ANI_PRIVATE_FILE_LOCK_OK) {
        return path_status;
    }
    private_win_security_t security;
    ...
    ani_private_lock_directory_t candidate;
...
ani_private_file_lock_status_t ani_private_file_lock_try_acquire(
    ...
    if (handle == INVALID_HANDLE_VALUE) {
        ani_private_file_lock_status_t status = private_win_open_failure_status(path);
        ani_private_file_lock_fork_guard_leave();
        free(path);
        return status;
    }
    ...
    if (!native_locked) {
        ...
        *lock_out = lock;
        ani_private_file_lock_fork_guard_leave();
        free(path);
        ani_private_file_lock_status_t failure_status = lock_error == ERROR_LOCK_VIOLATION
                                                            ? ANI_PRIVATE_FILE_LOCK_BUSY
                                                            : ANI_PRIVATE_FILE_LOCK_IO;
        ani_private_file_lock_status_t cleanup_status = ani_private_file_lock_release(lock_out);
        return cleanup_status == ANI_PRIVATE_FILE_LOCK_OK ? failure_status
                                                          : ANI_PRIVATE_FILE_LOCK_IO;
...
static bool private_win_payload_handle_valid(const ani_private_file_lock_t *lock) {
    BY_HANDLE_FILE_INFORMATION information;
...
static bool private_win_release_unlock(ani_private_file_lock_t *lock) {
    lock->test_unlock_attempts++;
    if (lock->test_fail_unlock_once) {
        lock->test_fail_unlock_once = false;
...
static bool private_win_release_close(ani_private_file_lock_t *lock) {
    lock->test_close_attempts++;
    if (lock->test_fail_close_once) {
        lock->test_fail_close_once = false;
        SetLastError(ERROR_GEN_FAILURE);
...
ani_private_fork_wait_status_t ani_private_fork_condition_wait_until_while_guarded(
    ...
    return GetLastError() == ERROR_TIMEOUT ? ANI_PRIVATE_FORK_WAIT_TIMEOUT
                                           : ANI_PRIVATE_FORK_WAIT_ERROR;
}
