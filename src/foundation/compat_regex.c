...
_Static_assert(sizeof(regex_t) <= ANI_SZ_256,
               "ani_regex_t opaque buffer too small for TRE regex_t");

static int translate_flags_tre(int flags) {
    int tre_flags = 0;
    if (flags & ANI_REG_EXTENDED)
        tre_flags |= REG_EXTENDED;
    if (flags & ANI_REG_ICASE)
        tre_flags |= REG_ICASE;
    if (flags & ANI_REG_NOSUB)
        tre_flags |= REG_NOSUB;
    if (flags & ANI_REG_NEWLINE)
...
int ani_regexec(const ani_regex_t *r, const char *str, int nmatch, ani_regmatch_t *matches,
    ...
    if (nmatch <= 0 || !matches) {
        ...
        return rc == 0 ? ANI_REG_OK : ANI_REG_NOMATCH;
    }
    regmatch_t pmatch[ANI_SZ_32];
    int n = nmatch > ANI_SZ_32 ? ANI_SZ_32 : nmatch;
    int rc = tre_regexec(re, str, (size_t)n, pmatch, eflags);
    if (rc != 0)
        return ANI_REG_NOMATCH;
    for (int i = 0; i < n; i++) {
 ...
 * If this fires, increase ani_regex_t.opaque size. */
_Static_assert(sizeof(regex_t) <= ANI_SZ_256, "ani_regex_t opaque buffer too small for regex_t");
