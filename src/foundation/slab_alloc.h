 ...
 *   Matches tree-sitter SubtreeHeapData (ANI_SZ_64 bytes). Backed by
 ...
 *
 * Usage:
 *   ani_slab_install();         // once, before any parsing
 *   ... parse files ...
 *   ani_slab_destroy_thread();  // on thread exit — frees owned memory
 */
#ifndef ANI_SLAB_ALLOC_H
#define ANI_SLAB_ALLOC_H
 ...
 * Only safe after ani_destroy_thread_parser() has been called. */
void ani_slab_reset_thread(void);
...
void ani_slab_destroy_thread(void);
...
void ani_slab_reclaim(void);

/* Test/diagnostic API: direct access to the slab allocator.
 * Use these to unit test slab (≤64B) and heap (>64B) paths. */
void *ani_slab_test_malloc(size_t size);
void ani_slab_test_free(void *ptr);
void *ani_slab_test_realloc(void *ptr, size_t size);
void *ani_slab_test_calloc(size_t count, size_t size);

#endif /* ANI_SLAB_ALLOC_H */
