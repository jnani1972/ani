 ...
 */
#ifndef ANI_SUBPROCESS_H
#define ANI_SUBPROCESS_H

#include <stdbool.h>
#include <stddef.h> /* size_t (ani_build_win_cmdline) */

/* How a supervised child ended. */
typedef enum {
    ANI_PROC_CLEAN = 0,    /* exited with code 0 */
    ANI_PROC_EXIT_NONZERO, /* exited with a nonzero code (a graceful failure) */
    ANI_PROC_CRASH,        /* died from a fault: POSIX SIGSEGV/BUS/ILL/FPE/ABRT/SYS,
                            * or a Windows NTSTATUS exception exit code (>= 0xC0000000) */
    ANI_PROC_HANG,         /* made no progress within the quiet-timeout; we killed it */
    ANI_PROC_KILLED,       /* terminated by a non-fault signal we did not initiate */
    ANI_PROC_SPAWN_FAILED  /* fork/exec/CreateProcess failed — no child ever ran */
} ani_proc_outcome_t;

typedef struct {
    ani_proc_outcome_t outcome;
...
typedef void (*ani_proc_log_cb)(const char *line, void *ud);
...
typedef struct {
    ...
    ani_proc_log_cb on_log_line;     /* optional per-line callback */
    ...
    int cancel_grace_ms;             /* graceful tree-termination window; <= 0 uses the finite
                                      * ANI_SUBPROCESS_DEFAULT_CANCEL_GRACE_MS */
    bool delete_log_on_exit;         /* unlink log_file after reaping */
} ani_proc_opts_t;

#define ANI_SUBPROCESS_DEFAULT_CANCEL_GRACE_MS 1000
#define ANI_SUBPROCESS_MAX_CANCEL_GRACE_MS 1000
#define ANI_SUBPROCESS_FORCE_SETTLE_MS 1000
...
typedef struct ani_subprocess ani_subprocess_t;

typedef enum {
    ANI_PROC_POLL_ERROR = -1, /* invalid handle/arguments */
    ANI_PROC_POLL_RUNNING = 0,
    ANI_PROC_POLL_TERMINAL = 1
} ani_proc_poll_t;
 ...
 * but the terminal outcome remains ANI_PROC_HANG. */
 ...
 * (out->outcome == ANI_PROC_SPAWN_FAILED). */
 ...
 * is unit-tested on Linux/macOS CI, and so both spawn sites (ani_subprocess_run and
 ...
 * command-language payload. Unlike ani_build_win_cmdline(), payload is not a C
