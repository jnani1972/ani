...
typedef enum {
    ANI_READ_OK = 0,    /* file read successfully */
    ANI_READ_OPEN_FAIL, /* could not open (missing / permission) */
    ANI_READ_EMPTY,     /* zero/negative size — benign, nothing to index */
    ANI_READ_OVERSIZED, /* size exceeds ani_max_file_bytes() */
    ANI_READ_OOM,       /* buffer allocation failed */
} ani_read_status_t;
 ...
 * never silently dropped. Override with ANI_MAX_FILE_BYTES (a positive integer
 ...
 * ANI_CYPHER_MAX_DEPTH (a positive integer). Default 10. */
int ani_cypher_max_depth(void);
 ...
 * Override with ANI_MCP_MAX_DEPTH (a positive integer). Default 15. */
int ani_mcp_max_depth(void);
