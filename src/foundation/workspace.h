#ifndef ANI_FOUNDATION_WORKSPACE_H
#define ANI_FOUNDATION_WORKSPACE_H
 ...
 * of ANI_WS_ALLOW here means "this path is not obviously too broad", never
...
typedef enum {
    ...
    ANI_WS_DENY_TOO_SHALLOW,
    /* A filesystem root, a drive root, a UNC share root, or a path that is or
     * contains the ani cache directory. Never overridable. */
    ANI_WS_DENY_ABSOLUTE,
 ...
 * human override for a ANI_WS_DENY_SENSITIVE path; absolute denials and shallow
 ...
 * ANI_ALLOWED_ROOT / session policy, treated as an additional grant, or NULL.
...
#endif /* ANI_FOUNDATION_WORKSPACE_H */

/*
 * ── Per-project request manifest (.anipathwhitelist) ─────────────────────────
 *
...
#define ANI_WS_MANIFEST_NAME ".anipathwhitelist"

enum { ANI_WS_MANIFEST_MAX_ENTRIES = 64 };

typedef struct {
    /* Requested roots, verbatim from the file (already control-char screened). */
    char entries[ANI_WS_MANIFEST_MAX_ENTRIES][1024];
    int count;
...
/* Read <project_root>/.anipathwhitelist. Returns false only on a malformed file
