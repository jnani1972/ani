 ...
 *   - Level filtering at runtime via ani_log_set_level() or the
 *     ANI_LOG_LEVEL env var (see ani_log_init_from_env)
...
typedef enum {
    ...
    ANI_LOG_WARN = 2,
    ANI_LOG_ERROR = 3,
    ANI_LOG_NONE = 4 /* disable all logging */
} ANILogLevel;

typedef enum {
    ANI_LOG_FORMAT_TEXT = 0,
    ANI_LOG_FORMAT_JSON = 1,
} ANILogFormat;

typedef enum {
    ANI_LOG_SINK_REPLACE = 0,
    ANI_LOG_SINK_TEE = 1,
} ANILogSinkMode;

/* Apply the ANI_LOG_LEVEL environment variable to the runtime log level.
 * Accepts (case-insensitive) "debug", "info", "warn", "error", "none", or
 * the numeric equivalents 0..4 matching ANILogLevel. Unknown, empty, or
 * unset values leave the level unchanged (fail-open).
 *
 * Also applies ANI_LOG_FORMAT=text|json. If unset, the current format is left
 ...
 *
 * Example:
 *   ani_log(ANI_LOG_INFO, "pass.timing",
...
/* Convenience macros. */
#define ani_log_debug(msg, ...) ani_log(ANI_LOG_DEBUG, msg, ##__VA_ARGS__, NULL)
#define ani_log_info(msg, ...) ani_log(ANI_LOG_INFO, msg, ##__VA_ARGS__, NULL)
...
#define ani_log_control(msg, ...) ani_log_control_record(msg, ##__VA_ARGS__, NULL)
#define ani_log_warn(msg, ...) ani_log(ANI_LOG_WARN, msg, ##__VA_ARGS__, NULL)
#define ani_log_error(msg, ...) ani_log(ANI_LOG_ERROR, msg, ##__VA_ARGS__, NULL)
...
typedef void (*ani_log_sink_fn)(const char *line);
