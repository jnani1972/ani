...
static void sha256_transform(ani_sha256_ctx *c, const uint8_t *data) {
    uint32_t m[64];
...
void ani_sha256_hex(const void *data, size_t len, char out[ANI_SHA256_HEX_LEN + 1]) {
    ...
    ani_sha256_ctx c;
    ani_sha256_init(&c);
    ani_sha256_update(&c, data, len);
    ani_sha256_final(&c, digest);
    ...
    ani_secure_zero(&c, sizeof(c));
    ani_secure_zero(digest, sizeof(digest));
}
...
void ani_hmac_sha256(const void *key, size_t key_len, const void *data, size_t data_len,
    ...
    uint8_t normalized_key[ANI_SHA256_DIGEST_LEN];
    uint8_t inner_pad[SHA256_BLOCK_LEN];
    uint8_t outer_pad[SHA256_BLOCK_LEN];
    uint8_t inner_digest[ANI_SHA256_DIGEST_LEN];

    if (key_len > SHA256_BLOCK_LEN) {
        ani_sha256_ctx key_hash;
        ani_sha256_init(&key_hash);
        ani_sha256_update(&key_hash, key, key_len);
        ani_sha256_final(&key_hash, normalized_key);
        ani_secure_zero(&key_hash, sizeof(key_hash));
    ...
    ani_sha256_ctx inner;
    ani_sha256_init(&inner);
    ani_sha256_update(&inner, inner_pad, sizeof(inner_pad));
    ani_sha256_update(&inner, data, data_len);
    ani_sha256_final(&inner, inner_digest);

    ani_sha256_ctx outer;
    ani_sha256_init(&outer);
    ani_sha256_update(&outer, outer_pad, sizeof(outer_pad));
    ani_sha256_update(&outer, inner_digest, sizeof(inner_digest));
    ani_sha256_final(&outer, out);

    ani_secure_zero(&inner, sizeof(inner));
    ani_secure_zero(&outer, sizeof(outer));
    ani_secure_zero(normalized_key, sizeof(normalized_key));
    ani_secure_zero(inner_pad, sizeof(inner_pad));
    ani_secure_zero(outer_pad, sizeof(outer_pad));
    ani_secure_zero(inner_digest, sizeof(inner_digest));
}
