 ...
 *
 * Linux PROD builds now set MI_MALLOC_OVERRIDE (see Makefile.ani), so the strong
...
void *__wrap_malloc(size_t size) {
    ...
    ani_mem_profile_alloc(block, size);
    return block;
}

void *__wrap_calloc(size_t count, size_t size) {
    void *block = mi_calloc(count, size);
    ani_mem_profile_alloc(block, count * size);
    return block;
}

void *__wrap_realloc(void *block, size_t size) {
    if (block) {
        ani_mem_profile_free(block);
    }
    void *grown = mi_realloc(block, size);
    ani_mem_profile_alloc(grown, size);
    return grown;
}
...
char *__wrap_strdup(const char *text) {
    ...
    if (copy) {
        ani_mem_profile_alloc(copy, mi_usable_size(copy));
    }
    return copy;
}
