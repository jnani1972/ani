...
long ani_max_file_bytes(void) {
    ...
    const char *raw = getenv("ANI_MAX_FILE_BYTES");
    if (raw && raw[0]) {
        errno = 0;
        char *end = NULL;
 ...
 * Read fresh each call (see ani_max_file_bytes rationale — cheap, test-friendly,
...
int ani_cypher_max_depth(void) {
    ...
    return env_positive_int("ANI_CYPHER_MAX_DEPTH", 10);
}
...
int ani_mcp_max_depth(void) {
    ...
    return env_positive_int("ANI_MCP_MAX_DEPTH", 15);
}
