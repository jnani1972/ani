...
static void weighted_minhash_update(ani_minhash_t *out, const char *trigram, int len, int w) {
    for (int k = 0; k < ANI_MINHASH_K; k++) {
...
static int hash_trigrams(const char **tokens, int token_count, ani_minhash_t *out) {
    for (int k = 0; k < ANI_MINHASH_K; k++) {
...
bool ani_minhash_compute(TSNode func_body, const char *source, int language, ani_minhash_t *out) {
    ...
    if (token_count < ANI_MINHASH_MIN_NODES) {
        return false;
    }
...
double ani_minhash_jaccard(const ani_minhash_t *a, const ani_minhash_t *b) {
    ...
    return (double)matching / (double)ANI_MINHASH_K;
}

/* ── Hex encoding/decoding ───────────────────────────────────────── */

void ani_minhash_to_hex(const ani_minhash_t *fp, char *buf, int bufsize) {
    if (!fp || !buf || bufsize < ANI_MINHASH_HEX_BUF) {
        if (buf && bufsize > 0) {
            buf[0] = '\0';
        }
        return;
...
bool ani_minhash_from_hex(const char *hex, ani_minhash_t *out) {
    ...
    if ((int)len != ANI_MINHASH_HEX_LEN) {
        return false;
    }
    for (int i = 0; i < ANI_MINHASH_K; i++) {
...
static uint32_t band_hash(const ani_minhash_t *fp, int band) {
    int base = band * ANI_LSH_ROWS;
    /* Combine r=2 values into a single hash */
    uint32_t combined[ANI_LSH_ROWS];
    for (int r = 0; r < ANI_LSH_ROWS; r++) {
...
ani_lsh_index_t *ani_lsh_new(void) {
    ani_lsh_index_t *idx = calloc(SKIP_ONE, sizeof(ani_lsh_index_t));
    return idx;
}
...
void ani_lsh_insert(ani_lsh_index_t *idx, const ani_lsh_entry_t *entry) {
    ...
    if (idx->entry_count >= idx->entry_cap) {
        ...
        ani_lsh_entry_t *new_entries =
            realloc(idx->entries, (size_t)new_cap * sizeof(ani_lsh_entry_t));
        if (!new_entries) {
            return;
        }
...
static bool result_push(ani_lsh_index_t *idx, const ani_lsh_entry_t *candidate) {
    if (idx->result_count >= idx->result_cap) {
        int new_cap =
            idx->result_cap < RESULT_INIT_CAP ? RESULT_INIT_CAP : idx->result_cap * GROW_FACTOR;
        const ani_lsh_entry_t **new_buf =
            realloc(idx->result_buf, (size_t)new_cap * sizeof(const ani_lsh_entry_t *));
        if (!new_buf) {
            return false;
        }
...
void ani_lsh_query(const ani_lsh_index_t *idx, const ani_minhash_t *fp,
    ...
    *count = 0;
    ...
    ani_lsh_index_t *mut_idx = (ani_lsh_index_t *)idx;
    ...
    for (int b = 0; b < ANI_LSH_BANDS; b++) {
        ...
        for (int i = 0; i < bucket->count; i++) {
            const ani_lsh_entry_t *candidate = &idx->entries[bucket->items[i]];
