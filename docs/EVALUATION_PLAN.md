# ani — Evaluation Plan (159 Languages)
...
Measure how well `ani`'s structured knowledge-graph queries answer real developer
...
| Old (v8) | New |
|----------|-----|
| 66 languages | **159 languages** (full `ANI_LANG_*` registry) |
...
The supported set is the `ANILanguage` enum in `internal/ani/ani.h` (`ANI_LANG_GO=0` …
`ANI_LANG_CFML`, before `ANI_LANG_COUNT`). The canonical short name used throughout this plan is the
lowercased enum suffix (`ANI_LANG_GO` → `go`, `ANI_LANG_CSHARP` → `csharp`,
`ANI_LANG_COMMONLISP` → `commonlisp`). The complete repo assignment for all 159 is the master table
in §8.

### The 9 LSP-hybrid languages (deep-dive cohort)

These have dedicated hybrid LSP modules under `internal/ani/lsp/` and therefore type-aware
...
**One language at a time, in the main channel, never concurrently.** The ANI/graph side of the
...
for each language L in order (skip if manifest says done):
  ...
  8. DELETE L's index from ~/.cache/ani so the next language starts clean,
...
for lang in $ALL_LANGS; do                      # ALL_LANGS = full 159-name list (§2 / §8)
  ...
  t0=$(now_ms)
  scripts/benchmark-index.sh ~/.local/bin/ani "$lang" /tmp/bench/"$lang" /tmp/eval-results
...
`MATCH ()-[r]->() RETURN type(r), count(*)`) / `get_graph_schema` / `ani_store_count_edges_by_type`,
...
file (`internal/ani/lang_specs.c`) and the tree-sitter node type that would fix it. Plus a
...
#### Challenge Review: EVALUATION_PLAN.md — ani 159-Language Benchmark
