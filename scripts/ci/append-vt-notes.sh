...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/ani-vt-notes.XXXXXX")"
...
START = "<!-- ani-security-verification:start -->"
END = "<!-- ani-security-verification:end -->"
TARGETS = (
    "linux-amd64",
    "linux-arm64",
...
candidate_meta, candidates = read_tsv(
    candidates_path, "ani-release-candidates-v1", CANDIDATE_FIELDS
)
result_meta, results = read_tsv(
    results_path, "ani-virustotal-results-v2", RESULT_FIELDS
)
selection_meta, selections = read_tsv(
    selection_path, "ani-release-selection-v1", SELECTION_FIELDS
)
...
# when an opaque classifier flags one of them, but a reader installing ani cares
