...
#
# Every ani 0.9.x binary in the field asks for
# ani-ui-<platform>.<ext> when its (now-removed) variant chooser
...
gh release download "$VERSION" --dir "$work" --repo "$REPOSITORY" \
  --pattern 'ani-*.tar.gz' --pattern 'ani-*.zip'

shopt -s nullglob
aliases=()
for archive in "$work"/ani-*; do
  base="$(basename "$archive")"
  case "$base" in
  ani-ui-*) continue ;;
  esac
  alias_path="$work/${base/ani-/ani-ui-}"
