...
per-grammar license list is internal/ani/vendored/grammars/MANIFEST.md.
"""
...
version = sys.argv[1]
n_grammars = len(
    [d for d in glob.glob("internal/ani/vendored/grammars/*") if os.path.isdir(d)]
)
sbom = {
    ...
    "name": f"ani-{version}",
    "documentNamespace": f"https://github.com/jnani1972/ani/releases/{version}",
    "creationInfo": {
        "created": datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "creators": ["Tool: ani-release-pipeline"],
    },
    "packages": [
