...
#   (ANI_SMOKE_ARTIFACT_DIR), whose completeness checks make a broken or
...
canonical wrapper, exactly like the release venue:
  unix:    scripts/smoke-local.sh with ANI_SMOKE_ARTIFACT_DIR
  windows: test-infrastructure/vm/vm-smoke.sh with ANI_SMOKE_ARTIFACT_DIR
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/ani-smoke-artifact.XXXXXX")"
cleanup() {
    local status=$?
    trap - EXIT
    if [ "$status" -eq 0 ]; then
...
SELECTED_NAME="ani"
[ "$GOOS" = "windows" ] && SELECTED_NAME="ani.exe"
