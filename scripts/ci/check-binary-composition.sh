...
SEAM_NEEDLES=(
    'ANI_TEST_WORKER_DESCENDANT_PID_FILE'
    'ANI_TEST_CRASH_ON'
    'ANI_TEST_HANG_ON'
)
...
check_file() {
    ...
    # "ani", so the parent directory disambiguates the log.
    ...
    # Sweep for seams nobody thought to pin: a new ANI_TEST_* env var added to
    ...
    seam_allowed='ANI_TEST_WINDOWS_USER_PATH_RUN_ID'
    seam_unexpected=''
    for found in $(LC_ALL=C grep -a -o -E 'ANI_TEST_[A-Za-z0-9_]+' "$file" 2>/dev/null |
        sort -u || true); do
    ...
    if [ -n "$seam_unexpected" ]; then
        report FAIL A2-no-test-seams "$token" \
            "unexpected ANI_TEST_* seam(s):$seam_unexpected (allowlist: $seam_allowed)"
    else
        report PASS A2-no-test-seams "$token" 'no ANI_TEST_* seams beyond the smoke allowlist'
    fi
    ...
    # ANI_CHECK_UI_ABSENT=1 still enforces absence for any future headless build.
    if [ "${ANI_CHECK_UI_ABSENT:-0}" = "1" ]; then
