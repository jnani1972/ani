...
CANONICAL_ARCHIVES = frozenset(
    [f"ani-{target}.tar.gz" for target in UNIX_TARGETS]
    + [f"ani-{target}.zip" for target in WINDOWS_TARGETS]
    + [f"ani-{target}.mcpb" for target in MCPB_TARGETS]
)
...
def main(argv: Sequence[str]) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix=".ani-release-scan-", dir=str(output_dir.parent)) as temporary:
        ...
        write_tsv(
            ...
            marker="ani-release-scan-associations-v3",
        ...
        write_tsv(
            ...
            marker="ani-release-scan-set-v2",
