<#
.SYNOPSIS
Sweep ani test residue from a Windows test host and assert a runner-like free
...
Sweeps only ani-prefixed directories under the profile and TEMP. The checkout
(C:\ani) is not under either, and is never touched.
...
# non-representable characters, deep .ani-retired/generations nesting - whose
...
foreach ($root in $roots) {
    ...
    Get-ChildItem -LiteralPath $root -Directory -Filter 'ani-*' -ErrorAction SilentlyContinue |
        ForEach-Object {
