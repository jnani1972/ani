...
# ANI_ARCH env var or --arch flag override (parsed by calling script)
ARCH="${ANI_ARCH:-$HW_ARCH}"
...
# Makefile.ani folds $(ARCHFLAGS) into CC/CXX so it reaches every compile and
...
verify_compiler() {
    ...
    if [[ "$OS" == "darwin" ]]; then
        local probe_out
        probe_out="$(mktemp -t ani-archprobe.XXXXXX)"
...
# ── Verified compiler cache (ccache, opt-out ANI_NO_CCACHE=1) ──
...
if [[ "${ANI_NO_CCACHE:-0}" != "1" ]] && command -v ccache >/dev/null 2>&1; then
    for _ani_ccache_masq in \
        /usr/lib/ccache \
        /opt/homebrew/opt/ccache/libexec \
        /usr/local/opt/ccache/libexec \
        /clang64/lib/ccache/bin \
        /clangarm64/lib/ccache/bin; do
        if [[ -d "$_ani_ccache_masq" ]]; then
            case ":$PATH:" in
            *":$_ani_ccache_masq:"*) ;;
            *) PATH="$_ani_ccache_masq:$PATH" ;;
            esac
        fi
    done
    unset _ani_ccache_masq
    export PATH
