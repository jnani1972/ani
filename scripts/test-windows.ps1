<#
.SYNOPSIS
    Run the native-Windows product-surface test suite for ani.
...
.DESCRIPTION
    ...
    Two categories of test:
      ...
      GUARDS      - regression guards for Windows bugs already fixed on main.
                    ...
                    regressed and fails this runner.
                      ...
                      * test_non_ascii_cache_dump.py guards #996 (writer ani_fopen)
    ...
    under the mandatory coordination daemon ANI_INDEX_SUPERVISOR=0 is a
...
.PARAMETER Target
    Makefile.ani target used when building: 'ani-with-ui' (default; needed for the
    drive-picker guard's embedded HTTP UI) or 'ani' (no UI - the drive guard then
...
.EXAMPLE
    pwsh -File scripts/test-windows.ps1 -GuardsOnly -Binary build\c\ani.exe
#>
[CmdletBinding()]
param(
    [string]$Binary,
    [ValidateSet("ani-with-ui", "ani")]
    [string]$Target = "ani-with-ui",
    [switch]$GuardsOnly,
    [string]$Make = "make"
)
...
function Resolve-Binary {
    ...
    $built = Join-Path $repoRoot "build\c\ani.exe"
    if (Test-Path $built) { return $built }
    Write-Host "Building $Target via Makefile.ani ..." -ForegroundColor Cyan
    & $Make "-j" "-f" "Makefile.ani" $Target "SANITIZE=" "TMP=$tmp" "TEMP=$tmp" "TMPDIR=$tmp" | Out-Host
...
try {
    ...
    $guardRoot = Join-Path $userProfile ("ani-windows-guards-root-" + [guid]::NewGuid().ToString("N"))
    ...
    $guardBundle = Join-Path $guardRoot ("ani-windows-guards-" + [guid]::NewGuid().ToString("N"))
    New-Item -ItemType Directory -Path $guardBundle | Out-Null
    $guardBin = Join-Path $guardBundle "ani.exe"
...
# UI (build target ani-with-ui); against a non-UI binary it reports a precondition
...
if ($guardSkips.Count -gt 0) {
    ...
    Write-Host "needs a UI build (-Target ani-with-ui, the default)." -ForegroundColor Yellow
}
