...
#   jobs defaults to ANI_TEST_PAR_JOBS, then the CPU count.

set -uo pipefail

RUNNER="${1:?usage: run-tests-parallel.sh <path-to-test-runner> [jobs]}"
JOBS="${2:-${ANI_TEST_PAR_JOBS:-}}"
...
# ANI_TEST_SHARD="i/N" runs this invocation's deterministic slice of the
...
SHARD_INDEX=1
SHARD_TOTAL=1
if [ -n "${ANI_TEST_SHARD:-}" ]; then
    if ! printf '%s' "$ANI_TEST_SHARD" | grep -qE '^[0-9]+/[0-9]+$'; then
        echo "FAIL: ANI_TEST_SHARD must be i/N, got '$ANI_TEST_SHARD'" >&2
        exit 1
    fi
    SHARD_INDEX="${ANI_TEST_SHARD%%/*}"
    SHARD_TOTAL="${ANI_TEST_SHARD##*/}"
    if [ "$SHARD_TOTAL" -lt 1 ] || [ "$SHARD_INDEX" -lt 1 ] ||
        [ "$SHARD_INDEX" -gt "$SHARD_TOTAL" ]; then
        echo "FAIL: ANI_TEST_SHARD out of range: $ANI_TEST_SHARD" >&2
        exit 1
    fi
fi
...
# against a mis-plumbed ANI_TEST_SHARD (two jobs running the same slice
...
run_wave() {
    ...
    if ! python3 "$SCHEDULER" \
        ...
        --jobs "$jobs" \
        --timeout "${ANI_SUITE_TIMEOUT:-900}" \
        --slow-timeout "${ANI_SUITE_TIMEOUT_SLOW:-3600}" \
        --kill-grace 15 \
        "$RUNNER"; then
...
# so a small fixed overlap (ANI_TAIL_JOBS, default 2) is safe and converts
...
run_wave "$FLEX_FILE" "${ANI_TAIL_JOBS:-2}"
