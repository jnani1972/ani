...
# re-runnable by construction (ANI_FUZZ_SEED overrides for replay).
FUZZ_SEED="${ANI_FUZZ_SEED:-$$}"
echo "fuzz seed: $FUZZ_SEED (replay: ANI_FUZZ_SEED=$FUZZ_SEED)"
...
FUZZ_TMPDIR="$ANI_TEST_RUNTIME_ROOT"
trap 'ani_test_runtime_cleanup "$BINARY"' EXIT
