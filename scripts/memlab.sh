...
if [[ "$BINARY" == *.exe ]] && command -v cygpath >/dev/null 2>&1 &&
    ...
    WIN_ROOT=$(mktemp -d "$(cygpath "$USERPROFILE")/ani-memlab.XXXXXX")
    ...
    *)
    ...
    cp "$BINARY" "$WIN_ROOT/ani.exe"
    BINARY="$WIN_ROOT/ani.exe"
    WORK="$WIN_ROOT"
...
if command -v cygpath >/dev/null 2>&1 && ! command -v winepath >/dev/null 2>&1; then
    mkdir -p "$WORK/cache"
    export ANI_CACHE_DIR="$(cygpath -w "$WORK/cache")"
else
    export ANI_CACHE_DIR="$WORK/cache"
fi
export ANI_MEM_PROFILE=1
export ANI_MEM_PROFILE_OUT="$PROFILE_OUT"
export ANI_MEM_CENSUS=1
export ANI_LOG_LEVEL=info
export ANI_LOG_FORMAT=text
...
RC=$?
# With ANI_CACHE_DIR set the process logs to its own file rather than stderr,
...
if [ "$CENSUS" -eq 0 ]; then
    echo "WARN: no census samples — check ANI_MEM_CENSUS wiring" >&2
fi
if [ "$SITES" -eq 0 ]; then
