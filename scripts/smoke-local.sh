...
#   <binary>  product binary to smoke (e.g. build/c/ani)
...
Environment:
  ANI_SMOKE_ARTIFACT_DIR   Release mode: an EXTRACTED release artifact
...
smoke-linux-portable (with ANI_SMOKE_ARTIFACT_DIR) · compose smoke services.
...
WORK_DIR=$(mktemp -d "${TMPDIR:-/tmp}/ani-smoke-server-XXXXXX")
...
EXPECTED_ARTIFACT="ani-${OS}-${ARCH}.tar.gz"
tar -czf "$FIXTURE_DIR/$EXPECTED_ARTIFACT" -C "$FIXTURE_DIR" \
    ani LICENSE install.sh THIRD_PARTY_NOTICES.md
...
if [ "$OS" = "linux" ]; then
    cp "$FIXTURE_DIR/$EXPECTED_ARTIFACT" \
        "$FIXTURE_DIR/ani-${OS}-${ARCH}-portable.tar.gz"
fi
