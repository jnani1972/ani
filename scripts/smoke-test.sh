...
smoke_mktemp_file() {
  if [ -n "${SMOKE_TEMP_ROOT:-}" ]; then
    mktemp "$SMOKE_TEMP_ROOT/ani-smoke.XXXXXX"
  else
    mktemp
  fi
}
...
smoke_mktemp_dir() {
  if [ -n "${SMOKE_TEMP_ROOT:-}" ]; then
    mktemp -d "$SMOKE_TEMP_ROOT/ani-smoke.XXXXXX"
  else
    mktemp -d
  fi
}
...
#                     ani_mem_init is decoration and freed pages stay committed
...
CFG_WATCH=$(ANI_CACHE_DIR="$CFG_HOME" "$BINARY" config get auto_watch 2>/dev/null || true)
...
if ANI_CACHE_DIR="$CFG_HOME" "$BINARY" config get totally_bogus_key >/dev/null 2>&1; then
...
SC=$(cli search_code --project "$PROJECT" --pattern ani_ --mode compact --limit 5)
...
SCW=$(cli search_code --project "$PROJECT" --pattern "ani_init|ani_nope" --regex false --limit 5)
...
SCA=$(cli search_code --project "$PROJECT" --pattern ani_ --file-pattern "*R&D*.c" --limit 5)
case "$SCA" in
...
if command -v pgrep &>/dev/null && [ "$(uname)" != "MINGW64_NT" ] 2>/dev/null; then
  ...
  sleep 1
  RESIDUAL=$(pgrep -f "ani.*cli" 2>/dev/null | wc -l | tr -d ' \n' || echo "0")
  RESIDUAL="${RESIDUAL:-0}"
  if [ "$RESIDUAL" -gt 0 ]; then
    echo "WARNING: $RESIDUAL residual ani process(es) found"
  else
...
mcp_dump_diagnostics() {
  ...
  local cache="${ANI_CACHE_DIR:-$HOME/.cache/ani}"
  for log in ani-daemon.log daemon-conflicts.ndjson; do
...
DRYRUN_CACHE="$DRYRUN_HOME/.cache/ani"
...
if [[ "$BINARY" == *.exe ]] &&
   [ -n "${ANI_TEST_WINDOWS_USER_PATH_RUN_ID:-}" ]; then
  if INVALID_PATH_OUT=$(
    ANI_TEST_WINDOWS_USER_PATH_RUN_ID=invalid \
...
copy_smoke_binary "$INSTALL_DIR/ani"
chmod 755 "$INSTALL_DIR/ani"

# Verify installed binary works
INSTALLED_VER=$("$INSTALL_DIR/ani" --version 2>&1)
...
copy_smoke_binary "$REPLACE_DIR/smoke-ani"

# 3. Simulate ani_replace_binary: unlink old, copy new
rm -f "$INSTALL_DIR/ani"
cp "$REPLACE_DIR/smoke-ani" "$INSTALL_DIR/ani"
chmod 755 "$INSTALL_DIR/ani"

# 4. Verify replaced binary works
REPLACED_VER=$("$INSTALL_DIR/ani" --version 2>&1)
...
# 5. Test replacement of read-only binary (edge case — ani_replace_binary
#    handles this via unlink-before-write, which works even on read-only files)
chmod 444 "$INSTALL_DIR/ani"
rm -f "$INSTALL_DIR/ani"
cp "$REPLACE_DIR/smoke-ani" "$INSTALL_DIR/ani"
chmod 755 "$INSTALL_DIR/ani"
READONLY_VER=$("$INSTALL_DIR/ani" --version 2>&1)
...
DEVIN_SKILL="$DEVIN_DIR/skills/ani/SKILL.md"
CODEBUDDY_MCP="$FAKE_HOME/.codebuddy/.mcp.json"
CODEBUDDY_INSTRUCTIONS="$FAKE_HOME/.codebuddy/CODEBUDDY.md"
CODEBUDDY_SKILL="$FAKE_HOME/.codebuddy/skills/ani/SKILL.md"
CODEBUDDY_AGENT="$FAKE_HOME/.codebuddy/agents/ani.md"
...
BOB_RULE="$FAKE_HOME/.bob/rules/ani.md"
BOB_SKILL="$FAKE_HOME/.bob/skills/ani/SKILL.md"
BOB_AGENT="$FAKE_HOME/.bob/agents/ani.md"
POCHI_MCP="$FAKE_HOME/.pochi/config.jsonc"
POCHI_INSTRUCTIONS="$FAKE_HOME/.pochi/README.pochi.md"
POCHI_SKILL="$FAKE_HOME/.pochi/skills/ani/SKILL.md"
POCHI_AGENT="$FAKE_HOME/.pochi/agents/ani.md"
ROVO_MCP="$FAKE_HOME/.rovodev/mcp.json"
ROVO_INSTRUCTIONS="$FAKE_HOME/.rovodev/AGENTS.md"
ROVO_SKILL="$FAKE_HOME/.rovodev/skills/ani/SKILL.md"
ROVO_AGENT="$FAKE_HOME/.rovodev/subagents/ani.md"
...
if [[ "$BINARY" == *.exe ]]; then
  SELF_PATH="$FAKE_HOME/.local/bin/ani.exe"
else
  cp "$BINARY" "$FAKE_HOME/.local/bin/ani"
  SELF_PATH="$FAKE_HOME/.local/bin/ani"
fi
create_agent_stub() {
  local name="$1"
  if [[ "$BINARY" == *.exe ]]; then
...
# Run install — override platform config dirs so ani_app_config_dir() and
# ani_app_local_dir() resolve to FAKE_HOME paths on all platforms.
PHASE8_INSTALL_RC=0
...
if [[ "$BINARY" == *.exe ]]; then
  ...
  PHASE8_CANONICAL="$FAKE_HOME/.local/bin/ani.exe"
  ...
  if [ -e "$FAKE_HOME/.local/bin/ani.payload.exe" ]; then
    ...
    if [ "$access" = "direct" ]; then
      if ! grep -Fq 'source read/grep fallback' "$file" 2>/dev/null; then
        ...
        exit 1
         ...
         grep -qE '(mcpServers|mcp__ani__|mcp_ani_|@ani/|ani/)' "$file" 2>/dev/null; then
...
assert_tier_profile_set_removed() {
  ...
  local suffix="$3"
  local slug file
  for slug in ani-scout ani ani-auditor; do
    file="$directory/$slug$suffix"
    if [ -e "$file" ]; then
...
CMD=$(json_get "$FAKE_HOME/.claude.json" "d.get('mcpServers',{}).get('ani',{}).get('command','')")
...
sys.exit(0 if 'ani' in d.get('mcpServers', {}) else 1)
" 2>/dev/null; then
...
CLAUDE_AGENT="$FAKE_HOME/.claude/agents/ani.md"
if ! grep -q '^mcpServers: \[ani\]$' "$CLAUDE_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__ani__search_graph' "$CLAUDE_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__ani__check_index_coverage' "$CLAUDE_AGENT" 2>/dev/null ||
   ! grep -q '^permissionMode: plan$' "$CLAUDE_AGENT" 2>/dev/null ||
   grep -qE 'mcp__ani__(index_repository|delete_project|manage_adr|ingest_traces)' "$CLAUDE_AGENT" 2>/dev/null; then
...
case "$(uname -s)" in
  MINGW*|MSYS*|CYGWIN*)
    GATE_SCRIPT="$FAKE_HOME/.claude/hooks/ani-code-discovery-gate.cmd"
    if [ ! -f "$GATE_SCRIPT" ]; then
      echo "FAIL 8e: .cmd shim missing on Windows"
      exit 1
    fi
    if [ -f "$FAKE_HOME/.claude/hooks/ani-code-discovery-gate" ]; then
      echo "FAIL 8e: legacy extensionless shim still installed on Windows"
      exit 1
    fi
    ;;
  *)
    GATE_SCRIPT="$FAKE_HOME/.claude/hooks/ani-code-discovery-gate"
...
# 8f-8h: Codex TOML
if ! grep -q '\[mcp_servers.ani\]' "$FAKE_HOME/.codex/config.toml"; then
...
if [ ! -f "$FAKE_HOME/.codex/AGENTS.md" ] || ! grep -q 'ani' "$FAKE_HOME/.codex/AGENTS.md"; then
...
CMD=$(json_get "$FAKE_HOME/.gemini/settings.json" "d['mcpServers']['ani']['command']")
...
GEMINI_AGENT="$FAKE_HOME/.gemini/agents/ani.md"
if ! grep -q '^name: ani$' "$GEMINI_AGENT" 2>/dev/null ||
   ...
   ! grep -q 'mcp_ani_search_graph' "$GEMINI_AGENT" 2>/dev/null ||
   ! grep -q 'mcp_ani_check_index_coverage' "$GEMINI_AGENT" 2>/dev/null ||
   grep -qE 'mcp_ani_(index_repository|delete_project|manage_adr|ingest_traces)' "$GEMINI_AGENT" 2>/dev/null; then
...
if [ -f "$ZED_CFG" ]; then
  CMD=$(json_get "$ZED_CFG" "d['context_servers']['ani']['command']")
  if ! path_match "$CMD" "$SELF_PATH"; then
...
if ! grep -q 'Ani' "$ZED_INSTR" 2>/dev/null ||
...
CMD=$(json_get "$FAKE_HOME/.config/opencode/opencode.json" "d['mcp']['ani']['command'][0]")
if [ -n "$CMD" ]; then
  if ! path_match "$CMD" "$SELF_PATH"; then
...
CMD=$(json_get "$FAKE_HOME/.gemini/config/mcp_config.json" "d['mcpServers']['ani']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
...
if [ -f "$FAKE_HOME/CONVENTIONS.md" ]; then
  if ! grep -q 'ani' "$FAKE_HOME/CONVENTIONS.md"; then
...
CMD=$(json_get "$KILO_CFG" "d['mcp']['ani']['command'][0]")
if ! path_match "$CMD" "$SELF_PATH"; then
...
KILO_RULE="$FAKE_HOME/.config/kilo/rules/ani.md"
if [ ! -f "$KILO_RULE" ]; then
...
KILO_AGENT="$FAKE_HOME/.config/kilo/agents/ani.md"
if ! grep -q '^mode: subagent$' "$KILO_AGENT" 2>/dev/null ||
   ! grep -Fq '"*": deny' "$KILO_AGENT" 2>/dev/null ||
   ! grep -Fq '"ani_search_graph": allow' "$KILO_AGENT" 2>/dev/null ||
   ! grep -Fq '"ani_get_code_snippet": allow' "$KILO_AGENT" 2>/dev/null ||
   ! grep -Fq '"ani_check_index_coverage": allow' "$KILO_AGENT" 2>/dev/null ||
   grep -Fq '"ani_*": allow' "$KILO_AGENT" 2>/dev/null ||
   grep -qE 'ani_(index_repository|delete_project|manage_adr|ingest_traces)' "$KILO_AGENT" 2>/dev/null ||
...
CMD=$(json_get "$VSCODE_CFG" "d['servers']['ani']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
  echo "FAIL 8v: VS Code command='$CMD'"
  exit 1
fi
echo "OK 8v: VS Code MCP"
CMD=$(json_get "$VSCODE_PROFILE_CFG" "d['servers']['ani']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
...
# 8w: OpenClaw MCP
CMD=$(json_get "$FAKE_HOME/.openclaw/openclaw.json" "d['mcp']['servers']['ani']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
  echo "FAIL 8w: OpenClaw command='$CMD'"
  exit 1
fi
ENABLED=$(json_get "$FAKE_HOME/.openclaw/openclaw.json" "d['mcp']['servers']['ani'].get('enabled')")
...
for OPENCLAW_CONTEXT in \
  ...
  if ! grep -q '^## Codebase Knowledge Graph (ani)$' "$OPENCLAW_CONTEXT" 2>/dev/null ||
     ! grep -q 'subagent' "$OPENCLAW_CONTEXT" 2>/dev/null; then
    echo "FAIL 8w-i: OpenClaw durable context missing in $OPENCLAW_CONTEXT"
    exit 1
  fi
done
OPENCLAW_COMPACTION=$(json_get "$FAKE_HOME/.openclaw/openclaw.json" "str('Codebase Knowledge Graph (ani)' in d['agents']['defaults']['compaction']['postCompactionSections'])")
...
CMD=$(json_get "$FAKE_HOME/.kiro/settings/mcp.json" "d['mcpServers']['ani']['command']")
KIRO_AGENT="$FAKE_HOME/.kiro/agents/ani.json"
KIRO_AGENT_CMD=$(json_get "$KIRO_AGENT" "d['mcpServers']['ani']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
   ! path_match "$KIRO_AGENT_CMD" "$SELF_PATH" ||
   ! grep -q 'search_graph' "$FAKE_HOME/.kiro/steering/ani.md" 2>/dev/null ||
   ! cat "$KIRO_AGENT" 2>/dev/null | python3 -c "
import json, sys
d = json.load(sys.stdin)
tools = d.get('tools', [])
server = d.get('mcpServers', {}).get('ani', {})
ok = (d.get('name') == 'ani' and tools[:3] == ['read', 'grep', 'glob'] and
      '@ani/search_graph' in tools and
      '@ani/check_index_coverage' in tools and
      all('@ani/' + name not in tools for name in
          ('index_repository', 'delete_project', 'manage_adr', 'ingest_traces')) and
      '@ani' not in tools and d.get('includeMcpJson') is False and
      set(d.get('mcpServers', {})) == {'ani'} and
...
if ! grep -q '^name: ani$' "$FAKE_HOME/.hermes/skills/ani/SKILL.md" 2>/dev/null ||
   ! grep -q 'delegate_task' "$FAKE_HOME/.hermes/skills/ani/SKILL.md" 2>/dev/null ||
   ! grep -q '`context`' "$FAKE_HOME/.hermes/skills/ani/SKILL.md" 2>/dev/null; then
...
# 8y: OpenHands MCP
CMD=$(json_get "$FAKE_HOME/.openhands/mcp.json" "d['mcpServers']['ani']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
  echo "FAIL 8y: OpenHands command='$CMD'"
  exit 1
fi
echo "OK 8y: OpenHands MCP"
if ! grep -q '^name: ani$' "$FAKE_HOME/.agents/skills/ani/SKILL.md" 2>/dev/null ||
   ! grep -q 'trace_path' "$FAKE_HOME/.agents/skills/ani/SKILL.md" 2>/dev/null; then
...
CMD=$(json_get "$FAKE_HOME/.qwen/settings.json" "d['mcpServers']['ani']['command']")
...
COPILOT_SKILL="$FAKE_HOME/.copilot/skills/ani/SKILL.md"
COPILOT_AGENT="$FAKE_HOME/.copilot/agents/ani.agent.md"
if ! grep -q 'search_graph' "$COPILOT_SKILL" 2>/dev/null ||
   ! grep -q '^tools:' "$COPILOT_AGENT" 2>/dev/null ||
   ! grep -q 'ani/trace_path' "$COPILOT_AGENT" 2>/dev/null ||
   grep -qE '^  - (edit|shell|bash|ani/(index_repository|delete_project|manage_adr|ingest_traces))$' "$COPILOT_AGENT" 2>/dev/null; then
...
COPILOT_HOOKS="$FAKE_HOME/.copilot/hooks/ani.json"
...
CMD=$(json_get "$FAKE_HOME/.factory/mcp.json" "d['mcpServers']['ani']['command']")
FACTORY_TYPE=$(json_get "$FAKE_HOME/.factory/mcp.json" "d['mcpServers']['ani']['type']")
...
FACTORY_AGENT="$FAKE_HOME/.factory/droids/ani.md"
if ! grep -q '^tools: \["Read", "LS", "Grep", "Glob",' "$FACTORY_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__ani__search_graph' "$FACTORY_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__ani__check_index_coverage' "$FACTORY_AGENT" 2>/dev/null ||
   grep -q '^mcpServers:' "$FACTORY_AGENT" 2>/dev/null ||
   grep -qE 'mcp__ani__(index_repository|delete_project|manage_adr|ingest_traces)' "$FACTORY_AGENT" 2>/dev/null; then
...
CMD=$(json_get "$FAKE_HOME/.config/crush/crush.json" "d['mcp']['ani']['command']")
CRUSH_TYPE=$(json_get "$FAKE_HOME/.config/crush/crush.json" "d['mcp']['ani']['type']")
CRUSH_CONTEXT=$(json_get "$FAKE_HOME/.config/crush/crush.json" "str(any(str(p).endswith('ani.md') for p in d['options']['context_paths']))")
if ! path_match "$CMD" "$SELF_PATH" || [ "$CRUSH_TYPE" != "stdio" ] ||
   [ "$CRUSH_CONTEXT" != "True" ] ||
   ! grep -q 'does not inherit MCP access' "$FAKE_HOME/.config/crush/ani.md" 2>/dev/null; then
...
if ! grep -q '^\[\[mcp_servers\]\]' "$FAKE_HOME/.vibe/config.toml" 2>/dev/null ||
   ! grep -q '^name = "ani"' "$FAKE_HOME/.vibe/config.toml" 2>/dev/null ||
...
VIBE_AGENT="$FAKE_HOME/.vibe/agents/ani.toml"
VIBE_PROMPT="$FAKE_HOME/.vibe/prompts/ani.md"
if ! grep -q '^agent_type = "subagent"$' "$VIBE_AGENT" 2>/dev/null ||
   ! grep -Fq 'enabled_tools = ["read_file", "grep_search", "ani_search_graph"' "$VIBE_AGENT" 2>/dev/null ||
   ! grep -Fq '"ani_get_code_snippet"' "$VIBE_AGENT" 2>/dev/null ||
   ! grep -Fq '"ani_check_index_coverage"' "$VIBE_AGENT" 2>/dev/null ||
   grep -Fq '"ani_*"' "$VIBE_AGENT" 2>/dev/null ||
   grep -qE 'ani_(index_repository|delete_project|manage_adr|ingest_traces)' "$VIBE_AGENT" 2>/dev/null ||
   ! grep -q '^system_prompt_id = "ani"$' "$VIBE_AGENT" 2>/dev/null ||
...
CMD=$(json_get "$FAKE_HOME/.codeium/windsurf/mcp_config.json" "d['mcpServers']['ani']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
...
AUGMENT_RULE="$FAKE_HOME/.augment/rules/ani.md"
AUGMENT_AGENT="$FAKE_HOME/.augment/agents/ani.md"
if [[ "$BINARY" == *.exe ]]; then
  AUGMENT_SCRIPT="$FAKE_HOME/.augment/hooks/ani-session.ps1"
else
  AUGMENT_SCRIPT="$FAKE_HOME/.augment/hooks/ani-session.sh"
fi
CMD=$(json_get "$AUGMENT_SETTINGS" "d['mcpServers']['ani']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
   ! grep -q 'search_graph' "$AUGMENT_RULE" 2>/dev/null ||
   ! grep -q '^name: ani$' "$AUGMENT_AGENT" 2>/dev/null ||
...
owned = [e for e in entries if any('ani-session' in str(h.get('command', '')) for h in e.get('hooks', []))]
...
SKILL_FILE="$FAKE_HOME/.claude/skills/ani/SKILL.md"
if [ ! -s "$SKILL_FILE" ]; then
  echo "FAIL 8ai: skill ani missing or empty"
  exit 1
fi
...
QODER_SKILL="$FAKE_HOME/.qoder/skills/ani/SKILL.md"
QODER_AGENT="$FAKE_HOME/.qoder/agents/ani.md"
CMD=$(json_get "$QODER_SETTINGS" "d['mcpServers']['ani']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
   ! grep -q 'search_graph' "$QODER_SKILL" 2>/dev/null ||
   ! grep -q '^tools: Read,Grep,Glob,mcp__ani__search_graph' "$QODER_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__ani__check_index_coverage' "$QODER_AGENT" 2>/dev/null ||
   ! grep -q '^mcpServers:$' "$QODER_AGENT" 2>/dev/null ||
   ! grep -q '^  - ani$' "$QODER_AGENT" 2>/dev/null ||
   grep -qE 'mcp__ani__(index_repository|delete_project|manage_adr|ingest_traces)' "$QODER_AGENT" 2>/dev/null ||
...
KIMI_SKILL="$CUSTOM_KIMI_HOME/skills/ani/SKILL.md"
KIMI_CONFIG="$CUSTOM_KIMI_HOME/config.toml"
CMD=$(json_get "$KIMI_MCP" "d['mcpServers']['ani']['command']")
...
PI_SKILL="$FAKE_HOME/.pi/agent/skills/ani/SKILL.md"
PI_EXTENSION="$FAKE_HOME/.pi/agent/extensions/aniem.ts"
...
if command -v node >/dev/null 2>&1; then
  ...
  python3 - "$PI_EXTENSION" "$PI_PROBE_DIR/aniem.mjs" <<'PYPIADAPTER'
import pathlib
import sys
...
process.exit(0);
PICHILD
  if ! grep -Fxq 'const BIN = process.execPath;' "$PI_PROBE_DIR/aniem.mjs" ||
...
import extension from './aniem.mjs';
...
WARP_SKILL="$FAKE_HOME/.agents/skills/ani/SKILL.md"
...
JUNIE_SKILL="$FAKE_HOME/.junie/skills/ani/SKILL.md"
JUNIE_AGENT="$FAKE_HOME/.junie/agents/ani.md"
CMD=$(json_get "$JUNIE_MCP" "d['mcpServers']['ani']['command']")
JUNIE_SCOUT_CMD=$(json_get "$JUNIE_MCP" "d['mcpServers']['ani-scout']['command']")
JUNIE_ANALYSIS_CMD=$(json_get "$JUNIE_MCP" "d['mcpServers']['ani-analysis']['command']")
JUNIE_SCOUT_ARGS=$(json_get "$JUNIE_MCP" "d['mcpServers']['ani-scout']['args']")
JUNIE_ANALYSIS_ARGS=$(json_get "$JUNIE_MCP" "d['mcpServers']['ani-analysis']['args']")
if ! path_match "$CMD" "$SELF_PATH" ||
   ...
   ! grep -q 'mcpServers: \["ani-analysis"\]' "$JUNIE_AGENT" 2>/dev/null ||
...
CMD=$(json_get "$ROO_CFG" "d['mcpServers']['ani']['command']")
...
CMD=$(json_get "$GITLAB_MCP" "d['mcpServers']['ani']['command']")
...
CMD=$(json_get "$DEVIN_CONFIG" "d['mcpServers']['ani']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
...
" 2>/dev/null ||
   ...
   ! grep -q 'ani-code-discovery-gate' "$FAKE_HOME/.claude/settings.json" 2>/dev/null; then
...
CMD=$(json_get "$CODEBUDDY_MCP" "d['mcpServers']['ani']['command']")
...
if ! path_match "$CMD" "$SELF_PATH" || [ "$CODEBUDDY_KEEP" != "codebuddy" ] ||
   ...
   ! grep -q '^tools: Read,Grep,Glob,mcp__ani__search_graph,' "$CODEBUDDY_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__ani__check_index_coverage' "$CODEBUDDY_AGENT" 2>/dev/null ||
   grep -qE 'mcp__ani__(index_repository|delete_project|manage_adr|ingest_traces)' "$CODEBUDDY_AGENT" 2>/dev/null ||
   grep -q '^tools:$' "$CODEBUDDY_AGENT" 2>/dev/null ||
   grep -q 'mcp__ani__search_graph' "$CODEBUDDY_AGENT" 2>/dev/null ||
   ! grep -q '^skills: ani$' "$CODEBUDDY_AGENT" 2>/dev/null ||
...
BOB_IDE_CMD=$(json_get "$BOB_IDE_MCP" "d['mcpServers']['ani']['command']")
BOB_SHELL_CMD=$(json_get "$BOB_SHELL_MCP" "d['mcpServers']['ani']['command']")
...
POCHI_CMD=$(sed '/^[[:space:]]*\/\//d' "$POCHI_MCP" 2>/dev/null | python3 -c "import json,sys; print(json.load(sys.stdin)['mcp']['ani']['command'])" 2>/dev/null || echo "")
...
ROVO_CMD=$(json_get "$ROVO_MCP" "d['mcpServers']['ani']['command']")
...
AMAZON_Q_CMD=$(json_get "$AMAZON_Q_MCP" "d['mcpServers']['ani']['command']")
...
for VIBE_SLUG in ani-scout ani ani-auditor; do
...
if ! grep -q '^\[mcp_servers\.ani\]$' "$GROK_CONFIG" 2>/dev/null ||
   [ "$(grep -c '^\[mcp_servers\.ani\]$' "$GROK_CONFIG" 2>/dev/null)" != "1" ] ||
...
if ! grep -q 'search_graph' "$FAKE_HOME/.grok/rules/ani.md" 2>/dev/null ||
   ! grep -q 'search_graph' "$FAKE_HOME/.grok/skills/ani/SKILL.md" 2>/dev/null; then
  echo "FAIL 8ax: Grok Build rules file or skill missing"
  exit 1
fi
GROK_AGENT="$FAKE_HOME/.grok/agents/ani.md"
if ! grep -Fq 'tools: read_file, grep, list_dir, search_tool, use_tool' "$GROK_AGENT" 2>/dev/null ||
   ! grep -Fq '    - ani' "$GROK_AGENT" 2>/dev/null ||
   ! grep -Fq 'ani__search_graph' "$GROK_AGENT" 2>/dev/null ||
   ! grep -Fq 'ani__check_index_coverage' "$GROK_AGENT" 2>/dev/null ||
   grep -Fq 'ani__*' "$GROK_AGENT" 2>/dev/null; then
...
import json, sys
d = json.load(sys.stdin)
sys.exit(1 if 'ani' in d.get('mcpServers', {}) else 0)
" 2>/dev/null; then
...
d = json.load(sys.stdin)
hooks = d.get('hooks', {})
found = any('ani-code-discovery-gate' in str(h) or
            'ani-session-reminder' in str(h) or
            'ani-subagent-reminder' in str(h)
            for entries in hooks.values() for h in entries)
sys.exit(1 if found else 0)
" 2>/dev/null; then
  for HOOK_SCRIPT in ani-code-discovery-gate ani-session-reminder ani-subagent-reminder; do
...
if grep -q '\[mcp_servers.ani\]' "$FAKE_HOME/.codex/config.toml" 2>/dev/null; then
...
import json, sys
d = json.load(sys.stdin)
has_mcp = 'ani' in d.get('mcpServers', {})
has_existing = d.get('existingKey', False)
hooks = d.get('hooks', {}).get('BeforeTool', [])
has_hook = any('ani' in str(h) for h in hooks)
...
import json, sys
d = json.load(sys.stdin)
sys.exit(1 if 'ani' in d.get('servers', {}) else 0)
" 2>/dev/null; then
...
import json, sys
d = json.load(sys.stdin)
has_mcp = 'ani' in d.get('mcp', {}).get('servers', {})
sections = d.get('agents', {}).get('defaults', {}).get('compaction', {}).get('postCompactionSections', [])
sys.exit(1 if has_mcp or 'Codebase Knowledge Graph (ani)' in sections else 0)
" 2>/dev/null; then
  if grep -q 'ani:start' "$FAKE_HOME/.openclaw/workspace/AGENTS.md" 2>/dev/null ||
     grep -q 'ani:start' "$FAKE_HOME/.openclaw/workspace/TOOLS.md" 2>/dev/null; then
...
import json, sys
d = json.load(sys.stdin)
sys.exit(1 if 'ani' in d.get('$ROOT', {}) else 0)
" 2>/dev/null; then
...
names = {'ani', 'ani-scout', 'ani-analysis'}
...
import json, sys
d = json.load(sys.stdin)
print('present' if 'ani' in d.get('mcp', {}) else 'absent')
...
if grep -q 'hook-augment' "$QODER_SETTINGS" 2>/dev/null ||
   ...
   grep -q 'hook-augment' "$FAKE_HOME/.copilot/hooks/ani.json" 2>/dev/null ||
   grep -q 'hook-augment' "$FAKE_HOME/.factory/hooks.json" 2>/dev/null ||
   grep -q 'ani-session' "$AUGMENT_SETTINGS" 2>/dev/null ||
...
if CRUSH_CONTEXT=$(json_get "$FAKE_HOME/.config/crush/crush.json" "str(any(str(p).endswith('ani.md') for p in d.get('options', {}).get('context_paths', [])))") &&
...
if grep -q '^  ani:' "$FAKE_HOME/.hermes/config.yaml" 2>/dev/null ||
   grep -q '^  pre_llm_call:' "$FAKE_HOME/.hermes/config.yaml" 2>/dev/null ||
   grep -q '^  ani:' "$GOOSE_CFG" 2>/dev/null ||
   grep -q 'ani' "$FAKE_HOME/.grok/config.toml" 2>/dev/null ||
   grep -q '^name = "ani"' "$FAKE_HOME/.vibe/config.toml" 2>/dev/null; then
...
for CONTEXT_FILE in \
  "$ZED_INSTR" \
  "$FAKE_HOME/.kiro/steering/ani.md" \
  "$FAKE_HOME/.factory/AGENTS.md" \
  "$AUGMENT_RULE" \
  "$FAKE_HOME/.config/crush/ani.md" \
  "$FAKE_HOME/.config/goose/.goosehints" \
  "$KILO_RULE" \
  "$CLINE_RULE" \
  "$FAKE_HOME/.vibe/AGENTS.md" \
  "$FAKE_HOME/.grok/rules/ani.md" \
  ...
  "$PI_INSTRUCTIONS"; do
  if grep -q 'ani:start' "$CONTEXT_FILE" 2>/dev/null; then
...
if [ -d "$FAKE_HOME/.claude/skills/ani" ] ||
   [ -d "$FAKE_HOME/.hermes/skills/ani" ] ||
   [ -d "$FAKE_HOME/.agents/skills/ani" ] ||
   [ -d "$FAKE_HOME/.qoder/skills/ani" ] ||
   [ -d "$CUSTOM_KIMI_HOME/skills/ani" ] ||
   [ -d "$FAKE_HOME/.pi/agent/skills/ani" ] ||
   [ -d "$FAKE_HOME/.junie/skills/ani" ] ||
   [ -d "$FAKE_HOME/.rovodev/skills/ani" ] ||
   [ -d "$FAKE_HOME/.copilot/skills/ani" ] ||
   [ -d "$FAKE_HOME/.vibe/skills/ani" ] ||
   [ -d "$FAKE_HOME/.grok/skills/ani" ] ||
   [ -e "$DEVIN_SKILL" ] ||
   [ -e "$CODEBUDDY_SKILL" ] ||
   [ -e "$BOB_SKILL" ] ||
   [ -e "$POCHI_SKILL" ] ||
...
# Note: ani_find_cli searches hardcoded paths (/usr/local/bin, /opt/homebrew/bin)
...
import json, sys
d = json.load(sys.stdin)
print(list(d.get('mcpServers',{}).keys()).count('ani'))
" 2>/dev/null || echo "0")
if [ "$COUNT" != "1" ]; then
...
copy_smoke_binary "$CORRUPT_HOME/.local/bin/ani"
...
copy_smoke_binary "$DBL_HOME/.local/bin/ani"
run_no_crash 9b-8-install env HOME="$DBL_HOME" "$BINARY" install -y
DBL_UNINSTALLER="$BINARY"
if [[ "$BINARY" == *.exe ]]; then
  DBL_UNINSTALLER="$DBL_HOME/.local/bin/ani.exe"
fi
...
if [ -n "${SMOKE_DOWNLOAD_URL:-}" ]; then
  ...
  # ANI_DOWNLOAD_URL test override, so point it directly at the same fixture.
  ...
  if [[ "$BINARY" == *.exe ]]; then
    cp "$BINARY" "$UPDATE_HOME/.local/bin/ani.exe"
    mkdir -p "$UPDATE_HOME/retired-install"
    cp "$BINARY" "$UPDATE_HOME/retired-install/ani.exe"
  else
    cp "$BINARY" "$UPDATE_HOME/.local/bin/ani"
    chmod 755 "$UPDATE_HOME/.local/bin/ani"
    mkdir -p "$UPDATE_HOME/retired-install"
    cp "$BINARY" "$UPDATE_HOME/retired-install/ani"
    chmod 755 "$UPDATE_HOME/retired-install/ani"
    if [ "$(uname -s)" = "Darwin" ]; then
      codesign --sign - --force "$UPDATE_HOME/.local/bin/ani" 2>/dev/null || true
      codesign --sign - --force "$UPDATE_HOME/retired-install/ani" \
        2>/dev/null || true
    fi
  fi
  ...
  # uninstall phases.
  if [[ "$BINARY" == *.exe ]]; then
    UPDATE_DRIVER="$UPDATE_HOME/.local/bin/ani.exe"
  else
    UPDATE_DRIVER="$UPDATE_HOME/.local/bin/ani"
  fi

  # Pre-install agent config with positive prior-install identity. POSIX runs
  # update from that exact retired ANI image, so refresh requires only string
  ...
  STALE_CMD="$STALE_CMD" python3 -c \
    'import json, os; print(json.dumps({"mcpServers":{"ani":{"command":os.environ["STALE_CMD"]}}}))' \
  ...
  HOME="$UPDATE_HOME" ANI_DOWNLOAD_URL="$UPDATE_DOWNLOAD_URL" \
  ...
  if [[ "$BINARY" == *.exe ]]; then
    UPD_BIN="$UPDATE_HOME/.local/bin/ani.exe"
  else
    UPD_BIN="$UPDATE_HOME/.local/bin/ani"
  fi
  if [ ! -f "$UPD_BIN" ]; then
  ...
  if [ -f "$UPDATE_HOME/.local/bin/ani" ] || [ -f "$UPDATE_HOME/.local/bin/ani.exe" ]; then
...
import json, sys
d = json.load(sys.stdin)
if 'ani' in d.get('mcpServers', {}): sys.exit(1)
sys.exit(0)
" 2>/dev/null; then
  ...
  copy_smoke_binary "$UPDATE_DIR/install/ani"
  chmod 755 "$UPDATE_DIR/install/ani"
  copy_smoke_binary "$UPDATE_DIR/smoke-downloaded"
  rm -f "$UPDATE_DIR/install/ani"
  cp "$UPDATE_DIR/smoke-downloaded" "$UPDATE_DIR/install/ani"
  chmod 755 "$UPDATE_DIR/install/ani"
  if [ "$(uname -s)" = "Darwin" ]; then
    codesign --sign - --force "$UPDATE_DIR/install/ani" 2>/dev/null || true
  fi
  if ! "$UPDATE_DIR/install/ani" --version > /dev/null 2>&1; then
...
DL_ARCHIVE="ani-${DL_OS}-${DL_ARCH}.${DL_EXT}"
...
if [ "$DL_OS" != "windows" ] && [ -f "$REPO_ROOT/install.sh" ]; then
  ...
  HOME="$INSTALL_TEST_HOME" ANI_DOWNLOAD_URL="$SMOKE_DOWNLOAD_URL" \
    "$REPO_ROOT/install.sh" --dir="$INSTALL_TEST_DIR" 2>&1 || true

  # 13b: binary placed
  if [ ! -f "$INSTALL_TEST_DIR/ani" ]; then
  ...
  if [ "$DL_OS" = "darwin" ]; then
    codesign --sign - --force "$INSTALL_TEST_DIR/ani" 2>/dev/null || true
  fi
  if ! "$INSTALL_TEST_DIR/ani" --version > /dev/null 2>&1; then
  ...
  if [ "$DL_OS" = "darwin" ]; then
    if codesign -v "$INSTALL_TEST_DIR/ani" 2>/dev/null; then
  ...
  if [ -f "$INSTALL_TEST_HOME/.claude.json" ] && grep -q 'ani' "$INSTALL_TEST_HOME/.claude.json" 2>/dev/null; then
  ...
  if ! HOME="$WIN_HOME" TEMP="$WIN_HOME" TMP="$WIN_HOME" \
    ANI_DOWNLOAD_URL="$WIN_URL" ANI_ARCH="$DL_ARCH" MSYS2_ARG_CONV_EXCL='*' \
  ...
  # 13g: binary placed
  PS1_BIN="$PS1_TEST_DIR/ani.exe"
  if [ ! -f "$PS1_BIN" ] && [ -f "$PS1_TEST_DIR/ani" ]; then
    PS1_BIN="$PS1_TEST_DIR/ani"
  fi
  if [ -f "$PS1_BIN" ]; then
