...
Environment:
  ANI_TEST_SEQUENTIAL=1   Single-process runner instead of the parallel harness.
  ANI_RUN_HANG_TEST=1     Opt-in C++ index-hang regression (#410, needs prod).
  ANI_NO_CCACHE=1         Disable the content-verified compiler cache.
  ANI_TEST_SHARD/_LEG     Set by CI's sharded legs; leave unset locally.
...
if [ "$TSAN" -eq 1 ]; then
    ...
    make -j"$NPROC" -f Makefile.ani "$BUILD_DIR/test-runner-tsan" ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
    make -f Makefile.ani test-tsan ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
    exit "$?"
fi
...
if [ -n "$SUITES" ]; then
    ...
    make -j"$NPROC" -f Makefile.ani "$BUILD_DIR/test-runner" ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
...
# ANI_TEST_SEQUENTIAL=1 restores the single-process runner.
make -j"$NPROC" -f Makefile.ani "$BUILD_DIR/test-runner" ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
if [ "${ANI_TEST_SEQUENTIAL:-0}" = "1" ]; then
    make -f Makefile.ani test ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
else
    make -f Makefile.ani test-par ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
fi
...
# Opt-in via ANI_RUN_HANG_TEST=1 (it needs the prod binary, which the ASan unit
# run above does not build). Skipped by default so the fast unit run stays fast.
if [ "${ANI_RUN_HANG_TEST:-0}" = "1" ]; then
...
make -j"$NPROC" -f Makefile.ani ani TEST_SEAMS=1 ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
WATCHDOG_BINARY="$ROOT/$BUILD_DIR/ani"
ANI_TEST_BINARY="$WATCHDOG_BINARY" bash "$ROOT/tests/test_parent_watchdog.sh"
...
ANI_TEST_BINARY="$WATCHDOG_BINARY" bash "$ROOT/tests/test_worker_watchdog.sh"
...
ANI_TEST_BINARY="$WATCHDOG_BINARY" bash "$ROOT/tests/test_worker_error_response.sh"
...
# ANI_TEST_HOOK_CLIENT_BUILD seam and asserts the stdout systemMessage. It is
...
#   make -f Makefile.ani ani TEST_SEAMS=1 && bash tests/test_hook_conflict_notice.sh
...
ANI_TEST_BINARY="$WATCHDOG_BINARY" bash "$ROOT/tests/test_watcher_disabled.sh"
