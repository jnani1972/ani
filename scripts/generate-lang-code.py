...
GRAMMAR_DIR = os.path.join(PROJECT_DIR, "internal", "ani")
...
def generate_enum(langs):
    """Print enum entries for ani.h."""
    print("\n=== Enum Entries (paste into ani.h before ANI_LANG_KUSTOMIZE) ===")
    for lang in langs:
        print(f"    ANI_LANG_{lang['enum']},")
...
def generate_specs(langs):
    ...
    for lang in langs:
        ...
        print(f"    // ANI_LANG_{lang['enum']}")
        print(
            f"    [ANI_LANG_{lang['enum']}] = {{ANI_LANG_{lang['enum']}, "
...
def generate_language_c(langs):
    ...
    for ext, enum, display in sorted(ext_entries, key=lambda x: x[0].lower()):
        print(f'    /* {display} */')
        print(f'    {{"{ext}", ANI_LANG_{enum}}},')
        print()

    print("\n=== FILENAME_TABLE Entries ===")
    for lang in langs:
        for fn in lang["filenames"]:
            print(f'    {{"{fn}", ANI_LANG_{lang["enum"]}}},')

    print("\n=== LANG_NAMES Entries ===")
    for lang in langs:
        print(
            f'    [ANI_LANG_{lang["enum"]}] = "{lang["display"]}",'
        )
