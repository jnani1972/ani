...
$Repo = "DeusData/ani"
$BinaryName = "ani"
$InstallDir = Join-Path $env:LOCALAPPDATA "ani"
...
function Configure-ClaudeCode($McpConfig) {
    Write-Host ""
    $answer = Read-Host "Configure Claude Code to use ani? [y/N]"
    ...
    if ($answer -match '^[Yy]$') {
        ...
        $settings["mcpServers"]["ani"] = $McpConfig
        ...
        Write-Host ""
        $snippet = @{ mcpServers = @{ "ani" = $McpConfig } }
...
Write-Host ""
Write-Host "ani installer (Windows)" -ForegroundColor White
Write-Host ""
...
if ($FromSource) {
    ...
    # Clone or update
    Write-Host ""
    $sourceDir = "/home/$wslUser/.local/share/ani"
    try {
    ...
    Invoke-WSL "mkdir -p /home/$wslUser/.local/bin && cd $sourceDir && scripts/build.sh && cp build/c/ani $wslBinaryPath"
    ...
    Write-Host "    wsl.exe -- rm -rf ~/.cache/ani/"
    ...
    $release = Invoke-RestMethod -Uri $releaseUrl -Headers @{ "User-Agent" = "ani-setup" }
    ...
    Write-Host "    Remove-Item -Recurse -Force `"$env:LOCALAPPDATA\ani`"  # graph database"
}
