...
from ani import _cli  # noqa: E402
...
class BinarySelectionTests(unittest.TestCase):
    ...
    def test_release_archives_carry_no_sidecars(self):
        ...
        for names in (_cli._WINDOWS_ARCHIVE_NAMES, _cli._UNIX_ARCHIVE_NAMES):
            self.assertNotIn("ani-integrations.json", names)
            self.assertFalse([n for n in names if n.startswith("ani-ui-")])
            self.assertEqual(len(names), 4)

    def test_update_guidance_uses_pip_and_names_managed_install(self):
        with mock.patch.object(_cli.sys, "argv", ["ani", "update"]), \
        ...
        self.assertIn(
            "python -m pip install --upgrade ani", output
        )
        self.assertIn("ani install --yes", output)
...
"""
            ...
            with self.subTest(crash_name=crash_name), tempfile.TemporaryDirectory() as root:
                ...
                try:
                    ...
                    backups = [
                        ...
                        if path.name.startswith(".ani-runtime-backup-")
                    ]
                    ...
                    self.assertFalse(
                        any(
                            path.name.startswith(".ani-runtime-backup-")
                            for path in directory.iterdir()
                        )
                    )
                    self.assertFalse(
    ...
    def test_publication_failure_restores_prior_complete_runtime_set(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            def fail_binary_publish(source, destination):
                ...
                if destination.parent.name.startswith(".ani-runtime-backup-"):
    ...
    def test_orphan_reconciliation_rejects_multiply_linked_backup_members(self):
        with tempfile.TemporaryDirectory() as root:
            directory = Path(root)
            binary = directory / "ani"
            backup = directory / f".ani-runtime-backup-{'a' * 32}"
            backup.mkdir()
