"""Downloads ani on first run, then runs its native entry point."""
...
_RUNTIME_LOCK_NAME = ".ani-runtime.lock"
...
_RUNTIME_BACKUP_PREFIX = ".ani-runtime-backup-"
_RUNTIME_BACKUP_RE = re.compile(r"\.ani-runtime-backup-[0-9a-f]{32}\Z")
...
def _validate_url_scheme(url: str) -> None:
    ...
    if (
        ...
        sys.exit(
            f"ani: refusing to fetch invalid, credentialed, or "
            f"non-https URL: {url}"
        )
...
def _download_https(url: str, dest: str, max_bytes: int = 0) -> None:
    ...
    for redirect_count in range(_MAX_REDIRECTS + 1):
        ...
        request = urllib.request.Request(
            current_url, headers={"User-Agent": "ani-installer"}
        )
        try:
...
def _safe_extract_tar(
    ...
    for member in members:
        if not member.isfile():
            sys.exit(
                f"ani: refusing unsafe tar entry "
    ...
    for name in runtime_names:
        ...
        if source is None:
            sys.exit(f"ani: could not read archive member: {name}")
...
def _safe_extract_zip(
    ...
    for info in zf.infolist():
        ...
        if (
            ...
            sys.exit(
                f"ani: refusing unsafe zip entry "
...
def _verify_checksum(archive_path: str, archive_name: str, version: str) -> None:
    ...
    try:
        ...
        with open(tmp_path, encoding="utf-8") as f:
            for line in f:
                ...
                if len(digest) != 64 or any(
                    ...
                    sys.exit(
                        f"ani: invalid SHA256 checksum for "
                        f"{archive_name}"
                    )
                if expected is not None and expected != digest:
                    sys.exit(
                        f"ani: conflicting SHA256 checksums for "
                        f"{archive_name}"
                    )
                expected = digest
        if expected is None:
            sys.exit(
                f"ani: no checksum for {archive_name} in checksums.txt"
            )
        h = hashlib.sha256()
        ...
        if expected != actual:
            sys.exit(
                f"ani: CHECKSUM MISMATCH for {archive_name}\n"
                f"  expected: {expected}\n"
                f"  actual:   {actual}"
            )
        print("ani: checksum verified.", file=sys.stderr)
    except SystemExit:
        raise
    except Exception as exc:
        sys.exit(f"ani: checksum verification failed: {exc}")
    finally:
...
def _os_name() -> str:
    ...
    if p == "win32":
        return "windows"
    sys.exit(f"ani: unsupported platform: {p}")
...
def _arch() -> str:
    ...
    if m in ("x86_64", "amd64"):
        return "amd64"
    sys.exit(f"ani: unsupported architecture: {m}")
...
def _copy_runtime_backup_file(
    ...
    descriptor, staged_name = tempfile.mkstemp(
        dir=str(target.parent), prefix=".ani-runtime-recovery-"
    )
...
def _download(version: str) -> Path:
    ...
    archive = f"ani-{os_name}-{arch}{portable}.{ext}"
    ...
    print(
        f"ani: downloading v{version} for {os_name}/{arch}...",
        file=sys.stderr,
    )

    with tempfile.TemporaryDirectory() as tmp:
        tmp_archive = os.path.join(tmp, f"ani.{ext}")
        try:
            _download_https(url, tmp_archive)
        except (OSError, RuntimeError, urllib.error.URLError) as e:
            sys.exit(
                f"ani: download failed ({e})\n"
                f"URL: {url}\n"
        ...
        bin_name = (
            _WINDOWS_BINARY_NAME if os_name == "windows" else "ani"
        )
        ...
        for name in runtime_names:
            ...
            if not _regular_file(extracted_path):
                sys.exit(
                    f"ani: required runtime member not found after "
                    f"extraction: {name}"
                )
            if name == bin_name:
        ...
        if _runtime_set_names(Path(tmp), bin_name) is None:
            sys.exit(
                "ani: extracted runtime set failed content "
                "verification"
            )
        try:
    ...
    if mutation in ("install", "uninstall"):
        ...
        if sys.platform == "win32" and result != 0 and mutation == "uninstall":
            print(
                ...
                '"python -m pip uninstall ani" for package maintenance. '
...
def _default_managed_install_dir() -> Path:
    ...
    return local_app_data / "Programs" / "ani"
...
def _run_cache_sensitive_mutation(version: str, executable: Path, args) -> int:
    ...
    try:
        ...
        thread = threading.Thread(
            name="ani-cache-lock-heartbeat", target=heartbeat, daemon=True
        )
        thread.start()
