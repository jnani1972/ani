...
// Install:
//
//	go install github.com/jnani1972/ani/pkg/go/cmd/ani@latest
package main
...
const (
	...
	runtimeSetLockName               = ".ani-runtime.lock"
	...
	runtimeBackupPrefix              = ".ani-runtime-backup-"
...
func main() {
	...
	if mutation == "update" {
		fmt.Fprintln(
			os.Stderr,
			"This Go wrapper is maintained by Go. Update it with \"go install github.com/jnani1972/ani/pkg/go/cmd/ani@latest\".",
		)
		os.Exit(2)
	}
	executable, err := ensureBinary()
	if err != nil {
		fmt.Fprintf(os.Stderr, "ani: %v\n", err)
		os.Exit(1)
	}
...
func printPortableMutationGuidance(args []string) {
	...
	packageCommand := "Remove-Item (Get-Command ani).Source"
	fmt.Fprintf(
		os.Stderr,
...
func defaultManagedInstallDir() string {
	if runtime.GOOS == "windows" {
		...
		return filepath.Join(base, "Programs", "ani")
	}
	if home, err := os.UserHomeDir(); err == nil {
		return filepath.Join(home, ".local", "bin")
	}
	return filepath.Join(os.TempDir(), "ani-managed")
}
...
func mutationSnapshotEnvironment() []string {
	blocked := []string{"ANI_ASSETS_DIR", "ANI_UI_ASSETS_DIR"}
...
func cacheDir() string {
	if d := os.Getenv("ANI_CACHE_DIR"); d != "" {
		return d
	}
	switch runtime.GOOS {
	case "windows":
		if d := os.Getenv("LOCALAPPDATA"); d != "" {
			return filepath.Join(d, "ani")
		}
	case "darwin":
		if home, err := os.UserHomeDir(); err == nil {
			return filepath.Join(home, "Library", "Caches", "ani")
		}
	}
	...
	if home, err := os.UserHomeDir(); err == nil {
		return filepath.Join(home, ".cache", "ani")
	}
	return filepath.Join(os.TempDir(), "ani")
}
...
func download(dest string) error {
	...
	fmt.Fprintf(os.Stderr, "ani: downloading v%s for %s/%s...\n", version, platform, arch)

	tmp, err := os.MkdirTemp("", "ani-install-*")
	if err != nil {
		return err
	}
	defer os.RemoveAll(tmp)

	archivePath := filepath.Join(tmp, "ani."+ext)
...
func execBinaryWithRuntimeLockAndRunner(
	...
	defer func() {
		...
		if cleanupErr := runtimeMutationSnapshotCleanup(snapshotDirectory); cleanupErr != nil {
			fmt.Fprintf(
				os.Stderr,
				"ani: warning: private mutation snapshot cleanup failed: %v\n",
				cleanupErr,
			)
		}
	}()
