...
func TestWindowsUsesOneDirectBinary(t *testing.T) {
	...
	if got := binaryNameForOS("windows"); got != "ani.exe" {
...
func TestMutationSnapshotEnvironmentIgnoresExternalAssetOverrides(t *testing.T) {
	t.Setenv("ANI_ASSETS_DIR", filepath.Join(t.TempDir(), "integrations"))
	t.Setenv("ANI_UI_ASSETS_DIR", filepath.Join(t.TempDir(), "ui"))
	for _, entry := range mutationSnapshotEnvironment() {
		name := entry
		if separator := strings.IndexByte(entry, '='); separator >= 0 {
			name = entry[:separator]
		}
		if strings.EqualFold(name, "ANI_ASSETS_DIR") ||
			strings.EqualFold(name, "ANI_UI_ASSETS_DIR") {
...
func TestMutationSnapshotRejectsExplicitTargetOverlappingCache(t *testing.T) {
	directory := t.TempDir()
	executable := filepath.Join(directory, "ani")
...
func TestMutationSnapshotCleanupFailurePreservesNativeResult(t *testing.T) {
	const exitHelper = "ANI_GO_MUTATION_EXIT_HELPER"
...
func TestKilledRuntimePublisherIsReconciledByLockedReadiness(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			command.Env = append(
				os.Environ(),
				"ANI_TEST_RUNTIME_CRASH_HELPER=1",
				"ANI_TEST_RUNTIME_CRASH_SOURCE="+source,
				"ANI_TEST_RUNTIME_CRASH_DESTINATION="+destination,
				"ANI_TEST_RUNTIME_CRASH_MARKER="+marker,
				"ANI_TEST_RUNTIME_CRASH_PHASE="+testCase.crashPhase,
			)
...
func TestRuntimePublicationCrashHelper(t *testing.T) {
	if os.Getenv("ANI_TEST_RUNTIME_CRASH_HELPER") != "1" {
		return
	}
	source := os.Getenv("ANI_TEST_RUNTIME_CRASH_SOURCE")
	destination := os.Getenv("ANI_TEST_RUNTIME_CRASH_DESTINATION")
	marker := os.Getenv("ANI_TEST_RUNTIME_CRASH_MARKER")
	crashPhase := os.Getenv("ANI_TEST_RUNTIME_CRASH_PHASE")
	binary := "ani"
...
func TestRuntimeSetLockSerializesProcesses(t *testing.T) {
	const helperEnv = "ANI_GO_RUNTIME_LOCK_HELPER"
	if os.Getenv(helperEnv) == "1" {
		destination := os.Getenv("ANI_GO_RUNTIME_LOCK_DIRECTORY")
		waitingPath := os.Getenv("ANI_GO_RUNTIME_LOCK_WAITING")
		acquiredPath := os.Getenv("ANI_GO_RUNTIME_LOCK_ACQUIRED")
		var waitOnce sync.Once
		runtimeSetLockWaitObserver = func() {
			waitOnce.Do(func() {
	...
	cmd.Env = append(
		os.Environ(),
		helperEnv+"=1",
		"ANI_GO_RUNTIME_LOCK_DIRECTORY="+destination,
		"ANI_GO_RUNTIME_LOCK_WAITING="+waitingPath,
		"ANI_GO_RUNTIME_LOCK_ACQUIRED="+acquiredPath,
	)
