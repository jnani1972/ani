...
test('Windows npm shim probes and executes the cached launcher', () => {
  ...
  for (const call of observed.calls) {
    assert.equal(path.basename(call.executable), 'ani.exe');
    assert.notEqual(
...
test('Windows npm shim keeps package-manager guidance after launcher refusal', () => {
  ...
  assert.equal(path.basename(observed.calls[1].executable), 'ani.exe');
  assert.match(observed.stderr, /npm install ani@latest/);
...
test('non-Windows npm shim keeps its native payload execution path', () => {
  ...
  assert.equal(path.basename(observed.calls[0].executable), 'ani');
});
