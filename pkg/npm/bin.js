...
const binName = isWindows ? 'ani.exe' : 'ani';
...
if (!cacheReady()) {
  ...
  process.stderr.write('ani: binary not found, downloading...\n');
  ...
  if (installResult.status !== 0 || !cacheReady()) {
    process.stderr.write(
      'ani: download failed.\n' +
      'Try reinstalling: npm install -g ani\n'
    );
    process.exit(1);
  }
}
...
if (result.error) {
  process.stderr.write(`ani: ${result.error.message}\n`);
  process.exit(1);
}
...
if (mutation) {
  ...
  process.stderr.write(
    ...
    '"ani install --yes" once for a managed install with its ' +
