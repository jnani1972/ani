...
const WINDOWS_BINARY_NAME = 'ani.exe';
...
const WINDOWS_PAIR_LOCK_NAME = '.ani-pair.lock';
...
function installWindowsBinaryAtomically(sourceDir, destDir, verifier = verifyCandidate) {
  ...
  try {
    ...
    try {
      for (const name of [WINDOWS_BINARY_NAME]) {
        const staged = path.join(destDir, `.ani-pair-stage-${transaction}-${name}`);
      ...
      for (const name of [WINDOWS_BINARY_NAME]) {
        ...
        const backup = path.join(destDir, `.ani-pair-backup-${transaction}-${name}`);
...
async function verifyChecksum(archivePath, archiveName) {
  ...
  try {
    ...
    process.stdout.write('ani: checksum verified.\n');
  } finally {
...
async function main() {
  ...
  const archive = `ani-${platform}-${arch}${variant}.${ext}`;
  const url = `https://github.com/${REPO}/releases/download/v${VERSION}/${archive}`;

  process.stdout.write(`ani: downloading v${VERSION} for ${platform}/${arch}...\n`);

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ani-install-'));
  const tmpArchive = path.join(tmpDir, `ani.${ext}`);
  ...
  try {
    ...
    process.stdout.write('ani: ready.\n');
    if (platform === 'windows') {
      process.stdout.write(
        'Windows package cache is portable. Run "ani install --yes" ' +
        'to create the managed launcher (use "npx ani install --yes" ' +
        'for a local npm install). Package updates/removal remain ' +
        '"npm install ani@latest" and ' +
        '"npm uninstall ani" (add -g for a global install).\n',
      );
    }
  } finally {
...
if (require.main === module || module.parent == null) {
  main().catch((err) => {
    process.stderr.write(`\nani: install failed — ${err.message}\n`);
